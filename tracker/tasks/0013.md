---
id: 0013
title: Repo-designated inbox and deposit defaults in .show-repo.json
status: backlog
project: repo
track: independent
opened: 2026-07-15
...
---
# Repo-designated inbox and deposit defaults in .show-repo.json
...
introducing a new mechanism: an `inbox` is one more field the `.show-repo.json`
...
- **Designate, with a soft convention fallback.** A repo declares its inbox in
  `.show-repo.json` (e.g. `inbox: "inbox/"`). A deposit that names no dir uses
...
- **The inbox belongs to the destination repo's manifest.** For a cross-repo
  ...
  fetch the destination repo's `.show-repo.json`; the stager probes only the
