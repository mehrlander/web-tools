...
- **Recurrence mechanism.** How a repeat is recorded (bump a count vs. append a
  ...
  tracker.
- **PR-body section + projector.** Name the guide-region section (`Snags:`),
  fix the per-entry line format (slug + one-liner + `→`), and build the
  non-destructive merge-and-count projector alongside `build-merge-guide.py`.
...
formalized `SNAGS.md` + PR-body `Snags:` section + projector, or a decision to
drop it.
