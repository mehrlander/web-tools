...
## In progress
- 🎫 App views - designate a page as an estate-level view (`claude/clone-home-repo-7486wk`) next: add appView/viewLabel to a pages entry; collect app views across configs via the config cache, the quickLink sibling; render live via toss-render #gh=
- 🎫 Generalize the gallery to a per-repo pages catalog (`claude/clone-home-repo-7486wk`) next: add a pages field to .web-tools.json and branch landingKind() on it; render tiles live via toss-render #gh= (no committed thumbs for private repos)
