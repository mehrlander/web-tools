...
* **Sync the body on every push (unconditional):** the pushed state must not lie, and a stale guide is worse than none, so the body tracks HEAD automatically. This is a default, never a question: don't offer to sync it, defer it, or fold it into the wrap-up. Do it on the same turn as the push, silently. Keep the machine-refreshed part inside the fenced managed region below, so a sync never clobbers hand-written text outside it. The cumulative diff lives in the PR's Files tab; the body's Changed list carries only the curated layer a diff can't show (⭐/🥏 preview links, `renders on:` lines, one-line whys).
