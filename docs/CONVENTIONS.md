...
* **Stage a fileset (transport) 🗂️:** a live view moves a *page*; a **stage link** moves a *fileset*. When the handoff is "here are these files, across repos, to look at or move," mint a `#stage=` link into the shared show-repo shell and **mark it 🗂️** (the way ⭐ marks a hosted view, 🥏 a toss, 📦 an artifact). It is the transfer-side sibling of the toss forms: the link carries the file **refs** (their content stays behind the viewer's token) plus, optionally, **authored commentary and a mode**, which ride the link because they are authored, not repo content. Grammar: `…/show-repo/show-repo.html#stage=owner/repo[@ref]:path1,path2;owner2/repo2:path3` (groups `;`-separated, paths `,`-separated, `@ref` optional), with an optional `&prompts=<base64url>` carrying a `{label, ask}` review-prompt list and an optional `&mode=diff` that opens the stage on its Diff tab and runs the compare on open. The same keys also ride the `?query` (via `StageLink.read`), so a stage survives a context that strips the fragment (a toss render, an email), with the fragment the default and private form. Opening it stages those refs for view, a concatenated-bundle copy/download, a two-tap copy into another repo, or a review diff. Honesty gate, same as `#gh=`: a stage link is **token-gated**, so it renders only for the token owner in a token-bearing browser, which the Claude app's in-app browser is only if a token was entered there; the token-less `#gz=`-style bundle form is contemplated, not built, so for a token-less reader download the bundle and hand it over (**Hand over the artifact**) instead. This is an inline handoff you offer when moving files, not a **surfacing caption** row. Full mechanics, plus the `.web-tools.json` manifest a repo uses to configure the shell: `docs/show-repo.md` in the canonical repo.
