...
- Make a task for work that must survive across sessions, not for every edit, and never for work the current session could simply do. Filing what you could finish now converts an hour of work into a standing item a later session must re-read, re-prioritize, and rebuild context for. Do the work, let the diff and the reply be the record, and file only what genuinely remains.
