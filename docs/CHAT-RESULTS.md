...
Fenced blocks inside any markdown here (narrative, excerpts, transcripts) become block artifacts via [`lib/chat-render.js`](../lib/chat-render.js): static code instantly, then on demand Render (html/svg, sandboxed iframe), Run (js, sandboxed console), Table (json arrays, Tabulator), Preview (md), and a CM6 editor behind the Edit pencil. Nothing executes without a click. The sandbox documents come from [`lib/kits/proof.js`](../lib/kits/proof.js), the same machinery behind the vanilla-demo pages.
