...
GitHub provides several native ways to expose repository state. This page is the mechanical reference for those surfaces: branches, commits, compare views, pull requests, permalinks, and serialized diffs. For how this repository hands one of them to a reader in chat, including the branch-page caption and its per-file fallback, see the primitives in [SURFACING.md](../SURFACING.md) and the fallback in [surfacing-extended.md](../surfacing-extended.md#the-caption-when-the-branch-page-cannot-be-used).
