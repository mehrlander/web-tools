...
| the governing-docs warner | a `PostToolUse` hook on `Edit`, `Write` and `MultiEdit` that puts a warning in front of the model when the edited file is documentation: the markdown globs of the repo's `sources.csv` retrieval registry where one exists, else the same shapes, never chron, tracker tasks, blog, or a dump tray. It fires after the edit lands, since the platform gives `PreToolUse` no non-blocking channel to the model, and the edit is still uncommitted |

That is the whole day-to-day set. Everything above the last six rows is invoked;
those six are not. **They are the pieces that run on their own**, which is why
