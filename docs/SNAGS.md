...
| `shell-pin-plus-frame-kills-the-tab` | undated | the render link crashed every phone it was handed to |
...
### shell-pin-plus-frame-kills-the-tab: the render link crashed every phone it was handed to

`scripts/showing.py` emitted `toss-render.html?use=<sha>#gh=owner/repo@<sha>:<path>`
for a page change. On an iPhone, tapping the fab on that link freezes the view
for a second or two, flashes twice, and lands on Safari's "A problem repeatedly
...
under the drawer, a long-press threshold): a pinned shell **with** a frame dies
whatever the frame holds, including a twenty-line page that loads nothing; the
same pin with **no** frame survives; a frame under an **unpinned** shell survives
even with the subject pinned to the branch. Only the intersection fails.

Two instrument defects cost more than the bug did, and both had the same shape,
a harness trusted without a control that could catch it lying. `$nextTick` and
`requestAnimationFrame` never fired on the device, so their absence read as "the
browser died drawing" on taps that had worked. And the report read its case tag
off the page that FILED it rather than the page that crashed, which in a matrix
run link by link shifted every result by one cell. → `scripts/showing.py`, gated
by `tools/test/showing-pick.test.mjs`.
