...
//
// WIDENED 2026-09-14 to a project's GitHub icon, which opens the workspace
// FOLDER and peeks that folder's README. GitHub renders a directory as its
// listing with the README beneath, so the card previews the destination the
// way a file peek does. A repo, a branch, or a menu still carries none.
//
...
(() => {
  ...
  // Fencing is the FALLBACK treatment, and where a reader can do better it
  // should: a fenced block is twenty lines of grey monospace above the document,
  // which buries the one line of it anyone wanted. splitFrontmatter below hands
  // back the two halves so a reader can render the metadata AS metadata.
  function fenceFrontmatter(text) {
    return String(text == null ? '' : text)
      .replace(/^---\r?\n([\s\S]*?)\r?\n---[ \t]*\r?\n?/, (m, y) => '```\n' + y + '\n```\n');
  }

  // The two halves of a document that opens with frontmatter, so a reader can
  // show the metadata as metadata instead of as a wall of fenced YAML.
  //
  // `fields` is deliberately a SHALLOW read and says so: top-level `key: value`
  // only, with a block scalar (`>-`, `|`, `>`) folded into one string and a
  // nested mapping reported by its keys rather than parsed. That covers what
  // this estate actually writes in frontmatter (a skill's name and description,
  // a task's status, date and tags) and nothing more. A document whose
  // frontmatter needs a real YAML parser is a document this should hand back
  // whole, which is what `raw` is for.
  //
  // Returns { raw, fields, body }. No frontmatter gives raw '' and fields [],
  // so a caller can branch on one thing.
  function splitFrontmatter(text) {
    const s = String(text == null ? '' : text);
    const m = /^---\r?\n([\s\S]*?)\r?\n---[ \t]*\r?\n?/.exec(s);
    if (!m) return { raw: '', fields: [], body: s };
    const raw = m[1];
    const body = s.slice(m[0].length);
    const fields = [];
    const lines = raw.split(/\r?\n/);
    for (let i = 0; i < lines.length; i++) {
      const top = /^([A-Za-z0-9_.-]+):[ \t]*(.*)$/.exec(lines[i]);
      if (!top) continue;                     // a continuation, taken below
      const key = top[1];
      let val = top[2].trim();
      // A block scalar: the value is the indented run that follows, joined.
      if (/^[>|][-+]?$/.test(val)) {
        const parts = [];
        ...
        // A nested mapping or list: name its keys rather than pretend to parse.
        const keys = [];
        while (i + 1 < lines.length && /^\s+\S/.test(lines[i + 1])) {
          const sub = /^\s+-?\s*([A-Za-z0-9_.-]+):/.exec(lines[++i]);
          if (sub) keys.push(sub[1]);
        }
        val = keys.length ? keys.join(', ') : '';
      }
      if (val) fields.push({ key, value: val.replace(/^["']|["']$/g, '') });
    }
  ...
  window.SourcePeek = {
    ...
    kindOf, excerpt, shape, jsonText, fenceFrontmatter, splitFrontmatter, body, blobUrl, frame, LINES,
    _cache: cache,
  };
