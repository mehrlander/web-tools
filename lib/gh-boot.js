...
return (async () => {
  ...
  // so surface what's running: the FAB drawer (version chip, Inspect tab,
  // Render/Toss-at-ref) is auto-mounted unless the page already mounts one,
  // and the fab's own launcher turns warning-tinted to flag the off-canonical
  // view (retiring the old dismissable corner pill). The load is deduped by
  // the registry above, so a page that pulls fab.js itself is unaffected.
