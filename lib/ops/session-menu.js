...
(function sessionMenu(input) {
  ...
  //
  // EVERY ROW HERE OPENS A URL except the verbs, deliberately. Choose-Claude
  // runs a row it cannot find in `urls` as a shortcut name, which is a measured
  // path; opening a `shortcuts://` link from inside a running shortcut is not,
  // and appears nowhere in fifteen library dumps. So a row that needs another
  // shortcut is spelled as that shortcut's name, and everything else is a page.
