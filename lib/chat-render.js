...
//   js          Code | Run      (hidden console frame, output streamed)
//   json array  Code | Table    (Tabulator, lazy)
//   md          Code | Preview  (marked)
//   other       Code
//
// Nothing executes on load: Render, Run, and Table build on first click.
// Chat code is arbitrary, so there is no unsandboxed 'parent' kind here.
// The Edit pencil swaps the static <pre> for a CM6 editor (kits/cm6.js,
// lazy-loaded); Render/Run then use the edited text.
//
...
(() => {
  ...
  const consoleOut = () => {
    const out = h('div', { class: 'rounded-box bg-base-100 border border-base-300 p-2 font-mono text-[11px] leading-relaxed min-h-[2.4rem]' });
  ...
  const blockViews = [
    ...
    {
      id: 'run', label: 'Run', icon: 'ph-play',
      test: b => ['js', 'javascript'].includes(b.lang),
      build(host, getCode, o) {
        ...
        const run = () => {
          ...
          runFrame.srcdoc = proof.doc('console', getCode(), {});
        };
        const bar = h('div', { class: 'flex justify-end' }, iconBtn('ph-play-fill', 'Run', run));
        host.append(out, bar);
        run();   // first build happens on tab click, an explicit gesture
      },
    },
    {
