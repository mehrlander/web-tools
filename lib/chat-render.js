...
(() => {
  ...
  function turnCard(m) {
    ...
    const body = h('div', { class: 'px-4 py-4 text-[15px] leading-6' }, markdown(m.md, { ...m._o }));
    return h('article', {
  ...
  // The swipe core: a snap-scrolling track, one slide per exchange, each slide
  // a vertical stack of turn cards that scrolls inside itself when long. Slides
  // build lazily (active plus neighbours), so a long chat pages cheaply: only
  // the visible exchange renders its markdown and code. Returns the track plus
  // an imperative handle (go, active, onSlide, count) so different chrome (an
  // inline nav, a fullscreen pager) can drive the same track.
  function deckCore(msgs, o = {}) {
    ensureDeckStyle();
    const groups = exchanges(msgs);
    const track = h('div', { class: 'cr-track flex h-full w-full min-h-0 snap-x snap-mandatory overflow-x-auto scroll-smooth overscroll-x-contain', style: 'scrollbar-width:none' });
    const slides = [];
    const built = new Array(groups.length).fill(false);
    groups.forEach(() => {
      const inner = h('div', { class: 'mx-auto max-w-2xl space-y-5' });
      slides.push(inner);
      track.append(h('section', { class: 'h-full min-w-full snap-center overflow-y-auto px-4 py-5 sm:px-8 sm:py-8' }, inner));
    });
    const buildSlide = i => {
      if (i < 0 || i >= groups.length || built[i]) return;
      built[i] = true;
      groups[i].forEach(m => slides[i].append(turnCard({ ...m, _o: o })));
    };
    ...
    track.addEventListener('scroll', () => {
      ...
      raf = requestAnimationFrame(() => {
        raf = 0;
        const a = active();
        buildSlide(a - 1); buildSlide(a); buildSlide(a + 1);
        listeners.forEach(cb => cb(a));
      });
    });
    requestAnimationFrame(() => { buildSlide(0); buildSlide(1); });
    return { track, go, active, count: groups.length, onSlide: cb => listeners.push(cb) };
  }
  ...
  // exchange per slide. A header (icon, title, provider/date, a n/total pill),
  // the swipe area, and a footer pager (prev, dot indicators or a progress bar
  // for long chats, next). Opening locks the background scroll and pushes a
  // history entry, so the phone back button, Escape, ←/→, and the ✕ all
  // dismiss it. Returns { el, close }. opts: { title, provider, date }.
  function openTranscript(messages, opts = {}) {
    const msgs = typeof messages === 'string' ? parse(messages) : (messages || []);
    const core = deckCore(msgs, opts);
    const total = core.count;
    ...
    const panel = h('div', { class: 'mx-auto grid h-full max-w-4xl grid-rows-[auto_1fr_auto] bg-base-100 shadow-xl sm:h-[calc(100dvh-2rem)] sm:my-4 sm:rounded-3xl sm:border sm:border-base-300 sm:overflow-hidden' });

    // header
    const closeBtn = h('button', { class: 'btn btn-ghost btn-sm btn-circle shrink-0', 'aria-label': 'Close' }, h('i', { class: 'ph ph-x text-lg' }));
    ...
    const header = h('div', { class: 'flex items-center gap-3 border-b border-base-300 bg-base-100/90 px-4 py-3 backdrop-blur sm:px-6' },
      closeBtn,
      h('div', { class: 'grid size-9 shrink-0 place-items-center rounded-xl bg-primary/10 text-primary' }, h('i', { class: 'ph ph-chats-circle text-xl' })),
      h('div', { class: 'min-w-0 flex-1' },
        h('h1', { class: 'truncate text-sm font-semibold sm:text-base' }, opts.title || 'Transcript'),
        h('p', { class: 'truncate text-xs text-base-content/50' }, [opts.provider, opts.date].filter(Boolean).join('  ·  ') || 'Swipe through the exchanges')),
      pill);

    // footer pager: dots for short chats, a progress bar for long ones
    const circle = (icon, onClick) => {
      const b = h('button', { class: 'btn btn-circle btn-sm border-base-300 bg-base-100 shadow-none', 'aria-label': icon }, h('i', { class: `ph ${icon} text-base` }));
      b.addEventListener('click', onClick);
      return b;
    };
    const prevB = circle('ph-arrow-left', () => core.go(core.active() - 1));
    const nextB = circle('ph-arrow-right', () => core.go(core.active() + 1));
    ...
    if (useDots) {
      for (let i = 0; i < total; i++) {
        const d = h('button', { class: 'h-1.5 rounded-full transition-all duration-200', 'aria-label': `Go to exchange ${i + 1}` });
        d.addEventListener('click', () => core.go(i));
    ...
    const footer = h('div', { class: 'grid grid-cols-[2.75rem_1fr_2.75rem] items-center gap-4 border-t border-base-300 bg-base-100 px-4 py-3 sm:px-6' }, prevB, center, nextB);

    panel.append(header, core.track, footer);
    ...
    const onKey = e => {
      ...
      else if (e.key === 'ArrowRight') core.go(core.active() + 1);
      else if (e.key === 'ArrowLeft') core.go(core.active() - 1);
    };
