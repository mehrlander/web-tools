...
//   chatRender.transcript(msgs, o?) -> element   // the full conversation, scrolled
//                                                // (msgs: array or house-format string;
//                                                //  o.chunk: turns per reveal batch, default 12)
//   chatRender.deck(msgs, o?)     -> element     // the same, as a swipeable card deck
//                                                // (one exchange per card; o.height: card height)
//
...
(() => {
  ...
  // one). Renders the first `chunk` turns up front and reveals the rest on one
  // click, appended a batch per animation frame: a long chat no longer builds
  // every turn and every code card synchronously on expand.
  ...
  // height (CSS length); per-message collapse is off inside a card.
  function deck(messages, o = {}) {
    ...
    const track = h('div', { class: 'cr-track flex overflow-x-auto snap-x snap-mandatory scroll-smooth overscroll-x-contain',
      style: `height:${height};scrollbar-width:none` });
    const bodies = [];
    ...
    if (groups.length > 1) {
      ...
      const nav = h('div', { class: 'flex items-center justify-center gap-3 pt-2.5' }, prevB, counter, nextB);
      const sync = a => {
  ...
  window.chatRender = { ready, parse, markdown, block, message, transcript, deck, blockViews, ROLES };
})();
