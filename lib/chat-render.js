...
(() => {
  ...
  // default on narrow viewports (nicer to swipe on a phone), Scroll on wide;
  // each view builds once and is kept. o.mode forces the initial view.
  function transcript(messages, o = {}) {
    ...
    const built = {};
    let mode = o.mode || (matchMedia('(max-width: 640px)').matches ? 'cards' : 'scroll');
    const render = () => {
