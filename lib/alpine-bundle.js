(function() {
    const registerMagics = () => {
        ...
        // A toast may carry ONE action, { label, run }, rendered as a button
        // beside its text. Added for the Lists view's undo: an edit there now
        // commits on a click elsewhere, so an accidental save needs a way back,
        // and a toast is where a reversal belongs rather than a control living
        // on in the row after the moment has passed. Optional, so every
        // existing three- and four-argument call is untouched.
        //
        // Dismissal is BY ID. It used to splice index 0 on a timer, which is
        // right only while toasts retire in the order they arrived; taking one
        // early, which an action button does, made the next timer remove the
        // wrong one. Nothing depended on the old behaviour: the id was already
        // there as the x-for key.
        const dropToast = (id) => {
            const i = toasts.findIndex(t => t.id === id)
            if (i > -1) toasts.splice(i, 1)
        }
        const toast = (icon, msg, cls = 'alert-info', ms = 3000, action = null) => {
            const id = Date.now() + Math.random()
            toasts.push({ icon, msg, cls, id, action: action && {
                label: action.label,
                run: () => { dropToast(id); action.run() },
            } })
