...
(() => {
  ...
  function inspectPowerShell(text) {
    ...
    for (const match of code.matchAll(/&&|\|\|/g)) compatibility(match, 'Pipeline chain operators require PowerShell 7 or later; Windows PowerShell 5.1 cannot use them.', 'ps51-pipeline-chain');
    for (const match of code.matchAll(/(?:\$\{[\w:]+\}|\$[\w:]+|\)|\])\s+(\?\?=?)\s*/g)) compatibility(match, 'Null-coalescing operators require PowerShell 7 or later.', 'ps51-null-coalescing');
