...
//   mdDoc.contain(el, o?)            -> el, nothing left that can widen a column
//                                       (`wrap` makes a fence wrap rather than
//                                       scroll, `numbers` puts a line gutter on
//                                       a fence long enough to want one; see the
//                                       notes above contain and NUM_MIN)
//   mdDoc.number(el, min)            -> el, every fence of min+ lines numbered
//   mdDoc.unnumber(el)               -> el, the gutter taken back off
...
(() => {
  ...
  // `numbering` says the caller has asked for a line gutter, which is the one
  // thing this function has to tell Prism about before it runs: see hookPrism.
  function labelFences(root, numbering) {
    ...
    needPrism().then((P) => {
      if (!P) return;
      if (numbering) hookPrism(P);
      try { P.highlightAllUnder(root); } catch (e) { /* a grammar that will not load */ }
    });
  }

  // ── Numbered lines ────────────────────────────────────────────────────────
  // A GUTTER ONLY WHERE IT IS EARNED, which is why this is off by default and
  // takes a minimum. Measured across this repo's `docs/` and `.claude/skills/`:
  // 107 fences, median 3 lines, 72% of two lines or more, 27% of eight or more,
  // the longest 136. A number beside a one-line shell command is noise in a
  // column that is already the narrowest thing on a phone; a number on line 74
  // of a 136-line sample is how a reader says which line they mean.
  const NUM_MIN = 8;

  // WHY NOT PRISM'S line-numbers PLUGIN. It draws a gutter of fixed-height rows
  // behind the block and counts LOGICAL lines, which is the same thing as
  // visual lines only while nothing wraps. `contain(el, {wrap: true})` exists
  // precisely to make fences wrap, so the plugin's numbers slide out of
  // register against their own code from the first wrapped line onward.
  //
  // One element per line and a CSS counter has no such assumption: the number
  // is drawn at the top left of the line's own block, so a line that takes
  // three visual rows still carries one number and its continuation rows
  // indent under the first character. The number is generated content, so it is
  // not selectable and does not travel with a copy, which the plugin's real
  // gutter elements do.
  //
  // Splitting has to re-open the ancestor chain, because a highlighted token
  // can span a newline (a block comment, a heredoc, a template literal). `cut`
  // returns one clone of a node per line the node spans, each holding that
  // line's share of its children, so `<span class="token comment">a\nb</span>`
  // becomes two comment spans on two lines rather than one span swallowing the
  // break.
  function splitLines(code) {
    const doc = code.ownerDocument;
    const cut = (node) => {
      if (node.nodeType === 3) return node.data.split('\n').map((t) => doc.createTextNode(t));
      const out = [node.cloneNode(false)];
      for (const kid of [...node.childNodes]) {
        cut(kid).forEach((part, i) => {
          if (i) out.push(node.cloneNode(false));
          out[out.length - 1].append(part);
        });
      }
      return out;
    };
    const rows = cut(code);
    // marked ends a fence with a newline, so the last row is empty and would
    // otherwise be numbered as a line the document does not have.
    while (rows.length > 1 && !rows[rows.length - 1].textContent) rows.pop();
    return rows;
  }

  // Every block this pass has claimed, with the source Prism should read rather
  // than the DOM. A WeakMap rather than a data attribute: a 136-line sample is
  // several kilobytes and belongs in the module, not in the markup.
  const NUMBERED = new WeakMap();

  function wrapLines(code, rows) {
    code.textContent = '';
    for (const row of rows) {
      const line = code.ownerDocument.createElement('span');
      line.className = 'md-line';
      while (row.firstChild) line.append(row.firstChild);
      code.append(line);
    }
    code.dataset.mdLines = String(rows.length);
    // The gutter is as wide as the widest number it has to hold, so a nine-line
    // block does not pay for three digits.
    code.parentElement.style.setProperty('--md-gutter', `${String(rows.length).length}ch`);
    code.parentElement.classList.add('md-numbered');
  }

  // THE HIGHLIGHTER RUNS TWICE AND NEITHER PASS IS THE ONE YOU PLANNED FOR.
  // Measured 2026-09-17 on this page: the autoloader's first pass completes
  // with the grammar still absent and zero tokens, then fetches
  // `prism-<lang>.min.js` and calls highlightElement AGAIN when it arrives, a
  // second or so later. So `await needPrism()` does not mean highlighting has
  // finished, and an ordering built on it numbered the block and then watched
  // Prism assign over the wrappers.
  //
  // Worse, it cannot simply be reversed. Numbering takes the newlines OUT of
  // the text (each line is a block, so the break comes from layout), which
  // leaves `code.textContent` a single joined line: the measured symptom was
  // `#!/usr/bin/env bashset -euo pipefail`, highlighted as one line, because
  // Prism reads the DOM to decide what to highlight.
  //
  // Two hooks settle it, and after them the order stops mattering. Prism's
  // `before-sanity-check` lets a hook replace `env.code`, so a numbered block
  // hands over the source it was built from rather than the joined text; its
  // `complete` fires after every pass, deferred ones included, so the wrappers
  // go back on whatever Prism has just written. Numbering can then run
  // synchronously in contain(), which is what lets a caller measure it without
  // waiting on a network fetch.
  let hooked = false;
  function hookPrism(P) {
    if (hooked) return;
    hooked = true;
    try {
      P.hooks.add('before-sanity-check', (env) => {
        const held = NUMBERED.get(env.element);
        if (held) env.code = held.src;
      });
      P.hooks.add('complete', (env) => {
        const held = NUMBERED.get(env.element);
        if (!held) return;
        wrapLines(env.element, splitLines(env.element));
      });
    } catch (e) { hooked = false; /* a Prism with no hooks is unhighlighted, not broken */ }
  }

  function numberLines(root, min) {
    if (!root) return root;
    for (const code of root.querySelectorAll('pre > code')) {
      if (NUMBERED.has(code)) continue;
      const rows = splitLines(code);
      if (rows.length < min) continue;
      NUMBERED.set(code, { src: code.textContent, min });
      wrapLines(code, rows);
    }
    return root;
  }

  // TAKING IT BACK OFF, which is what makes the gutter a control rather than a
  // page-load setting. It unwraps rather than re-rendering from the source it
  // holds, so a block that Prism has already coloured keeps its colours: the
  // line elements are unwrapped in order with the newline they stood for put
  // back between them, which is the exact tree wrapLines was handed.
  //
  // The two pieces of bookkeeping have to go with it. Dropping the WeakMap
  // entry is what lets a later numberLines claim the block again; without it
  // the gutter could be turned off once and never back on. And Prism's hooks
  // read the same map, so an un-numbered block stops being re-wrapped by a
  // highlighter pass that lands afterwards.
  function unnumberLines(root) {
    if (!root) return root;
    for (const code of root.querySelectorAll('pre > code')) {
      const lines = [...code.children].filter((c) => c.classList.contains('md-line'));
      if (!lines.length) continue;
      const doc = code.ownerDocument;
      const frag = doc.createDocumentFragment();
      lines.forEach((line, i) => {
        if (i) frag.append(doc.createTextNode('\n'));
        while (line.firstChild) frag.append(line.firstChild);
      });
      code.textContent = '';
      code.append(frag);
      delete code.dataset.mdLines;
      code.parentElement.classList.remove('md-numbered');
      code.parentElement.style.removeProperty('--md-gutter');
      NUMBERED.delete(code);
    }
    return root;
  }
  ...
  // The two containment choices a caller makes, pulled out so html(), render()
  // and enhance() forward them rather than each naming them again. A caller
  // that reaches contain() directly passes the same two.
  const CONTAIN_OPTS = (o) => ({ wrap: o.wrap, numbers: o.numbers });
  ...
  function contain(root, o = {}) {
    ...
    if (o.numbers) numberLines(root, o.numbers === true ? NUM_MIN : Number(o.numbers) || NUM_MIN);
    labelFences(root, !!o.numbers);
  ...
  window.mdDoc = {
    KIND, split, reference, payload, html, render, enhance, contain, declare, declaredIn, locate, sourceRef,
                   ...
                   // The gutter as a pair a host can drive after the render, not
                   // only as an option at it. NUM_MIN is the default minimum.
                   number: numberLines, unnumber: unnumberLines, NUM_MIN };
})();
