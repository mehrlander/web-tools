    ...
    mount(container, { rows = [], columns = null, name = 'data.csv', opts = {} } = {}) {
      ...
      function renderColumnsView() {
        ...
        function updateSplitView() {
          ...
          if (colSearch) cols = cols.filter(c => c.name.toLowerCase().includes(colSearch));

          if (colSort === 'name') cols.sort((a, b) => a.name.localeCompare(b.name));
          ...
          for (const col of cols) {
            ...
            const tr = h('tr', {
              ...
              h('td', { class: 'font-mono text-xs font-semibold' },
                h('span', { title: col.name, text: col.name })
              ),
              h('td', {},
        ...
        function renderInspector(col) {
          ...
          const hd = h('div', { class: 'flex items-start justify-between gap-2 pb-2 border-b border-base-300' },
            h('div', { class: 'min-w-0 flex-1' },
              ...
              h('h3', { class: 'font-mono text-sm font-bold truncate text-base-content', title: col.name, text: col.name })
            ),
          ...
          if (col.topValues && col.topValues.length && col.type !== 'empty') {
            ...
            for (const tv of col.topValues) {
              const row = h('div', {
                ...
                title: `Filter Data view by ${col.name} = "${tv.value}"`,
