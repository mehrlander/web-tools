        ...
        if (node.nodeName === 'row') {
          ...
          for (const c of directChildren(node, 'c')) {
            ...
            cols.set(idx, {
              ...
              // Whether the VALUE was computed. The sheet total already counts
              // formulas; which column they are in is what says a column is
              // derived rather than entered, and profileColumns reads it.
              formula: !!directChild(c, 'f'),
            });
          }
      ...
      sheet.rows = [...sheet.rawRows.entries()]
        ...
        .map(([row, { cols, ht, hidden }]) => ({
          ...
          formulas: Object.fromEntries([...cols.entries()]
            .filter(([, c]) => c.formula).map(([idx]) => [idx, true])),
        }));
