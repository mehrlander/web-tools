...
(() => {
  ...
  // OpenXML Chart parser (<c:chartSpace> -> structured chart model).
  const CHART_POINT_LIMIT = 10000;
  const chartNumber = (value) => {
    if (value == null || String(value).trim() === '') return null;
    const n = Number(value);
    return Number.isFinite(n) ? n : null;
  };
  const chartDescendants = (node, name) => [...(node?.getElementsByTagName('*') || [])].filter(e => tag(e) === name);
  const chartTitle = (node) => {
    const title = kidNamed(node, 'title');
    return chartDescendants(title, 't').map(e => e.textContent).join('') ||
      chartDescendants(title, 'v')[0]?.textContent || '';
  };

  function parseChartXml(docOrStr) {
    const doc = typeof docOrStr === 'string' ? new DOMParser().parseFromString(docOrStr, 'application/xml') : docOrStr;
    if (!doc || doc.getElementsByTagName('parsererror').length) return null;
    const chartEl = chartDescendants(doc, 'chart')[0];
    const plot = kidNamed(chartEl, 'plotArea');
    if (!plot) return null;
    const types = [...plot.children].filter(e => /Chart$/.test(tag(e)));
    const typeNode = types[0];
    const chartType = typeNode ? tag(typeNode) : 'unknown';
    const grouping = kidNamed(typeNode, 'grouping')?.getAttribute('val') || null;
    const subType = chartType === 'barChart'
      ? (kidNamed(typeNode, 'barDir')?.getAttribute('val') === 'bar' ? 'bar' : 'column')
      : chartType.replace(/Chart$/, '');
    const unsupported = [];
    if (types.length > 1) unsupported.push('Combination charts');
    if (!['barChart', 'lineChart', 'pieChart', 'doughnutChart'].includes(chartType)) unsupported.push(`${subType} charts`);
    if (grouping && !['clustered', 'standard'].includes(grouping)) unsupported.push(`${grouping} grouping`);
    if (kidNamed(plot, 'dateAx')) unsupported.push('Date axes');
    const legendEl = kidNamed(chartEl, 'legend');
    const legend = legendEl ? { pos: kidNamed(legendEl, 'legendPos')?.getAttribute('val') || 'r' } : null;
    const readAxis = (el) => {
      const scaling = kidNamed(el, 'scaling');
      if (kidNamed(scaling, 'logBase')) unsupported.push('Logarithmic axes');
      return {
        min: chartNumber(kidNamed(scaling, 'min')?.getAttribute('val')),
        max: chartNumber(kidNamed(scaling, 'max')?.getAttribute('val')),
        majorUnit: chartNumber(kidNamed(el, 'majorUnit')?.getAttribute('val')),
        reversed: kidNamed(scaling, 'orientation')?.getAttribute('val') === 'maxMin',
        title: chartTitle(el),
        formatCode: kidNamed(el, 'numFmt')?.getAttribute('formatCode') || '',
      };
    };
    if (kidsNamed(plot, 'valAx').length > 1) unsupported.push('Multiple value axes');

    // The point index is a position, not a sorting hint. Absent points stay null.
    const readData = (el, numeric) => {
      const pts = chartDescendants(el, 'pt');
      const count = chartNumber(chartDescendants(el, 'ptCount')[0]?.getAttribute('val')) || 0;
      const indices = pts.map(p => Number(p.getAttribute('idx')));
      const length = Math.max(count, ...indices.map(i => i + 1), 0);
      if (!Number.isInteger(length) || length > CHART_POINT_LIMIT || indices.some(i => !Number.isInteger(i) || i < 0)) {
        unsupported.push('Chart data exceeds preview limits');
        return { values: [], ref: null, formatCode: '' };
      }
      const values = Array(length).fill(null);
      pts.forEach((p, i) => {
        const text = kidNamed(p, 'v')?.textContent;
        values[indices[i]] = numeric ? chartNumber(text) : (text ?? null);
      });
      return {
        values,
        ref: chartDescendants(el, 'f')[0]?.textContent?.trim() || null,
        formatCode: chartDescendants(el, 'formatCode')[0]?.textContent || '',
      };
    };
    const defaultMarkers = kidNamed(typeNode, 'marker')?.getAttribute('val') === '1';
    const series = kidsNamed(typeNode, 'ser').map((ser, index) => {
      const tx = kidNamed(ser, 'tx');
      const categories = readData(kidNamed(ser, 'cat'), false);
      const values = readData(kidNamed(ser, 'val'), true);
      const markerNode = kidNamed(ser, 'marker');
      const marker = {
        symbol: kidNamed(markerNode, 'symbol')?.getAttribute('val') || (defaultMarkers ? 'auto' : 'none'),
        size: chartNumber(kidNamed(markerNode, 'size')?.getAttribute('val')) || 5,
      };
      if (!['auto', 'none', 'circle', 'square', 'diamond', 'triangle', 'x', 'plus', 'dash', 'dot', 'star'].includes(marker.symbol)) unsupported.push('Custom markers');
      const spPr = kidNamed(ser, 'spPr');
      const line = kidNamed(spPr, 'ln');
      const colorNode = chartDescendants(line || spPr, 'srgbClr')[0];
      return {
        name: kidNamed(tx, 'v')?.textContent || chartDescendants(tx, 'v')[0]?.textContent || `Series ${index + 1}`,
        categories: categories.values,
        categoriesRef: categories.ref,
        values: values.values,
        valuesRef: values.ref,
        formatCode: values.formatCode,
        color: hexOf(colorNode?.getAttribute('val')),
        marker,
        lineWidth: chartNumber(line?.getAttribute('w')) == null ? 3 : Number(line.getAttribute('w')) / 9525,
        noLine: !!kidNamed(line, 'noFill'),
      };
    });
    const sharedCategories = series.find(s => s.categories.length)?.categories || [];
    series.forEach(s => { if (!s.categories.length && !s.categoriesRef) s.categories = sharedCategories; });
    return {
      title: chartTitle(chartEl), chartType, subType, grouping, legend, series,
      blanks: kidNamed(chartEl, 'dispBlanksAs')?.getAttribute('val') || 'gap',
      valueAxis: readAxis(kidNamed(plot, 'valAx')),
      categoryAxis: readAxis(kidNamed(plot, 'catAx')),
      unsupported,
    };
  }

  // Resolve direct worksheet references only after sheets and shared strings
  // are complete. A chart cache can store zero for a genuinely blank cell.
  // Formula cells use their saved value; this reader does not recalculate Excel.
  function resolveChartData(chart, xl) {
    const readRange = (ref, numeric) => {
      const m = /^(?:'((?:[^']|'')+)'|([^'!]+))!\$?([A-Z]+)\$?(\d+)(?::\$?([A-Z]+)\$?(\d+))?$/i.exec(ref || '');
      if (!m) return null;
      const name = (m[1]?.replace(/''/g, "'") || m[2]).toLowerCase();
      const sheet = Object.values(xl.sheets).find(s => s.name?.toLowerCase() === name);
      if (!sheet) return null;
      const c1 = colIndexFromRef(m[3].toUpperCase()), c2 = colIndexFromRef((m[5] || m[3]).toUpperCase());
      const r1 = Number(m[4]), r2 = Number(m[6] || m[4]);
      const count = (r2 - r1 + 1) * (c2 - c1 + 1);
      if (c1 !== c2 && r1 !== r2 || count < 1 || count > CHART_POINT_LIMIT || r1 < 1 || c2 > 16383 || r2 > 1048576) return null;
      const rows = new Map(sheet.rows.map(row => [row.row, row]));
      const values = [];
      for (let r = r1; r <= r2; r++) for (let c = c1; c <= c2; c++) {
        const row = rows.get(r), raw = row?.cells[c];
        if (row?.types[c] === 'e') chart.unsupported.push('Error values in chart data');
        if (row?.formulas[c] && (raw == null || raw === '')) chart.unsupported.push('Chart formulas without saved results');
        values.push(numeric ? chartNumber(raw) : (raw == null || raw === '' ? null : String(raw)));
      }
      return values;
    };
    for (const series of chart.series) {
      for (const [key, numeric] of [['categories', false], ['values', true]]) {
        const ref = series[key + 'Ref'];
        if (!ref) continue;
        const values = readRange(ref, numeric);
        if (values) series[key] = values;
        else chart.unsupported.push('Chart source cannot be resolved');
      }
    }
    chart.unsupported = [...new Set(chart.unsupported)];
  }
  ...
  // A CACHE VALUE, in the six shapes OOXML writes it. The same element names
  // serve a cacheField's <sharedItems> and a record's cells, so one reader does
  // both. `x` is the seventh and appears only in a record: an index into its
  // field's shared items, resolved in finalize because the records part may be
  // walked before the definition that explains it.
  //
  // A boolean is kept as a boolean and a missing cell as null rather than as
  // the empty string, since "" is a value a text field can hold and a blank is
  // not the same fact.
  function cacheValue(node) {
    const v = node.getAttribute('v');
    switch (tag(node)) {
      case 's': return unescapeXlsx(v ?? '');
      case 'n': return Number(v);
      case 'd': return v ?? '';
      case 'b': return v === '1' || v === 'true';
      case 'e': return v ?? '#N/A';
      case 'm': return null;
      default: return undefined;      // not a value element
    }
  }

  // A pivot cache can hold every row of its source, so the records are capped
  // and the cap is REPORTED rather than silently applied: a reader comparing a
  // record count against what is on screen has to be able to see the cut.
  const MAX_PIVOT_RECORDS = 20000;

  // <connection type="N">. The code, not the provider: a reader wants to know
  // a query is a web query without the kit claiming to understand the URL.
  const CONNECTION_KIND = {
    1: 'ODBC', 2: 'DAO', 3: 'File', 4: 'Web', 5: 'OLE DB', 6: 'Text', 7: 'ADO', 8: 'DSP',
  };
  ...
  function analyze(parts) {
    ...
    const xl = {
      sheets: {},                // sheetN -> { usedStrings:Set, usedStyles:Set, cellCount, formulas, mergedCells, dimension, sharedFormulas, cols, merges, freeze, rawRows:Map<row,{cols,ht,hidden}> }
      strings: [],
      styles: new Set(),
      comments: {},
      relationships: {},
      ...
      // ---- WHAT THE WORKBOOK REACHES OUTSIDE ITSELF, and what it summarises
      // of its own data. Both were walked as XML and interpreted as nothing
      // until 2026-09-13, so a workbook driven by a query or organised around
      // a pivot read here as a plain grid with unexplained parts beside it.
      connections: [],           // xl/connections.xml -> [{ id, name, kind, detail }]
      pivotCaches: {},           // cache part -> { recordCount, source, fields, sharedItems }
      pivotRecords: {},          // records part -> [[cell…]], resolved in finalize
      pivotTables: {},           // table part -> { name, cacheId, ref, rowFields… }
      pivotCacheRefs: {},        // workbook cacheId -> rId, joined to a part in finalize
      tables: {},                // table part -> { id, name, displayName, ref, headerRowCount, totalsRowCount, styleInfo }
      ...
      charts: {},                // chart part -> parsed chart model
    ...
    function walkPart(fn, xmlString) {
      ...
      if (/charts?\/chart\d*\.xml$/i.test(fn)) {
        touch(fn, 'chart', ALL_SHEETS);
        const parsed = parseChartXml(doc);
        if (parsed) xl.charts[fn] = parsed;
      }
    ...
    function recordConnections(node, fn, path) {
      ...
      if (sheetName) {
        const sheet = (xl.sheets[sheetName] ??= {
          ...
          // The used range as the file declares it (`<dimension ref>`), and
          // the text of each shared-formula master by its `si` index.
          dimension: null, sharedFormulas: {},
        ...
        if (node.nodeName === 'dimension' && path.endsWith('worksheet.dimension')) {
          sheet.dimension = node.getAttribute('ref') || null;
        }
        ...
        if (node.nodeName === 'row') {
          ...
          for (const c of directChildren(node, 'c')) {
            ...
            nextIdx = idx + 1;
            // A value can sit in three places, not one. t="s" indexes sharedStrings and is
            ...
            // THE FORMULA, as the file writes it: no leading '=', since that
            // is Excel's UI rather than the stored text. `true` where the cell
            // is computed and the text is not there to have, which is what a
            // SHARED formula's followers look like: the master carries the
            // text and every cell after it carries `<f t="shared" si="0"/>`
            // and nothing else. Recovering those means rewriting relative
            // references per cell, which is a formula engine, so the honest
            // answer is "computed, text not stored here".
            //
            // Truthy either way, which is the contract profileColumns already
            // reads (a computed cell counted apart from an entered one). It
            // was `true` for every computed cell until 2026-09-14; nothing
            // read the value, so widening it costs no caller.
            const f = directChild(c, 'f');
            // THE SHARED-FORMULA INDEX rides beside the text, or beside the
            // absence of it: the master carries `si` and the text, a follower
            // carries `si` alone. Kept per cell, and the master's text kept
            // per sheet under that index, so a serializer can write a
            // follower as `=[fill N] <master text>`, which is the form the
            // PowerShell exporter (Get-WsDetail) already emits and the fund
            // view's reader already resolves. Still not a formula engine:
            // nothing here shifts a reference.
            const si = f && f.getAttribute('t') === 'shared' ? f.getAttribute('si') : null;
            if (si != null && f.textContent) sheet.sharedFormulas[si] = f.textContent;
            cols.set(idx, {
              shared: t === 's', raw, type: t || null, style: c.getAttribute('s') || null,
              formula: f ? (f.textContent || true) : false,
              sharedId: si,
            });
          }
      ...
      if (/^xl\/drawings\/drawing\d+\.xml$/.test(fn)) {
        ...
        if (kind === 'twoCellAnchor' || kind === 'oneCellAnchor') {
          ...
          if (!inFallback) {
            ...
            // The blip's r:embed or chart's r:id points at the media or chart part through
            // the DRAWING's own rels, not the sheet's.
            const blip = [...node.getElementsByTagName('*')].find(e => tag(e) === 'blip');
            const embed = blip && (blip.getAttribute('r:embed') ||
              blip.getAttributeNS?.('http://schemas.openxmlformats.org/officeDocument/2006/relationships', 'embed'));
            const chartEl = [...node.getElementsByTagName('*')].find(e => tag(e) === 'chart');
            const chartRid = chartEl && (chartEl.getAttribute('r:id') ||
            ...
            const name = nv?.getAttribute('name') || nv?.getAttribute('descr') || '';
            const baseAnchor = {
      ...
      // EVERY EXTERNAL SOURCE THE WORKBOOK DECLARES, in one part. `type` is a
      // numeric code and CONNECTION_KIND names it; the kit classifies the code
      // rather than interpreting the provider string, the same line cellFormat
      // draws around a number format. The detail is whichever child element the
      // kind puts it in, which is why one field cannot be read off `connection`
      // alone.
      if (fn.endsWith('connections.xml')) {
        touch(fn, 'connections', ALL_SHEETS);
        if (node.nodeName === 'connection') {
          const db = kidNamed(node, 'dbPr'), web = kidNamed(node, 'webPr'), txt = kidNamed(node, 'textPr');
          const command = db?.getAttribute('command') || '';
          const connStr = db?.getAttribute('connection') || '';
          xl.connections.push({
            id: node.getAttribute('id') || '',
            name: unescapeXlsx(node.getAttribute('name') || ''),
            type: node.getAttribute('type') || '',
            kind: CONNECTION_KIND[node.getAttribute('type')] || 'Unknown',
            description: unescapeXlsx(node.getAttribute('description') || ''),
            // A Power Query connection is an OLE DB connection whose provider
            // is Microsoft's Mashup engine, so the M it runs is NOT here: it is
            // in the DataMashup blob readMashup() opens. Flagged so a reader
            // following a query lands there instead of on this string.
            powerQuery: /Microsoft\.Mashup/i.test(connStr),
            detail: unescapeXlsx(
              web?.getAttribute('url') || txt?.getAttribute('sourceFile') || command || connStr),
          });
        }
      }

      // A PIVOT CACHE IS THE DATA; A PIVOT TABLE IS ONE VIEW OF IT. They live in
      // separate parts and several tables may share one cache, so the two are
      // collected apart and joined in finalize through the workbook's own
      // cacheId map rather than by part number, which nothing guarantees.
      if (/pivotCacheDefinition\d*\.xml$/.test(fn)) {
        touch(fn, 'pivot-cache', ALL_SHEETS);
        const cache = (xl.pivotCaches[fn] ??= {
          recordCount: null, source: null, fields: [], sharedItems: [], recordsPart: null,
        });
        if (node.nodeName === 'pivotCacheDefinition') {
          const n = Number(node.getAttribute('recordCount'));
          cache.recordCount = isFinite(n) ? n : null;
        }
        // The range the cache was built from, named by DISPLAY sheet name
        // (the one thing in an .xlsx that addresses a sheet the way a reader
        // does) or, for a cache over a table, by that table's name.
        if (node.nodeName === 'worksheetSource') {
          cache.source = {
            sheet: unescapeXlsx(node.getAttribute('sheet') || ''),
            ref: node.getAttribute('ref') || '',
            name: unescapeXlsx(node.getAttribute('name') || ''),
          };
        }
        if (node.nodeName === 'cacheField' && path.endsWith('cacheFields.cacheField')) {
          cache.fields.push(unescapeXlsx(node.getAttribute('name') || ''));
          // Parallel to `fields` by index, the shape the row record already
          // uses for styles and types. A field with no <sharedItems> keeps an
          // empty list, which is what a records part's <x> would then index
          // into and miss, honestly.
          cache.sharedItems.push([]);
        }
        // EXACTLY the shared-items path. A cacheField may also carry a
        // <fieldGroup><groupItems> of the same element names, and folding those
        // in would shift every <x> index in the records by however many groups
        // the author made.
        if (path.endsWith(`cacheFields.cacheField.sharedItems.${node.nodeName}`)) {
          const value = cacheValue(node);
          if (value !== undefined) cache.sharedItems[cache.sharedItems.length - 1]?.push(value);
        }
      }

      // THE ROWS BEHIND A PIVOT. Stored as read and resolved in finalize: an
      // <x> cell is an index into the cache definition's shared items and the
      // definition may not have been walked yet, which is the same reason
      // shared strings resolve there.
      if (/pivotCacheRecords\d*\.xml$/.test(fn)) {
        touch(fn, 'pivot-records', ALL_SHEETS);
        const rec = (xl.pivotRecords[fn] ??= { rows: [], total: 0, truncated: false });
        if (node.nodeName === 'r' && path.endsWith('pivotCacheRecords.r')) {
          rec.total++;
          if (rec.rows.length >= MAX_PIVOT_RECORDS) { rec.truncated = true; return; }
          rec.rows.push([...node.children].map(c => {
            if (tag(c) === 'x') return { x: Number(c.getAttribute('v') || 0) };
            const value = cacheValue(c);
            return value === undefined ? null : { v: value };
          }));
        }
      }

      if (/tables?\/table\d*\.xml$/i.test(fn)) {
        touch(fn, 'table', ALL_SHEETS);
        const tbl = (xl.tables[fn] ??= {
          id: '', name: '', displayName: '', ref: '', sheet: null,
          headerRowCount: 1, totalsRowCount: 0,
          styleInfo: null, autoFilter: null, columns: [],
        });
        if (node.nodeName === 'table') {
          tbl.id = node.getAttribute('id') || '';
          tbl.name = unescapeXlsx(node.getAttribute('name') || '');
          tbl.displayName = unescapeXlsx(node.getAttribute('displayName') || tbl.name);
          tbl.ref = node.getAttribute('ref') || '';
          const hrc = node.getAttribute('headerRowCount');
          tbl.headerRowCount = hrc != null ? Number(hrc) : 1;
          const trc = node.getAttribute('totalsRowCount');
          tbl.totalsRowCount = trc != null ? Number(trc) : 0;
        }
        if (node.nodeName === 'autoFilter') {
          tbl.autoFilter = node.getAttribute('ref') || '';
        }
        ...
        if (node.nodeName === 'tableStyleInfo') {
          tbl.styleInfo = {
            name: node.getAttribute('name') || '',
            showFirstColumn: node.getAttribute('showFirstColumn') === '1',
            showLastColumn: node.getAttribute('showLastColumn') === '1',
            showRowStripes: node.getAttribute('showRowStripes') !== '0',
            showColumnStripes: node.getAttribute('showColumnStripes') === '1',
          };
        }
      }

      if (/pivotTable\d*\.xml$/.test(fn)) {
        touch(fn, 'pivot-table', ALL_SHEETS);   // narrowed to its own sheet in finalize
        const pt = (xl.pivotTables[fn] ??= {
          name: '', cacheId: '', ref: '', sheet: null, cachePart: null,
          rowFields: 0, colFields: 0, pageFields: 0, dataFields: [],
          firstHeaderRow: 1, firstDataRow: 1, firstDataCol: 1, styleInfo: null, rowGrandTotals: true,
        });
        if (node.nodeName === 'pivotTableDefinition') {
          pt.name = unescapeXlsx(node.getAttribute('name') || '');
          pt.cacheId = node.getAttribute('cacheId') || '';
          pt.rowGrandTotals = node.getAttribute('rowGrandTotals') !== '0';
        }
        if (node.nodeName === 'location') {
          pt.ref = node.getAttribute('ref') || '';
          const fhr = node.getAttribute('firstHeaderRow');
          if (fhr != null) pt.firstHeaderRow = Number(fhr);
          const fdr = node.getAttribute('firstDataRow');
          if (fdr != null) pt.firstDataRow = Number(fdr);
          const fdc = node.getAttribute('firstDataCol');
          if (fdc != null) pt.firstDataCol = Number(fdc);
        }
        if (node.nodeName === 'pivotTableStyleInfo') {
          pt.styleInfo = {
            name: node.getAttribute('name') || '',
            showRowHeaders: node.getAttribute('showRowHeaders') === '1',
            showColHeaders: node.getAttribute('showColHeaders') === '1',
            showRowStripes: node.getAttribute('showRowStripes') === '1',
            showColStripes: node.getAttribute('showColStripes') === '1',
            showLastColumn: node.getAttribute('showLastColumn') === '1',
          };
        }
        if (node.nodeName === 'rowFields') pt.rowFields = kidsNamed(node, 'field').length;
        if (node.nodeName === 'colFields') pt.colFields = kidsNamed(node, 'field').length;
        if (node.nodeName === 'pageFields') pt.pageFields = kidsNamed(node, 'pageField').length;
        // The measures, which are the only fields with a name a reader wrote.
        if (node.nodeName === 'dataField' && path.endsWith('dataFields.dataField'))
          pt.dataFields.push(unescapeXlsx(node.getAttribute('name') || ''));
      }

      // The workbook's cacheId -> relationship map, the only statement of which
      // cache part a pivot table's `cacheId` means.
      if (fn.endsWith('workbook.xml') && path.endsWith('pivotCaches.pivotCache')) {
        const id = node.getAttribute('cacheId');
        const rid = node.getAttribute('r:id') || node.getAttribute('id') || '';
        if (id && rid) xl.pivotCacheRefs[id] = rid;
      }
      ...
      if (fn.endsWith('workbook.xml') && path.endsWith('sheets.sheet')) {
        xl.workbookSheets.push({
          ...
          // Omitted means visible. `veryHidden` is kept distinct from `hidden`
          // rather than folded in: only the second can be unhidden from
          // Excel's own menu, so a reader auditing a workbook wants the two
          // apart.
    ...
    for (const sheet of Object.values(xl.sheets)) {
      sheet.rows = [...sheet.rawRows.entries()]
        ...
        .map(([row, { cols, ht, hidden }]) => ({
          ...
          // The formula text where the file stored one, `true` where the cell
          // is computed and it did not. Only computed cells appear.
          formulas: Object.fromEntries([...cols.entries()]
            .filter(([, c]) => c.formula).map(([idx, c]) => [idx, c.formula])),
          // The shared-formula index of each cell that has one, master and
          // follower alike; the master's text is sheet.sharedFormulas[si].
          shared: Object.fromEntries([...cols.entries()]
            .filter(([, c]) => c.sharedId != null).map(([idx, c]) => [idx, c.sharedId])),
        }));
      delete sheet.rawRows;
    }

    for (const chart of Object.values(xl.charts)) resolveChartData(chart, xl);
    ...
    // Each sheet's pictures and charts, joined through the sheet's own rels to the drawing
    // part and through THAT part's rels to the media or chart. Two hops, and the second
    // is the one worth naming: a blip's r:embed or chart's r:id is scoped to the drawing, so
    ...
    for (const [key, sheet] of Object.entries(xl.sheets)) {
      ...
      const relsMap = Object.fromEntries(
        (xl.relationships[drawingPart] || []).map(r => [r.id, normalizePath(drawingPart, r.target)]));
      const drawings = (xl.drawings[drawingPart] || []).map(a => {
        if (a.kind === 'chart' || a.chartRid) {
          const chartPart = relsMap[a.chartRid];
          return { ...a, kind: 'chart', chartPart, chart: xl.charts[chartPart] || { title: '', series: [], unsupported: ['Chart data could not be read'] } };
        }
        const mediaPart = relsMap[a.embed];
        return { ...a, kind: 'image', part: mediaPart || null };
      }).filter(a => ((a.kind === 'chart' && a.chart) || (a.kind === 'image' && a.part)) && a.from);
      sheet.drawings = drawings;
      sheet.images = drawings.filter(d => d.kind === 'image');
    }
    ...
    // A sheet's own visibility, which lives only in workbook.xml. A sheet the
    // workbook never claims keeps `null` rather than a guessed 'visible', the
    // same rule the name and index join follows.
    for (const ws of xl.workbookSheets) {
      const target = relsById[ws.rid] || `worksheets/sheet${ws.sheetId}.xml`;
      const sheet = xl.sheets[sheetOf(target.replace(/^\//, '')) || ''];
      if (sheet) sheet.visibility = ws.state;
    }
    for (const sheet of Object.values(xl.sheets)) sheet.visibility ??= null;

    // Each pivot table to the sheet it is drawn on and to the cache it reads.
    // The sheet comes from the sheet's own rels, the cache from the workbook's
    // cacheId map; a table whose cache the file does not declare keeps a null
    // part rather than the nearest-numbered one.
    const cachePartById = Object.fromEntries(
      Object.entries(xl.pivotCacheRefs)
        .map(([id, rid]) => [id, relsById[rid] ? normalizePath('xl/', relsById[rid]) : null])
        .filter(([, part]) => part));
    for (const [key, sheet] of Object.entries(xl.sheets)) {
      const rels = xl.relationships[`xl/worksheets/${key}.xml`] || [];
      sheet.pivotTables = rels
        .filter(r => /pivotTable\d*\.xml$/i.test(r.target || ''))
        .map(r => normalizePath('xl/worksheets/', r.target))
        .filter(part => xl.pivotTables[part]);
      for (const part of sheet.pivotTables) xl.pivotTables[part].sheet = key;
    }
    for (const pt of Object.values(xl.pivotTables)) pt.cachePart = cachePartById[pt.cacheId] || null;

    for (const [key, sheet] of Object.entries(xl.sheets)) {
      const rels = xl.relationships[`xl/worksheets/${key}.xml`] || [];
      sheet.tables = rels
        .filter(r => /table\d*\.xml$/i.test(r.target || ''))
        .map(r => normalizePath('xl/worksheets/', r.target))
        .filter(part => xl.tables[part]);
      for (const part of sheet.tables) xl.tables[part].sheet = key;
    }

    // Each cache definition to the records part it declares, through its OWN
    // rels. Not by matching the number in the filename: the pairing is
    // conventional rather than guaranteed, and the rels state it.
    for (const [part, cache] of Object.entries(xl.pivotCaches)) {
      const rel = (xl.relationships[part] || [])
        .find(r => /pivotCacheRecords\d*\.xml$/i.test(r.target || ''));
      if (!rel) continue;
      const target = normalizePath(part, rel.target);
      if (!xl.pivotRecords[target]) continue;
      cache.recordsPart = target;
      // An <x> cell is an index into its OWN field's shared items, so the
      // column position is what picks the list. A record longer than the field
      // list, or an index the list does not reach, resolves to null rather than
      // to a neighbouring field's value.
      const items = cache.sharedItems;
      xl.pivotRecords[target].rows = xl.pivotRecords[target].rows.map(row =>
        row.map((cell, i) => {
          if (!cell) return null;
          if (!('x' in cell)) return cell.v;
          const list = items[i];
          return list && cell.x < list.length ? list[cell.x] : null;
        }));
    }
    // The sheet each pivot part actually belongs to, now that it is known: a
    // pivot table touched every sheet while it was only reachable as ALL_SHEETS.
    for (const [part, pt] of Object.entries(xl.pivotTables))
      if (pt.sheet && conns[part]) conns[part].sheets = new Set([pt.sheet]);
    for (const [part, tbl] of Object.entries(xl.tables))
      if (tbl.sheet && conns[part]) conns[part].sheets = new Set([tbl.sheet]);
  ...
  // COLUMN PROFILING: what each column of a sheet actually holds. Neither the
  // value grid nor the structure views answer it, and it is the first question
  // anyone asks of a data table. The browser counterpart to wps's
  // Get-ColumnRole and Format-DistinctPreview, which is the one capability the
  // crosswalk recorded as present in PowerShell and absent here.
  //
  // KIND IS DECIDED BY MAJORITY of the non-empty cells, not by the first one. A
  // numeric column with a "Total" label in its last row is a number column with
  // a label in it, and reading row 200 as the answer is the failure this guards
  // against. `mixed` is reported only when no kind holds a majority.
  //
  // A column is addressed by its zero-based index, the same as sheet.rows'
  // cells, with the A1 letter beside it for a reader.
  function profileColumns(sheet, xl, opts) {
    const cfg = opts || {};
    const headerRow = cfg.headerRow === undefined ? 1 : cfg.headerRow;
    const keep = cfg.distinct === undefined ? 12 : cfg.distinct;
    const all = sheet.rows || [];
    const header = headerRow == null ? null : all.find(r => r.row === headerRow);
    const body = headerRow == null ? all : all.filter(r => r.row !== headerRow);
    const scanned = cfg.maxRows ? body.slice(0, cfg.maxRows) : body;

    // Every column any scanned row writes to, plus any the header names: a
    // column whose values are all blank is still a column if it has a heading.
    const cols = new Set();
    for (const r of [...scanned, ...(header ? [header] : [])])
      for (const k of Object.keys(r.cells || {})) cols.add(Number(k));

    const cellKind = (raw, t, style) => {
      if (t === 's' || t === 'inlineStr' || t === 'str') return 'text';
      if (t === 'b') return 'boolean';
      if (t === 'e') return 'error';
      const kind = cellFormat(xl, style)?.kind;
      if (kind === 'date' || kind === 'datetime' || kind === 'time') return 'date';
      // An unstyled cell with no `t` is a number in OOXML, but a hand-built
      // part and a text-typed column both reach here, so the value decides.
      return isFinite(Number(raw)) && String(raw).trim() !== '' ? 'number' : 'text';
    };

    return [...cols].sort((a, b) => a - b).map(idx => {
      const counts = new Map();          // value -> times seen
      const kinds = new Map();           // kind -> times seen
      let filled = 0, computed = 0, min = null, max = null;
      for (const r of scanned) {
        const raw = r.cells?.[idx];
        if (raw === undefined || raw === null || String(raw).trim() === '') continue;
        filled++;
        if (r.formulas?.[idx]) computed++;
        const value = String(raw);
        counts.set(value, (counts.get(value) || 0) + 1);
        const kind = cellKind(raw, r.types?.[idx], r.styles?.[idx]);
      ...
      const top = ranked[0];
      const kind = !top ? 'empty' : top[1] * 2 > filled ? top[0] : 'mixed';
      // The ROLE a column plays, which is a different question from what type
      // its cells are: an id column and a code column are both text. The
      // ladder and its names are Get-ColumnRole's from wps's ExcelCom.ps1, so
      // the two routes report a column the same way and a reader moving
      // between them is not learning a second vocabulary.
      const role =
        !filled ? 'Empty'
        : counts.size === 1 ? 'Constant'
        : counts.size === filled && filled === scanned.length ? 'Key'
        : kind === 'number' ? 'Measure'
        : kind === 'text' && counts.size <= (cfg.categorical === undefined ? 25 : cfg.categorical) ? 'Categorical'
        : 'Plain';
      return {
        col: idx,
        letter: colLetter(idx),
        header: header ? String(header.cells?.[idx] ?? '') : '',
        rows: scanned.length,
        filled,
        blank: scanned.length - filled,
        // A column every one of whose values was computed is a derived column,
        // and that is a different thing from data somebody entered.
        computed,
        distinct: counts.size,
        kind,
        role,
        kinds: Object.fromEntries(ranked),
        min, max,
        // Most common first, so a coded column shows its codes and a free-text
        // column shows that its values barely repeat.
        top: [...counts.entries()].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))
          .slice(0, keep).map(([value, count]) => ({ value, count })),
      };
    });
  }
  ...
  function cellStyle(xl, styleIndex) {
    ...
    return {
      ...
      border: border && (border.top || border.right || border.bottom || border.left) ? border : (xf.borderId > 0 && border ? border : null),
  ...
  function sheetLayout(sheet, xl, opts) {
    ...
    // The drawn range covers the ANCHORS too, not only the cells. A picture or
    // chart on an otherwise empty sheet is the whole content of that sheet, and
    // stopping at the last row that holds a value drops it.
    const anchored = [...(sheet?.drawings || []), ...(sheet?.images || [])];
    ...
    if (lastRow < 1 || lastCol < 0) {
      return { cols: [], rows: [], maxCol: -1, freeze: null, truncated: null,
               images: [], drawings: [], cfSkipped: 0, empty: true };
    }
    ...
    markLinks(out, sheet);
    markTablesAndPivots(out, sheet, xl);
    const drawings = placeDrawings(sheet, xl, out, cols);
    const images = drawings.filter(d => d.kind === 'image');

    return { cols, rows: out, maxCol: lastCol, freeze: sheet.freeze || null,
             truncated, images, drawings, cfSkipped, empty: false };
  }
  ...
  // Regional marking for native Excel Tables (ListObjects) and PivotTables.
  // Neither writes visual theme styles into cell formatting (cellXfs), so
  // this labels structural roles (headers, striping, subtotals, grand totals,
  // filter carets, and hierarchy parent toggles) from the table bounding boxes.
  function markTablesAndPivots(rows, sheet, xl) {
    if (!xl) return;

    // 1. Tables (ListObjects / QueryTables)
    const tables = (sheet.tables || []).map(p => xl.tables?.[p]).filter(Boolean);
    for (const tbl of tables) {
      const rng = parseRange(tbl.ref);
      if (!rng) continue;
      const hrc = tbl.headerRowCount != null ? tbl.headerRowCount : 1;
      const trc = tbl.totalsRowCount != null ? tbl.totalsRowCount : 0;
      const dataStart = rng.r1 + hrc;
      const dataEnd = rng.r2 - trc;
      const showStripes = tbl.styleInfo ? tbl.styleInfo.showRowStripes : true;

      for (const r of rows) {
        if (r.row < rng.r1 || r.row > rng.r2) continue;
        const isHeader = r.row < dataStart;
        const isTotal = trc > 0 && r.row > dataEnd;
        const isData = !isHeader && !isTotal;
        const isStripe = isData && showStripes && ((r.row - dataStart) % 2 === 1);

        for (const cell of r.cells) {
          if (cell.col < rng.c1 || cell.col > rng.c2) continue;
          if (isHeader) {
            cell.tableRole = 'tbl-hdr';
            // Excel draws the dropdown only where the table keeps its filter.
            if (tbl.autoFilter) cell.hasFilter = true;
          } else if (isTotal) {
            cell.tableRole = 'tbl-total';
          } else if (isStripe) {
            cell.tableRole = 'tbl-stripe';
          } else {
            cell.tableRole = 'tbl-data';
          }
        }
      }
    }
    ...
    // 2. Pivot Tables
    const pivots = (sheet.pivotTables || []).map(p => xl.pivotTables?.[p]).filter(Boolean);
    for (const pt of pivots) {
      const rng = parseRange(pt.ref);
      if (!rng) continue;
      const fdr = pt.firstDataRow != null ? pt.firstDataRow : 1;
      const dataStartRow = rng.r1 + fdr;
      const fdc = pt.firstDataCol != null ? pt.firstDataCol : 1;
      const dataStartCol = rng.c1 + fdc;

      for (const r of rows) {
        if (r.row < rng.r1 || r.row > rng.r2) continue;
        const isHeaderRow = r.row < dataStartRow;
        const isGrandTotal = pt.rowGrandTotals && r.row === rng.r2;

        let isSubtotal = false;
        if (!isHeaderRow && !isGrandTotal) {
          for (const c of r.cells) {
            if (c.col >= rng.c1 && c.col < dataStartCol) {
              const txt = (c.text || '').trim();
              if (txt && /total$/i.test(txt) && !/grand total$/i.test(txt)) {
                isSubtotal = true;
                break;
              }
            }
        ...
        for (const cell of r.cells) {
          if (cell.col < rng.c1 || cell.col > rng.c2) continue;
          if (isHeaderRow) {
            cell.pvtRole = 'pvt-hdr';
            // The dropdown sits on the field buttons only: the row-label
            // headers just above the data, and the column-label header on the
            // top row. Item captions such as FY2028 carry none.
            if (cell.text && ((r.row === dataStartRow - 1 && cell.col < dataStartCol) ||
                (pt.colFields > 0 && r.row === rng.r1 && cell.col === dataStartCol))) cell.hasFilter = true;
          } else if (isGrandTotal) {
            cell.pvtRole = 'pvt-grandtotal';
          } else if (isSubtotal) {
            cell.pvtRole = 'pvt-subtotal';
          } else if (cell.col < dataStartCol) {
            cell.pvtRole = 'pvt-row-hdr';
            // A collapse box means an item has children, so only a pivot with
            // more than one row field draws one.
            if (pt.rowFields > 1 && cell.text && !cell.text.includes('Total') && cell.col === rng.c1) {
              cell.hasHierarchyToggle = true;
            }
          } else {
            cell.pvtRole = 'pvt-data';
          }
        }
      }
    }
  ...
  // A drawing's place (picture or native chart), expressed as an OFFSET FROM A CELL
  // rather than from the top-left of the sheet. The renderer then positions it
  // inside that cell and never has to know the height of its own header row or the
  // width of its gutter, and an item stays put when a row above it is hidden.
  //
  // A cell covered by a merge is not drawn, so an item anchored inside one is
  // moved to the merge's anchor cell with the difference added to its offset.
  function placeDrawings(sheet, xl, rows, cols) {
    ...
    for (const d of drawings) {
      const isChart = d.kind === 'chart';
      const src = isChart ? null : xl?.media?.[d.part];
      if (!isChart && !src) continue;
      if (!drawn.has(d.from.row + 1)) continue;   // from.row is 0-based
      let row = d.from.row + 1, col = d.from.col;
      let dx = emuToPx(d.from.colOff), dy = emuToPx(d.from.rowOff);
      ...
      let w, h;
      if (d.to) {
        w = span(d.from.col, d.to.col) - emuToPx(d.from.colOff) + emuToPx(d.to.colOff);
        h = heightBetween(d.from.row + 1, d.to.row + 1) - emuToPx(d.from.rowOff) + emuToPx(d.to.rowOff);
      } else if (d.ext) {
        w = emuToPx(d.ext.cx); h = emuToPx(d.ext.cy);
      }
      if (!(w > 0) || !(h > 0)) continue;
      out.push({
        kind: isChart ? 'chart' : 'image',
        row, col, dx, dy, width: w, height: h,
        src,
        chart: d.chart || null,
        name: d.name || '',
      });
  ...
  const placeImages = placeDrawings;
  ...
  function connectionsView({ xl }) {
    return Object.keys(xl.sheets).sort().map(name => {
      ...
      return {
        ...
        // WHAT IS HERE BUT NOT ON SCREEN. A hidden sheet, row or column is
        // still data, and a reader auditing a workbook is usually looking for
        // exactly this; a value grid cannot show it by construction.
        Visibility: s.visibility ?? '',
        'Hidden Rows': (s.rows || []).filter(r => r.hidden).length,
        'Hidden Cols': hiddenColumnCount(s),
        Pivots: (s.pivotTables || []).length,
      };
    });
  }

  // THE ROWS BEHIND A PIVOT, as objects keyed by the cache's field names, which
  // is the shape sheetRows returns for a worksheet. Takes the cache PART, since
  // several pivot tables may share one cache and the records belong to the
  // cache rather than to any of them.
  //
  // Returns null when that cache declares no records part, which is a real
  // case: a workbook saved with "refresh on open" and no cached data has a
  // definition and nothing behind it.
  function pivotRecords(xl, cachePart, opts) {
    const cache = xl.pivotCaches?.[cachePart];
    if (!cache?.recordsPart) return null;
    const held = xl.pivotRecords[cache.recordsPart];
    if (!held) return null;
    const limit = opts?.limit;
    const rows = (limit ? held.rows.slice(0, limit) : held.rows).map(row =>
      Object.fromEntries(cache.fields.map((name, i) => [name || colLetter(i), row[i] ?? null])));
    return {
      rows,
      fields: cache.fields,
      // `total` is what the part actually contained and `declared` is what the
      // definition claims. The two disagreeing is a fact about the file worth
      // seeing rather than one to reconcile away.
      total: held.total,
      declared: cache.recordCount,
      truncated: held.truncated || (limit != null && held.rows.length > limit),
    };
  }

  // The last column any row of this sheet writes to, zero-based; -1 for empty.
  // A <col> range may run to 16384, so a hidden-column count has to be read
  // against what the sheet actually uses or it reports the spreadsheet's
  // maximum width as a finding.
  function usedWidth(sheet) {
    let max = -1;
    for (const r of sheet.rows || [])
      for (const k of Object.keys(r.cells || {})) max = Math.max(max, Number(k));
    return max;
  }

  function hiddenColumnCount(sheet) {
    const last = usedWidth(sheet);
    if (last < 0) return 0;
    let n = 0;
    for (const c of sheet.cols || []) {
      if (!c.hidden) continue;
      // <col min/max> is 1-based and inclusive; `last` is a zero-based index.
      const from = Math.max(1, c.min), to = Math.min(c.max, last + 1);
      if (to >= from) n += to - from + 1;
    }
    return n;
  }

  // ONE ROW PER EXTERNAL SOURCE. Deliberately not folded into connectionsView,
  // which despite its name is the per-sheet resource table: these are the
  // workbook's connections in the Excel sense, and the two answer different
  // questions.
  function sourcesView({ xl }) {
    return xl.connections.map(c => ({
      Id: c.id,
      Name: c.name,
      Kind: c.kind,
      Type: c.type,
      'Power Query': c.powerQuery ? 'yes' : '',
      Detail: c.detail,
      Description: c.description,
    }));
  }

  // One row per pivot table, joined to the cache it reads. `Records` and
  // `Cache Fields` are the cache's, so two tables over one cache repeat them,
  // which is the truth about the file rather than a duplicate.
  function pivotsView({ xl }) {
    return Object.entries(xl.pivotTables).map(([part, pt]) => {
      const cache = pt.cachePart ? xl.pivotCaches[pt.cachePart] : null;
      const sheet = pt.sheet ? xl.sheets[pt.sheet] : null;
      return {
        Pivot: pt.name,
        Sheet: sheet?.name || pt.sheet || '',
        Range: pt.ref,
        Rows: pt.rowFields,
        Cols: pt.colFields,
        Filters: pt.pageFields,
        Measures: pt.dataFields.join(', '),
        Source: cache?.source ? `${cache.source.sheet || cache.source.name}!${cache.source.ref}` : '',
        Records: cache?.recordCount ?? '',
        'Cache Fields': cache ? cache.fields.length : '',
        Part: part,
      };
    });
  }
  ...
  window.xlsxKit = {
    ...
    analyze,
    views: {
      paths: pathsView, connections: connectionsView, unconnected: unconnectedView, files: filesView,
      // `connections` is the per-sheet resource table and predates these two;
      // `sources` is the workbook's connections in the Excel sense.
      sources: sourcesView, pivots: pivotsView,
    },
    profileColumns,
    pivotRecords,
    sheetRows,
