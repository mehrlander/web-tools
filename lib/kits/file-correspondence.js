...
(() => {
  ...
  const decodeBytes = (bytes, encoding = 'auto') => {
    ...
    try {
      ...
      throw new Error('Could not decode comparison file as ' + chosen
        + '. Choose its encoding in Compare copy and try again. ' + (e?.message || e));
    }
  };
  ...
  const stageLocal = (text, name, previous) => {
    ...
    store.stageFocus = window.StageIntake.keyOf(local);
    return local;
  };
  ...
  const open = async ({ target, text, name = '', source = 'paste', record = true,
    ...
    if (record) {
      ...
      } catch (e) { throw retainedError(e, false); }
    }
    let sha, file;
    try {
    ...
    const recordValue = {
      ...
      incomingName, source, incomingSha256: await digest(text), content: text, exact,
  ...
  window.FileCorrespondence = { declaration, validPath, applies, same, decodeBytes,
    history, pending, open, retry, reopen };
})();
