...
(() => {
  ...
  function block(spec, o = {}) {
    ...
    if (!fill && lines > COLLAPSE_LINES) {
      ...
      const fade = h('div', { class: 'absolute inset-x-0 bottom-0 h-14 bg-gradient-to-t from-[var(--color-base-100)] to-transparent flex items-end justify-center pb-1.5' });
      ...
      if (open) {
        ...
        requestAnimationFrame(() => {
          if (pre.isConnected && pre.scrollHeight > pre.clientHeight)
            host.append(h('div', { class: 'pointer-events-none absolute inset-x-0 bottom-0 h-12 bg-gradient-to-t from-[var(--color-base-100)] to-transparent' }));
        });
      }
    };
  ...
  //   alternating transcript four other cues already say which is which
  ...
  //
  // EXPORTED, because a second surface needs the same mark. The session
  // outline's rows clamp their two lines with CSS rather than cutting the text,
  // so their count is measured rather than carried; what must not differ is the
  // mark itself, since a reader meets both within two taps. `title` is the one
  // thing that legitimately differs: there the chip IS the way to the rest, so
  // the sentence naming a longer route would be wrong.
  const moreChip = (n, tappable, title) => h('span', {
    class: 'inline whitespace-nowrap rounded px-1 bg-base-content/10'
         + ' font-mono text-[9.5px] text-base-content/50 not-prose',
    title: title || (n.toLocaleString() + ' more characters in this turn. '
  ...
  function message(m, o = {}) {
    ...
    // turn.
    //
    // The order that produced this is not noise: each cue was removed alone
    ...
    if (limit) requestAnimationFrame(() => {
      ...
      const fade = h('div', { class: 'absolute inset-x-0 bottom-0 h-16 bg-gradient-to-t from-[var(--color-base-200)]/90 to-transparent flex items-end justify-center pb-1' });
    ...
    if (o.tap) {
      // AND IT SAYS SO ON HOVER. A tappable turn carried only the cursor, which
      // is the one affordance a touch screen cannot show and a desktop reader
      // has to be over the turn to see. A tint on the turn's own box says which
      // turn a tap would take, which is the question a dense card of forty of
      // them raises. It is the neutral at /5, under the ask's own fill rather
      // than beside it: this theme's greys are cool, so anything stronger reads
      // as a second blue behind a blue turn, which is the wash this estate has
      // now removed twice (session-export's row band, and the one before it).
      //
      // base-200, not base-content at a low alpha. /5 is not a step Tailwind
      // generates, so it renders at FULL strength and the hover would have been
      // a solid black band; `npm run opacity-scan` is the gate that caught it
      // before this shipped. The ramp's own floor, /10, is heavier than a hover
      // wants behind a tinted ask, and base-200 is the theme's own next surface
      // up, which is what a hovered row is everywhere else in this estate.
      el.classList.add('cursor-pointer', 'rounded', 'transition-colors',
                       'hover:bg-base-200');
  ...
  window.chatRender = { ready, parse, exchanges, markdown, block, message, transcript, deck, openTranscript, blockViews, moreChip, ROLES };
})();
