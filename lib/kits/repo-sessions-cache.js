...
(() => {
  ...
  function buildMenuIndex(cache) {
    ...
    // The fourth field is the branch, and only `recent` needs it: in `branches`
    // the key already is one. A row's label leads with the branch slug, which
    // is the ask already reduced to a topic and short enough to read on a phone,
    // so without this the recent rows fall back to clipped prose.
    const entry = r => [r.id, lastAt(r), menuAsk(r.ask),
    ...
    for (const r of rows)
      for (const b of (r.branches || []))
        ...
        // Three fields here, four in `recent`: the key already names the
        // branch, so repeating it costs about a third of the file across 331
        // of them for nothing a reader gains.
        if (String(b).includes('/') && !(b in branches)) branches[b] = entry(r).slice(0, 3);
    return {
