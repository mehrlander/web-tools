...
(() => {
  ...
  // the startup helpers answer whether session start delivered what it was
  ...
  // 17 adds `at`, the turn's instant, carried whole by fullTurns below.
  // 18 adds `attached` (record schema 8), the container's repo list beside the
  // transcript's. Bumped rather than left, per the rule below: a row built by an
  // older summarizer will never see its record's sha move again, so the version
  // is the only thing that can heal it. It is 18 and not 17 because two
  // branches spent 17 at once, on `at` and on `attached`; a row cached by
  // either one is missing the other's field, and only a number past both
  // invalidates it.
  //
  // 19 adds `agents`, the fan-out weight, and is 19 for the reason 18 was 18: a
  // THIRD branch also spent 17, on this field, so a row cached by any of the
  // three is missing the other two's. It reads schema 8's `agents_total` where
  // there is one and falls back to the `tools` tally, which is present from
  // schema 4: the accurate field answers on 1 of the store's 341 records and
  // the tally on 30, so the union is the only reading that is both precise and
  // present. A row that says nothing and a row that says none are the same
  // pixel, which is the mistake bump 10 was already written about.
  //
  // 20 adds `agentCalls` and `agentsRefused`, which 19 needed and did not have.
  // 19 shipped only on an unmerged branch, so no row on main carries it, but a
  // preview link crawls like any other reader and a browser can hold one.
  //
  // 21 adds `attachments` and CHANGES `ask`, which is why it is a bump rather
  // than a display fix. See askParts below: an ask that opened with attached
  // files spent the whole ASK_CHARS budget on machine-written upload paths, so
  // the rows that need healing are ones whose `ask` is already wrong, and only
  // a re-summarize can put the question in it.
  //
  // 22 adds `beats`, the minute offsets of every user turn from the row's own
  // `started`. It is the first bump made FOR A DRAWING rather than for a fact:
  // the Activity list draws a rail per session across a shared span, and until
  // now the row could say only how long a session ran.
  //
  // An unhealed row is MARKED rather than left blank. A row at 21 still knows
  // when it ran, so the list draws its extent and dashes it: an empty rail
  // among full ones reads as "this session said nothing", where a dashed one
  // says the detail is missing and the note names the refresh that fills it.
  // Healing is four crawls at SESSIONS_MAX_FETCH records apiece.
  //
  // It is ONE field where the draft carried two. A companion `stateBeats` was
  // written, measured and cut before any of this shipped; beatsOf below records
  // the measurement, since the argument for it is good and the numbers are what
  // defeat it.
  const ROW_V = 22;
  ...
  // Agent dispatches, from the record's tool tally. `Task` is the dispatch
  // tool's older name, counted as a fallback rather than as a case anyone has
  // seen: all 341 records in the store today use `Agent` and none use `Task`,
  // so the term costs nothing and stops an imported older record from reading
  // as zero, which is a row that silently hides rather than one that is wrong.
  function agentsOf(r) {
    const t = r?.tools || {};
    return (t.Agent || 0) + (t.Task || 0);
  }
  ...
  // ── WHEN the session spoke, as minutes off its own start ─────────────────
  // The one field on the lean row that carries a session's INTERNAL shape. The
  // row has held `started`, `ended` and `mins` since the beginning, which draws
  // a session as a bar: it can say a session ran for nine hours and never that
  // eight of them were one burst at midnight and a single question at dawn.
  //
  // MINUTES, AND WHY THAT IS ENOUGH. The Activity list draws these against a
  // span the reader picks, the widest being 30 days across roughly 1,200
  // pixels, where one pixel is 36 minutes; the narrowest, 24 hours, still gives
  // 1.2 minutes to the pixel. So a minute is already finer than any span can
  // draw, and a second would cost the digits to render the same mark.
  //
  // OFFSETS, NOT INSTANTS, for the same reason: an ISO instant is 20 characters
  // and repeats the row's own `started` on every entry. Measured over the 389
  // records on file 2026-09-13, the 6,815 user turns in them cost 26 KB as
  // offsets against 154 KB as instants, on a cache file of 1.33 MB. So the
  // field is 2% of the file where the honest-looking version would be 11%.
  //
  // NEGATIVES ARE LEFT ALONE. `started` and the first prompt agree on 194 of
  // 225 records (see clock above), so a handful of sessions have a prompt
  // before their own recorded start. Clamping those to zero would invent a
  // burst at the origin; the renderer drops anything outside its window
  // anyway, which is the honest treatment of a stamp that disagrees.
  //
  // This is the PROSE CUT's counter-example and stays out of PROSE_KEYS
  // deliberately: `turns` left the row because the list never drew it and the
  // cards fetch the record. The list draws this on every visible row, so
  // fetching it per row is 298 record reads to paint one pane.
  const beatOf = (t0, at) => {
    const t = Date.parse(at || '');
    return Number.isFinite(t) && t0 ? Math.round((t - t0) / 60000) : null;
  };
  // One beat per USER TURN, which is what `exchanges` counts and what the
  // session card's rail already draws. A beat per assistant message would draw
  // the same conversation at twice the density and say nothing more about when
  // it happened, since a reply lands within a minute of the turn it answers.
  //
  // IT IS COUNTED OFF `prompts`, NOT OFF `exchanges`, and the two can disagree:
  // checked against all 389 records on file 2026-09-13, they agree on 388 and
  // one (fb30b17a) stores 16 prompts under an `exchanges` of 8, every prompt a
  // distinct question with its own stamp. The stored list is the one that can
  // be drawn, so the rail draws it and its tooltip counts the same list rather
  // than reaching for the counter beside it. Nothing here corrects
  // `exchanges`: that number is the recorder's and this file does not own it.
  function beatsOf(r) {
    const t0 = Date.parse(r?.started || '');
    if (!Number.isFinite(t0)) return [];
    return (r?.prompts || [])
      .map(p => beatOf(t0, p?.at))
      .filter(n => n !== null)
      .sort((a, b) => a - b);
  }
  // AND NOT THE CLOSING STATES, which this field was drafted carrying. The
  // case for them was that a state is a different KIND of event from a turn: it
  // is where a stretch of work came to rest, so a second channel on the rail
  // would show a reader scanning a week where things actually landed.
  //
  // Measured before drawing any, over all 389 records on file 2026-09-13:
  // 5,145 states against 6,815 user turns, 0.75 to the turn, and 132 records
  // carry MORE states than turns. So a state is not a sparse punctuation of the
  // conversation, it is very nearly the reply to each turn, and a second
  // channel at that density would double the marks on the rail to say "the
  // assistant answered", which the turn already implies. The rail's one rule is
  // that time and density are its only variables; a channel that tracks the
  // channel beside it spends a variable on nothing.
  //
  // The measurement is the durable part. A later session proposing state marks
  // on this rail should re-run it rather than re-argue it, and note that the
  // last state of all is at the session's own end, which the rail draws as the
  // right edge of the row's segment.
  ...
  // The two caps, and each answers a different question.
  //
  ...
  //
  // THERE IS NO THIRD CAP, and the one there was is worth the paragraph.
  // TURNS_KEPT bounded the scroll back at 60 entries, newest first, and
  // reported the drop as `turnsCut` so the card could say the front was gone.
  // The number was measured against a COMMITTED cost: `turns` rode in
  // state/sessions.json, so a 144-entry session paid for itself in every
  // reader's first fetch of the whole store.
  //
  // That cost went away on 2026-09-02, when leanRow started stripping every
  // PROSE_KEY on the way into the cache: all 345 rows on file carry no `turns`
  // at all, and the field is now rebuilt by summarize() from a record the card
  // has already read whole (estate.js, ensureProse). So the cap was charging
  // the reader for a saving nobody banks. Removed 2026-09-08. Measured over
  // the same 345 records: 32 of them (9%) were being cut, the worst is a
  // 206-turn session whose card grows from 9.6 KB to 32.1 KB, and the other
  // 313 are unchanged because they never reached 60. One card, in memory,
  // built from bytes already on the page.
  //
  // The other half of the cost is drawing them, since the card renders every
  // turn through chat-render on mount. Timed in Chromium against a 110-exchange
  // record: 0.1 ms a turn, so 220 turns spend 21-30 ms inside the renderer
  // against 5-11 ms for 62. The mount as a whole runs 70-180 ms either way,
  // dominated by the renderer's own load and Alpine rather than by the count.
  //
  // What is left of the honesty rule the cap owed: nothing to report, because
  // nothing is dropped. `turnsCut` went with it rather than staying as a field
  // that is structurally always empty. STATES_KEPT is a different cap on a
  // different field and still stands.
  const TURN_HEAD = 240;
  ...
  // nothing headed. priorTurns is this list headed, entry for entry, so entry i
  // of `row.turns` is entry i of fullTurns(record) and a card turn can be
  // traded for its full text by index rather than matched on a timestamp.
  // They were only the same length from the tail while the cap stood; now they
  // are the same list, which is what makes the index a safe key.
  //
  ...
  function priorTurns(r) {
    ...
    // was. Absent rather than zero, because this list runs to a couple of
    // hundred entries on a long session and a zero on every whole turn is pure
    // weight.
    return fullTurns(r)
      .map(e => {
  ...
  // resolved here.
  //
  // The tuple is positional and grew from two to four. `via` names the CHANNEL
  ...
  // many bytes of that document actually went onto the channel. It is
  ...
  // ── An attached file, as the prompt carries it ─────────────────────────────
  //
  // Claude Code writes one `@"<absolute upload path>"` at the head of the
  // prompt per file the person attached, so a prompt opening with three
  // attachments opens with about 300 characters nobody typed. `ask` is capped
  // at ASK_CHARS, and the cap was landing inside those paths: measured
  // 2026-09-10 across the 372 rows on file, 7 asks open with an upload mention
  // and on 3 of them the budget was spent before the question began. Row
  // d657f526 is the worked case. Its record holds 444 characters and the
  // question starts at 301 ("Please review the attached documents with the drs
  // budget app submittal view"); its row held 240 characters of UUID and did
  // not hold the question at all. The row drew two clamped lines of path, the
  // phone menu's ask line drew the same, and the search box added those UUIDs
  // to the corpus in place of what was asked.
  //
  // SO THIS IS A CACHE TRANSFORM, and it is the exception the ask's markdown
  // stripper argues against one screen down. That one is a DISPLAY transform on
  // the stated ground that stripping in `summarize` would cost every consumer
  // the ability to search for what was typed. The ground does not hold here,
  // twice over: nobody typed these paths, and stripping at render time cannot
  // reach the three rows whose 240 characters are paths and nothing else, since
  // there is no question left in them to reveal. The RECORD is untouched, as
  // always, so EstateSearch.sessions still greps the prompt exactly as sent.
  //
  // ANCHORED ON THE UPLOADS DIRECTORY, never on the `@` sigil, which is the
  // distinction that makes this safe to run over every ask: `@docs/SURFACING.md`
  // is a reference somebody typed on purpose and must stay in the ask and in
  // the search corpus, while `/root/.claude/uploads/<session>/<hash>-<name>` is
  // a path the harness wrote and no one would ever search for. The bare form is
  // read beside the quoted one because only the quoted one has been observed
  // and a path with no spaces has no need of quotes.
  const UPLOAD_MENTION = /@(?:"([^"]*\/\.claude\/uploads\/[^"]*)"|(\S*\/\.claude\/uploads\/\S*))/g;

  // What survives of the path: the last segment below the uploads directory,
  // minus the eight hex characters the harness prefixes so two uploads of one
  // name cannot collide.
  //
  // THE FLOOR IS STRUCTURAL, and it is what makes failing open mean anything.
  // The harness writes `<uploads>/<session id>/<hash>-<name>`, so a path with
  // nothing beneath the session directory is a shape this does not know and is
  // left where it stands. Taking "the last segment" on its own does not fail
  // open at all: it reads `@"…/uploads/"` as an attachment called `uploads` and
  // deletes the mention, which is the worse of the two wrong answers.
  //
  // Requiring exactly eight lowercase hex AND a hyphen for the prefix is what
  // leaves a real name intact: `0e001840-02.02DecisionPackageReduction.docx`
  // loses the prefix and keeps the `02.02`, because the test is the shape of
  // the prefix rather than "leading digits". A genuine `deadbeef-report.docx`
  // would lose its head, a cosmetic loss on a name nobody has yet uploaded.
  const UPLOADS_DIR = '/.claude/uploads/';
  const UPLOAD_PREFIX = /^[0-9a-f]{8}-/;
  function attachName(path) {
    const p = String(path || '');
    const at = p.lastIndexOf(UPLOADS_DIR);
    if (at < 0) return '';
    const segs = p.slice(at + UPLOADS_DIR.length).split('/').filter(Boolean);
    if (segs.length < 2) return '';
    return segs[segs.length - 1].replace(UPLOAD_PREFIX, '');
  }

  // Names kept per row. The observed maximum is 4 and this is not a budget
  // anyone is near; it is here so a paste of forty files cannot put forty
  // strings on a row that every pane reads on mount.
  const ATTACH_KEPT = 12;

  // The ask with its attachments lifted off it. Returns the prose and the
  // names, and rewrites nothing else: whitespace inside what was typed is left
  // exactly as typed, and only the ends are trimmed, since every observed
  // mention sits at the head of the prompt and leaves its gap there.
  //
  // FAIL OPEN. A mention whose path yields no filename is left where it stands,
  // so an upload shape this has not seen degrades to the old behaviour (the
  // path in the ask) rather than to a silently emptied one.
  function askParts(text) {
    const files = [];
    const ask = String(text || '').replace(UPLOAD_MENTION, (whole, quoted, bare) => {
      const name = attachName(quoted || bare);
  ...
  function summarize(rec, sha) {
    const r = rec || {};
    // Before the cap, deliberately: see askParts for what the cap was spending
    // itself on when it ran over the prompt as sent.
    const opening = askParts(r.opening_ask);
    ...
    return {
      ...
      ask: opening.ask.slice(0, ASK_CHARS),
      // The files the prompt arrived with, by name. Present only where there
      // were some, on `title`'s contract rather than on the rest of the row's:
      // 365 of 372 rows would otherwise carry an empty array saying nothing,
      // and an absent key reads as "none attached" without a reader having to
      // know the difference between that and "not captured" (every record that
      // can carry a prompt can carry this, so there is no second case).
      ...(opening.files.length ? { attachments: opening.files.slice(0, ATTACH_KEPT) } : {}),
      ...
      // turns, all of them, each cut to its opening sentence.
      turns: priorTurns(r),
      // WHEN it spoke, and where it came to rest. See beatsOf: minute offsets
      // from `started`, the one shape-of-the-session field the lean row keeps,
      // because the Activity list draws it on every visible row.
      beats: beatsOf(r),
      repos,
      // SCOPE, beside where the work happened. Record schema 8: the checkouts
      // the session HAD, names only, read off the container rather than the
      // transcript. `repos` above is narrower and says so in the recorder's own
      // docstring, since an absolute-path Read and `git -C` never move the cwd.
      //
      // Carried as [] for every record written before schema 8, and a consumer
      // must read the empty as "this record cannot say" rather than as "nothing
      // was attached". Records are never revisited, so those stay empty
      // permanently and `repos` is the only answer they will ever have; the
      // estate falls back to it rather than showing a session no scope at all.
      attached: Array.isArray(r.attached) ? r.attached.slice() : [],
      ...
      // Agents dispatched: the OTHER body of work a session did, which on a
      // fan-out dwarfs its own. `calls` above counts what this session ran;
      // this counts the runs it started, whose own calls and returns are not
      // in any figure beside it. It is what makes a fan-out findable in a list
      // that otherwise shows it as an ordinary session: the store's largest run
      // reads as 8 asks and 269 calls without it.
      //
      // SCHEMA 8 FIRST, WITH THE TALLY BEHIND IT, which is a union rather than
      // a choice: `agents_total` is the accurate count and exists on 1 of the
      // store's 341 records, the tally is approximate and answers on 30, so
      // leading with the accurate one loses no row and gains precision where
      // there is any. Reading only `agents_total` would draw one row and leave
      // 29 fan-outs indistinguishable from sessions that ran none; reading only
      // the tally would count attempts on the one record that can do better.
      //
      // They differ by the refusals. On that record the tally is 126 and
      // `agents_total` is 124, and record.py accounts for every one: 126
      // attempted, 2 refused at the concurrency cap (`agents_refused`), 124
      // launched (`agents_dispatched`), 124 captured. `agents_unaccounted` is
      // 0 and correct, since it reconciles against the launches rather than
      // the attempts. A refused launch did no work, so what ran is the truer
      // weight.
      //
      // Kept out of `tools` even though the fallback reads it from there:
      // `tools` keeps only the busiest six, so a one-agent session drops out of
      // it while a 126-agent session does not, and a figure that appears only
      // above a threshold is worse than no figure.
      agents: r.agents_total || agentsOf(r) || 0,
      // The tally on its own, because the tools card states its note about the
      // CALL LIST and the call list counts attempts. Above they are the same
      // number on every record but the one, and that record is the reason the
      // note may not reach for `agents`.
      agentCalls: agentsOf(r),
      tokens: r.tokens || null,
      // Agent spend, the refusals and the silent drop: all schema 8, none
      // derivable from the tally. Refusals and unaccounted runs are counted
      // apart because one is visible in the tool result and the other is not,
      // which record.py makes the case for. Spend stays a separate field for
      // the reason the record keeps it separate, that one total cannot be
      // split back apart afterwards.
      agentTokens: r.agent_tokens || null,
      agentsRefused: r.agents_refused || 0,
  ...
  const PROSE_KEYS = ['turns', 'states', 'statesCut', 'reply', 'replyAt', 'replyCut'];
  ...
  //
  // The Sessions pane's text box matches on this, and it reaches exactly as far
  // as the row does, which is the one thing a consumer has to be able to state.
  // THE PROSE LEFT THE ROW on 2026-09-02 (see PROSE_KEYS above), so a phrase
  // said in the middle of a session is not here and no filter over these rows
  // can find it. What is here is the opening ask at ASK_CHARS, the two names,
  // and the work's own vocabulary: repos, branches, files, docs, skills, tools.
  // The exhaustive pass is EstateSearch.sessions, which opens every record and
  // costs the whole store; the pane routes to it rather than pretending to it.
  //
  // LABELLED, like nameSegs in kits/estate-search.js and searchSegs in
  // kits/chat-archive.js: one box searches a dozen fields, so the matched
  // segment can become a hit's note line and say which field answered.
  //
  // A PATH IS A SEGMENT OF ITS OWN, never folded into a run of paths, because
  // the terms-are-ANDed rule below is applied across the joined text: two
  // terms that each match a different file are a real answer here, since the
  // question is about the SESSION rather than about one path.
  function searchSegs(row) {
    const r = row || {};
    const segs = [];
    const push = (label, text) => { if (text) segs.push(label + ': ' + text); };
    push('title', r.title);
    const name = nameOf(r);
    if (name) {
      push('name', name);
      // The hyphenated slug as it is stored AND as it is remembered. Same
      // courtesy nameSegs pays: "fab naming" has to find "fab-naming".
      if (name.includes('-')) push('name', name.replace(/-/g, ' '));
    }
    push('ask', r.ask);
    // The attached files, by name, and kept apart from the ask for the reason
    // askParts lifted them out of it: "COREPAM_Decision_Package.docx" is worth
    // finding a session by and the UUID directory above it is not.
    for (const a of r.attachments || []) push('attachment', a);
    push('id', r.id);
    push('day', r.day);
    for (const x of r.repos || []) {
      push('repo', x && x.name);
      push('branch', x && x.branch);
    }
    for (const nm of r.attached || []) push('attached', nm);
    for (const b of r.branches || []) push('branch', b);
    for (const f of r.files || []) push('file', Array.isArray(f) ? f[0] : f);
    for (const f of r.docFiles || []) push('doc', Array.isArray(f) ? f[0] : f);
    for (const g of r.guides || []) push('guide', Array.isArray(g) ? g[0] : g);
    for (const k of r.skillCalls || []) push('skill', Array.isArray(k) ? k[0] : k);
    for (const t of r.tools || []) push('tool', Array.isArray(t) ? t[0] : t);
    return segs;
  }

  // Every term present somewhere in the row. AND across terms, OR across
  // fields, which is what a person types: "home merge guide" means all three
  // words are about this session, not that one field holds the phrase.
  ...
  //
  // A SECOND, PURPOSE-BUILT FILE, not a view of the one above. The phone's
  // Claude menu (lib/ops/session-menu.js, run after a double back tap) needs
  // three facts about a session and nothing else: its id, when it was last
  // active, and one line of its ask. Reading the full cache to get them cost
  // 1.16 MB over cellular, and then a filter and a sort on a phone, to draw at
  // most thirteen rows. This is 46 KB, 16 KB on the wire, and answers a branch
  // with one dictionary lookup.
  //
  // KEYED BY BRANCH, because one branch is one session. Measured over the 341
  // rows on file 2026-09-08: 331 distinct `claude/…` branches, 331 sessions,
  // none with two. The harness mints a branch per session, so a map loses
  // nothing a list would keep, and `buildCache`'s ROW_CAP bounds this file's
  // growth for free.
  //
  // Written by the same crawl that writes the cache and stamped with the same
  // `generatedAt`, so the two can never disagree about their own freshness.
  // The op shows that stamp whenever a branch matched nothing, which is the one
  // case where a reader cannot otherwise tell "no such session" from "the crawl
  // has not run since this one started".
  const MENU_PATH = 'state/session-menu.json';
  const MENU_ASK = 56;      // characters of the ask a phone row carries
  const MENU_RECENT = 16;   // recent entries kept, enough to fill a 12-row menu after dedupe

  // An ask is markdown and a menu row is not: fences, link brackets and URLs,
  // emphasis and heading marks go. Done here rather than on the phone so the
  // op receives a string it can put straight on a row.
  function menuAsk(md) {
    const s = String(md || '').replace(/```[\s\S]*?```/g, ' ')
      .replace(/\[([^\]]*)\]\([^)]*\)/g, '$1').replace(/[*_`#>]+/g, '')
      .replace(/\s+/g, ' ').trim();
    return s.length > MENU_ASK ? s.slice(0, MENU_ASK - 1) + '\u2026' : s;
  }

  function buildMenuIndex(cache) {
    const rows = [...(cache?.rows || [])].sort((a, b) => lastAt(b).localeCompare(lastAt(a)));
    // FIELD 3 IS THE claude.ai SESSION ID, and it is the field that lets the
    // back tap reach the conversation rather than a page about it. The whole
    // join already existed and stopped one hop short: the record carries
    // `agent_session`, the cache row carries it as `agent`, and session.html
    // renders an "Open this session in Claude Code" button from it, so the
    // menu was the only surface that had to spend a page load and two taps on
    // an address the index could have handed over. Measured 2026-09-19 on the
    // live store: 384 of 400 rows carry it, and all 13 menu branches missing it
    // are from 2026-08 and an older schema, so the op offers the row only where
    // the field is there rather than promising an address it may not have.
    //
    // It costs about 34 bytes a branch, near 14 KB on a 57 KB payload. The bare
    // id is stored whole rather than with its constant `session_` prefix
    // stripped: three of those kilobytes are not worth a stored value that is
    // no longer the identifier it claims to be.
    //
    // The branch moves to field 4 and only `recent` needs it: in `branches` the
    // key already is one. A row's label leads with the branch slug, which is
    // the ask already reduced to a topic and short enough to read on a phone,
    // so without that field the recent rows fall back to clipped prose.
    const entry = r => [r.id, lastAt(r), menuAsk(r.ask), sessionIdOf(r.agent),
                        (r.branches || []).find(b => String(b).includes('/')) || ''];
    const branches = {};
    for (const r of rows)
      for (const b of (r.branches || []))
        // A slash is what the op's branchOf requires before it will call a
        // string a branch, so a name without one could never be looked up here
        // and carrying it is dead weight. `branches` already excludes `main`.
        // Four fields here, five in `recent`: the key already names the branch,
        // so repeating it costs about a third of the file across 381 of them
        // for nothing a reader gains.
        if (String(b).includes('/') && !(b in branches)) branches[b] = entry(r).slice(0, 4);
    return {
      generatedAt: cache?.generatedAt || '',
      recent: rows.slice(0, MENU_RECENT).map(entry),
      branches,
    };
  }
  // Same gate as the cache's, and for the same reason: the crawl stamps
  // `generatedAt` on every pass whether or not anything moved, so comparing
  // whole documents would commit on every crawl.
  function menuChanged(prev, next) {
    const bare = d => JSON.stringify({ recent: d?.recent || [], branches: d?.branches || {} });
    return bare(prev) !== bare(next);
  }
  ...
  window.RepoSessionsCache = {
    ...
    priorTurns, fullTurns, closingAt, head, IMAGE_ONLY,
    buildCache, pathOf, nameOf, labelOf, searchSegs, matches, pointerOf, stalePaths, material,
    askParts, attachName, ATTACH_KEPT,
    cacheChanged, isRecordPath,
    MENU_PATH, MENU_ASK, MENU_RECENT, menuAsk, buildMenuIndex, menuChanged,
