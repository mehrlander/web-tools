...
(() => {
  ...
  // three is missing the other two's. It is read off the record's own `tools`
  // tally rather than off schema 8's `agents_total`, and the choice matters:
  // the tally is present from schema 4, so 30 of the store's 341 records answer
  // today, while `agents_total` exists on the handful written since 2026-09-08
  // and would render every older fan-out as zero. A row that says nothing and a
  // row that says none are the same pixel, which is the mistake bump 10 was
  ...
  function summarize(rec, sha) {
    ...
    return {
      ...
      //
      // READ FROM THE TALLY FIRST, with schema 8's `agents_total` behind it,
      // and the order is the whole point. `agents_total` counts the subagent
      // transcripts the recorder FOUND and exists on 1 of the store's 341
      // records; the tally counts the dispatches this session MADE and answers
      // on 30. Leading with the newer field would draw one row and leave 29
      // fan-outs indistinguishable from sessions that ran none. On the single
      // record carrying both they disagree, 124 found against 126 dispatched,
      // with `agents_unaccounted` at 0: two runs left no transcript and were
      // not flagged, so the found count is not a dispatch count and cannot be
      // relabelled into one.
      //
      // Kept out of `tools` even though that is where it is read from: `tools`
      // keeps only the busiest six, so a one-agent session drops out of it
      // while a 126-agent session does not, and a figure that appears only
      // above a threshold is worse than no figure.
      agents: agentsOf(r) || r.agents_total || 0,
      tokens: r.tokens || null,
      // Agent spend and the silent drop, both schema 8 and neither derivable
      // from the tally. Spend stays a separate field for the reason the record
      // keeps it separate, that one total cannot be split back apart
      // afterwards.
