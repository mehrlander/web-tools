...
(() => {
  ...
  //
  // 17 adds `agents`, the fan-out weight. It is read off the record's own
  // `tools` tally rather than off schema 8's `agents_total`, and the choice
  // matters: the tally is present from schema 4, so 27 of the store's 341
  // records answer today, while `agents_total` exists on the handful written
  // since 2026-09-08 and would render every older fan-out as zero. A row that
  // says nothing and a row that says none are the same pixel, which is the
  // mistake bump 10 was already written about.
  const ROW_V = 17;
  ...
  function summarize(rec, sha) {
    ...
    return {
      ...
      // in any figure beside it. Dispatches rather than completions, since
      // the tally cannot tell a refused one apart; the card carries the rest.
      //
      // Kept out of `tools` deliberately even though that is where it is read
      // from: `tools` keeps only the busiest six, so a one-agent session drops
      // out of it while a 126-agent session does not, and a figure that
      // appears only above a threshold is worse than no figure.
      agents: agentsOf(r),
