...
// projects/text/: passages.jsonl, one line per distinct passage {id, text};
...
// path, commit}. from and to are passage ids everywhere. This kit reads the
...
//   TextCollection.chain(index, passageId)              -> recorded predecessors, matched passage first
//
...
// its text. A proposal id is the sha256 of the Python json.dumps of {from, to,
...
(() => {
  ...
  const PATHS = { passages: 'projects/text/passages.jsonl', proposals: 'projects/text/proposals.jsonl', revisions: 'projects/text/revisions.jsonl' };
  ...
  // ── Build ─────────────────────────────────────────────────────────────────
  // `files` carries the three JSONL texts under `passages`, `proposals`, and
  ...
  async function build({ files, metadata = {}, repo = '', ref = 'main' }) {
    ...
    for (const row of parseJsonl(files.proposals, PATHS.proposals)) {
      ...
      if (!(from in passages) || !(to in passages)) throw new Error(`${PATHS.proposals} names a passage the collection does not hold`);
      if (from === to) throw new Error(`${PATHS.proposals} proposes a passage as its own replacement`);
    ...
    for (const row of parseJsonl(files.revisions, PATHS.revisions)) {
      ...
      if (!(revision.from in passages) || !(revision.to in passages)) throw new Error(`${PATHS.revisions} names a passage the collection does not hold`);
      if (revision.from === revision.to) throw new Error(`${PATHS.revisions} revises a passage into itself`);
  ...
  function view(index, proposalOrId) {
    ...
    return {
      id: proposal.id,
      from: { passage_id: proposal.from, text: index.passages[proposal.from] },
      to: { passage_id: proposal.to, text: index.passages[proposal.to] },
  ...
  function lookup(index, value) {
    ...
    const exact = index.proposals.filter(row => index.passages[row.from] === text).map(row => view(index, row));
    return { selection: { text }, exact, warnings: exact.length ? [SCOPE_WARNING] : [] };
  }

  // Recorded predecessors, starting with the matched passage: each step is a passage, the revision
  ...
  // and the rest are listed on the step. File order does not prove chronology
  // or the history of an occurrence at a particular repo/path. Bounded and cycle-safe.
  function chain(index, passageId, { limit = 20 } = {}) {
    const into = new Map();
    ...
    let id = passageId;
    let by = null;
    while (id && id in index.passages && !seen.has(id) && steps.length < limit) {
      seen.add(id);
      const arrivals = into.get(id) || [];
      steps.push({
        passage_id: id,
        text: index.passages[id],
        revision: by,
