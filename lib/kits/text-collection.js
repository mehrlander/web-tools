...
//
// The collection is two committed JSONL files in mehrlander/home,
// projects/text/: passages.jsonl, one line per distinct passage {id, text},
// and proposals.jsonl, one line per proposed edit {from, to, author, purpose}.
// from and to are passage ids everywhere. This kit reads the two files as they
...
// it from git and consults this kit only for the proposals made against each
// earlier text. (A third file, revisions.jsonl, carried eight git-derived
// edges from 2026-09-22 until the same day's History rebuild retired it.)
//
...
(() => {
  ...
  const PATHS = { passages: 'projects/text/passages.jsonl', proposals: 'projects/text/proposals.jsonl' };
  ...
  // ── Build ─────────────────────────────────────────────────────────────────
  // `files` carries the two JSONL texts under `passages` and `proposals`;
  // `metadata` the contents-API facts for each.
