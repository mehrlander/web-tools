...
//   MdSurface.edits / reject / patch the edit as blocks to reject, and as a patch
//
...
(() => {
  ...
  // ── The edit as a whole: its blocks, rejecting one, and the patch ─────────
  // EDITS are block-level, the unit a reader rejects: a paragraph rewritten, an
  // item added, a heading removed. Blocks come from mdDiff.blocks (a fence kept
  // whole), compared by jsdiff's array diff over whitespace-normalised text, and
  // each edit carries the source range it spans on each side, INCLUDING the
  // separator up to the next block, so putting the old range back leaves the
  // blank lines between blocks exactly as they were. Needs window.mdDiff and
  // window.Diff; without either there are no edits to list.
  const edits = (base, text) => {
    const out = [];
    if (!window.Diff || !window.mdDiff || base == null || base === text) return out;
    const A = window.mdDiff.blocks(base), B = window.mdDiff.blocks(text);
    const norm = (x) => x.text.replace(/\s+/g, ' ').trim();
    const edge = (list, i, whole) => (i < list.length ? list[i].start : whole.length);
    let ai = 0, bi = 0, pend = null;
    const flush = () => {
      if (!pend) return;
      const o = [edge(A, pend.a0, base), edge(A, ai, base)], n = [edge(B, pend.b0, text), edge(B, bi, text)];
      out.push({ kind: o[0] === o[1] ? 'added' : n[0] === n[1] ? 'removed' : 'changed',
                 old: o, new: n, oldText: base.slice(o[0], o[1]).trim(), newText: text.slice(n[0], n[1]).trim() });
      pend = null;
    };
    for (const part of window.Diff.diffArrays(A.map(norm), B.map(norm))) {
      const k = part.value.length;
      if (part.added || part.removed) { pend ||= { a0: ai, b0: bi }; if (part.added) bi += k; else ai += k; }
      else { flush(); ai += k; bi += k; }
    }
    flush();
    // Whitespace-only differences normalise away above, so a text that differs
    // only there has no edit to list, which is what the marks say about it too.
    return out;
  };
  // The buffer with one edit taken back: the new side's range swapped for the
  // old side's text. Later edits' ranges shift, so a caller recomputes.
  const reject = (base, text, e) => text.slice(0, e.new[0]) + base.slice(e.old[0], e.old[1]) + text.slice(e.new[1]);
  ...
  window.MdSurface = { paint, offsetAt, hitsText, tidy, backspace, enter, lineAt, changes, removals,
                       edits, reject, patch, _pointAt: pointAt };
})();
