...
(() => {
  ...
  //
  // A card has two readings, and only the first takes the caret:
  //
  ...
  //   old      the base's text for the whole run, read-only, labelled.
  //
  // Which card shows `old` is host.__readings[i], set by the caller, and it is
  // forgotten whenever the text changes: a reading is a look back, and the
  // next edit, or a reject, is the reader returning to the document. So no
  // card is ever left showing a copy the caret cannot enter.
  //
  // Cards carry data-md-card=<i>; the number is a button with
  // data-md-card-badge, and every control is data-md-ui so a host's gestures
  // can tell a control from the text. host.__cards[i] is that card's entries.
  ...
  const renderTracked = (host, text, base) => {
    ...
    for (const r of runs) {
      ...
      if (old) {
        ...
        tag.className = PILL + ' right-1 px-1.5 py-0.5 opacity-70';
        tag.textContent = 'original';
        tag.dataset.mdUi = '';
        wrap.appendChild(tag);
        const body = doc.createElement('div');
        body.innerHTML = window.mdDoc.html(r.map((e) => e.old).filter(Boolean).join('\n\n'), { proseClass: '' });
        ...
        const flow = wrap.appendChild(doc.createElement('div'));
        const bodies = r.filter((e) => e.newRange).map((e) => {
          const body = flow.appendChild(doc.createElement('div'));
          body.innerHTML = window.mdDoc.html(text.slice(e.newRange.start, e.newRange.end), { proseClass: '' });
          return [body, e];
        });
        const was = r.filter((e) => e.oldRange).sort((x, y) => x.oldRange.start - y.oldRange.start).map((e) => e.old).join('\n\n');
        try { window.mdDiff.mark(flow, window.mdDiff.runs(was ? window.mdDiff._textOf(was) : '', flow.textContent)); } catch {}
        for (const [body, e] of bodies) map(body, text, e.newRange.start, 'del, [data-md-ui]');
      }
      host.appendChild(wrap);
    }
    host.__cards = cards;
  ...
  const setReading = (host, i, mode) => {
    if ((host.__cards || [])[i]) (host.__readings ||= {})[i] = mode;
  };
  const readingOf = (host, i) => ((host.__readings || {})[i] === 'old' ? 'old' : 'inline');
  const cards = (host) => host.__cards || [];
  window.MdSurface = { paint, offsetAt, hitsText, step, rectAt: caretRect, tidy, backspace, enter, lineAt, changes, removals, setReading, readingOf, cards,
