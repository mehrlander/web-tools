...
(() => {
  ...
  // With a base, each block that differs from it is drawn as a CARD in the
  // flow: its reading of the change, a number, and three stops. Unchanged
  // blocks render as ordinary text. Every reading but `old` is the buffer and
  // takes the caret; `old` is the base, shown and never mapped.
  //
  //   inline   the new text with the change laid over it: removed words
  //            struck in place (a <del>, never mapped, so the caret skips it)
  //            and added words washed green
  //   new      the new text, clean
  //   old      the base's text, read-only
  //
  // A removed block has one reading, its old text over a red wash. Which
  // reading each card shows is host.__readings[i], set by the caller; the
  // default is inline. Cards carry data-md-card=<i>; the number is a button
  // with data-md-card-badge, the stops data-md-card-read="<i>:<mode>", and all
  // of it data-md-ui so a host's gestures can tell a control from the text.
  const CARD = 'relative my-3 rounded-[4px] border border-base-300 bg-base-200/30 px-2 pt-3 pb-1';
  ...
  const MODES = ['old', 'inline', 'new'];
  // A card's reading is remembered by where its block sits in the BASE, which
  // editing never moves, so typing that adds or removes a card elsewhere does
  // not hand one card's reading to its neighbour. An added block has no base
  // position and falls back to its number.
  const cardKey = (e, i) => (e.oldRange ? 'o' + e.oldRange.start : 'a' + i);
  const renderTracked = (host, text, base) => {
    ...
    for (const e of window.mdDiff.align(base, text)) {
      ...
      if (e.kind === 'same') {
        wrap.innerHTML = window.mdDoc.html(text.slice(e.newRange.start, e.newRange.end), { proseClass: '' });
        map(wrap, text, e.newRange.start);
        host.appendChild(wrap);
        continue;
      }
      const i = cards.length;
      cards.push(e);
      const modes = e.kind === 'removed' ? ['gone'] : e.kind === 'added' ? ['inline'] : MODES;
      const want = readings[cardKey(e, i)];
      const mode = modes.includes(want) ? want : modes.includes('inline') ? 'inline' : modes[0];
      wrap.className = CARD;
      ...
      if (modes.length > 1) {
        ...
        for (const m of modes) {
          ...
          b.className = 'rounded-full px-1.5 py-0.5 cursor-pointer ' + (m === mode ? 'bg-base-300 font-medium' : 'opacity-60');
          b.textContent = m;
      ...
      const nt = e.newRange ? text.slice(e.newRange.start, e.newRange.end) : '';
      if (mode === 'gone' || mode === 'old') {
        body.innerHTML = window.mdDoc.html(e.old, { proseClass: '' });
        body.dataset.mdGhost = '';
        body.className = mode === 'gone' ? 'bg-error/15 rounded-[3px] opacity-80' : 'opacity-70';
      } else {
        body.innerHTML = window.mdDoc.html(nt, { proseClass: '' });
        if (mode === 'inline') {
          try { window.mdDiff.mark(body, window.mdDiff.runs(e.old ? window.mdDiff._textOf(e.old) : '', body.textContent)); } catch {}
        }
        map(body, text, e.newRange.start, 'del, [data-md-ui]');
      }
  ...
  // Which reading card i shows; the next paint redraws it.
  const setReading = (host, i, mode) => {
    const e = (host.__cards || [])[i];
    if (e) (host.__readings ||= {})[cardKey(e, i)] = mode;
  };
  const readingOf = (host, i) => {
    const e = (host.__cards || [])[i];
    return (e && (host.__readings || {})[cardKey(e, i)]) || (e && e.kind === 'removed' ? 'gone' : 'inline');
  };
  const cards = (host) => host.__cards || [];
  window.MdSurface = { paint, offsetAt, hitsText, tidy, backspace, enter, lineAt, changes, removals, setReading, readingOf, cards,
