...
(() => {
  ...
  //
  // ONE THING THE SEAM LOSES, and it is worth stating rather than working
  // around here: pvTree sorts each node's children by value descending, so the
  // blocks arrive in magnitude order and the order an analyst wrote the rows in
  // is already gone by the time this sees them. A long-format rows file read
  // through build() keeps its authored order; a pivot read through this one
  // cannot. Restoring it would mean changing what the Pivot view itself draws,
  // which is a decision for that view rather than for its reader.
  function fromPartition(res, opts) {
    ...
    const leaves = (node, path) => {
      ...
      for (const kid of node.children || []) leaves(kid, [...path, kid.label]);
    };
    for (const top of res.tree.children || []) {
