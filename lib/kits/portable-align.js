...
// the roster live in the Map and Repos views (alpineComponents/map.js and
// alpineComponents/estate.js), the
...
(() => {
  ...
  // Does the repo's CLAUDE.md wire the conventions in (the current default
  // skill, a legacy import/invocation, or the plugin itself)? Text heuristic on
  // purpose: presence of intent, not proof of invocation. The retired spelling
  // remains a signal because this assessment is also how a stale consumer is
  // found rather than silently classified as unwired.
  function conventionsWired(claudeMd) {
    return /CONVENTIONS\.md|\/portable:default\b|\/web-tools\b|portable@web-tools|web-tools skill/i.test(claudeMd || '');
  }
