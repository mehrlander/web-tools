...
(() => {
  ...
  function assemble({ repo, branch, base, compare, noBase, pull, pulls, authored, facts, prFallback } = {}) {
    ...
    return {
      repo, branch, base,
      // Where the branch last agreed with its base: the commit GitHub's
      // three-dot compare diffs from, so the file list above is exactly this
      // commit against the tip. A card comparing against it shows the same
      // change the list names, before the merge and, for a squash merge, after
      // it; comparing against the base's tip instead reads a squash-merged
      // branch as unchanged. Empty until the compare is read.
      mergeBase: cmp?.merge_base_commit?.sha || (prFallback ? (shown?.base?.sha || '') : ''),
      ...
      state: prFallback ? (shown && !shown.merged_at && shown.state === 'closed' ? 'unrelated' : 'landed')
           : (cmp ? state(cmp)
           : (noBase ? 'unrelated'
           : (f.ahead == null ? '' : state({ ahead_by: f.ahead })))),
      ahead: prFallback ? 0 : (cmp?.ahead_by ?? f.ahead ?? null),
      behind: prFallback ? 0 : (cmp?.behind_by ?? f.behind ?? null),
  ...
  async function fetchCompare(gh, { branch, base, pull } = {}) {
    try {
      const res = await gh.compare(base, branch);
      if (res && res.ahead_by === 0 && pull?.base?.sha && pull?.head?.sha && (pull.merged_at || pull.state === 'closed')) {
        try {
          const fallback = await gh.compare(pull.base.sha, pull.head.sha);
          if (fallback && ((fallback.total_commits || 0) > 0 || (fallback.files?.length || 0) > 0)) {
            return { compare: fallback, noBase: false, prFallback: true };
          }
        } catch {}
      }
      return { compare: res, noBase: false };
    } catch (err) {
      if (err && err.status === 404) {
        ...
        return { compare: null, noBase: true };
      }
      throw err;
    }
  }
  ...
  //
  // When a branch has merged or its ref was pruned on GitHub, compare(base, branch)
  // yields either 0 commits ahead or 404. If the branch has an associated merged
  // or closed PR with boundary SHAs, we fall back to compare(pull.base.sha, pull.head.sha)
  // so the historical diff and commit list remain intact.
  async function fetchBrief(gh, o = {}) {
    const [cmp, guide] = await Promise.all([fetchCompare(gh, o), fetchGuide(gh, o)]);
    const pull = o.pull || guide.pull;
    if ((cmp.noBase || cmp.compare?.ahead_by === 0) && pull?.base?.sha && pull?.head?.sha) {
      if (pull.merged_at || pull.state === 'closed' || cmp.noBase) {
        try {
          const fallback = await gh.compare(pull.base.sha, pull.head.sha);
          if (fallback && ((fallback.total_commits || 0) > 0 || (fallback.files?.length || 0) > 0)) {
            return { compare: fallback, noBase: false, prFallback: true, ...guide };
          }
        } catch {}
      }
    }
  ...
  async function readCompare(gh, o = {}) {
    const key = 'cmp:' + briefKey(o.repo, o.branch, o.base);
    const hit = _reads.get(key);
    const now = Date.now();
    let p;
    ...
    const res = await p;
    const pull = o.pull;
    if ((res.noBase || res.compare?.ahead_by === 0) && pull?.base?.sha && pull?.head?.sha) {
      if (pull.merged_at || pull.state === 'closed' || res.noBase) {
        try {
          const fallback = await gh.compare(pull.base.sha, pull.head.sha);
          if (fallback && ((fallback.total_commits || 0) > 0 || (fallback.files?.length || 0) > 0)) {
            const fbRes = { compare: fallback, noBase: false, prFallback: true };
            _reads.set(key, { at: Date.now(), p: Promise.resolve(fbRes) });
            return fbRes;
          }
        } catch {}
      }
  ...
  function readBrief(gh, opts = {}) {
    return Promise.all([readCompare(gh, opts), readGuide(gh, opts)])
      .then(async ([cmp, guide]) => {
        const pull = opts.pull || guide.pull;
        if ((cmp.noBase || cmp.compare?.ahead_by === 0) && pull?.base?.sha && pull?.head?.sha) {
          if (pull.merged_at || pull.state === 'closed' || cmp.noBase) {
            try {
              const fallback = await gh.compare(pull.base.sha, pull.head.sha);
              if (fallback && ((fallback.total_commits || 0) > 0 || (fallback.files?.length || 0) > 0)) {
                const fbRes = { compare: fallback, noBase: false, prFallback: true };
                _reads.set('cmp:' + briefKey(opts.repo, opts.branch, opts.base), {
                  at: Date.now(),
                  p: Promise.resolve(fbRes),
                });
                return { ...fbRes, ...guide };
              }
            } catch {}
  ...
  async function fetchPullTarget(gh, number) {
    ...
    const branch = pr?.head?.ref || pr?.head?.sha;
