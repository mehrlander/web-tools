...
(() => {
  ...
  async function build({ history, reconsiderations, proposalIndex, repo = '', ref = 'main', sources = {} }) {
    ...
    const proposalView = id => window.TextProposals?.view?.(proposals, id) || null;
    ...
    const reviewViews = reviews.map(row => {
      ...
      const earlierProposals = earlierIds.map(id => {
        ...
        const sourceOrigin = list(proposal.origins).some(origin => {
          const document = origin.document || {};
          const location = historicalSource.location || {};
          const pinnedDocument = document.repo === historicalSource.document.repo
            && document.path === historicalSource.document.path
            && document.commit === historicalSource.document.commit
            && Number(document.start_line) === Number(location.start_line)
            && Number(document.end_line) === Number(location.end_line);
          const pinnedRecord = !historicalSource.source?.record || !origin.record
            || historicalSource.source.record === origin.record;
          return pinnedDocument && pinnedRecord;
        });
        if (!sourceOrigin) {
          throw new Error(`Reconsideration ${row.id} earlier proposal ${id} has no origin at its pinned proposal source occurrence`);
        }
        return proposal;
      });
