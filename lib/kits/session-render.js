...
(() => {
  ...
  function outline(record) {
    ...
    return cards.map((card, i) => {
      ...
      return {
        ...
        role: lead.role || 'meta',
        ts: lead.ts || '',
        // `ts` is a UTC clock with no date on it, which is enough to PRINT
        // beside a row and not enough to place one: a time cannot become the
        // reader's own zone, and a session that ran past midnight UTC (one on
        // file spans 27 hours) cannot be ordered on it. So the instant rides
        // along, off the same lead turn, and the two cannot disagree about
        // which turn they describe. The outline's rail reads it.
        at: lead.at || '',
        ran, calls,
        turns: card.length,
  ...
  function describe(record) {
    ...
    // THE ATTACHMENTS COME OFF THE TITLE, for the reason they come off the
    // row's ask one kit over: a prompt opening with three attached files opens
    // with about 300 characters of machine-written upload path, so the header
    // of a brief read as a UUID. The rule lives in repo-sessions-cache
    // (askParts), which owns it because it owns the field, and is reached
    // through the global rather than imported: this kit renders on pages that
    // never load that one, and a title that keeps the paths is exactly the
    // behaviour before this, so a miss costs nothing.
    //
    // A prompt that was ONLY attachments has no prose to title with, so the
    // names stand in. That is a real case: 3 of the 7 such records on file
    // 2026-09-10 had their question past the row's cap, and one could yet be
    // written with no question at all.
    const opening = window.RepoSessionsCache?.askParts?.(r.opening_ask)
                 || { ask: r.opening_ask || '', files: [] };
    const asked = opening.ask.replace(/\s+/g, ' ').trim() || opening.files.join(', ');
    return {
      ...
      title: asked ? cap(asked) : (r.short || 'Session'),
