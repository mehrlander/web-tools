...
//
// WIDENED AGAIN 2026-09-14, and this one IS a GitHub icon: a project's icon
// opens the workspace FOLDER and now carries a peek of that folder's README.
// The rule it bends is "an icon that can show you the file is pointing at a
// file". The reason it is not broken is that GitHub renders a directory as its
// listing with the README beneath, so the card still previews the destination
// instead of naming a file the tap will not open. The narrow rule holds
// everywhere else: a repo, a branch, or a menu still carries none, and only a
// directory with a README joins the file case. It arrived when a project's
// README stopped being a tab of its own (app/index.html, the project pane).
//
