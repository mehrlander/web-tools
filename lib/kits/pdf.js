...
(() => {
  ...
  const config = {
    ...
    infer: true,      // close open table perimeters and split unruled headers; every inferred edge is marked.
    reach: 36,        // farthest text may sit past an open side's rule ends and still move that side out.
    background: '#ffffff',
  };
  ...
  // ---- inferred geometry --------------------------------------------------
  //
  // Everything above reports only what a mark on the page supports. The three
  // functions below are the exception, and they exist because a common
  // government table draws its interior rules and leaves the outer border off:
  // with no top-left corner to start from, the walk above finds nothing, or
  // finds only the interior cells and drops the first and last rows and columns.
  //
  // The rule for inventing geometry is that it must never be mistaken for the
  // drawn kind. So nothing here edits a cell's claim silently. `close` returns
  // the edges it inferred with the evidence for each; `mark` re-derives, per
  // cell, which sides a drawn rule actually covers, by comparing against the
  // drawn lines rather than trusting bookkeeping; `project` does the same for
  // header splits. A consumer comparing lattice against stream should discount
  // an edge whose basis is `text`, since that edge came from the same evidence
  // stream reads, and agreement on it is not independent.

  // Coverage of [a, b] by a set of collinear segments, as a union: two drawn
  // segments meeting at a crossing cover the edge between them together.
  const covered = (segs, a, b, tol) => {
    let reach = a;
    for (const [s, e] of segs.toSorted((p, q) => p[0] - q[0])) {
      if (s > reach + tol) break;
      reach = Math.max(reach, e);
      if (reach >= b - tol) return true;
    }
    return reach >= b - tol;
  };

  // Close each ruled region's open sides. A region is a set of rules connected
  // through junctions, with at least two junctions: one crossing is a stray
  // cross, not a table.
  //
  // For each side, the frontier is the outermost end of any rule in the region.
  // If a perpendicular rule lies along the frontier, the side is drawn and left
  // alone. Otherwise the side is open, and it closes at the frontier (basis
  // `rules`: the rule ends are real ink, only the edge joining them is not), or
  // further out if text in the region's band starts beyond it (basis `text`).
  // Text reach grows outward item by item, each within `reach` of the last, so
  // a left-aligned first column is caught by its longest entry.
  //
  // Only rules that END at an open frontier are extended to the new edge. A
  // partial rule ending at an interior vertical is not, so it cannot invent a
  // split. The same test keeps a stray rule from manufacturing cells: a lone
  // signature line running past a drawn border opens that side, but with no
  // second rule reaching the new edge no fourth corner ever closes.
  const close = (h, v, textItems = [], opts = {}) => {
    const tol = opts.intersect ?? config.intersect;
    const snapTol = opts.snap ?? config.snap;
    const reach = opts.reach ?? config.reach;
    const js = junctions(h, v, opts);
    ...
    for (const j of js) {
      const ids = [...[...j.h].map(i => 'h' + i), ...[...j.v].map(i => 'v' + i)];
      ids.forEach(k => union(ids[0], k));
    }
    const count = new Map();
    for (const j of js) { const r = find('h' + [...j.h][0]); count.set(r, (count.get(r) ?? 0) + 1); }

    const outH = h.map(l => ({ ...l })), outV = v.map(l => ({ ...l }));
    const edges = [];
    const text = textItems.filter(t => t.str?.trim());

    for (const [root, n] of count) {
      if (n < 2) continue;
      const H = h.map((l, i) => [l, i]).filter(([, i]) => parent.has('h' + i) && find('h' + i) === root);
      const V = v.map((l, i) => [l, i]).filter(([, i]) => parent.has('v' + i) && find('v' + i) === root);
      const near = (a, b) => Math.abs(a - b) <= tol;

      const frontier = {
        left: Math.min(...H.map(([l]) => l.x1), ...V.map(([l]) => l.x)),
        right: Math.max(...H.map(([l]) => l.x2), ...V.map(([l]) => l.x)),
        top: Math.max(...V.map(([l]) => l.y2), ...H.map(([l]) => l.y)),
        bottom: Math.min(...V.map(([l]) => l.y1), ...H.map(([l]) => l.y)),
      };
      const drawn = {
        left: V.some(([l]) => near(l.x, frontier.left)),
        right: V.some(([l]) => near(l.x, frontier.right)),
        top: H.some(([l]) => near(l.y, frontier.top)),
        bottom: H.some(([l]) => near(l.y, frontier.bottom)),
      };
      const at = { ...frontier };

      // Grow open sides outward over text until nothing moves. Left and right
      // read the band between top and bottom, and the reverse, so a header
      // above an open top edge can widen the band a first-column label needs.
      for (let pass = 0, moved = true; moved && pass < 4; pass++) {
        moved = false;
        const inY = t => (t.y1 + t.y2) / 2 >= at.bottom - tol && (t.y1 + t.y2) / 2 <= at.top + tol;
        const inX = t => (t.x1 + t.x2) / 2 >= at.left - tol && (t.x1 + t.x2) / 2 <= at.right + tol;
        const grow1 = (side, pick, test, next) => {
          if (drawn[side]) return;
          for (let e = at[side], hit; (hit = text.filter(t => pick(t) && test(t, e))).length; ) {
            const nx = next(hit, e);
            if (nx === e) break;
            at[side] = e = nx; moved = true;
          }
        };
        grow1('left', inY, (t, e) => t.x1 < e - tol && t.x2 >= e - reach, (hs) => Math.min(...hs.map(t => t.x1)));
        grow1('right', inY, (t, e) => t.x2 > e + tol && t.x1 <= e + reach, (hs) => Math.max(...hs.map(t => t.x2)));
        grow1('top', inX, (t, e) => t.y2 > e + tol && t.y1 <= e + reach, (hs) => Math.max(...hs.map(t => t.y2)));
        grow1('bottom', inX, (t, e) => t.y1 < e - tol && t.y2 >= e - reach, (hs) => Math.min(...hs.map(t => t.y1)));
      }
      // A text-placed edge sits a snap outside the ink, so rounding cannot
      // leave the outermost item straddling it.
      const sign = { left: -1, right: 1, top: 1, bottom: -1 };
      for (const side of Object.keys(at))
        if (!drawn[side] && !near(at[side], frontier[side])) at[side] = num(at[side] + sign[side] * snapTol);

      const page = (H[0] ?? V[0])[0].page;
      for (const side of ['left', 'right', 'top', 'bottom']) {
        if (drawn[side]) continue;
        const basis = near(at[side], frontier[side]) ? 'rules' : 'text';
        // Extend the rules that end at this frontier to the new edge.
        if (side === 'left') H.forEach(([l, i]) => { if (near(l.x1, frontier.left)) outH[i].x1 = at.left; });
        if (side === 'right') H.forEach(([l, i]) => { if (near(l.x2, frontier.right)) outH[i].x2 = at.right; });
        if (side === 'top') V.forEach(([l, i]) => { if (near(l.y2, frontier.top)) outV[i].y2 = at.top; });
        if (side === 'bottom') V.forEach(([l, i]) => { if (near(l.y1, frontier.bottom)) outV[i].y1 = at.bottom; });
        const line = side === 'left' || side === 'right'
          ? { x: at[side], y1: at.bottom, y2: at.top }
          : { y: at[side], x1: at.left, x2: at.right };
        (side === 'left' || side === 'right' ? outV : outH)
          .push({ ...line, id: `inferred:${side}:${edges.length}`, page, origin: 'inferred', inferred: true });
        const [from, to] = side === 'left' || side === 'right' ? [at.bottom, at.top] : [at.left, at.right];
        edges.push({ edge: side, at: at[side], from, to, basis });
      }
    }
    return { h: outH, v: outV, edges };
  };

  // Which sides of each cell a DRAWN rule covers. Re-derived from the drawn
  // lines rather than carried through the walk, so the marking cannot drift
  // from the evidence. A cell whose every side is drawn gets no field at all,
  // which keeps a fully ruled table's output byte-identical to before.
  const mark = (cellList, h, v, opts = {}) => {
    const tol = (opts.snap ?? config.snap);
    const track = (lines, along, s, e) => lines.map(l => [l[along], [l[s], l[e]]]);
    const hs = track(h, 'y', 'x1', 'x2'), vs = track(v, 'x', 'y1', 'y2');
    const on = (segs, pos, a, b) => covered(segs.filter(([p]) => Math.abs(p - pos) <= tol).map(([, r]) => r), a, b, tol);
    return cellList.map(c => {
      const inferred = [
        !on(hs, c.y2, c.x1, c.x2) && 'top',
        !on(vs, c.x2, c.y1, c.y2) && 'right',
        !on(hs, c.y1, c.x1, c.x2) && 'bottom',
        !on(vs, c.x1, c.y1, c.y2) && 'left',
      ].filter(Boolean);
      return inferred.length ? { ...c, inferred } : c;
    });
  };

  // Subdivide a header cell the body's column rules stop under. A header row
  // ruled above and below with no separators is walked, correctly, as one wide
  // cell, and its logical columns are lost. Split it at an interior column
  // anchor only where all three hold:
  //
  //   - a drawn vertical rises from below to meet the cell's bottom edge there;
  //   - no text in the cell crosses that anchor;
  //   - every resulting piece holds text.
  //
  // The third is what spares a genuine spanning header: "Fiscal Year 2025"
  // centred over three columns leaves the outer pieces empty, so the cell stays
  // whole. Needs text, so it runs after `assign`, and only on a placed grid.
  const project = (grid, v, opts = {}) => {
    const tol = opts.snap ?? config.snap;
    const pad = opts.pad ?? config.pad;
    const splits = [];
    const cells = grid.cells.flatMap(c => {
      if (c.colspan < 2 || !c.text?.length) return [c];
      const cuts = [];
      for (let k = c.col + 1; k < c.col + c.colspan; k++) {
        const x = grid.colAnchors[k];
        const rises = v.some(l => !l.inferred && Math.abs(l.x - x) <= tol && Math.abs(l.y2 - c.y1) <= tol);
        const crossed = c.text.some(t => t.x1 < x - pad && t.x2 > x + pad);
        if (rises && !crossed) cuts.push(k);
      }
      if (!cuts.length) return [c];
      const bounds = [c.col, ...cuts, c.col + c.colspan];
      const pieces = bounds.slice(1).map((k2, i) => {
        const k1 = bounds[i], x1 = grid.colAnchors[k1], x2 = grid.colAnchors[k2];
        const text = c.text.filter(t => (t.x1 + t.x2) / 2 >= x1 && (t.x1 + t.x2) / 2 < x2);
        const inferred = [...(c.inferred ?? []).filter(s => s === 'top' || s === 'bottom'
            || (s === 'left' && i === 0) || (s === 'right' && k2 === c.col + c.colspan)),
          ...(i > 0 ? ['left'] : []), ...(k2 < c.col + c.colspan ? ['right'] : [])];
        return { ...c, x1, x2, col: k1, colspan: k2 - k1, text, inferred,
                 str: text.toSorted((a, b) => b.base - a.base || a.x1 - b.x1)
                   .map(t => t.str).join(' ').replace(/\s+/g, ' ').trim() };
      });
      if (pieces.some(p => !p.text.length)) return [c];
      for (const k of cuts) splits.push({ edge: 'interior', at: grid.colAnchors[k], basis: 'projected', row: c.row });
      return pieces;
    });
    return { ...grid, cells: cells.toSorted((a, b) => a.row - b.row || a.col - b.col), splits };
  };
  ...
  const grids = (paths, textItems = [], opts = {}) => {
    ...
    // Inference is its own stage between reduction and the walk, so the walk
    // itself never learns the difference and the marking is derived after.
    const closed = o.infer ? close(h, v, textItems, o) : { h, v, edges: [] };
    const js = junctions(closed.h, closed.v, o);
    const cs = o.infer ? mark(cells(js, o), h, v, o) : cells(js, o);
    const near = (a, b) => Math.abs(a - b) <= o.snap;
    // An edge belongs to a table when it lies along one of the table's sides
    // AND spans it. Position alone is not enough: two tables stacked in one
    // column share their left x, and would claim each other's edges.
    const along = (e, t) => {
      const [at, a, b] = { left: [t.x1, t.y1, t.y2], right: [t.x2, t.y1, t.y2],
                           top: [t.y2, t.x1, t.x2], bottom: [t.y1, t.x1, t.x2] }[e.edge];
      return near(e.at, at) && e.from <= a + o.snap && e.to >= b - o.snap;
    };
    return cluster(cs, o).map(table => {
      const grid = anchorGrid(table, o);
      let placed = textItems.length ? assign(grid, textItems, o) : grid;
      if (o.infer && textItems.length) placed = project(placed, v, o);
      const { splits = [], ...rest } = placed;
      const out = { ...rest, x1: table.x1, y1: table.y1, x2: table.x2, y2: table.y2,
                    matrix: toMatrix(placed, o) };
      if (o.infer) out.inferred = [...closed.edges.filter(e => along(e, table)), ...splits];
      return out;
    });
  };

  const lattice = { junctions, cells, cluster, anchorGrid, assign, toMatrix, close, mark, project, grids };
