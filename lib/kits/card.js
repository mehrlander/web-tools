...
//   Card.wire(el, { onClose, except, stale })  -> { detach() }
...
`;
  ...
  function wire(el, { onClose, except = [], stale = true } = {}) {
    ...
    const onOver = (ev) => {
      if (stale !== true || !shown(el)) return;
      if (near(ev.target)) { clear(); return; }
      if (fade === null) fade = setTimeout(() => { fade = null; onClose(); }, GRACE_MS);
    };
    ...
    const onScroll = (ev) => {
      if (!stale || !shown(el)) return;
      if (el.contains(ev.target)) return;
      clear(); onClose();
    };
    document.addEventListener('scroll', onScroll, true);
    const onGone = () => { if (stale && shown(el)) { clear(); onClose(); } };
