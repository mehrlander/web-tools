...
(() => {
  ...
  // <script src> load from the same ?use=<ref> as the host page. Read at
  // call time: mount() sets it before any block builds a doc.
  let base = '';
  ...
  function block(cfg) {
    ...
    const docOpts = () => ({ tw: cfg.tw, daisy: cfg.daisy, inject: cfg.inject, base, context: cfg.context });
