...
(() => {
  ...
  const S = {
    ...
    compMend: false,
    // Custom drag-resized dimensions from user interaction:
    userWidth: null,
    userHeight: null,
    resizeBtn: null,
    deckBtn: null,
    notesDeckHandle: null,
  };
  ...
  const syncPlace = () => {
    ...
    if (b._glyph) b._glyph.className = 'ph ph-' + (S.inPlace ? 'eye-slash' : 'eye');
    if (b._label) b._label.textContent = S.inPlace ? 'Show notes in card' : 'Show notes on page';
    if (b._hint) b._hint.textContent = S.inPlace ? 'take pins off the page' : 'draw pins beside what they mark';
    b.title = S.inPlace
  ...
  const dockDictate = async (opts = {}) => {
    const w = win() || window;
    const canDock = w.innerWidth >= DOCK_MIN;
    const full = !!opts.full;
    if (!full && !canDock) return false;
  ...
  const goDictate = async (btn, note, opts = {}) => {
    ...
    // The handoff is written either way; open in swipe-deck viewer if available.
    if (await dockDictate(opts)) return;
    try { (win() || window).location.href = DICTATE_URL; } catch { window.location.href = DICTATE_URL; }
  };

  const openNotesDeck = async (startIdx = 0) => {
    if (!S.items.length) return false;
    if (!window.swipeDeck && window.gh?.load) {
      try { await window.gh.load('kits/swipe-deck.js'); } catch {}
    }
    if (!window.swipeDeck) return false;
    if (S.panel) {
      S.panelHidden = true;
    ...
    const notes = S.items;
    S.notesDeckHandle = window.swipeDeck.open({
      count: notes.length,
      start: Math.max(0, Math.min(startIdx, notes.length - 1)),
      title: 'Notes',
      subtitle: (window.Annotate?.subject?.title) || (S.doc && S.doc.title) || '',
      icon: 'ph-note-pencil',
      index: i => ({ title: `${i + 1}. ${describe(notes[i]?.target, 40)}`, subtitle: notes[i]?.note?.slice(0, 60) }),
      render: (i, slide) => {
        const it = notes[i];
        if (!it) return;
        slide.innerHTML = '';
        const card = el('div', 'max-w-2xl mx-auto p-6 flex flex-col gap-4 text-base-content;');
        const headRow = el('div', 'flex items-center justify-between gap-3 text-sm text-base-content/60 border-b border-base-300 pb-2;');
        headRow.innerHTML = `<span class="font-mono font-medium">Note ${i + 1} of ${notes.length}</span><span class="italic truncate text-xs text-warning-content bg-warning/20 px-2 py-0.5 rounded">${describe(it.target, 50)}</span>`;
        card.appendChild(headRow);

        const quote = it.target && (it.target.quote || it.target.exact);
        if (quote) {
          const qBox = el('blockquote', 'border-l-4 border-warning/60 pl-3 py-1 text-sm italic text-base-content/80 bg-base-200/50 rounded-r;');
          qBox.textContent = quote;
          card.appendChild(qBox);
        }

        const noteText = el('div', 'text-base font-normal whitespace-pre-wrap leading-relaxed py-2;', it.note || '(no text)');
        card.appendChild(noteText);

        const addr = addressText(it.target);
        if (addr) {
          const aBox = el('div', 'text-xs font-mono text-base-content/50 break-all;', addr);
          card.appendChild(aBox);
        }

        const actRow = el('div', 'flex items-center gap-2 pt-4 border-t border-base-200;');
        const editBtn = el('button', 'btn btn-sm btn-outline gap-1.5', 'Edit');
        editBtn.addEventListener('click', () => {
          S.notesDeckHandle?.close?.();
          editNote(it.id);
        });
        const copyBtn = el('button', 'btn btn-sm btn-ghost gap-1.5', 'Copy');
        copyBtn.addEventListener('click', () => copyNote(it.id, copyBtn));
        const dictateBtn = el('button', 'btn btn-sm btn-ghost gap-1.5', 'Dictate');
        dictateBtn.addEventListener('click', () => {
          S.notesDeckHandle?.close?.();
          goDictate(dictateBtn, it, { full: true });
        });
        actRow.append(editBtn, copyBtn, dictateBtn);
        card.appendChild(actRow);
        slide.appendChild(card);
      },
      onClose: () => {
        S.notesDeckHandle = null;
        showPanelAgain();
      },
    });
    return true;
  ...
  const bindDragResize = (handle, edge = 'both') => {
    if (!handle) return;
    handle.addEventListener('pointerdown', (e) => {
      ...
      const w = win() || window;
      const vh = (w.visualViewport && w.visualViewport.height) || w.innerHeight || 600;
      const vw = (w.visualViewport && w.visualViewport.width) || w.innerWidth || 800;

      const isBottomAnchored = root.style.top === 'auto' || (!root.style.top && root.style.bottom);
      ...
      const move = (ev) => {
        ...
        if (edge === 'r' || edge === 'both') {
          const maxW = Math.max(260, vw - rootRect.left - 12);
          const newW = Math.max(260, Math.min(maxW, Math.round(startW + dx)));
          S.userWidth = newW;
        ...
        if (edge === 't') {
          const maxH = Math.max(160, isBottomAnchored ? (rootRect.bottom - 12) : (vh - 24));
          const newH = Math.max(160, Math.min(maxH, Math.round(startH - dy)));
          S.userHeight = newH;
          p.style.height = newH + 'px';
          p.style.maxHeight = newH + 'px';
          if (!isBottomAnchored) {
            root.style.top = Math.max(12, Math.round(rootRect.top + (startH - newH))) + 'px';
          }
        } else if (edge === 'both') {
          const maxH = Math.max(160, vh - rootRect.top - 12);
          const newH = Math.max(160, Math.min(maxH, Math.round(startH + dy)));
          S.userHeight = newH;
          p.style.height = newH + 'px';
          p.style.maxHeight = newH + 'px';
  ...
  const sizeExpanded = () => {
    ...
    const maxAllowedH = Math.max(160, vh - bottom - 12);
    let h;
    if (S.userHeight != null) {
      h = Math.max(160, Math.min(S.userHeight, maxAllowedH));
    } else {
      h = Math.max(204, Math.min(374, Math.round(vh * 0.47), maxAllowedH));
    }
    p.style.height = h + 'px';
    p.style.maxHeight = h + 'px';
    if (S.userWidth != null && S.ui) {
      const vw = (w.visualViewport && w.visualViewport.width) || w.innerWidth || 0;
      const left = parseFloat(S.ui.style.left) || 0;
      const maxAllowedW = Math.max(240, vw - left - 12);
      const uw = Math.max(260, Math.min(S.userWidth, maxAllowedW));
      S.ui.style.width = uw + 'px';
      p.style.width = uw + 'px';
      S.ui.style.maxWidth = 'calc(100vw - 24px)';
    }
  };
  ...
  const syncBody = () => {
    ...
    if (S.readBar) S.readBar.style.display = (set && !S.draft) ? 'flex' : 'none';
    if (S.compCap) S.compCap.style.display = S.draft ? 'block' : 'none';
  ...
  const mountUI = () => {
    ...
    const p = el('div', 'display:none;flex-direction:column;position:relative;'
    ...
    // THE CAPTION IN THE HEADER: what the draft is about sits directly in
    // the header row, saving vertical room across the card and eliminating
    // the extra row that used to sit inside the composer.
    S.compCap = el('div', 'display:none;color:#a16207;font-size:11.5px;font-style:italic;font-weight:500;'
      + 'overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex:1;min-width:0;padding:2px 4px;');
    head.appendChild(S.compCap);
    ...
    const sep = el('div', 'height:1px;background:#e4e4e7;margin:3px 2px;');
    S.aimMenu.appendChild(sep);

    // Show notes on page (moved to dropdown menu to save header space)
    S.placeBtn = aimItem(
      S.inPlace ? 'Show notes in card' : 'Show notes on page',
      S.inPlace ? 'take pins off the page' : 'draw pins beside what they mark',
      S.inPlace ? 'eye-slash' : 'eye',
      'Show every note on the page, beside what it is pinned to',
      () => showInPlace(!S.inPlace)
    );
    syncPlace();

    // Expand to full screen (moved to dropdown menu to save header space)
    S.outBtn = aimItem(
      'Expand to full screen',
      'open in swipe deck viewer',
      'arrows-out',
      'Open in full screen swipe deck viewer',
      () => {
        const it = S.draft ? { target: S.draft.target, note: draftText() } : (S.selId ? S.items.find(x => x.id === S.selId) : S.items[0]);
        goDictate(S.outBtn, it, { full: true });
      }
    );

    // Browse notes deck (swipe deck viewer for notes)
    S.deckBtn = aimItem(
      'Browse notes deck',
      'swipe through notes as slides',
      'cards',
      'Open notes in swipe deck viewer',
      () => openNotesDeck(S.selId ? S.items.findIndex(x => x.id === S.selId) : 0)
    );
    ...
    // Resize grips: bottom-right corner, top border, right border
    const gripBR = el('div', 'position:absolute;bottom:0;right:0;width:18px;height:18px;'
      + 'cursor:nwse-resize;display:flex;align-items:flex-end;justify-content:flex-end;padding:2px;'
      + 'touch-action:none;user-select:none;-webkit-user-select:none;z-index:20;color:#a1a1aa;opacity:.6;');
    gripBR.innerHTML = '<svg width="10" height="10" viewBox="0 0 10 10" fill="none" stroke="currentColor"><path d="M9 1L1 9M9 5L5 9M9 8L8 9" stroke-width="1.5" stroke-linecap="round"/></svg>';
    gripBR.title = 'Drag to resize';
    bindDragResize(gripBR, 'both');
    p.appendChild(gripBR);

    const gripT = el('div', 'position:absolute;top:0;left:0;right:0;height:6px;'
      + 'cursor:ns-resize;touch-action:none;z-index:20;');
    bindDragResize(gripT, 't');
    p.appendChild(gripT);

    const gripR = el('div', 'position:absolute;top:6px;bottom:18px;right:0;width:6px;'
      + 'cursor:ew-resize;touch-action:none;z-index:20;');
    bindDragResize(gripR, 'r');
    p.appendChild(gripR);
  ...
  const cancelDraft = () => {
    ...
    if (S.compose) {
      ...
      S.compInterim = '';
      if (S.compCap) {
        S.compCap.style.display = 'none';
        S.compCap.textContent = '';
      }
      if (S.readBar) S.readBar.style.display = !S.inPlace ? 'flex' : 'none';
    }
  ...
  const beginDraft = (target, caption, rectDoc, opts = {}) => {
    ...
    S.compCap.style.display = 'block';
    if (S.readBar) S.readBar.style.display = 'none';
  ...
  const disable = () => {
    ...
    S.pageChip = S.placeBtn = S.jotBtn = S.outBtn = S.deckBtn = null;
    S.userWidth = S.userHeight = null;
  ...
  window.Annotate = {
    ...
    startPick, startRegion, notePage, noteSection, editNote, showInPlace, noteSelection, openNotesDeck,
