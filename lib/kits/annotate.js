  ...
  const dockDictate = async () => {
    ...
    S.dockHandle = window.swipeDeck.open({
      ...
      render: (i, slide) => {
        ...
        // the same complaint as a slow open wearing different clothes. The
        // note goes UNDER the frame rather than beside it, so nothing has to
        // be removed on load and a frame that paints early simply covers it.
