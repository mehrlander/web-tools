...
(() => {
  ...
  const S = {
    ...
    sel: null,
    // The dictation panel, where there was room to open one beside the page
    // rather than navigating to it. See dockDictate.
    dock: null,
    dockFrame: null,
  ...
  const handoffText = () => {
    const words = String(draftText() || '').trim();
    const head = [];
    const here = hereUrl();
    ...
    try {
      ...
      const target = (it && it.target) || (S.draft && S.draft.target)
  ...
  // a panel on the right edge and the page, the card and the notes all stay.
  //
  // A WIDTH, NOT A MEDIA QUERY, and not the swipe deck's dock hook either. The
  // deck asks its HOST whether it may dock (kits/swipe-deck.js `__deckPane`),
  // because only the host knows what the deck would sit beside and how to give
  // the width back. This kit has no host contract: it mounts into whatever
  // document it was aimed at, so it cannot reflow that page and does not try.
  // The panel OVERLAYS the right edge and the page scrolls underneath, which is
  // the one behaviour available to a guest. The number is the question the deck
  // answers with a query ("is there room beside"), asked directly.
  //
  // 1024 is the deck's own breakpoint, taken rather than re-derived: below it a
  // 420px panel leaves under 600px of page, which is no longer a page with a
  // panel beside it.
  const DOCK_MIN = 1024;

  const closeDock = () => {
    if (S.dock) { try { S.dock.remove(); } catch {} }
    S.dock = S.dockFrame = null;
  };
  ...
  const dockDictate = () => {
    ...
    // end takes the handoff at boot and only at boot, so a panel already
    ...
    closeDock();
    // A FLOATING PANEL, NOT A FULL-BLEED EDGE, and the insets are load-bearing
    // rather than styling. It is the card's sibling: same rounded, shadowed
    // thing over the same page, so it reads as another surface the annotator
    // put up rather than as browser chrome that appeared.
    //
    // THE BOTTOM INSET CLEARS THE LAUNCHER. The fab floats 56px at the
    // viewport's bottom-right corner and sits ABOVE everything on purpose
    // (alpineComponents/fab.js: a control that owns a surface must sit above
    // it), so a panel reaching the bottom edge puts the launcher on top of the
    // far end's own destination row. Measured at 1280x800: the launcher
    // covered the rightmost button of that row outright. The card lives with
    // the same overlap because a card can be dragged out from under it; this
    // cannot, so it stops short instead.
    const panel = el('div', 'position:fixed;top:12px;right:12px;bottom:88px;'
      + 'width:min(420px, 38vw);display:flex;flex-direction:column;'
      + 'background:#fff;border:1px solid #d4d4d8;border-radius:12px;overflow:hidden;'
      + 'box-shadow:0 8px 28px rgba(0,0,0,.16);'
      // Under the card, which is bottom-LEFT and so does not overlap it at any
      // width this opens at. The order still matters for a card the reader has
      // dragged: a dragged card must not end up behind this.
      + `z-index:${Z - 50};`);
    panel.setAttribute('data-annotate-dock', '');
    const bar = el('div', 'flex:0 0 auto;display:flex;align-items:center;gap:6px;'
      + 'padding:6px 6px 6px 10px;border-bottom:1px solid #e4e4e7;background:#fafafa;');
    bar.appendChild(el('div', 'flex:1;min-width:0;color:#3f3f46;'
      + 'font:600 12px ui-sans-serif,system-ui;', 'Dictation'));
    // ONE CONTROL, and it says what it does to the PANEL. It is not a way back:
    // the words are on the dictation page now and that page keeps its own
    // draft, so closing this puts a panel away rather than taking anything
    // with it. A second control meaning "bring them back" would promise a
    // return trip nothing here makes.
    const shut = icon('x', 'border:0;background:none;color:#71717a;cursor:pointer;'
      + 'padding:4px;display:flex;align-items:center;', 'Put the dictation panel away');
    shut.addEventListener('click', () => closeDock());
    bar.appendChild(shut);
    panel.appendChild(bar);
    const frame = el('iframe', 'flex:1;min-height:0;width:100%;border:0;display:block;');
  ...
  // Load-on-press rather than at kit load: the carrier is 2K and this button is
  ...
  const mountUI = () => {
    ...
    // across. It sits beside the eye because the two are the same kind of
    // control, a different ROOM for the same set, where the aim menu past the
    // auto margin is about the note itself.
    //
    ...
    S.outBtn = icon('arrows-out', HEAD_KEY);
    S.outBtn.title = 'Open the words on the dictation page, where they can be read back and sent'
  ...
  //
  // THE QUERY LIVES IN kits/src-doc.js, the carrier every kind declares
