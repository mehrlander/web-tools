...
(() => {
  ...
  const mountUI = () => {
    ...
    // capital behind it, and putting that right is two edits at one seam. The
    // top key becomes the key that closes it: the cell that writes a full stop
    // is the cell that takes one back, which is why it replaces `.` rather
    // than standing somewhere new. Nothing is lost while it is showing, since
    // a full stop is the one mark that cannot be wanted where one is already
    // sitting.
    //
    // A CARET IN THE GAP, AND ONLY THAT. kits/dictate.js `stitchAim` also
    // answers 'back', meaning "no caret aim, so the newest break in the
    // buffer", and that reading is deliberately not offered here: it is true
    // of almost any buffer holding two sentences, so a cell borrowed for it
    // would never hand the full stop back. With a caret in the gap the cell is
    // free, because a full stop cannot be wanted where one is already sitting.
    //
    // SO THE REACH AND ITS STEP-BACK KEY ARE THE FULL PAGE'S. Its keys lie
    // ACROSS, where a seventh cell costs width off six. Here they stack in a
    // 56px column beside a text box capped at 172px, so a fifth cell is 34px
    // tall against the 43 these have, and this column's own argument (above)
    // is that three keys is what makes each big enough to hit without looking.
    // The card repairs the break the caret is in, which is what it has always
    // done; reaching an older one means tapping the gap first.
    //
    ...
    // swap can take all three because a selection means the reader is fixing a
    // word rather than writing.
    const STITCHED = [{ s: true, icon: 'arrows-in-line-horizontal',
                        title: 'Undo the sentence break the pause invented: the full stop goes and the capital comes down' },
                      { m: ',' }, { m: '?' }];
    const padSet = () => (S.dict && S.dict.hasSelection) ? CASING
      : S.compShift ? SHIFTED
      : (S.dict && S.dict.stitchAim === 'caret') ? STITCHED
      : PRIMARY;
    S.compShift = false;
