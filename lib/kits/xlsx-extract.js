...
(() => {
  ...
  //
  //   { id, label, match: { sheets: [name…] }, omit: [name…],
  //     readings: [kind…], cap, dest, file, note }
  //
  // `match.sheets` are all required, and matching is by sheet set rather than
  // by file name because a workbook is renamed more often than its tabs are.
  // Every sheet not in `omit` is taken, with the readings it has any of.
  // `cap` defaults to null (no row cut), since a landed extract is a file
  // rather than a link. `dest` and `file` may carry `{date}` and `{stem}`.
  function receptions(cat, declared) {
    ...
    for (const r of declared || []) {
      ...
      const sheets = cat.sheets.filter(s => !omit.has(s.name))
        .map(s => ({ name: s.name, kinds: readings.filter(id => (s.counts?.[id] || 0) > 0) }))
        .filter(s => s.kinds.length);
      out.push({
        reception: r,
        pick: { sheets, objects: [] },
        omitted: [...omit].filter(n => names.has(n)),
  ...
  // Where a reception's extract lands: `{date}` is the day (UTC) and `{stem}`
  // the workbook's name without its extension, reduced to filename characters.
