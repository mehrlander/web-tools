...
(() => {
  ...
  const EXT_CAT = (() => {
    ...
    put('data', 'json jsonl ndjson csv tsv xml yaml yml toml ini lock env properties parquet geojson');
  ...
  function squarify(items, x, y, w, h) {
    ...
    while (i < items.length) {
      ...
      let row = [], rowSum = 0, worst = Infinity;
      let j = i;
      while (j < items.length) {
        ...
        for (const v of test) {
          const len = v / thick;
          wst = Math.max(wst, thick / len, len / thick);
        }
        if (wst > worst && row.length) break;
        row = test; rowSum = sum; worst = wst; j++;
      }
      const thick = Math.min(rowSum / side, vertical ? ch : cw);
      let off = 0;
      for (let k = 0; k < row.length; k++) {
        const len = Math.min(row[k] / thick, side - off);
        rects.push(vertical
