...
//     opts: { tw?, daisy?, inject?, context? }
//           tw      — inject Tailwind into the frame
//           daisy   — Tailwind + daisyUI theme sheet + Phosphor (implies tw)
//           inject  — scripts run in the frame head, in order, before any
//                     body script: a URL string loads as <script src>, and
//                     { code } runs inline (how a caller hands in repo code
//                     it read at its own ref)
...
(() => {
  ...
  const injectTags = list => (list || []).map(s => typeof s === 'string'
    ? `<scr` + `ipt src="${s}"><\/scr` + `ipt>`
    : `<scr` + `ipt>${guard(s.code)}<\/scr` + `ipt>`).join('');
  ...
  const htmlOpen = o => `<!doctype html><html${o.daisy ? ' data-theme="light"' : ''}>`;
