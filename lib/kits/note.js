...
`;
  ...
  // repositioning mid-scroll is not worth the frame budget. Blur is the third
  // door onto the same fact and the one that was missing: the window can lose
  // focus mid-hover, and the pointerout that would have closed the note never
  // arrives, so it is still there on the next visit to the tab. Card.wire
  // carries all three; the two halves of the rule now agree on the set.
  addEventListener('scroll', () => { if (current) close(); }, true);
  addEventListener('resize', () => { if (current) close(); });
  addEventListener('blur', () => { if (current) close(); });
