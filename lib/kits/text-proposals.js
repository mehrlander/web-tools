...
(() => {
  ...
  function spansIn(haystack, needle) {
    ...
    while (at !== -1) {
      ...
      if (leftOk && rightOk) spans.push([at, end]);
  ...
  async function load(gh, { specPath = DEFAULT_SPEC, fresh = false, quiet = false } = {}) {
    ...
    if (!fresh && loads.has(key)) return loads.get(key);
    ...
    const promise = (async () => {
      const specResult = await gh.get(specPath, readOptions);
      ...
      const results = await Promise.all(roles.map(async role => {
        ...
        return [role, await gh.get(path, readOptions)];
      }));
