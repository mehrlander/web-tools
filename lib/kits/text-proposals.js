...
(() => {
  ...
  // path:pN import_id is provenance only; the link opens the live path on the
  // scanned repo's default branch tip (not a pinned blob).
  ...
  function view(index, proposalOrId) {
    ...
    return {
      id: proposal.id,
      lane,
      action: verdict || decision || halfLength || '',
      verdict,
      decision,
      operation,
  ...
  async function load(gh, { specPath = DEFAULT_SPEC, fresh = false, quiet = false } = {}) {
    ...
    const unavailable = (error) => {
      ...
      const wrapped = new Error('Prior revisions are unavailable'
