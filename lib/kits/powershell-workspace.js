...
(() => {
  ...
  const readFile = async ({ gh, path, revision, blobSha }) => {
    ...
    return { text: await blobText(gh, sha), sha, revision };
  };
  ...
  window.PowerShellWorkspace = { COLLECTION, validPath, validRef, draftId, snapshot, readFile,
