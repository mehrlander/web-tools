...
`;
  ...
  // A page error, in the trace beside the guard decisions rather than only in a
  // console nobody has open on a phone. The run-length collapse means a page
  // erroring every frame is one line with a count, so a storm cannot flood the
  // sixteen lines the overlay draws.
  //
  // Reported 2026-09-10 by a reader whose browser raised a banner over
  // `ResizeObserver loop completed with undelivered notifications`, which is a
  // notice rather than an exception: it has no stack and stops nothing, and it
  // means an observer callback changed layout in a way that left the browser
  // unable to deliver the rest of that frame's notifications. What a capture
  // could not say was whether a resize storm ran with it. Now it can.
  const onErr = (ev) => {
    let m = '';
    try { m = String((ev && (ev.message || (ev.error && ev.error.message))) || ev); } catch (e) { m = '?'; }
    log('page:error', m.slice(0, 120));
  };
