...
`;
  ...
  const yesno = (v) => v
    ? '<span class="wt-p-yes">yes</span>'
    : '<span class="wt-p-no">no</span>';
  ...
  function render() {
    const e = env();
    const rows = [];
    rows.push(`<span class="wt-p-head">probe <b>${ID}</b></span>`
      + `<span class="wt-p-dim">  ${stamp().toFixed(1)}s`
      + (held ? '  <span class="wt-p-hot">HELD</span>' : '') + '</span>');
    rows.push(`<span class="wt-p-dim">card.js</span> ${yesno(e.card)}`
      + `  <span class="wt-p-dim">probe hook</span> ${yesno(e.hook)}`
      + `  <span class="wt-p-dim">framed</span> ${yesno(e.framed)}`);
    rows.push(`<span class="wt-p-dim">hover</span> ${yesno(e.hover)}`
      + `  <span class="wt-p-dim">any-hover</span> ${yesno(e.anyHover)}`
      + `  <span class="wt-p-dim">coarse</span> ${yesno(e.coarse)}`
      + `  <span class="wt-p-dim">touch</span> ${e.touch}`
      + `  <span class="wt-p-dim">${e.w}×${e.h}</span>`);
    for (const [name, fn] of watchers) {
      let v;
      try { v = fn(); } catch (err) { v = '!' + (err && err.message); }
      rows.push(`<span class="wt-p-dim">${esc(name)}</span> ${esc(v)}`);
    }
    rows.push(`<span class="wt-p-dim">under</span> ${esc(lastUnder || '(no pointer yet)')}`);
    rows.push('<span class="wt-p-rule">' + '─'.repeat(46) + '</span>');
    const tail = trace.slice(-SHOW_TRACE);
    for (const r of tail) {
      const hot = /close|arm|esc/.test(r.tag) ? ' wt-p-hot' : '';
      rows.push(`<span class="wt-p-dim">${r.t.toFixed(2).padStart(6)} +${r.d.toFixed(2)}</span>`
        + ` <span class="${hot.trim()}">${esc(r.tag).padEnd(16)}</span>`
        + ` <span class="wt-p-dim">${esc(r.detail)}</span>`
        + (r.n > 1 ? ` <span class="wt-p-hot">×${r.n}</span>` : ''));
    }
    if (!tail.length) rows.push('<span class="wt-p-dim">(no events yet)</span>');
    rows.push('<span class="wt-p-rule">' + '─'.repeat(46) + '</span>');
    rows.push('<span class="wt-p-dim">alt+shift+ P file · H hold · M move · C clear</span>'
      + (fileNote ? `  <span class="wt-p-hot">${esc(fileNote)}</span>` : ''));
    return rows.join('\n');
  }
  ...
  function snapshot() {
    ...
    return {
      probeId: ID, probe: param('probe'), at: stamp(),
  ...
  async function file(reason) {
    ...
    if (!window.PageReport) { note('no PageReport kit'); return { ok: false, why: 'no PageReport' }; }
    try {
