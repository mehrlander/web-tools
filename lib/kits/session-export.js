...
(() => {
  ...
  function index(record, opts = {}) {
    ...
    //
    // A record with no instants on its cards (the schema-3 shape, and any card
    // whose lead is the renderer's own note) still gets a rail, spaced evenly
    // and with no clock labels: even spacing claims nothing about time, which
    // is honest, where a label over it would not be.
