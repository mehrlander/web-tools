...
(() => {
  ...
  function index(record, opts = {}) {
    ...
    const peek = h('div', {
      ...
      // the card's own edge.
      // 26rem. It went to 14rem when the panel first held one comment instead of
      // an exchange, and came back the same day the chip became its trigger:
      // the reason to keep it short was that a panel could open on a hover
      // anywhere along a line, so its height was a cost paid on every pass of
      // the pointer. A chip is aimed at, so the height is only ever paid for
      // deliberately, and the panel's job is to hold the rest of the comment.
    ...
    // `role` and a tabindex rather than a real `button`. The reason was the
    // trigger being a CLAMPED LINE: Safari sizes a button from its unclipped
    // content, so the row stood at its full height with the text clipped inside
    // it, which this branch shipped once (snags:
    // safari-button-sizes-from-unclipped-content). The trigger is the chip now,
    // which is not clamped, so that hazard no longer reaches it. The mechanism
    // stayed because a chip wants exactly the same reach and a button buys
    // nothing more here.
    ...
    // chat-render's own chip, so the two surfaces cannot draw the same mark two
    // ways. Resolved per call rather than held, the same shape SR() takes: the
    // load order is the host's business, and a host that never loaded
    // chat-render gets a row with no chip rather than a thrown list.
    const CR_CHIP = (n, tappable, title) => (window.chatRender?.moreChip
      ? window.chatRender.moreChip(n, tappable, title)
      : h('span', { class: 'inline whitespace-nowrap rounded px-1 bg-base-content/10'
                         + ' font-mono text-[9.5px] text-base-content/50 not-prose', title },
          '+' + n.toLocaleString() + ' chars'));

    // ── the card list ──
    const list = h('div', { class: 'flex flex-col' });
    // Every clamped line, with the chip that reports what its clamp hid. Filled
    // as the rows are built and walked once by measureChips below.
    const chips = [];

    // ── The chip, and what it reports ────────────────────────────────────
    // The row's two lines are clamped by CSS, not cut in the data, so unlike
    // the transcript card there is no dropped count to print: it is measured
    // off the rendered line (see measureChips below). The chip is chat-render's
    // own, exported for this, since a reader meets both marks within two taps.
    //
    // INSIDE THE LINE, and the line is CUT to make room for it. The chip was
    // placed absolutely in a reserved padding-right, which made it a column at
    // the right margin and, worse, shortened EVERY line of a two-line ask to
    // hold room for a chip that belongs on the last one. Reported 2026-09-08:
    // "it should be in line. It shouldn't block and act like its own column."
    //
    // So the text is truncated to what the clamp was going to show anyway, less
    // a little for the chip, and the chip follows it as ordinary inline
    // content. That also retires the circularity the gutter existed to break:
    // the count is no longer "what the clamp hides", which moves when the chip
    // takes space, but "what this line is not showing", which is exact by
    // construction whatever the digits are.
    //
    // The original children are kept so a re-measure can put them back: a reply
    // line carries bold runs from richLine, so the cut has to preserve
    // structure rather than reset textContent.
    const withChip = (el) => {
      const chip = CR_CHIP(0, false, 'Read the rest of this line');
      chip.hidden = true;
      chip.classList.add('not-prose');
      chips.push({ el, chip, orig: [...el.childNodes].map(n => n.cloneNode(true)) });
      return el;
    };
    // The row elements, in order, which is what the rail below reads to say
    // which card the reader is standing on and what it scrolls to on a tap.
    const boxes = [];
    ...
    line.forEach((card, ci) => {
      ...
      const title = h('div', {
        ...
        //
        // It was the last mark of a distinction this file already retired
      ...
      // 3px at /40, up from 2px at /20 on 2026-09-08. It was drawn to be the
      // quiet answer to a fill and was quieter than that: at 2px and /20 it is
      // about a fifth of a hairline against a cool grey page, and a reader
      // asked for a hover effect on the row while this one was already there.
      // Still a strip at the card's own edge rather than a wash, which is the
      // part that must not change: a tint behind the head washes the blue ask
      // sitting on it, reported twice on 2026-09-01 and removed twice.
      const scope = h('div', {
        class: 'pointer-events-none absolute inset-y-1 left-0 w-[3px] rounded-full '
          + 'bg-base-content/40 opacity-0 transition-opacity group-hover:opacity-100' });
      const box = h('div', {
        // scroll-mt is what the rail's jump lands against: scrollIntoView puts
        // the box at the top of the scroller, and the top of the scroller is
        // under the rail and, in document flow, under the host's chrome too.
        // Without it a tapped card arrives hidden behind both.
        class: 'group relative mb-0.5 border-b border-base-200 py-1 pl-3 pr-2.5 last:border-b-0 '
          + 'scroll-mt-[calc(var(--chrome-h,0px)+2rem)]',
      }, scope, head);
      // ── One panel per LINE, carrying that line whole ─────────────────────
      // The row is two clamped lines, an ask and the reply that answered it,
      // and the panel un-clamps the one under the pointer. It carried the whole
      // exchange until 2026-09-08 and was reported awkward on a desktop, which
      // it was for a reason the argument for it missed: hovering either line
      // opened a panel holding BOTH, so a reader who wanted the rest of a
      // sentence was handed a second copy of the line above it and a scroll.
      //
      // What settles it is that the exchange has a better home. The deck reads
      // one card whole, with its tool calls, at reading size, and the glyph to
      // its right is one tap away on every row. So the panel does the small job
      // (this line, unclamped) and stops competing for the large one.
      //
      // THE LINE IS THE TRIGGER and the CARD is still the scope: the rule at
      // the box's left edge lights on the row, and the panel hangs under the
      // line that opened it, since `box` is the positioned ancestor and
      // showPeek reads offsetTop against it.
      //
      ...
      const fillTurn = (t) => async (node) => {
        ...
        await SR().card([t], node, { dense: true, collapse: 0 });
      ...
      // ── THE CHIP IS THE TRIGGER, and the line is not ─────────────────────
      // The line was, from 2026-09-08 morning, and a whole line is a large
      // target for a panel: on a desktop the pointer crosses one on its way to
      // anywhere, so the panel opened on the way past. Reported the same day.
      // A chip is aimed at, so the panel only ever opens on purpose, and
      // hovering the prose is left to say the two quiet things it should: which
      // tick this row is, and that the row is under the pointer.
      //
      // It also gates itself on there being anything to show. A chip exists
      // only where the clamp actually hid something, so a line that fits has no
      // trigger at all, which is the honest shape: a panel repeating a line the
      // reader can already see whole is the copy this row does not need.
      //
      // The TURN'S own clock labels it, not the card's, which is the card's
      // first turn and so is wrong on the reply by however long the work took.
      const askChip = ask ? chips[chips.length - (answer ? 2 : 1)] : null;
      const repChip = answer ? chips[chips.length - 1] : null;
      if (askChip) asTrigger(askChip.chip, ci + ':a', box, ask.ts || card.ts || '', fillTurn(ask));
      if (repChip) asTrigger(repChip.chip, ci + ':r', box, last.ts || '', fillTurn(last));
    ...
    // One tick a card, placed by WHEN it happened across the session's span
    // rather than evenly, so the rail draws the shape of the conversation: a
    // burst reads as a cluster, an hour away as a gap, and a session that ran
    // overnight reads as one. The vocabulary is the transcript card's, in the
    // estate (estate.js, "THE SESSION'S OWN TIMELINE"), which is the surface
    // this was asked to match; both descend from the State view's pulse rail.
    //
    // IT LIVES HERE, NOT IN THE HOST, because it needs two things the host does
    // not have: the card boxes, to say which one the reader is standing on, and
    // the lead instants that place the ticks. A host that mounted its own would
    // be reading into this kit's DOM.
    //
    // AND IT IS A CONTROL. Tapping or dragging scrolls to the nearest card,
    // which is the one gesture that crosses forty of them; that is why the
    // whole strip carries the pointer and the height, rather than each 1px
    // tick.
    const railTicks = h('div', { class: 'absolute inset-0' });
    // One element per card, kept so a row can light its own; see lightTick.
    const tickEls = [];
    const railHere = h('div', {
      // Full strength, wider and taller than a tick, so a cluster still says
      // which card the rail is naming.
      class: 'absolute top-1/2 hidden w-[3px] h-3 -translate-y-1/2 -translate-x-px rounded-full bg-primary',
    });
    const railStrip = h('div', {
      // touch-none, or the drag belongs to the scroller behind it and the rail
      // gets one frame before the page takes over. h-4 rather than the rail's
      // own 12px, so the strip a finger has to find is the row and not the
      // hairline.
      class: 'relative h-4 flex-1 cursor-pointer touch-none',
      title: 'the session, start to end. Tap or drag to move through it.',
    },
      h('div', { class: 'absolute inset-x-0 top-1/2 h-px -translate-y-1/2 bg-base-content/20' }),
      railTicks, railHere);
    const railA = h('span', { class: 'shrink-0 font-mono text-[9px] tabular-nums text-base-content/30' });
    const railB = h('span', { class: 'shrink-0 font-mono text-[9px] tabular-nums text-base-content/30' });
    const rail = h('div', {
      // STICKY, and the offset is the host's to declare. Framed, this sits
      // above a scroller as a plain flex item and never moves, so top-0 is
      // inert; in document flow the whole pane scrolls and the rail has to
      // clear the host's own sticky chrome, whose height sessionBrief publishes
      // as --chrome-h. One class covers both, and a host that publishes nothing
      // gets 0, which is the correct answer for a host with no chrome.
      class: 'sticky top-[var(--chrome-h,0px)] z-20 hidden shrink-0 items-center gap-1.5 '
        + 'border-b border-base-200 bg-base-100 px-3 py-1.5',
    }, railA, railStrip, railB);

    // ── Placing the ticks ─────────────────────────────────────────────────
    // The span is the RECORD'S, start to end, not the first card to the last:
    // the last card's lead is said before the session's final write, and a rail
    // that ended there would draw the tail as if nothing happened in it.
    //
    // PLACEMENT IS PER TICK, AND THE MIXED CASE IS THE NORM. A card with no
    // instant falls back to its index, spaced evenly, while its dated
    // neighbours are placed by time. That reads as a rare schema-3 path and is
    // not one: every record in the store ends with the renderer's own summary
    // card ("What this session touched"), which carries a sort sentinel rather
    // than a time, so the fallback fires on all 345 of them (measured
    // 2026-09-08 over web-tools-private sessions/).
    //
    // It is correct because that card is LAST, where the fallback puts it at
    // 100%, which is the right end for a summary of the whole. session-render's
    // turns() appends it positionally, and groups() folds a leading meta note
    // into the first card rather than leaving it a card of its own, so an
    // undated card in the MIDDLE is the shape that does not occur. It is also
    // the shape this would get wrong: placed by index among neighbours placed
    // by time, it would land wherever the count fell rather than where it
    // happened. Held by tools/test/session-export-open.test.mjs.
    //
    // Where NOTHING is dated the whole rail spaces evenly and drops its clock
    // labels: even spacing claims nothing about time, which is honest, where a
    // label over it would not be.
    const ms = (v) => { const t = Date.parse(v || ''); return Number.isFinite(t) ? t : 0; };
    const cardAt = line.map(c => ms(c.at));
    const spanA = ms(record?.started) || cardAt.find(Boolean) || 0;
    const spanB = ms(record?.ended) || [...cardAt].reverse().find(Boolean) || 0;
    const spanMs = spanB > spanA ? spanB - spanA : 0;
    const pctOf = (i) => (spanMs && cardAt[i]
      ? Math.max(0, Math.min(100, ((cardAt[i] - spanA) / spanMs) * 100))
      : (line.length > 1 ? (i / (line.length - 1)) * 100 : 0));
    const pcts = line.map((_, i) => pctOf(i));
    const clock = (at) => new Date(at).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' });

    if (line.length > 1) {
      rail.classList.remove('hidden');
      rail.classList.add('flex');
      tickEls.push(...pcts.map(pct => h('div', {
        class: 'absolute top-1/2 w-px h-2 -translate-y-1/2 bg-base-content/30',
        style: 'left:' + pct + '%',
      })));
      railTicks.append(...tickEls);
      if (spanMs) { railA.textContent = clock(spanA); railB.textContent = clock(spanB); }
    }

    // ── Where the reader is standing ──────────────────────────────────────
    // The rail's own bottom edge is the reference line, and it is the right one
    // in BOTH layouts without either being detected: framed it is the top of
    // the scroller, and in flow the rail is stuck under the chrome, so it is the
    // top of what can be seen. Measured in viewport coordinates for the same
    // reason: `scrollTop` would need to know which box is doing the scrolling.
    //
    // The card the line falls INSIDE, not the last one that started above it.
    // Those differ exactly at a boundary, which is where a list scrolled to its
    // end sits.
    let hereFrame = 0;
    const readHere = () => {
      hereFrame = 0;
      if (!rail.isConnected || line.length < 2) return;
      const y = rail.getBoundingClientRect().bottom + 1;
      let cur = boxes.length - 1;
      for (let i = 0; i < boxes.length; i++) {
        if (boxes[i].getBoundingClientRect().bottom > y) { cur = i; break; }
      }
      railHere.classList.remove('hidden');
      railHere.style.left = pcts[cur] + '%';
    };
    // One read a frame. A scroll fires far faster than the layout it is asking
    // about can change, and every read here is a forced reflow.
    const onScroll = () => {
      if (hereFrame) return;
      hereFrame = requestAnimationFrame(readHere);
    };
    // CAPTURE ON THE WINDOW, which is the only listener that catches both
    // layouts: a scroll event does not bubble, so a listener on `el` sees the
    // inner scroller and never the page. It outlives this element, which is
    // exactly the leak the panel handlers above are hung on `el` to avoid, so
    // this one is handed back for the host to release.
    window.addEventListener('scroll', onScroll, true);
    requestAnimationFrame(readHere);

    // Tap the rail, land on that card. The one control here that can move a
    // reader a long way in one gesture. Held down it scrubs, and a tap is the
    // same gesture with no movement in it, so there is one code path and no
    // separate click handler: a click after a drag would jump a second time, to
    // wherever the finger was lifted.
    //
    // scrollIntoView rather than arithmetic on a scrollTop, because it is the
    // one call that finds the right scroller by itself. The cards carry
    // scroll-mt for the sticky rail above them.
    // ── What the line is not showing, counted exactly ───────────────────
    // The transcript card CARRIES its dropped count, because its text was cut
    // in the data at 240 characters. These lines are cut by CSS at one line and
    // two, which is responsive: the same reply hides 54 characters at 1280 and
    // 208 at 390. So the cut point is measured off the rendered line, the text
    // is truncated there, and the count is what the truncation left behind.
    //
    // BINARY SEARCH ON A RANGE, not a walk. The question is where the clamp cut,
    // and a Range's last client rect answers "is offset k still on a visible
    // line" directly; halving gets there in about eight probes rather than one
    // per character. Every probe is a READ and the pass writes nothing until it
    // is done, so the whole thing is one reflow: measured 2026-09-08 at 19ms
    // over 188 lines of a 47-card session, at both 1280 and 390.
    //
    // CHIP_ROOM is how much of the last line the chip needs, in characters of
    // the text it follows. The chip is 9.5px mono where the line is 13px sans,
    // so its widest form here is about a dozen of them; 16, with a snap back to
    // the last word boundary, keeps it on the line without a visible gap.
    const CHIP_ROOM = 16;
    ...
    // The offset the clamp cut at, or -1 where this browser cannot say.
    const visibleTo = (el) => {
      if (!el.getClientRects) return -1;
      const bottom = el.getBoundingClientRect().bottom;
      const nodes = textNodes(el);
      if (!nodes.length) return -1;
      const full = nodes.reduce((n, x) => n + x.length, 0);
      const at = (k) => {
        let left = k;
        for (const nd of nodes) { if (left <= nd.length) return [nd, left]; left -= nd.length; }
      ...
      while (lo < hi) {
        ...
        r.setEnd(nd, off);
        // The LAST rect, since a Range over wrapped text reports one per line.
        // A half-pixel of slack, because a line box's bottom and the box's own
        // are equal to within rounding on the last visible line.
        const rects = r.getClientRects();
        const bot = rects.length ? rects[rects.length - 1].bottom : 0;
        if (bot <= bottom + 0.5) lo = mid; else hi = mid - 1;
      }
      return lo;
    };
    // Truncate to `keep` characters in place, so the bold runs on a reply line
    // survive the cut. Emptied nodes are REMOVED rather than left blank: an
    // empty span from richLine still takes the word-space around it.
    const cutTo = (el, keep) => {
      let left = keep;
      for (const n of textNodes(el)) {
        if (left >= n.length) { left -= n.length; continue; }
        n.data = n.data.slice(0, left);
        left = 0;
      }
      for (const n of textNodes(el)) if (!n.data) n.remove();
      for (const kid of [...el.querySelectorAll('*')]) if (!kid.textContent) kid.remove();
    };
    // Cut at `to`, snapped back to the last word boundary within reach, so a
    // line ends on a word rather than mid-syllable.
    const wordCut = (text, to) => {
      const at = Math.max(0, to);
      const space = text.lastIndexOf(' ', at);
      return space > at - 24 ? space + 1 : at;
    };
    // Truncate to `m.keep` and hang the chip off the end. Idempotent against an
    // already-placed mark, which is what lets the correction below re-cut: the
    // chip and its lead-in come off first, so the count is of the line's own
    // text either way.
    const place = (m) => {
      m.c.chip.remove();
      cutTo(m.c.el, m.keep);
      m.c.chip.textContent = '+' + (m.full - m.keep).toLocaleString() + ' chars';
      m.c.chip.hidden = false;
      m.c.el.append(document.createTextNode('\u2026 '), m.c.chip);
    };
    const measureChips = () => {
      if (!el.isConnected || !chips.length) return;
      // Restored first, every time. A previous pass CUT these lines, so asking a
      // truncated line whether it overflows answers no on every resize after the
      // first and the chips would never move again.
      for (const c of chips) {
        c.el.replaceChildren(...c.orig.map(n => n.cloneNode(true)));
        c.chip.hidden = true;
      }
      const over = chips.filter(c => overflows(c.el));
      if (!over.length) return;
      // One read pass, then one write pass, so the reflow is not paid per line.
      const marks = over.map(c => ({ c, full: c.el.textContent.length, to: visibleTo(c.el) }));
      const placed = [];
      for (const m of marks) {
        // A line whose VISIBLE part is no longer than the room the chip needs
        // cannot be cut for one: there would be nothing left in front of it.
        // Unreachable on a genuinely overflowing line, which by definition fills
        // its clamp, and the guard is for the degenerate case rather than a
        // measurement this trusts.
        if (m.to <= CHIP_ROOM) continue;              // nothing to cut, or no layout
        m.keep = wordCut(m.c.el.textContent, m.to - CHIP_ROOM);
        if (m.full - m.keep <= 0) continue;
        place(m);
        placed.push(m);
      }
      // ── And a correction, because CHIP_ROOM is an estimate ───────────────
      // How much of a line a chip needs is a width in characters of whatever
      // font that line is set in, and the ask is a size larger than the reply.
      // Estimated too tight, the chip wraps to a line the clamp then hides, so
      // the row shows a truncated sentence and nothing saying it was truncated.
      // Rather than pad the estimate until nothing wraps, which costs every
      // line the width of the worst case, the pass ASKS: any line still
      // overflowing gets another bite, twice.
      for (let round = 0; round < 2 && placed.length; round++) {
        const still = placed.filter(m => overflows(m.c.el));
        if (!still.length) break;
        for (const m of still) {
          m.keep = wordCut(m.c.el.textContent, m.keep - CHIP_ROOM);
          place(m);
        }
      }
    };
    // After the first layout, and again whenever the column resizes. Debounced
    // to a frame, since a drag fires the observer far faster than a reflow the
    // pass is going to force anyway.
    let measureFrame = 0;
    const remeasure = () => {
      if (measureFrame) return;
      measureFrame = requestAnimationFrame(() => { measureFrame = 0; measureChips(); });
    };
    requestAnimationFrame(remeasure);
    // Observed on the SCROLLER rather than on the window: this list is mounted
    // inside a swiper slide and a deck panel as well as on a page, so its column
    // changes width without the window doing anything.
    const sizeObs = typeof ResizeObserver === 'function' ? new ResizeObserver(remeasure) : null;
    sizeObs?.observe(scroll);

    // ── Hovering a row lights its tick ───────────────────────────────────
    // The join the list and the rail were missing: the rail said where the
    // reader was SCROLLED to, and nothing said where the row under the pointer
    // sat in the session. On a bursty rail that is the interesting question,
    // since the row two lines down can be four hours later.
    //
    ...
    // rule at all; this file already carries that scar as the dead-opacity
    // gate. The colours come off the theme's own custom properties, which is
    // the same route the closing-state disc takes.
    //
    // DISTINCT FROM THE POSITION MARKER, deliberately. That one is primary,
    // 3px and rounded, and it answers "where am I"; this is a full-strength
    // neutral at the tick's own width, and it answers "where is that row". Two
    // primaries would have made the rail say one thing twice.
    //
    // enter and leave, NOT over and out: those bubble from the row's own
    // children and would fire on every crossing inside it. And the panel is
    // appended INTO the box, so a pointer that moves off the line and into the
    // panel it opened has not left the row: the tick stays lit while the reader
    // reads, which is the behaviour a lit tick is promising.
    const lightTick = (i, on) => {
      const t = tickEls[i];
      if (!t) return;
      t.style.background = on ? 'var(--color-base-content, currentColor)' : '';
      t.style.height = on ? '12px' : '';
      t.style.width = on ? '2px' : '';
    };
    boxes.forEach((box, i) => {
      box.addEventListener('pointerenter', () => lightTick(i, true));
      box.addEventListener('pointerleave', () => lightTick(i, false));
      // Focus too, so a keyboard walking the list gets the same answer. It
      // rides focusin/focusout because the focus lands on a LINE inside the
      // box, not on the box itself.
      box.addEventListener('focusin', () => lightTick(i, true));
      box.addEventListener('focusout', () => lightTick(i, false));
    });

    const jump = (ev) => {
      const b = ev.currentTarget?.getBoundingClientRect();
      if (!b?.width || !boxes.length) return;
      const pct = ((ev.clientX - b.left) / b.width) * 100;
      let best = 0;
      pcts.forEach((p, i) => { if (Math.abs(p - pct) < Math.abs(pcts[best] - pct)) best = i; });
      boxes[best]?.scrollIntoView({ block: 'start', behavior: 'auto' });
    };
    let railHeld = false;
    railStrip.addEventListener('pointerdown', (ev) => {
      railHeld = true;
      railStrip.setPointerCapture?.(ev.pointerId);
      jump(ev);
    });
    railStrip.addEventListener('pointermove', (ev) => { if (railHeld) jump(ev); });
    const railRelease = (ev) => {
      railHeld = false;
      railStrip.releasePointerCapture?.(ev.pointerId);
    };
    railStrip.addEventListener('pointerup', railRelease);
    railStrip.addEventListener('pointercancel', railRelease);
    ...
    el.append(rail, scroll);
    // `destroy` releases the one listener that is not hung on an element inside
    // `el`: see the rail's own note. Everything else here dies with the tree.
    // A host that never calls it leaks one scroll handler per mount, which on a
    // swiper of sessions is a handler per slide visited.
    return {
      el,
      // Re-run the chip pass by hand. The ResizeObserver above covers the
      // normal case, which is the column changing width; this is for the two it
      // cannot see, a web font landing after first paint and a host that mounts
      // the list detached and attaches it later. jsdom has no layout at all, so
      // it is also how the tests drive the pass.
      measure: measureChips,
      destroy: () => {
