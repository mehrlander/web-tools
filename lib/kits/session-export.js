...
(() => {
  ...
  function index(record, opts = {}) {
    ...
    // ── The chip, and what it is measured against ────────────────────────
    ...
    //
    // THE GUTTER IS CONSTANT AND THE CHIP SITS IN IT, which is what makes the
    // count exact rather than circular. Inline after the text, the chip's own
    // box would narrow the line, hiding more characters, widening the number,
    // narrowing the line again. Absolutely placed in a padding-right that
    // does not vary with the digits, the layout the count describes is the
    // layout the reader sees. The gutter is removed again from any line that
    // did not overflow, so a short line is not paying 76px for a chip it
    // never gets.
    //
    // It hangs at the BOTTOM right, which is the last visible line of either
    // clamp, and that is where the text it follows ends: a line only gets a
    // chip when it overflows, and an overflowing line fills its width, so the
    // chip lands after the last word rather than floating away from it.
    // INSIDE THE LINE, not in a wrapper around it. A wrapper would have become
    // the grid item and pushed the text a level down, and this row's alignment
    // is the point of its grid: two glyphs in one gutter, two texts on one
    // edge, every part a direct child. An absolutely placed child is out of
    // flow, so it neither joins the clamped text nor changes what the grid
    // holds. The line takes `relative` to be what the chip is placed against.
    const withChip = (el) => {
      const chip = CR_CHIP(0, false, 'Read the rest of this line');
      chip.hidden = true;
      chip.classList.add('absolute', 'bottom-0', 'right-0', 'not-prose');
      el.classList.add('relative');
      el.append(chip);
      // The gutter is an inline STYLE, not a utility class, and the ask is why:
      // it carries its own `style.padding` for the blue fill, and an inline
      // declaration beats any class, so a `pr-*` utility was silently dropped
      // there and the chip sat on top of the words. Held against whatever
      // padding the line already had rather than replacing it.
      chips.push({ el, chip, pad: el.style.paddingRight });
      return el;
    };
    ...
    // ── What the clamp hid, counted exactly ──────────────────────────────
    ...
    // 208 at 390. So the number is measured off the rendered line, and it has
    // to be re-measured whenever the column changes width.
    //
    ...
    //
    // TWO PASSES, and the second is what makes the count exact. Pass one asks
    // which lines overflow with no gutter reserved. Only those get the gutter
    // and the chip, and only those are measured again, now against the layout
    // the reader will actually see. Measuring once with the gutter always
    // reserved would report a chip on lines that only overflowed because of the
    // space held for it.
    // 5.5rem holds the widest chip this can print, `+999,999 chars` at 9.5px
    // mono plus its own padding. Constant, which is the whole point: a gutter
    // that grew with the digits would change what the clamp hides, which would
    // change the digits.
    const GUTTER = '5.5rem';
    const gutter = (c, on) => {
      c.el.style.paddingRight = on
        ? (c.pad ? 'calc(' + c.pad + ' + ' + GUTTER + ')' : GUTTER)
        : c.pad;
    };
    const overflows = (el) => el.scrollHeight > el.clientHeight + 1;
    const hiddenChars = (el, chip) => {
      if (!el.getClientRects) return 0;
      const bottom = el.getBoundingClientRect().bottom;
      // The chip lives INSIDE the line, so its own "+1,234 chars" would be
      // counted as part of the text it is reporting on. Skipped here rather
      // than subtracted, since its length changes with the number it holds.
      const nodes = [];
      const walk = document.createTreeWalker(el, NodeFilter.SHOW_TEXT, {
        acceptNode: (n) => (chip && chip.contains(n)
          ? NodeFilter.FILTER_REJECT : NodeFilter.FILTER_ACCEPT),
      });
      for (let n = walk.nextNode(); n; n = walk.nextNode()) nodes.push(n);
      if (!nodes.length) return 0;
      const full = nodes.reduce((a, n) => a + n.length, 0);
      const at = (k) => {
        let left = k;
      ...
      while (lo < hi) {
        ...
        const b = rects.length ? rects[rects.length - 1].bottom : 0;
        if (b <= bottom + 0.5) lo = mid; else hi = mid - 1;
      }
      return Math.max(0, full - lo);
    };
    const measureChips = () => {
      if (!el.isConnected || !chips.length) return;
      // Pass one: who overflows with nothing reserved.
      for (const c of chips) gutter(c, false);
      const over = chips.filter(c => overflows(c.el));
      for (const c of chips) { if (!over.includes(c)) c.chip.hidden = true; }
      if (!over.length) return;
      for (const c of over) gutter(c, true);
      // Pass two: the exact count, against the gutter the chip sits in.
      const counts = over.map(c => hiddenChars(c.el, c.chip));
      over.forEach((c, i) => {
        const n = counts[i];
        if (!n) { c.chip.hidden = true; gutter(c, false); return; }
        c.chip.textContent = '+' + n.toLocaleString() + ' chars';
        c.chip.hidden = false;
      });
    };
