...
(() => {
  ...
  function index(record, opts = {}) {
    ...
    const peek = h('div', {
      ...
      // the card's own edge.
      // 14rem, down from 26rem on 2026-09-08 with the content it caps. A panel
      // holding a whole exchange needed the room; one comment rarely does, and
      // the old ceiling let a single long reply stand taller than the list it
      // was covering. Still a scroll rather than a clip, because the exception
      // is real: the ceiling is what a reader can take in without leaving the
      // row, not what every reply fits into.
      class: 'hidden absolute left-2 right-2 z-30 max-w-[34rem] max-h-[min(45vh,14rem)] overflow-y-auto '
    ...
    line.forEach((card, ci) => {
      ...
      // The TURN'S own clock, not the card's, which is the card's first turn
      // and so is wrong on the reply by however long the work took.
      if (ask) asTrigger(title, ci + ':a', box, ask.ts || card.ts || '', fillTurn(ask));
      if (answer) asTrigger(replyText, ci + ':r', box, last.ts || '', fillTurn(last));
    ...
    return { el, destroy: () => window.removeEventListener('scroll', onScroll, true) };
  }
