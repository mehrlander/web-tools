...
(() => {
  ...
  // estate keeps on an icon control. Losing that floor was a side effect once:
  // the branch page's heading row was tightened to a 36px band on 2026-09-07,
  // the bump came out of the shared class to do it, and six other doors on
  // five surfaces silently dropped from 44px to 32px on a phone. One of them,
  // the session brief's, then sat 12px short of the three siblings on its own
  // row. So the floor is the default and 'tight' is the opt-out, which puts
  // the declaration on the one surface that asked for it.
