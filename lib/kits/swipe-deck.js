...
(() => {
  ...
  function core(count, render, o = {}) {
    ...
    // AND THE PROPERTY IS NOT ENOUGH, which took a device to find out.
    // `scroll-snap-stop: always` is applied, computes as `always` on every
    // slide, and a fling on iOS still crossed the middle stop of a three-stop
    // track and landed on the far side. So the option keeps the property, for
    // the engines that honour it, and adds the guard that does not depend on
    // one: remember where a gesture started, and when the scroll finally
    // settles more than one slide away, step back to one.
    //
    // A correction rather than a capture. Taking the drag away from the browser
    // means rebuilding momentum, rubber-banding and every accessibility
    // behaviour that comes free with a real scroller, to win a case that is one
    // slide wide. `scrollend` is the moment after momentum, which is the only
    // moment the overshoot is known; where it is missing the settle is read off
    // a quiet scroll instead.
    if (o.snapStop) {
      let from = -1, settle = 0;
      const mark = () => { if (from < 0) from = active(); };
      const landed = () => {
        if (from < 0) return;
        const a = active(), start = from;
        from = -1;
        if (Math.abs(a - start) > 1) go(start + Math.sign(a - start));
      };
      track.addEventListener('touchstart', mark, { passive: true });
      track.addEventListener('pointerdown', mark, { passive: true });
      if ('onscrollend' in track) track.addEventListener('scrollend', landed);
      else track.addEventListener('scroll', () => {
        clearTimeout(settle);
        settle = setTimeout(landed, 140);
      }, { passive: true });
    }
