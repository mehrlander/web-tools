...
//   behavior        how a programmatic go() moves: 'smooth' (default) or
//                   'instant'. See the note by go(); a deck driven by clicks
//                   or keys on a wide screen wants the second.
...
//                   dots.
//   slidePad   the padding classes a SCROLLING slide carries, replacing the
//              default `px-4 py-5 sm:px-8 sm:py-8`. The default is a reading
//              measure and right for a slide that is prose; it is wrong for one
//              whose content sets its own edges, which is what kits/file-deck.js
//              passes a tighter value for. Ignored under `slideScroll: false`,
//              where the slide has no padding to give.
...
//                   rows [{icon, label, onClick?, href?, active?, options?}].
//                   `options` [{label, title?, onClick}] makes the row a verb
//                   with its variants as small buttons on its right, the FAB's
//                   Note row: "Download  pp. 4–5  All" rather than two rows
//                   that repeat the verb. The row itself is then not a button.
...
(() => {
  ...
  const ensureStyle = () => {
    ...
    // A SLIDE'S SCROLLBAR TAKES ITS WIDTH OUT OF ONE SIDE, which is invisible
    // until something in the slide runs to its edges. kits/md-diff.js does: a
    // changed block bleeds past the text so its tinted ring is visible, and
    // with a classic scrollbar the left ring sat 4px from the viewport and the
    // right ring 4px plus the bar. Reported 2026-09-18 from a desktop browser.
    //
    // `stable both-edges` reserves the space on both sides so the slide's
    // content stays centred, and the 640px floor is there because `stable`
    // reserves it whether or not the scrollbar would have taken it: measured
    // unconditional, it cost 20px of width at every size, so a phone whose
    // scrollbar overlays and takes nothing would have paid 10 a side for a bar
    // that does not exist. There is no media query for "this browser draws
    // classic scrollbars", so the width stands in for it.
    //
    // AND ONLY ON A SLIDE THAT SCROLLS (2026-09-18). `stable` reserves the
    // gutter on any slide whose overflow is not visible, and a filled slide is
    // `overflow-hidden`: it never draws a scrollbar, yet it paid the same 10px
    // a side, so a sheet opened in home's submittal reader sat inset from the
    // pane on both edges with nothing to explain the gap. The scrolling slide
    // is the one with `overflow-y-auto`, which is the class the rule keys on.
    //
    // A stylesheet rather than a class, for the reason `scroll-snap-stop` is an
    // inline style below: a class arriving in the DOM from a JS string exists
    // only if the browser build happened to generate a rule for it, and a media
    // query cannot be an inline style at all.
    s.textContent = '.sd-track::-webkit-scrollbar{display:none}'
      + '.sd-slide{scrollbar-width:thin}'
      + '@media (min-width:640px){.sd-slide.overflow-y-auto{scrollbar-gutter:stable both-edges}}';
  ...
  function core(count, render, o = {}) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const section = h('section', {
        class: 'sd-slide h-full w-full shrink-0 snap-center '
          + (scrolls ? 'overflow-y-auto ' + (o.slidePad || 'px-4 py-5 sm:px-8 sm:py-8') + ' '
                     : 'overflow-hidden ')
          + (o.slideClass || ''),
      }, inner);
      // ONE STOP PER GESTURE, for a deck whose middle slide is the point.
      // `snap-mandatory` says where a scroll may COME TO REST, not how far one
      // flick may carry: momentum crosses as many snap points as it has speed
      // for. On a three-stop track that means a normal flick skips the middle
      // reading, which is the one a reader is usually going to, and lands on the
      // far side. `scroll-snap-stop: always` makes each point a wall.
      //
      // An option rather than the default, because the cost is real the other
      // way: a deck of forty file cards is flicked THROUGH, and stopping at
      // every one would make that crawl. A deck of three readings is stepped.
      //
      // An inline style rather than Tailwind's `snap-always`, for the reason
      // md-doc's containment is inline: a class arriving in the DOM from a JS
      // string exists only if the browser build happened to generate a rule for
      // it, and when it has not there is no error anywhere.
      if (o.snapStop) section.style.scrollSnapStop = 'always';
    ...
    // `behavior` lets a caller say how a programmatic move should feel.
    // Smooth is right for a deck a thumb drags, where the animation matches
    // the motion the reader started. It is wrong for one driven by a click or
    // a key on a wide screen: Chrome scales a smooth scroll's duration with
    // the DISTANCE, so the same change of slide takes proportionally longer
    // the wider the track, and a caller with no gesture behind it is left
    // waiting on an animation nobody asked for. `open()` already reached for
    // 'instant' on its start jump for the same reason.
    const go = (i, b) => track.scrollTo({
      left: Math.max(0, Math.min(count - 1, i)) * width(),
      behavior: b || o.behavior || 'smooth',
    });
    // AND THE PROPERTY WAS NEVER THE PROBLEM, which took a side test to find
    // out. A guard sat here from 2026-09-17 to 2026-09-18: remember where a
    // gesture started, and when the scroll settled more than one slide away,
    // step back to one. It never worked on the device it was written for, and
    // pages/probe-snap.html said why. Five tracks of identical content, one
    // variable each, flicked by hand on iOS: the only one that snapped
    // correctly was the one whose slides were all the SAME HEIGHT. Dropping the
    // guard changed nothing. Deferring the caller's height write until the
    // scroll went quiet changed nothing. Writing it once before the drag
    // changed nothing.
    //
    // So the failure is not overshoot to be corrected, it is a scroller whose
    // box the caller keeps resizing, and the fix belongs to the caller: give
    // every slide one height. `snapStop` is the property alone again, honoured
    // where an engine honours it and free where it does not. A guard that
    // fires `go()` into a snap already in flight is two animations on one
    // scrollLeft, which is a way to make things worse, and it was buying
    // nothing.
  ...
  function open(o = {}) {
    ...
    //
    // `--deck-bottom` is the fourth of these and the last to arrive (2026-09-08,
    // for kits/annotate.js docking a reader on a page that has no host at all).
    // Chrome BELOW the content is rarer than a header or a sidebar, but it is
    // the same fact: something is floating there that the reader is meant to
    // keep using, and a takeover reaching the bottom edge puts itself on top of
    // it. The fab launcher is the case (alpineComponents/fab.js, fixed at the
    // bottom-right and painting above everything on purpose), and it landed on
    // the far end's own destination row.
    //
    ...
    const overlay = h('div', { class: 'sd-overlay fixed right-0 bottom-[var(--deck-bottom,0px)] top-[var(--deck-top,0px)] left-[var(--deck-left,0px)] z-[70] overflow-hidden' });
    ...
    // AND IT GOES ENTIRELY ON A DECK OF ONE, with the pager, for the same
    // reason: "1 / 1" is a position within a set of one, which is not a
    // position. See the note by the footer.
    const pill = h('div', { class: (total > 1 ? 'hidden sm:block' : 'hidden')
      + ' shrink-0 rounded-full border border-base-300 bg-base-200 px-3 py-1 font-mono text-xs tabular-nums text-base-content/60' },
    ...
    // `relative z-10` so the header, and a menu hung from it, paint over the
    // track: backdrop-blur makes the header a stacking layer with no z-index,
    // and any positioned element inside a slide (a daisyUI active tab, a
    // sticky row) then painted over the open menu. The index sheet is z-20.
    const header = h('div', { class: 'sd-header row-start-1 relative z-10 flex items-center gap-3 border-b border-base-300 bg-base-100/90 px-4 py-3 backdrop-blur sm:px-6' },
      closeBtn, mark,
    ...
    const menu = (btn, rows, o2 = {}) => {
      ...
      for (const r of rows) {
        if (r.options && r.options.length) {
          const opts = r.options.map(o => {
            const b = h('button', { class: 'btn btn-ghost btn-xs', title: o.title || '' }, o.label || '');
            b.addEventListener('click', (e) => { e.stopPropagation(); box.remove(); if (o.onClick) o.onClick(); });
            return b;
          });
          box.append(h('li', {}, h('div', { class: 'flex items-center gap-2' },
            h('i', { class: 'ph ' + (r.icon || 'ph-dot') }), h('span', {}, r.label || ''),
            h('span', { class: 'ml-auto flex gap-0.5' }, ...opts))));
          continue;
        }
      ...
      // A header ACTION sits in a `display: contents` wrapper (actionWrap), which
      // has no box: measured, it reads as the viewport's left edge, and the
      // menu opened there rather than under its button (seen 2026-09-24 on the
      // PowerShell file deck at desktop width). Hang it from the row that
      // wrapper's children lay out in, offset to the button's own left edge.
      let host = btn.parentElement, dx = 0;
      if (host && getComputedStyle(host).display === 'contents' && host.parentElement) {
        host = host.parentElement;
        dx = btn.getBoundingClientRect().left - host.getBoundingClientRect().left;
      }
      ...
      // this can compute.
      //
      // AND MEASURED AGAIN ON THE NEXT FRAME. A page on the Tailwind browser
      // build writes a class's CSS only after the class first appears in the
      // DOM, so the first menu a session opens can be measured before `absolute`
      // and its width exist. Measured 2026-09-24 on the budget-drs request
      // viewer at 1320px: the first open read an overflow of about 8px against
      // a real 82px and hung 74px off the right edge; the second open, with the
      // CSS in place, was correct. The second read settles on the real box.
      const place = () => {
        ...
        if (over > 0) {
          ...
          box.style.left = Math.max(8 - anchor.left, dx - over) + 'px';
        }
      };
      place();
    ...
    const footer = h('div', { class: 'row-start-4 grid grid-cols-[2.75rem_1fr_2.75rem] items-center gap-4 border-t border-base-300 bg-base-100 px-4 py-3 sm:px-6' },
      prevB, center, nextB);
    // A DECK OF ONE HAS NOTHING TO PAGE. Both arrows are invisible at every
    // index, and the position is a single dot reporting 1 of 1, so the row is
    // 45px saying that there is a set here when there is not. It matters
    // because this container is used for one thing as well as for many: a
    // reader framing a single document or a single embedded app is a deck of
    // one, and it should not advertise siblings it does not have.
    //
    // Hidden rather than omitted, and the grid is why it is safe: every child
    // NAMES its row, so a `display:none` footer collapses row 4 and moves
    // nothing above it. Omitting the element instead would leave `immersive`
    // and the sync below reaching for a node that is not there.
    if (total <= 1) footer.classList.add('hidden');
    ...
    //
    // A SIBLING OF THE OVERLAY, not its child. The overlay has to clip the
    // horizontal slide track, while a handle on its left edge has to straddle
    // that edge. Putting the seam inside overflow-hidden cut off the left half
    // of the grip and forced the percentage badge off to the right as a
    // workaround. Fixed against the same root variables, the seam follows the
    // deck exactly without inheriting its clip, so both affordances can sit on
    // the divider they describe.
    const seam = h('div');
    Object.assign(seam.style, {
      position: 'fixed', left: 'var(--deck-left,0px)',
      top: 'var(--deck-top,0px)', bottom: 'var(--deck-bottom,0px)',
      width: '6px', transform: 'translateX(-50%)', zIndex: '75',
    });
    const syncSeam = () => {
