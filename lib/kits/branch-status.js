...
(() => {
  ...
  // ── The pages a branch can be LOOKED AT through ──────────────────────────
  // The renderable paths out of the compare, which is the one list the row
  // could not draw: the shape digest says a branch changed two .html files and
  // cannot say which, and the paths themselves were fetched, counted and
  // dropped. Storing them is what lets a branch row offer the render link
  // instead of describing the change and leaving the reader to mint one.
  //
  // Removed files are dropped, since nothing renders a deletion, and the cap is
  // the strip's cap rather than the truth's: past a few chips a row says less,
  // and `stats` still carries the honest count beside them.
  //
  // It stores PATHS and not addresses. Which address a path deserves is the
  // framed-app question kits/guide-render.js answers from the repo's own
  // manifest, and a crawl that baked an address in would freeze today's answer
  // into a cache read for weeks.
  function renderablePages(files, cap = PAGES_CAP) {
    return (files || [])
      .filter(f => f && f.status !== 'removed' && /\.html?$/i.test(f.filename || ''))
      .map(f => f.filename)
      .slice(0, cap);
  }
  ...
  const PAGES_CAP = 6;   // renderable pages kept per branch row
  ...
  async function scanBranchLive(gh, branch, main, opts = {}){
    ...
    let unique = [], noBase = false, stats = null, pages = [];
    ...
    try {
      ...
      // Off the same wider range every other count here is drawn from, so a
      // no-merge-base row's chips can name a page the branch itself did not
      // touch. The row says "no merge base" beside them, which is the same
      // caveat its numbers carry.
      pages = renderablePages(fb.files);
    ...
    return {
      ...
      missingPaths: s.missingPaths, stats, pages, group,
    };
  }
  ...
  // ── THE HORIZON AND THE BUDGET ARE TWO NUMBERS ───────────────────────────
  //
  // `cap` used to be one number doing both jobs, and the second job was the
  // accident: `older.slice(0, cap)` truncated the QUEUE, so a branch past the
  // cap got no row at all, not even a carried one, and a branch that fell out
  // of the top `cap` lost the verdict a previous crawl had already paid for.
  // Measured 2026-09-09 at cap 30: web-tools stored 30 rows against 401 older
  // branches, home 30 against 425, and the estate's stranded reading therefore
  // described its freshest fifth while reading as if it described the whole.
  //
  //   keep  the HORIZON: how many older branches keep a row. A row costs
  //         storage in the cache, not a request, since a row the gate carries
  //         is free.
  //   cap   the BUDGET: how many rows this pass may derive live. This is the
  //         request cost and the thing worth rationing, which is what the
  //         setting was named for all along.
  //
  // Coverage now grows monotonically: each pass spends its budget, and what it
  // learns is carried by every later pass rather than falling off the end.
  function scanPriority(b, prior){
    const row = prior.get(b.name);
    // NEVER SCANNED FIRST, and this ordering is the whole reason coverage
    // grows. `mainMoved` marks every carried row as needing a rescan, so a
    // budget spent in list order would re-derive the same freshest rows on
    // every crawl and never reach the tail. A verdict nobody has is worth more
    // than one that main may have nudged, and the nudge only ever moves a
    // branch toward `landed`, never toward `stranded`.
    if (!row || !row.sha) return 0;
    if (row.sha !== b.sha) return 1;             // the branch itself moved
    return 2;                                    // a main-moved rescan
  }

  async function scanOlder(gh, opts = {}){
    const older = opts.older || [];
    const keep = opts.keep ?? older.length;
    ...
    const queue = older.slice(0, keep);
    const cap = opts.cap ?? queue.length;
    const need = queue.filter(b => needsScan(b, prior, mainMoved));
    // Stable within a priority band, so the list's own newest-first order still
    // decides between two branches nobody has scanned.
    const live = need
      .map((b, i) => ({ b, i, p: scanPriority(b, prior) }))
    ...
    const worker = async () => {
      while (i < queue.length){
        ...
        if (!liveSet.has(b.name)) {
          const was = prior.get(b.name);
          // NO PRIOR AND NO BUDGET IS NOT `done`. Once the horizon outgrew the
          // budget, a queue entry could reach here having never been scanned by
          // anyone, and the old shape reported it as a finished row with no
          // `group` at all: every scope test then quietly excluded it, so a
          // branch nobody had looked at read the same as one that had been
          // looked at and matched nothing. It says so instead, and `group` stays
          // absent because there is no verdict to state.
          rows[idx] = was
            ? { ...was, ...b, state: 'done', carried: true }
            : { ...b, state: 'unscanned' };
          onRow(rows[idx]);
          continue;
        }
    ...
    // `pending` is what the budget could not reach this pass: the honest count
    // behind "coverage grows", and the number that should fall to zero after
    // enough quiet crawls. A caller that stores it can say how far along the
    // estate is instead of implying it is finished.
    return { truncated, rows: rows.filter(Boolean), scanned: live.length,
             carried: queue.length - live.length,
             pending: Math.max(0, need.length - live.length),
             beyondHorizon: Math.max(0, older.length - queue.length) };
  }
  ...
  window.BranchStatus = {
    ...
    fileStats, fileClass, fileKind, renderablePages, PAGES_CAP,
