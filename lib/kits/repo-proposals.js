// Proposals: the write-side counterpart to the reads an errand asks for
// (lib/kits/errands.js). An agent session with limited repo scope drops a
...
// lacks, the same trade an errand's read makes, but for a write.
//
// NEVER AUTO-APPLIES. A proposal writes to a repo the agent could not reach,
// so the manual confirm is the whole safety hinge, as it is for every errand
// (kits/errands.js). Nothing in this file applies anything on its own:
...
//
// A FOURTH KIND THAT IS NOT A FILE EDIT, and why it is named rather than
// general. `delete-issue` runs a GraphQL mutation, because GitHub's REST API
// has no issue-delete endpoint at all: the capability exists only in GraphQL,
// and this sandbox cannot POST GraphQL (the proxy serves a pinned set of
// operations). So the browser is the only place in the estate that can do it,
// which is the same trade the other three kinds make, for an act REST cannot
// express.
//
// The tempting shape was one general `graphql-mutation` kind carrying an
// arbitrary document. It is refused, and the reason is a capability
// escalation rather than a taste: anything able to drop a file into
// proposals/pending would then be able to execute ANY mutation with the
// owner's full token. A file proposal is reviewable as bytes, and resolve()
// shows the reviewer exactly what will be written; an arbitrary mutation is
// reviewable only as intent, and a card cannot show what it will do. Naming
// one operation per kind keeps the blast radius equal to what the card can
// actually describe. "Three kinds, all small on purpose" was already the rule;
// this is the same rule applied where the stakes are higher.
//
// NO `deliver` FOR A MUTATION. commit / branch / pr are three ways of landing
// a FILE change, and a PR is the honest route for anything that wants review.
// A deletion has no softer form: it fires or it does not. Validation refuses a
// `deliver` on a mutation kind rather than ignoring it, since accepting it
// would promise a gentleness the operation cannot deliver.
//
// THE STALENESS GUARD, IN THE ONLY CURRENCY AN ISSUE HAS. `expectSha` guards a
// file because a blob sha moves when the bytes do. An issue has no such sha,
// so the analogue is `expectComments` (and optionally `expectTitle`): an issue
// that has gained a comment since the proposal was written is not the issue
// that was proposed for deletion, and someone should look again before it
// goes. Same refuse-by-default-and-record-the-override behaviour as expectSha.
//
// THE TWO GRAPHQL DOCUMENTS ARE NOT COVERED BY THE PRUNED-SCHEMA CHECK.
// tools/build/graphql-schema.mjs discovers queries by running lib/gh-fetch.js
// against a stub window, so a document living in a kit is invisible to it.
// Both were validated by hand against GitHub's published SDL on 2026-08-25 and
// both are valid; that is a dated claim rather than a standing gate, and
// widening the pruner to read kits is the honest fix if more of these appear.
//
...
(() => {
  ...
  const KINDS = ['put-file', 'set-json-field', 'unset-json-field', 'delete-issue'];
  // Kinds that act on the GitHub object graph rather than on a file in a repo.
  // They carry no `path`, resolve to an object rather than to bytes, and take
  // no `deliver`. Named one operation at a time, on purpose; see the header.
  const MUTATION_KINDS = ['delete-issue'];
  ...
  // errands, and for the same reason: gh-store has no delete, so a spent
  ...
  function validate(p) {
    ...
    // A mutation acts on an object in GitHub's graph, so it carries an issue
    // number where a file kind carries a path, and it refuses the delivery
    // menu outright rather than accepting one it cannot honor.
    if (MUTATION_KINDS.includes(p.kind)) {
      if (!Number.isInteger(p.issue) || p.issue < 1) {
        return { ok: false, error: p.kind + ' needs an issue number (a positive integer)' };
      }
      if (p.path !== undefined) return { ok: false, error: p.kind + ' takes no path (it acts on an issue, not a file)' };
      if (p.deliver !== undefined) {
        return { ok: false, error: p.kind + ' takes no deliver: a mutation fires or it does not, and there is no branch or PR form of it' };
      }
      if (p.expectSha !== undefined) {
        return { ok: false, error: p.kind + ' takes no expectSha (an issue has no blob sha; use expectComments)' };
      }
      if (p.expectComments !== undefined && (!Number.isInteger(p.expectComments) || p.expectComments < 0)) {
        return { ok: false, error: 'expectComments must be a non-negative integer when present' };
      }
      if (p.expectTitle !== undefined && typeof p.expectTitle !== 'string') {
        return { ok: false, error: 'expectTitle must be a string when present' };
      }
      if (typeof p.why !== 'string' || !p.why.trim()) {
        return { ok: false, error: 'missing why (a proposal a reviewer cannot read is not reviewable)' };
      }
      return { ok: true };
    }
  ...
  function checks(p, r) {
    ...
    // A mutation's premises are about an object, so its three checks read the
    // same way but resolve differently: the issue exists, it has not moved
    // since the record was written, and deleting it is still something left
    // to do.
    if (MUTATION_KINDS.includes(p.kind)) {
      const it = r.issue;
      add('readable', 'Issue is readable', r.exists ? 'pass' : 'pass',
        r.exists ? '#' + p.issue + ' ' + (it?.state || '').toLowerCase() +
          ', ' + (it?.comments ?? 0) + ' comment' + ((it?.comments ?? 0) === 1 ? '' : 's')
          : 'issue #' + p.issue + ' was not found');
      const gone = !r.exists;
      add('needed', 'Change is still needed', gone ? 'done' : 'pass',
        gone ? 'the issue is already gone' : '');
      if (p.expectComments !== undefined || p.expectTitle !== undefined) {
        const note = !r.stale ? ''
          : (p.expectComments !== undefined && it && it.comments !== p.expectComments
              ? 'now ' + it.comments + ' comment(s), was ' + p.expectComments
              : 'the title has changed');
        add('unchanged', 'Issue unchanged since proposed', r.stale ? 'fail' : 'pass', note);
      } else {
        add('unchanged', 'Issue unchanged since proposed', 'skip',
          'no comment count was recorded when this was written');
      }
      const blocking = list.some(c => c.state === 'fail');
      return { list, blocking, done: gone };
    }
  ...
  // The two GraphQL documents a mutation kind needs. Named operations, because
  // gh-fetch's error path prints the operation name and an anonymous one tells
  // the reader nothing. Both validated against GitHub's published SDL; see the
  // header for why the pruned-schema check does not reach them.
  const Q_ISSUE = `
    query ProposalIssue($owner: String!, $name: String!, $number: Int!) {
      repository(owner: $owner, name: $name) {
        issue(number: $number) { id title state url comments { totalCount } }
      }
    }`;
  const M_DELETE_ISSUE = `
    mutation ProposalDeleteIssue($issueId: ID!) {
      deleteIssue(input: { issueId: $issueId }) { repository { nameWithOwner } }
    }`;

  // The mutation analogue of current(): read the object the proposal names, so
  // the card describes what is about to happen against the target AS IT STANDS
  // rather than as it stood when the record was written. An issue that is
  // already gone reports exists:false, which checks() reads as done rather
  // than as failure, the same way an absent key does for unset-json-field.
  async function currentIssue(p, { GH, token }) {
    const [owner, name] = String(p.repo).split('/');
    const gh = new GH({ token, repo: p.repo, ref: '' });
    if (typeof gh.graphql !== 'function') {
      return { exists: false, error: 'gh-fetch.js is not loaded (graphql missing)' };
    }
    try {
      const d = await gh.graphql(Q_ISSUE, { owner, name, number: p.issue });
  ...
  async function resolve(p, deps) {
    ...
    // A mutation resolves to an OBJECT, not to bytes. There is no after-text
    // to show, so the card gets the issue's identity and the reviewer judges
    // the act rather than a diff.
    if (MUTATION_KINDS.includes(p.kind)) {
      const cur = await currentIssue(p, deps);
      if (cur.error) return { ok: false, error: cur.error };
      const it = cur.issue || null;
      const stale = !!(cur.exists && (
        (p.expectComments !== undefined && it.comments !== p.expectComments) ||
        (p.expectTitle !== undefined && it.title !== p.expectTitle)
      ));
      return { ok: true, exists: cur.exists, issue: it, stale, gone: false };
    }
  ...
  async function apply(p, { GH, token, now, message, force, deliver }) {
    ...
    const base = {
      id: p?.id, kind: p?.kind, repo: p?.repo,
      ...(MUTATION_KINDS.includes(p?.kind) ? {} : { path: p?.path, ref: p?.ref || '' }),
    ...
    if (!MUTATION_KINDS.includes(p?.kind) && !DELIVERIES.includes(mode)) {
      return { ...base, ok: false, error: 'unsupported deliver: ' + mode };
    }
    ...
    if (pre.done) {
      return { ...base, ok: false, done: true,
        error: MUTATION_KINDS.includes(p.kind)
          ? 'already done: issue #' + p.issue + ' no longer exists'
          : 'already applied: the target already carries this exact change' };
    }
    ...
    if ((r.stale || r.gone) && !force) {
      // The refusal names what moved, in the target's own currency: a blob sha
      // for a file, a comment count or title for an issue. A message that
      // reached for expectSha on a mutation threw instead of refusing, which
      // is a guard failing open in the worst possible way.
      const why = MUTATION_KINDS.includes(p.kind)
        ? (p.expectComments !== undefined && r.issue && r.issue.comments !== p.expectComments
            ? 'the issue has changed since this was proposed (expected ' + p.expectComments +
              ' comment(s), found ' + r.issue.comments + ')'
            : 'the issue has changed since this was proposed (the title no longer matches)')
        : r.gone
          ? 'the target no longer exists (this proposal was written against ' + String(p.expectSha).slice(0, 7) + ')'
          : 'the target changed since this was proposed (expected ' + String(p.expectSha).slice(0, 7) +
            ', found ' + String(r.sha || '?').slice(0, 7) + ')';
      return { ...base, ok: false, stale: true, error: why };
    }
    // A mutation has no bytes to write, so it leaves the file path entirely.
    // The guards above have already run: `done` retired it, a failed premise
    // refused it, and a stale issue refused unless forced.
    if (MUTATION_KINDS.includes(p.kind)) {
      try {
        const gh = new GH({ token, repo: p.repo, ref: '' });
        if (typeof gh.graphql !== 'function') {
          return { ...base, ok: false, error: 'gh-fetch.js is not loaded (graphql missing)' };
        }
        const forcedFields = r.stale
          ? { forced: true, expectedComments: p.expectComments ?? null, foundComments: r.issue?.comments ?? null }
          : {};
        await gh.graphql(M_DELETE_ISSUE, { issueId: r.issue.id });
        // An issue's number and title are the only durable trace once the
        // object is gone, so the applied record keeps them. Nothing else can
        // answer "what was #498" afterwards.
        return { ...base, ok: true, issue: p.issue,
          deletedTitle: r.issue?.title || null, deletedComments: r.issue?.comments ?? null,
          ...forcedFields };
      } catch (e) {
        return { ...base, ok: false, issue: p.issue, error: String(e?.message || e) };
      }
    }
    try {
      const gh = new GH({ token, repo: p.repo, ref: p.ref || '' });
      ...
      const verb = p.kind === 'put-file' ? 'Write'
        ...
        : 'Set ' + p.field + ' in';
      const msg = message || `${verb} ${p.path} (proposal ${p.id}) via Web Tools`;
  ...
  window.RepoProposals = {
    PENDING_DIR, APPLIED_DIR, ATTEMPT_DIR, KINDS, JSON_KINDS, MUTATION_KINDS, DELIVERIES,
    pending, validate, toBase64, applyField, unsetField, current, currentIssue, resolve, apply,
