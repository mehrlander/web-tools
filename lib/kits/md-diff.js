...
(() => {
  ...
  // document. No card, no border, no header. A bordered box per change turns a
  // page of prose into a stack of panels, and the reader then reads the panels
  // rather than the text. The block sits at the same left edge and in the same
  // rhythm as the paragraphs around it, and the only chrome is one small line
  // beneath it.
  //
  ...
  // interface and the line beneath is a label that happens to be tappable.
  ...
  function container(chg, i, total) {
    const box = el('section', 'md-diff-change grid grid-cols-[minmax(0,1fr)] rounded-[3px]');
    box.dataset.mdDiffChange = String(i);

    const modes = chg.kind === 'added' ? ['inline', 'new']
                : chg.kind === 'removed' ? ['gone']
                : ['old', 'inline', 'new'];
    const start = modes.indexOf('inline') < 0 ? 0 : modes.indexOf('inline');
    const LABEL = { gone: 'removed', inline: 'inline', old: 'old', new: 'new' };

    const host = el('div', 'flex min-h-0 transition-[height] duration-200');
    // The label line: what you are looking at, and how to look at the others.
    // Right-aligned and small enough to read as a caption rather than a
    // toolbar, which is the whole point of it.
    const foot = el('div', 'flex justify-end gap-3 pt-0.5 text-[11px] leading-none select-none');
    const tabs = modes.map((m) => {
      const b = el('button', 'cursor-pointer transition-opacity', LABEL[m] || m);
      foot.append(b);
      return b;
    });
    box.append(host, foot);

    const paint = (a) => tabs.forEach((b, n) => {
      b.classList.toggle('opacity-70', n === a);
      b.classList.toggle('opacity-25', n !== a);
    });

    // slideScroll: false hands the vertical axis to the content, and takes the
    // slide's padding with it. Both matter here: the padding is what would set
    // a changed block in from the prose around it, and a scroller inside the
    // document is a second place to drag.
    const deck = window.swipeDeck.core(modes.length, (n, slide) => {
      slide.append(VIEWS[modes[n]](chg));
    }, { slideScroll: false, innerClass: 'w-full', slideClass: '' });
    host.append(deck.track);

    // The slide's own box is `h-full` under slideScroll:false, so measuring it
    // answers with the height already set. The content inside it is the only
    // box whose height is the content's.
    const fit = (a) => {
    ...
    tabs.forEach((b, n) => { b.onclick = () => deck.go(n); });
    ...
    if (typeof ResizeObserver === 'function') {
      const ro = new ResizeObserver(() => fit(deck.active()));
  ...
  async function render(host, oldMd, newMd, opts = {}) {
    ...
    // A count and two arrows is the whole of "take me to the next one", so it is
    // sized as a caption rather than as a toolbar. It stays sticky, since its
    // whole job is to be reachable from the middle of a long document.
    const strip = el('div', 'sticky top-0 z-10 flex items-center gap-3 py-1 text-[11px] '
                          + 'leading-none bg-base-100/85 backdrop-blur');
    strip.append(el('span', 'opacity-40 tabular-nums',
    ...
    const goto = (i) => {
      ...
      // A tint rather than a ring. A ring on a block that has no border draws
      // the box this design exists to remove, for the second the reader lands.
      box.classList.add('bg-primary/10');
      setTimeout(() => box.classList.remove('bg-primary/10'), 900);
    };
