...
(() => {
  ...
  function container(chg, i, total) {
    ...
    // THE BLOCK'S BACKGROUND SAYS WHICH VERSION IS ON SCREEN. Without it, a
    // block sitting on `new` is prose with a faint grey wash, which is
    // indistinguishable from prose at a glance: the reader has scrolled past
    // the marks and there is nothing left saying what they are looking at.
    // Red for the old text, green for the new, neutral for the marked reading,
    // which needs no colour of its own because the marks are the colour.
    const TINTS = ['bg-base-200/45', 'bg-error/10', 'bg-success/10'];
    const tint = (m) => {
      box.classList.remove(...TINTS);
      box.classList.add(m === 'old' || m === 'gone' ? 'bg-error/10'
                      : m === 'new' ? 'bg-success/10'
                      : 'bg-base-200/45');
    };
    ...
    function side() {
      ...
      for (const [which, md] of cols) {
        const col = el('div', 'min-w-0 rounded-[3px] px-2 py-1 '
                            + (which === 'old' ? 'bg-error/10' : 'bg-success/10'));
    ...
    function build(layout) {
      ...
      if (paired) { box.classList.remove(...TINTS); side(); }
      else swipe();
    }
  ...
  async function render(host, oldMd, newMd, opts = {}) {
    ...
    //
    // Selection is an OUTLINE, because the background now belongs to the
    // version on screen. Painting selection into the background too would mean
    // one channel carrying two facts, and the reader would lose the version
    // colour on the one block they are working in.
    const SEL = ['ring-1', 'ring-[color-mix(in_oklab,var(--color-primary)_55%,transparent)]'];
    let ci = -1;
    const mark = () => {
      boxes.forEach((b, n) => {
        b.el.classList.toggle(SEL[0], n === ci);
        b.el.classList.toggle(SEL[1], n === ci);
      });
