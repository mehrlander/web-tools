...
//   opts.numbers    a line gutter on any fence long enough to want one, `true`
//                   for the kit's own minimum or a number to set it. Off by
//                   default: see the note by NUM_MIN in kits/md-doc.js for the
//                   measured distribution behind that, and note that a change
//                   container is the narrowest surface the estate has, so the
//                   gutter costs more here than in a full-width reading view.
//                   The handle's `setNumbers` moves it after the render, which
//                   is what a host needs to offer it as a toggle.
...
(() => {
  ...
  let bodySize = null;
  // The document's line-number setting, set once by render() for the same
  // reason bodySize is: it is a property of the document being read, never of
  // one view of one block.
  let bodyNumbers = null;
  function renderMd(md, o = {}) {
    ...
    if (o.contain !== false && window.mdDoc && window.mdDoc.contain) window.mdDoc.contain(box, CONTAIN());
    return box;
  }

  // One object, three call sites, so a fence in a change reads the same whether
  // the reader got to it by swiping, by comparing, or by scrolling past it in
  // the unchanged prose.
  const CONTAIN = () => ({ wrap: true, numbers: bodyNumbers });
  ...
  const inlineView = (chg) => {
    ...
    if (window.mdDoc && window.mdDoc.contain) window.mdDoc.contain(box, CONTAIN());
    return box;
  };
  ...
  function container(chg, i, total) {
    ...
    function side() {
      ...
      for (const [which, md] of cols) {
        ...
        if (window.mdDoc && window.mdDoc.contain) window.mdDoc.contain(body, CONTAIN());
  ...
  async function render(host, oldMd, newMd, opts = {}) {
    ...
    // THE GUTTER IS A CONTROL, NOT A PAGE LOAD, and that is not a preference.
    // A host's first try at a toggle here reloaded the page with a `?numbers=`
    // query, which works when the page is opened at its own URL and cannot work
    // in a toss: the framed document's location is a `blob:` URL, so assigning
    // `location.search` navigates it to a blob the store has never heard of and
    // the frame goes blank. Measured 2026-09-17 on this branch's own demo,
    // which is the route every branch render is handed over on, so a reload
    // control is broken exactly where a reader meets it.
    //
    // Two halves, because the document is built two ways. A change container is
    // rebuilt from its source, which re-renders and re-measures the deck's
    // height for the gutter's extra width; the unchanged prose is rendered once
    // straight into the host and is numbered in place, since re-rendering it
    // would buy nothing and throw away any highlighting it has.
    const setNumbers = (v) => {
      bodyNumbers = v || null;
      const min = bodyNumbers === true ? (window.mdDoc && window.mdDoc.NUM_MIN) || 8
                                       : Number(bodyNumbers) || 0;
      if (window.mdDoc) {
        for (const kid of [...host.children]) {
          if (kid.classList.contains('md-diff-change')) continue;
          if (bodyNumbers) window.mdDoc.number(kid, min);
          else window.mdDoc.unnumber(kid);
        }
      }
      boxes.forEach((b) => b.build(layout));
      return bodyNumbers;
    };
    ...
    return { count: changed.length, goto, next: () => goto(ci + 1), prev: () => goto(ci - 1),
             ...
             setNumbers, get numbers() { return bodyNumbers; } };
  }
