...
(() => {
  ...
  function container(chg, i, total) {
    ...
    const RING = {
      old: 'ring-[color-mix(in_oklab,var(--color-error)_80%,transparent)]',
      gone: 'ring-[color-mix(in_oklab,var(--color-error)_80%,transparent)]',
      new: 'ring-[color-mix(in_oklab,var(--color-success)_80%,transparent)]',
      inline: 'ring-[color-mix(in_oklab,var(--color-base-content)_22%,transparent)]',
    };
    ...
    const idx = el('button', PILL + ' left-1 cursor-pointer px-1.5 py-0.5 '
                           + 'tabular-nums transition-colors', String(i + 1));
    ...
    const select = (on) => {
      idx.classList.toggle('bg-base-300', on);
      idx.classList.toggle('font-medium', on);
      idx.classList.toggle('opacity-100', on);
      idx.classList.toggle('opacity-55', !on);
    };
    select(false);
  ...
  async function render(host, oldMd, newMd, opts = {}) {
    ...
    //
    // A color-mix rather than `bg-primary/6`, for two reasons. daisyUI ships
    // its opacity ramp in tens, so 6 would be composed by Tailwind and is dead
    // on any page that has not registered the theme names as colours
    // (scripts/dead-opacity.py); and a kit has no say in which pages load it.
    // The arbitrary value resolves everywhere.
    const SEL = 'bg-[color-mix(in_oklab,var(--color-primary)_4%,transparent)]';
    let ci = -1;
    const mark = () => {
      boxes.forEach((b, n) => {
    ...
    const away = (e) => {
      ...
      if (ci < 0 || e.target.closest('.md-diff-change') || controls.contains(e.target)) return;
      // (a tap on the index badge lands inside .md-diff-change, so it is
      // already excluded by the closest() test above)
      ci = -1;
      mark();
    };
    ...
    return { count: changed.length, goto, next: () => goto(ci + 1), prev: () => goto(ci - 1),
             controls, setLayout, get layout() { return layout; } };
  }
