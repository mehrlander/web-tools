...
//   mdDiff.revert(old, new, entry)       -> new, with that change (or an array of them) taken back
...
//     opts.menu(entry, i) -> rowMenu items, opened from a change's index
//
...
(() => {
  ...
  //
  // Two rules cover what shared words cannot see, both reported from an edit
  // to a one-word heading ("Surfacing" to "Suacing"), which shares no word
  // with its old self and so came back as a removal and an addition. Blocks of
  // the same KIND (heading, list, quote, table, fence, paragraph) are also
  // scored on shared letter pairs, which a typo barely moves, at a higher
  // floor, since two unrelated paragraphs share plenty of letter pairs. And a
  // run of exactly one old block and one new block of the same kind is one
  // block edited, whatever the scores: nothing else is there for either to be.
  ...
  const BIGRAM_FLOOR = 0.5;  // the same, measured on letter pairs, same kind only
  const kindOf = (t) => {
    const s = String(t || '').replace(/^\s+/, '');
    return /^#{1,6}\s/.test(s) ? 'h' : /^([-*+]|\d+[.)])\s/.test(s) ? 'li' : s[0] === '>' ? 'q'
      : s[0] === '|' ? 't' : /^(`{3,}|~{3,})/.test(s) ? 'f' : 'p';
  };
  const bigrams = (t) => {
    const s = String(t || '').toLowerCase().replace(/[^\w]+/g, ' ').trim(), out = [];
    for (let k = 0; k + 1 < s.length; k++) if (s[k] !== ' ' && s[k + 1] !== ' ') out.push(s.slice(k, k + 2));
    return out;
  };
  const dice = (a, b) => {
    if (!a.length || !b.length) return 0;
    const have = new Map();
    for (const w of a) have.set(w, (have.get(w) || 0) + 1);
    let hit = 0;
    for (const w of b) { const n = have.get(w); if (n) { hit++; have.set(w, n - 1); } }
    return (2 * hit) / (a.length + b.length);
  };

  function pairUp(olds, news) {
    if (!olds.length) return news.map((t, j) => ({ kind: 'added', old: '', new: t, ni: j, at: j }));
    if (!news.length) return olds.map((t, i) => ({ kind: 'removed', old: t, new: '', oi: i, at: 0 }));

    const ok = olds.map(kindOf), nk = news.map(kindOf);
    if (olds.length === 1 && news.length === 1 && ok[0] === nk[0])
      return [{ kind: 'changed', old: olds[0], new: news[0], oi: 0, ni: 0, at: 0 }];
    const ot = olds.map(TOKENS), nt = news.map(TOKENS);
    const ob = olds.map(bigrams), nb = news.map(bigrams);
    const cand = [];
    for (let i = 0; i < olds.length; i++) {
      const have = new Set(ot[i]);
      for (let j = 0; j < news.length; j++) {
        let score = 0;
        if (ot[i].length && nt[j].length) {
          let hit = 0;
          for (const w of nt[j]) if (have.has(w)) hit++;
          score = (2 * hit) / (ot[i].length + nt[j].length);
        }
        if (score >= PAIR_FLOOR) { cand.push({ i, j, score }); continue; }
        // A letter-pair match ranks below any word match: scaled under the floor.
        const b = ok[i] === nk[j] ? dice(ob[i], nb[j]) : 0;
        if (b >= BIGRAM_FLOOR) cand.push({ i, j, score: PAIR_FLOOR * b });
      }
    }
    ...
    // `oi` and `ni` index the run's own olds and news, so align() can say where
    // in each text a change sits; `at` is the new block a removal stands before.
    const events = [];
    for (let j = 0; j < news.length; j++) {
      events.push(forNew[j] >= 0
        ? { at: j, rank: 1, kind: 'changed', old: olds[forNew[j]], new: news[j], oi: forNew[j], ni: j }
        : { at: j, rank: 1, kind: 'added', old: '', new: news[j], ni: j });
    }
    for (let i = 0; i < olds.length; i++) {
      if (forOld[i] >= 0) continue;
      let at = news.length;
      for (let k = i + 1; k < olds.length; k++) if (forOld[k] >= 0) { at = forOld[k]; break; }
      events.push({ at, rank: 0, kind: 'removed', old: olds[i], new: '', oi: i });
    }
    events.sort((x, y) => x.at - y.at || x.rank - y.rank);
    return events.map(({ kind, old, new: nw, oi, ni, at }) => ({ kind, old, new: nw, oi, ni, at }));
  }
  ...
  // Each entry also says WHERE it is, for a host that acts on one (a reject):
  // `oldRange` and `newRange` are { start, end, until } in each text, `until`
  // running to the next block so a block taken out takes its blank line with
  // it; a removal has no new range and carries `insertAt`, where its old text
  // would go back.
  function align(a, b) {
    const reach = (list, whole) => list.map((x, k) => ({ ...x, until: k + 1 < list.length ? list[k + 1].start : whole.length }));
    const A = reach(blocks(a), a), B = reach(blocks(b), b);
    ...
    const flush = () => {
      if (!pending) return;
      const OB = A.slice(pending.a0, ai), NB = B.slice(pending.b0, bi);
      const after = bi < B.length ? B[bi].start : b.length;
      for (const e of pairUp(pending.old, pending.new)) {
        const o = e.oi != null ? OB[e.oi] : null, n = e.ni != null ? NB[e.ni] : null;
        const out = { kind: e.kind, old: e.old, new: e.new };
        if (o) out.oldRange = { start: o.start, end: o.end, until: o.until };
        if (n) out.newRange = { start: n.start, end: n.end, until: n.until };
        if (!n) out.insertAt = e.at < NB.length ? NB[e.at].start : after;
        seq.push(out);
      }
      pending = null;
    };
    for (const part of parts) {
      ...
      if (part.added) {
        (pending ||= { old: [], new: [], a0: ai, b0: bi }).new.push(...B.slice(bi, bi + n).map((x) => x.text));
        bi += n;
      } else if (part.removed) {
        (pending ||= { old: [], new: [], a0: ai, b0: bi }).old.push(...A.slice(ai, ai + n).map((x) => x.text));
        ai += n;
      } else {
        flush();
        for (let k = 0; k < n; k++) {
          const x = B[bi + k];
          seq.push({ kind: 'same', new: x.text, newRange: { start: x.start, end: x.end, until: x.until } });
        }
        ai += n; bi += n;
      }
    }
  ...
  // The new text with one change taken back, from an entry align() returned
  // for these same two texts: a changed block gets its old text again, an
  // added one goes with its blank line, and a removed one returns where it
  // stood. Positions are the entry's, so a caller re-aligns after each revert.
  //
  // An array is one RUN, the entries align() returned between two unchanged
  // blocks, and it goes back as a span: the run is contiguous in both texts,
  // so its stretch of the new text is swapped for its stretch of the old.
  // Reverting its entries one by one would not do, since pairing is by shared
  // words and can cross ("Gone para." pairs with "Added para." below a block
  // that moved), which puts every block back but not in its old order.
  const revert = (a, b, e) => {
    if (Array.isArray(e)) {
      const o = e.filter((x) => x.oldRange), n = e.filter((x) => x.newRange);
      if (!o.length) return revert(a, b, n.length === 1 ? n[0] : { kind: 'added', newRange:
        { start: Math.min(...n.map((x) => x.newRange.start)), until: Math.max(...n.map((x) => x.newRange.until)) } });
      const os = Math.min(...o.map((x) => x.oldRange.start)), ou = Math.max(...o.map((x) => x.oldRange.until));
      if (!n.length) return b.slice(0, e[0].insertAt) + a.slice(os, ou) + b.slice(e[0].insertAt);
      const ns = Math.min(...n.map((x) => x.newRange.start)), nu = Math.max(...n.map((x) => x.newRange.until));
      return b.slice(0, ns) + a.slice(os, ou) + b.slice(nu);
    }
    if (e.kind === 'changed') return b.slice(0, e.newRange.start) + a.slice(e.oldRange.start, e.oldRange.end) + b.slice(e.newRange.end);
    if (e.kind === 'added') return b.slice(0, e.newRange.start) + b.slice(e.newRange.until);
    if (e.kind === 'removed') return b.slice(0, e.insertAt) + a.slice(e.oldRange.start, e.oldRange.until) + b.slice(e.insertAt);
    return b;
  };
  ...
  let bodySize = null;
  let menuFor = null;          // render()'s opts.menu, read by each container's index
  ...
  function container(chg, i, total) {
    ...
    // NO HEIGHT TRANSITION, because there is no longer a height change to
    // ease. The box is sized once to the tallest reading and stays there; see
    // `swipe()` below for what that bought and what it cost. The transition
    // was written for a box that resized on every swap, and animating the
    // scroller's own box under a moving finger is what broke the gesture.
    const host = el('div', 'flex min-h-0');
    ...
    // AND IT OPENS WHAT A HOST OFFERS FOR THE CHANGE (`opts.menu`, a function of
    // the entry and its index returning rowMenu items), since the number is the
    // change's own handle: a reject belongs to the change, not to the document.
    idx.onclick = (ev) => {
      live = true; mine();
      const items = menuFor ? menuFor(chg, i) : null;
      if (items && items.length && window.rowMenu) {
        ev.stopPropagation();
        const r = idx.getBoundingClientRect();
        window.rowMenu.open({ x: r.left, y: r.bottom + 4, items });
      }
    };
    ...
    function swipe() {
      ...
      deck = window.swipeDeck.core(modes.length, (n, slide) => {
        slide.append(VIEWS[modes[n]](chg));
           ...
           // momentum carried straight past it to `new`. The property alone
           // never stopped that on iOS; the equal heights below are what did.
           // It stays because it costs nothing and it is correct where an
           // engine honours it.
           snapStop: true,
           behavior: quick() ? 'instant' : 'smooth' });
      host.append(deck.track);

      // ONE HEIGHT FOR ALL THREE READINGS, and it is the whole fix for the
      // gesture. The box used to follow the reading on screen: `fit(a)` from
      // inside the scroll handler, a fresh `scrollHeight` every animation
      // frame, and a 200ms height transition that therefore began at the
      // midpoint of every flick. On iOS that scroller then crossed the middle
      // stop and landed on the far side, or came to rest BETWEEN two stops,
      // which `scroll-snap-type: mandatory` is supposed to forbid.
      //
      // pages/probe-snap.html measured it by hand on the device, five tracks
      // differing in one thing each. Deferring the write until the scroll went
      // quiet did not fix it. Writing it once before the drag did not fix it.
      // Removing swipe-deck's overshoot guard did not fix it. Giving every
      // slide the same height fixed it, and was the only lane that snapped.
      // So a scroller here is sized once and never resized, and the guard is
      // gone from the kit.
      //
      // The cost is honest and small: a one-line `old` sits in a box built for
      // a three-line `inline`, so short readings carry whitespace below them.
      // That is the price of a gesture that lands where it is aimed, and the
      // readings of one paragraph are usually within a line or two of each
      // other anyway. `added` and `removed` have one reading, so they pay
      // nothing.
      //
      // Every slide is built up front rather than lazily. Three is not a
      // number worth deferring, and a max across slides the deck has not built
      // yet is a max over nothing. `keep` is 2, so nothing is pruned back out.
      for (let n = 0; n < modes.length; n++) deck.build(n);
      ...
      const fit = () => {
        ...
        for (const sec of deck.track.children) {
          ...
          if (content) tall = Math.max(tall, content.scrollHeight);
        }
        // Guarded, because the ResizeObserver below fires on every reflow and
        // an unchanged write is still a write on a box the engine is watching.
        if (tall && host.style.height !== tall + 'px') host.style.height = tall + 'px';
      };
      deck.onSlide((a) => { paint(a); mine(); });
      deck.track.addEventListener('pointerdown', () => { live = true; });
      tabs.forEach((b, n) => { b.onclick = () => { live = true; deck.go(n); mine(); }; });

      paint(start);
      requestAnimationFrame(() => { fit(); if (start) deck.go(start); });
      ...
      // rather than being read once. This is the one write left after mount,
      // and it happens when content settles rather than when a finger moves.
  ...
  async function render(host, oldMd, newMd, opts = {}) {
    ...
    menuFor = typeof opts.menu === 'function' ? opts.menu : null;
    ...
    // `md-diff-doc` is the one stable name on the rendered document, and it is
    // there so chrome OUTSIDE this kit can find the document it belongs to.
    // The handle's `next`/`prev` only reach a host holding the handle, and a
    // page that draws its jump button in a deck header two components away
    // does not: pages/approve.html renders the comparison inside a
    // file-review card inside a deck slide. A class plus an event is the
    // shortest path that does not thread a handle through both.
    // `px-1` IS THE ROOM THE CONTAINERS BLEED INTO. A changed block carries
    // `-mx-2 px-2`, which puts its tinted edge OUTSIDE the text so the block is
    // delineated without being indented. With a host at zero that edge lands
    // exactly on whatever bounds the document, so on a phone the coloured ring
    // ran off both sides of the screen and a reader could not see the box they
    // were swiping. Four pixels is the whole fix: the ring sits 4px inside the
    // host and the text is unmoved, since 4 of padding less 8 of bleed plus 8
    // of the block's own padding puts a changed line exactly where an
    // unchanged one already sits.
    //
    // The kit reserves it rather than each consumer, because the bleed is the
    // kit's decision. pages/approve.html and kits/file-deck.js both mount this
    // and would otherwise each have to know the number.
    host.classList.add('md-diff-doc', 'grid', 'grid-cols-[minmax(0,1fr)]', 'gap-2', 'px-1');
    ...
    // IT NEVER CHANGES WIDTH, because it sits in a floating pill and the pill
    // is anchored on its right edge: anything that resizes the readout slides
    // the whole control under the reader's thumb. Measured 2026-09-18 at 1200:
    // 191px saying "10 changes", 162 at "1 / 10", 168 at "10 / 10", so it
    // shifted twice, once on the first tap and again crossing into two digits.
    //
    // One shape for both states is what fixes the first shift: `– / 10` before
    // a change is claimed says the same thing "10 changes" said, in the form it
    // will take once the reader moves. `tabular-nums` and a `ch` floor fix the
    // second, since with equal-width digits the widest string this document can
    // show is exactly twice its digit count plus ' / '.
    const where = el('span', 'opacity-40 tabular-nums text-right shrink-0');
    ...
    // THE STEP BELONGS WITH THE COUNT. It sat in pages/approve.html's deck
    // header, which is where a reader looks for controls about the FILE: its
    // github links, which reading to show. Stepping change to change is about
    // the COMPARISON, which is what this strip is, and the readout beside it
    // already says which change of how many. Putting them together also gave
    // the header back its title, which at 390px had been squeezed to 38px by
    // ten buttons.
    //
    // It wraps where a container's three stops clamp, since this is the walk
    // through the whole set and the tenth of ten should return to the first.
    const step = el('button', 'btn btn-ghost btn-xs btn-circle shrink-0 -ml-1');
    step.innerHTML = '<i class="ph ph-arrow-down text-sm"></i>';
    step.title = 'Next change';
    step.setAttribute('aria-label', 'Next change');
    step.addEventListener('click', () => goto(ci + 1));
    if (changed.length) controls.append(step);
    if (changed.length) where.style.minWidth = (String(changed.length).length * 2 + 3) + 'ch';
    if (opts.readout !== false) controls.append(where);
    // SHOW ME ALL OF THEM. The document reads as prose by design: an unchanged
    // paragraph is plain and a changed one carries a thin ring, which is right
    // for reading and wrong for the first ten seconds, when the question is
    // where the changes ARE. This lifts every block to the wash the claimed one
    // wears, so the shape of the change is visible at a glance, and drops it
    // again. A toggle rather than a mode: it is a way of looking, not a state
    // the document is in.
    //
    // THE SAME WASH THE CLAIM WEARS, not a fainter one. Half strength was the
    // first try and it was barely there: the point is to answer "where are the
    // changes" across a screen, and a tint you have to look for does not. The
    // claim stays legible because the wash was never its loudest channel: the
    // numbered badge is nearly solid yellow while the wash sits at 8%, so with
    // every block lit the badge is still the only one of them that is.
    let lit = false;
    const spot = el('button', 'btn btn-ghost btn-xs btn-circle shrink-0');
    spot.innerHTML = '<i class="ph ph-highlighter text-sm"></i>';
    spot.title = 'Highlight every change';
    spot.setAttribute('aria-label', 'Highlight every change');
    spot.setAttribute('aria-pressed', 'false');
    const relight = () => {
      for (const b of boxes) b.el.classList.toggle(SEL, lit || boxes.indexOf(b) === ci);
      spot.classList.toggle('btn-active', lit);
      spot.setAttribute('aria-pressed', String(lit));
    };
    spot.addEventListener('click', () => { lit = !lit; relight(); });
    if (changed.length) controls.append(spot);
    controls.append(lay);
    ...
    if (changed.length && opts.controls) {
      ...
      // IT FLOATS, rather than taking a row across the top. A full-width bar
      // was right when this kit drew the only chrome a card had; it is wrong
      // beside a deck header that already carries the file's controls, where
      // it reads as a second header and pushes the document down by its own
      // height. Reported 2026-09-18.
      //
      // `sticky` and not `absolute`, because the reader wants the layout
      // toggle wherever they have scrolled to, and a `-mb-` equal to its own
      // height takes back the grid row it would otherwise occupy, so the
      // document begins where it would have begun with no strip at all. The
      // backdrop is what keeps the line behind it readable, since the prose
      // now runs underneath.
      //
      // `--md-diff-top` is the strip's one host contract: how far down the
      // host's top is. A page with a sticky header would otherwise have the
      // strip slide underneath it and disappear.
      const strip = el('div', 'sticky z-20 -mb-7 ml-auto w-fit flex items-center gap-2 '
                            + 'rounded-full border border-base-300 bg-base-100/90 '
                            + 'px-2 py-1 shadow-sm backdrop-blur');
      strip.style.top = 'var(--md-diff-top, 0.25rem)';
    ...
    const mark = () => {
      boxes.forEach((b, n) => {
        b.el.classList.toggle(SEL, lit || n === ci);
    ...
    // THE SAME JUMP, FOR CHROME THAT CANNOT HOLD THE HANDLE. `next` and `prev`
    // are for a host that called render() and kept what it returned. A page
    // whose jump button lives in a deck header, with a component boundary in
    // between, has the element and not the handle, so it dispatches here:
    //
    //     doc.dispatchEvent(new CustomEvent('md-diff:jump', { detail: { d: 1 } }))
    //
    // Wrapping rather than clamping, because this is the jump the reader uses
    // to walk the whole set and the tenth of ten should return to the first
    // rather than stop dead. That is the opposite of a container's stops,
    // which clamp; the two are different journeys.
    host.addEventListener('md-diff:jump', (e) => {
      const d = (e.detail && e.detail.d) || 1;
      goto(ci + (d < 0 ? -1 : 1));
    });
    ...
    // that block's stops.
    //
    // ON SCREEN IS THE GATE FOR UP AND DOWN, not the claim. The claim was the
    // gate for both until 2026-09-18, and it meant a reader who had clicked the
    // document and pressed Down got a scroll: the keys only started working
    // once they had tapped a block, which is the move the keys were supposed to
    // save them. Down with nothing claimed now lands on the first change.
    //
    // A deck mounts three of these at once, so "is this the one" has to be
    // answered by geometry rather than by the claim: every slide is in the tree
    // and laid out, and only the active one overlaps the viewport.
    const onScreen = () => {
      const r = host.getBoundingClientRect();
      return r.width > 0 && r.left < window.innerWidth && r.right > 0;
    };
    // IN THE CAPTURE PHASE, and it stops what it handles. kits/swipe-deck.js
    // binds Left and Right on `window` to step the DECK, and both listeners
    // hear the same event: `preventDefault` does not stop a sibling listener,
    // and the deck's was registered first (it opened before this document
    // rendered), so it ran first and swiped the whole deck out from under a
    // reader who was walking one block's readings. Capture runs before any
    // bubble listener whatever the registration order, so this one decides.
    const keys = (e) => {
      if (!host.isConnected) { document.removeEventListener('keydown', keys, true); return; }
      if (e.metaKey || e.ctrlKey || e.altKey) return;
      // Not while someone is typing, which includes a field a host put on the
      // page around this document.
      const t = e.target;
      if (t && (t.isContentEditable || /^(INPUT|TEXTAREA|SELECT)$/.test(t.tagName || ''))) return;
      // UP AND DOWN WALK THE CHANGES, left and right walk one change's stops.
      // That is the same split the screen has: the readings of a block sit on
      // the horizontal axis and the blocks sit on the vertical one, so the keys
      // read as the directions they move in rather than as a mapping to learn.
      const v = e.key === 'ArrowDown' ? 1 : e.key === 'ArrowUp' ? -1 : 0;
      if (v) {
        if (!changed.length || !onScreen()) return;
        e.preventDefault();
        e.stopPropagation();
        // From nothing, Down opens on the first change and Up on the last,
        // which is what each key means before there is a position to move from.
        goto(ci < 0 ? (v > 0 ? 0 : changed.length - 1) : ci + v);
        return;
      }
      const d = e.key === 'ArrowRight' ? 1 : e.key === 'ArrowLeft' ? -1 : 0;
      // Left and right stay keyed to the claim, because with none there is no
      // block whose readings they would walk, and the deck's own use of them is
      // then the right one.
      if (!d || ci < 0 || !onScreen()) return;
      e.preventDefault();
      e.stopPropagation();
      boxes[ci].step(d);
    };
  ...
  // needDiff is exported so a card counting changed lines loads jsdiff through
  // the one loader, and the counts come from the engine the rendered diff uses.
  window.mdDiff = { blocks, align, changes, revert, runs, mark, render, needDiff, _textOf: textOf };
})();
