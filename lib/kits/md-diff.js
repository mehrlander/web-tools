...
(() => {
  ...
  function container(chg, i, total) {
    const box = el('section', 'md-diff-change relative grid grid-cols-[minmax(0,1fr)] '
                            + 'rounded-[4px] px-2 -mx-2 py-1 bg-base-200/45 '
  ...
  async function render(host, oldMd, newMd, opts = {}) {
    ...
    strip.append(where, el('div', 'grow'), prev, next);
    ...
    const mark = () => {
      boxes.forEach((b, n) => {
        b.classList.toggle('bg-base-200/45', n !== ci);
        b.classList.toggle('bg-primary/10', n === ci);
      });
    ...
    const goto = (i) => {
      ...
      mark();
      boxes[ci].scrollIntoView({ block: 'center', behavior: 'smooth' });
    };
