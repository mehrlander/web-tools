...
(() => {
  ...
  function container(chg, i, total) {
    ...
    // Selected, the index fills in the same primary the block's background
    // uses, so the two say one thing rather than two.
    const SEL_IDX = 'bg-[color-mix(in_oklab,var(--color-primary)_18%,transparent)]';
    const select = (on) => {
      idx.classList.toggle(SEL_IDX, on);
      idx.classList.toggle('opacity-100', on);
      idx.classList.toggle('opacity-80', !on);
      idx.style.borderColor = on
