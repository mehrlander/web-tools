...
// 25-50K tokens to 10-15K. app/index.html is the one page this
...
(() => {
  ...
  const FLOOR = {
    ...
    'alpineComponents/fab.js':      'The floating action button: render/inspect drawer, branch scan, export.',
  ...
  const MACHINERY = new Set(['kits/brief.js', 'kits/io.js', 'kits/export.js', 'build.js',
                             'kits/annotate.js', 'kits/dictate.js', 'kits/peek.js']);

  // A brief over this many tokens of own modules is refused: the reader it is
  // for cannot take it, and copying it silently is worse than saying so. The
  // estimate is read off gh-boot's byte ledger (window.__ghFiles) before any
  // fetch, so the FAB can show the size on the row. Until 2026-09-05 the gate
  // was a path regex that matched the app at its old address and nothing at
  // its new one, and the LLM row on app/index.html copied 2.7MB.
  const BRIEF_CAP = 120000;
  ...
  const plan = (opts = {}) => {
    ...
    const files = opts.files || window.__ghFiles || {};
    ...
    // Own modules only: the page's own source is not in the ledger until it
    // is fetched, and it is the smaller part on every page that matters here.
    const bytes = own.reduce((n, m) => n + ((files['lib/' + m.path] || {}).bytes || 0), 0);
    ...
    return {
      ...
      own, floor, reads,
      vendor: vendors(),
      bytes, tokens: est, cap: BRIEF_CAP,
      wholeLib: est > BRIEF_CAP,
    };
  };
  ...
  const assemble = async (opts = {}) => {
    ...
    if (p.wholeLib && !opts.force) {
      throw new Error('This page\'s own modules run to ~' + Math.round(p.tokens / 1000) + 'K tokens, over the '
        + Math.round(BRIEF_CAP / 1000) + 'K cap. Pick a Region instead (the FAB\'s take grid), or pass force.');
    }
  ...
  const stageUrl = (opts = {}) => {
    ...
    let u = 'https://mehrlander.github.io/web-tools/app/' +
  ...
  window.brief = { plan, assemble, copy, stageUrl, FLOOR, BRIEF_CAP };
})();
