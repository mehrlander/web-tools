...
(() => {
  ...
  const buildStore = (dbName, storeName) => {
    ...
    // Cached connections must yield to version bumps — another store thunk
    // layering a sibling store onto this db, or another tool on the origin,
    // reopens at version+1 and an open connection with no versionchange
    // handler would block that upgrade forever. Close and drop the cache so
    // the next call here reopens at the new version.
    const adopt = (db) => {
    ...
    const ensureDb = () => new Promise((resolve, reject) => {
      ...
      req.onsuccess = () => {
        const db = req.result;
        if (db.objectStoreNames.contains(storeName)) { resolve(adopt(db)); return; }
        ...
        up.onsuccess = () => resolve(adopt(up.result));
      };
    });

    return (txMode, callback) => {
      // One retry: a versionchange may close the cached connection between
      // the promise settling and the transaction starting.
      const run = (attempt) => {
        dbp ??= ensureDb();
        return dbp.then((db) => {
          try {
            ...
            if (e.name === 'InvalidStateError' && attempt === 0) {
              dbp = null;
              return run(1);
            }
            throw e;
