...
(() => {
  ...
  const manifest = json => {
    ...
    const correspondence = (Array.isArray(m.correspondence) ? m.correspondence : [])
      ...
      .map(c => ({ repo: c.repo, area: c.area || c.repo, note: c.note || '',
    ...
    const localAreas = (Array.isArray(m.local_areas) ? m.local_areas : [])
      ...
      .map(a => ({ name: a.name, status: a.status === 'local-only' ? 'local-only' : 'unresolved',
        shape: a.shape || '', policy: a.policy || '' }));
    return { asOf: m.as_of || '', root: m.root || '', observations: m.observations || '',
      correspondence, localAreas };
  };
  ...
  const place = (rel, m) => {
    for (const c of m.correspondence) {
      ...
      if (!hit) continue;
      if (c.installs === null) return { area: c.area, installs: null, note: c.note };
      if (c.installs === '') return { area: c.area, installs: '', note: c.note };
      return { area: c.area, installs: c.installs + (c.repo.endsWith('/') ? rel.slice(c.repo.length) : ''), note: c.note };
    }
    return null;
  };
  ...
  const groups = (items, m) => {
    ...
    return [...areas.entries()]
      ...
      .map(([area, units]) => {
        ...
        return { area, installs: c.installs, note: c.note || '',
