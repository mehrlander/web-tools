  ...
  const JsonExplorer = {
    mount(container, options = {}) {
      ...
      const renderSchema = () => {
        ...
        // One compact line, not a row of stat tiles (skills/daisy-alpine rule 1).
        const metrics = h('div', { 'data-schema-summary': '', class: 'font-mono text-base-content/70' },
          [fmtBytes(profile.byteSize),
           `${profile.nodeCount.toLocaleString()} nodes`,
           `depth ${profile.maxDepth}`,
           `${profile.collections.length} ${profile.collections.length === 1 ? 'collection' : 'collections'}`,
          ].filter(Boolean).join(' · ')
        );
