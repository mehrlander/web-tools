...
// browser runs; scripts/annotate/ops.py is what a session runs, and it is
...
// scripts/annotate/materialize.py are compared byte for byte in
...
(() => {
  ...
  // scripts/annotate/materialize.py reads. Stated only where the document
  ...
  // since scripts/annotate/ops.py is stdlib Python with no markdown parser
  ...
  // RENDERED WHITESPACE IS NOT ALWAYS THE SOURCE'S, in two ways that each cost
  // a whole document before 2026-09-24. A soft break inside a list item is a
  // newline plus indentation in the source and a bare newline in the render,
  // and a line break inside a code span renders as a single space. Either way
  // the exact search fails, and a short run of the node can then match much
  // further on and strand every node between: docs/loader.md mapped 43% of its
  // text and docs/SNAGS.md 2%. So a node is split at its line breaks first (a
  // line break renders the same whether or not it sits in a node of its own),
  // and a run with whitespace in it is also sought with each whitespace run
  // matching any, taking whichever match comes first.
  function splitLines(root) {
    const w = root.ownerDocument.createTreeWalker(root, 4 /* SHOW_TEXT */);
    const nodes = [];
    for (let n = w.nextNode(); n; n = w.nextNode()) if (n.data.includes('\n') && n.data.trim()) nodes.push(n);
    for (let node of nodes) {
      let i = node.data.indexOf('\n');
      while (i >= 0) {
        if (i > 0) node = node.splitText(i);
        const ws = /^\s+/.exec(node.data)[0].length;
        if (ws >= node.data.length) break;
        node = node.splitText(ws);
        i = node.data.indexOf('\n');
      }
    }
  }
  // `>` rides with the whitespace: a break inside a block quote continues the
  // next line after its quote marker.
  // And a symbol may have been written as an entity (`&rarr;` renders `→`).
  const ENTITYISH = /[^\x00-\x7f]|[&<>"']/;
  const looseRe = (t) => new RegExp(t.split(/(\s+)/).map((x) => /^\s+$/.test(x) ? '[\\s>]+'
    : [...x].map((c) => {
        const e = c.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        return ENTITYISH.test(c) ? '(?:' + e + '|&#?\\w+;)' : e;
      }).join('')).join(''), 'g');
  // A run this short matches almost anywhere. Found far ahead, it is far more
  // likely a later copy than its own place, and taking it would strand every
  // node between, so it is left unstamped instead.
  const SHORT = 3, FAR = 200;

  // `opts.skip` is a selector for runs that are shown but are not the source:
  // a host drawing removed words beside the text (a <del>), or its own
  // controls inside the rendered tree. They get no offset and do not move the
  // cursor, so they cannot pull the search past the text that follows them.
  function mapText(root, src, from = 0, opts = {}) {
    ...
    for (let node of nodes) {
      const t = node.data;
      if (!t) continue;
      if (opts.skip && node.parentElement && node.parentElement.closest(opts.skip)) continue;
      ...
      let i = src.indexOf(t, cur);
      // Only when the exact search failed or ran far: a regex per node over the
      // rest of the document cost 67ms of a 160k document's render.
      if ((i < 0 || i - cur > FAR) && (/\s/.test(t.trim()) || ENTITYISH.test(t))) {
        const re = looseRe(t);
        re.lastIndex = cur;
        const m = re.exec(src);
        if (m && (i < 0 || m.index < i)) {
          // Same length: whitespace swapped one for one, so offsets inside the
          // node still line up. Otherwise each word is stamped on its own,
          // found inside the match, and the whitespace between goes unstamped.
          if (m[0].length === t.length) { stamp(node, m.index); cur = m.index + t.length; continue; }
          let at = m.index;
          for (const part of t.split(/(\s+)/)) {
            if (!part) continue;
            const piece = node;
            if (part.length < node.data.length) node = node.splitText(part.length);
            if (/^\s+$/.test(part)) continue;
            const k = src.indexOf(part, at);
            // A word found where the source holds an entity is the decoded
            // character's lookalike (`&` of `&amp;`), and a caret after it
            // would sit inside the entity. Refused, like any decoded run.
            const inEntity = k >= 0 && /&#?\w+;/.test(src.slice(k, k + part.length + 10))
              && /&/.test(src.slice(k, k + part.length));
            if (k < 0 || k >= m.index + m[0].length || inEntity) { unmapped++; continue; }
            stamp(piece, k);
            at = k + part.length;
          }
      ...
      if (i < 0 || (t.trim().length <= SHORT && i - cur > FAR)) { unmapped++; continue; }
      stamp(node, i);
      cur = i + t.length;
    }
  ...
  // it cannot run. scripts/annotate/materialize.py is the same projection
  ...
  //
  // DROP removes its span and an insertion places its text. A kept unit that
  // carries a note stands and is reported, since the note may ask for a
  // rewording and inventing the new text is the one thing a projection must
  // not do. So what comes out is a draft plus its brief, not a finished rewrite.
  const BLANK = /\n\s*\n/;
  ...
  function materialize(so, text) {
    ...
    return { out, report: {
      ...
      standing: units.filter(u => u.verdict !== 'DROP' && u.note)
        .map(u => ({ uid: u.uid, verdict: u.verdict || 'KEEP', label: u.label || '', note: u.note,
