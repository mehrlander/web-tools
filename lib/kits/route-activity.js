...
//
//   routeActivity.routesByFile(manifest)          -> Map path -> [route keys]
...
(() => {
  ...
  // still dates the row when it is the only file there is; what it loses is
  ...
  function routesByFile(manifest) {
    const m = new Map();
  ...
  function rowFor(route, { byFile, touches, branches, shell }) {
    const files = list(route.files)
      .filter(p => p !== shell)
      .map(p => {
        const keys = byFile.get(p) || [];
    ...
    // touches a narrow file, and merely NEAR it when its only hit is a file
  ...
  // avoid making. So a row dated by its OWN narrow file outranks every
  ...
  function rank(manifest, opts = {}) {
    ...
    const byFile = routesByFile(manifest);
    const shell = manifest?.shell || '';
    const rows = list(manifest?.routes).map((r, i) =>
      ({ ...rowFor(r, { byFile, touches, branches, shell }), _i: i }));
    rows.sort((a, b) => {
  ...
  function pathsToRead(manifest) {
    const s = new Set(routesByFile(manifest).keys());
  ...
  // a hit on a narrow file is ON the route, a hit only on a file three or
  ...
  function routesTouched(manifest, files) {
    ...
    for (const r of list(manifest?.routes)) {
      ...
      for (const p of list(r.files)) {
        ...
        any.push(p);
        if ((byFile.get(p) || []).length < WIDE) own.push(p);
      }
  ...
  function manifest(routeRows, vocabRows) {
    ...
    return {
      ...
      // path attribution has to exclude.
      app: shell?.files?.[0] || '',
      shell: shell?.files?.[0] || '',
  ...
  window.routeActivity = { WIDE, APP, BUNDLE, manifest, openable, viewUrl, bundleStale,
                           routesByFile, rank, stops, routesTouched, nearNote, shellRow,
                           pathsToRead, newest };
})();
