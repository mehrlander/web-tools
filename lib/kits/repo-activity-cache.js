...
// branches view's per-visit API fanout (~2 + 2N calls to scan N branches) for
...
//            and the any-state PR index (branchPRs / prReach) that tells a
//            branch row whether its PR merged, closed, or never existed.
//            This tier feeds the cross-repo reads.
//   scan   the capped landed/stranded rollup from lib/kits/branch-status.js,
//            computed once on the crawl and stored whole, so the per-repo
//            branches view renders from the cache instead of re-scanning.
//
...
(() => {
  ...
  // scannedAt) are excluded so a crawl that found nothing new hashes
  ...
  // commits and scan rows reduce to their identifying shas / groups, so a
  ...
  function material(entry) {
    ...
    return {
      ...
      openPRs: (entry.openPRs || []).map(p => [p.number, p.updatedAt || '', p.session || '', p.aheadBy ?? '',
                                               p.firstDate || '', (p.pages || []).join(',')]),
      // The any-state PR index (gh.branchPulls). It rides the hash because a
      // PR merging changes nothing else here: the branch keeps its sha, its
      // group, and its dates, so a crawl that learned "this branch's PR merged"
      // would hash identically to the crawl before it and never commit the one
      // thing it found. `prReach` rides too, since a row's honesty about an
      // unmatched branch is read off it.
      // `updatedAt` is left out on purpose: it moves on every comment and
      // review, and nothing here renders it, so hashing it would restamp the
      // whole cache for activity the view does not show.
      branchPRs: (entry.branchPRs || []).map(p => [p.head, p.number, p.state || '', p.draft ? 1 : 0, p.count ?? 1, p.title || '']),
      prReach: entry.prReach || '',
      ...
      // `pages` rides the hash for the reason the comment above gives and the
      // two fields before it learned the hard way: a crawl that newly stores a
      // field hashes identically to the crawl before it without this line, skips
      // the commit, and leaves the field dark until the branch happens to move.
      scan: (entry.scan?.branches || []).map(b => [b.name, b.sha, b.group, b.aheadBy ?? '',
  ...
  // that the fresh crawl replaces. A crawl that skipped the scan (summary-only
  // pass) keeps the prior scan rather than dropping it.
  function mergeRepo(prev, fetched, nowISO, commitCap = COMMIT_CAP) {
    ...
    const scan = f.scan !== undefined ? f.scan : (prev?.scan || null);
    // Counts that DESCRIBE the scan travel with it. A QUICK pass measures
    // branches/active/older/openPRs correctly from the branch list it read, but
    // reports landed/stranded/scanned as zero because it ran no scan. Taking
    // those wholesale would blank the stranded chip on every repo card while
    // the scan rows behind it are still there: a cache reporting less than it
    // holds.
    //
    // Keyed on an explicit `partial` flag, not on the absence of `scan`.
    // Inferring it from the absent field looked equivalent and was not: plenty
    // of callers hand over an entry with no scan and mean it, and treating
    // those as partial rewrote counts they had deliberately set.
    const SCAN_COUNTS = ['landed', 'stranded', 'scanned'];
    const counts = !f.counts ? (prev?.counts || {})
      : f.partial && prev?.counts
        ? { ...f.counts, ...Object.fromEntries(SCAN_COUNTS.map(k => [k, prev.counts[k] ?? 0])) }
        : f.counts;
    const entry = {
      generatedAt: nowISO,
      ...
      // Snapshot, like openPRs: a crawl that read the index replaces it whole
      // (a PR that merged since must not linger as open), and a crawl that
      // could not read it keeps the prior one rather than blanking every row's
      // PR state. The reach travels with the rows it describes, or the two
      // disagree about what the index covers.
      branchPRs: Array.isArray(f.branchPRs) ? f.branchPRs : (prev?.branchPRs || []),
      prReach: Array.isArray(f.branchPRs) ? (f.prReach || '') : (prev?.prReach || ''),
      scan,
      // Snapshot, like scan: a crawl that skipped checks keeps the prior ones
