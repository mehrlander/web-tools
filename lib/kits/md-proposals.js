...
// projects/text/, three files: passages, proposals, and revisions by passage id) retains edit
...
//   mdProposals.index({ token })             -> the shared collection, loaded
//
...
//
// Depends on kits/md-diff.js for the rendering and kits/text-collection.js for
// the collection. Both are loaded on demand.
(() => {
  if (window.mdProposals) return;

  // Where the collection lives; a page that has not loaded it reaches it
  // through index() below.
  ...
  // held weakly, since a collection index is loaded once and read by every document.
  ...
  function table(index) {
    ...
    for (const p of index.proposals || []) {
      const key = flat(index.passages?.[p.from]);
      if (!key) continue;
  ...
  // proposals as TextCollection views and the index of the one to substitute.
  // `picks` is keyed by the block's position in the returned list.
  function plan(md, index, { picks = {} } = {}) {
    const T = window.TextCollection;
    if (!T || !window.mdDiff) throw new Error('mdProposals.plan needs mdDiff and TextCollection');
  ...
  async function render(host, md, index, opts = {}) {
    ...
    if (!window.mdDiff || !window.TextCollection) throw new Error('mdProposals.render: kits unavailable');
  ...
  async function index({ token, fresh = false, quiet = true } = {}) {
    ...
    return window.TextCollection.load(gh, { fresh, quiet });
  }
