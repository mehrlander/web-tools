...
// projects/text/) retains edit proposals keyed on the exact source string of a
// paragraph: a phrase review, a document audit, an overnight half-length
// rewrite, a local model's clarity pass. Text Lab lists them and the FAB's Text
// tab looks them up for the page at hand, but neither puts them ON the
// document. A reader with a markdown file open had to leave it to learn that
...
//
// READ-ONLY, like everything in this lane. It writes nothing and offers no
...
(() => {
  ...
  // original, and a way to Text Lab for the full record. Under it, when the
  // block has more than one proposal, the alternatives as a row of stops in
  // the same pill idiom md-diff's readings use, so a reader learns one
  // vocabulary. The note is the proposer's own words and is labelled as such.
  const LANE = { 'phrase-reviews': 'phrase review', 'audit-packet': 'document audit',
                 'web-tools-paragraphs': 'half-length', 'local-proposals': 'local model' };
  function provenance(entry, i, onPick) {
    ...
    const name = o.agent || v.author || 'unknown proposer';
    const who = el('span', 'inline-flex items-center gap-1');
    who.append(Object.assign(el('i', 'ph ph-sparkle'), { 'aria-hidden': 'true' }));
    who.append(el('span', 'font-medium', name));
    foot.append(who);
    const kind = LANE[v.lane] || v.lane || '';
    // A lane whose proposer is named after it says so once.
    const what = [v.action && v.action !== kind ? v.action : '', kind !== name ? kind : '']
      .filter(Boolean).join(' · ');
    if (what) foot.append(el('span', '', what));
    const ratio = Number.isFinite(o.ratio) ? o.ratio
                : Number.isFinite(v.analysis?.ratio) ? v.analysis.ratio : null;
    if (ratio !== null) foot.append(el('span', 'tabular-nums', `${Math.round(ratio * 100)}% of the words`));
    if (o.rationale) {
      const note = el('span', 'italic truncate max-w-[40ch]', o.rationale);
      note.title = o.rationale;
      foot.append(note);
    }
    ...
    if (entry.proposals.length > 1) {
      ...
      entry.proposals.forEach((alt, k) => {
        const lane = LANE[alt.lane] || alt.lane || '';
        const b = el('button', 'cursor-pointer rounded-full px-1.5 py-0.5 transition-colors '
                     ...
                     alt.action || lane || String(k + 1));
        b.dataset.mdProposalsPick = String(k);
        b.title = `${alt.origins?.[0]?.agent || alt.author || ''} · ${alt.purpose || ''}`.trim();
