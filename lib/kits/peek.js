...
// it as a defect.
//
//   Peek.enable({doc, onSelect, onTake, takeLabel})   arm; doc defaults to this one
//   Peek.disable()
...
(() => {
  ...
  const S = {
    ...
    onSelect: null, onTake: null, takeLabel: '', cleanup: [],
  };
  ...
  const wrapInfo = (el) => {
    ...
    for (const n of doc.head.querySelectorAll('link[rel="stylesheet"][href], script[src], style')) {
      ...
      if (/\/\/mehrlander\.github\.io\/|\/mehrlander\//.test(u)) { skipped.push(u); continue; }
...
`;
  ...
  const render = () => {
    ...
    S.panel.innerHTML = `
      ...
      <div style="${ROW}padding:7px 10px;border-top:1px solid ${LINE}">
        ...
        <span style="flex:1"></span>
        ${S.onTake ? `<button data-peek-act="take" style="${BTN}background:#15803d;border-color:#15803d;color:#fff">${esc(S.takeLabel || 'Take')}</button>` : ''}
  ...
  const enable = (o = {}) => {
    ...
    S.onTake = o.onTake || null;
    S.takeLabel = o.takeLabel || '';
    S.view = o.view || 'facts';
    ...
    on(S.panel, 'click', (e) => {
      ...
      if (a === 'take' && S.onTake) {
        ...
        Promise.resolve(S.onTake(current(), facts(current()))).then(msg => {
  ...
  const disable = () => {
    ...
    S.onSelect = S.onTake = null; S.takeLabel = '';
    S.chain = []; S.at = 0;
    return window.Peek;
  };
