...
//       auto-bootstrap tail + `export default`). Optional `data` is
...
//       Carry the build in the page and point its entry.js import at it, so the
...
(() => {
  ...
  // gh-api.js minus its auto-bootstrap tail and `export default`,
...
`;
  ...
  // Rewrite the page's entry.js import to the carried build. The import's
  // `await` then drives the build's top-level await (gh-boot runs), and
  // `export default GH` satisfies a caller that reads mod.default.
  //
  // Match the import CALL, not a literal URL: `[^()]*` holds it to one call with
  // no nesting, and stays blind to quoting. Every loader page boots through the
  // one lib/entry.js import since 2026-09-26; before that, two idioms spelled the
  // specifier differently and a literal pattern missed the larger half.
  const GH_IMPORT_RE = /import\(\s*[^()]*lib\/entry\.js[^()]*\)/g;
  ...
  const bake = (pageHtml, buildJs) => {
    ...
    if (!GH_IMPORT_RE.test(pageHtml)) {
      throw new Error('bake: no entry.js import call found in page to rewrite');
    }
    GH_IMPORT_RE.lastIndex = 0;
  ...
  const collectCache = async (gh, opts = {}) => {
    ...
    const loaded = opts.scripts || (typeof window !== 'undefined' ? window.__loadedScripts : null);
    if (!loaded) throw new Error('collectCache: no __loadedScripts to read');
    // Plus the lib files a page READ to run elsewhere (vanilla-demo's proof-frame
    // injects): not loaded here, but served from the same cache in a baked copy.
    const scripts = [...loaded, ...(typeof window !== 'undefined' && window.__frameScripts || [])];
