// Public browse: a no-token file browser for any PUBLIC GitHub repo. The
// listing is one anonymous GitHub API call (GH.flatTree, git trees recursive)
// and a file's bytes come from raw.githubusercontent (GH.rawUrl), which is not
// rate-limited. This is the intentional non-auth capability of show-repo: the
// GitHub API allows 60 anonymous requests an hour, and a browse spends one of
// them rather than one per folder. It works signed in too, but earns its place
// in the public case. Limits: public repos only, and a repo too large for one
// tree response lists truncated. Registered as Alpine.data('publicBrowse').
document.addEventListener('alpine:init', function() {
  Alpine.data('publicBrowse', function() {
    ...
    return {
      description: 'No-token public-repo file browser, read through one GitHub tree call and raw.githubusercontent, the rate-safe non-auth capability of show-repo',
      ...
      template: `
        <div class="mx-auto max-w-5xl px-1">
          ...
          <div x-show="error" class="alert alert-warning py-2 px-3 text-base mb-4">
                ...
                <div class="border border-base-300 rounded-lg overflow-hidden flex flex-col min-h-0" style="max-height:32rem">
                  <div class="p-2 border-b border-base-200 shrink-0">
                    <label class="input input-bordered flex items-center gap-2 font-mono">
                      ...
                      <!-- data-find-box: the way this view's file list is narrowed. -->
                      <input type="text" x-model="filter" placeholder="filter paths" data-find-box class="grow" />
                    </label>
                  </div>
                  ...
                  <template x-if="sel">
                    <div class="flex flex-col min-h-0 flex-1">
                      <div class="flex items-center gap-2 p-2 border-b border-base-200 shrink-0">
                        ...
                        <a :href="rawHref(sel)" target="_blank" class="btn btn-ghost btn-square" title="Raw" aria-label="Raw">
                          <i class="ph ph-cloud-arrow-down text-base"></i></a>
                        <a :href="'https://github.com/' + repo + '/blob/' + ref + '/' + sel" target="_blank"
                           class="btn btn-ghost btn-square" title="On GitHub" aria-label="On GitHub">
          ...
          <div x-show="!error && !files.length && !loading" class="text-base text-base-content/40 py-10 text-center">
      ...
      async browse() {
        ...
        try {
          ...
          if (!files.length) this.error = 'GitHub returned no files for ' + repo + '@' + ref + '. The ref may be empty.';
        } catch (e) {
          // Specific over vague: name why the listing is empty, and what to do.
          if (e.status === 404) {
            this.error = 'GitHub has no public ' + repo + '@' + ref + '. Without a token it answers for public repos only, '
              + 'so a private repo, a wrong name, and a missing branch all look like this. '
              + 'Check the name, or sign in (shield icon) to browse private repos through the GitHub API.';
          } else if (e.status === 403) {
            this.error = 'GitHub refused the listing (HTTP 403), most likely the 60-an-hour anonymous limit. '
              + 'Sign in (shield icon) or try again within the hour.';
          } else {
            this.error = 'Could not list ' + repo + '@' + ref + ' (' + (e.message || e) + ').';
          }
        } finally {
          this.loading = false;
        }
      ...
      async open(path) {
        ...
        try {
          ...
          this.preview = { path, text: '', size, loading: false, error: 'Could not fetch this file from raw.githubusercontent (' + (e.message || e) + ').', binary: false };
        }
      },
