        ...
        <div class="mx-auto max-w-5xl px-1">
          <div class="mb-4">
            ...
            <p class="text-sm text-base-content/60 mt-1">
              Reads any <span class="font-medium">public</span> repo through jsDelivr's CDN, so it needs
              <span class="font-medium">no token</span> and never spends your GitHub rate limit. Private repos
              and pushes from the last few hours may not appear yet.
            </p>
          </div>

          <div class="flex items-center gap-2 mb-4">
            <label class="input input-sm input-bordered flex items-center gap-2 flex-1 font-mono text-xs">
            ...
            <button @click="browse()" :disabled="loading" class="btn btn-sm btn-primary gap-1">
              <i class="ph" :class="loading ? 'ph-circle-notch animate-spin' : 'ph-magnifying-glass'"></i>
              Browse
            </button>
          </div>

          <div x-show="error" class="alert alert-warning py-2 px-3 text-sm mb-4">
              ...
              <div class="grid gap-3 md:grid-cols-2">
                ...
                <div class="border border-base-300 rounded-lg overflow-hidden flex flex-col min-h-0" style="max-height:32rem">
                  <div class="p-2 border-b border-base-200 shrink-0">
                    <label class="input input-xs input-bordered flex items-center gap-2 font-mono">
                  ...
                  <div class="overflow-y-auto flex-1">
                    <template x-for="f in filtered" :key="f.path">
                      <button @click="open(f.path)"
                        ...
                        <i class="ph ph-file text-xs text-base-content/40 shrink-0"></i>
                        <span class="font-mono text-xs truncate flex-1" :title="f.path" x-text="f.path"></span>
                        <span class="text-[10px] text-base-content/40 shrink-0 font-mono" x-text="fmtSize(f.size)"></span>
                      </button>
                    </template>
                    <div x-show="!filtered.length" class="text-xs text-base-content/40 p-3 text-center">
                ...
                <div class="border border-base-300 rounded-lg overflow-hidden flex flex-col min-h-0" style="max-height:32rem">
                  <template x-if="!sel">
                    <div class="flex-1 flex items-center justify-center text-xs text-base-content/40 p-6 text-center">
                  ...
                  <template x-if="sel">
                    <div class="flex flex-col min-h-0 flex-1">
                      <div class="flex items-center gap-2 p-2 border-b border-base-200 shrink-0">
                        <span class="font-mono text-xs truncate flex-1" :title="sel" x-text="sel"></span>
                        <a :href="rawHref(sel)" target="_blank" class="btn btn-ghost btn-xs btn-square tooltip tooltip-bottom" data-tip="Raw (CDN)">
                          <i class="ph ph-cloud-arrow-down text-sm"></i></a>
                        <a :href="'https://github.com/' + repo + '/blob/' + ref + '/' + sel" target="_blank"
                           class="btn btn-ghost btn-xs btn-square tooltip tooltip-bottom" data-tip="On GitHub">
                          <i class="ph ph-github-logo text-sm"></i></a>
                      </div>
                      <div x-show="preview.loading" class="flex justify-center py-10">
                        <span class="loading loading-dots loading-sm opacity-30"></span>
                      </div>
                      <div x-show="!preview.loading && preview.error" class="text-xs text-warning p-3" x-text="preview.error"></div>
                      <div x-show="!preview.loading && preview.binary" class="text-xs text-base-content/50 p-4 flex flex-col gap-2">
                      ...
                      <pre x-show="!preview.loading && !preview.error && !preview.binary"
                           class="overflow-auto flex-1 text-[11px] leading-snug p-3 font-mono whitespace-pre"><code x-text="preview.text"></code></pre>
                    </div>
                  </template>
                </div>
              </div>
          ...
          <div x-show="!error && !files.length && !loading" class="text-sm text-base-content/40 py-10 text-center">
