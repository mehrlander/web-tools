      ...
      template: `
        <div class="mx-auto max-w-5xl px-1">
          ...
          <div class="flex items-center gap-2 mb-4">
            <label class="input input-sm input-bordered flex items-center gap-2 flex-1 font-mono text-sm">
          ...
          <template x-if="!error && files.length">
            <div>
              <div class="flex items-center gap-3 text-sm text-base-content/50 mb-2">
              ...
              <div class="grid gap-3 md:grid-cols-2">
                  ...
                  <div class="overflow-y-auto flex-1">
                    <template x-for="f in filtered" :key="f.path">
                      <button @click="open(f.path)"
                        ...
                        <i class="ph ph-file text-sm text-base-content/40 shrink-0"></i>
                        <span class="font-mono text-sm truncate flex-1" :title="f.path" x-text="f.path"></span>
                        <span class="text-sm text-base-content/40 shrink-0 font-mono" x-text="fmtSize(f.size)"></span>
                      </button>
                    </template>
                    <div x-show="!filtered.length" class="text-sm text-base-content/40 p-3 text-center">
                ...
                <div class="border border-base-300 rounded-lg overflow-hidden flex flex-col min-h-0" style="max-height:32rem">
                  <template x-if="!sel">
                    <div class="flex-1 flex items-center justify-center text-sm text-base-content/40 p-6 text-center">
                  ...
                  <template x-if="sel">
                    <div class="flex flex-col min-h-0 flex-1">
                      <div class="flex items-center gap-2 p-2 border-b border-base-200 shrink-0">
                        <span class="font-mono text-sm truncate flex-1" :title="sel" x-text="sel"></span>
                        <a :href="rawHref(sel)" target="_blank" class="btn btn-ghost btn-sm btn-square tooltip tooltip-bottom" data-tip="Raw (CDN)">
                          <i class="ph ph-cloud-arrow-down text-base"></i></a>
                        <a :href="'https://github.com/' + repo + '/blob/' + ref + '/' + sel" target="_blank"
                           class="btn btn-ghost btn-sm btn-square tooltip tooltip-bottom" data-tip="On GitHub">
                      ...
                      <div x-show="!preview.loading && preview.error" class="text-sm text-warning p-3" x-text="preview.error"></div>
                      <div x-show="!preview.loading && preview.binary" class="text-sm text-base-content/50 p-4 flex flex-col gap-2">
                      ...
                      <pre x-show="!preview.loading && !preview.error && !preview.binary"
                           class="overflow-auto flex-1 text-sm leading-snug p-3 font-mono whitespace-pre"><code x-text="preview.text"></code></pre>
                    </div>
                  </template>
                </div>
              </div>
