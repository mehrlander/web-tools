    ...
    return {
      ...
      template: `
        <div class="flex flex-col gap-4">
          <div class="flex items-center justify-between flex-wrap gap-2">
            ...
            <div class="flex items-center gap-1" x-show="items.length">
              <button @click="copyLink()" class="btn btn-xs btn-ghost gap-1"
                      title="Copy a #stage= link that reopens this exact stage (ref items only)">
                <i class="ph" :class="linkCopied ? 'ph-check' : 'ph-link'"></i>Stage link
              </button>
          ...
          <!-- Recent across the estate's repos: the latest committed files,
               each one tap from staged. Row anatomy is deliberate: the
               basename leads, the repo and folder sit beneath it in the muted
               line, so "where is this from" reads without decoding a long
               path. -->
          <div x-show="recentLoading || recent.length" class="flex flex-col gap-1">
            <div class="flex items-center justify-between">
              <button @click="recentOpen = !recentOpen"
                      class="flex items-center gap-1.5 text-xs font-bold opacity-70 hover:opacity-100">
                <span>Recent</span>
                <span x-show="recent.length" class="badge badge-ghost badge-xs" x-text="recent.length"></span>
                <i class="ph ph-caret-down text-[10px] transition-transform" :class="!recentOpen && '-rotate-90'"></i>
              </button>
              <button @click="loadRecent(true)" class="btn btn-ghost btn-xs btn-square" title="Refresh">
                <i class="ph" :class="recentLoading ? 'ph-circle-notch animate-spin' : 'ph-arrows-clockwise'"></i>
              </button>
            </div>
            <div x-show="recentLoading && !recent.length" class="flex justify-center py-4">
              <span class="loading loading-dots loading-sm opacity-30"></span>
            </div>
            <!-- Constrained and self-scrolling, so a deep history scrolls
                 back inside its own box instead of stretching the page. -->
            <div x-show="recentOpen" class="flex flex-col max-h-72 overflow-y-auto overscroll-contain rounded-lg">
              <template x-for="it in recent" :key="'r:' + it.repo + ':' + it.path">
                <button @click="toggleRecent(it)"
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="flex items-baseline justify-between gap-2">
                      ...
                      <span class="shrink-0 text-[10px] opacity-50" x-text="ago(it.date)"></span>
                    </div>
