...
// prompts are a follow-up: for now an agent hands those over as plain chat
// text alongside the stage link, rather than through this fixed panel.
const DIFF_PROMPTS = [
...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
            ...
            <template x-if="outTab === 'diff'">
              <div class="flex flex-col gap-2">
                ...
                <div x-show="diffRows" class="border border-base-300 rounded-lg overflow-hidden">
                  <template x-for="(p, idx) in diffPrompts" :key="p.label">
      ...
      // The review-prompts list: fixed general asks (DIFF_PROMPTS). A getter,
      // not a stored field, so it never needs its own reactivity wiring.
      get diffPrompts() {
        return DIFF_PROMPTS.map(([label, ask]) => ({ label, ask }));
      },
