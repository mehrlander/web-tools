...
// preloaded with them, and a #gz= fragment carries LOCAL TEXT itself, for a
// sender with no repo address to point at. Both are built; see the gz payload
// section below for the encoding and its budget.
//
  ...
  Alpine.data('stager', function() {
    ...
    return {
      ...
      flavorActions(fl) {
        ...
        if (IN.isMarkup(fl.name)) out.push({ id: 'markdown', icon: 'ph-text-align-left', label: 'Markdown' });
        out.push({ id: 'base64', icon: 'ph-brackets-curly', label: 'Base64' });
      ...
      _rMenu(btn, rows) {
        document.querySelectorAll('.stage-hdr-menu').forEach(m => m.remove());
        if (!rows.length) return;
        const box = document.createElement('ul');
        box.className = 'stage-hdr-menu menu absolute right-0 top-full z-[80] mt-1 w-44 '
                      + 'rounded-box border border-base-300 bg-base-200 p-2 shadow-xl';
        for (const r of rows) {
          const li = document.createElement('li');
          const a = document.createElement(r.href ? 'a' : 'button');
          if (r.href) { a.href = r.href; a.target = '_blank'; }
          if (r.active) a.className = 'menu-active active';
          a.innerHTML = `<i class="ph ${r.icon || 'ph-dot'}"></i><span></span>`;
          a.querySelector('span').textContent = r.label;
          if (r.onClick) a.addEventListener('click', () => { box.remove(); r.onClick(); });
          else a.addEventListener('click', () => box.remove());
          li.append(a);
          box.append(li);
        }
