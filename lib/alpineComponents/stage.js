...
// .show-repo.json save; the ref items carry those.
//
...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
          ...
          <div x-show="items.length" class="border border-base-300 rounded-lg bg-base-100 p-3 flex flex-col gap-2">
            ...
            <div class="flex items-center justify-between gap-2">
              <button @click="save()" :disabled="savingStage"
                      ...
                      title="Write the staged ref list to .show-repo.json in the open repo (local files excluded)">
      ...
      // whatever else .show-repo.json declares. Local files can't serialize, so
      ...
      async save() {
        ...
        try {
          ...
          toast('push-pin', 'Stage saved to .show-repo.json' + note, 'alert-success', 3000);
        } catch (e) {
