...
// preloaded with them. Ref content stays behind the viewer's token; local
// TEXT rides the link itself, gzipped into the fragment (#gz= alone for an
// all-local stage, &gz= beside a spec), so a stage that is nothing but a
// paste opens for a reader with no token and no access to any repo, which
// tools/test/stage-gz-review.mjs holds with api.github.com blocked outright.
//
...
// gh.copyTo, local bytes via gh.saveBytes/save. Only a local BINARY is left
// out of the link: bytes gain nothing from gzip and inflate through base64,
// so they have to be sent to a repo instead. The manifest seed (stage.files)
// is refs either way, since it addresses files rather than carrying them.
//
// Grammar, both directions (StageLink.parseLink / StageLink.mint):
//
//   #stage=owner/repo[@ref]:path1,path2;owner2/repo2:path3
//     [&gz=<b64url>][&prompts=<b64url>][&mode=diff]
//     [&cmp=<item spec>][&view=unified|split|patch][&dest=<spec>]
//
...
// '/' left readable. A stage of only local text leads with '#gz=' instead,
// since an empty '#stage=' counts as a miss in read().
//
// A link is one object with four parts: REFS (the stage spec above), CONTENT
// (&gz=, the local text items gzipped in), COMMENTARY (&prompts=, a
// base64url'd JSON list of {label, ask} review asks), and the READING (&mode=,
// the intent that this stage opens as a diff run without a click; &cmp=, which
// staged item the comparison is against; &view=, which of the three ways it is
// drawn). Refs are pointers, so their content stays behind the token;
// everything else is authored or carried, so it rides the link itself. mint()
// serializes the refs, the commentary and the reading, mintWithLocals() folds
// the content in first; parseLink() returns { items, prompts, mode, cmp, view, dest, gz }, and
// the bare parse() keeps returning just the items for callers (the shell seed)
// that only want refs. `dest` is READ and never minted here: a host mints it
// (the branch page's add-file plus) to open the stage already aimed. This is
// the seed of a richer "surface" schema: the same {refs, commentary, mode}
// shape a manifest's stage block or a future surface file would carry.

// The comparison's review-prompts panel: a fixed set of general review asks,
...
window.StageLink = (() => {
  ...
  // into chats and issue bodies that wrap or truncate, so the cap below keeps
  // a wide margin and still carries a large document: this page's own 7 KB HTML
  // paste encodes to roughly 2 KB. Over budget, minting
  // reports the overflow instead of handing back a link that fails somewhere
  // else, later.
  ...
  const decodeLocals = async (payload) => {
    if (!payload) return [];
    try {
      const parsed = JSON.parse(await gunzipText(bytesFromB64url(payload)));
      ...
      if (parsed && typeof parsed === 'object') {
        if (parsed.schema === 'git-change/1' || parsed.$schema === 'git-change/1') {
          const branch = parsed.proposed_branch || parsed.branch || '';
          const slug = branch.replace(/^[^/]+\//, '').replace(/[^a-zA-Z0-9._-]/g, '-');
          const name = (slug || 'change') + '.git-change.json';
          return [{ name, text: JSON.stringify(parsed, null, 2) }];
        }
      }
      return [];
    } catch { return []; }
  ...
  const parseLink = (hash) => {
    ...
    // synchronous and hands back the raw payload; the component's init decodes
    // it after mount, and the app's boot route keys on the payload being
    // present rather than on items it cannot have yet.
    ...
    // take: one piece of text carried PLAIN, percent-encoded rather than
    // gzipped, with an optional `name`. It exists for a sender that cannot
    // gzip, which is an iPhone shortcut: `Stage-Input` in shortcut-tools
    // URL-encodes whatever it was shared and opens this link in Safari. It is
    // `gz`'s simpler sibling, not its replacement, so the same budget applies
    // in spirit and the sender decides what is small enough to ride.
    // `take` is always the LAST param and runs to the end of the fragment, so
    // an encoder that leaves a literal '&' in the text (whether Shortcuts'
    // URL Encode does is unmeasured) cannot cut the text short at it.
    const dec = (v) => { try { return decodeURIComponent(v); } catch { return ''; } };
    const rawHash = String(hash || '');
    const at = rawHash.search(/(^|[#&])take=/);
    const take = at < 0 ? '' : dec(rawHash.slice(rawHash.indexOf('take=', at) + 5));
    const takeName = take && p.name != null ? dec(p.name) : '';
    return { items, prompts: decodePrompts(p.prompts), mode: p.mode === 'diff' ? 'diff' : '',
             cmp, view,
             dest, gz: p.gz || '', take, takeName };
  };
  ...
  // deliberate consequence of the shared rule: an empty '#stage=' counts as a
  // miss, so a truncated link that kept the fragment key but lost its value
  // falls back to a populated ?stage= instead of silently staging nothing.
  // The fragment itself still goes to parseLink raw, not through
  ...
  // Without them a `mode=diff` link reopens the recipient on the default pair
  // in the default view, so the refs are right and the thing being pointed at
  // is not.
  //
...
// The bench mounts on your first visit to the Stage, so intake cannot live
// inside it: anything staged before that visit would have nothing to land in,
// which is why a drop anywhere else in a host app has to reach this module.
//
// This is the intake with the view taken out. It owns the DECISIONS (is this
...
window.StageIntake = (() => {
  ...
  // routes it to a view it cannot fill costs the reader the content.
  // GIT CHANGE HANDOFF ENVELOPE: an unauthenticated change package carrying
  // metadata and a thin Git bundle. Recognized by schema git-change/1 and bundle payload.
  const isGitChange = (text) => {
    if (typeof text !== 'string') return false;
    const s = text.trim();
    if (!s.startsWith('{') || !s.endsWith('}')) return false;
    try {
      const o = JSON.parse(s);
      return Boolean(o && (o.schema === 'git-change/1' || o.$schema === 'git-change/1') &&
        ((o.payload && o.payload.data) || o.bundle_base64));
    } catch {
      return false;
    }
  };
  ...
  const nameForText = (text) => {
    if (isGitChange(text)) {
      try {
        ...
        const branch = o.proposed_branch || o.branch || '';
        const slug = branch.replace(/^[^/]+\//, '').replace(/[^a-zA-Z0-9._-]/g, '-');
        return (slug || 'change') + '.git-change.json';
      } catch {}
    }
    return new Date().toISOString().slice(0, 10) + '-paste.' + extForText(text);
  };

  // THE SNIFF ITSELF, split from the name so a base64 decode can ask what its
  // bytes turned out to be without minting a name it does not want.
  const extForText = (text) => {
    ...
    return /^(<!doctype html|<html)/i.test(s) ? 'html'
      ...
      : isGitChange(full) ? 'git-change.json'
      : isJson(full) ? 'json'
  ...
  // the same data is when it arrives as a file's contents. Both are read: a tab
  // and its equivalent CSV must not open as a table and a wall of text.
  //
  ...
  // ArrayBuffer, so stamping `isText: false` on that basis alone holds a
  // dropped .md as opaque bytes: nothing to read but a "Not text" note, no
  // diff, no bundle block, and no link to carry it, while pasting the same
  // characters stages them as text.
  //
  ...
  // EVERY INTAKE APPENDS, and where an item lands is irrelevant: the reader
  // picks its other side by NAME (compareWith) rather than by position. An
  // intake that took a position instead would be fusing a paste with a
  // selection, which is the one thing an intake must not decide.
  const put = (fresh) => {
  ...
  const textItem = (name, text) => {
    const isGc = isGitChange(text) || String(name || '').endsWith('.git-change.json');
    return {
      local: true, id: ++seq, name, path: name,
      size: (text || '').length, type: isGc ? 'application/json' : 'text/plain',
      isText: true, text,
      isGitChange: isGc,
    };
  };
  ...
  // ── STANDOFF NOTES ────────────────────────────────────────────────────────
  //
  // A comment about a staged file is a FILE, and that is the whole design. It
  // lands in the destination beside its subject, named for it, carrying a
  // pointer back to it, and from there it is content: readable, greppable,
  // promotable, and drained by whatever drains the folder.
  //
  ...
  // JSON case and changes what the file IS: a CSV inside it is a quoted string
  // with escaped newlines, an image has to be base64'd, and anything downstream
  // expecting the original gets the wrapper instead.
  //
  // AND NOT THE COMMIT MESSAGE, which was the cheap answer and the wrong one: a
  // commit message is a fact git records ABOUT a write, so the comment would
  // exist everywhere except in the tree the reader is looking at.
  //
  // The name keeps the subject's own deposit-relative path, so the pair lands
  // adjacent whatever the destination layout does with folders, and stripping
  // the suffix is all the pairing a reader or a drain ever needs.
  const NOTE_SUFFIX = '.note.md';
  const subjectPath = (it) => String((it && (it.path || it.name)) || '');
  const noteNameFor = (it) => subjectPath(it) + NOTE_SUFFIX;
  const isNote = (it) => !!(it && it.noteFor);

  // `about:` names the subject by BASENAME, because that is where the note sits
  // once deposited: a note for `lib/gh-api.js` lands at
  // `lib/gh-api.js.note.md`, in the same folder, so a pointer carrying the full
  // path would not resolve from where the note actually is. Standoff annotation
  // resolves relative to itself.
  //
  // `source:` appears only on a note about a ref, and carries the one fact the
  // deposit otherwise destroys: a copied file lands under its own name with no
  // record of the repo and ref it came from.
  const noteDoc = (it, body, today) => {
    const lines = ['---',
                   'about: ' + subjectPath(it).split('/').pop(),
                   'added: ' + (today || new Date().toISOString().slice(0, 10))];
    if (it && !it.local) lines.push('source: ' + keyOf(it));
    lines.push('---', '', String(body || '').trim(), '');
    return lines.join('\n');
  };

  // The body back out of a note, so re-opening the editor shows what was
  // written rather than the front matter wrapped around it. Tolerant of a note
  // that arrived some other way (a `#gz=` link, a hand edit): no front matter
  // means the whole file is the body.
  const noteBody = (text) => {
    const s = String(text || '');
    const m = /^---\n[\s\S]*?\n---\n?/.exec(s);
  ...
  const take = (d) => {
    ...
    if (d.text != null) {
      // A SIGNED ERRAND RESULT names its errand, so it is staged as that
      // errand's file and the errand panel is told, wherever it was pasted.
      const env = window.Errands?.envelope(d.text);
      if (env) {
        const f = window.Errands.envelopeFile(env);
        const added = put([{ ...textItem(f.name, f.text), errand: env.errand }]);
        window.dispatchEvent(new CustomEvent('errand-result', { detail: env }));
        return added;
      }
  ...
  // EVERY FLAVOR A PASTE CARRIED, named, THE TAKEN ONE INCLUDED. Dropping it
  // would make the bar an ADD list, and choosing between two readings of one
  // copy is the commoner want. The bench derives each chip's state from the
  // stage itself, which is what keeps a chip honest when the item is removed
  ...
  // it: the bench mounts on the first visit to the Stage, so a paste on any
  // other view has nothing to land in while the fold lives inside it.
  //
  ...
  // takeClipboard for the handoff (kits/stage-handoff.js): a
  ...
  // to get.
  //
  // THAT ASK IS ANSWERED BY THE GENERAL CASE, not by a tool of its own:
  ...
  // where the second is a reduction of the first. What that costs is the
  // reduction itself, a bare list with the prose stripped out, and nothing
  // wants it often enough to earn a second pill and a second reader of html.
  ...
  // ── MARKDOWN, WHICH IS WHAT A DERIVATION SHOULD BE ───────────────────────
  //
  // Copying out is the whole errand, so a derivation has to be a thing you can
  // paste. Markdown renders a converted page as a tappable list (READ_MODE
  // sends `.md` to preview), its raw mode is `- [text](url)` lines that paste
  // anywhere, and it generalizes past links, because markdown is what ANY html
  // is worth converting into. A `.csv` of `text,url` is the anti-pattern here,
  // reasoned from the machinery rather than the reader: it opens as a
  // filterable table and `transformKindOf` calls it rows, and none of that is
  // something you can copy out and use.
  //
  ...
  return { take, takeFile, takeDrop, takePaste, takeClipboard, takeFlavors, takeFlavor, offerable, focus,
           keyOf, textItem, noteNameFor, noteDoc, noteBody, isNote,
           flavorsOf, nameForFlavor, nameForText, isDelimited,
           delimiterOf, isRowsFn, isJson, isPowerShell, isGitChange, transformKindOf, textFromBytes,
           derivedName, opensRaw, mdOf, isMarkup, extForText,
           b64OfText, b64OfBytes, looksBase64, base64Info, sniffBytes };
})();

// THE COURIER INTAKE'S FLAG, read at load. The shell rebuilds the query from
// the parameters it knows on its first syncUrl, which can run before the bench
// mounts, so `courier=1` is captured here rather than in init.
// Where the courier's default capture goes when no errand names a place: the
// registry, which is private, since a captured page can be anything.
const COURIER_CAPTURES = 'mehrlander/web-tools-private@main:courier/captures';
// ?errand=<id>, the card a session's link opens on. Read at load for the same
// reason as the courier flag below: the shell rewrites the query it does not know.
const ERRAND_BOOT = (() => {
  try { return new URLSearchParams(location.search).get('errand') || ''; } catch { return ''; }
})();
const COURIER_BOOT = (() => {
  try { return new URLSearchParams(location.search).get('courier') === '1'; } catch { return false; }
})();
...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    // The courier's pointer, the bookmarklet the errand panel offers to install.
    // Public, in this repo; the errands it runs live in the private registry.
    const COURIER_REPO = 'mehrlander/web-tools';
    const COURIER_POINTER = 'bookmarklets/courier-stage.js';
    ...
    return {
      ...
      template: `
        <!-- A CONTAINER, so the bench reflows on the width it actually has.
             Without it the two panes below split on the WINDOW, and the window
             is the wrong question the moment the deck docks: the dock narrows
             this pane through --deck-dock-left and leaves the window where it
             was, so a wide window with a narrow pane still matches the wide
             breakpoint, lays out an aside too big for the pane, and paints it
             over the deposit column. Same fix and same reason as the Docs tab
             (lib/alpineComponents/map.js). -->
        <div class="@container relative min-h-[55vh]"
             x-init="$nextTick(() => mountStageSplit())"
             @dragenter.prevent="_dragDepth++"
             @dragover.prevent
             @dragleave.prevent="_dragDepth = Math.max(0, _dragDepth - 1)"
             @drop.prevent="onPageDrop($event)">

          <!-- The Stage split is keyed to this view's CONTAINER, not the
               window: a docked reader can narrow the view without changing
               the viewport. Below 42rem the old single-column flow remains;
               above it the shared dock-split handle gets a real track between
               the two panes. The fluid default reproduces the former 22rem,
               26rem and 30rem aside at the three documented thresholds, while
               --stage-aside-width records a reader's own allocation. -->
          <style>
            .stage-panes{display:grid;grid-template-columns:minmax(0,1fr);row-gap:1rem}
            .stage-split{display:none}
            @container (min-width:42rem){
              .stage-panes{grid-template-columns:minmax(0,1fr) 6px minmax(0,var(--stage-aside-width,clamp(22rem,calc(25cqw + 10rem),30rem)));column-gap:5px}
              .stage-split{display:flex;align-self:stretch;min-height:12rem}
            }
          </style>
          ...
          <!-- No title row and no mode pill. The bench mounts under the Stage
               ...
               NO EMPTY-STATE DROP BOX. The whole VIEW takes the drop (@drop
               on the root above) and Add file and Paste live in the aside, so
               a dashed "drop files here" panel carries no capability of its
               own: only a caption ABOUT the destination, standing where the
               real sections belong. Both sections render from the start,
               empty, so an aimed stage IS its aim and a reader sees the two
               things a deposit needs (where it goes, what is in it) before
               supplying either. -->
          <!-- Two panes: the lens (Out, and what is asked of you) leads;
          ...
          <div x-ref="splitFrame" class="stage-panes items-start">

            <!-- MAIN: the lens. -->
            <div class="flex flex-col gap-2 min-w-0">
              <div class="flex flex-col gap-2">
                <!-- THE COURIER INTAKE: what the visited page sent, and the
                     errand that ran on it. Shown only in the popup. -->
                <div x-show="courier" x-cloak class="flex flex-col gap-1 border border-base-300 rounded-lg bg-base-200/40 p-2.5">
                  <div class="flex items-center gap-1.5 font-semibold">
                    <i class="ph ph-person-simple-walk text-lg"></i>
                    <span>Courier</span>
                    <span class="font-mono text-base-content/60 truncate" x-text="courier?.host"></span>
                  </div>
                  <div x-show="courier?.state === 'waiting'" class="text-base-content/60">Waiting for the page.</div>
                  <div x-show="courier && courier.state !== 'waiting'" x-text="(courier?.links ?? 0) + ' links staged'"></div>
                  <div x-show="courier?.errand" class="flex items-center gap-1.5">
                    <span class="badge badge-sm" :class="{ 'badge-info': courier?.state === 'running', 'badge-success': courier?.state === 'ran', 'badge-error': courier?.state === 'failed' }" x-text="courier?.state"></span>
                    <span class="truncate" x-text="courier?.errand?.title"></span>
                  </div>
                  <div x-show="courier?.note" class="text-error font-mono text-sm break-all" x-text="courier?.note"></div>
                </div>
                <!-- GIT CHANGE HANDOFF CARD -->
                <div x-show="gitChangeItem" class="border border-base-300 rounded-lg bg-base-100 p-3 flex flex-col gap-2.5">
                  <div class="flex items-center justify-between gap-2 flex-wrap border-b border-base-200 pb-2">
                    <a :href="'https://github.com/' + (gitChangeEnv?.repository || '') + '/tree/' + (gitChangeEnv?.proposed_branch || '')"
                       target="_blank" rel="noopener"
                       class="link link-hover font-mono font-semibold flex items-center gap-1.5 text-base text-primary"
                       title="Open proposed branch on GitHub">
                      <i class="ph ph-git-pull-request text-lg"></i>
                      <span x-text="(gitChangeEnv?.repository || '—') + '@' + (gitChangeEnv?.proposed_branch || '—')"></span>
                      <i class="ph ph-arrow-square-out text-xs opacity-60"></i>
                    </a>
                    <div class="flex items-center gap-1.5">
                      <span x-show="gitChangeVerified === 'verified'" class="badge badge-success badge-sm gap-1 font-mono cursor-help"
                            :title="'SHA-256 payload checksum matches (' + (gitChangeEnv?.payload?.sha256 || '') + '). Confirms transmission integrity against corruption, not authorship or safety.'">
                        <i class="ph ph-shield-check"></i>sha256 verified
                      </span>
                      <span x-show="gitChangeVerified === 'mismatch'" class="badge badge-error badge-sm gap-1 font-mono cursor-help"
                            :title="gitChangeVerificationError || 'Checksum mismatch: payload does not match declared SHA-256'">
                        <i class="ph ph-warning-circle"></i>sha256 mismatch
                      </span>
                      <span x-show="gitChangeVerified === 'verifying'" class="badge badge-ghost badge-sm gap-1 font-mono">
                  ...
                  <div class="flex items-center gap-3 text-xs font-mono flex-wrap">
                    <div class="flex items-center gap-1">
                      ...
                      <span class="font-semibold" x-text="(gitChangeEnv?.base || '').slice(0, 10)"></span>
                      <button type="button" @click="copyGitChangeText(gitChangeEnv?.base, 'Base SHA copied')" title="Copy base SHA" class="btn btn-ghost btn-xs w-4 h-4 p-0">
                        <i class="ph ph-copy"></i>
                      </button>
                    </div>
                    <span class="opacity-30">→</span>
                    <div class="flex items-center gap-1">
                      <span class="opacity-50">expected tree:</span>
                      <span class="font-semibold text-primary" x-text="(gitChangeEnv?.expected_tree || '').slice(0, 10)"></span>
                      <button type="button" @click="copyGitChangeText(gitChangeEnv?.expected_tree, 'Tree SHA copied')" title="Copy tree SHA" class="btn btn-ghost btn-xs w-4 h-4 p-0">
                        <i class="ph ph-copy"></i>
                      </button>
                    </div>
                    <template x-if="gitChangeEnv?.commit?.author?.name">
                      <div class="flex items-center gap-1 opacity-70">
                        <span class="opacity-50">by</span>
                        <span x-text="gitChangeEnv?.commit?.author?.name"></span>
                      </div>
                    </template>
                  </div>
                  ...
                  <div class="flex flex-col gap-0.5 text-xs">
                    <div class="font-semibold text-sm text-base-content" x-text="gitChangeEnv?.commit?.subject || '—'"></div>
                    <div x-show="gitChangeEnv?.commit?.body" class="text-xs opacity-70 font-mono whitespace-pre-wrap max-h-24 overflow-y-auto" x-text="gitChangeEnv?.commit?.body"></div>
                  </div>

                  <!-- Changed files summary -->
                  <div class="flex flex-col gap-1 text-xs">
                    <div class="flex items-center justify-between opacity-50 font-mono">
                      <span>Changed Files</span>
                      <span x-text="(gitChangeEnv?.changed_files || []).length + ' file(s)'"></span>
                    </div>
                    <div class="max-h-28 overflow-y-auto border border-base-300 rounded p-1 font-mono text-xs flex flex-col gap-0.5 bg-base-200/30">
                      <template x-for="f in (gitChangeEnv?.changed_files || [])" :key="f.path">
                        <div class="flex items-center gap-1.5 py-0.5 px-1 rounded hover:bg-base-200">
                          <span class="badge badge-xs font-mono font-bold"
                                :class="f.status === 'A' ? 'badge-success' : (f.status === 'D' ? 'badge-error' : 'badge-warning')"
                                x-text="f.status || 'M'"></span>
                  ...
                  <!-- Downloads & CLI fallback actions -->
                  <div class="flex items-center justify-between gap-2 flex-wrap pt-1 border-t border-base-200/60">
                    <div class="flex items-center gap-1.5 flex-wrap">
                      <button type="button" @click="downloadGitChangeEnvelope()" class="btn btn-xs btn-ghost border border-base-300 gap-1 font-normal" title="Download envelope file">
                        <i class="ph ph-download-simple"></i>Envelope
                      </button>
                      <button type="button" @click="downloadGitBundle()" class="btn btn-xs btn-ghost border border-base-300 gap-1 font-normal" title="Download thin Git bundle">
                        <i class="ph ph-package"></i>Bundle
                      </button>
                      <button type="button" @click="copyGitChangeInstructions()" class="btn btn-xs btn-ghost border border-base-300 gap-1 font-normal" title="Copy fallback git CLI commands">
                        <i class="ph" :class="gitChangeInstructionsCopied ? 'ph-check text-success' : 'ph-terminal-window'"></i>CLI
                      </button>
                      <button type="button" @click="copyGitChangeStageLink()" class="btn btn-xs btn-ghost border border-base-300 gap-1 font-normal" title="Copy Stage URL link">
                        <i class="ph" :class="gitChangeLinkCopied ? 'ph-check text-success' : 'ph-link'"></i>Stage link
                      </button>
                    </div>
                    <span class="text-xs opacity-50 font-mono italic">Apply via Out below</span>
                  </div>

                  <!-- Status message / errors -->
                  <div x-show="gitChangeApplyStatus || gitChangeApplyError || gitChangeLinkError" class="text-xs font-mono pt-1">
                    <span x-show="gitChangeApplyStatus" class="text-info" x-text="gitChangeApplyStatus"></span>
                    <span x-show="gitChangeApplyError" class="text-error" x-text="gitChangeApplyError"></span>
                    <span x-show="gitChangeLinkError" class="text-warning" x-text="gitChangeLinkError"></span>
                  </div>
                </div>

                <!-- NO LENS STRIP. Out and Diff are not two views of one
                     thing: Out is where the set LEAVES (bundle, send), and a
                     diff is a way to READ two of its files. Reading happens
                     where reading happens, in the reader, which already walks
                     the staged set and can therefore pair two of it without a
                     second set of controls. So this side keeps only the
                     deposit surface, and it needs no tab to name it. -->
                ...
                <div class="flex items-center gap-1.5 flex-wrap">
                  ...
                  <button @click="send()" :disabled="sending || !destSpec.trim() || !items.length"
                    ...
                    <i class="ph" :class="sending ? 'ph-circle-notch animate-spin' : (gitChangeItem ? 'ph-git-commit' : 'ph-paper-plane-tilt')"></i>
                    <span x-text="sendLabel"></span>
                  </button>
                </div>

                <!-- Patch options when a git-change is staged -->
                <div x-show="gitChangeItem" class="flex items-center gap-4 flex-wrap text-xs pt-0.5">
                  <label class="flex items-center gap-1.5 cursor-pointer">
                    <input type="checkbox" x-model="gitChangeCreateBranch" class="checkbox checkbox-xs checkbox-primary">
                    <span>Create branch if absent</span>
                  </label>
                  <label class="flex items-center gap-1.5 cursor-pointer">
                    <input type="checkbox" x-model="gitChangeOpenDraftPr" class="checkbox checkbox-xs checkbox-primary">
                ...
                <!-- DECLARED destinations, one tap each. The picker above lists
                     ...
                     deposit lands somewhere reasonable and is never seen
                     again. Absent when nothing is declared, so a repo with no
                     pill is visibly missing a declaration rather than quietly
                     defaulting. -->
                ...
                <!-- ERRANDS. Everything a session needs your browser for, in one
                     list: a read of one of your repos, code for you to run
                     (its run block: where, how, and how the output returns),
                     or material only you have.
                     Nothing here runs on its own. Each card says in one line
                     whether the errand got what it came for, and the one green
                     button is the next step. The note is plain text through
                     x-text, never markup, since a session wrote it.
                     No backticks in this file's markup: it is a template
                     literal, and one ends the string mid-element. -->
                <div x-show="errands.length || errandsOpen" x-cloak class="flex flex-col gap-2 pt-2">
                  <div class="flex items-center gap-1.5 text-base font-semibold uppercase tracking-wide text-base-content/40">
                    <i class="ph ph-moped text-lg"></i><span>Errands</span>
                    <span x-show="errands.length" class="opacity-70" x-text="'· ' + errands.length"></span>
                    <span x-show="errandsLoading" class="loading loading-spinner loading-xs"></span>
                  </div>
                  <div x-show="errandsErr" x-cloak class="text-base text-error font-mono" x-text="errandsErr"></div>
                  <div x-show="!errandsLoading &amp;&amp; !errandsErr &amp;&amp; !errands.length" x-cloak
                       class="text-base opacity-60">No errands are open.</div>
                  <template x-for="e in errands" :key="e.id">
                    <div class="border rounded-lg bg-base-100 p-2.5 flex flex-col gap-2"
                         :class="e.id === errandFocus ? 'border-primary ring-1 ring-[var(--color-primary)]' : 'border-base-300'">
                      <div class="flex items-center gap-2 min-w-0">
                        <i class="ph text-lg shrink-0" :class="errandIcon(e)"></i>
                        <span class="font-semibold truncate" x-text="e.title"></span>
                        <span class="badge badge-ghost badge-sm shrink-0" x-text="errandKind(e)"></span>
                        <span x-show="e.purpose" class="badge badge-outline badge-sm shrink-0" x-text="e.purpose"></span>
                        <span x-show="e.age" class="text-sm opacity-60 shrink-0" x-text="e.age"></span>
                      </div>
                      <div x-show="e.note &amp;&amp; e.note !== e.title" class="text-base whitespace-pre-line" x-text="e.note"></div>
                      <div class="flex items-center gap-x-3 gap-y-1 flex-wrap font-mono text-sm opacity-60">
                        <span x-show="e.repo" class="inline-flex items-center gap-1.5 min-w-0">
                          <i class="ph ph-git-branch shrink-0"></i><span class="truncate" x-text="e.repo"></span></span>
                        <span x-show="e.host" class="inline-flex items-center gap-1.5 min-w-0">
                          <i class="ph ph-globe-simple shrink-0"></i><span class="truncate" x-text="e.host"></span></span>
                        <span x-show="e.dest" class="inline-flex items-center gap-1.5 min-w-0">
                          <i class="ph ph-tray-arrow-down shrink-0"></i><span class="truncate" x-text="e.dest"></span></span>
                      </div>
                      <!-- THE RUN BLOCK, resolved: what the kit filled from the
                           method registry shows the same as what the record
                           said, with the method's rules beside the script's
                           bytes, so what you are about to run is on the card. -->
                      <template x-if="e.run">
                        <div class="flex flex-col gap-1.5">
                          <div class="flex items-center gap-1 flex-wrap">
                            <span class="badge badge-sm badge-ghost" x-text="e.run.venue"></span>
                            <span class="badge badge-sm badge-ghost" x-text="e.run.outputReturn"></span>
                            <span x-show="e.run.outputType" class="badge badge-sm badge-ghost" x-text="e.run.outputType"></span>
                            <span class="badge badge-sm" :class="e.run.outputSigned ? 'badge-info' : 'badge-ghost'"
                                  x-text="e.run.outputSigned ? 'signed' : 'unsigned'"></span>
                          </div>
                          <div class="flex flex-col gap-1.5 lg:flex-row">
                            <ul class="text-sm list-disc pl-4 opacity-70 lg:w-56 shrink-0">
                              <template x-for="rule in errandRules(e)" :key="rule"><li x-text="rule"></li></template>
                            </ul>
                            <div class="flex flex-col gap-1 min-w-0 grow">
                              <a :href="errandScriptUrl(e)" target="_blank" rel="noopener"
                                 class="link link-hover font-mono text-sm truncate" x-text="e.run.script"></a>
                              <pre x-show="e.scriptText" class="font-mono text-sm bg-base-200 rounded p-2 max-h-64 overflow-auto whitespace-pre"
                                   x-text="e.scriptText"></pre>
                              <div x-show="e.scriptErr" class="text-sm text-error font-mono" x-text="e.scriptErr"></div>
                            </div>
                          </div>
                        </div>
                      </template>
                      <div x-show="errandGrade(e)" class="flex items-center gap-1.5 text-base"
                           :class="{ 'text-success': errandGrade(e)?.level === 'green', 'text-warning': errandGrade(e)?.level === 'amber', 'opacity-60': errandGrade(e)?.level === 'red' }">
                        <i class="ph shrink-0" :class="{ 'ph-check-circle': errandGrade(e)?.level === 'green', 'ph-warning-circle': errandGrade(e)?.level === 'amber', 'ph-circle-dashed': errandGrade(e)?.level === 'red' }"></i>
                        <span x-text="errandGrade(e)?.text"></span>
                      </div>
                      <div class="flex items-center gap-1.5 flex-wrap">
                        <button x-show="errandIsRead(e)" type="button" @click="runErrand(e)" :disabled="!!e.busy"
                                class="btn btn-xs btn-ghost border border-base-300 gap-1 font-normal">
                          <i class="ph ph-play"></i><span x-text="e.outcome ? 'Run again' : 'Run'"></span></button>
                        <button x-show="e.run &amp;&amp; e.run.method !== 'courier-bookmark'" type="button" @click="copyErrandScript(e)"
                                :disabled="!e.scriptText"
                                class="btn btn-xs btn-ghost border border-base-300 gap-1 font-normal">
                          <i class="ph" :class="e.copied ? 'ph-check' : 'ph-copy'"></i>Copy script</button>
                        <a x-show="e.run?.method === 'courier-bookmark'" :href="e.url" target="_blank" rel="noopener"
                           class="btn btn-xs btn-ghost border border-base-300 gap-1 font-normal">
                          <i class="ph ph-arrow-square-out"></i>Open the page</a>
                        <button x-show="!errandIsRead(e)" type="button" @click="aim(e.dest)"
                                class="btn btn-xs btn-ghost border border-base-300 gap-1 font-normal">
                          <i class="ph ph-crosshair-simple"></i>Aim here</button>
                        <a x-show="errandTaskUrl(e)" :href="errandTaskUrl(e)" target="_blank" rel="noopener"
                           class="btn btn-xs btn-ghost border border-base-300 gap-1 font-normal">
                          <i class="ph ph-ticket"></i>Task</a>
                      </div>
                      <div class="flex items-center gap-1.5 flex-wrap">
                        <input type="text" x-model="e.message" :disabled="!!e.busy"
                               placeholder="a note on closing (required to decline)"
                               class="input input-xs input-bordered grow min-w-40">
                        <button type="button" @click="completeErrand(e)"
                                :disabled="!!e.busy || !errandGrade(e) || errandGrade(e).level === 'red'"
                                class="btn btn-xs gap-1 font-normal"
                                :class="errandGrade(e)?.level === 'green' ? 'btn-success' : 'btn-warning'">
                          <i class="ph ph-check"></i><span x-text="errandOkLabel(e)"></span></button>
                        <button type="button" @click="closeErrand(e, false)" :disabled="!!e.busy"
                                class="btn btn-xs btn-ghost border border-base-300 gap-1 font-normal">
                          <i class="ph ph-prohibit"></i>Decline</button>
                      </div>
                    </div>
                  </template>
                  <!-- Drag installs it, tapping copies it. Both are by hand:
                       a page cannot write a bookmark, on any browser. -->
                  <div x-show="pointer &amp;&amp; (errandsOpen || errands.some(x => x.run?.method === 'courier-bookmark'))" x-cloak
                       class="flex items-center gap-1.5 flex-wrap pt-0.5">
                    <a :href="pointer" @click.prevent="copyPointer()" draggable="true"
                       class="btn btn-xs btn-ghost border border-base-300 gap-1 font-normal"
                       title="Drag to your bookmarks bar to install, or tap to copy it for a new bookmark's URL">
                      <i class="ph" :class="pointerCopied ? 'ph-check' : 'ph-bookmark-simple'"></i>Courier bookmarklet</a>
                    <a href="https://github.com/mehrlander/web-tools/blob/main/courier/README.md"
                       target="_blank" rel="noopener"
                       class="btn btn-xs btn-ghost border border-base-300 gap-1 font-normal">
                      <i class="ph ph-book-open"></i>How it works</a>
                  </div>
                </div>
              </div>
            </div>

            <!-- Same house handle as the docked swipe-deck. It is a real grid
                 item rather than an absolutely-positioned decoration, so its
                 centre is the boundary the two pane sizes are measured from. -->
            <div x-ref="splitHandle" class="stage-split" aria-label="Resize the Stage panes"></div>

            <!-- ASIDE: what is staged, then where to get more. -->
            <div x-ref="splitAside" class="flex flex-col gap-3 min-w-0">
              ...
              <div class="flex flex-col gap-1.5">
                <!-- The label row carries the set's own actions, in the order
                     they escalate: a link serializes it into a URL, a sweep
                     empties it. Both act on exactly the list beneath them,
                     where the per-item x already removes one. Send lives with the bundle instead,
                ...
                <div class="flex items-center gap-1 text-base font-semibold uppercase tracking-wide text-base-content/40">
                  ...
                  <!-- The GLYPHS match the row above (text-xl); the BOXES stay
                       btn-sm. This many default-size buttons plus the label
                       overflow the sidebar, wrapping the row and clipping the
                       last one. The icon is what reads as size, so growing it
                       and leaving the box compact answers the complaint
                       without the wrap.
                  ...
                  <!-- Fourth intake, and the one that was invisible: material
                       on a page no session can reach, carried here by a
                       bookmarklet you run ON that page. The button cannot
                       fetch anything, since reaching the page is the whole
                       capability the app lacks; it shows what is open and
                       hands over the pointer. -->
                  <button @click="toggleErrands()" class="btn btn-square btn-ghost btn-sm"
                          :class="errandsOpen ? 'text-primary' : 'hover:text-primary'"
                          title="Errands: what sessions need your browser for, and the courier bookmarklet">
                ...
                <div x-show="dictOpen" x-cloak data-no-swipe x-ref="dictLayer"
                    ...
                    <div class="flex flex-col gap-1 w-12 shrink-0">
                      ...
                      <template x-for="(m, i) in dictMarks" :key="i">
                        <button @pointerdown.prevent="dictMark(m)"
                                ...
                                :title="m === 'stitch' ? 'Undo the sentence break the pause invented: the full stop goes and the capital comes down'
                  ...
                  <div class="flex flex-wrap items-center gap-2">
                    ...
                    <!-- The pencil is a MODE SWITCH, not a one-way door, which
                         ...
                         grows the word DONE and wears a CHECK, green, matching
                         the pin cluster's confirm key. NOT A MICROPHONE and not
                         an amber fill: with the real mic button dimmed beside
                         it, those signals stack into "recording" while the
                         keyboard is open and nothing is being heard. -->
                ...
                <!-- WHAT THE PASTE CARRIED, and which of it you want. One copy
                     ...
                     EVERY flavor is listed, the taken one ticked. A list of
                     leftovers alone is an ADD list: bringing the html in costs
                     one tap and choosing it INSTEAD costs a tap plus hunting
                     the text down in the list below. A tick that toggles says
                     both in one control.
                     ...
                     all on a phone, which is where this bar mostly gets read,
                     so without the eye the flavor with the addresses in it
                     cannot be opened before you commit to staging it.
                ...
                <div x-show="offers.length" x-cloak
                  ...
                  <!-- ONE PILL PER FLAVOR, and its menu is what can be done
                       with that flavor. A conversion on a pill of its own puts
                       a derivation beside the formats it is made from; on the
                       flavor it converts, "to markdown" sits
                ...
                <!-- THE NOTE EDITOR, one panel above the staged set rather than
                     a field inside a row. A note can be about a ref or about a
                     local file, and those are drawn in two different lists, so
                     one panel is also the only place that serves both without
                     being written twice. Enter inserts a newline, because this
                     is prose; the commit is the button or Cmd/Ctrl-Enter. -->
                <template x-if="noteSubject">
                  <div class="border border-primary/40 rounded-lg bg-base-100 p-2 flex flex-col gap-2">
                    <div class="flex items-center gap-1.5 text-base min-w-0">
                      <i class="ph ph-chat-teardrop-text text-primary shrink-0"></i>
                      <span class="opacity-60 shrink-0">Note on</span>
                      <span class="truncate font-mono"
                            x-text="noteSubject.path || noteSubject.name"></span>
                    </div>
                    <textarea x-ref="noteInput" x-model="noteDraft" rows="4"
                              @keydown.escape.prevent.stop="cancelNote()"
                              @keydown.enter.meta.prevent="commitNote()"
                              @keydown.enter.ctrl.prevent="commitNote()"
                              placeholder="What is this file, and why is it going there?"
                              class="textarea textarea-bordered w-full text-lg sm:text-base leading-relaxed"></textarea>
                    <div class="flex items-center gap-2">
                      <!-- The deposited name, shown rather than explained: the
                           whole convention is that the pair is legible by name,
                           so the one place to prove it is where the note is
                           written. -->
                      <span class="font-mono text-sm opacity-50 truncate"
                            x-text="noteName(noteSubject)"></span>
                      <span class="grow"></span>
                      <button @click="cancelNote()" class="btn btn-ghost btn-sm">Cancel</button>
                      <button @click="commitNote()" class="btn btn-primary btn-sm"
                              x-text="noteDraft.trim() ? 'Save note' : 'Remove note'"></button>
                    </div>
                  </div>
                </template>
                    ...
                    <div class="p-1">
                      <template x-for="it in g.items" :key="itemKey(it)">
                        <div class="group flex items-center justify-between gap-2 px-2 py-1 rounded hover:bg-base-200 text-base">
                          ...
                          <div class="grow"></div>
                          <!-- A ref takes a note as readily as a local file
                               does, and it is the case where one is worth most:
                               a copied file lands under its own name and the
                               deposit keeps no record of the repo it came from,
                               so without the note's source line the origin
                               is simply gone. Filled where a note exists, so
                               the row says whether there is one to read. -->
                          <button @click="startNote(it)" :title="noteLabel(it)"
                                  class="btn btn-ghost w-5 h-5 min-h-0 p-0 shrink-0 hover:text-primary"
                                  :class="noteOf(it) ? 'text-primary opacity-100' : 'opacity-30 hover:opacity-100'">
                            <i :class="noteOf(it) ? 'ph-fill ph-chat-teardrop-text' : 'ph ph-chat-teardrop-text'"></i>
                          </button>
                      ...
                      <div class="group flex items-center gap-2 px-2 py-1 rounded hover:bg-base-200 text-base">
                        ...
                        <template x-if="renameId !== it.id">
                          <button @click="view(it)" class="flex items-center gap-1.5 min-w-0 grow cursor-pointer hover:text-primary text-left">
                            <!-- A note draws as a note. It is an ordinary local
                                 file and deposits as one, so it keeps its row
                                 rather than hiding inside its subject's, but
                                 the icon says which of the two it is without
                                 reading the name. -->
                            <i class="ph shrink-0"
                               :class="it.isGitChange ? 'ph-git-commit text-primary' : (it.noteFor ? 'ph-chat-teardrop-text text-primary' : 'ph-file-dashed text-warning')"></i>
                            ...
                            <span x-show="it.isGitChange" class="badge badge-xs badge-primary font-mono shrink-0">git-change</span>
                        ...
                        <!-- A note row carries no bubble of its own: a note
                             about a note is a thing this design has no answer
                             for and no call for. -->
                        <button x-show="renameId !== it.id && !it.noteFor" @click="startNote(it)"
                                :title="noteLabel(it)"
                                class="btn btn-ghost w-5 h-5 min-h-0 p-0 shrink-0 hover:text-primary"
                                :class="noteOf(it) ? 'text-primary opacity-100' : 'opacity-30 hover:opacity-100'">
                          <i :class="noteOf(it) ? 'ph-fill ph-chat-teardrop-text' : 'ph ph-chat-teardrop-text'"></i>
              ...
              <!-- ── Add: three pills ────────────────────────────────────────
                   ...
                   The part of one box that was about cost rather than layout
                   survives: BROWSE AND SEARCH SHARE ONE TREE CACHE. Entering
                   a repo reads its tree and Search reads only what is still
                   missing, so browsing pays
          ...
          <!-- THE PREVIEW IS NOT HERE. It is a swipe-deck takeover, opened from
               ...
               there is no markup for it in this template. kits/swipe-deck.js
               owns the header title and subtitle, the touch swipe, the arrow
               keys, the jump-out link, history-backed dismissal and correct
               nesting. -->
      ...
      renameId: null,
      renameDraft: '',
      // The note editor is keyed on the SUBJECT'S KEY rather than on an id,
      // because a note can be about a ref as readily as about a local file and
      // only a ref has no id. One subject edits at a time, which is what lets
      // the panel below keep a single x-ref.
      noteId: null,
      noteDraft: '',
      ...
      // own state and reads none of the others': they are three questions, and
      // the template above says why one box cannot ask them.
      ...
      // Errands: what the courier can fetch from a page no session reaches.
      // Lazily loaded, because an unopened panel should cost no request.
      errandsOpen: false,
      errands: [],          // every open errand, normalized by kits/errands.js, plus card state
      errandsLoading: false,
      errandsErr: '',
      errandFocus: ERRAND_BOOT,   // the card a ?errand= link opened
      lastSend: null,       // what the last deposit did, read by an errand's send-and-close
      // The courier intake: this window opened as a popup by the courier
      // bookmarklet, receiving a page's material by postMessage. Plain data
      // only; the opener window is never held in reactive state.
      courier: null,        // { host, url, links, errand, state, note }
      pointer: '',          // the bookmarklet source, read from the same place
      pointerCopied: false,
      sendStatus: '',
      linkCopied: false,
      ...
      bundleCopied: false,
      _cache: {},
      // The Stage's standing desktop allocation. Null means the fluid CSS
      // default above; once moved it becomes the percentage occupied by the
      // right-hand staged-items pane. The gesture and accessibility contract
      // stay in kits/dock-split.js, shared with the swipe-deck.
      _stageAsidePct: null,
      _stageSplit: null,
      _stageSplitRO: null,
      ...
      init() {
        ...
        // prevent.
        this.loadErrands();
        // `?view=stage&tab=errands` opens the errand panel on arrival, which is
        // where the courier sends you when you fire it on our own origin and it
        // has nothing to run on the page you are reading.
        if (window.__shell?.stageTab === 'errands') this.toggleErrands();
        ...
        this.$watch(() => Alpine.store('browser').stageCompare, () => this.focusFromStore());
        this.$watch(() => window.__shell?.view, view => { if (view === 'stage') this.focusFromStore(); });
        ...
        // have landed (the shell seeds them after mount). The diff lives in the
        // reader, so the link's intent is served by putting the reader in front
        // of it rather than by selecting anything on the page.
        // AN AIM HANDED OVER BEFORE THE BENCH MOUNTED. The viewer's reception
        // stages an extract and opens the stage in one gesture, and the bench
        // mounts on the way in, after the call that aimed it; so the aim is
        // parked on the store and read here, once, ahead of the link's.
        try {
          const pending = Alpine.store('browser').stageAim;
          if (pending && !this.destSpec.trim()) { this.destSpec = pending; Alpine.store('browser').stageAim = ''; }
        } catch { /* no store, no aim */ }
        try {
          ...
          if (lk.take) this.takeFromLink(lk);
        } catch {}
        this.courierIntake();
        window.addEventListener('errand-result', (ev) => this.errandResult(ev.detail));
        const autoDiff = () => {
        ...
        autoDiff();
        const checkGitChange = () => {
          if (this.gitChangeItem) {
            this.verifyGitChange();
            const env = this.gitChangeEnv;
            if (env?.repository && env?.proposed_branch && (!this.destSpec?.trim() || this.destSpec === this._lastGitChangeDest)) {
              this._lastGitChangeDest = env.repository + '@' + env.proposed_branch;
              this.aim(this._lastGitChangeDest);
            }
          }
        };
        this.$watch(() => this.gitChangeItem, checkGitChange);
        checkGitChange();
        // THE PASTE LISTENER IS THE HOST'S, and its absence here is deliberate.
        // A window listener registered here would have to gate on
        // `view === 'stage'`, which makes the gesture work only on the view you
        // are pasting INTO and only after the bench has mounted. The fold is
        // window.StageIntake.takePaste and show-repo's shell owns the gesture,
        // the way it already owns the drop. A second listener here could not
        // coordinate with it in any
      ...
      stageSplitMin() {
        const w = this.$refs.splitFrame?.getBoundingClientRect().width || 1;
        // Eighteen rem keeps the staged rows and their action strip useful.
        return Math.min(50, Math.ceil(288 / w * 100));
      },

      stageSplitMax() {
        const w = this.$refs.splitFrame?.getBoundingClientRect().width || 1;
        // Reserve the same floor for Out, plus the divider and its two gaps.
        return Math.max(this.stageSplitMin(), Math.floor((w - 304) / w * 100));
      },

      stageSplitValue() {
        if (Number.isFinite(this._stageAsidePct)) return this._stageAsidePct;
        const frame = this.$refs.splitFrame?.getBoundingClientRect();
        const aside = this.$refs.splitAside?.getBoundingClientRect();
        return frame?.width ? Math.round(aside.width / frame.width * 100) : 50;
      },

      setStageSplit(pct) {
        const lo = this.stageSplitMin(), hi = this.stageSplitMax();
        const next = Math.max(lo, Math.min(hi, Number(pct) || 0));
        this._stageAsidePct = next;
        this.$refs.splitFrame?.style.setProperty('--stage-aside-width', next + '%');
        return next;
      },

      mountStageSplit() {
        if (this._stageSplit || !window.dockSplit) return;
        const frame = this.$refs.splitFrame, handle = this.$refs.splitHandle;
        if (!frame || !handle) return;
        let remembered = 0;
        try { remembered = Number(localStorage.getItem('stageAsidePct')); } catch {}
        if (remembered > 0 && remembered < 100) this.setStageSplit(remembered);
        this._stageSplit = window.dockSplit.attach(handle, {
          axis: 'col', from: 'end', label: 'Resize the Stage panes',
          bounds: () => frame.getBoundingClientRect(),
          value: () => this.stageSplitValue(),
          min: () => this.stageSplitMin(),
          max: () => this.stageSplitMax(),
          onChange: pct => this.setStageSplit(pct),
          onCommit: pct => {
            const next = this.setStageSplit(pct);
            try { localStorage.setItem('stageAsidePct', String(next)); } catch {}
          },
        });
        // Container width can change without a window resize when a reader is
        // docked. Re-clamp a remembered split and keep its ARIA bounds current.
        if (window.ResizeObserver) {
          this._stageSplitRO = new ResizeObserver(() => {
            if (Number.isFinite(this._stageAsidePct)) this.setStageSplit(this._stageAsidePct);
            this._stageSplit?.refresh();
          });
          this._stageSplitRO.observe(frame);
        }
      },

      destroy() {
        this._stageSplitRO?.disconnect();
        this._stageSplitRO = null;
        this._stageSplit?.destroy();
        this._stageSplit = null;
      },

      // ── GIT CHANGE HANDOFF ────────────────────────────────────────────────
      gitChangeVerified: 'idle',
      gitChangeVerificationError: '',
      gitChangeApplyArmed: false,
      gitChangeApplying: false,
      gitChangeApplyStatus: '',
      gitChangeApplyError: '',
      gitChangeCommitResult: null,
      gitChangePrCreated: null,
      gitChangeCreateBranch: true,
      gitChangeOpenDraftPr: true,
      gitChangeInstructionsCopied: false,
      gitChangeLinkCopied: false,
      gitChangeLinkError: '',

      get gitChangeItem() {
        return this.items.find(it => it.isGitChange || (it.local && it.isText && window.StageIntake?.isGitChange?.(it.text))) || null;
      },

      get gitChangeEnv() {
        const it = this.gitChangeItem;
      ...
      armGitChangeApply() {
        this.gitChangeApplyArmed = true;
        this.sendArmed = true;
      },

      disarmGitChangeApply() {
        this.gitChangeApplyArmed = false;
        this.sendArmed = false;
      },

      async verifyGitChange() {
        const env = this.gitChangeEnv;
        if (!env) {
          this.gitChangeVerified = 'idle';
          return;
        }
        this.gitChangeVerified = 'verifying';
        this.gitChangeVerificationError = '';
        try {
          if (!window.GitChange && window.gh?.load) await window.gh.load('kits/git-change.js');
          if (!window.GitChange) throw new Error('GitChange kit unavailable');
          const res = await window.GitChange.verifyEnvelope(env);
          if (res.valid) {
            this.gitChangeVerified = 'verified';
          } else {
            this.gitChangeVerified = 'mismatch';
            this.gitChangeVerificationError = res.error || 'Checksum mismatch';
          }
        } catch (e) {
          this.gitChangeVerified = 'mismatch';
          this.gitChangeVerificationError = e.message || String(e);
        }
      },

      copyGitChangeText(text, toastMsg) {
        if (!text) return;
        const toast = Alpine.store('toast');
        if (navigator.clipboard?.writeText) {
          navigator.clipboard.writeText(text)
            .then(() => toast?.('copy', toastMsg || 'Copied to clipboard', 'alert-success', 2000))
            .catch(() => toast?.('warning', 'Clipboard write failed', 'alert-error', 3000));
        }
      },

      downloadGitChangeEnvelope() {
        const it = this.gitChangeItem;
        if (!it || !it.text) return;
        const blob = new Blob([it.text], { type: 'application/json' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = it.name || 'change.git-change.json';
      ...
      downloadGitBundle() {
        const env = this.gitChangeEnv;
        if (!env || !env.payload?.data) return;
        try {
          const binaryStr = atob(env.payload.data);
          ...
          const a = document.createElement('a');
          a.href = url;
          const branch = env.proposed_branch || 'change';
          const slug = (branch.replace(/^[^/]+\//, '') || 'change').replace(/[^a-zA-Z0-9._-]/g, '-');
          a.download = slug + '.git-bundle';
          document.body.appendChild(a);
          a.click();
          document.body.removeChild(a);
          URL.revokeObjectURL(url);
        } catch (e) {
          Alpine.store('toast')?.('warning', 'Bundle download failed: ' + e.message, 'alert-error', 4000);
        }
      },

      copyGitChangeInstructions() {
        const env = this.gitChangeEnv;
        if (!env) return;
        const branch = env.proposed_branch || 'feature-branch';
        const slug = (branch.replace(/^[^/]+\//, '') || 'change').replace(/[^a-zA-Z0-9._-]/g, '-');
        const bundleFile = slug + '.git-bundle';
        const text =
`# Apply Git Change via CLI:
# 1. Download ${bundleFile}
# 2. In your local repository clone:
git fetch ${bundleFile} refs/heads/${branch}:${branch}
git checkout ${branch}`;
        if (navigator.clipboard?.writeText) {
          navigator.clipboard.writeText(text).then(() => {
            this.gitChangeInstructionsCopied = true;
            setTimeout(() => { this.gitChangeInstructionsCopied = false; }, 2500);
          });
        }
      },

      async copyGitChangeStageLink() {
        const it = this.gitChangeItem;
        if (!it) return;
        this.gitChangeLinkError = '';
        try {
          const base = (typeof location !== 'undefined' && location.origin && location.pathname)
            ? (location.origin + location.pathname)
            : '';
          const url = await window.StageLink.mintWithLocals([it], base);
          if (navigator.clipboard?.writeText) {
            await navigator.clipboard.writeText(url);
            this.gitChangeLinkCopied = true;
            setTimeout(() => { this.gitChangeLinkCopied = false; }, 2500);
          }
        } catch (e) {
          this.gitChangeLinkError = e.message || 'Link too large for URL fragment';
          Alpine.store('toast')?.('warning', this.gitChangeLinkError, 'alert-warning', 4000);
        }
      },

      async executeGitChangeApply(destOverride) {
        if (this.gitChangeApplying || this.sending) return;
        const env = this.gitChangeEnv;
        if (!env) return;

        const gh = this.gh || Alpine.store('browser').gh;
        if (!gh || !gh.token) {
          Alpine.store('toast')?.('warning', 'GitHub authentication token required to apply changes', 'alert-error', 5000);
          return;
        }

        this.gitChangeApplying = true;
        this.sending = true;
        this.gitChangeApplyStatus = this.sendStatus = 'Verifying change package...';
        this.gitChangeApplyError = '';

        try {
          if (!window.GitChange && window.gh?.load) await window.gh.load('kits/git-change.js');
          if (!window.GitChange) throw new Error('GitChange kit unavailable');

          const inspected = await window.GitChange.inspectChange(env, {
            fetchBaseBlob: async (sha) => {
              if (gh.getRawBlob) return gh.getRawBlob(sha);
              if (gh.req) return gh.req('git/blobs/' + sha);
              if (gh.getBlob) return gh.getBlob(sha);
              throw new Error('Cannot fetch base blob ' + sha + ': no API available');
            }
          });

          const repo = destOverride?.repo || env.repository;
          const branch = destOverride?.ref || env.proposed_branch;

          if (branch === 'main' || branch === 'master') {
            throw new Error('Refusing to write directly to default branch: ' + branch);
          }

          const targetGh = (gh.repo === repo) ? gh : new gh.constructor({ token: gh.token, repo, ref: branch });

          if (!targetGh.commitFiles && window.gh?.load) await window.gh.load('gh-transfer.js');
          if (!targetGh.commitFiles) throw new Error('gh-transfer.js not available');

          this.gitChangeApplyStatus = this.sendStatus = 'Extracting changed files...';
          const filesToCommit = [];
          for (const f of inspected.changedFiles) {
            const obj = inspected.objects.get(f.sha);
            filesToCommit.push({
              path: f.path,
              mode: f.mode || '100644',
              sha: f.sha,
          ...
          let commitMessage = (env.commit && env.commit.message) || env.commit?.subject || 'Apply git change';
          const trailer = 'Co-Authored-By: Codex <codex@openai.com>';
          if (!commitMessage.includes(trailer)) {
            commitMessage = commitMessage.trimEnd() + '\n\n' + trailer + '\n';
          }

          this.gitChangeApplyStatus = this.sendStatus = 'Committing to ' + repo + '@' + branch + '...';
          const commitResult = await targetGh.commitFiles(filesToCommit, {
            branch,
            message: commitMessage,
            refuseDefaultBranch: true,
            requireBaseCommit: env.base,
            expectedTree: env.expected_tree,
            createBranchIfAbsent: this.gitChangeCreateBranch,
            baseCommit: env.base,
            onProgress: (done, total, path, phase) => {
              const msg = phase === 'ok' ? 'Uploaded ' + done + '/' + total + ' files' : 'Uploading ' + path + '...';
              this.gitChangeApplyStatus = this.sendStatus = msg;
            }
          });

          this.gitChangeCommitResult = commitResult;

          if (commitResult.alreadyApplied) {
            const msg = 'Branch ' + branch + ' already has tree ' + env.expected_tree.slice(0, 8);
            this.gitChangeApplyStatus = this.sendStatus = msg;
            Alpine.store('toast')?.('info', 'Branch ' + branch + ' is already up-to-date', 'alert-info', 4000);
          } else {
            const msg = 'Committed ' + commitResult.sha.slice(0, 8) + ' to ' + branch;
            this.gitChangeApplyStatus = this.sendStatus = msg;
            Alpine.store('toast')?.('success', 'Applied commit ' + commitResult.sha.slice(0, 8) + ' to ' + branch, 'alert-success', 4000);
          }

          if (this.gitChangeOpenDraftPr && targetGh.createPull) {
            this.gitChangeApplyStatus = this.sendStatus = 'Opening draft PR...';
            const prOpts = {
              title: env.draft_pr?.title || env.commit?.subject || ('Changes for ' + branch),
              body: env.draft_pr?.body || ('Automated git change handoff applied via Web Tools Stage.\n\n' + commitMessage),
              base: env.draft_pr?.base || 'main',
              head: branch,
              draft: true,
            };
            const pr = await targetGh.createPull(prOpts);
            this.gitChangePrCreated = pr;
            if (pr.alreadyExists) {
              const prMsg = ' (PR #' + pr.number + ' already exists)';
              this.gitChangeApplyStatus += prMsg;
              this.sendStatus += prMsg;
            } else {
              const prMsg = ' (PR #' + pr.number + ' created)';
              this.gitChangeApplyStatus += prMsg;
              this.sendStatus += prMsg;
              Alpine.store('toast')?.('git-pull-request', 'Opened draft PR #' + pr.number, 'alert-success', 5000);
            }
          }
          ...
          this.gitChangeApplyError = err.message || String(err);
          this.sendStatus = '';
          Alpine.store('toast')?.('warning', 'Apply failed: ' + this.gitChangeApplyError, 'alert-error', 6000);
        } finally {
          this.gitChangeApplying = false;
          this.sending = false;
        }
      },
      ...
      async onPageDrop(e) {
        ...
        if (files.length === 1 && await window.__shell?.takeCorrespondenceFile?.(files[0])) return;
        if (!files.length && await window.__shell?.takeCorrespondence?.(e.dataTransfer?.getData('text') || '', { source: 'drop' })) return;
      ...
      // A NOTE SITS DIRECTLY UNDER ITS SUBJECT rather than wherever the intake
      // appended it, since the pair is the unit a reader is looking at. A note
      // about a REF has no row to sit under here, because the ref groups are
      // drawn above this list, so it trails under its own name, which carries
      // the subject's path and therefore says what it is about.
      get localItems() {
        const all = this.items.filter(it => it.local);
        const notes = all.filter(it => it.noteFor);
        const placed = new Set();
        const out = [];
        for (const it of all) {
          if (it.noteFor) continue;
          out.push(it);
          for (const n of notes) {
            if (n.noteFor === this.itemKey(it)) { out.push(n); placed.add(n); }
          }
        }
        return out.concat(notes.filter(n => !placed.has(n)));
      },
      get groups() {
        const map = new Map();
      ...
      get sendLabel() {
        ...
        if (this.gitChangeItem) {
          if (!this.sendArmed) return 'Apply patch';
          const branch = this.resolvedDest?.ref || this.gitChangeEnv?.proposed_branch || 'branch';
          return 'Apply to ' + branch + ' ?';
        }
      ...
      // ── The courier intake ───────────────────────────────────────────────
      // THE STAGE AS THE COURIER'S POPUP. The bookmarklet opens this view with
      // `courier=1` and keeps the window; the visited page's code sends its
      // material here by postMessage, and this window holds the token. Three
      // rules make that safe with a sender from any origin:
      //   only the window that opened this one is heard, and only in this mode;
      //   what arrives is content, staged for reading, never sent on arrival;
      //   the destination comes from an errand record read here, never from a
      //   message;
      //   the host is the sender's origin as the browser reports it, never the
      //   url a message claims, so a page is sent only its own errand's script.
      // The one thing that goes back is an errand's script, to run on its page.
      // Contract: courier/README.md.
      // A `#take=` link's text goes through the one fold every intake uses, so
      // it is named, sniffed and drawn exactly as a paste of the same text
      // would be. A reload of the same link re-reads the fragment, so text
      // already on the stage is not staged a second time.
      takeFromLink(lk) {
        const text = String(lk?.take || '');
        if (!text) return [];
        if (this.localItems.some(it => it.text === text)) return [];
        return window.StageIntake.take({ text, name: lk.takeName || undefined });
      },

      courierIntake() {
        const opener = window.opener;   // a local, never state: a reactive proxy probing a cross-origin window throws
        if (!COURIER_BOOT || !opener) return;
        this.courier = { host: '', url: '', links: 0, errand: null, state: 'waiting', note: '' };
        window.addEventListener('message', (e) => {
          if (e.source !== opener) return;
          const d = e.data;
          if (!d || typeof d !== 'object') return;
          if (d.type === 'courier-page') this.courierPage(d, (msg) => opener.postMessage(msg, e.origin), e.origin);
          else if (d.type === 'courier-ran') this.courierRan(d);
        });
        opener.postMessage({ type: 'courier-ready' }, '*');
      },

      async courierPage(d, reply, origin) {
        let host = '';
        try { host = new URL(String(origin)).hostname; } catch { return; }
        if (!host) return;
        const links = (Array.isArray(d.links) ? d.links : [])
          .map(l => ({ text: String(l?.text || '').replace(/\s+/g, ' ').trim(), href: String(l?.href || '') }))
          .filter(l => /^https?:/i.test(l.href));
        const cell = (v) => v.replace(/\|/g, '\\|');
        const sel = String(d.selection || '').trim();
        const md = [
          '# ' + (String(d.title || '').trim() || host),
          '',
          '- Page: ' + d.url,
          '- Collected: ' + new Date().toISOString(),
          '- Links: ' + links.length,
          ...(sel ? ['', '## Selection', '', ...sel.split('\n').map(l => '> ' + l)] : []),
          '',
          '| Text | URL |',
          '| --- | --- |',
          ...links.map(l => '| ' + cell(l.text) + ' | ' + cell(l.href) + ' |'),
        ].join('\n') + '\n';
        const stamp = new Date().toISOString().slice(0, 10);
        this.courierStage([window.StageIntake.textItem(host + '-' + stamp + '-links.md', md)]);
        this.courier = { ...this.courier, host, url: String(d.url), links: links.length, state: 'staged' };
        // THE DEFAULT PLACE, so a page with no errand is one send from saved. An
        // errand below re-aims at its own result; a destination you typed wins.
        if (!this.destSpec.trim()) this.aim(COURIER_CAPTURES);
        const errand = await this.courierErrandFor(host);
        if (!errand) return;
        this.courier = { ...this.courier, errand: { id: errand.id, title: errand.title, file: errand.file }, state: 'running' };
        if (errand.dest) this.aim(errand.dest);
        this.errandFocus = errand.id;
        reply({ type: 'courier-run', id: errand.id, code: errand.code, errand: { id: errand.id, url: errand.url } });
      },

      courierStage(fresh) {
        Alpine.store('browser').stage = [...(Alpine.store('browser').stage || []), ...fresh];
      },

      courierRan(d) {
        const e = this.courier?.errand;
        if (!e || d.id !== e.id) return;
        if (d.error) { this.courier = { ...this.courier, state: 'failed', note: String(d.error) }; return; }
        const name = e.file || e.id + '.md';
        this.courierStage([window.StageIntake.textItem(name, String(d.text ?? ''))]);
        this.courier = { ...this.courier, state: 'ran' };
      },

      // The open courier-bookmark errand for a host, with its script read from
      // the repo, ref and path its run block names.
      async courierErrandFor(host) {
        if (!this.errands.length) await this.loadErrands();
        const e = this.errands.find(x => x.run?.method === 'courier-bookmark' && x.host === host);
        if (!e) return null;
        const at = window.Errands.parseScript(e.run.script);
        try { return { ...e, code: (await this.srcGh(at.repo, at.ref).get(at.path)).text }; }
        catch (err) { this.courier = { ...this.courier, state: 'failed', note: 'Could not read ' + e.run.script + ': ' + err.message }; return null; }
      },

      // ── Errands ──────────────────────────────────────────────────────────
      // Every open errand, from the one place one is filed: the errands folder
      // in the private registry, read with the token. Signed out there is
      // nothing to show, and the panel says why.
      async loadErrands() {
        const E = window.Errands;
        const repo = window.__shell?.REGISTRY_REPO || 'mehrlander/web-tools-private';
        this.errandsLoading = true;
        this.errandsErr = '';
        try {
          if (!E) throw new Error('the errands kit is not loaded');
          if (!Alpine.store('browser').gh?.token) throw new Error('Sign in to see errands: they live in the private registry.');
          const reg = this.srcGh(repo, 'main');
          const ls = async (dir) => {
            try { return (await reg.ls(dir)).filter(f => f.type === 'file').map(f => f.name); }
            catch { return []; }   // 404: that folder does not exist yet
          };
          const out = [];
          for (const name of E.pending(await ls(E.DIR.req), await ls(E.DIR.res))) {
            let raw;
            try { raw = JSON.parse((await reg.get(E.DIR.req + '/' + name)).text); } catch { continue; }
            const e = E.normalize(raw, { repo, res: E.DIR.res, name });
            if (e && E.validate(e).ok && !out.some(x => x.id === e.id)) out.push(e);
          }
          const focus = this.errandFocus;
          out.sort((x, y) => (y.id === focus) - (x.id === focus));
          this.errands = out.map(e => ({ ...e, outcome: null, grade: null, message: '', busy: false, armed: false,
                                         scriptText: '', scriptErr: '', copied: false,
                                         age: this.askAge(e.createdAt) }));
          for (const e of this.errands) if (e.run) this.loadErrandScript(e);
          if (focus) {
            this.errandsOpen = true;
            // A read opened by its own link is previewed on arrival: nothing is
            // sent back until OK, and reading your own repo is the preview.
            const f = this.errands.find(x => x.id === focus);
            if (f && this.errandIsRead(f)) await this.runErrand(f);
          }
        } catch (e) {
          this.errandsErr = String(e?.message || e);
        } finally { this.errandsLoading = false; }
        if (!this.pointer) {
          try { this.pointer = (await this.srcGh(COURIER_REPO, 'main').get(COURIER_POINTER)).text.trim(); } catch {}
        }
      },

      async toggleErrands() {
        this.errandsOpen = !this.errandsOpen;
        // The panel is addressable, so the address follows it. Without this a
        // link that opened it would keep `&tab=errands` after you closed it,
        // and a reload would reopen what you had just dismissed.
        const sh = window.__shell;
        if (sh) { sh.stageTab = this.errandsOpen ? 'errands' : ''; sh.syncUrl?.(); }
        if (this.errandsOpen && !this.errands.length && !this.errandsLoading) await this.loadErrands();
      },

      errandIsRead(e) { return (window.Errands?.READS || []).includes(e?.action); },
      // What the badge says: the read, the run method, or `note` for the plain case.
      errandKind(e) { return e?.action || e?.run?.method || 'note'; },
      errandIcon(e) {
        if (e?.action) return { tree: 'ph-tree-structure', branches: 'ph-git-branch', fetch: 'ph-file-arrow-down' }[e.action] || 'ph-moped';
        if (e?.run) return e.run.venue === 'browser' ? 'ph-globe-simple' : 'ph-terminal-window';
        return 'ph-hand-waving';
      },
      errandRules(e) { return window.Errands?.METHODS[e?.run?.method]?.rules || []; },
      errandScriptUrl(e) {
        const at = window.Errands?.parseScript(e?.run?.script);
        return at ? 'https://github.com/' + at.repo + '/blob/' + at.ref + '/' + at.path : '';
      },
      async loadErrandScript(e) {
        const at = window.Errands?.parseScript(e.run?.script);
        if (!at) return;
        try { e.scriptText = (await this.srcGh(at.repo, at.ref).get(at.path)).text; }
        catch (err) { e.scriptErr = 'Could not read the script: ' + (err?.message || err); }
      },
      // The copy puts the errand's id in, so a signed envelope names it; the
      // committed script stays generic.
      async copyErrandScript(e) {
        if (!e.scriptText) return;
        const text = window.Errands.withErrandId(e.scriptText, e.run.method, e.id);
        try { await navigator.clipboard.writeText(text); } catch { return Alpine.store('toast')?.('warning', 'Copy failed', 'alert-error', 4000); }
        e.copied = true;
        setTimeout(() => { e.copied = false; }, 1500);
      },
      // A pasted signed result: open the panel on its errand. The item itself
      // was staged by the fold, tagged with the errand's id.
      async errandResult(env) {
        this.errandFocus = env.errand;
        if (!this.errandsOpen) await this.toggleErrands();
        else if (!this.errands.length && !this.errandsLoading) await this.loadErrands();
        if (!this.errands.some(x => x.id === env.errand)) {
          Alpine.store('toast')?.('warning', 'No open errand ' + env.errand + ' for this result', 'alert-warning', 5000);
        }
      },

      // The staged text an errand is graded against: the items a signed result
      // tagged for it when there are any, else every untagged text item.
      errandFiles(e) {
        const text = this.localItems.filter(it => it.isText);
        const mine = text.filter(it => it.errand && it.errand === e?.id);
        return (mine.length ? mine : text.filter(it => !it.errand)).map(it => ({ name: it.name, text: it.text }));
      },
      errandGrade(e) {
        const E = window.Errands;
        if (!E || !e) return null;
        if (this.errandIsRead(e)) return e.outcome ? E.verdict(e, e.outcome) : null;
        return E.verdict(e, { files: this.errandFiles(e) });
      },
      errandOkLabel(e) {
        if (this.errandIsRead(e)) return 'OK, send it back';
        const k = this.localItems.length;
        return e.armed ? 'Tap again: send ' + k + ' to ' + e.dest : 'Send and close';
      },
      ...
      async runErrand(e) {
        ...
        e.busy = true;
        try {
          e.outcome = await E.fulfill(e, { GH: window.GH, token: Alpine.store('browser').gh?.token });
        } finally { e.busy = false; }
      },

      // The green button. A read closes with what it read; any other errand
      // sends what is staged to its own destination and closes on a
      // clean deposit. Two taps for the send, the Stage's own rule for a write.
      async completeErrand(e) {
        if (e.busy) return;
        if (this.errandIsRead(e)) return this.closeErrand(e, true);
        if (!e.armed) {
          e.armed = true;
          setTimeout(() => { e.armed = false; }, 3000);
          return;
        }
        e.armed = false;
        e.busy = true;
        ...
        const s = this.lastSend;
        if (s && s.ok > 0 && !s.failures) await this.closeErrand(e, true);
      },

      // Closing is the answer: a record at the errand's own name in its results
      // folder, the same rule for every source. A decline needs its reason,
      // which the kit enforces rather than this button.
      async closeErrand(e, answered) {
        const E = window.Errands;
        const toast = Alpine.store('toast');
        if (!E || e.busy) return;
        const grade = this.errandGrade(e);
        const rec = E.close(e, { answered, message: e.message, grade: grade || undefined,
                                 data: answered && this.errandIsRead(e) ? e.outcome : undefined });
        if (!rec.ok) return toast('warning', rec.error, 'alert-error', 5000);
        e.busy = true;
        try {
          await this.srcGh(e.src.repo, 'main').save(e.src.res + '/' + e.src.name, rec,
            (answered ? 'Complete' : 'Decline') + ' errand ' + e.id + ' via the stage');
          this.errands = this.errands.filter(x => x.id !== e.id);
          toast('success', answered ? 'Errand done' : 'Errand declined', '', 3000);
        } catch (err) {
          toast('warning', 'Could not close the errand: ' + (err?.message || err), 'alert-error', 5000);
        } finally { e.busy = false; }
      },

      // Coarse on purpose: "has this been sitting a while", not the hour. A
      // record with no date shows nothing rather than a guess.
      askAge(iso) {
        if (!iso) return '';
      ...
      // A task citation is 'owner/repo[@ref]:path'; the card links it so the
      // note can stay short.
      errandTaskUrl(e) {
        const addr = window.RepoAddress?.parse?.(e?.for || '');
        if (!addr) return '';
        return 'https://github.com/' + addr.repo + '/blob/' + (addr.ref || 'HEAD') + '/' + addr.path;
      },

      // The bookmarklet is installed by hand either way: dragged to the bar
      // from the link beside this, or pasted into a new bookmark's URL field.
      // Copying is the phone route, where there is nothing to drag to.
      async copyPointer() {
        const toast = Alpine.store('toast');
        try {
          await navigator.clipboard.writeText(this.pointer);
          this.pointerCopied = true;
          setTimeout(() => { this.pointerCopied = false; }, 1500);
          toast('link', 'Courier bookmarklet copied: paste it as a new bookmark\'s URL', 'alert-success', 3500);
        } catch (e) {
          toast('warning', 'Could not copy: ' + (e?.message || e), 'alert-error', 5000);
        }
      },
      ...
      aim(spec) {
        this.destSpec = spec;
        // A query, not $refs: the x-ref sits on the picker's own x-data root,
        // which registers it in the picker's scope, so this.$refs.destPicker is
        // undefined here and the label was never set (measured 2026-09-23).
        const p = this.$root.querySelector('[x-ref="destPicker"]')?.__pathPicker;
        if (p) p.label = spec;
      },
      ...
      focusFromStore() {
        // The bench stays mounted after its first visit. An arrival on Files
        // must wait for the shell to show Stage before opening its reader.
        if (typeof window.__shell?.view === 'string' && window.__shell.view !== 'stage') return;
        const s = Alpine.store('browser');
        const pair = s.stageCompare;
        if (pair) {
          s.stageCompare = null;
          const i = this.items.findIndex(x => this.itemKey(x) === pair.a);
          const other = this.items.find(x => this.itemKey(x) === pair.b);
          if (i >= 0 && other) {
            this.compareKey = pair.b;
            this.$nextTick(() => this.readerAt(i, 'diff'));
          }
          return;
        }
      ...
      // outcome of tapping Paste before copying anything, and the error colour
      // reads as "this button is broken" rather than "there was nothing to
      // take". It is reported as information instead, and
      ...
      async pasteIn() {
        if (window.__shell?.pasteAnywhere) return window.__shell.pasteAnywhere();
        const toast = Alpine.store('toast');
        try {
      ...
      // ── THE TRANSFORM DOOR ────────────────────────────────────────────────
      //
      // The workbench boots on every load, since the pre-build globs
      // lib/alpineComponents, so nothing here has to fetch it. What it lacked
      // was a way in from the app. This is that door.
      tfBusy: false,
      _tfDeck: null,
      ...
      // says nothing. That is why the scenario asserts the DRAWN rows rather
      // than the parsed ones.
      //
      // Same CDN URLs the shared viewer uses for its own table mode, so
      ...
      //
      // The pill is the SUBJECT and its menu is the verbs. A conversion belongs
      // on the flavor it converts rather than on a pill of its own, which would
      // put a derivation beside the formats it is made from; as a menu item it
      // gets base64, its inverse, and copying as siblings rather than as three
      // more pills.
      //
      ...
      mdBusy: false,
      _mdText: null,
      // KEYED BY NAME, VALIDATED AGAINST THE SOURCE TEXT, which makes it a
      // cache of the text rather than of the name: two pastes on one day carry
      // the same sniffed name (a dated `-paste.html`), so an entry whose `src`
      // is not this flavor's text is a miss and the conversion runs again.
      // Clearing on a new paste would not do instead, since takePaste writes
      // the store directly rather than through this component's setter.
      async mdFor(fl) {
      ...
      //
      // THE HOUSE HOVER CARD, not one of this component's own and not a daisyUI
      // tooltip, which is a dark box in a light app. kits/source-peek.js owns
      // the card: its theme, its head and body, its placement and flip, the
      // hover dwell, the keyboard path, and the absence of a touch path.
      //
      ...
      //
      // The extension is what decides the rendition, so a `-markdown.md` peek
      // renders as the list it will become and a `.html` flavor shows as
      // source. Two pills carry no peek: an image, since this card reads text,
      ...
      // ONE TAP EACH WAY: an add-only bar makes choosing the html INSTEAD of
      // the text mean staging both and then hunting the wrong one down in the
      // list. Tapping a staged flavor removes it, and the chip stays on the bar
      // afterwards, since the paste still carried it and taking the row away
      ...
      dataUri(it) {
        ...
        // can be read here is the set the viewer can actually render. An image
        // map instead sends a dropped PDF or workbook down the text path, to be
        // shown as mojibake of its own bytes.
      ...
      // ── Standoff notes ────────────────────────────────────────────────────
      // A note is a staged FILE bound to another staged item, so everything
      // that already moves files moves this one: the deposit writes it, the
      // bundle carries it, the reader opens it, a `#gz=` link takes it along.
      // What is new is only the binding (`noteFor`, holding the subject's key)
      // and the authoring below. Nothing was added to the transport, because a
      // comment that is content needs no transport of its own.
      noteOf(it) {
        const key = this.itemKey(it);
        return this.items.find(x => x.local && x.noteFor === key) || null;
      },
      // The open editor's subject, or null. A link that outlives the set it was
      // minted from can leave `noteId` naming an item nobody has: the panel
      // draws nothing rather than a header about a file that is not there.
      get noteSubject() {
        return this.noteId == null ? null
          : (this.items.find(x => this.itemKey(x) === this.noteId) || null);
      },
      // THE EDITOR IS A PANEL, NOT AN INLINE FIELD, unlike the rename beside
      // it. A filename is one line and belongs where the name is; a note is
      // prose and wants the width, which on a phone is the whole difference
      // between writing a sentence and writing three words.
      startNote(it) {
        if (!it || it.noteFor) return; // a note takes no note of its own
        this.noteId = this.itemKey(it);
        this.noteDraft = window.StageIntake.noteBody(this.noteOf(it)?.text || '');
        this.$nextTick(() => this.$refs.noteInput?.focus());
      },
      cancelNote() { this.noteId = null; this.noteDraft = ''; },
      // Commit is idempotent and safe to call twice, like commitRename. An
      // EMPTIED note removes the file rather than depositing an empty one,
      // since a note with nothing in it is one more thing to land, read and
      // drain for no reason.
      commitNote() {
        const key = this.noteId;
        if (key == null) return;
        this.noteId = null;
        ...
        const it = this.items.find(x => this.itemKey(x) === key);
        if (!it) return;
        const have = this.noteOf(it);
        if (!body) { if (have) this.rm(have); return; }
        const text = window.StageIntake.noteDoc(it, body);
        if (have) { have.text = text; have.size = text.length; return; }
        const s = Alpine.store('browser');
        s.stage = [...this.items,
                   { ...window.StageIntake.textItem(window.StageIntake.noteNameFor(it), text),
                     noteFor: key }];
      },
      // What the row's bubble says it will do, and what the panel is headed
      // with. Named for the subject, since the note's own filename is derived
      // and saying it twice tells the reader nothing.
      noteLabel(it) {
        return (this.noteOf(it) ? 'Edit the note on ' : 'Add a note on ') + (it.path || it.name);
      },
      noteName(it) { return window.StageIntake.noteNameFor(it); },
      ...
      commitRename() {
        ...
        it.sniffed = false;
        // A NOTE IS NAMED FOR ITS SUBJECT, so the rename has to carry it. The
        // pair is recognized at the destination by name and by nothing else,
        // and a note left under the old name points at a file that will not be
        // there. The `about:` line is rewritten for the same reason. The
        // binding itself survives untouched: a local item's key is its id, not
        // its name.
        const note = this.noteOf(it);
        if (note) {
          note.name = note.path = window.StageIntake.noteNameFor(it);
          note.text = window.StageIntake.noteDoc(it, window.StageIntake.noteBody(note.text));
          note.size = note.text.length;
        }
      ...
      // under the last line belongs to the box, and a tap there reaches no
      // handler at all while every tap on words works.
      dictBind(host) {
        this._dictHost = host;
      ...
      // A CARET IN THE GAP, AND ONLY THAT. kits/dictate.js `stitchAim` also
      // answers 'back' (no caret aim, so the newest break in the buffer), which
      // is true of almost any buffer holding two sentences: a cell borrowed for
      // it would never hand the full stop back. With a caret in the gap the
      // cell is free, since a full stop cannot be wanted where one already
      // sits. So the reach, and the step-back key that walks to an older break,
      // are pages/dictate.html's, whose keys lie across the page where a
      // seventh cell fits and this three-cell column has no room for a fourth.
      get dictStitch() { this.dictBeat; return (this._dict && this._dict.stitchAim) === 'caret'; },
      get dictMarks() {
      ...
      dictEditOpen() {
        ...
        this._dict.flush();
        // Remember whether the engine was actually running. Starting it
        // unconditionally on close, on the reading that "dictation is the
        // default mode", hands a reader who had stopped listening, tapped the
        // pencil and tapped Done a live microphone they never asked for.
      ...
      // destination writes; save/saveBytes send the instance ref as the PUT's
      // branch, so a dest of owner/repo@branch:dir lands local items ON that
      // branch rather than silently on the default.
      srcGh(repo, ref) {
      ...
      // A NOTE GOES WITH ITS SUBJECT. An orphan note deposited on its own is
      // the failure this convention exists to prevent: a file in the tray whose
      // whole content is a pointer at something not there. Removing the note
      // itself is the other direction and takes nothing with it, since nothing
      // is bound to a note.
      rm(it) {
        const s = Alpine.store('browser');
        const key = this.itemKey(it);
        if (this.noteId === key) this.cancelNote();
        s.stage = s.stage.filter(x => this.itemKey(x) !== key && x.noteFor !== key);
      },
      clearAll() {
      ...
      // cannot be shown.
      //
      // The deck kit owns the shell, and what that buys is the header's TITLE
      // and SUBTITLE (the file's name and its repo@ref, without which a link's
      // recipient walks five files knowing only "3 / 5"), touch swipe, arrow
      // keys, Escape, the phone back button, background scroll lock, lazy build
      // with a two-slide keep window, and correct nesting when the stage is
      // itself opened from a deck.
      //
      // The comparison is not a mode over this deck but a deck one level down
      // (openCompare), indexing the same set: the pair is read off the position
      // (readerPair), so every slide has one and stepping re-pairs.
      view(it) {
      ...
      //
      // A IS ALWAYS THE FILE YOU ARE ON, and not `min(i, n-2)`: sliding the
      // pair backwards to keep it valid at the end of the list makes side A the
      // file BEFORE the one on screen, running the diff in a direction the
      // reader did not ask for. Fixing A to the position costs the last slide
      // that old direction, which is the trade.
      //
      ...
      //
      // NOT A MODE over the reader's own deck, and the header's ✕ is why: it
      // dismisses the overlay, so from inside a mode it reads as "leave the
      // comparison" and does "leave the file as well", which puts the wrong
      // meaning on the one control every reader reaches for.
      //
      ...
      // deck paging a list that no longer exists. The signature is the item
      // keys, not the count, since a swap keeps the count and changes
      ...
      // that does not identify anything, reading "docs/f..." for a file whose
      // name was the whole point.
      _rShortName(i) {
      ...
      _rSyncChrome(n) {
        ...
        // SIZE FIRST. The subtitle truncates at phone width, so a size appended
        // after a repo path falls past the ellipsis, cutting off the one fact
        // the viewer contributed. The repo survives in the contents sheet and
        // the sidebar; the size is stated nowhere else, since the viewer draws
        // no header.
      ...
      // track, off the action cluster it was raised from. The width stays this
      ...
      _rActions(n) {
        const out = [];
        // NAME THE PAIR, NOT THE WIRING. What a reader wants before tapping is
        // which two files they are about to see, not how the control is built,
        // and readerPairLabel already assembles exactly that for the header.
        //
      ...
      async _rDrive(i, el, slide) {
        ...
        if (it.local) {
          ...
          // renders. The alternative refuses by NAME rather than by capability,
          // which stages, deposits and never once shows a pasted screenshot.
      ...
      // The diff slide, carrying everything a comparison needs: the tagged
      // rows, the copy, the hand-off to the Diff page, and the review prompts
      // (link-carried bespoke asks first, then the fixed set).
      ...
      _cmpSplit(d) {
        ...
        // ten, so a `/25` background falls back to transparent and paints
        // nothing at all, which reads as "subtle" rather than as broken.
      ...
      // commits list plus a batch of commit details), run in parallel; a repo
      // that fails just contributes nothing.
      ...
      // `estate: true`, via the config cache), not the shell's quickLinks,
      // which is a second list and does not name every member.
      pickerRoots() {
        const s = Alpine.store('browser');
        const seen = new Set();
        const roots = [];
      ...
      // are excluded, and the note says which rather than a blanket "local
      // files excluded" covering both.
      async copyLink() {
        const toast = Alpine.store('toast');
        ...
        try {
          url = await window.StageLink.mintWithLocals(carried, location.origin + location.pathname,
            // A diff intent rides the link when this stage HAS been diffed,
            // which with no Diff tab to be showing is the honest signal.
      ...
      // Line diff via kits/text-diff.js, which is patience diff, so no pair is
      // refused; a block past its cap degrades to a replacement run and sets
      // `lastWarning`, which the caller surfaces. Returns [{t:'ctx'|'add'|'del', line}]
      // with full context, since the row set here IS the whole compare.
      ...
      // One side's text: a local text item reads its held text, a ref reads
      // from its origin. Reads the bundle's content cache first: it is keyed
      ...
      async diffSide(it) {
        ...
        if (it.correspondenceText != null) return it.correspondenceText;
        if (it.local) {
      ...
      // A change to the A/B selection makes the shown diff stale: its rows and
      // stored text are from the last runDiff, but the
      ...
      // a different pair. Anything in here writing diffA/diffB/diffRows lets
      // the last builder win, which publishes a pair nobody is looking at,
      // leaves a neighbour with no rows at all (its runDiff returns on
      // `diffBusy`), and draws a third slide's heading over the first pair's
      // rows. A STAGE OF TWO HIDES ALL OF IT, since readerPair clamps both
      // slides onto 0,1: coverage for this needs three.
      async _rDiff(a, b) {
        if (!window.textDiff) throw new Error('text-diff kit not loaded');
        const itemA = this.items[a], itemB = this.items[b];
        const [textA, textB] = await Promise.all([
          this.diffSide(itemA),
          this.diffSide(itemB),
        ]);
        const paired = (itemA.local && !itemB.local && itemA.correspondenceFor === this.itemKey(itemB))
          || (itemB.local && !itemA.local && itemB.correspondenceFor === this.itemKey(itemA));
        // Correspondence compares line content for display. Keep the originals
        // for review prompts and all actions that transfer a staged item.
        const displayA = paired ? String(textA).replace(/\r\n?/g, '\n') : String(textA);
        const displayB = paired ? String(textB).replace(/\r\n?/g, '\n') : String(textB);
      ...
      // A compare side's display label: repo@ref:path for a ref item, or
      // "(local) name" for a dropped or pasted one. Used by the diff dump and
      // the review prompts, so both name what was actually compared.
      diffLabel(it) {
        if (!it) return '?';
      ...
      depositMessage(files, dest) {
        ...
        // `via Web Tools` marks a write a PERSON made by tapping in the app, as
        // against a Claude session, a hook, or the app's own crawl. The crawl's
        // cache refreshes carry no such trailer: their subject already names a
        // derived file, and claiming a person acted on 563 of them buried the
        // twenty that a person did.
        const subject = 'Add ' + n + ' file' + (n === 1 ? '' : 's') + ' to ' + where + ' via Web Tools';
        const CAP = 20;
      ...
      async send() {
        ...
        if (this.gitChangeItem && this.gitChangeEnv) {
          if (!dest.ref) dest = { ...dest, ref: this.gitChangeEnv.proposed_branch };
          ...
          this.sendArmed = false;
          return this.executeGitChangeApply(dest);
        }
        ...
        try {
          ...
          this.lastSend = { ok, total, failures: failures.length, sha };
