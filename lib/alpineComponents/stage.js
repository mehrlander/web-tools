...
//
//   #stage=owner/repo[@ref]:path1,path2;owner2/repo2:path3[&prompts=<b64url>]
//
...
// '/' left readable.
//
// A link is one object with two halves: REFS (the stage spec above) and
// COMMENTARY (an optional &prompts= param, a base64url'd JSON list of
// {label, ask} review asks). Refs are pointers, so their content stays behind
// the token; the prompts are authored text, yours to expose, so they ride the
// link itself. mint() serializes both from that object; parseLink() returns
// { items, prompts }, and the bare parse() keeps returning just the items for
// callers (the shell seed) that only want refs. This is the seed of a richer
// "surface" schema: the same {refs, commentary} shape a manifest's stage block
// or a future surface file would carry, with content the file-only extra.
...
window.StageLink = (() => {
  ...
  const parseLink = (hash) => {
    ...
    return { items, prompts: decodePrompts(p.prompts) };
  };
  const parse = (hash) => parseLink(hash).items;

  const mint = (items, base, prompts) => {
    ...
    const enc = encodePrompts(prompts);
    return (base || '') + '#stage=' + spec + (enc ? '&prompts=' + enc : '');
  };

  return { parse, parseLink, mint, parseItem, fmtItem, encodePrompts, decodePrompts };
})();
  ...
  Alpine.data('stager', function() {
    ...
    return {
      ...
      template: `
        <div class="flex flex-col gap-4">
          ...
          <div class="flex flex-col gap-1">
            ...
            <!-- Repo pills: one per repo present in Recent; single-select —
                 tap to show only that repo, tap again for all. -->
          ...
          <!-- Out / Diff: two lenses on what's staged, in the finder's open
               style (no outer card). Out is the deposit surface — the
               concatenated bundle (view toggles, copy, download) and the
               send-to-repo — since copying out and sending are the same act
               with different destinations. Diff is a line compare of two
               staged items, or one item against another ref. -->
          <div x-show="items.length" class="flex flex-col gap-1.5">
            <div class="flex items-center justify-between gap-2">
              <div class="flex items-center gap-0.5">
                <button @click="outTab = 'out'"
                        class="btn btn-xs gap-1" :class="outTab === 'out' ? 'btn-active btn-ghost' : 'btn-ghost opacity-60'">
              ...
              <!-- Icon-only bundle actions: view toggle, refresh, copy, download. -->
              <div class="flex items-center gap-0.5" x-show="outTab === 'out'">
                <button @click="bundleShow = !bundleShow" class="btn btn-xs btn-square btn-ghost"
                ...
                <button @click="copyBundle()" class="btn btn-xs btn-square btn-ghost" title="Copy the concatenated block">
                  <i class="ph" :class="bundleCopied ? 'ph-check' : 'ph-copy'"></i>
                </button>
                <button @click="download()" class="btn btn-xs btn-square btn-ghost" title="Download the concatenated block">
            ...
            <template x-if="outTab === 'out'">
              <div class="flex flex-col gap-2">
                <!-- The block itself is on demand (the toggle above): the copy
                     and download actions never needed it on screen. -->
                ...
                <!-- Send: the destination is the tap-through picker in dir
                     mode; its committed spec drives the two-tap send. -->
                ...
                <!-- Save stage: persisting the ref list is explicit about
                     WHERE it lands — a stage is nobody's property, so saving
                     one means naming the manifest that holds it. Defaults to
                     the registry, the home for a general staging. -->
              ...
              <div class="flex flex-col gap-2">
                ...
                <div class="flex flex-col sm:flex-row gap-1.5">
                  <div class="flex items-center gap-1 flex-1 min-w-0">
                    ...
                    <select x-model.number="diffA" @change="_diffTouched = true; invalidateDiff()" class="select select-xs select-bordered font-mono flex-1 min-w-0">
                      <template x-for="(it, i) in items" :key="'a' + itemKey(it)">
                        <option :value="i" x-text="bundleHeader(it).replace('// === ', '')"></option>
                      </template>
                    </select>
                    <input x-show="!items[diffA]?.local" x-model="diffARef" @change="invalidateDiff()" placeholder="as staged"
                  ...
                  <div class="flex items-center gap-1 flex-1 min-w-0">
                    ...
                    <select x-model.number="diffB" @change="_diffTouched = true; invalidateDiff()" class="select select-xs select-bordered font-mono flex-1 min-w-0">
                      <template x-for="(it, i) in items" :key="'b' + itemKey(it)">
                        <option :value="i" x-text="bundleHeader(it).replace('// === ', '')"></option>
                      </template>
                    </select>
                    <input x-show="!items[diffB]?.local" x-model="diffBRef" @change="invalidateDiff()" placeholder="as staged"
                  ...
                  <button x-show="diffRows" @click="copyDiff()" class="btn btn-xs btn-ghost btn-square shrink-0" title="Copy the diff">
                    <i class="ph" :class="diffCopied ? 'ph-check' : 'ph-copy'"></i>
                  </button>
                </div>
                <div x-show="diffRows"
                ...
                <!-- Review prompts: a condensed, scannable list, not cards —
                     each row is the whole affordance (click to copy), the ask
                     shown as a tooltip so the list stays one line per prompt. -->
                <div x-show="diffRows" class="border border-base-300 rounded-lg overflow-hidden">
                  <template x-for="(p, idx) in diffPrompts" :key="(p.bespoke ? 'b:' : 'f:') + p.label">
                    <button @click="copyPrompt(p.ask, idx)" :title="p.ask"
                            class="w-full flex items-center justify-between gap-2 px-2 py-1.5 text-left text-xs hover:bg-base-200 border-t border-base-300 first:border-t-0">
                      <span class="flex items-center gap-1.5 min-w-0">
                        <i x-show="p.bespoke" class="ph ph-sparkle text-primary/70 shrink-0" title="Tailored to this edit"></i>
      ...
      init() {
        ...
        autoPair();
        // Bespoke review prompts ride the opening #stage= link's &prompts=
        // param (the shell seeds the refs from the same hash and leaves it in
        // place). Read them once; they show first in the Diff lens panel.
        try { this.linkPrompts = window.StageLink.parseLink(location.hash).prompts; } catch {}
      },
      ...
      copyLink() {
        ...
        const url = window.StageLink.mint(refs, location.origin + location.pathname, this.linkPrompts);
