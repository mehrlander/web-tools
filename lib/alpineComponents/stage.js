...
// serializes the refs and the reading, mintWithLocals() folds the content in
// first; parseLink() returns { items, prompts, mode, cmp, view, dest, gz }, and
...
window.StageLink = (() => {
  ...
  // a wide margin and still carries a large document. Over budget, minting
...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
      ...
      // Line diff via kits/text-diff.js, which is patience diff, so there is no
      // size ceiling to report against. Returns [{t:'ctx'|'add'|'del', line}]
