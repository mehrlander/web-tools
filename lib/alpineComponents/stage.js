  ...
  Alpine.data('stager', function() {
    ...
    return {
        ...
        <div class="relative min-h-[55vh]"
          ...
          <div class="flex items-center justify-between flex-wrap gap-2 mb-4">
            ...
            <div class="flex items-center gap-1">
              <button @click="$refs.fileInput.click()" class="btn btn-xs btn-ghost gap-1" title="Add a local file (or drop / paste anywhere on this view)">
                <i class="ph ph-paperclip"></i><span class="hidden sm:inline">Add file</span>
              </button>
              <button x-show="items.length" @click="copyLink()" class="btn btn-xs btn-ghost gap-1"
              ...
              <button x-show="items.length" @click="clearAll()" class="btn btn-xs btn-ghost gap-1 hover:text-error">
          ...
          <div class="grid gap-4 items-start lg:grid-cols-[minmax(0,1fr)_20rem]">
            ...
            <div class="flex flex-col gap-2 min-w-0">
              ...
              <div x-show="!items.length"
                ...
                <p class="text-sm text-base-content/60">Drop files or paste refs anywhere, or grab from a repo.</p>
                <p class="text-xs text-base-content/40 mt-1">Two files with the same name open ready to diff.</p>
              </div>
                ...
                <template x-if="outTab === 'out'">
                  <div class="flex flex-col gap-2">
                    ...
                    <div x-show="bundleShow && !bundleBusy && bundleText"
                         class="overflow-auto font-mono text-[11px] leading-snug border border-base-300 rounded p-2 bg-base-200/40 max-h-[45vh] lg:max-h-[60vh] whitespace-pre"
                ...
                <template x-if="outTab === 'diff'">
                  <div class="flex flex-col gap-2">
                    ...
                    <div class="flex items-center gap-1.5">
                      ...
                      <select x-model.number="diffA" @change="_diffTouched = true; invalidateDiff()" class="select select-sm select-bordered font-mono flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-1.5">
                      ...
                      <select x-model.number="diffB" @change="_diffTouched = true; invalidateDiff()" class="select select-sm select-bordered font-mono flex-1 min-w-0">
                    ...
                    <div x-show="diffRows"
                         class="overflow-auto font-mono text-[11px] leading-snug border border-base-300 rounded bg-base-200/40 max-h-[45vh] lg:max-h-[60vh] whitespace-pre">
                    ...
                    <div x-show="diffRows" class="border border-base-300 rounded-lg overflow-hidden">
                      <template x-for="(p, idx) in diffPrompts" :key="(p.bespoke ? 'b:' : 'f:') + p.label">
                        <button @click="copyPrompt(p.ask, idx)" :title="p.ask"
                                class="w-full flex items-center justify-between gap-2 px-2.5 py-2 text-left text-xs hover:bg-base-200 border-t border-base-300 first:border-t-0">
            ...
            <div class="flex flex-col gap-3 min-w-0">
              ...
              <div x-show="items.length" class="flex flex-col gap-1.5">
                <div class="text-[11px] font-semibold uppercase tracking-wide text-base-content/40">Staged</div>
                <template x-for="g in groups" :key="g.key">
                  <div class="border border-base-300 rounded-lg bg-base-100 overflow-hidden">
                    <div class="px-2.5 py-1 bg-base-200/60 font-mono text-[11px] flex items-center gap-1.5">
                    ...
                    <div class="p-1">
                      <template x-for="it in g.items" :key="itemKey(it)">
                        <div class="group flex items-center justify-between gap-2 px-2 py-1 rounded hover:bg-base-200 text-sm">
                          <button @click="view(it)" class="flex items-center gap-1.5 min-w-0 cursor-pointer hover:text-primary text-left">
                            ...
                            <span class="truncate font-mono text-xs" x-text="it.path"></span>
                          </button>
                          <button @click="rm(it)"
                                  class="btn btn-ghost btn-xs w-5 h-5 min-h-0 p-0 opacity-30 hover:opacity-100 hover:text-error shrink-0">
                            <i class="ph ph-x"></i>
                          </button>
                        </div>
                      </template>
                    ...
                    <template x-for="it in localItems" :key="itemKey(it)">
                      <div class="group flex items-center justify-between gap-2 px-2 py-1 rounded hover:bg-base-200 text-sm">
                        <button @click="view(it)" class="flex items-center gap-1.5 min-w-0 cursor-pointer hover:text-primary text-left">
                          ...
                          <span class="truncate font-mono text-xs" x-text="it.name"></span>
                          <span class="opacity-50 shrink-0 text-[10px]" x-text="fmtSize(it.size)"></span>
                        </button>
                        <button @click="rm(it)"
                                class="btn btn-ghost btn-xs w-5 h-5 min-h-0 p-0 opacity-30 hover:opacity-100 hover:text-error shrink-0">
                          <i class="ph ph-x"></i>
              ...
              <div class="flex flex-col gap-1.5">
                <div x-show="items.length" class="text-[11px] font-semibold uppercase tracking-wide text-base-content/40">Add</div>
          ...
          <div x-show="preview" x-cloak
            ...
            <div class="bg-base-100 border border-base-300 rounded-xl shadow-xl w-full max-w-3xl max-h-[80vh] flex flex-col overflow-hidden">
              <div class="px-3 py-2 bg-base-200/60 flex items-center gap-2 text-xs font-mono border-b border-base-300">
              ...
              <div class="overflow-auto font-mono text-[11px] leading-snug p-3 whitespace-pre"
      ...
      init() {
        ...
        try {
          const lk = window.StageLink.parseLink(location.hash);
