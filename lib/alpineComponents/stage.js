...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    // The courier's public half: its errand list and the pointer that runs it.
    // Named here rather than taken from the shell, whose repo constant is the
    // PRIVATE registry: these two files are public on purpose and read with no
    // token, exactly as the bookmarklet reads them on a foreign page, so the
    // panel shows a tokenless reader the same list the courier would.
    const COURIER_REPO = 'mehrlander/web-tools';
    const COURIER_LIST = 'courier/errands.json';
            ...
            <div class="flex flex-col gap-2 min-w-0">
              <div class="flex flex-col gap-2">
                ...
                <!-- ERRANDS. Everything a session needs your browser for, in one
                     list: a read of one of your repos, a script to run on a
                     page a session cannot reach, or material only you have.
                ...
                <div x-show="errands.length || errandsOpen" x-cloak class="flex flex-col gap-2 pt-2">
                  ...
                  <template x-for="e in errands" :key="e.id">
                    <div class="border rounded-lg bg-base-100 p-2.5 flex flex-col gap-2"
                      ...
                      <div class="flex items-center gap-2 min-w-0">
                        ...
                        <span class="badge badge-ghost badge-sm shrink-0" x-text="e.action"></span>
                      ...
                      <div class="flex items-center gap-1.5 flex-wrap">
                        ...
                        <a x-show="e.action === 'courier'" :href="e.url" target="_blank" rel="noopener"
                           class="btn btn-xs btn-ghost border border-base-300 gap-1 font-normal">
                  ...
                  <div x-show="pointer &amp;&amp; (errandsOpen || errands.some(x => x.action === 'courier'))" x-cloak
...
git checkout ${branch}`;
      ...
      // The mailbox's fourth kind; the template above says what that means and
      // why it renders where it does. Read on mount, from the registry repo the
      // shell names. Failure-tolerant throughout: no token, no mailbox
      // directory, an unparsable record, or a malformed ask each drop that one
      // row rather than the section. A half-written record must not be able to
      ...
      courierIntake() {
        ...
        window.addEventListener('message', (e) => {
          ...
          if (d.type === 'courier-page') this.courierPage(d, (msg) => opener.postMessage(msg, e.origin));
          else if (d.type === 'courier-ran') this.courierRan(d);
        });
        opener.postMessage({ type: 'courier-ready' }, '*');
      },

      async courierPage(d, reply) {
        let host = '';
        try { host = new URL(String(d.url)).hostname; } catch { return; }
      ...
      // The open courier errand for a host, with its script read from the repo
      // its record came from: the registry for a private errand, web-tools for
      // one on the public list.
      async courierErrandFor(host) {
        if (!this.errands.length) await this.loadErrands();
        const e = this.errands.find(x => x.action === 'courier' && x.host === host);
        if (!e) return null;
        const from = e.src.list || e.src.repo;
        try { return { ...e, code: (await this.srcGh(from, 'main').get(e.script)).text }; }
        catch (err) { this.courier = { ...this.courier, state: 'failed', note: 'Could not read ' + e.script + ': ' + err.message }; return null; }
      },

      // ── Errands ──────────────────────────────────────────────────────────
      // Every open errand, from every place one can be filed: the errands
      // folder in the registry, the mailbox folders it replaces, and the
      // courier's public list, whose errands close by a private result like
      // the rest. Read with the token, so the private ones are reachable;
      // signed out there is nothing to show, and the panel says why.
      async loadErrands() {
        ...
        try {
          ...
          const closed = await ls(E.DIR.res);
          const out = [];
          for (const box of [E.DIR, E.MAILBOX]) {
            const results = box === E.DIR ? closed : await ls(box.res);
            for (const name of E.pending(await ls(box.req), results)) {
              let raw;
              try { raw = JSON.parse((await reg.get(box.req + '/' + name)).text); } catch { continue; }
              const e = E.normalize(raw, { repo, res: box.res, name });
              if (e && E.validate(e).ok && !out.some(x => x.id === e.id)) out.push(e);
            }
          }
          try {
            const list = JSON.parse((await this.srcGh(COURIER_REPO, 'main').get(COURIER_LIST)).text);
            for (const raw of list.errands || []) {
              if (!raw || raw.status !== 'open' || closed.includes(raw.id + '.json')) continue;
              const e = E.normalize(raw, { repo, res: E.DIR.res, name: raw.id + '.json', list: COURIER_REPO });
              if (e && E.validate(e).ok && !out.some(x => x.id === e.id)) out.push(e);
            }
          } catch { /* the public list is optional */ }
      ...
      errandIcon(e) {
        return { tree: 'ph-tree-structure', branches: 'ph-git-branch', fetch: 'ph-file-arrow-down',
                 courier: 'ph-globe-simple', hand: 'ph-hand-waving' }[e?.action] || 'ph-moped';
      },

      // The staged text a courier or hand errand is graded against.
      errandFiles() {
        return this.localItems.filter(it => it.isText).map(it => ({ name: it.name, text: it.text }));
      },
      errandGrade(e) {
        const E = window.Errands;
        if (!E || !e) return null;
        if (this.errandIsRead(e)) return e.outcome ? E.verdict(e, e.outcome) : null;
        return E.verdict(e, { files: this.errandFiles() });
      },
      errandOkLabel(e) {
      ...
      async runErrand(e) {
        ...
        e.busy = true;
        try {
          e.outcome = await M.fulfill({ id: e.id, kind: e.action, repo: e.repo, ref: e.ref || undefined, paths: e.paths },
                                      { GH: window.GH, token: Alpine.store('browser').gh?.token });
        } finally { e.busy = false; }
      },

      // The green button. A read closes with what it read; a courier or hand
      // errand sends what is staged to its own destination and closes on a
