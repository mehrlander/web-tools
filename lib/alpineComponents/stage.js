        ...
        <div class="relative min-h-[55vh]"
          ...
          <div class="flex items-center justify-between flex-wrap gap-2 mb-4">
            <h2 class="text-lg font-bold flex items-center gap-2">
              ...
              <span x-show="linkMode === 'diff'" class="badge badge-primary badge-outline gap-1" title="This stage opened as a diff">
                <i class="ph ph-git-diff"></i>diff
              </span>
            </h2>
            <div class="flex items-center gap-1">
              <button @click="$refs.fileInput.click()" class="btn btn-ghost gap-1" title="Add a local file (or drop / paste anywhere on this view)">
                <i class="ph ph-paperclip"></i><span class="hidden sm:inline">Add file</span>
              </button>
              <button x-show="items.length" @click="copyLink()" class="btn btn-ghost gap-1"
              ...
              <button x-show="items.length" @click="clearAll()" class="btn btn-ghost gap-1 hover:text-error">
          ...
          <div class="grid gap-4 items-start lg:grid-cols-[minmax(0,1fr)_20rem]">
            ...
            <div class="flex flex-col gap-2 min-w-0">
              ...
              <div x-show="!items.length"
                ...
                <p class="text-lg text-base-content/60">Drop files or paste refs anywhere, or grab from a repo.</p>
              ...
              <div x-show="items.length" class="flex flex-col gap-2">
                ...
                <div class="flex items-center justify-between gap-2 border-b border-base-300 pb-1.5">
                  <div class="flex items-center gap-0.5">
                    <button @click="outTab = 'out'"
                            class="btn gap-1" :class="outTab === 'out' ? 'btn-active btn-ghost' : 'btn-ghost opacity-60'">
                      <i class="ph ph-export"></i>Out
                    </button>
                    <button @click="outTab = 'diff'"
                            class="btn gap-1" :class="outTab === 'diff' ? 'btn-active btn-ghost' : 'btn-ghost opacity-60'">
                      <i class="ph ph-git-diff"></i>Diff
                    </button>
                    <span class="opacity-60 font-mono text-base ml-1"
                    ...
                    <div class="flex items-center justify-between gap-2 flex-wrap">
                      <div class="flex items-center gap-1.5">
                        <input x-model="saveTarget" placeholder="owner/repo"
                               class="input input-bordered font-mono w-52" :disabled="savingStage">
                        <button @click="save()" :disabled="savingStage || !saveTarget.trim()"
                                class="btn btn-ghost gap-1 opacity-70 hover:opacity-100"
                ...
                <template x-if="outTab === 'diff'">
                  <div class="flex flex-col gap-2">
                    ...
                    <div class="flex items-center gap-1.5">
                      <span class="badge badge-ghost font-bold w-6 justify-center shrink-0">A</span>
                    ...
                    <div class="flex items-center gap-1.5">
                      <span class="badge badge-ghost font-bold w-6 justify-center shrink-0">B</span>
            ...
            <div class="flex flex-col gap-3 min-w-0">
              ...
              <div x-show="items.length" class="flex flex-col gap-1.5">
                <div class="text-base font-semibold uppercase tracking-wide text-base-content/40">Staged</div>
                  ...
                  <div class="border border-base-300 rounded-lg bg-base-100 overflow-hidden">
                    ...
                    <div class="p-1">
                      <template x-for="it in g.items" :key="itemKey(it)">
                        <div class="group flex items-center justify-between gap-2 px-2 py-1 rounded hover:bg-base-200 text-lg">
              ...
              <div class="flex flex-col gap-1.5">
                <div x-show="items.length" class="text-base font-semibold uppercase tracking-wide text-base-content/40">Add</div>
                ...
                <div class="flex flex-col gap-1">
                  <div class="flex items-center justify-between">
                    <div class="flex items-center gap-0.5">
                      <button @click="finderTab = 'recent'; recentOpen = true"
                              class="btn gap-1" :class="finderTab === 'recent' ? 'btn-active btn-ghost' : 'btn-ghost opacity-60'">
                        Recent<span x-show="recent.length" class="badge badge-ghost" x-text="recent.length"></span>
                      </button>
                      <button @click="finderTab = 'search'; recentOpen = true; ensureTrees()"
                              class="btn gap-1" :class="finderTab === 'search' ? 'btn-active btn-ghost' : 'btn-ghost opacity-60'">
                        Search
                      </button>
                    </div>
                    <div class="flex items-center gap-0.5">
                      <button x-show="finderTab === 'recent'" @click="loadRecent(true)" class="btn btn-ghost btn-square" title="Refresh">
                        <i class="ph" :class="recentLoading ? 'ph-circle-notch animate-spin' : 'ph-arrows-clockwise'"></i>
                      </button>
                      <button @click="recentOpen = !recentOpen" class="btn btn-ghost btn-square" title="Collapse">
                  ...
                  <div x-show="recentOpen && finderTab === 'recent' && repoPills().length > 1" class="flex flex-wrap gap-1">
                    <template x-for="pl in repoPills()" :key="pl.repo">
                      <button @click="togglePill(pl.repo)"
                              class="badge cursor-pointer gap-1 transition-opacity"
                  ...
                  <label x-show="recentOpen && finderTab === 'search'" class="input input-bordered flex items-center gap-2">
                    <i class="ph ph-magnifying-glass opacity-50"></i>
                    <input x-model="searchQ" type="text" placeholder="File name contains…"
                           autocomplete="off" autocapitalize="off" spellcheck="false"
                           class="grow font-mono text-xl sm:text-lg">
                  </label>
                      ...
                      <button @click="toggleRecent(it)"
                        ...
                        <div class="min-w-0 flex-1">
                          <div class="flex items-baseline justify-between gap-2">
                            <span class="truncate font-mono text-lg" x-text="baseName(it.path)"></span>
                            <span x-show="it.date" class="shrink-0 text-base opacity-50" x-text="ago(it.date)"></span>
                          </div>
                          <div class="truncate font-mono text-base opacity-50" :title="it.repo + ':' + it.path"
                               x-text="whereFrom(it)"></div>
                        </div>
                      </button>
                    </template>
                    <div x-show="finderTab === 'search' && searchQ.trim().length >= 2 && !treesLoading && !finderRows().length"
                         class="py-4 text-center text-lg text-base-content/50">No matching files.</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <!-- Inline preview as a centered overlay, so a wide code view never
               fights the two-pane grid. Backdrop or X dismisses. -->
          <div x-show="preview" x-cloak
               class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40"
               @click.self="preview = null">
            <div class="bg-base-100 border border-base-300 rounded-xl shadow-xl w-full max-w-3xl max-h-[80vh] flex flex-col overflow-hidden">
              <div class="px-3 py-2 bg-base-200/60 flex items-center gap-2 text-base font-mono border-b border-base-300">
                ...
                <button @click="preview = null" class="btn btn-ghost btn-square opacity-60 hover:opacity-100">
                  <i class="ph ph-x"></i>
                </button>
              </div>
              <div class="overflow-auto font-mono text-base leading-snug p-3 whitespace-pre"
