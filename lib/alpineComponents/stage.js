...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    const COURIER_POINTER = 'bookmarklets/courier.js';
            ...
            <div class="flex flex-col gap-2 min-w-0">
              <div class="flex flex-col gap-2">
                ...
                <!-- ASKED OF YOU. The mailbox's fourth kind, and the only one a
                     ...
                     No step of it is a new transport. -->
                <div x-show="asks.length" x-cloak class="flex flex-col gap-2 pt-2">
                  ...
                  <template x-for="a in asks" :key="a.name">
                    <div class="border border-base-300 rounded-lg bg-base-100 p-2.5 flex flex-col gap-2">
                      <!-- whitespace-pre-line, so a note written with line
                           breaks keeps them. The note is plain text, never
                           markdown: x-text is what stops a record authored
                           elsewhere from injecting markup here, and the cost is
                           that structure comes from newlines, not syntax.
                           No backticks in this file's markup: it is a template
                           literal, and one ends the string mid-element. -->
                      <div class="text-base whitespace-pre-line" x-text="a.note"></div>
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <div class="flex items-center gap-1 text-base font-semibold uppercase tracking-wide text-base-content/40">
                  ...
                  <button @click="toggleErrands()" class="btn btn-square btn-ghost btn-sm"
                          ...
                          title="Errands: what the courier can fetch from a page a session cannot reach">
                ...
                <!-- The errand panel. A list and a pointer, nothing more. The
                     errand's own note says why it exists and what it is for,
                     so nothing here explains the courier; that is
                     courier/README.md's job. -->
                <div x-show="errandsOpen" x-cloak class="flex flex-col gap-2 border border-base-300 rounded-lg bg-base-200/40 p-2.5">
                  <div x-show="errandsLoading" class="text-base opacity-60">Reading the errand list…</div>
                  ...
                  <template x-for="e in errands" :key="e.id">
                    <div class="border border-base-300 rounded-lg bg-base-100 p-2.5 flex flex-col gap-2">
                      <div class="text-base font-semibold" x-text="e.title"></div>
                      <div class="text-base" x-text="e.note"></div>
                      <!-- Each glyph wraps WITH its text: as two loose flex
                           children they broke apart at this width, leaving the
                           destination icon on the line above its path. -->
                      <div class="flex items-center gap-x-3 gap-y-1 flex-wrap font-mono text-sm opacity-60">
                        <span class="inline-flex items-center gap-1.5 min-w-0">
                          <i class="ph ph-globe-simple shrink-0"></i>
                          <span class="truncate" x-text="e.host"></span>
                        </span>
                        <span class="inline-flex items-center gap-1.5 min-w-0">
                          <i class="ph ph-tray-arrow-down shrink-0"></i>
                          <span class="truncate" x-text="e.result &amp;&amp; e.result.path"></span>
                        </span>
                      </div>
                      <div class="flex items-center gap-1.5 flex-wrap">
                        <!-- The act itself: you have to BE on the page, which
                             is the whole reason the errand exists. -->
                        <a :href="e.url" target="_blank" rel="noopener"
                           class="btn btn-xs btn-primary gap-1 font-normal">
                          <i class="ph ph-arrow-square-out"></i>Open the page</a>
                        <a :href="'https://github.com/mehrlander/web-tools/blob/main/' + e.script"
                           target="_blank" rel="noopener"
                           class="btn btn-xs btn-ghost border border-base-300 gap-1 font-normal">
                          <i class="ph ph-code"></i>Script</a>
                      </div>
                    </div>
                  </template>
                  <!-- Drag installs it, tapping copies it. Both are by hand:
                       a page cannot write a bookmark, on any browser. -->
                  <div x-show="pointer" x-cloak class="flex items-center gap-1.5 flex-wrap pt-0.5">
...
git checkout ${branch}`;
      ...
      async courierPage(d, reply) {
        ...
        if (!errand) return;
        this.courier = { ...this.courier, errand: { id: errand.id, title: errand.title, result: errand.result }, state: 'running' };
        if (errand.result?.repo) {
          const name = errand.result.path.split('/').pop();
          const dir = errand.result.path.slice(0, -name.length).replace(/\/$/, '');
          this.aim(errand.result.repo + '@' + (errand.result.branch || 'main') + (dir ? ':' + dir : ''));
        }
        reply({ type: 'courier-run', id: errand.id, code: errand.src, errand: { id: errand.id, url: errand.url } });
      },
      ...
      courierRan(d) {
        ...
        const name = String(e.result?.path || e.id + '.md').split('/').pop();
        this.courierStage([window.StageIntake.textItem(name, String(d.text ?? ''))]);
        this.courier = { ...this.courier, state: 'ran' };
      },

      // The open errand for a host, with its script read. The private list in
      // the registry is read first and the public one second, so an errand can
      // live in either; a script is read from the repo its list lives in.
      async courierErrandFor(host) {
        const lists = [[window.__shell?.REGISTRY_REPO || 'mehrlander/web-tools-private', 'courier/errands.json'],
                       [COURIER_REPO, COURIER_LIST]];
        for (const [repo, path] of lists) {
          let list;
          try { list = JSON.parse((await this.srcGh(repo, 'main').get(path)).text); } catch { continue; }
          const e = (list.errands || []).find(x => x.status === 'open' && x.host === host && x.script);
          if (!e) continue;
          try { return { ...e, src: (await this.srcGh(repo, 'main').get(e.script)).text }; }
      ...
      // ── Errands: what the courier can fetch that no session can ──────────
      // The fifth intake, and the only one with no control until now: material
      // that lives on a web page a session cannot reach, because the host
      // refuses a sandbox (Cloudflare) or CORS refuses the app. The courier is
      // a bookmarklet you run ON that page, and it sends its result here as a
      // #gz= payload. See courier/README.md.
      //
      // This panel is the list, not the run: nothing here can fetch the page,
      // because reaching it is precisely what the browser has and the app does
      // not. What it can do is say what is open and hand over the pointer.
      async toggleErrands() {
      ...
      // Read the courier's two public files the way the courier itself does:
      // api.github.com, no token, `cache: no-store`. Not the CDN, which holds
      // five minutes at the edge, so an errand opened a moment ago could be
      // invisible. The pointer is fetched rather than inlined so there is one
      // copy of it in the repo and this panel cannot hand over a stale one.
      async loadErrands() {
        this.errandsLoading = true;
        this.errandsErr = '';
        const raw = async (path) => {
          const r = await fetch('https://api.github.com/repos/' + COURIER_REPO + '/contents/' + path,
                                { headers: { Accept: 'application/vnd.github.raw' }, cache: 'no-store' });
          if (!r.ok) throw new Error('HTTP ' + r.status);
          return r.text();
        };
        try {
          const [list, pointer] = await Promise.all([raw(COURIER_LIST), raw(COURIER_POINTER)]);
          this.errands = (JSON.parse(list).errands || []).filter(e => e && e.status === 'open');
          this.pointer = pointer.trim();
        } catch (e) {
          const msg = String(e?.message || e);
          // A 403 here is the rate limit, not a permission problem, and saying
          // so saves the reader checking a token they do not need.
          this.errandsErr = msg + (msg.includes('403') ? ': the API allows 60 reads an hour without a token' : '');
