    ...
    return {
                ...
                <div x-show="gitChangeItem" class="border border-base-300 rounded-lg bg-base-100 p-3 flex flex-col gap-2.5">
                  <div class="flex items-center justify-between gap-2 flex-wrap border-b border-base-200 pb-2">
                    <div class="flex items-center gap-2">
                      <i class="ph ph-git-pull-request text-lg text-primary shrink-0"></i>
                      <span class="font-semibold text-base">Git Change Handoff</span>
                    </div>
                    <div class="flex items-center gap-1.5">
                      <span x-show="gitChangeVerified === 'verified'" class="badge badge-success badge-sm gap-1">
                        <i class="ph ph-shield-check"></i>SHA-256 verified
                      </span>
                      <span x-show="gitChangeVerified === 'mismatch'" class="badge badge-error badge-sm gap-1">
                        <i class="ph ph-warning-circle"></i>SHA-256 mismatch
                      </span>
                      <span x-show="gitChangeVerified === 'verifying'" class="badge badge-ghost badge-sm gap-1">
                  ...
                  <div class="text-xs opacity-75 bg-base-200/50 p-2 rounded border border-base-300/60 flex items-start gap-1.5">
                    <i class="ph ph-info shrink-0 text-sm mt-0.5"></i>
                    <span><strong>Integrity Notice:</strong> Checksum verifies bundle bytes against corruption, not authorship or safety. Inspect files before applying.</span>
                  </div>

                  <!-- Envelope details -->
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-mono">
                    <div class="flex flex-col gap-0.5">
                      <span class="opacity-50">Target Repository</span>
                      <span class="font-semibold truncate" x-text="gitChangeEnv?.repository || '—'"></span>
                    </div>
                    <div class="flex flex-col gap-0.5">
                      <span class="opacity-50">Target Branch</span>
                      <span class="font-semibold text-primary truncate" x-text="gitChangeEnv?.proposed_branch || '—'"></span>
                    </div>
                    <div class="flex flex-col gap-0.5">
                      <span class="opacity-50">Base Commit</span>
                      <div class="flex items-center gap-1">
                        <span class="truncate" x-text="(gitChangeEnv?.base || '').slice(0, 10)"></span>
                    ...
                    <div class="flex flex-col gap-0.5">
                      ...
                      <div class="flex items-center gap-1">
                        <span class="truncate" x-text="(gitChangeEnv?.expected_tree || '').slice(0, 10)"></span>
                  ...
                  <div class="flex flex-col gap-1 text-xs">
                    <span class="opacity-50 font-mono">Commit Subject</span>
                  ...
                  <!-- Apply Actions: Two-step confirmation -->
                  <div class="flex items-center gap-2 flex-wrap pt-1">
                    <template x-if="!gitChangeApplyArmed">
                      <button type="button" @click="armGitChangeApply()" :disabled="gitChangeApplying || gitChangeVerified === 'mismatch'"
                              class="btn btn-sm btn-primary gap-1 font-normal">
                        <i class="ph ph-git-commit"></i>
                        <span>Apply change to GitHub</span>
                      </button>
                    </template>
                    <template x-if="gitChangeApplyArmed">
                      <div class="flex items-center gap-1.5">
                        <button type="button" @click="executeGitChangeApply()" :disabled="gitChangeApplying"
                                class="btn btn-sm btn-error gap-1 font-normal">
                          <i class="ph" :class="gitChangeApplying ? 'ph-circle-notch animate-spin' : 'ph-check'"></i>
                          <span x-text="'Confirm apply to ' + (gitChangeEnv?.proposed_branch || 'branch')"></span>
                        </button>
                        <button type="button" @click="disarmGitChangeApply()" :disabled="gitChangeApplying"
                                class="btn btn-sm btn-ghost font-normal">Cancel</button>
                      </div>
                    </template>
      ...
      init() {
        ...
        const checkGitChange = () => {
          if (this.gitChangeItem) this.verifyGitChange();
        };
      ...
      copyGitChangeText(text, toastMsg) {
        ...
        if (navigator.clipboard?.writeText) {
          navigator.clipboard.writeText(text)
            ...
            .catch(() => toast?.('warning', 'Could not copy to clipboard', 'alert-error', 3000));
        }
      },
      ...
      downloadGitChangeEnvelope() {
        ...
        const name = it.name || 'change.git-change.json';
        ...
        Alpine.store('toast')?.('download-simple', 'Downloaded ' + name, 'alert-success', 2500);
      },

      downloadGitBundle() {
        const env = this.gitChangeEnv;
        if (!env) return;
        const b64 = (env.payload && env.payload.data) || env.bundle_base64;
        if (!b64) return;
        const bin = atob(b64);
        ...
        const slug = branch.replace(/^[^/]+\//, '').replace(/[^a-zA-Z0-9._-]/g, '-');
        const filename = (slug || 'change') + '.git-bundle';
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = filename;
        a.click();
        setTimeout(() => URL.revokeObjectURL(url), 1000);
        Alpine.store('toast')?.('download-simple', 'Downloaded ' + filename, 'alert-success', 2500);
      },

      copyGitChangeInstructions() {
        const env = this.gitChangeEnv;
        const branch = env?.proposed_branch || 'branch-name';
        const slug = branch.replace(/^[^/]+\//, '').replace(/[^a-zA-Z0-9._-]/g, '-');
        const bundleFile = (slug || 'change') + '.git-bundle';
        const text = [
          '# Fetch change from bundle into local branch:',
        ...
        if (navigator.clipboard?.writeText) {
          navigator.clipboard.writeText(text).then(() => {
            ...
            Alpine.store('toast')?.('terminal-window', 'Copied Git CLI instructions', 'alert-success', 2500);
          }).catch(() => {
            Alpine.store('toast')?.('warning', 'Could not copy to clipboard', 'alert-error', 3000);
          });
        }
      },
      ...
      async copyGitChangeStageLink() {
        ...
        try {
          const lk = await window.StageLink.mintWithLocals([it]);
          if (navigator.clipboard?.writeText) {
            await navigator.clipboard.writeText(lk);
            this.gitChangeLinkCopied = true;
            setTimeout(() => { this.gitChangeLinkCopied = false; }, 2500);
            Alpine.store('toast')?.('link', 'Copied Stage link to clipboard', 'alert-success', 2500);
          }
        } catch (e) {
      ...
      async executeGitChangeApply() {
        if (this.gitChangeApplying) return;
        const env = this.gitChangeEnv;
        if (!env) return;
        ...
        this.gitChangeApplyStatus = 'Verifying change package...';
        ...
        try {
          ...
          const branch = env.proposed_branch;
          ...
          this.gitChangeApplyStatus = 'Extracting changed files...';
          const filesToCommit = [];
          ...
          this.gitChangeApplyStatus = 'Committing to ' + repo + '@' + branch + '...';
          const commitResult = await targetGh.commitFiles(filesToCommit, {
            ...
            onProgress: (done, total, path, phase) => {
              this.gitChangeApplyStatus = phase === 'ok' ? 'Uploaded ' + done + '/' + total + ' files' : 'Uploading ' + path + '...';
            }
          });
          ...
          if (commitResult.alreadyApplied) {
            this.gitChangeApplyStatus = 'Branch ' + branch + ' already has tree ' + env.expected_tree.slice(0, 8);
            Alpine.store('toast')?.('info', 'Branch ' + branch + ' is already up-to-date', 'alert-info', 4000);
          } else {
            this.gitChangeApplyStatus = 'Committed ' + commitResult.sha.slice(0, 8) + ' to ' + branch;
          ...
          if (this.gitChangeOpenDraftPr && targetGh.createPull) {
            this.gitChangeApplyStatus = 'Opening draft PR...';
            const prOpts = {
            ...
            if (pr.alreadyExists) {
              this.gitChangeApplyStatus = 'PR #' + pr.number + ' already exists';
            } else {
              this.gitChangeApplyStatus = 'Created draft PR #' + pr.number;
