                ...
                <div x-show="errands.length || errandsOpen" x-cloak class="flex flex-col gap-2 pt-2">
                  ...
                  <template x-for="e in errands" :key="e.id">
                    <div class="border rounded-lg bg-base-100 p-2.5 flex flex-col gap-2"
                      ...
                      <div x-show="errandBody(e)" class="text-base whitespace-pre-line" x-text="errandBody(e)"></div>
                      <div class="flex items-center gap-x-3 gap-y-1 flex-wrap font-mono text-sm opacity-60">
                        ...
                        <span x-show="e.run" class="inline-flex items-center gap-1.5 min-w-0">
                          <i class="ph ph-desktop shrink-0"></i><span class="truncate" x-text="e.run?.venue"></span></span>
                        <a x-show="e.run" :href="errandScriptUrl(e)" target="_blank" rel="noopener"
                           class="inline-flex items-center gap-1.5 min-w-0 link link-hover">
                          <i class="ph ph-file-code shrink-0"></i><span class="truncate" x-text="errandScriptPath(e)"></span></a>
                      ...
                      <!-- A run errand's script, whole: what you are about to run. -->
                      <pre x-show="e.run &amp;&amp; e.scriptText" class="font-mono text-sm bg-base-200 rounded p-2 max-h-96 overflow-auto whitespace-pre"
...
git checkout ${branch}`;
      ...
      // The note past its first line when that line is the title, so the card
      // does not print the same sentence twice.
      errandBody(e) {
        const note = String(e?.note || '');
        const [first, ...rest] = note.split('\n');
        return (first.trim() === String(e?.title || '').trim() ? rest.join('\n') : note).trim();
      },
      errandScriptPath(e) { return window.Errands?.parseScript(e?.run?.script)?.path || ''; },
      errandScriptUrl(e) {
        const at = window.Errands?.parseScript(e?.run?.script);
