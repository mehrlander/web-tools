document.addEventListener('alpine:init', function() {
  Alpine.data('pages', function() {
    return {
      ...
      // The rendered URL is the GitHub Pages address for the file. Off the
      // default ref, append ?use=<ref> so pages that honor the convention
      // load their code from the same ref being browsed.
      renderUrl(path) {
        const [owner, name] = this.repo.split('/');
        const base = 'https://' + owner + '.github.io/' + name + '/' + path;
        const defaultRef = Alpine.store('browser').defaultRef;
        return defaultRef && this.ref !== defaultRef
          ? base + '?use=' + encodeURIComponent(this.ref)
          : base;
      },
