  ...
  Alpine.data('searchView', function () {
    ...
    return {
            ...
            <div x-show="open" x-cloak class="min-w-0 flex flex-col gap-2">
              <div class="flex items-center gap-1 flex-wrap text-base">
                ...
                <button x-show="window.FileCorrespondence?.applies(open)" @click="toggleCompare()"
                        class="btn btn-ghost btn-sm gap-1" title="Compare submitted PowerShell text with this repository file">
              ...
              <div x-show="compareOpen && window.FileCorrespondence?.applies(open)" x-cloak
                ...
                <div class="flex flex-wrap items-center gap-2">
                  ...
                  <label class="btn btn-outline btn-sm cursor-pointer">Choose file
                    <input type="file" accept=".ps1,.psm1,.psd1,.ps1xml,.txt" class="hidden"
      ...
      async toggleCompare() {
        ...
        try { this.compareHistory = await window.FileCorrespondence.history(this.open); }
        catch (e) { this.compareError = 'Could not read checks: ' + (e?.message || e); }
      },
      async submitCompare() {
      ...
      async comparePicked(e) {
        ...
        e.target.value = '';
        if (file) await window.__shell.takeCorrespondenceFile(file, 'file picker');
      },
      async compareDrop(e) {
        const files = Array.from(e.dataTransfer?.files || []);
        if (files.length === 1) await window.__shell.takeCorrespondenceFile(files[0]);
      ...
      async showFile(f) {
        ...
        this.compareOpen = false; this.compareDraft = ''; this.compareHistory = []; this.compareError = '';
