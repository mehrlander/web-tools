...
//   Sessions     what was SAID, answered from the monthly search index rather
//                than by opening every record, unioned with what each session
//                row says about itself; hits open the Sessions pane's paged
//                reader via web-tools:open-session.
//   Chats        the chat archive's catalog layers (what a chat was ABOUT, as
//                a hand or machine summary describes it), across every month
//                the frontier names; a hit opens the chat on its provider's
//                own site. It is the sessions lane's opposite number and the
//                pair is worth reading together: sessions quotes what was said
//                and costs the whole record store, chats matches what a
//                summary says and costs about 10 MB of shards. Neither reaches
//                the chat snapshots' full text; that is CLI-only, in
//                chat-histories tools/search_chats.py.
//
  ...
  Alpine.data('searchView', function () {
    ...
    return {
      ...
      template: `
        <div class="flex flex-col gap-3">
          <div class="flex items-center gap-2">
            ...
            <div class="grow"></div>
            <button @click="refreshCaches()" title="Forget fetched trees, session records and chat shards, so the next run reads fresh"
          ...
          <div x-show="!overCorpus" class="flex flex-wrap items-center gap-1">
          ...
          <div x-show="!overCorpus && scope" class="flex items-center gap-1 flex-wrap font-mono text-sm">
          ...
          <!-- What a mode cannot see, and ONLY that. This slot used to carry a
               ...
               only prose can state is a LIMIT, since nothing in
          ...
          <div class="grid gap-4 lg:items-start" :class="open && 'lg:grid-cols-[minmax(0,20rem)_minmax(0,1fr)]'">
            <div class="flex flex-col min-w-0 gap-2">
              <!-- THE COUNT AND ITS TWO ACTIONS BELONG TO THE LIST, so they ride
                   ...
                   it holds. -->
              <!-- WHILE A MULTI-SHARD READ RUNS, what it has read. It sits
                   in the count's own slot and under the same column, so the
                   line the reader watches for the answer is the line that says
                   how far off the answer is. Only the chats lane fills it. -->
              <div x-show="busy && progressLine" class="flex items-center gap-2 text-base text-base-content/50">
                <span class="loading loading-dots loading-xs opacity-50"></span>
                <span x-text="progressLine"></span>
              </div>
            ...
            <div x-show="open" x-cloak class="min-w-0 flex flex-col gap-2">
              <div class="flex items-center gap-1 flex-wrap text-base">
                ...
                <button x-show="window.FileCorrespondence?.applies(open)" @click="toggleCompare()"
                        class="btn btn-ghost btn-sm gap-1" title="Compare submitted text with this repository file">
                  <i class="ph ph-git-diff text-lg"></i><span>Compare copy</span></button>
              ...
              <div x-show="compareOpen && window.FileCorrespondence?.applies(open)" x-cloak
                ...
                <textarea x-model="compareDraft" class="textarea textarea-bordered w-full min-h-36 font-mono text-sm"
                          placeholder="Paste the copy to compare; paste here keeps the field's text"></textarea>
                <div class="flex flex-wrap items-center gap-2">
                  <button @click="submitCompare()" :disabled="!!(!compareDraft || compareBusy)"
                          class="btn btn-primary btn-sm">Compare text</button>
                  <label class="btn btn-outline btn-sm cursor-pointer">Choose file
                    <input type="file" accept=".ps1,.psm1,.psd1,.ps1xml,.xaml,.txt" class="hidden"
                           @change="comparePicked($event)"></label>
                ...
                <div x-show="comparePending.length" class="border-t border-base-200 pt-2 flex flex-col gap-2">
                  <span class="text-sm font-medium text-base-content/60">Unfinished submissions in this browser</span>
                  <template x-for="submission in comparePending" :key="submission.id">
                    <div class="rounded border border-warning/30 px-2 py-1 text-sm min-w-0">
                      <div class="font-mono text-xs break-all" x-text="submission.path"></div>
                      <div class="text-xs text-base-content/60"
                           x-text="new Date(submission.submittedAt).toLocaleString() + ' · ' + submission.incomingName"></div>
                      <div x-show="submission.lastError" class="text-xs text-warning break-words" x-text="submission.lastError"></div>
                      <div class="flex flex-wrap gap-1 mt-1">
                        <button @click="retryPending(submission)" class="btn btn-outline btn-xs">Retry address</button>
                        <button x-show="submission.path !== open.path" @click="retryPending(submission, open)"
                                class="btn btn-outline btn-xs">Compare with open file</button>
                      </div>
                    </div>
                  </template>
                </div>
                <div x-show="compareHistory.length" class="border-t border-base-200 pt-2 flex flex-col gap-1">
                  <div class="flex items-center gap-2 text-sm font-medium text-base-content/60">
                    <span>Checks in this browser</span><div class="grow"></div>
                    <button @click="downloadChecks()" class="btn btn-ghost btn-xs">Export JSON</button>
                  </div>
                  <template x-for="check in compareHistory" :key="check.id">
                    <button @click="reopenCheck(check)" class="text-left rounded px-2 py-1 hover:bg-base-200 flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 text-sm min-w-0">
                      <span class="text-xs tabular-nums whitespace-nowrap" x-text="new Date(check.checkedAt).toLocaleString()"></span>
                      <span class="text-xs" :class="check.exact ? 'text-success' : 'text-warning'"
                            x-text="check.exact ? 'submitted text matched revision' : check.lineEndingsOnly ? 'line endings only' : 'submitted text differed'"></span>
                      <span class="font-mono text-xs text-base-content/50 sm:ml-auto" x-text="(check.branch || 'default') + ' · ' + check.revision.slice(0, 10)"></span>
                    </button>
                  </template>
                </div>
              </div>
      ...
      MODES: [
        ...
        { key: 'chats',    label: 'Chats',     icon: 'ph-chats-circle' },
      ],
      // The two lanes that search a CORPUS rather than the repos, so neither
      // takes a repo, a ref, or a folder: each one's subject is a single store
      // and picking web-tools out of it would mean nothing. Named once here
      // because three controls hide on it and a fourth mode would otherwise
      // have to be added to each of them by hand.
      CORPUS_MODES: ['sessions', 'chats'],
      ...
      init() {
        ...
        const apply = (seed) => {
          if (!seed) return;
          this.q = seed.q || '';
          if (['names', 'contents', 'sessions', 'chats'].includes(seed.mode)) this.mode = seed.mode;
          this.repo = seed.repo || '';
          this.ref = seed.ref || '';
          this.path = seed.path || '';
      ...
      // A corpus lane searches one store and takes no repo, ref or folder, so
      // the three controls that name those stand down together.
      get overCorpus() { return this.CORPUS_MODES.includes(this.mode); },
      // How far a multi-shard read has got, or null. Only the chats lane sets
      // it: forty shard reads is tens of seconds, and a spinner that cannot say
      // whether it is at month 2 or month 38 is the thing the estate's crawl
      // bars exist to avoid.
      progress: null,
      get progressLine() {
        const p = this.progress;
        return p && p.total ? p.done + ' of ' + p.total + ' months read' : '';
      },
      ...
      get queryPlaceholder() {
        if (this.mode === 'chats') return 'Search chat summaries…';
      ...
      get caveat() {
        ...
        if (this.mode === 'sessions') {
          return 'Matches every stored prompt and reply through the search index, plus what each session row says: its names, its opening ask, its attachments, the repos and files it worked. Whole words and substrings, not phrases.';
        }
        // The one caveat this lane cannot leave out. The catalog is what a
        // summary SAYS ABOUT a chat, so a phrase spoken in the chat and absent
        // from its summary is unreachable here, and the reader has to know
        // that before reading a miss as an answer.
        if (this.mode === 'chats') {
          return 'Matches the archive\'s catalog: each chat\'s title, tags and its hand or machine summary, not the words spoken in it. Reads every month, about 10 MB, once per session.';
        }
      ...
      async run() {
        ...
        try {
          if (this.mode === 'names' && this.browsing) {
            ...
            // The kit is loaded here rather than at boot, the way the chats
            // lane loads its own: this is the only consumer outside the
            // sessions crawl, and a reader who never searches should not pay
            // for it.
            if (!window.SessionIndex) await window.gh.load('kits/session-index.js');
            const res = await ES.sessions({
              q, registry: S.REGISTRY_REPO, token: window.TOKEN,
              onProgress: (p) => { this.progress = p; },
            });
            this.progress = null;
            // A registry whose crawl has not written a shard yet can only
            // answer on rows, which is a narrower search than the one that was
            // asked for. Saying so beats reporting it as a complete result.
            if (!res.indexed) {
              this.error = 'No search index in the registry yet, so this matched the session rows only, '
                         + 'not what was said. Refresh the Sessions cache to build it.';
            }
            ...
            // The kit is loaded here rather than at boot: this is the only
            // consumer outside the Chats pane, and a reader who never opens
            // either should not pay for it.
            if (!window.chatArchive) await window.gh.load('kits/chat-archive.js');
            const res = await ES.chats({
              q, repo: S.CHATS_REPO, token: window.TOKEN,
              onProgress: (p) => { this.progress = p; },
            });
            this.progress = null;
            // A month the frontier names with no shard in EITHER layer is a
            // hole in the archive, not a failed search, so it is reported
            // beside the hits rather than thrown over them.
            if (res.missing.length) {
              this.error = 'No catalog shard for ' + res.missing.sort().reverse().join(', ')
                         + '; those months were not searched.';
            }
            this.hits = res.hits.map(h => ({
              key: 'h:' + h.url,
              icon: h.hand ? 'ph-hand-pointing' : 'ph-chats-circle',
              tint: h.hand ? 'text-primary' : '',
              label: h.title || h.url,
              // Provider beside the date, because the archive is three corpora
              // and which one answered changes what the hit even is: a Gemini
              // row is a 30-minute-gap thread rather than a conversation, and
              // it has no address to open.
              sub: [h.date, h.provider].filter(Boolean).join(' · '),
              note: h.frag,
              kind: 'chat', url: h.url, open: h.open,
            }));
            this.total = res.total;
          }
          this.ran = true;
        ...
        // The progress line is cleared HERE and not only on the happy path: a
        // read that throws at month 30 would otherwise leave "30 of 41" under
        // the error, reading as a partial answer rather than as no answer.
      ...
      openHit(h) {
        ...
        // A chat is READ where it lives. The archive holds a summary of it,
        // never the conversation in a form this app renders, so the honest
        // destination is the provider's own page. A Gemini row has no address
        // at all (Takeout keeps no per-conversation URL), so the row reports
        // that instead of opening a window on nothing.
        if (h.kind === 'chat') {
          if (h.open) window.open(h.open, '_blank', 'noopener');
          else this.error = 'That is a Gemini session, which has no per-chat address to open. '
                          + 'Its rendered thread is in the archive at '
                          + '2026-06-01-gemini-export/conversations/.';
        }
      },
      ...
      async toggleCompare() {
        ...
        if (!this.compareOpen || !this.open) return;
        try {
          [this.compareHistory, this.comparePending] = await Promise.all([
            window.FileCorrespondence.history(this.open),
            window.FileCorrespondence.pending(this.open.repo),
          ]);
        } catch (e) { this.compareError = 'Could not read checks or unfinished submissions: ' + (e?.message || e); }
      },
      async submitCompare() {
        if (!this.compareDraft || this.compareBusy) return;
        this.compareBusy = true; this.compareError = '';
        try { await window.__shell.takeCorrespondence(this.compareDraft, { source: 'field' }); }
        catch (e) { this.compareError = e?.message || String(e); }
        finally { this.compareBusy = false; }
      },
      async comparePicked(e) {
        const file = e.target.files?.[0];
        e.target.value = '';
        if (file) {
          await window.__shell.takeCorrespondenceFile(file, 'file picker', this.compareEncoding);
          this.compareError = window.__shell.correspondenceFileError || '';
        }
      },
      async compareDrop(e) {
        const files = Array.from(e.dataTransfer?.files || []);
        if (files.length === 1) {
          await window.__shell.takeCorrespondenceFile(files[0], 'drop', this.compareEncoding);
          this.compareError = window.__shell.correspondenceFileError || '';
        }
        else if (!files.length) await window.__shell.takeCorrespondence(e.dataTransfer?.getData('text') || '', { source: 'drop' });
      },
      async retryPending(submission, target = null) {
        await window.__shell.retryCorrespondence(submission, target);
        this.comparePending = await window.FileCorrespondence.pending(this.open.repo);
        this.compareHistory = await window.FileCorrespondence.history(this.open);
      },
      async reopenCheck(check) {
        try { await window.FileCorrespondence.reopen(check); window.__shell.goStage(); }
        catch (e) { this.compareError = 'Could not reopen check: ' + (e?.message || e); }
      },
      downloadChecks() {
        window.io?.saveJson?.(this.compareHistory, 'wps-checks-' + this.open.path.split('/').pop() + '.json');
      },
      async showFile(f) {
        if (!f?.repo || !f?.path) return;
        this.compareOpen = false; this.compareDraft = ''; this.compareHistory = []; this.comparePending = []; this.compareError = '';
        this.open = { repo: f.repo, ref: f.ref || '', path: f.path };
        if (window.__shell) window.__shell.searchOpenFile = this.open;
      ...
      closeFile() {
        ...
        if (window.__shell) window.__shell.searchOpenFile = false;
        this.stamp();
      },
      // ── The deck ─────────────────────────────────────────────────────────
      //
