  ...
  Alpine.data('repo', function(opts) {
    ...
    const dialogHtml = `
        <dialog id="repoModal" class="modal" onclick="if(event.target===this)this.close()">
          <div class="modal-box shadow-none border border-base-300 bg-base-100 p-0 w-[32rem] max-w-[calc(100vw-2rem)] h-[34rem] max-h-[calc(100vh-4rem)] flex flex-col overflow-hidden">
            ...
            <div class="flex-1 min-h-0 px-4 pb-2 flex flex-col">
              ...
              <div x-show="!dlg.loading && dlg.estate" class="flex-1 min-h-0 overflow-y-auto flex flex-col gap-3 pt-1">
                ...
                <div x-show="auth" class="flex flex-col gap-1">
                  <button @click="window.__shell?.refreshConfigs()" :disabled="window.__shell?.configRefreshing"
                          class="btn btn-ghost justify-start gap-2 border border-base-300 disabled:opacity-60">
              ...
              <div x-show="!dlg.loading && (tab==='settings' || tab==='config') && dlgLegacy"
                   class="alert alert-warning py-1.5 px-3 text-sm flex items-center gap-2 mb-2">
    ...
    return {
      ...
      template: (inline ? `
        <!-- Header in-repo readout: owner/name as a plain label (the repo-list
             ...
             and the GitHub jump. The ref switcher left the header: branch
             selection is a file-viewing concern, so it lives at the top of the
             Files view (explorer.js); rendering the app at a ref is the FAB's
             Render tab. Renders only inside a repo; the estate context shows
             nothing here. -->
        <div class="flex items-center gap-1.5 font-mono text-base" x-show="repo && !estate" x-cloak>
      ...
      : `
        <div class="mb-4 border-b border-base-200 pb-3">
          <div class="flex items-center justify-between">
            ...
            <button @click="openDialog()" class="tooltip tooltip-left flex items-center gap-1 btn btn-ghost" :data-tip="auth?'Authenticated: '+user:(tokenExpired?'Token expired':'Public only')">
          ...
          <div x-show="offMain" class="mt-2 alert alert-warning py-1.5 px-3 text-sm flex items-center justify-between">
            <div class="flex items-center gap-2">
