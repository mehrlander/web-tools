document.addEventListener('alpine:init', function() {
  Alpine.data('repo', function(opts) {
    ...
    // The repo's .web-tools.json config, edited on the dialog's Config tab. An
    // all-empty template is seeded for a repo that has none yet (the empty
    // values are inert, so saving the template as-is is harmless). Field-format
    // help lives in the docs, linked below, not inline.
    // Fields a repo may self-declare. estate/group/note/order/quickLink/nestUnder
    // are the estate placement (the repo owns how it appears on the estate);
    // icon/landing/pins/stage are the rest of its web-tools config. All inert
    // until set, so saving the template as-is is harmless.
    ...
    const configSection = `
            <div x-show="tab==='config'" class="flex flex-col gap-2">
              <div class="flex items-center gap-1.5 text-xs font-semibold text-base-content/70 px-1">
                <i class="ph ph-gear-six"></i><span class="font-mono" x-text="repo"></span>
                <span class="text-[10px] font-normal ml-0.5"
                      :class="cfgConnected ? 'text-success' : 'text-base-content/40'"
                      x-text="cfgConnected ? '.web-tools.json' : 'no config yet'"></span>
              </div>
              ...
              <div x-show="cfgOffMain" class="text-[11px] text-warning flex items-center gap-1.5 px-1">
              ...
              <textarea x-model="cfgDraft" spellcheck="false" rows="12"
              ...
              <a href="${CFG_DOCS_URL}" target="_blank" rel="noopener"
                 class="link link-primary text-xs flex items-center gap-1 w-fit px-1">
                <i class="ph ph-book-open"></i>Config format &amp; fields</a>
              <div class="flex items-center justify-end gap-2 mt-1">
                <button x-show="!auth" @click="tab='info'" class="text-[11px] text-primary underline mr-auto">Sign in to save</button>
    ...
    const dialogHtml = `
        <dialog id="repoModal" class="modal" onclick="if(event.target===this)this.close()">
          <div class="modal-box shadow-none border border-base-300 bg-base-100 p-4 max-w-lg overflow-x-hidden">
            <!-- Two tabs: repo info & links (default), and the .web-tools.json
                 config editor. Switching to Config seeds the editor. -->
            <div class="flex items-center justify-between gap-2 mb-3">
              <div class="px-1 font-mono text-sm font-bold truncate min-w-0" x-text="repo"></div>
              <div class="join shrink-0">
                ...
                <button @click="tab='config'; cfgOpen()" class="btn btn-xs btn-square join-item tooltip tooltip-bottom"
                        :class="tab==='config' ? 'btn-primary btn-active' : 'btn-ghost'" data-tip="Config">
                  <i class="ph ph-gear-six text-sm"></i></button>
              </div>
            </div>
            <div x-show="tab==='info'" class="flex flex-col">
