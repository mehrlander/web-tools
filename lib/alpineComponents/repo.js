document.addEventListener('alpine:init', function() {
  Alpine.data('repo', function(opts) {
    ...
    const repoDropdown = `
              <div class="dropdown-content z-10 p-2.5 shadow-lg bg-base-200 rounded-box w-72 max-h-[28rem] overflow-y-auto mt-1 border border-base-300">
                <div class="text-[10px] font-bold uppercase tracking-wider opacity-40 px-2 mb-1" x-text="owner"></div>
                <template x-for="r in repos" :key="r.name">
                  <a @click="pick(r)" class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg cursor-pointer hover:bg-base-300 transition-colors"
                    :class="r.name === name ? 'bg-primary/10 text-primary font-semibold' : ''">
                    <i class="ph text-sm shrink-0" :class="r.private ? 'ph-lock-simple opacity-40' : 'ph-globe-simple opacity-30'"></i>
                    <span class="font-mono text-sm truncate" x-text="r.name"></span>
                  </a>
                </template>
                <div x-show="!repos.length" class="text-xs opacity-40 px-2 py-2">No repositories loaded.</div>
              </div>`;
    const dialogHtml = `
