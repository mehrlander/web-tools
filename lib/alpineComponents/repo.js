document.addEventListener('alpine:init', function() {
  Alpine.data('repo', function(opts) {
    ...
    // One dialog, three tabs, targeting dlg.repo — which may differ from the
    // open repo, so it opens for an estate card's repo without navigating there:
    //   Links     repo info + token + the repo's URLs
        ...
        <dialog id="repoModal" class="modal" onclick="if(event.target===this)this.close()">
          <div class="modal-box shadow-none border border-base-300 bg-base-100 p-0
            ...
            <div class="flex items-center justify-between gap-2 p-4 pb-2 shrink-0">
              ...
              <div class="join shrink-0">
                <button @click="dlgTab('info')" class="btn btn-xs btn-square join-item tooltip tooltip-bottom"
                        :class="tab==='info' ? 'btn-primary btn-active' : 'btn-ghost'" data-tip="Links">
            ...
            <div class="flex-1 overflow-y-auto min-h-0 px-4 pb-2">
              ...
              <div x-show="!dlg.loading && tab==='settings'" class="flex flex-col gap-2">
              ...
              <!-- CONFIG tab: the whole file as raw JSON -->
              <div x-show="!dlg.loading && tab==='config'" class="flex flex-col gap-1">
                <textarea x-model="dlg.draft" spellcheck="false" rows="15"
    ...
    return {
      ...
      dlg: { repo: '', ref: 'main', repoObj: null, config: null, configName: null,
             gh: null, obj: {}, draft: '{}', loading: false, saving: false },
