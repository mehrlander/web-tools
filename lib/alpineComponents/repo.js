    ...
    return {
      ...
      template: (inline ? `
        <div class="flex items-center gap-1.5 font-mono text-sm">
          ...
          <span class="text-base-content/30 text-xs" x-show="repo">@</span>
          <details class="dropdown dropdown-bottom" x-show="repo" @toggle="if($event.target.open) $store.browser.ensureBranches()">
            <summary class="cursor-pointer flex items-center gap-0.5 text-xs"
              :class="offMain ? 'text-warning' : 'text-base-content/50'">
              <i class="ph text-[11px]" :class="offMain ? 'ph-git-branch' : 'ph-git-commit'" style="text-decoration:none"></i>
