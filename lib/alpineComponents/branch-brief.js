...
(function () {
  ...
  Alpine.data('branchBrief', function (opts) {
    ...
    return {
          ...
          <div class="flex flex-col gap-0.5"
            ...
            <div data-swipe-section x-show="!loading"
              ...
              <!-- ONE HEADER ROW, as the full deck has. The slide's card is
                   ...
                   (No backticks in this markup: it is a JS template literal.) -->
              <div data-swipe-header class="relative flex items-center gap-1 shrink-0 pl-1 pr-1 py-1 bg-base-200 border-b border-base-300 rounded-t-lg">
                ...
                <span x-show="!!swipeFiles[at]" data-slide-name class="font-mono text-sm min-w-0 flex items-baseline gap-1.5"
                      :title="swipeFiles[at]?.path">
                  <span class="text-xs w-3 text-center shrink-0 font-semibold"
                        :class="card?.statusClass" x-text="card?.statusTag || ''"></span>
                ...
                <details x-show="!!card?.read && (card?.comparable || card?.compareOff)"
                         data-compare-menu class="dropdown dropdown-end shrink-0 max-sm:static" x-ref="slideCmp">
                  <summary class="btn btn-ghost btn-sm gap-1 px-1.5 cursor-pointer" title="Compare against">
                    ...
                    <span class="max-sm:hidden text-xs font-normal"
                          x-text="(card?.compareChoices || []).find(c => c.on)?.label || ''"></span>
                    <i class="ph ph-caret-down text-[10px] opacity-50"></i>
                  </summary>
                ...
                <div x-show="!!card?.viewModes?.length" data-view-modes class="join shrink-0">
                  <template x-for="m in (card?.viewModes || [])" :key="m.key">
                    ...
                    <button type="button" class="btn btn-sm btn-square join-item"
                ...
                <div x-show="swipeFiles.length > 1" data-pager class="flex items-center shrink-0">
                  ...
                  <span class="font-mono text-xs opacity-60 tabular-nums px-1" x-text="(at + 1) + '/' + swipeFiles.length"></span>
                  <button type="button" class="btn btn-xs btn-ghost btn-square max-sm:hidden"
      ...
      get cmpBaseName() {
        const mb = this.brief?.mergeBase;
        return mb ? mb.slice(0, 7) : (o.base || '');
      },
      get compareChoices() {
        const out = [];
        if (this.brief?.mergeBase) out.push({ base: this.brief.mergeBase,
          label: 'Branch changes \u00b7 ' + this.brief.mergeBase.slice(0, 7) });
      ...
      // A document opens on itself; code opens on its diff, with the file one
      // tap away in the header's view buttons.
      slideCardOpts(f) {
        const { action, ...rest } = this.cardOpts(f);
        return { ...rest, open: true, read: true, hosted: true, fill: true,
                 openOn: this.isReviewable(f.path) ? '' : 'diff',
