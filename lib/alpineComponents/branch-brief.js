...
(function () {
  ...
  Alpine.data('branchBrief', function (opts) {
    ...
    return {
        ...
        <div class="mx-auto w-full flex flex-col px-4 pt-2 pb-2 gap-0.5"
          ...
          <div class="flex flex-col gap-0.5"
            ...
            <div data-swipe-section x-show="!loading"
              ...
              <div class="flex items-center gap-1 shrink-0 pl-1 pr-2 py-1 bg-base-200 border-b border-base-300 rounded-t-lg">
                <button type="button" data-file-list-btn class="btn btn-sm btn-ghost btn-square"
                ...
                <!-- "Files", not the file's name: the card under this row names it. -->
                <span class="text-xs uppercase tracking-wide opacity-60">Files</span>
                <span x-show="swipeFiles.length < 2" class="font-mono text-xs opacity-50" x-text="fileCount"></span>
                <div class="grow"></div>
                <div x-show="swipeFiles.length > 1" class="flex items-center gap-1 shrink-0">
                  ...
                  <span class="font-mono text-xs opacity-60 tabular-nums" x-text="(at + 1) + '/' + swipeFiles.length"></span>
                  <button type="button" class="btn btn-xs btn-ghost btn-square"
                ...
                <div x-ref="swipeStrip" @scroll.passive="stripScroll()"
                  ...
                  <template x-for="(f, i) in swipeFiles" :key="f.path">
                    <div data-slide class="relative w-full shrink-0 snap-center flex flex-col min-h-0">
                      ...
                      <div class="overflow-hidden overflow-y-auto flex-1 min-h-0">
      ...
      slideCardOpts(f) {
        const { action, ...rest } = this.cardOpts(f);
        return { ...rest, open: true, read: this.isReviewable(f.path), fill: true };
      },
