        ...
        <div class="mx-auto w-full flex flex-col px-4 pt-2 pb-2 gap-0.5"
          ...
          <div class="flex flex-col gap-0.5"
            ...
            <!-- FLUSH TO THE ROW ABOVE, which is this list's heading and not a
                 ...
                 (No backticks in this markup: it is a JS template literal.) -->
            <!-- 50/50 SPLIT: Locked to the top half (basis-1/2 max-h-[50%]) so the
                 files list sticks to the top half regardless of whether reviewable docs
                 exist on the branch. -->
            <div x-ref="files" x-show="!loading && topPane === 'files'"
                 class="-mt-0.5 flex flex-col gap-1.5 flex-1 min-h-0"
                 :class="!framed && 'roomy:min-h-0 roomy:flex-1 roomy:basis-1/2 roomy:max-h-[50%]'">
              ...
              <div x-ref="fileList" x-show="filesShown && !!listFiles.length && !filesLoading"
                   class="rounded-lg border border-base-300 bg-base-100 overflow-hidden flex-1 min-h-0 overflow-y-auto"
            ...
            <div x-ref="guide" x-show="!loading && topPane === 'guide'"
                 class="relative flex flex-col gap-1 flex-1 min-h-0 roomy:flex-1 roomy:min-h-0"
                 :class="!framed && 'roomy:basis-1/2 roomy:max-h-[50%]'">
             <div class="flex flex-col gap-3 overflow-y-auto rounded-box flex-1 min-h-0">
            ...
            <div data-rev-section x-show="!loading"
                 class="mt-1 flex flex-col gap-0.5 min-h-0 shrink-0"
                 :class="!framed && 'roomy:basis-1/2 roomy:flex-1 roomy:min-h-0 roomy:max-h-[50%]'">
             <div x-show="reviewableFiles.length > 0"
                  class="flex-1 min-h-0 flex flex-col gap-0.5 h-[17rem] roomy:h-full">
              <div x-ref="revStrip" @scroll.passive="revScroll()"
               ...
               <template x-for="f in reviewableFiles" :key="f.path">
                 <div data-rev class="relative w-full shrink-0 snap-center h-full flex flex-col">
                   ...
                   <div class="rounded-box overflow-y-auto flex-1 min-h-0">
             ...
             <div data-rev-placeholder x-show="reviewableFiles.length === 0"
                  class="flex-1 min-h-0 flex items-center justify-center p-4 rounded-box border border-dashed border-base-300/60">
               <span class="text-xs opacity-40 font-mono">no reviewable docs on this branch</span>
             </div>
            </div>
