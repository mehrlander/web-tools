...
(function () {
  ...
  Alpine.data('branchBrief', function (opts) {
    ...
    return {
        ...
        <div class="mx-auto w-full flex flex-col px-4 pt-2 pb-2 gap-0.5"
          ...
          <div class="flex flex-col gap-0.5"
            ...
            <!-- FLUSH TO THE ROW ABOVE, which is this list's heading and not a
                 ...
                 (No backticks in this markup: it is a JS template literal.) -->
            <!-- 50/50 SPLIT: Top half (basis-1/2 max-h-[50%]) holding files and guide
            ...
            <div data-top-section x-show="!loading"
                 ...
                 @pointerdown="topHovered = true; revHovered = false"
                 class="-mt-0.5 flex flex-col min-h-0"
                 :class="framed ? 'flex-1 min-h-0 basis-1/2 max-h-[50%]'
                                : 'roomy:min-h-0 roomy:flex-1 roomy:basis-1/2 roomy:max-h-[50%]'">
              <div x-ref="topStrip" @scroll.passive="topScroll()"
                ...
                <!-- ══ Files ════════════════════════════════════════════════════ -->
                <div x-ref="files"
                     class="relative w-full shrink-0 snap-center h-full flex flex-col gap-1.5 min-h-0"
                     :class="framed ? 'flex-1 min-h-0 basis-1/2 max-h-[50%]'
                                    : 'roomy:min-h-0 roomy:flex-1 roomy:basis-1/2 roomy:max-h-[50%]'">
              ...
              <div x-ref="fileList" x-show="filesShown && !!listFiles.length && !filesLoading"
                   class="rounded-box border border-base-300 bg-base-100 overflow-hidden flex-1 min-h-0 overflow-y-auto"
            ...
            <!-- CLIPPED TO ITS HEAD, with the page's one expander. It led the
                 ...
                 (No backticks in this markup: it is a JS template literal.) -->
            <div x-ref="guide" x-show="hasGuide"
                 class="relative w-full shrink-0 snap-center h-full flex flex-col gap-1 min-h-0"
                 :class="framed ? 'flex-1 min-h-0 basis-1/2 max-h-[50%]'
                                : 'roomy:flex-1 roomy:min-h-0 roomy:basis-1/2 roomy:max-h-[50%]'">
             <div class="rounded-box border border-base-300 bg-base-100 overflow-hidden flex-1 min-h-0 overflow-y-auto flex flex-col gap-3"
                  :class="!framed && 'roomy:min-h-0 roomy:overflow-y-auto'">
            ...
            <!-- 50/50 SPLIT: The reviewable section takes the bottom half (basis-1/2 max-h-[50%]).
                 When reviewables exist, the strip fills the bottom half without a fixed rem cap.
                 When none exist, a quiet placeholder preserves the bottom half so the top pane
                 remains anchored to the top half. -->
            <div data-rev-section x-show="!loading"
                 @pointerenter="revHovered = true" @pointerleave="revHovered = false"
                 @pointerdown="revHovered = true; topHovered = false"
                 class="mt-1 flex flex-col gap-0.5 min-h-0"
                 :class="framed ? 'flex-1 min-h-0 basis-1/2 max-h-[50%]'
                                : 'roomy:basis-1/2 roomy:flex-1 roomy:min-h-0 roomy:max-h-[50%]'">
             <div x-show="reviewableFiles.length > 0"
                  class="flex-1 min-h-0 flex flex-col gap-0.5 h-full">
              <div x-ref="revStrip" @scroll.passive="revScroll()"
               ...
               <template x-for="f in reviewableFiles" :key="f.path">
                 <div data-rev class="relative w-full shrink-0 snap-center h-full flex flex-col">
                   <div x-show="!fileState && stateOf(f.path) === 'missing'"
                   ...
                   <div class="rounded-box border border-base-300 bg-base-100 overflow-hidden overflow-y-auto flex-1 min-h-0">
              ...
              <div data-rev-pager x-show="reviewableFiles.length > 1"
                   class="flex flex-wrap items-center justify-center shrink-0">
              ...
              <div data-rev-placeholder x-show="reviewableFiles.length === 0"
                   class="flex-1 min-h-0 flex items-center justify-center p-4 rounded-box border border-dashed border-base-300/60 h-full">
      ...
      topPanels() {
        const el = this.$refs.topStrip;
        if (!el) return [];
        const panels = [];
        if (this.$refs.files) panels.push(this.$refs.files);
        if (this.$refs.guide && this.hasGuide) panels.push(this.$refs.guide);
        return panels;
      },
      topX(i) {
        const el = this.$refs.topStrip, k = this.topPanels()[i];
        if (!el || !k) return null;
        return k.getBoundingClientRect().left - el.getBoundingClientRect().left + el.scrollLeft;
      },
      topScroll() {
        ...
        const x0 = el.getBoundingClientRect().left;
        let best = 0, near = Infinity;
        this.topPanels().forEach((k, i) => {
          const d = Math.abs(k.getBoundingClientRect().left - x0);
          if (d < near) { near = d; best = i; }
        });
        const pane = (best === 1 && this.hasGuide) ? 'guide' : 'files';
        if (this.topPane !== pane) {
          this.topPaneAsked = pane;
          this.pane = pane;
          if (pane === 'files' && this.brief?.pending) this.ensureCompare();
          this.reportMeta();
        }
      },
      syncTopStrip(smooth = false) {
        const el = this.$refs.topStrip;
        if (!el || !this.hasGuide) return;
        const target = this.topPane === 'guide' ? 1 : 0;
        const x = this.topX(target);
        if (x === null) return;
        if (smooth && el.scrollTo) el.scrollTo({ left: x, behavior: 'smooth' });
        else el.scrollLeft = x;
      },
