...
(function () {
  ...
  Alpine.data('branchBrief', function (opts) {
    ...
    return {
            ...
            <div x-show="!loading && hasLook"
              ...
              <template x-for="t in pageChips" :key="t.addr">
                <a :href="t.url" target="_blank" rel="noopener" :title="t.title"
                  ...
                  <span class="text-sm leading-none" x-text="t.mark"></span><span x-text="t.label"></span></a>
              </template>
          ...
          <div class="flex flex-col gap-0.5"
            ...
            <div class="flex items-center gap-2 shrink-0 sticky top-0 z-20 -mx-4 px-4 bg-base-200">
                ...
                <div class="flex items-baseline gap-1.5 min-w-0 shrink">
                  <!-- TWO TABS, NOT A TOGGLE AND A JUMP. The row carried a caret
                       that opened the file list in place and, beside it, a
                       marker that scrolled to the guide: two different gestures
                       for two things that are alternatives, and neither said
                       what the other was doing. They are alternatives, so they
                       are tabs, which is also what makes the pane below them a
                       fixed box the locked layout can divide.
                       What went with the switch: the caret, which said open or
                       shut about a thing that is now always one of two, and the
                       guide marker's down arrow, which pointed at a section
                       that no longer sits anywhere in particular.
                       (No backticks in this markup: it is a JS template literal.) -->
                  <div role="tablist" class="tabs tabs-box tabs-xs bg-base-200 p-0.5 shrink-0 flex-nowrap">
                    <a role="tab" class="tab gap-1 whitespace-nowrap"
                       :class="topPane === 'files' && 'tab-active'"
                       @click="setPane('files')" title="The files this branch changed">
                      <span class="font-medium">Files</span>
                      <span class="font-mono opacity-45" x-text="fileCount"></span></a>
                    <a role="tab" x-show="hasGuide" class="tab gap-1 whitespace-nowrap"
                       :class="topPane === 'guide' && 'tab-active'"
                       @click="setPane('guide')" :title="guideJumpTitle">
                      <span class="font-medium">Guide</span>
                      <span class="font-mono opacity-45"
                            x-text="guidePr ? '#' + guidePr.number : ''"></span></a>
                  </div>
                </div>
                <div class="grow"></div>
            ...
            <!-- FLUSH TO THE ROW ABOVE, which is this list's heading and not a
                 ...
                 (No backticks in this markup: it is a JS template literal.) -->
            <!-- NO CAP ANY MORE. The 40% ceiling existed to stop a long list
                 crushing the guide when both were on screen at once. They are
                 tabs now, so only one is, and each takes the whole pane. -->
            <div x-ref="files" x-show="!loading && topPane === 'files'"
                 class="-mt-0.5 flex flex-col gap-1.5"
                 :class="!framed && 'roomy:min-h-0 roomy:flex-1'">
              ...
              <div x-ref="fileList" x-show="filesShown && !!listFiles.length && !filesLoading"
                ...
                <!-- The caveat itself, opened by its own chip. Both of these say
                     ...
                     tooltip should be the only place to say. -->
            ...
            <!-- CLIPPED TO ITS HEAD, with the page's one expander. It led the
                 ...
                 (No backticks in this markup: it is a JS template literal.) -->
            <div x-ref="guide" x-show="!loading && topPane === 'guide'"
                 class="relative flex flex-col gap-1 roomy:flex-1 roomy:min-h-0">
             <div class="flex flex-col gap-3 overflow-y-auto rounded-box
                         roomy:flex-1 roomy:min-h-0 roomy:max-h-none"
            ...
            <template x-if="!hasGuide">
             ...
             <button type="button" x-show="!clipExpanded('guide') && !!clipTall['guide']"
                     ...
                     class="roomy:hidden absolute inset-x-0 bottom-0 h-16 flex items-end justify-center pb-2
                            rounded-b-box bg-gradient-to-b from-transparent via-[var(--color-base-100)]/80 to-[var(--color-base-100)]
             ...
             <button type="button" x-show="clipExpanded('guide')" @click="toggleClip('guide')"
                     class="roomy:hidden w-full pt-1 flex items-center justify-center gap-1
            ...
            <!-- gap-1, not the container's gap-2: the dots belong to the strip
                 ...
                 heading row. -->
            <!-- mt-1 on top of the container's 2px gap, so the seam between the
                 two PANELS is 6px where a chrome seam is 2. The one-gap rule
                 below still holds for chrome; content panels are the exception
                 the reader asked for on 2026-09-07, and an exception with a
                 reason is not the inverted scale the rule was written against. -->
            <div x-show="!loading && reviewableFiles.length > 0"
                 class="mt-1 shrink-0 flex flex-col gap-0.5">
             <!-- LOCKED, THE STRIP IS THE FIXED ONE and the guide takes the
                  slack, which is what the reader asked for. It needs a definite
                  height to say so: the panels inside are h-full, so without one
                  here each panel sizes from its own content, the strip sizes
                  from the tallest panel, and the guide gets whatever is left of
                  a box the strip has already spent. 22rem is the clip the
                  panels carried before they became scrollers. -->
             <div x-ref="revStrip" @scroll.passive="revScroll()"
                  class="roomy:h-[22rem] roomy:shrink-0 roomy:items-stretch
                         flex items-start gap-3 snap-x snap-mandatory
                         overflow-x-auto overflow-y-hidden overscroll-x-contain
                         [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              <template x-for="f in reviewableFiles" :key="f.path">
                <div data-rev class="relative w-full shrink-0 snap-center roomy:h-full roomy:flex roomy:flex-col">
                  <div x-show="!fileState && stateOf(f.path) === 'missing'"
                  ...
                  <!-- LOCKED, THIS SCROLLS AND DOES NOT CLIP. The panel has the
                       strip's height, the card inside is told to fill rather
                       than bound itself, and what does not fit is scrolled
                       here, which is one scroller and not two. The more/less
                       expander goes with the clip: there is nowhere to expand
                       into in a locked box, and it was only ever standing in
                       for content the clip had put out of reach.
                       Unlocked (a phone turned sideways) the clip and its
                       expander are what there is, since the document scrolls
                       and a panel has no height to fill.
                       (No backticks in this markup: it is a JS template literal.) -->
                  <div class="rounded-box overflow-hidden
                              roomy:flex-1 roomy:min-h-0 roomy:max-h-none roomy:overflow-y-auto"
      ...
      setPane(p) {
        this.pane = p;
        if (p === 'files' || p === 'guide') this.topPaneAsked = p;
        this.reportMeta();
      ...
      get topPane() {
        ...
        // An address can ask for the guide on a branch that has none, and the
        // Guide tab is not drawn there, so honouring the ask would leave both
        // tabs inactive over an empty pane.
      ...
      async openFileFromAddress(path, pass) {
        ...
        try {
          ...
          if (!this.isReviewable(path)) {
            ...
            if (!g) return;
            if (!this.filesShown) this.topPaneAsked = 'files';
