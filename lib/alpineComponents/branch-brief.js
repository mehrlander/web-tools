  ...
  Alpine.data('branchBrief', function (opts) {
    ...
    return {
      ...
      template: `
        ...
        <div class="mx-auto w-full flex flex-col px-4 pt-2 pb-2 gap-2"
          ...
          <div class="shrink-0 flex flex-col gap-1">
            ...
            <div class="flex flex-col gap-0.5 min-w-0">
              ...
              <!-- THE FIGURES, ON A LINE RATHER THAN IN A CARD, and part of the
                   ...
                   (No backticks in this markup: it is a JS template literal.) -->
              <!-- No mt-1. This is the block's third line and the block already
                   spaces its lines; a margin on one of three doubled the gap
                   above this one alone, which is a rhythm with an exception in
                   it rather than a rhythm. -->
            ...
            <div x-show="!loading && hasLook"
              ...
              <span x-show="!!routeChips?.nearCount" :data-note="routeChips?.nearTitle"
          ...
          <div class="flex flex-col gap-2"
               :class="framed ? 'flex-1 min-h-0 overflow-y-auto pb-4'
                              : 'roomy:flex-1 roomy:min-h-0 roomy:overflow-y-auto roomy:pb-4'">
            ...
            <!-- THE HEADING ROW IS THE SHELL'S CHILD, NOT THE HEAD'S, and that is
              ...
              <!-- STICKY, UNCONDITIONALLY, because it only bites where it is
                   ...
                   for the same reason. -->
              <!-- NO VERTICAL PADDING. This row is a toolbar and its height is
                 its tap targets: four controls at max-sm:h-11, which is the 44px
                 a phone wants. py-2 on top of that was padding an affordance
                 that already carries its own room, and it cost 16px of the
                 first screen to say nothing: a 60px band around a 20px heading,
                 reported 2026-09-07. Separation from the sections either side is
                 the container's gap.
                 The floor here is the tap target and not the padding: 44px is
                 the whole row now, so anything tighter means giving the target
                 up.
                 (No backticks in this markup: it is a JS template literal.) -->
            <div class="flex items-center gap-2 shrink-0 sticky top-0 z-20 -mx-4 px-4 bg-base-200">
                ...
                <div class="flex items-baseline gap-1.5 min-w-0 shrink">
                  <!-- THE HEADING IS THE LIST'S SWITCH, which is where a section
                       toggle belongs: the row is already the list's chrome and
                       already sticky, so the control that shows and hides it
                       costs no line of its own. Blank until the compare lands
                       rather than reading 0, which a branch with no changed files
                       would also read. -->
                  <button type="button" @click="toggleFiles()"
                          class="flex items-baseline gap-1.5 shrink-0 hover:opacity-70 transition-opacity"
                          :title="filesShown ? 'Hide the file list' : 'Show the file list'">
                    <i class="ph text-xs opacity-50 self-center"
                       :class="filesShown ? 'ph-caret-down' : 'ph-caret-right'"></i>
                    <span class="text-sm font-medium">Files</span>
                    <span class="font-mono text-sm opacity-45" x-text="fileCount"></span></button>
                  <!-- The marker carries the guide's TITLE where there is room
                       for it, and the number everywhere. It is the first thing
                       dropped at 390px, where four controls to the right leave
                       the row nothing to spend.
                       IT POINTS DOWN, because the guide is below this row.
                       That has been true, then not, then true again inside two
                       days, so it is worth saying why the arrow is worth
                       maintaining at all: this row is pinned while everything
                       else scrolls under it, and the marker is the one thing on
                       it that says a guide exists and where it is. An arrow is
                       cheap and pointing it the wrong way is not. -->
            ...
            <!-- NO HEIGHT RULE. This carried roomy:max-h-[45%] to stop a long
                 list crushing the guide while the two shared a locked box as
                 panes: measured 2026-09-04 at 1440x900, a 658px box splitting
                 296 files and 346 guide. The container above is one scroller
                 now and the guide is clipped and scrolls in place, so there is
                 nothing here to protect it from and the list is as long as it
                 is. Do not reintroduce a cap without reintroducing the pane it
                 defended against. -->
            <!-- FLUSH TO THE ROW ABOVE, which is this list's heading and not a
                 third thing between the guide and the documents. -mt-2 cancels
                 the container's own gap for this one seam. It also buys back
                 the second gap a SHUT list was costing: a zero-height flex
                 child still takes a gap on each side, which is why the row sat
                 16px from the strip while the guide sat 8 from the row.
                 (No backticks in this markup: it is a JS template literal.) -->
            <div x-ref="files" x-show="!loading" class="-mt-2 flex flex-col gap-1.5">
              ...
              <div x-ref="fileList" x-show="filesShown && !!listFiles.length && !filesLoading"
            ...
            <!-- CLIPPED TO ITS HEAD, with the page's one expander. It led the
                 page for a day and the clip was what made leading with it
                 affordable; it sits under a shut file list now and the clip
                 matters for the same reason from the other side, since a
                 two-thousand-word body here would put the presented documents
                 screens below it.

                 EIGHTEEN REM, against the presented files' twenty-two, and the
                 difference is the point rather than an oversight. A panel in
                 the strip IS the document, which is what the page exists to
                 show; this is a preview of prose whose full text is one tap
                 away, on the PR and in the expander. The shorter block is the
                 one a reader passes through on the way to the other two.

                 SCROLLS AS WELL AS EXPANDS, which is the difference between
                 this block and a panel of the strip. A panel's card owns a
                 scroller inside its own pane, so a second one there would be a
                 scrollbar inside a scrollbar; the guide card is one block of
                 markdown under three short header lines and owns none, so the
                 clip can be the scroller. Shipped overflow-hidden on 2026-09-06
                 and reported the next day: a reader who wants to skim the body
                 without giving it the whole page had nothing to drag.

                 The fade stays, and now means two things at once, which is
                 honest: there is more below, and the whole thing is one tap
                 away. Nothing pins overscroll-y, so a drag that reaches the end
                 of the guide carries on down the page rather than stopping
                 dead under the finger.

                 It was a PANE until 2026-09-06, growing into whatever the file
                 list left and scrolling the prose inside it, which is how it
                 stayed reachable under a long list. The clip does that job now
                 wherever the guide sits, so the pane and the second page-level
                 scrollbar are both gone. See the container note above.
                 (No backticks in this markup: it is a JS template literal.) -->
            <div x-ref="guide" x-show="!loading" class="relative flex flex-col gap-1">
             <div class="flex flex-col gap-3 overflow-y-auto rounded-box"
                  x-init="watchClip($el, 'guide')"
                  :class="clipExpanded('guide') ? '' : 'max-h-[18rem]'">
            ...
            <template x-if="!hasGuide">
             ...
             <button type="button" x-show="!clipExpanded('guide') && !!clipTall['guide']"
                     @click="toggleClip('guide')"
                     class="absolute inset-x-0 bottom-0 h-16 flex items-end justify-center pb-2
                            rounded-b-box bg-gradient-to-b from-transparent via-base-100/80 to-base-100
                            text-xs text-base-content/60 hover:text-base-content transition-colors">
               <span class="flex items-center gap-1"><i class="ph ph-caret-down"></i>more</span></button>
             <button type="button" x-show="clipExpanded('guide')" @click="toggleClip('guide')"
                     class="w-full pt-1 flex items-center justify-center gap-1
                            text-xs text-base-content/50 hover:text-base-content transition-colors">
               <i class="ph ph-caret-up"></i>less</button>
            </div>

            <!-- ══ Reviewable ═══════════════════════════════════════════════
                 The pages and the prose, out of the file list and below it.
                 They were mid-list until 2026-09-05, sorted by path among the
                 code, then briefly a group inside the list, then a section
                 above it; a section is what they are, and 2026-09-06 settled
                 where. What a reader reviews by READING is a different question
                 from who wrote a file, and the list answers the second.

                 LAST, wherever the other two sit. Above the list they were the
                 first thing met, which put a document in front of a reader who
                 had not yet seen what the branch touched or been told what it
                 was for. Reading one is the deepest thing this page offers, so
                 it comes after the two that are read at a glance.

                 NOT A DUPLICATE OF THE LOOK ROW ABOVE. That row's chip opens the
                 RENDER of a page; a row here opens its DIFF, and a deleted page
                 has a row here and no chip at all.

                 ONE CONTAINER, SWIPED, not a stack. Stacked, a branch with three
                 documents was three screens of scroll before the file list, and
                 the reader met each one by scrolling past the last. They are
                 alternatives to each other, so they share a place: a CSS
                 scroll-snap strip, which is native scrolling with momentum and
                 needs no kit. The full-screen deck is still there and still
                 opens on this same set; this is the on-page reading of them.

                 THE DOTS ARE NOT DECORATION. A touch that starts inside an
                 iframe never reaches the parent scroller, so on the html panel
                 swiping does nothing at all and the dots are the only way off
                 it. They are also the position, which a strip has no other way
                 to state.

                 overflow-y-hidden is explicit because a box scrolling on one
                 axis computes the other from visible to auto, which would give
                 the strip a second scrollbar. items-start keeps the strip at the
                 tallest panel rather than stretching the short ones.
                 (No backticks in this markup: it is a JS template literal.) -->
            <!-- gap-1, not the container's gap-2: the dots belong to the strip
                 above them, and spacing is how a page says so. At the section
                 gap they read as a peer of the strip rather than its pager,
                 which is the mistake the file list was making against its own
                 heading row. -->
            <div x-show="!loading && reviewableFiles.length > 0"
                 class="shrink-0 flex flex-col gap-1">
             <div x-ref="revStrip" @scroll.passive="revScroll()"
                  class="flex items-start gap-3 snap-x snap-mandatory
                         overflow-x-auto overflow-y-hidden overscroll-x-contain
                         [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              <template x-for="f in reviewableFiles" :key="f.path">
                <div data-rev class="relative w-full shrink-0 snap-center">
                  <div x-show="!fileState && stateOf(f.path) === 'missing'"
                  ...
                  <div class="overflow-hidden rounded-box"
                       x-init="watchClip($el, f.path)"
                       :class="clipExpanded(f.path) ? '' : 'max-h-[22rem]'">
                    <div x-data="fileReview(reviewCardOpts(f))"></div>
                  </div>
                  <!-- The fade IS the clip saying so, which is why it overlays
                       the card's last lines rather than sitting under them: a
                       rule below the clip would read as the end of the file. It
                       fades to base-100 because that is the CARD's ground, not
                       the page's; to-base-200 put a grey wash over white and the
                       word floated on the prose with nothing behind it. The
                       stops are transparent and /80, not /0 and /85: neither of
                       those generates a rule, so the first pass shipped a
                       gradient that was not there (npm run dead-opacity, which
                       the suite runs). -->
                  <button type="button" x-show="!clipExpanded(f.path) && !!clipTall[f.path]"
                          @click="toggleClip(f.path)"
                          class="absolute inset-x-0 bottom-0 h-16 flex items-end justify-center pb-2
                                 rounded-b-box bg-gradient-to-b from-transparent via-base-100/80 to-base-100
                                 text-xs text-base-content/60 hover:text-base-content transition-colors">
                    <span class="flex items-center gap-1"><i class="ph ph-caret-down"></i>more</span></button>
                  <button type="button" x-show="clipExpanded(f.path)" @click="toggleClip(f.path)"
                          class="w-full pt-1 flex items-center justify-center gap-1
                                 text-xs text-base-content/50 hover:text-base-content transition-colors">
                    <i class="ph ph-caret-up"></i>less</button>
                </div>
             ...
             <!-- Tap targets, not dots: the dot is 6px and the button around it
                  is 28, which is the difference between a control and a hint. -->
             <div data-rev-pager x-show="reviewableFiles.length > 1"
                  class="flex flex-wrap items-center justify-center">
               <template x-for="(f, i) in reviewableFiles" :key="f.path">
                 <button type="button" @click="goRev(i)" :title="f.path"
                         :aria-current="revAt === i" class="p-2.5 cursor-pointer group">
                   <span class="block w-1.5 h-1.5 rounded-full transition-colors"
                         :class="revAt === i ? 'bg-base-content/60'
                                             : 'bg-base-content/20 group-hover:bg-base-content/40'"></span>
                 </button>
               </template>
             </div>
            </div>
      ...
      // WHETHER THE LIST IS DRAWN AT ALL, which is a section-level answer and
      // not the group toggles one level down. `null` means the branch decides:
      // shut where there is a reviewable section to stand in its place, open
      // where there is not. Measured over 20 merged branches, twelve changed no
      // .html and two changed neither .html nor .md, so the second case is
      // ordinary and a page that collapsed to nothing but a caret would be the
      // common one.
      filesOpen: null,
      get filesShown() { return this.filesOpen ?? this.reviewableFiles.length === 0; },
      toggleFiles() { this.filesOpen = !this.filesShown; },
      ...
      // THE REVIEWABLE SECTION IS ALWAYS IN, because it has no toggle to be out
      // by: it is a section, not a group. The list's groups keep the rule they
      // had, where a collapsed one is a reader saying the machine's output is
      // not what they came for and the deck honours that. And the whole list
      // being shut is a third state above them, which is why filesShown gates
      // the groups rather than each group answering for itself.
      //
      // What this guarantees is the thing that decided the shape: the deck is
      // never empty on a branch with a page or a doc in it. Its button keys
      // x-show on deckFiles.length, so an empty deck takes the page's one
      // accented control off the screen with it.
      get deckFiles() {
        const inList = this.filesShown
          ? this.fileGroups.filter(g => this.groupOpen(g)).flatMap(g => g.files) : [];
      ...
      async openFileFromAddress(path, pass) {
        ...
        try {
          ...
          if (!this.isReviewable(path)) {
            const g = this.fileGroups.find(x => x.files.some(f => f.path === path));
            if (!g) return;
            if (!this.filesShown) this.filesOpen = true;
            if (!this.groupOpen(g)) this.groupState[g.mode] = true;
          } else if (!this.reviewableFiles.some(f => f.path === path)) {
            return;
          }
      ...
      reviewCardOpts(f) {
        ...
        return { ...rest, open: true, read: true };
      },
