    ...
    return {
      ...
      async load() {
        ...
        try {
          ...
          const at = { repo: this.repo, branch: this.branch, base: this.base, pull: o.pull || null };
          ...
          this._pulls = r.pulls;
          this.brief = this.assemble({ compare: r.compare || null, noBase: !!r.noBase, prFallback: !!r.prFallback });
      ...
      assemble({ compare, noBase, prFallback }) {
        return window.BranchBrief.assemble({
          repo: this.repo, branch: this.branch, base: this.base,
          compare, noBase, pull: this._pulls[0] || o.pull || null, pulls: this._pulls,
      ...
      async _readCompare(pass) {
        ...
        try {
          const gh = new window.GH({ token: window.TOKEN, repo: this.repo, ref: this.branch });
          const { compare, noBase, prFallback } = await window.BranchBrief.readCompare(gh, {
            repo: this.repo, branch: this.branch, base: this.base, pull: this._pulls?.[0] || o.pull || null,
          });
          if (pass !== this._pass) return;             // a step landed while reading
          this.brief = this.assemble({ compare, noBase, prFallback });
      ...
      reportMeta() {
        ...
        try {
          o.onMeta?.({ repo: this.repo, branch: this.branch, base: this.base,
                       ...
                       prFallback: !!this.brief?.prFallback });
        } catch {}
      },
