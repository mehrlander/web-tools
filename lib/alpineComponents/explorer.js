document.addEventListener('alpine:init', function() {
  Alpine.data('explorer', function() {
    ...
    return {
      ...
      template: `
        <div>
          <div class="flex items-center gap-1 font-mono text-sm flex-wrap min-h-8">
          ...
          <div class="border border-base-300 rounded-lg bg-base-100 overflow-hidden mt-2">
            ...
            <div x-show="!loading" class="divide-y divide-base-200">
              <div x-show="path" @click="load(parentPath)"
                   class="px-3 py-1.5 hover:bg-base-200 cursor-pointer font-mono text-sm opacity-50">..</div>
              <template x-for="f in tree" :key="f.path">
                <div @click="f.type==='dir' ? load(f.path) : sel(f.path)"
                     class="group flex items-center justify-between gap-2 px-3 py-1.5 hover:bg-base-200 cursor-pointer text-sm"
                  ...
                  <div class="flex items-center gap-2 shrink-0">
                    <span x-show="f.type!=='dir'" class="text-[10px] font-mono text-base-content/40 hidden sm:inline"
              ...
              <div x-show="!tree.length" class="px-3 py-4 text-sm opacity-40 font-mono">empty</div>
            </div>
          </div>
        </div>`,
