          ...
          <div class="border border-base-300 rounded-lg bg-base-100 overflow-hidden mt-2">
            ...
            <div x-show="!loading" class="max-h-[60vh] overflow-y-auto p-1">
              <div x-show="path" @click="load(parentPath)"
                   class="px-2 py-1.5 rounded hover:bg-base-200 cursor-pointer font-mono text-sm opacity-50">..</div>
              <template x-for="f in tree" :key="f.path">
                <div @click="f.type==='dir' ? load(f.path) : sel(f.path)"
                     class="group flex items-center justify-between gap-2 px-2 py-1.5 rounded hover:bg-base-200 cursor-pointer text-sm"
              ...
              <div x-show="!tree.length" class="px-2 py-4 text-sm opacity-40 font-mono">empty</div>
            </div>
          </div>
        </div>`,
