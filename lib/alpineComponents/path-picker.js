      ...
      template: `
        <div class="relative" @click.outside="open = false">
          ...
          <section x-cloak x-show="open"
            ...
            <header class="flex min-h-10 shrink-0 items-center gap-1 border-b border-base-300 px-1.5">
              <button type="button" class="btn btn-ghost btn-xs shrink-0" :disabled="!scope.length"
              ...
              <nav aria-label="Current path"
                ...
                <button type="button" class="btn btn-ghost btn-xs shrink-0 font-normal"
                        :class="!scope.length && 'bg-base-200 font-medium'" @pointerdown.prevent="jump(0)">Repos</button>
                <template x-for="(node, index) in scope" :key="index + ':' + node.name">
                  <span class="contents">
                    <span class="shrink-0 text-base-content/30">/</span>
                    <button type="button" class="btn btn-ghost btn-xs shrink-0 font-normal font-mono"
              ...
              <button x-show="mode === 'dir'" type="button" :disabled="!dirSpec()"
                      class="btn btn-primary btn-xs shrink-0 gap-1" @pointerdown.prevent="pickDir()">
                <i class="ph ph-check"></i>Here
              </button>
              <button type="button" class="btn btn-ghost btn-xs btn-square shrink-0"
                      aria-label="Close" @pointerdown.prevent="open = false">
                <i class="ph ph-x"></i>
              </button>
            </header>
