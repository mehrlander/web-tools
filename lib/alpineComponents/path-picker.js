            ...
            <div x-ref="options" role="listbox" class="min-h-0 flex-1 overscroll-contain overflow-y-auto p-1">
              ...
              <template x-for="(node, index) in (loading ? [] : children())" :key="index + ':' + node.name">
                <button type="button" role="option" :data-index="index"
                  ...
                  <span class="min-w-0 flex-1 truncate font-mono text-base" x-text="node.name"></span>
                  <span x-show="node.children" class="badge badge-ghost badge-xs shrink-0" x-text="node.children?.length"></span>
                  <span x-show="node.children" class="shrink-0 text-base-content/35"><i class="ph ph-caret-right text-sm"></i></span>
                </button>
              </template>
              <div x-show="!loading && !children().length" class="grid min-h-20 place-items-center px-3 text-center text-base text-base-content/50">
                Empty.
              </div>
            </div>
