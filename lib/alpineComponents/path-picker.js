...
//   inline: true — the panel is part of the page rather than a dropdown: open
//         from the start, no close button, filling its host's height. For a
//         host whose job IS the tree (the project and repo Files views,
//         alpineComponents/file-browser.js).
//   base: {repo, ref?, dir?} — start inside that folder and never climb above
//         it: the crumbs begin at the base, "Up" stops there, and `roots` is
//         not consulted. The picker enters the repo and descends on first
//         open, so a project's tree opens on the project.
//   start: a folder under the base (a path from the repo root) to open on,
//         with the floor still at the base: a repo's tree opening on the
//         folder of a file it was asked to show, with the crumbs above it
//         still walkable.
...
const registerPathPicker = function() {
  Alpine.data('pathPicker', function(opts) {
    ...
    return {
      ...
      open: cfg.inline === true,
      scope: [],
      floor: 0,                   // with a base, the scope depth the reader cannot climb above
      picked: '',                 // file mode: the last picked path, marked in the list
      tree: [],
      loading: false,
      truncated: false,
      error: '',
      ...
      template: `
        <div class="relative" :class="inline && 'h-full'" @click.outside="inline || (open = false)">
          ...
          <section x-cloak x-show="open"
                   class="flex flex-col overflow-hidden rounded-box border border-base-300 bg-base-100"
                   :class="inline ? 'h-full' : 'absolute inset-x-0 top-full z-40 mt-1 max-h-[50vh] shadow-xl'">
            ...
            <header class="flex min-h-8 shrink-0 items-center gap-0.5 border-b border-base-300 px-1">
              <button type="button" class="btn btn-ghost btn-xs btn-square shrink-0" :disabled="scope.length <= floor"
              ...
              <nav aria-label="Current path"
                ...
                <button x-show="!floor" type="button" class="shrink-0 rounded px-1 py-0.5 text-sm hover:bg-base-200"
                ...
                <template x-for="(node, index) in scope" :key="index + ':' + node.name">
                  <span class="contents" x-show="index >= floor - 1">
                    <span x-show="index > floor - 1 || !floor" class="shrink-0 text-base-content/30 text-sm">/</span>
              ...
              <button x-show="!inline" type="button" class="btn btn-ghost btn-xs btn-square shrink-0"
            ...
            <div x-ref="options" role="listbox" class="min-h-0 flex-1 overscroll-contain overflow-y-auto p-1">
              ...
              <template x-for="(node, index) in (loading ? [] : matches())" :key="index + ':' + node.name">
                <button type="button" role="option" :data-index="index" :aria-selected="active === index"
                        ...
                        :class="[active === index || (picked && picked === pathOf(node)) ? 'bg-primary/10 text-primary' : 'hover:bg-base-200',
      ...
      init() {
        ...
        if (this.inline) this.ensureTree();
      },
      ...
      async ensureTree() {
        ...
        try {
          roots = base?.repo ? [{ repo: base.repo, ref: base.ref || '' }]
            : (await (typeof cfg.roots === 'function' ? cfg.roots() : cfg.roots)) || [];
        ...
        if (base?.repo) await this.enterBase(String(base.dir || '').replace(/^\/+|\/+$/g, ''),
          String((typeof cfg.start === 'function' ? cfg.start() : cfg.start) || '').replace(/^\/+|\/+$/g, ''));
      },
      // Walk into the base: the repo, then each folder of `dir`. The floor is
      // where that walk ends, so the crumbs and "Up" treat it as the top. A
      // folder the tree does not have (renamed, or not on this ref) stops the
      // walk where it failed and says so, rather than showing the repo root as
      // though it were the folder.
      async enterBase(dir, start = '') {
        const repoNode = this.tree[0];
        if (!repoNode) return;
        await this.loadRepo(repoNode);
        const scope = [repoNode];
        // A tree that could not be read already says so; the folder walk
        // would only replace that reason with a wrong one.
        if (this.error) { this.scope = scope; this.floor = 1; return; }
        for (const name of dir ? dir.split('/') : []) {
          const next = (scope[scope.length - 1].children || []).find(n => n.children && n.name === name);
          if (!next) { this.error = 'No folder ' + dir + ' in ' + repoNode.repo + (repoNode.ref ? '@' + repoNode.ref : '') + '.'; break; }
          scope.push(next);
        }
        this.floor = scope.length;
        // Then on to the start folder, when it lies under the base; a start
        // outside it is ignored rather than allowed to lift the floor.
        const within = !dir || start === dir || start.startsWith(dir + '/');
        const rest = start && within ? start.slice(dir ? dir.length + 1 : 0) : '';
        for (const name of rest ? rest.split('/') : []) {
          const next = (scope[scope.length - 1].children || []).find(n => n.children && n.name === name);
          if (!next) break;
          scope.push(next);
        }
      ...
      // A node's path inside its repo, for marking the picked file.
      pathOf(node) { return [...this.scope.slice(1).map(n => n.name), node.name].join('/'); },
      ...
      jump(depth) {
        this.scope = this.scope.slice(0, Math.max(depth, this.floor));
        this.query = '';
        this.active = 0;
        this.resetScroll();
      },
      up() { this.jump(Math.max(this.floor, this.scope.length - 1)); },
