        ...
        <div>
          ...
          <div x-show="tab==='activity'">
            ...
            <div x-show="authed && !activityLoading && !activityRepos.length"
                 class="rounded-xl bg-base-200/40 p-4 text-sm text-base-content/60 max-w-lg">
              No activity cached yet. It builds on the throttled crawl (~12h), or now via Refresh.
            </div>

            <div x-show="authed && activityRepos.length" class="grid gap-5 lg:grid-cols-3 items-start">
              <!-- Per-repo rollups: the summary, first on mobile and 2/3 on
                   desktop. Soft filled panels (no borders); filled chips carry
                   the counts, each row deep-links into that repo's branch review. -->
              <div class="lg:col-span-2 grid gap-3 sm:grid-cols-2">
                <template x-for="r in activityRepos" :key="r.repo">
                  <div class="rounded-xl bg-base-200/40 p-3.5 flex flex-col gap-2 min-w-0">
                    <div class="flex items-center gap-1.5 min-w-0">
                      <button @click="openRepoBranches(r.repo)"
                              class="font-mono text-sm font-semibold truncate hover:text-primary transition-colors text-left min-w-0"
                              x-text="repoShort(r.repo)"></button>
                      <div class="grow"></div>
                      <span x-show="r.pushedAt" class="text-[10px] text-base-content/40 shrink-0" x-text="agoOf(r.pushedAt)"></span>
                      <a :href="'https://github.com/'+r.repo" target="_blank" @click.stop
                         class="text-base-content/25 hover:text-base-content/60 transition-colors shrink-0" title="Open on GitHub">
                        <i class="ph ph-github-logo text-sm leading-none"></i></a>
                    </div>

                    <div class="flex flex-wrap items-center gap-1 text-[11px] font-mono">
                      <button @click="openRepoBranches(r.repo)"
                              class="inline-flex items-center gap-1 rounded-md bg-base-100 px-1.5 py-0.5 hover:text-primary transition-colors"
                              title="Branch review here">
                        <i class="ph ph-git-branch text-[11px]"></i><span x-text="r.counts?.branches || 0"></span></button>
                      <span x-show="r.counts?.active" class="inline-flex items-center gap-1 rounded-md bg-base-100 px-1.5 py-0.5 text-success" title="Active in the last 14 days">
                        <i class="ph ph-pulse text-[11px]"></i><span x-text="r.counts.active"></span></span>
                      <span x-show="r.counts?.landed" class="inline-flex items-center gap-1 rounded-md bg-base-100 px-1.5 py-0.5 text-base-content/50" title="Landed: older, content on the default branch">
                        <i class="ph ph-check-circle text-[11px]"></i><span x-text="r.counts.landed"></span></span>
                      <span x-show="r.counts?.stranded" class="inline-flex items-center gap-1 rounded-md bg-warning/10 px-1.5 py-0.5 text-warning" title="Stranded: content nowhere on the default branch">
                        <i class="ph ph-warning-circle text-[11px]"></i><span x-text="r.counts.stranded"></span></span>
                      <span x-show="r.counts?.openPRs" class="inline-flex items-center gap-1 rounded-md bg-primary/10 px-1.5 py-0.5 text-primary" title="Open pull requests">
                        <i class="ph ph-git-pull-request text-[11px]"></i><span x-text="r.counts.openPRs"></span></span>
                    </div>

                    <!-- Open PR peek (up to three). -->
                    <div x-show="r.openPRs && r.openPRs.length" class="flex flex-col gap-0.5">
                      <template x-for="p in (r.openPRs || []).slice(0, 3)" :key="p.number">
                        <a :href="'https://github.com/'+r.repo+'/pull/'+p.number" target="_blank"
                           class="text-[11px] text-base-content/55 hover:text-primary truncate flex items-center gap-1 min-w-0">
                          <i class="ph ph-git-pull-request text-[10px] shrink-0" :class="p.draft ? 'text-base-content/30' : 'text-primary/60'"></i>
                          <span class="font-mono text-base-content/30 shrink-0" x-text="'#'+p.number"></span>
              ...
              <!-- Recent commits: a compact, height-capped feed. Soft fill, no
                   borders; each row a hover target linking the commit. -->
              <div class="lg:col-span-1 min-w-0">
                <div class="text-[10px] font-mono uppercase tracking-widest text-base-content/40 mb-2 px-1">Recent commits</div>
                <div class="rounded-xl bg-base-200/40 p-1.5 max-h-80 lg:max-h-[34rem] overflow-y-auto flex flex-col gap-0.5">
                  <template x-for="c in activityStream.slice(0, 20)" :key="c.repo + c.sha">
                    <a :href="commitUrl(c.repo, c.sha)" target="_blank"
                       class="block rounded-lg px-2 py-1.5 hover:bg-base-100 transition-colors min-w-0">
                      <div class="text-xs truncate" :title="c.msg" x-text="c.msg"></div>
                      <div class="flex items-center gap-1.5 text-[10px] text-base-content/40 mt-0.5 min-w-0">
                        <span class="font-mono truncate" x-text="repoShort(c.repo)"></span>
                        <span class="shrink-0">·</span><span class="shrink-0" x-text="agoOf(c.date)"></span>
                      </div>
                    </a>
                  </template>
                </div>
