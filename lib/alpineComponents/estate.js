...
${opts.nested ? '' : `
    ...
    return {
          ...
          <div x-show="isActivityTab"
            ...
            <div class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 w-fit flex-wrap" role="tablist">
              <button role="tab" @click="goSub('sessions')"
                      class="flex items-center gap-1.5 px-2 md:px-3 py-1.5 rounded-md text-base font-medium transition-colors"
                      :class="tab === 'sessions' ? 'bg-base-100 text-primary shadow-sm' : 'text-base-content/60 hover:text-base-content'">
                <i class="ph ph-terminal-window text-lg hidden md:inline-block"></i>Sessions<span x-show="activityCounts" class="font-mono text-sm opacity-60"
              ...
              <button role="tab" @click="goSub('branches')"
                ...
                <i class="ph ph-git-branch text-lg hidden md:inline-block"></i>Branches<span x-show="activityCounts" class="font-mono text-sm opacity-60"
              ...
              <button role="tab" @click="goSub('writes')"
                ...
                <i class="ph ph-pen-nib text-lg hidden md:inline-block"></i>Writes<span x-show="activityCounts" class="font-mono text-sm opacity-60"
              ...
              <button role="tab" @click="goSub('chats')"
                ...
                <i class="ph ph-chats text-lg hidden md:inline-block"></i>Chats<span x-show="activityCounts" class="font-mono text-sm opacity-60"
            ...
            <!-- ── The Activity query ──────────────────────────────────────
                 ...
                 and Recent on Branches, say how far back to browse; a typed
                 word is a lookup, and a lookup that stops at 24 hours finds
                 last Tuesday's session only as a digit on the Week chip. So a
                 query of three characters or more moves a windowed scope to
                 All, and shortening or clearing it puts the window back, unless
                 the reader picked another scope meanwhile. The chips still
                 recount under the query.
            ...
            <label class="input input-sm flex items-center gap-1.5 grow basis-40 min-w-0 max-w-md">
              ...
              <input type="search" x-model.debounce.150ms="activityQuery" data-find-box
                     placeholder="Search activity…" class="grow min-w-0 text-base"
      ...
      set activityQuery(v){
        const L = this.ACTIVITY_LIFT_AT;
        const was = String(this.activityQuery_ || '').trim().length >= L;
        const now = String(v || '').trim().length >= L;
        this.activityQuery_ = v;
        if (!was && now) this.liftActivityWindows();
        else if (was && !now) this.restoreActivityWindows();
      },
      // HOW LONG A QUERY MUST BE BEFORE IT LIFTS. One letter matched 760 of
      // 889 sessions and 1,123 branches on the live store, and the lift then
      // drew every one of them: an unpaged list of that size is a frozen tab,
      // which is how the first build of this shipped. Below three characters a
      // query still filters, inside the window already lit; at three it is a
      // lookup and reaches every date.
      ACTIVITY_LIFT_AT: 3,
      ...
      // rail and swipe deck still reads the whole list.
      LIST_PAGE: 60,
      sessionShowN: 60,
      branchShowN: 60,
