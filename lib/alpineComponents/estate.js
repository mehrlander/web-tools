...
${opts.nested ? '' : `
    ...
    return {
        ...
        <div :class="tab==='lists' && 'flex-1 min-h-0 flex flex-col'">
          ...
          <div x-show="isActivityTab"
            ...
            <div class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 w-fit flex-wrap" role="tablist">
              ...
              <button role="tab" @click="goSub('writes')"
                ...
                <i class="ph ph-pen-nib text-lg"></i>Writes</button>
          ...
          <div class="flex-1 min-w-0" :class="tab==='sessions' ? '' : 'hidden'"
            ...
            <div x-show="authed" class="flex items-center gap-2 mb-3 flex-wrap">
              ...
              <!-- ── The text box ───────────────────────────────────────────
                   BESIDE THE LENS, not on a row of its own: it narrows the two
                   row lenses and the chip rows below it recount against what it
                   leaves, so it is the outermost filter and belongs at the top
                   of the stack. It wraps to its own line on a phone, where the
                   lens tabs already take the width.

                   ONE BOX FOR BOTH ROW LENSES. The Table lens carried a global
                   "Filter any column" input of its own until this landed, which
                   meant the same question was asked in two places and answered
                   differently: that box matched the VISIBLE columns, so hiding
                   a column changed what a search could find. This one matches a
                   fixed set of fields (RepoSessionsCache.searchSegs) whatever
                   the table is showing, and the per-column header filters stay
                   exactly as they were, since they are how a precise question
                   is asked and this is how a word is found.
                   ...
                   THE PLACEHOLDER NAMES THE FIELDS, which is the one place that
                   fact can go. "Filter these sessions" said what the control
                   does and not what it reads, so a reader could not tell a miss
                   on the title from a miss on the whole row, and the house rule
                   forbids a line of prose above the box saying so. Four fields
                   and an ellipsis: the rest are in the tooltip, and the whole
                   list is RepoSessionsCache.searchSegs. That tooltip is a list
                   and not a sentence, because the CONTRAST it used to spell out
                   ("not what was said later") is carried by the button's own
                   label now, which is what relabelling it bought.

                   AND THE COUNT NAMES THE DENOMINATOR. "12 of 372" is what
                   answers the other half of the question, whether the query ran
                   inside the open scope or across the store: it ran across the
                   store, and the chips below then narrow what it left. The List
                   lens is the one row surface with no total of its own (the
                   Table has tableCount, the Chats pane has its months line), so
                   this fills a gap rather than repeating a number. Only while a
                   query stands, since a count against no filter is furniture. -->
              <label class="input input-sm flex items-center gap-1.5 grow min-w-40 max-w-sm"
                     x-show="sessionRowsLens"
                     data-note="Title, session name, opening ask, attached filenames, and the work: repos, branches, files, docs, skills, tools.">
                <i class="ph ph-magnifying-glass text-base-content/40"></i>
                <input type="search" x-model.debounce.150ms="sessionQuery" data-find-box
                       placeholder="Filter by title, ask, repo, file…" class="grow text-base"
                       aria-label="Filter sessions">
              </label>
              <span x-show="sessionRowsLens && sessionQuery.trim()"
                    class="shrink-0 font-mono text-sm text-base-content/50 tabular-nums"
                    x-text="sessionQueryCount"></span>
              <button x-show="sessionRowsLens && sessionQuery.trim()" @click="openSessionGrep()"
            ...
            <div x-show="authed && sessionLens === 'list' && !sessionsLoading && !sessionNodes.length"
              ...
              <span x-show="sessionQuery.trim() && allSessionRows.length"
                    x-text="' No session here matches ' + JSON.stringify(sessionQuery.trim())
          ...
          <div class="flex-1 min-w-0" :class="tab==='branches' ? '' : 'hidden'"
            ...
            <div x-show="authed" class="flex items-center gap-2 mb-2">
              ...
              <div class="ml-auto shrink-0 flex items-center gap-2">
                <button @click="toggleGraphQL()"
                        class="flex items-center gap-1.5 text-base px-2 py-1 rounded-lg transition-colors"
                        :class="showGraphQL ? 'bg-primary/10 text-primary font-medium' : 'text-base-content/50 hover:text-primary hover:bg-base-200'"
                        title="Inspect the GraphQL query and response used to crawl branches and sessions">
            ...
            <!-- ── GraphQL Inspector (Activity Crawl Transparency) ────────── -->
            <div x-show="authed && showGraphQL" x-cloak
                 class="rounded-xl border border-base-300 bg-base-100 shadow-sm p-4 mb-4">
              <!-- Header: title, repo selector, actions -->
              <div class="flex items-center justify-between flex-wrap gap-2 pb-3 border-b border-base-200">
                <div class="flex items-center gap-2">
                  ...
                  <span class="font-semibold text-base">GraphQL Activity Discovery</span>
                  <span class="badge badge-sm badge-ghost font-mono text-xs">BranchesDatedSessions</span>
                </div>
                <div class="flex items-center gap-2">
                  <label class="text-xs text-base-content/60">Repo:</label>
                  <select x-model="graphQLRepo" @change="fetchActivityGraphQL(graphQLRepo, true)"
                          class="select select-bordered select-xs font-mono">
                    <template x-for="r in graphQLReposList" :key="r">
                      <option :value="r" x-text="r" :selected="r === graphQLRepo"></option>
                    </template>
                  </select>
                  <button @click="fetchActivityGraphQL(graphQLRepo, true)" :disabled="graphQLLoading"
                          class="btn btn-xs btn-outline gap-1" title="Send live GraphQL query to GitHub">
                    <i class="ph" :class="graphQLLoading ? 'ph-circle-notch animate-spin' : 'ph-arrow-clockwise'"></i>
                    <span x-text="graphQLLoading ? 'Querying…' : 'Fetch live'"></span>
                  </button>
                  <button @click="showGraphQL = false" class="btn btn-xs btn-ghost btn-square" title="Close inspector">
                    <i class="ph ph-x text-sm"></i>
                  </button>
                </div>
              </div>

              <!-- Transparency explanation note -->
              <div class="text-xs text-base-content/70 bg-base-200/50 rounded-lg p-2.5 my-3 flex items-start gap-2">
                <i class="ph ph-info text-base text-primary shrink-0 mt-0.5"></i>
                <div class="leading-relaxed">
                  <span>This GraphQL query powers the branch update process. In one round trip, it retrieves branches ordered chronologically by commit date (<code>TAG_COMMIT_DATE</code>) and inspects recent commit history (<code>depth: 8</code>) to extract <code>Claude-Session: &lt;url&gt;</code> git trailers. This eliminates N+1 API calls and avoids missing recent branches.</span>
                </div>
              </div>

              <!-- Error message if any -->
              <div x-show="graphQLErr" class="rounded-lg bg-error/10 border border-error/30 text-error p-2.5 mb-3 text-xs font-mono"
                   x-text="graphQLErr"></div>

              <!-- Metrics bar -->
              <div x-show="currentGraphQL" class="flex items-center flex-wrap gap-x-4 gap-y-1 text-xs text-base-content/60 mb-3 font-mono">
                <span>Queried: <span class="text-base-content font-medium" x-text="currentGraphQL?.at ? agoShort(Date.parse(currentGraphQL.at)) + ' ago' : 'live'"></span></span>
                <span x-show="currentGraphQL?.duration != null">Duration: <span class="text-base-content font-medium" x-text="(currentGraphQL?.duration != null ? currentGraphQL.duration + 'ms' : '')"></span></span>
              ...
              <!-- Sub-tabs -->
              <div class="flex items-center justify-between border-b border-base-200 mb-3">
                <div class="flex gap-1">
                  <button @click="graphQLTab = 'response'"
                          class="px-3 py-1 text-xs font-medium border-b-2 transition-colors"
                          :class="graphQLTab === 'response' ? 'border-primary text-primary' : 'border-transparent text-base-content/60 hover:text-base-content'">
                    Response JSON
                  </button>
                  <button @click="graphQLTab = 'extracted'"
                          class="px-3 py-1 text-xs font-medium border-b-2 transition-colors"
                          :class="graphQLTab === 'extracted' ? 'border-primary text-primary' : 'border-transparent text-base-content/60 hover:text-base-content'">
                  ...
                  <button @click="graphQLTab = 'query'"
                          class="px-3 py-1 text-xs font-medium border-b-2 transition-colors"
                          :class="graphQLTab === 'query' ? 'border-primary text-primary' : 'border-transparent text-base-content/60 hover:text-base-content'">
                    Query & Variables
                  </button>
                </div>
                <div>
              ...
              <div x-show="graphQLTab === 'response'">
                <div x-show="!currentGraphQL && !graphQLLoading" class="text-xs text-base-content/50 py-6 text-center">
                  No GraphQL response recorded yet. Click "Fetch live" above to run the query.
                </div>
                ...
                <pre x-show="currentGraphQL && !graphQLLoading"
                     class="bg-base-200/80 rounded-lg p-3 text-xs font-mono max-h-72 overflow-y-auto overflow-x-auto text-base-content/80 select-all"
              ...
              <div x-show="graphQLTab === 'extracted'" class="overflow-x-auto max-h-72 overflow-y-auto">
                <table class="table table-xs w-full">
                  <thead>
                    <tr class="text-base-content/60 border-b border-base-200">
                      <th>Branch</th>
                      <th>Committed</th>
                      <th>Headline</th>
                  ...
                  <tbody>
                    <template x-for="b in (currentGraphQL?.branches || [])" :key="b.name">
                      <tr class="border-b border-base-200/40 hover:bg-base-200/30 font-mono text-xs">
                        ...
                        <td class="whitespace-nowrap text-base-content/60" x-text="b.ago || b.date"></td>
                        <td class="truncate max-w-xs text-base-content/80 font-sans" :title="b.subject" x-text="b.subject"></td>
                        <td class="whitespace-nowrap">
                          <template x-if="currentGraphQL?.sessions?.[b.name]">
                            <a :href="currentGraphQL.sessions[b.name]" target="_blank" rel="noopener"
                               class="text-xs text-secondary hover:underline flex items-center gap-1"
                               :title="currentGraphQL.sessions[b.name]">
                              <i class="ph ph-arrow-square-out"></i>
                              <span x-text="currentGraphQL.sessions[b.name].split('/').pop()"></span>
                            </a>
                          </template>
                          <template x-if="!currentGraphQL?.sessions?.[b.name]">
              ...
              <!-- Tab 3: GraphQL Query & Variables -->
              <div x-show="graphQLTab === 'query'" class="space-y-2">
                <div>
                  <div class="text-xs font-semibold text-base-content/70 mb-1">Query Document</div>
                  <pre class="bg-base-200/80 rounded-lg p-3 text-xs font-mono max-h-48 overflow-y-auto overflow-x-auto text-base-content/80 select-all"
                ...
                <div>
                  ...
                  <pre class="bg-base-200/80 rounded-lg p-3 text-xs font-mono max-h-32 overflow-y-auto overflow-x-auto text-base-content/80 select-all"
                       x-text="currentGraphQL?.variables ? JSON.stringify(currentGraphQL.variables, null, 2) : '{}'"></pre>
                </div>
              </div>
            </div>
          ...
          <div class="flex-1 min-w-0" :class="tab==='chats' ? '' : 'hidden'"
            ...
            <!-- ── The text box ─────────────────────────────────────────────
                 IT FILTERS THE LOADED MONTHS AND NOTHING ELSE, which is the
                 same bargain the tag chips already make one block down and the
                 reason the button beside it exists. This pane pages one month
                 at a time on purpose (two small requests a tap), so a box that
                 quietly read all 41 shards would undo the pane's whole economy
                 the first time anyone typed in it. Searching the archive is the
                 Files view's Chats lane, which reads every month and says how
                 far along it is; this is the cheap narrowing of what is already
                 in hand.
            ...
            <div x-show="authed && chatRows.length" class="flex items-center gap-2 mb-2 flex-wrap">
              <label class="input input-sm flex items-center gap-1.5 grow min-w-40 max-w-sm"
                     data-note="Title, tags and summary, across the months loaded below.">
                <i class="ph ph-magnifying-glass text-base-content/40"></i>
                <input type="search" x-model.debounce.150ms="chatQuery" data-find-box
                       placeholder="Filter by title, tag, summary…" class="grow text-base"
            ...
            <div x-show="authed && chatMonths.length" class="mt-4 flex items-center gap-3">
              ...
              <button x-show="chatFiltered" @click="chatProvider = ''; chatTag = ''; chatHandOnly = false; chatQuery = ''"
            ...
            <div x-show="authed">
              ...
              <div class="flex flex-wrap items-center gap-1.5 mb-3">
                <button @click="writeKind = ''"
                  ...
                  All <span class="font-mono text-xs" x-text="writeRows.length"></span>
                </button>
      ...
      sessionRepoFilter: '',
      sessionStateFilter: '',
      // The text box, held HERE and not in the address, which is the same call
      // the repo and state chips make one line up. The lens is in the URL
      // because a render link had to be able to name it; a filter is what a
      // reader is doing right now, and stamping a half-typed word into history
      // would push an entry per keystroke. Change this if a query ever needs to
      // be handed over, and change the chips with it rather than on its own.
      sessionQuery: '',
      openSessionId: '',
      sessionDetail: null,
      ...
      get writeKindsWithCount(){
        const K = window.WriteKinds;
        if (!K) return [];
        const t = K.tally(this.writeRows);
      ...
      // The text box, over the LOADED months only. Held here rather than in the
      // address for the same reason sessionQuery is: it is what a reader is
      // doing now, and the chips beside it are not linkable either.
      chatQuery: '',
      ...
      get chatFiltered(){
        return !!(this.chatProvider || this.chatTag || this.chatHandOnly || this.chatQuery.trim());
      },
      ...
      openChatSearch(){
        window.__shell?.goSearch?.({ q: this.chatQuery.trim(), mode: 'chats' });
      },
      ...
      get sessionRailAll(){
        ...
        const key = [sp.a, sp.b, this.allSessionRows.length, this.sessionsGeneratedAt,
                     this.sessionQuery, this.sessionRepoFilter, this.sessionStateFilter,
      ...
      railWhen(at){
        ...
        return this.sessionRailSpan.days === 1 ? clock
      ...
      get queriedSessions(){
        const q = this.sessionQuery.trim();
        const match = window.RepoSessionsCache?.matches;
        if (!q || !match) return this.allSessionRows;
        return this.allSessionRows.filter(r => match(r, q));
      },
      ...
      nodeMatchesQuery(n){
        ...
        if (n.row && match(n.row, q)) return true;
      ...
      get sessionQueryCount(){
        if (!this.sessionQuery.trim()) return '';
      ...
      toggleGraphQL(){
        ...
        if (this.showGraphQL) {
          if (!this.graphQLRepo) {
            this.graphQLRepo = this.activeRepoFilter || this.openRepos?.[0]?.repo || 'mehrlander/web-tools';
          }
          if (!this.currentGraphQL && this.authed && !this.graphQLLoading) {
            this.fetchActivityGraphQL(this.graphQLRepo);
          }
        }
      },
      get currentGraphQL(){
        const repo = this.graphQLRepo || this.activeRepoFilter || this.openRepos?.[0]?.repo || 'mehrlander/web-tools';
        if (typeof window !== 'undefined') {
          return window.__graphQLByRepo?.[repo] || (window.__lastActivityGraphQL?.repo === repo ? window.__lastActivityGraphQL : null) || window.__lastActivityGraphQL || null;
        }
        return null;
      },
      async fetchActivityGraphQL(repo, force = false){
        if (!repo) repo = this.graphQLRepo || this.activeRepoFilter || 'mehrlander/web-tools';
        this.graphQLRepo = repo;
        this.graphQLErr = '';
        if (!force && window.__graphQLByRepo?.[repo]) return;
        if (!this.authed) {
          this.graphQLErr = 'Authentication required: Add a GitHub token on Repos to query GraphQL.';
          return;
        }
      ...
      get graphQLReposList(){
        const set = new Set();
        if (this.REGISTRY_REPO) set.add(this.REGISTRY_REPO);
        set.add('mehrlander/web-tools');
        for (const r of (this.openRepos || [])) if (r.repo) set.add(r.repo);
        if (typeof window !== 'undefined' && window.__graphQLByRepo) {
          for (const r of Object.keys(window.__graphQLByRepo)) set.add(r);
        }
        return [...set];
      },
      ...
      get sessionNodes(){
        ...
        const nodes = this.sessionTree.nodes.filter(n =>
      ...
      // text search at all. The pane's own box (sessionQuery) narrows the rows
