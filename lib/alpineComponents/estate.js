    ...
    return {
      ...
      template: `
        <div :class="tab==='lists' && 'flex-1 min-h-0 flex flex-col'">
          ...
          <!-- ── The Activity query ────────────────────────────────────────
               ONE BOX FOR EVERY PANE, above the pills rather than inside any
               one of them. Sessions and Chats each carried a box of their own
               until 2026-09-24 and Branches and Writes carried none, so a
               branch name typed on Sessions found the sessions that left it
               and never the branch. The work the four panes describe is one
               body of work seen four ways, and a word a reader remembers
               belongs to that body, not to whichever pane happened to be open.

               It stays put across a tab switch, each pane applies it to its
               own rows, and each pill counts what it matched. Each pane's
               deeper pass (every stored reply, every archive month) stays in
               that pane, beside the rows it would extend.

               A QUERY LIFTS THE TIME WINDOW. Day, Week and Month on Sessions,
               and Recent on Branches, are how far back to browse; a typed word
               is a lookup, and a lookup that stops at 24 hours finds last
               Tuesday's session only as a digit on the Week chip. So the first
               character moves a windowed scope to All, and clearing the box
               puts it back, unless the reader picked another scope meanwhile.
               The chips still recount under the query, so they still say where
               in time the matches sit. -->
          <div x-show="isActivityTab" class="mb-3">
            <label class="input input-sm flex items-center gap-1.5 w-full max-w-md"
                   data-note="Sessions: title, ask, repos, branches, files. Branches: repo, name, PR number and title. Writes: commit message, repo, SHA. Chats: title, tags and summary, in the loaded months.">
              ...
              <input type="search" x-model.debounce.150ms="activityQuery" data-find-box
                     placeholder="Search activity: branch, title, repo, file…" class="grow text-base"
          ...
          <div x-show="isActivityTab"
            ...
            <div class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 w-fit flex-wrap" role="tablist">
              <button role="tab" @click="goSub('sessions')"
                      class="flex items-center gap-1.5 px-2 sm:px-3 py-1.5 rounded-md text-base font-medium transition-colors"
                      :class="tab === 'sessions' ? 'bg-base-100 text-primary shadow-sm' : 'text-base-content/60 hover:text-base-content'">
                <i class="ph ph-terminal-window text-lg hidden sm:inline-block"></i>Sessions<span x-show="activityCounts" class="font-mono text-sm opacity-60"
              ...
              <button role="tab" @click="goSub('branches')"
                ...
                <i class="ph ph-git-branch text-lg hidden sm:inline-block"></i>Branches<span x-show="activityCounts" class="font-mono text-sm opacity-60"
              ...
              <button role="tab" @click="goSub('writes')"
                ...
                <i class="ph ph-pen-nib text-lg hidden sm:inline-block"></i>Writes<span x-show="activityCounts" class="font-mono text-sm opacity-60"
              ...
              <button role="tab" @click="goSub('chats')"
                ...
                <i class="ph ph-chats text-lg hidden sm:inline-block"></i>Chats<span x-show="activityCounts" class="font-mono text-sm opacity-60"
            ...
            <!-- State: the caches behind every other pill in this row. It sits
                 ...
                 past the spacer at the row's right end, in a group of its own,
                 ...
                 than a sibling set aside. So below sm the pills drop their
                 icons and some padding, and State keeps only its icon, which
                 puts all five on the line with State still at its right end. -->
            <div class="flex items-center rounded-lg bg-base-200/60 p-0.5 w-fit" role="tablist">
      ...
      //
      // A SETTER, so the window lift below happens in the same step as the
      // keystroke that caused it. A watcher would run a tick later, and a scope
      // read between the two would answer for the old window.
      activityQuery_: '',
      get activityQuery(){ return this.activityQuery_; },
      set activityQuery(v){
        const was = !!String(this.activityQuery_ || '').trim();
        const now = !!String(v || '').trim();
        this.activityQuery_ = v;
        if (!was && now) this.liftActivityWindows();
        else if (was && !now) this.restoreActivityWindows();
      },
      // Which scopes are WINDOWS rather than categories. A window says how far
      // back to browse, and a lookup has no business stopping there; Snagged,
      // Open, Stranded and the rest say what kind of row, which a query does
      // not overrule.
      ACTIVITY_WINDOWED: { session: ['day', 'week', 'month'], branch: ['active'] },
      // What the lift replaced, so clearing the box can put it back. Null when
      // no lift stands.
      activityLift: null,
      liftActivityWindows(){
        const W = this.ACTIVITY_WINDOWED, lift = {};
        if (W.session.includes(this.sessionScope)) { lift.session = this.sessionScope; this.sessionScope = 'all'; }
        if (W.branch.includes(this.branchScope)) { lift.branch = this.branchScope; this.branchScope = 'all'; }
        this.activityLift = (lift.session || lift.branch) ? lift : null;
      },
      // Restores only a scope that is still the one the lift set. A reader who
      // tapped Week while the query stood chose Week, and clearing the box is
      // not a reason to take that choice back.
      restoreActivityWindows(){
        const lift = this.activityLift;
        this.activityLift = null;
        if (!lift) return;
        if (lift.session && this.sessionScope === 'all') this.sessionScope = lift.session;
        if (lift.branch && this.branchScope === 'all') this.branchScope = lift.branch;
      },
