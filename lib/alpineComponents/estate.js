          ...
          <div x-show="tab==='activity'">
            <div class="flex items-center gap-2 mb-3" x-show="authed">
              ...
              <div class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 lg:hidden" x-show="activityRepos.length">
                <button @click="actSub='repos'" class="px-2.5 py-1 rounded-md text-xs font-medium transition-colors"
                        :class="actSub==='repos' ? 'bg-base-100 text-primary shadow-sm' : 'text-base-content/60'">Repos</button>
                <button @click="actSub='commits'" class="px-2.5 py-1 rounded-md text-xs font-medium transition-colors"
                        :class="actSub==='commits' ? 'bg-base-100 text-primary shadow-sm' : 'text-base-content/60'">Commits</button>
              </div>
              <span x-show="activityGeneratedAt" class="hidden sm:inline text-xs text-base-content/40" x-text="'as of ' + agoOf(activityGeneratedAt)"></span>
              <div class="grow"></div>
              <button @click="refreshActivity()" :disabled="activityBusy"
                      class="flex items-center gap-1.5 text-xs text-base-content/60 hover:text-primary px-2 py-1 rounded-lg hover:bg-base-200 transition-colors">
            ...
            <div x-show="authed && activityRepos.length" class="grid gap-4 lg:grid-cols-3 items-start">
                    ...
                    <div class="rounded-lg px-2 py-1.5 hover:bg-base-100 transition-colors min-w-0">
                      <div class="flex items-center gap-2 min-w-0">
                        <button @click="openRepoBranches(r.repo)"
                                class="font-mono text-xs font-semibold truncate hover:text-primary transition-colors text-left min-w-0 flex-1"
                                x-text="repoShort(r.repo)"></button>
                        <div class="flex items-center gap-2 text-[10px] font-mono shrink-0">
                          <button @click="openRepoBranches(r.repo)" class="flex items-center gap-0.5 text-base-content/50 hover:text-primary transition-colors" title="Branches (open the branch review)">
                        ...
                        <span x-show="r.pushedAt" class="text-[10px] text-base-content/35 shrink-0 w-9 text-right tabular-nums"
                              :title="'pushed ' + agoOf(r.pushedAt)" x-text="agoShort(r.pushedAt)"></span>
                      </div>
                      <!-- Open-PR peek: the newest, one line, with a +N tail. -->
                      <div x-show="r.openPRs && r.openPRs.length" class="flex items-center gap-1 mt-0.5 pl-0.5 text-[10px] min-w-0">
                        <i class="ph ph-git-pull-request text-[10px] text-primary/50 shrink-0"></i>
                        <a :href="'https://github.com/'+r.repo+'/pull/'+(r.openPRs?.[0]?.number)" target="_blank"
                           class="text-base-content/50 hover:text-primary truncate min-w-0" x-text="r.openPRs?.[0]?.title"></a>
                        <span x-show="r.openPRs.length > 1" class="text-base-content/30 shrink-0" x-text="'+' + (r.openPRs.length - 1)"></span>
                      </div>
                    </div>
                  </template>
                </div>
              ...
              <div class="lg:col-span-1 min-w-0" :class="actSub!=='commits' && 'hidden lg:block'">
                <div class="hidden lg:block text-[10px] font-mono uppercase tracking-widest text-base-content/40 mb-2 px-1">Recent commits</div>
                  ...
                  <template x-for="c in activityStream.slice(0, 25)" :key="c.repo + c.sha">
                    <a :href="commitUrl(c.repo, c.sha)" target="_blank"
                      ...
                      <div class="flex items-baseline gap-1.5 min-w-0">
                        <span class="text-xs truncate flex-1" :title="c.msg" x-text="c.msg"></span>
                        <span class="text-[10px] text-base-content/35 shrink-0 tabular-nums" x-text="agoShort(c.date)"></span>
                      </div>
                      <span class="text-[10px] font-mono text-base-content/40 truncate block" x-text="repoShort(c.repo)"></span>
                    </a>
                  </template>
                </div>
              </div>
