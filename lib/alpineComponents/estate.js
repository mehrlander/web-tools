...
${opts.nested ? '' : `
    ...
    return {
            ...
            <div x-show="authed && sessionLens === 'list' && sessionNodes.length && sessionRailSpan.ms"
              ...
              <div class="relative h-3 mt-1" tabindex="0" role="group"
                ...
                <template x-for="m in sessionRailAll" :key="m.k">
                  <div class="absolute w-[3px] -ml-px rounded-[1px] pointer-events-none transition-colors"
                       :class="railHover && railHover.k === m.k
                               ? 'bg-primary -inset-y-[2px]' : 'bg-primary/60 inset-y-0'"
                       :style="'left:' + m.p + '%'"></div>
                </template>
                  ...
                  <div x-show="sessionRailSpan.ms && row.started" class="mt-2 -mb-0.5">
                    <div class="relative h-3" :data-note="sessionRailNote(row)">
                      ...
                      <template x-for="(p, i) in sessionRailTicks(row)" :key="i">
                        <div class="absolute top-1/2 w-px h-2 -translate-y-1/2 bg-primary/70"
                             :style="'left:' + p + '%'"></div>
                      </template>
                    </div>
                  </div>
      ...
      get sessionRailAll(){
        ...
        for (const n of this.sessionNodes){
          ...
          for (const m of r.beats){
            ...
            const k = Math.min(RAIL_BINS - 1, Math.floor((p / 100) * RAIL_BINS));
            let b = bins.get(k);
      ...
      // Which mark the pointer is resolved to, or null. Reactive, because three
      // things render from it: the mark's own emphasis, the cursor the note
      // anchors to, and what a tap or an Enter jumps to.
      railHover: null,
      ...
      railTrack(ev){
        if (!ev){ this.railHover = null; window.Note?.close?.(); return; }
        const marks = this.sessionRailAll;
        const box = ev.currentTarget?.getBoundingClientRect();
        if (!marks.length || !box?.width) return;
      ...
      // Settle on one mark and put the note over it. The cursor has to take the
      // new text BEFORE the panel opens, or the panel is positioned for the new
      // mark and reads the old one's note; hence the tick.
      railAt(m){
        this.railHover = m;
        this.$nextTick(() => {
