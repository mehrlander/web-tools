document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    return {
        ...
        <div>
          ...
          <div x-show="tab==='repos'">
            ...
            <p class="text-sm text-base-content/60 mb-6">
              ...
              <span x-show="!authed">Signed out: the estate shows only the public default repo, and surfaces and app views stay hidden.</span>
            </p>

            <!-- Activity strip: the landing widget. A few of the freshest commits
                 across the estate, from the private activity cache (no per-visit
                 fanout), with a jump to the full Activity view. -->
            <div x-show="authed && activityStream.length" class="card bg-base-100 border border-base-300 shadow-sm mb-6">
              <div class="card-body p-4 gap-2">
                <div class="flex items-center gap-1.5">
                  <i class="ph ph-pulse text-primary"></i>
                  <span class="text-sm font-semibold">Recent activity</span>
                  <span x-show="activityGeneratedAt" class="text-[10px] text-base-content/40" x-text="'as of ' + agoOf(activityGeneratedAt)"></span>
                  <div class="grow"></div>
                  <button @click="window.__shell?.goActivity()" class="btn btn-ghost btn-xs gap-1 text-primary">
                    All activity<i class="ph ph-arrow-right"></i></button>
                </div>
                <div class="flex flex-col divide-y divide-base-200">
                  <template x-for="c in activityStream.slice(0, 6)" :key="c.repo + c.sha">
                    <div class="flex items-center gap-2 py-1 text-xs">
                      <button @click="openRepoBranches(c.repo)" class="font-mono text-base-content/40 shrink-0 w-28 truncate text-left hover:text-primary" x-text="repoShort(c.repo)"></button>
                      <a :href="commitUrl(c.repo, c.sha)" target="_blank" class="truncate hover:text-primary min-w-0 flex-1" :title="c.msg" x-text="c.msg"></a>
                      <span class="text-[10px] text-base-content/40 shrink-0" x-text="agoOf(c.date)"></span>
                    </div>
                  </template>
                </div>
              </div>
            ...
            <!-- Public (no-token) banner: say exactly what is and isn't available
                 and give the two real next steps, a token or the rate-safe public
                 browser, instead of a vague "set a token" aside. -->
            <div x-show="!authed" class="card bg-base-100 border border-base-300 shadow-sm mb-6">
              <div class="card-body p-4 gap-2">
                <div class="text-sm font-semibold flex items-center gap-1.5">
                  <i class="ph ph-globe text-primary"></i>You are browsing without a token
                </div>
                <div class="text-xs text-base-content/60">
                  A GitHub token unlocks the full estate (private repos, surfaces, app views, config editing)
                  and lifts the 60-request/hour anonymous rate limit. Without one you can still explore any
                  <span class="font-medium">public</span> repo through Public browse, which reads via jsDelivr
                  and never spends that limit.
                </div>
                <div class="flex flex-wrap items-center gap-2 mt-1">
                  <button @click="document.getElementById('repo')?.__repo?.openDialog()"
                          class="btn btn-primary btn-xs gap-1"><i class="ph ph-key"></i>Add a token</button>
                  <button @click="window.__shell?.goPublicBrowse()"
                          class="btn btn-ghost btn-xs gap-1 border border-base-300"><i class="ph ph-cloud-arrow-down"></i>Public browse</button>
                  <a href="https://github.com/settings/tokens/new?scopes=repo&description=web-tools" target="_blank"
                     rel="noopener" class="text-[11px] text-base-content/40 hover:text-primary underline ml-auto flex items-center gap-1">
                      ...
                      <div class="card-body p-4 gap-1.5">
                        <div class="flex items-center gap-1.5">
                          ...
                          <button @click.stop="window.__shell?.goPublicBrowse(face(e).repo)"
                            ...
                            <i class="ph ph-cloud-arrow-down text-sm leading-none"></i></button>
                        ...
                        <template x-if="cardActivity(face(e).repo)">
                          <div class="flex flex-wrap items-center gap-1 mt-0.5">
                            <button @click="openRepoBranches(face(e).repo)"
                              ...
                              <i class="ph ph-git-branch text-[10px]"></i><span x-text="cardActivity(face(e).repo).counts?.branches || 0"></span>
                            </button>
                            <span x-show="cardActivity(face(e).repo).counts?.stranded"
                              ...
                              <i class="ph ph-warning-circle text-[10px]"></i><span x-text="cardActivity(face(e).repo).counts.stranded"></span>
                            </span>
                            <span x-show="cardActivity(face(e).repo).counts?.openPRs"
                              ...
                              <i class="ph ph-git-pull-request text-[10px]"></i><span x-text="cardActivity(face(e).repo).counts.openPRs"></span>
                            </span>
                          </div>
                        </template>
                      </div>
          ...
          <div x-show="tab==='activity'">
            <div class="flex items-center gap-2 mb-1 flex-wrap">
              ...
              <span x-show="activityGeneratedAt" class="text-xs text-base-content/40" x-text="'as of ' + agoOf(activityGeneratedAt)"></span>
              <div class="grow"></div>
            ...
            <p class="text-sm text-base-content/60 mb-6">
              <span x-show="authed">A cross-repo read off the private activity cache: recent commits, branch rollups, and open PRs, without a per-visit API fanout. Refresh re-crawls the estate (a capped branch survey per repo).</span>
              <span x-show="!authed">Signed out: activity lives in the private registry. Add a token via the sidebar shield to see it.</span>
            </p>
            ...
            <div x-show="authed && !activityLoading && !activityRepos.length"
                 class="card bg-base-100 border border-base-300 shadow-sm max-w-lg">
              <div class="card-body p-4 gap-2 text-sm text-base-content/60">
                No activity cached yet. It builds on the throttled crawl (~12h), or now via Refresh.
              </div>
            </div>

            <div x-show="authed && activityRepos.length" class="grid gap-6 lg:grid-cols-3">
              <!-- Recent-commit stream -->
              <div class="lg:col-span-1">
                <h2 class="text-xs font-mono uppercase tracking-widest text-base-content/40 mb-3 flex items-center gap-2">
                  <i class="ph ph-git-commit"></i>Recent commits
                </h2>
                <div class="flex flex-col divide-y divide-base-200 border border-base-300 rounded-lg bg-base-100 max-h-[75vh] overflow-y-auto">
                  <template x-for="c in activityStream" :key="c.repo + c.sha">
                    <div class="p-2 text-xs">
                      <a :href="commitUrl(c.repo, c.sha)" target="_blank" class="hover:text-primary block truncate" :title="c.msg" x-text="c.msg"></a>
                      <div class="flex items-center gap-1.5 text-[10px] text-base-content/40 mt-0.5">
                        <button @click="openRepoBranches(c.repo)" class="font-mono hover:text-primary" x-text="repoShort(c.repo)"></button>
                        <span>·</span><span x-text="agoOf(c.date)"></span>
                        <span x-show="c.author" class="truncate">· <span x-text="c.author"></span></span>
                      </div>
                    </div>
                  </template>
                </div>
                  ...
                  <div class="card bg-base-100 border border-base-300 shadow-sm">
                    <div class="card-body p-4 gap-2">
                      <div class="flex items-center gap-1.5 flex-wrap">
                        <button @click="openRepoBranches(r.repo)" class="font-mono text-sm font-semibold hover:text-primary text-left truncate" x-text="repoShort(r.repo)"></button>
                        <span x-show="r.pushedAt" class="text-[10px] text-base-content/40 flex items-center gap-1">
                          <i class="ph ph-clock"></i><span x-text="'pushed ' + agoOf(r.pushedAt)"></span></span>
                        <div class="grow"></div>
                        <a :href="'https://github.com/'+r.repo+'/branches'" target="_blank" class="text-base-content/30 hover:text-base-content/70 transition-colors" title="Branches on GitHub"><i class="ph ph-git-branch"></i></a>
                        <a :href="'https://github.com/'+r.repo+'/pulls'" target="_blank" class="text-base-content/30 hover:text-base-content/70 transition-colors" title="Pull requests on GitHub"><i class="ph ph-git-pull-request"></i></a>
                        <a :href="'https://github.com/'+r.repo" target="_blank" class="text-base-content/30 hover:text-base-content/70 transition-colors" title="Open on GitHub"><i class="ph ph-github-logo"></i></a>
                      </div>
                      ...
                      <div class="flex flex-wrap items-center gap-1.5 text-xs">
                        <button @click="openRepoBranches(r.repo)"
                                ...
                                title="Branch review here">
                          <i class="ph ph-git-branch"></i><span x-text="(r.counts?.branches || 0) + ' branches'"></span></button>
                        <span x-show="r.counts?.active" class="badge badge-ghost gap-1 text-success" title="Active in the last 14 days">
                          <i class="ph ph-pulse"></i><span x-text="r.counts.active"></span></span>
                        <span x-show="r.counts?.landed" class="badge badge-ghost gap-1" title="Older branches whose content is on the default branch">
                          <i class="ph ph-check-circle"></i><span x-text="r.counts.landed"></span></span>
                        <span x-show="r.counts?.stranded" class="badge badge-ghost gap-1 text-warning" title="Older branches with content nowhere on the default branch">
                          <i class="ph ph-warning-circle"></i><span x-text="r.counts.stranded"></span></span>
                        <span x-show="r.counts?.openPRs" class="badge badge-ghost gap-1 text-primary" title="Open pull requests">
                          <i class="ph ph-git-pull-request"></i><span x-text="r.counts.openPRs"></span></span>
                        <span x-show="r.survey?.truncated" class="badge badge-ghost gap-1 text-warning" title="Default-branch tree truncated by the API; counts may undercount">
                          <i class="ph ph-warning"></i>partial</span>
                        <span x-show="r.survey && r.survey.surveyed < r.survey.older" class="text-[10px] text-base-content/40"
                              :title="'Surveyed ' + r.survey.surveyed + ' of ' + r.survey.older + ' older branches (cap ' + r.survey.cap + ')'"
                              x-text="'+' + (r.survey.older - r.survey.surveyed) + ' unsurveyed'"></span>
                      </div>

                      <!-- Open PR titles (a peek). -->
                      <div x-show="r.openPRs && r.openPRs.length" class="flex flex-col gap-0.5 mt-0.5">
                        <template x-for="p in (r.openPRs || []).slice(0, 5)" :key="p.number">
                          <a :href="'https://github.com/'+r.repo+'/pull/'+p.number" target="_blank"
                             class="text-[11px] text-base-content/60 hover:text-primary truncate flex items-center gap-1">
                            <i class="ph ph-git-pull-request text-[10px]" :class="p.draft ? 'text-base-content/30' : 'text-primary/60'"></i>
                            <span class="font-mono text-base-content/30" x-text="'#'+p.number"></span>
                            <span class="truncate" x-text="p.title"></span>
                          </a>
                        </template>
                      </div>

                      <!-- Stranded branch names (a peek), each to its GitHub tree. -->
                      <div x-show="strandedNames(r).length" class="flex flex-wrap items-center gap-1 mt-0.5 text-[11px]">
                        <i class="ph ph-warning-circle text-[10px] text-warning"></i>
                        <template x-for="n in strandedNames(r)" :key="n">
                          <a :href="'https://github.com/'+r.repo+'/tree/'+encodeURIComponent(n)" target="_blank"
                             class="font-mono text-warning/80 hover:text-warning underline decoration-dotted" x-text="n"></a>
                        </template>
                      </div>
                    </div>
                  </div>
      ...
      // A peek at a repo's stranded branch names, for the Activity view.
      strandedNames(r){
        return (r.survey?.branches || []).filter(b => b.group === 'stranded').map(b => b.name).slice(0, 6);
      },
