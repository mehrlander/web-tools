document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    return {
                ...
                <div class="rounded-lg border-l-4 pl-3 pr-3 py-2 transition-colors hover:brightness-[1.02]"
                  ...
                  <div class="flex items-center flex-wrap gap-x-3 gap-y-1 mt-1.5 text-sm">
                    <button @click="openBranchFiles(row.repo, row.name)"
                      ...
                      <i class="ph ph-folder-open"></i>Browse</button>
      ...
      // Open a live branch straight into the file browser at its ref.
      async openBranchFiles(repo, name){
        if (!window.__shell) return;
        await window.__shell.ensureBrowser(repo, name);
