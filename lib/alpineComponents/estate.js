                      ...
                      <div class="mb-1.5">
                        ...
                        <div class="grid gap-x-4 gap-y-0.5" style="grid-template-columns:repeat(auto-fill,minmax(20rem,1fr))">
                          <template x-for="it in g.items" :key="it.id">
                            <div class="flex items-start gap-2 px-2 py-1 rounded-lg transition-colors duration-75 hover:bg-sky-100 group min-w-0">
                              ...
                              <button type="button" @click="startPinEdit(it)" x-show="editId !== it.id"
                                      class="p-2.5 -my-1.5 -mx-0.5 rounded-md opacity-0 group-hover:opacity-100 [@media(hover:none)]:opacity-100 text-base-content/30 hover:text-primary transition-opacity duration-75 shrink-0"
                              ...
                              <button type="button" @click="deletePin(it)" x-show="editId !== it.id"
                                      class="p-2.5 -my-1.5 -mx-0.5 rounded-md opacity-0 group-hover:opacity-100 [@media(hover:none)]:opacity-100 text-base-content/30 hover:text-error transition-opacity duration-75 shrink-0"
                      ...
                      <template x-for="it in todoOpen" :key="it.id">
                        <div class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg border-l-4 transition-colors duration-75 group"
                          ...
                          <!-- The checkbox in its OWN label, wearing the same hit
                               ...
                               p-2.5 -my-1.5 -mx-0.5. Measured at 390px with touch
                               ...
                               the row to hit. The recipe pads to 36 x 44, which
                               is 1,584 square and clears Apple's 44px in the
                               axis that matters for a list, and the negative
                               margins hand most of the space back so the row
                               grows from 36px to 44 rather than to 52.

                               The margins are -1.5 and -0.5 rather than a
                               symmetrical pull because the tighter option
                               measured worse where it counts: at -my-2 -mx-1 the
                               row stayed 40px but the verbs sat 2px apart and
                               touched the row below, which buys a bigger target
                               and spends it on mis-taps. -->
                          <label class="flex items-center p-2.5 -my-1.5 -mx-0.5 rounded-md cursor-pointer shrink-0">
                          ...
                          <template x-if="editId === it.id">
                            <button type="button" @click="commitTodoEdit(it)" title="Save"
                                    class="p-2.5 -my-1.5 -mx-0.5 rounded-md text-base-content/40 hover:text-primary shrink-0">
                              <i class="ph ph-check"></i></button>
                          </template>
                          <template x-if="editId === it.id">
                            <button type="button" @click="cancelEdit()" title="Discard"
                                    class="p-2.5 -my-1.5 -mx-0.5 rounded-md text-base-content/40 hover:text-error shrink-0">
                              <i class="ph ph-x"></i></button>
                          </template>
                          ...
                          <span x-show="editId !== it.id" class="p-2.5 -my-1.5 -mx-0.5 rounded-md relative shrink-0 flex items-center">
                          ...
                          <button type="button" @click="toggleUrgent(it)" x-show="editId !== it.id"
                                  class="p-2.5 -my-1.5 -mx-0.5 rounded-md shrink-0 transition-colors"
                        ...
                        <template x-if="todoShowDone">
                          <div class="flex flex-col gap-1">
                            ...
                            <template x-for="it in todoDone" :key="it.id">
                              <div class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg border-l-4 border-transparent transition-colors duration-75 hover:bg-sky-100 group">
                                <label class="flex items-center p-2.5 -my-1.5 -mx-0.5 rounded-md cursor-pointer shrink-0">
                                  <input type="checkbox" :checked="!!it.done" @change="toggleTodo(it)"
                                         class="checkbox checkbox-sm">
                                </label>
                  ...
                  <div x-show="!jotLoading" class="flex-1 min-h-0 overflow-y-auto -mx-1 px-1">
                    <div class="flex flex-col gap-1">
                      ...
                      <template x-for="it in jotPile" :key="it.id">
                        <div class="flex items-start gap-2.5 px-2 py-1.5 rounded-lg transition-colors duration-75 hover:bg-sky-100 group">
