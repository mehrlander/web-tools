...
${opts.nested ? '' : `
    ...
    return {
      ...
      get tableColumns(){
        ...
        return [
          this.stateColumn(),
          // SECOND, ahead of the name. What a row IS gets read left to right,
          // and which repos a session touched narrows it faster than its title
          // does: four glyphs against a sentence. The id sits behind them both
          // in the column menu.
          this.reposColumn(),
          this.idColumn(),
          this.nameColumn(),
      ...
      // right edge on a 1500px screen. 128 is four glyphs plus the overflow
      // counter; it was 300 while this column held name pills.
      reposColumn(){
        return { title: 'Repos', field: 'repos', width: 128, headerFilter: 'input',
          formatter: (c) => this.repoIconCell(c.getValue(), c.getRow().getData()), dflt: true };
      },
      idColumn(){
