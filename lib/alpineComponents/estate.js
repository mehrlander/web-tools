...
${opts.nested ? '' : `
                    ...
                    <template x-if="branchPages(row).length">
                      <button type="button"
                        ...
                        <i class="ph ph-caret-down text-[10px] opacity-40"></i>
                      </button>
                    </template>
            ...
            <template x-if="rowCard">
              <div class="flex flex-col">
                ...
                <div x-show="rowCard.kind === 'renders'" class="flex flex-col py-1">
                  <p class="px-3 py-1.5 border-b border-base-200 text-xs text-base-content/60">
                    Pages this branch changed, rendered via toss.
                  </p>
                  <template x-for="p in (rowCard.pages || [])" :key="p.addr">
                    <div class="flex items-center gap-2 px-3 py-2 border-b border-base-200 last:border-b-0 hover:bg-base-200/50 transition-colors">
                      <i class="ph ph-browsers text-base text-base-content/40 shrink-0"></i>
                      <div class="flex-1 min-w-0 flex flex-col gap-0.5">
                        <div class="flex items-baseline gap-1.5">
                          <span class="font-mono text-xs font-semibold text-base-content/80 truncate" x-text="p.label"></span>
                          <span x-show="p.path && p.path !== p.label" class="font-mono text-[10px] text-base-content/40 truncate" x-text="p.path"></span>
                        </div>
                        <span class="text-xs text-base-content/60 leading-snug" x-text="p.title"></span>
                      </div>
                      <a :href="p.url" target="_blank" rel="noopener"
                         class="btn btn-xs btn-primary gap-1 font-mono shrink-0 normal-case">
                        <span>Render</span>
                        <i class="ph ph-arrow-square-out"></i>
                      </a>
                    </div>
