...
${opts.nested ? '' : `
    ...
    return {
      ...
      get tableColumns(){
        ...
        const dateCol = { title: 'Last', field: 'at', width: 64, headerFilter: 'input',
          headerFilterPlaceholder: 'm/dd',
        ...
        return [
          // WHEN, THEN WHAT, THEN WHICH, THEN WHO. The date leads because it is
          // the axis the table already sorts on, so the column a reader scans
          // to find their place is the one their eye lands on first. Repos
          // follows the state because which repos a session touched narrows a
          // row faster than its title does: four glyphs against a sentence.
          // The id and the duration sit behind them in the column menu.
          { ...dateCol, dflt: true },
          this.stateColumn(),
          this.reposColumn(),
          this.idColumn(),
          ...
          // wants magnitude gets it in one tap from the column menu. Elapsed
          // time is also the weakest of the three counts here, since it counts
          // the night a session was left open; Calls counts what it did.
