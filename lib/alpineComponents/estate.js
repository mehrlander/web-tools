...
${opts.nested ? '' : `
    ...
    return {
      ...
      //
      // TWO LAYERS, and the split is deliberate. branchAssistant() reads what
      // the branch DECLARES (a prefix, a session, a trailer) and is what the
      // filter chips count, so a chip stays a record. branchGuess() is
      // assistant-guess.js offering an opinion on a row that declared nothing,
      // and it reaches the MARK only. So the four unprefixed Gemini branches
      // of 2026-09-18 draw a dashed sparkle while still counting under Human,
      // which is the honest pair: the eye is told, the tally is not moved.
      branchAssistant(row){
        return row?.assistant || window.assistantMark?.classify?.(row) || 'human';
      },
      branchGuess(row){
        if (this.branchAssistant(row) !== 'human') return null;
        return window.assistantGuess?.predict?.(row) || null;
      },
      branchAssistantMark(row){
        const a = this.branchAssistant(row);
        if (a === 'human') {
          const g = this.branchGuess(row);
          return g ? window.assistantMark?.svg?.(g.assistant, { cls: 'w-4 h-4 shrink-0', guess: true }) || '' : '';
        }
        return window.assistantMark?.svg?.(a, { cls: 'w-4 h-4 shrink-0' }) || '';
      },
      branchAssistantColor(row){
        const a = this.branchAssistant(row);
        const key = a === 'human' ? (this.branchGuess(row)?.assistant || a) : a;
        return window.assistantMark?.color?.(key) || window.assistantMark?.COLOR?.[key] || '#64748b';
      },
      branchAssistantTitle(row){
        const a = this.branchAssistant(row);
        // A guess has to say that it is one, how sure it is, and what drove
        // it, in the tooltip that opens on the mark itself. A guessed mark a
        // reader cannot interrogate is worse than an empty slot.
        if (a === 'human') {
          const g = this.branchGuess(row);
          if (!g) return 'Human branch';
          const label = window.assistantMark?.label?.(g.assistant) || g.assistant;
          if (g.basis === 'identity') return label + ', from the first commit\u2019s author';
          const acc = window.assistantGuess?.MODEL?.fitted?.loo_accuracy;
          return 'Probably ' + label + ' (' + Math.round(g.p * 100) + '%), guessed from writing style'
            + (g.why?.length ? ': ' + g.why.join(', ') : '')
            + '. Nothing is declared on this branch'
            + (acc ? '; the model reads ' + Math.round(acc * 100) + '% on held-out branches.' : '.');
        }
