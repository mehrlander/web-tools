document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    // (`estate.nest`: a companion repo as a compact strip inside its parent's
    // card). Curated surfaces from the registry's surfaces/ dir render
    // beneath. Public (no token): the public default only, and no surfaces.
                ...
                <template x-for="e in row" :key="e.repo">
                  <div class="card bg-base-100 border border-base-300 shadow-sm hover:border-primary/40 transition-colors
                    ...
                    <div class="card-body p-4 gap-1.5">
                      ...
                      <!-- Nested companion repo (estate.nest): a compact strip
                           for a repo that rides inside this card instead of
                           holding its own — same identity row and the same
                           pin/jump vocabulary, aligned under the parent's. -->
                      <template x-if="e.child">
                        <div class="mt-1 pt-2 border-t border-base-200 flex flex-col gap-1">
                          <div class="flex items-center gap-1.5">
                            <i class="ph text-base text-primary shrink-0" :class="e.child.icon"></i>
                            <button @click="openRepo(e.child.repo)"
                                    class="font-mono text-xs font-semibold truncate hover:text-primary transition-colors cursor-pointer text-left"
                                    :title="e.child.meta?.ago ? 'pushed ' + e.child.meta.ago : ''"
                                    x-text="e.child.repo.split('/')[1]"></button>
                            <div class="grow"></div>
                            <i x-show="e.child.meta" class="ph text-sm leading-none shrink-0 text-base-content/40"
                               :class="e.child.meta?.priv ? 'ph-lock' : 'ph-globe'"
                               :title="e.child.meta?.priv ? 'private' : 'public'"></i>
