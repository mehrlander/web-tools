...
${opts.nested ? '' : `
    ...
    return {
      ...
      // ONE TURN, named. The number is the turn's index in the SESSION, not in
      // what the rail drew, so the first mark on a clipped row reads "turn 34
      // of 68" and states the clipping by being an ordinal. The sentence after
      // it is there anyway, because a reader should not have to infer a missing
      // third of the session from a number that starts at 34.
      railTurnNote(row, t){
        const beats = Array.isArray(row?.beats) ? row.beats : [];
        const sp = this.sessionRailSpan;
        const t0 = Date.parse(row?.started || '');
        const before = beats.filter(m => t0 + m * 60000 < sp.a).length;
        const one = before === 1;
        return {
          when: this.railWhen(t.at),
          note: 'Turn ' + (t.i + 1) + ' of ' + beats.length + ' in this session.'
              + (before ? ' ' + before + (one ? ' earlier turn falls' : ' earlier turns fall')
                        + ' before this window and ' + (one ? 'is' : 'are') + ' not drawn.' : ''),
        };
      },
