document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    // (`estate.nest`: a companion repo shows only its three header icons —
    // lock, gear, github — directly under the parent's three; the lock under
    // the globe is the whole identity cue, and opens the repo). Curated
                ...
                <template x-for="e in row" :key="e.repo">
                  <div class="card bg-base-100 border border-base-300 shadow-sm hover:border-primary/40 transition-colors
                    ...
                    <div class="card-body p-4 gap-1.5">
                      ...
                      <!-- Note line, with the nested companion's three header
                           icons (estate.nest) floated at its right edge —
                           directly under the parent's three, without pushing
                           the note down (the text flows beside them). The
                           lock leading where the globe sits above is what
                           says "the private counterpart"; it also opens the
                           repo here. Same sizes and tones as the parent's
                           row — no extra dimming. -->
                      <div class="min-h-8">
                        <template x-if="e.child">
                          <div class="float-right flex items-center gap-1.5 ml-2 mb-1">
                            <button @click="openRepo(e.child.repo)"
                                    ...
                                    :title="e.child.repo.split('/')[1] + ' (private): open here'">
                              <i class="ph ph-lock text-sm leading-none"></i></button>
                        ...
                        <p class="text-xs text-base-content/70" x-text="e.note || e.meta?.desc || ''"></p>
                      </div>
