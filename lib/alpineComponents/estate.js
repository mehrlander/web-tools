...
${opts.nested ? '' : `
    ...
    return {
            ...
            <div x-show="authed" class="flex items-center gap-2 mb-3 flex-wrap">
              ...
              <label class="input input-sm flex items-center gap-1.5 grow min-w-40 max-w-sm"
                ...
                <input type="search" x-model.debounce.150ms="sessionQuery" data-find-box
                       placeholder="Filter these sessions…" class="grow text-base"
                       aria-label="Filter sessions">
              </label>
              <button x-show="sessionRowsLens && sessionQuery.trim()" @click="openSessionGrep()"
                      data-note="These rows hold the opening ask and the names, not what was said later. This runs the same query over every captured record, which is the whole store and a slow first pass."
                      class="btn btn-ghost btn-sm gap-1.5 text-base text-base-content/60 shrink-0">
                <i class="ph ph-magnifying-glass-plus"></i>Search what was said</button>
            </div>
            ...
            <div x-show="authed && chatRows.length" class="flex items-center gap-2 mb-2 flex-wrap">
              <label class="input input-sm flex items-center gap-1.5 grow min-w-40 max-w-sm">
                ...
                <input type="search" x-model.debounce.150ms="chatQuery" data-find-box
                       placeholder="Filter loaded months…" class="grow text-base"
      ...
      // carrier here is weight: both are plain, and unknown is fainter. Keeping
