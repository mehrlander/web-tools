document.addEventListener('alpine:init', function() {
  ...
  Alpine.data('estate', function() {
    ...
    //
    // Routes left on 2026-09-08. It was the one pane here keyed to something
    // other than the estate: a hand-authored registry of ONE repo's own
    // destinations, which is what every Map tab is, sitting in a row of
    // cross-repo crawl caches. ?view=routes still resolves; the shell sends it
    // to Map's Views tab.
    const ACTIVITY_TABS = ['activity', 'sessions', 'chats', 'state'];
...
${opts.nested ? '' : `
    ...
    return {
        ...
        <div :class="tab==='lists' && 'flex-1 min-h-0 flex flex-col'">
          ...
          <div class="flex-1 min-w-0" :class="tab==='sessions' ? '' : 'hidden'"
            ...
            <!-- Repo chips, same contract as the Open view's: only repos that
                 ...
                 paragraph about it. -->
            <div x-show="authed && sessionRowsLens && !sessionsLoading && sessionRepos.length > 1"
            ...
            <div x-show="authed && sessionRowsLens && !sessionsLoading && sessionStates.length > 1"
                 class="flex items-center gap-1.5 mb-3 -mx-1 px-1 pb-0.5 overflow-x-auto">
            ...
            <p x-show="authed && sessionRowsLens && !sessionsLoading && sessionsBehindState"
            ...
            <!-- ── Table: the same rows, queryable ────────────────────────────
                 ...
                 and the column menu holds everything not in the phone set, so
                 a column is a tap away rather than off the right edge. -->
            <div x-show="authed && sessionLens === 'table'" class="flex flex-col min-w-0">
              ...
              <!-- Grain, query and columns on one row: what a row IS, how to
                   ...
                   because it is the answer to the two controls before it. -->
              <div class="flex items-center gap-2 mb-2 flex-wrap">
                <div class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 w-fit flex-wrap" role="tablist">
                  <template x-for="g in GRAINS" :key="g.key">
                    <button role="tab" @click="sessionGrain = g.key" :data-note="g.note"
                ...
                <label class="input input-sm flex items-center gap-1.5 grow min-w-32 max-w-64">
                  <i class="ph ph-magnifying-glass text-base-content/40"></i>
                  <input type="search" x-model="tableQuery" placeholder="Filter"
                         class="grow text-base" aria-label="Filter rows">
                </label>

                <!-- Columns in a dropdown, not a chip row: the house rule for a
                     browsing surface is that filters go in the header, and a
                     twelve-chip row would cost more viewport than the columns
                     it turns on. -->
                <div class="dropdown dropdown-end">
                  <button tabindex="0" class="btn btn-ghost btn-sm gap-1.5 text-base">
                    <i class="ph ph-columns"></i><span x-text="shownColumns.length"></span></button>
                  <ul tabindex="0" class="dropdown-content z-30 menu menu-sm bg-base-100 rounded-box shadow-lg border border-base-300 w-52 max-h-80 flex-nowrap overflow-y-auto">
                    <template x-for="c in tableColumns" :key="c.key">
                      <li><button @click="toggleCol(c.key)" class="flex items-center gap-2 text-base">
                        <i class="ph shrink-0" :class="tableCols.includes(c.key) ? 'ph-check-square text-primary' : 'ph-square text-base-content/30'"></i>
                        <span x-text="c.label || c.head || c.key"></span></button></li>
                    </template>
                  </ul>
                </div>

                <span class="font-mono text-sm text-base-content/50 tabular-nums"
                      x-text="tableCount"></span>
              </div>

              <!-- The table scrolls in its own box rather than the page: a wide
                   column set must never make the whole view scroll sideways.
                   min-w-max is what makes the box the wider thing and the
                   table its natural width, so a narrow set does not stretch to
                   fill and a wide one is reachable. No w-full for the same
                   reason: stretching a five-column set across a desktop pushes
                   the slack into the gaps BETWEEN columns, which reads as four
                   unrelated lists rather than one row. -->
              <div class="overflow-x-auto -mx-1 px-1 border-y border-base-300/60">
                <table class="min-w-max text-base border-collapse">
                  <thead>
                    <tr class="sticky top-0 z-10 bg-base-100">
                      <template x-for="c in shownColumns" :key="c.key">
                        <th class="text-left font-medium text-sm text-base-content/60 whitespace-nowrap px-2 py-1.5 border-b border-base-300">
                          <button @click="sortBy(c)" class="flex items-center gap-1 hover:text-base-content transition-colors"
                                  :class="tableSort.key === c.key ? 'text-primary' : ''"
                                  :data-note="c.head || ''">
                            <span x-text="c.label"></span>
                            <i class="ph text-xs" :class="sortMark(c)" x-show="sortMark(c)"></i>
                          </button>
                        </th>
                      </template>
                    </tr>
                  ...
                  <tbody>
                    <template x-for="(row, i) in tableRows" :key="sessionGrain + i">
                      <tr @click="openTableRow(row)"
                          class="cursor-pointer hover:bg-base-200/50 transition-colors align-baseline">
                        <template x-for="c in shownColumns" :key="c.key">
                          <td class="px-2 py-1.5 border-b border-base-300/40"
                              :class="c.num ? 'text-right tabular-nums font-mono whitespace-nowrap'
                                            : c.key === 'ask' || c.key === 'title' || c.key === 'subject'
                                              ? 'max-w-96 truncate' : 'whitespace-nowrap'">
                            <!-- The state column draws the glyph the reply
                                 itself ended with, the same one the list rail
                                 carries, rather than the key behind it. -->
                            <template x-if="c.key === 'state'">
                              <span class="leading-none"
                                    :data-note="sessionStateNote(sessionGrain === 'edge' ? row.row : row)"
                                    x-text="sessionStateMark(sessionGrain === 'edge' ? row.row : row)"></span>
                            </template>
                            <template x-if="c.key === 'at' || c.key === 'first'">
                              <span class="text-base-content/60" x-text="cellText(c, row)"></span>
                            </template>
                            <template x-if="c.key === 'id' || c.key === 'name' || c.key === 'branch'">
                              <span class="font-mono" x-text="cellText(c, row)"></span>
                            </template>
                            <template x-if="c.key === 'group'">
                              <span class="badge badge-sm badge-ghost font-medium" x-show="cellText(c, row)"
                                    x-text="cellText(c, row)"></span>
                            </template>
                            <template x-if="c.key === 'pr'">
                              <span x-show="cellText(c, row) !== '0'" x-text="'#' + cellText(c, row)"></span>
                            </template>
                            <template x-if="!['state','at','first','id','name','branch','group','pr'].includes(c.key)">
                              <span x-text="cellText(c, row)"></span>
                            </template>
                          </td>
                        </template>
                      </tr>
              ...
              <!-- Empty is an answer and says which control produced it, since
                   four of them are live at once here and a bare "nothing"
                   makes the reader clear all four to find out which. -->
              <p x-show="!tableRows.length" class="text-base text-base-content/50 py-6"
                 x-text="grainRows.length
                   ? 'No ' + grainMeta.unit + ' matches ' + JSON.stringify(tableQuery) + ' in the shown columns.'
                   : 'Nothing in ' + sessionScopeMeta.label + ' at this grain.'"></p>

              <!-- THE WINDOW, stated. The caches are capped (400 session rows,
                   and the branch scan is capped per repo), so the oldest row
                   here is a horizon that moves as new work pushes old work off
                   the end. A card list that ends looks like a window; a
                   sortable table looks like a census, which is exactly the
                   reading this line exists to refuse. -->
              <p class="text-sm text-base-content/40 mt-2" x-show="tableRows.length">
                A window, not a census: the caches keep the newest 400 session records and cap the
                branch scan per repo, so sorting to the oldest row reaches the edge of what is
                kept rather than the beginning of the record.
              </p>
            </div>
      ...
      // drawn lenses answer "what shape is this". None of them answers "find
      // me the one where X", which on 344 session rows and 434 branch rows is
      // not a scrolling problem but a query one: the pane's whole filter
      // surface is three chip rows, and nothing on it sorts.
      //
      ...
      // one join that sessionTree already builds, and the question that needs a
      ...
      //
      // Hand-rolled rather than Tabulator, which the viewer already loads for
      // CSV and workbooks. Three reasons and the first is decisive: this
      // component has no asset loader, so adopting it here means duplicating
      // the viewer's or growing one. The row counts are two orders of magnitude
      // under where virtual scrolling starts to pay, and every cell wants a
      // helper this component already has (the state glyph, the ago, the deck
      // opener), which is a formatter callback in Tabulator and an expression
      // here.
      ...
      sessionGrain: 'session',
      tableQuery: '',
      tableSort: { key: 'at', dir: -1 },
      ...
      // job: "713 edges" is the sentence that stops 713 being read as sessions.
      GRAINS: [
      ...
      // ── Columns ──────────────────────────────────────────────────────────
      // `get` returns the SORT VALUE and the plain-text the query matches; the
      // cell itself is drawn in the markup, which is what lets a cell carry a
      // glyph, a link or a chip while the column still sorts and filters on
      // something sensible. `num` sorts descending first, because on every
      // numeric column here the interesting end is the big one.
      //
      // `dflt` is the phone set. Everything else is one tap away in the column
      // menu rather than off the right edge, which is the difference between a
      // column a reader can reach and one they have to know about.
      get tableColumns(){
        const G = this.sessionGrain;
        if (G === 'branch') return [
          { key: 'repo',   label: 'Repo',    dflt: true,  get: r => this.repoShort(r.repo) || '' },
          { key: 'name',   label: 'Branch',  dflt: true,  get: r => r.name || '' },
          { key: 'group',  label: 'State',   dflt: true,  get: r => r.group || '' },
          { key: 'at',     label: 'Last',    dflt: true,  num: true, get: r => Date.parse(r.date || '') || 0,
            text: r => (r.date || '').slice(0, 10) },
          { key: 'first',  label: 'First',   dflt: false, num: true, get: r => Date.parse(r.first || '') || 0,
            text: r => (r.first || '').slice(0, 10) },
          { key: 'pr',     label: 'PR',      dflt: false, num: true, get: r => r.pr?.number || r.prLast?.number || 0 },
          { key: 'ahead',  label: 'Ahead',   dflt: false, num: true, get: r => r.ahead ?? -1 },
          { key: 'behind', label: 'Behind',  dflt: false, num: true, get: r => r.behind ?? -1 },
          { key: 'nses',   label: 'Sessions',dflt: false, num: true, get: r => (r.sessions || []).length },
          { key: 'subject',label: 'Subject', dflt: true,  get: r => r.subject || '' },
        ];
        if (G === 'edge') return [
          { key: 'state',  label: 'State',   dflt: true,  get: r => r.row?.state || '',
            head: 'The state the session\'s closing reply ended in' },
          { key: 'id',     label: 'Session', dflt: true,  get: r => r.row?.id || '' },
          { key: 'title',  label: 'Name',    dflt: true,  get: r => this.sessionLabel(r.row) || '' },
          { key: 'repo',   label: 'Repo',    dflt: true,  get: r => r.repo || '' },
          { key: 'branch', label: 'Branch',  dflt: true,  get: r => r.branch || '' },
          { key: 'group',  label: 'Branch state', dflt: false, get: r => r.group || '' },
          { key: 'lines',  label: 'Lines',   dflt: false, num: true, get: r => r.lines || 0 },
          { key: 'at',     label: 'Last',    dflt: true,  num: true, get: r => Date.parse(r.at || '') || 0,
            text: r => (r.at || '').slice(0, 10) },
          { key: 'ask',    label: 'Ask',     dflt: false, get: r => r.row?.ask || '' },
        ];
        return [
          { key: 'state',  label: 'State',   dflt: true,  get: r => r.state || '',
            head: 'The state the session\'s closing reply ended in' },
          { key: 'id',     label: 'Session', dflt: true,  get: r => r.id || '' },
          { key: 'title',  label: 'Name',    dflt: true,  get: r => this.sessionLabel(r) || '' },
          { key: 'at',     label: 'Last',    dflt: true,  num: true, get: r => Date.parse(this.sessionLastAt(r)) || 0,
            text: r => (this.sessionLastAt(r) || '').slice(0, 10) },
          { key: 'mins',   label: 'Ran',     dflt: true,  num: true, get: r => r.mins || 0 },
          { key: 'repos',  label: 'Repos',   dflt: false, get: r => (r.repos || []).map(x => x.name).join(' ') },
          { key: 'nbr',    label: 'Branches',dflt: false, num: true, get: r => (r.branches || []).length },
          { key: 'lines',  label: 'Lines',   dflt: false, num: true,
            get: r => (r.repos || []).reduce((n, x) => n + (x.lines || 0), 0) },
          { key: 'calls',  label: 'Calls',   dflt: false, num: true, get: r => r.calls || 0 },
          { key: 'fails',  label: 'Failed',  dflt: false, num: true, get: r => r.failures || 0 },
          { key: 'files',  label: 'Files',   dflt: false, num: true, get: r => r.filesTotal || 0 },
          { key: 'ask',    label: 'Ask',     dflt: false, get: r => r.ask || '' },
        ];
      },
      ...
      // middle of the effect that reads it. That settles rather than loops here
      // (the second pass finds it seeded), but it is a write-during-read either
      // way, and the store it needs is small enough to build in the setter.
      get tableCols(){
        const held = this.tableCols_ && this.tableCols_[this.sessionGrain];
        return held || this.tableColumns.filter(c => c.dflt).map(c => c.key);
      },
      toggleCol(key){
        ...
        const next = i < 0 ? [...on, key] : on.length > 1 ? on.filter(k => k !== key) : on;
        // A fresh object rather than a keyed write, so the dependency Alpine
        // tracks is the store itself and not a property that did not exist when
        // the effect first read it.
        this.tableCols_ = { ...(this.tableCols_ || {}), [this.sessionGrain]: next };
      },
      get shownColumns(){
        const on = this.tableCols;
        return this.tableColumns.filter(c => on.includes(c.key));
      },

      // ── Rows, per grain ──────────────────────────────────────────────────
      // Session grain reuses the list's own filtered set, so the chips above
      // mean exactly what they mean on the list. The other two apply the same
      // three filters themselves, since neither has a pre-filtered set to
      // borrow: scope by the row's own clock, repo by the row's repo, state by
      // the session behind it (a branch with no session drops out under a state
      // filter rather than riding along unfiltered, which is the rule the tree
      // already applies to stubs).
      get grainRows(){
        const G = this.sessionGrain;
        if (G === 'session') return this.sessionRows;

        const repo = this.activeSessionRepo, st = this.activeSessionState;
        if (G === 'branch') {
          const byBranch = new Map();
          for (const n of this.sessionTree.nodes)
            for (const b of n.children || [])
              byBranch.set(this.repoShort(b.repo) + ' ' + b.name, n.row);
          return this.allBranchRows.filter(b => {
            if (!this.inBranchTableScope(b)) return false;
            if (repo && this.repoShort(b.repo) !== repo) return false;
            if (st) {
              const s = byBranch.get(this.repoShort(b.repo) + ' ' + b.name);
              if (!s || s.state !== st) return false;
            }
            return true;
          });
        }

        // Edge grain: the session's own account of what it committed where,
        // joined to the branch row for the classification the record cannot
        // know (whether the branch landed). An edge with no branch row is kept
        ...
        for (const r of this.sessionRows) {
          for (const x of r.repos || []) {
            if (repo && x.name !== repo) continue;
            const b = branchAt.get(x.name + ' ' + (x.branch || ''));
            out.push({ row: r, repo: x.name, branch: x.branch || '', lines: x.lines || 0,
                       group: b?.group || '', branchRow: b || null,
                       at: this.sessionLastAt(r) });
          }
        }
        return out;
      ...
      inBranchTableScope(b){
        ...
        // answer. It shows every branch rather than none: an empty pane under
        // a lit chip reads as a bug, where the full list under a scope that
        // does not apply to this grain reads as what it is.
        if (s === 'failed') return true;
        const days = s === 'day' ? 1 : s === 'week' ? 7 : 30;
        return Date.parse(b.date || '') >= Date.now() - days * 864e5;
      },

      // Query, then sort. The query runs over the SHOWN columns only, which is
      // the behaviour a visible table implies: hiding a column hides what it
      // matches on, so a search that hit nothing on screen cannot be explained
      // by a column the reader cannot see.
      get tableRows(){
        const cols = this.shownColumns;
        const q = this.tableQuery.trim().toLowerCase();
        let rows = this.grainRows;
        if (q) {
          const terms = q.split(/\s+/);
          rows = rows.filter(r => {
            const hay = cols.map(c => String((c.text || c.get).call(this, r) ?? '')).join(' ').toLowerCase();
            return terms.every(t => hay.includes(t));
          });
        }
        const s = this.tableSort;
        const col = this.tableColumns.find(c => c.key === s.key) || cols[0];
        if (!col) return rows;
        const dir = s.dir;
        return [...rows].sort((a, b) => {
          const x = col.get.call(this, a), y = col.get.call(this, b);
          if (col.num) return (Number(y) - Number(x)) * (dir < 0 ? 1 : -1);
          return String(x).localeCompare(String(y)) * (dir < 0 ? -1 : 1);
        });
      },
      // A numeric column opens on its big end and a text one on A, which is
      // what each is asked for first. Clicking the column already sorted
      // reverses it.
      sortBy(col){
        if (this.tableSort.key === col.key) this.tableSort = { key: col.key, dir: -this.tableSort.dir };
        else this.tableSort = { key: col.key, dir: col.num ? -1 : 1 };
      },
      sortMark(col){
        if (this.tableSort.key !== col.key) return '';
        return this.tableSort.dir < 0 ? 'ph-caret-down' : 'ph-caret-up';
      },
      // What a cell says. Kept beside the columns rather than in the markup so
      // a column is one object: its label, its sort value, its query text and
      // its rendering.
      cellText(col, row){
        const v = (col.text || col.get).call(this, row);
        if (col.key === 'mins') return this.durLabel(row.mins || 0);
        if (col.num && !col.text) return v ? String(v) : (v === 0 ? '0' : '');
      ...
      openTableRow(row){
        const G = this.sessionGrain;
        if (G === 'session') return this.openSessionDetail(row, this.tableRows);
        if (G === 'branch') return this.openBranchDetail(row, this.tableRows);
        if (row.branchRow) return this.openBranchDetail(row.branchRow);
        return this.openSessionDetail(row.row);
      },
      // The one figure the header owes the reader, and the reason it names the
      // unit: under the edge grain these are pairs, and a filtered pair count
      // read as a session count is off by a factor of two here.
      get tableCount(){
        const n = this.tableRows.length, total = this.grainRows.length;
