                ...
                <div x-data="{ get row(){ return n.row } }">
                  ...
                  <div class="flex items-center flex-wrap gap-x-4 gap-y-1.5 mt-2 text-base">
                    ...
                    <!-- The Claude session itself, when the record could name
                         ...
                         w-5, not the kit's default w-6, and the pair below
                         matches it. The counts beside it are Phosphor glyphs at
                         text-lg, whose drawn extent is 14.75px inside an 18px
                         box; the mark is a stroked path that fills its own box,
                         so at w-6 it drew 23px of ink against their 14.75 and
                         sat visibly above the line. w-5 takes it to 19.2px,
                         which reads level: the mark is sparse where a glyph is
                         solid, so matching ink exactly would make it the
                         faintest thing on the row. -->
                    <a x-show="row.agent" :href="row.agent" target="_blank"
                       title="Open this session in Claude Code"
                       class="flex items-center gap-0.5 hover:opacity-75 transition-opacity"
                       x-html="window.claudeMark.svg({ cls: 'w-5 h-5 shrink-0' })"></a>
                    <span x-show="!row.agent" class="flex items-center gap-1 text-base-content/30 font-mono"
                      ...
                      <span class="flex shrink-0"
                            x-html="window.claudeMark.svg({ cls: 'w-5 h-5 shrink-0', color: 'currentColor' })"></span><span>&mdash;</span></span>
