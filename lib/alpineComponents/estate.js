document.addEventListener('alpine:init', function() {
  ...
  let flashT = 0;
  // ── THE HOT COLUMN, lit by ONE STYLESHEET RULE rather than by a binding ──
  //
  ...
  // lists 298 rows and draws six to eighteen thousand row ticks, so a binding
  // that reads the hovered column would be that many Alpine effects re-running
  // on every column the pointer crosses, for a background colour. One rule
  // rewritten on the same event is O(1) in the rows, and the browser's own
  // selector matching does the rest.
  //
  // TWO SELECTORS, so the rule outranks the utility class it overrides without
  // !important: an attribute selector alone ties with .bg-primary/60 and would
  // then depend on which stylesheet the browser saw last, which is not a thing
  // to leave to load order.
  let hotStyle = null;
  function railHot(k) {
    if (!hotStyle) {
      hotStyle = document.createElement('style');
      hotStyle.id = 'wt-rail-hot';
      document.head.appendChild(hotStyle);
    }
    hotStyle.textContent = (k === null || k === undefined) ? '' :
      '[data-session-lane] [data-tick="' + k + '"],'
      + '[data-session-node] [data-tick="' + k + '"]{'
      ...
      + 'background-color:var(--color-primary);transform:scale(1.5,1.35)}';
  }
...
${opts.nested ? '' : `
    ...
    return {
      ...
      get sessionRailAll(){
        ...
        return [...bins.values()].sort((a, b) => a.k - b.k).map(b => {
          ...
          return {
            k: b.k,
            // The bin's centre, not its left edge: the mark is three pixels
            // wide and pulled half a pixel left, so centring it on the column
      ...
      railTrack(ev){
        if (!ev){ this.railHover = null; railHot(null); window.Note?.close?.(); return; }
      ...
      rowRailTrack(ev, row){
        const ticks = this.sessionRailTicks(row);
        const pct = this.railPct(ev);
        if (!ticks.length || pct === null) return;
        let best = ticks[0];
        for (const p of ticks) if (Math.abs(p - pct) < Math.abs(best - pct)) best = p;
        const k = this.railBin(best);
        if (this.railHover?.k === k) return;
        const m = this.sessionRailAll.find(x => x.k === k);
      ...
      railAt(m, note = true){
        ...
        if (!note) return;
        this.$nextTick(() => {
          if (this.railHover?.k === m.k) window.Note?.open?.(this.$refs.railCursor);
        });
      },
