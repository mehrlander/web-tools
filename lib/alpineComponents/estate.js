document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    //               (estate:true, group, note, icon, order, nestUnder); the
    ...
    return {
        ...
        <div>
          ...
          <div x-show="tab==='repos'">
            ...
            <!-- The grid: a section per group (header + count), each a full-width
                 ...
                 within-group order come from each repo's own order weight;
                 nested entries (nestUnder) render inside their parent. -->
              ...
              <section class="mb-8">
                ...
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  <!-- One card, possibly two faces: face(e) is the entry being
                       ...
                       (nestUnder) when the visibility toggle has flipped the
                       card. Every field below reads face(e). -->
          ...
          <!-- ── Placement editor: the card's placement (icon, group, note),
               written to the REPO's OWN .web-tools.json (single source of truth),
               not the registry. A link drops through to the raw config editor
               for landing/pins/stage; "Remove from estate" clears estate:true. -->
          <dialog x-ref="entryDlg" class="modal" onclick="if(event.target===this)this.close()">
            <div class="modal-box shadow-none border border-base-300 bg-base-100 p-4 max-w-sm overflow-x-hidden">
              <div class="flex items-center gap-1.5 text-sm font-semibold mb-0.5">
                <i class="ph ph-gear-six text-primary"></i><span>Placement</span>
              </div>
              <div class="font-mono text-xs text-base-content/50 mb-3" x-text="entryRepo"></div>
              <p class="text-[11px] text-base-content/40 mb-3">
                Written to this repo's own <span class="font-mono">.web-tools.json</span>. The repo is the single source of truth for how it appears.
              </p>
              <div class="flex flex-col gap-2">
                <label class="text-[11px] font-semibold text-base-content/60 flex items-center gap-1.5">
                  <i class="ph text-sm" :class="entryIcon.trim() || 'ph-bookmark-simple'"></i>Icon (Phosphor class)
                </label>
                <input x-model="entryIcon" placeholder="ph-bookmark-simple"
                       autocapitalize="off" autocorrect="off" spellcheck="false"
                       class="input input-sm input-bordered font-mono text-xs">
                <label class="text-[11px] font-semibold text-base-content/60 mt-1">Group</label>
                <input list="estate-group-options" x-model="entryGroup" placeholder="group (optional)"
                       autocapitalize="off" autocorrect="off" spellcheck="false"
                       class="input input-sm input-bordered text-xs">
                <label class="text-[11px] font-semibold text-base-content/60 mt-1">Note</label>
                <textarea x-model="entryNote" rows="2" placeholder="note (optional)"
                          class="textarea textarea-bordered text-xs leading-snug"></textarea>
              </div>
              <button @click="$refs.entryDlg.close(); entryFace && openRepoConfig(entryFace)"
                      class="btn btn-ghost btn-xs gap-1.5 mt-3 text-base-content/60 hover:text-primary">
                <i class="ph ph-arrow-square-out"></i>Edit full repo config (landing, pins, stage)
              </button>
              <div class="flex items-center gap-2 mt-3">
                <button @click="removeFromEstate()" :disabled="entrySaving || !authed"
                        class="btn btn-ghost btn-xs text-error/70 hover:text-error mr-auto">Remove from estate</button>
                <button @click="$refs.entryDlg.close()" class="btn btn-ghost btn-sm text-xs">Cancel</button>
                <button @click="saveEntry()" :disabled="entrySaving || !authed"
                        class="btn btn-primary btn-sm text-xs gap-1.5">
                  <span x-show="entrySaving" class="loading loading-spinner loading-xs"></span>
                  <span x-text="entrySaving ? 'Saving…' : 'Save placement'"></span>
                </button>
              </div>
            </div>
          </dialog>
      ...
      loading: true,
      authed: false,
      entries: [],     // [{repo, icon, note, group, order, meta, err, pins, hasLanding, nestUnder, child}]
      ...
      // Placement editor state: the card's placement (icon/group/note), written
      // to the repo's OWN config.
      entryRepo: '',
      entryRef: 'main',
      entryFace: null,
      entryIcon: '',
      ...
      async load(){
        ...
        if (!this.authed){
          ...
          this.entries = [{ repo: def, icon: 'ph-toolbox', note: '', group: '', order: 0,
                            meta: null, err: false, pins: [], hasLanding: false, nestUnder: '', child: null, showChild: false }];
          this.enrichMeta();
          this.loading = false;
          return;
        }
      ...
      // Nesting: a repo naming `nestUnder: parent` rides inside the parent's
      // card instead of holding its own. The relationship is owned by the child.
      applyNesting(){
        for (const child of this.entries){
          if (!child.nestUnder) continue;
          const parent = this.entries.find(e => e.repo === child.nestUnder);
      ...
      // shield dialog's gear tab, bound to the open repo). Switches to the repo.
      ...
      // ── Placement editor: writes the repo's OWN config ──────────────────────
      editEntry(en){
        if (!en) return;
        this.entryFace = en;
        this.entryRepo = en.repo;
        this.entryRef = en.meta?.ref || 'main';
        this.entryIcon = en.icon || '';
        this.entryGroup = en.group || '';
        this.entryNote = en.note || '';
        this.$refs.entryDlg?.showModal();
      },
      async _writeEntry(mutate, doneMsg){
        if (!this.hasToken() || !this.entryRepo) return;
        const toast = Alpine.store('toast');
        this.entrySaving = true;
        try {
          const g = new window.GH({ token: window.TOKEN, repo: this.entryRepo, ref: this.entryRef });
          let cfg = {};
          try { cfg = JSON.parse((await g.get('.web-tools.json')).text); } catch {}
          if (!cfg || typeof cfg !== 'object' || Array.isArray(cfg)) cfg = {};
          mutate(cfg);
          if (typeof g.save !== 'function' && window.gh?.load) await window.gh.load('gh-store.js');
          await g.save('.web-tools.json', cfg, 'Update estate placement via show-repo');
          if (toast) toast('check-circle', doneMsg, 'alert-success', 3000);
          this.$refs.entryDlg?.close();
          window.__shell?.refreshConfigCache?.(true);
          document.dispatchEvent(new CustomEvent('web-tools:config-saved', { detail: { repo: this.entryRepo } }));
        } catch(e){
          if (toast) toast('warning', 'Save failed: ' + (e?.message || e), 'alert-error', 6000);
        } finally { this.entrySaving = false; }
      },
      async saveEntry(){
        await this._writeEntry((cfg) => {
          cfg.estate = true;
          cfg.icon = this.entryIcon.trim() || 'ph-bookmark-simple';
          if (this.entryGroup.trim()) cfg.group = this.entryGroup.trim(); else delete cfg.group;
          if (this.entryNote.trim()) cfg.note = this.entryNote.trim(); else delete cfg.note;
        }, 'Saved placement for ' + this.entryRepo);
      },
      async removeFromEstate(){
        await this._writeEntry((cfg) => { delete cfg.estate; }, 'Removed ' + this.entryRepo + ' from the estate');
      },
