...
${opts.nested ? '' : `
    ...
    return {
                ...
                <div x-data="{ get row(){ return n.row } }">
                  ...
                  <div class="flex items-center flex-wrap gap-x-4 gap-y-1.5 mt-2 text-base">
                    ...
                    <!-- The fan-out. Absent rather than zero on a session that
                         ...
                         count is dispatches, off the tool tally, so it answers
                         on the 30 of 341 records that fanned out rather than on
                         the 1 written since schema 8; the summarizer's agents
                         field says why that order. Warning-coloured when a
                         dispatch launched and never reported, which needs
                         schema 8 and is the silent drop worth seeing. -->
      ...
      agentsLabel(row){
        // "dispatched", not "ran": the figure counts the calls this session
        // MADE, and the one record carrying schema 8's found-transcript count
        // beside it shows the two are not the same number (126 dispatched, 124
        // found, and nothing flagged as unaccounted for the gap).
        const bits = [row.agents + (row.agents === 1 ? ' subagent dispatched'
      ...
      openSessionCard(row, cls, ev){
        ...
        if (cls === 'tools') {
          ...
          note = [rows.length ? '' : 'This record kept no per-tool breakdown.',
                  ...
                  row.agents && !named
                    ? row.agents + ' of these calls dispatched an agent.' : ''
                 ].filter(Boolean).join(' ');
        } else if (cls === 'files') {
