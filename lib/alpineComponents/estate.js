    ...
    return {
      ...
      template: `
        <div>
            ...
            <div x-show="addOpen" class="card bg-base-100 border border-base-300 shadow-sm max-w-md mb-6">
              <div class="card-body p-4 gap-2">
                ...
                <div class="text-[11px] text-base-content/40">
                  Estate placement: writes the registry (web-tools-private). Icon, group, and note are the card's placement here and stay editable with the gear on the card. The repo's own landing and pins live in its config.
                </div>
                      ...
                      <div class="card-body p-4 gap-1.5">
                        <div class="flex items-center gap-1.5">
                          ...
                          <button x-show="authed" @click="editEntry(face(e))"
                                  ...
                                  title="Estate placement: icon, group, note (and repo config)">
                        ...
                        <!-- Direct-to-goal jumps: the shown repo's own pinned
                             folders / files. The card title already opens the
                             repo; Files/Atlas are one sidebar tap from there, so
                             the card carries only the pins the repo designates. -->
          ...
          <!-- ── Estate-entry editor: the card's placement on the estate — icon,
               group, note — which live in the registry (web-tools-private), NOT
               the repo's own config. Editing these closes the gap where a group
               could only be set at add-time. A link drops through to the repo's
               own config (landing, pins, stage), so the delineation is explicit:
               placement here, self-description there. -->
          <dialog x-ref="entryDlg" class="modal" onclick="if(event.target===this)this.close()">
            <div class="modal-box shadow-none border border-base-300 bg-base-100 p-4 max-w-sm overflow-x-hidden">
              <div class="flex items-center gap-1.5 text-sm font-semibold mb-0.5">
                <i class="ph ph-gear-six text-primary"></i><span>Estate placement</span>
              </div>
              <div class="font-mono text-xs text-base-content/50 mb-3" x-text="entryRepo"></div>
              <p class="text-[11px] text-base-content/40 mb-3">
                How this repo sits on the estate. Written to the registry (web-tools-private), not the repo itself.
              </p>
              <div class="flex flex-col gap-2">
              ...
              <button @click="$refs.entryDlg.close(); entryFace && openRepoConfig(entryFace)"
                ...
                <i class="ph ph-arrow-square-out"></i>Edit repo's own config (landing, pins, stage)
              </button>
              <div class="flex items-center justify-end gap-2 mt-3">
      ...
      // Estate-entry editor state: the card's placement (icon/group/note), which
      // lives in the registry (web-tools-private), not the repo's own config.
      entryRepo: '',
      entryFace: null,
      entryIcon: '',
      entryGroup: '',
      ...
      // ── Estate-entry editor ────────────────────────────────────────────────
      // The card's placement fields (icon, group, note) are the registry's, so
      // this reads and writes web-tools-private's repos[] entry, not the repo's
      // own config. Seeds from the shown face's live values.
      editEntry(en){
        if (!en) return;
        this.entryFace = en;
      ...
      async saveEntry(){
        ...
        try {
          ...
          const item = cfg.repos.find(r => r && r.repo === this.entryRepo);
          if (!item){
            if (toast) toast('warning', this.entryRepo + ' is not on the estate', 'alert-warning', 4000);
            this.entrySaving = false; return;
          }
          item.icon = this.entryIcon.trim() || 'ph-bookmark-simple';
          if (this.entryGroup.trim()) item.group = this.entryGroup.trim(); else delete item.group;
          if (this.entryNote.trim()) item.note = this.entryNote.trim(); else delete item.note;
          if (typeof reg.save !== 'function' && window.gh?.load) await window.gh.load('gh-store.js');
          await reg.save('.web-tools.json', cfg, 'Update estate placement for ' + this.entryRepo + ' via show-repo');
          if (toast) toast('check-circle', 'Saved placement for ' + this.entryRepo, 'alert-success', 3000);
