document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    return {
      ...
      template: `
        <div>
          ...
          <div x-show="tab==='surfaces'">
            <!-- Header: the surface tabs (segmented pill, the same control the
                 Open view uses) sit inline with New, so switching a surface and
                 adding one share a row instead of stacking. One surface: no tabs,
                 New alone on the right. -->
            <div class="flex items-center gap-2 mb-4" x-show="authed">
              <div x-show="surfaces.length > 1" role="tablist"
                   class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 overflow-x-auto">
                <template x-for="(s, i) in surfaces" :key="s.file">
                  <button role="tab" @click="surfActive=i"
                          class="px-2.5 py-1 rounded-md text-sm font-medium transition-colors whitespace-nowrap"
                          :class="i===surfActive ? 'bg-base-100 text-primary shadow-sm' : 'text-base-content/60 hover:text-base-content'"
                          x-text="s.manifest.name || s.file"></button>
                </template>
              </div>
              <div class="grow"></div>
              <button @click="newSurface()"
                      class="btn btn-sm btn-ghost gap-1.5 text-base-content/60 hover:text-primary border border-dashed border-base-300 shrink-0">
            ...
            <template x-if="authed">
              <div>
                <form @submit.prevent="addTodo()" class="flex gap-2 mb-4">
                  ...
                  <button type="submit" class="btn btn-primary btn-sm gap-1" :disabled="!todoDraft.trim()">
                ...
                <div x-show="!todoLoading" class="flex flex-col gap-1">
                  ...
                  <p x-show="!todoOpen.length && !todoDone.length" class="text-sm text-base-content/40 italic px-2 py-6 text-center">
                  ...
                  <div x-show="todoDone.length" class="mt-3 pt-2 border-t border-base-300/60">
                    <button @click="todoShowDone = !todoShowDone"
                            class="flex items-center gap-1 text-sm text-base-content/50 hover:text-base-content/80 px-2 mb-1">
                      <i class="ph text-xs" :class="todoShowDone ? 'ph-caret-down' : 'ph-caret-right'"></i>
                ...
                <div x-show="todoErr" class="text-sm text-error font-mono mt-2" x-text="todoErr"></div>
              </div>
            </template>
          </div>
          ...
          <div x-show="tab==='activity'">
            <div class="flex items-center gap-2 mb-4" x-show="authed">
              ...
              <span x-show="openBranches.length" class="text-sm font-mono text-base-content/40"
                    x-text="openBranches.length"></span>
              <span x-show="activityGeneratedAt" class="hidden sm:inline text-sm text-base-content/45"
              ...
              <template x-for="row in openBranches" :key="row.repo + '/' + row.name">
                <div class="rounded-lg border-l-4 pl-3 pr-3 py-2 transition-colors hover:brightness-[1.02]"
                  ...
                  <div class="flex items-center gap-2 min-w-0">
                    <span class="font-mono text-sm text-base-content/45 shrink-0" x-text="repoShort(row.repo)"></span>
                    ...
                    <template x-if="row.pr">
                      <a :href="prUrl(row.repo, row.pr.number)" target="_blank" :title="row.pr.title"
                         class="badge badge-sm badge-outline gap-1 shrink-0"
                         :class="row.pr.draft ? 'badge-warning' : 'badge-success'">
                        <span x-text="'#' + row.pr.number"></span>
                        <span x-text="row.pr.draft ? 'draft' : 'ready'"></span>
                      </a>
                    </template>
                    <span x-show="!row.pr" class="badge badge-sm badge-ghost shrink-0">no PR</span>
                    <div class="grow"></div>
                    <span x-show="row.date" class="text-sm text-base-content/45 shrink-0 tabular-nums"
                          :title="agoOf(row.date)" x-text="agoShort(row.date)"></span>
                  </div>
                  <p x-show="row.subject" class="text-sm text-base-content/55 truncate mt-0.5"
                     :title="row.subject" x-text="row.subject"></p>
                  <div class="flex items-center flex-wrap gap-x-3 gap-y-1 mt-1.5 text-sm">
                    <button @click="stageBranchDiff(row.repo, row.name, row.def)" :disabled="isStaging(row.repo, row.name)"
                            class="flex items-center gap-1 text-base-content/55 hover:text-primary transition-colors disabled:opacity-60"
                            title="Stage this branch's changed files">
                      <i class="ph" :class="isStaging(row.repo, row.name) ? 'ph-circle-notch animate-spin' : 'ph-stack'"></i>Stage</button>
                    <a :href="treeUrl(row.repo, row.name)" target="_blank"
                       class="flex items-center gap-1 text-base-content/55 hover:text-primary transition-colors">
                    ...
                    <a x-show="row.pr" :href="row.pr ? prUrl(row.repo, row.pr.number) : '#'" target="_blank"
                       class="flex items-center gap-1 text-base-content/55 hover:text-primary transition-colors">
                      <i class="ph ph-compass"></i>PR</a>
                    <a x-show="row.pr && row.pr.session" :href="row.pr?.session" target="_blank"
                       class="flex items-center gap-1 text-base-content/55 hover:text-primary transition-colors">
                      <i class="ph ph-robot"></i>Session</a>
                    <button @click="openRepoBranches(row.repo)"
                            class="flex items-center gap-1 text-base-content/40 hover:text-primary transition-colors ml-auto"
                            title="All branches in this repo">
                      <i class="ph ph-list-magnifying-glass"></i>Branches</button>
                  </div>
                </div>
              </template>
            </div>
      ...
      // Every branch in flight across the estate, unioned by repo+name and
      // freshest first: an active branch (recent work ahead of the default) or
      // the head of an open PR. Landed/stranded older branches are the per-repo
      // Branches drill-down's job, not this cross-repo "what's open" read. Each
      // row carries its open PR when one matches (joined by pr.head === branch),
      // so the caption-style link cluster can point at the PR and the authoring
      // session with no extra fetch.
      get openBranches(){
        const out = [];
        for (const [repo, e] of Object.entries(this.activity)){
          ...
          for (const b of (e.survey?.branches || [])){
            if (b.group !== 'active' || b.name === def) continue;
            seen.add(b.name);
            out.push({ repo, def, name: b.name, date: b.date || '', subject: b.subject || '', pr: prByHead.get(b.name) || null });
          }
          // An open PR whose branch fell outside the recent window is still live
          // work, so surface it even though the survey did not tag it active.
      ...
      // draft PR (the normal in-flight state), or a live branch with no PR yet.
      ...
      async stageBranchDiff(repo, name, def){
        ...
        try {
          ...
          const cmp = await gh.compare(def || 'main', name);
          const paths = (cmp.files || []).filter(f => f.status !== 'removed').map(f => f.filename);
          if (!paths.length){ toast?.('git-branch', 'No changed files to stage on ' + name, 'alert-info', 2600); return; }
