document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    // (`estate.nest`: a companion repo as one extra, unlabeled action row
    // inside its parent's card — no header, identity only in the leading
    // glyph and tooltips). Curated surfaces from the registry's surfaces/ dir
    // render beneath. Public (no token): the public default only, and no
    // surfaces. See docs/show-repo.md "The estate".
    const KIND_ICONS = {
    ...
    return {
                ...
                <template x-for="e in row" :key="e.repo">
                  <div class="card bg-base-100 border border-base-300 shadow-sm hover:border-primary/40 transition-colors
                    ...
                    <div class="card-body p-4 gap-1.5">
                      ...
                      <!-- Nested companion repo (estate.nest): no header, no
                           card of its own — just one more, dimmer action row
                           under the parent's, its identity carried by the
                           small leading glyph and the tooltips. -->
                      <template x-if="e.child">
                        <div class="flex flex-wrap items-center gap-1 opacity-75">
                          <button @click="openRepo(e.child.repo)"
                                  class="text-base-content/40 hover:text-primary transition-colors shrink-0 px-0.5 cursor-pointer"
                                  :title="e.child.repo.split('/')[1]">
                            <i class="ph text-sm leading-none" :class="e.child.icon"></i></button>
                          <template x-for="p in (e.child.pins || [])" :key="p">
                            <button @click="openRepoAt(e.child, p)"
                                    class="badge badge-sm badge-ghost gap-1 font-mono cursor-pointer
                                           hover:bg-primary/10 hover:text-primary transition-colors"
                                    :title="e.child.repo.split('/')[1] + ': ' + p">
                              <i class="ph text-[10px]" :class="pinIsFile(p) ? 'ph-file' : 'ph-folder'"></i>
                              <span x-text="pinLabel(p)"></span>
                            </button>
                          </template>
                          <div class="grow"></div>
                          <button x-show="authed" @click="openRepoConfig(e.child)"
                                  class="btn btn-ghost btn-xs btn-square opacity-40 hover:opacity-100"
                                  :title="e.child.repo.split('/')[1] + ': edit web-tools config'">
                            <i class="ph ph-gear-six"></i></button>
                          <a :href="'https://github.com/' + e.child.repo" target="_blank" @click.stop
                             class="btn btn-ghost btn-xs btn-square opacity-40 hover:opacity-100"
                             :title="e.child.repo.split('/')[1] + ' on GitHub'">
                            <i class="ph ph-github-logo"></i></a>
                          <button x-show="e.child.hasLanding" @click="openRepo(e.child.repo)"
                                  class="btn btn-ghost btn-xs btn-square opacity-40 hover:opacity-100"
                                  :title="e.child.repo.split('/')[1] + ': landing'"><i class="ph ph-planet"></i></button>
                          <button @click="openRepoView(e.child, 'files')"
                                  class="btn btn-ghost btn-xs btn-square opacity-40 hover:opacity-100"
                                  :title="e.child.repo.split('/')[1] + ': files'"><i class="ph ph-folders"></i></button>
                          <button @click="openRepoView(e.child, 'atlas')"
                                  class="btn btn-ghost btn-xs btn-square opacity-40 hover:opacity-100"
                                  :title="e.child.repo.split('/')[1] + ': atlas'"><i class="ph ph-map-trifold"></i></button>
                        </div>
                      </template>
      ...
      async load(){
        ...
        this.entries = list.map(e => ({
          ...
          meta: null, err: false, pins: [], hasLanding: false, child: null,
        }));
