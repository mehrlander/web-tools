...
${opts.nested ? '' : `
                    ...
                    <button @click.stop="stageBranchDiff(row.repo, row.name, row.def)"
                            ...
                            :data-note="'Stage the files this branch changed against ' + row.def + ', on the Stage'"
                            data-note-bare
        ...
        for (const [repo, e] of Object.entries(this.activity)){
          ...
          for (const b of (e.scan?.branches || [])){
            ...
            out.push({ repo, def, name: b.name, date: b.date || '', subject: b.subject || '', pr,
                       ...
                       assistant: window.assistantMark?.classify?.({ name: b.name, session: b.session || pr?.session, subject: b.subject || pr?.title, pr }) || 'human',
          ...
          for (const p of (e.openPRs || [])){
            ...
            out.push({ repo, def, name: p.head, date: p.updatedAt || '', subject: p.title || '', pr: p,
                       ...
                       assistant: window.assistantMark?.classify?.({ name: p.head, session: p.session, subject: p.title, pr: p }) || 'human',
