                      ...
                      <div class="mb-1.5">
                        ...
                        <div class="grid gap-x-4 gap-y-0.5" style="grid-template-columns:repeat(auto-fill,minmax(20rem,1fr))">
                          <template x-for="it in g.items" :key="it.id">
                            <div class="flex items-start gap-2 px-2 py-1 rounded-lg transition-colors duration-75 hover:bg-primary/10 group min-w-0">
                              ...
                              <button type="button" @click="startPinEdit(it)" x-show="editId !== it.id"
                                      class="opacity-0 group-hover:opacity-100 [@media(hover:none)]:opacity-100 text-base-content/30 hover:text-primary transition-opacity duration-75 shrink-0 mt-0.5"
                              ...
                              <button type="button" @click="deletePin(it)" x-show="editId !== it.id"
                                      class="opacity-0 group-hover:opacity-100 [@media(hover:none)]:opacity-100 text-base-content/30 hover:text-error transition-opacity duration-75 shrink-0 mt-0.5"
                  ...
                  <div x-show="!todoLoading" class="flex-1 min-h-0 overflow-y-auto -mx-1 px-1">
                    <div class="flex flex-col gap-1">
                      ...
                      <!-- THE ROW TINT WAS SPECIFIED TOO QUIETLY TO SEE. Every
                           ...
                           nudge. base-300 is a 6.7-point drop, which is a
                           legible wash rather than a block of colour.
                      ...
                      <template x-for="it in todoOpen" :key="it.id">
                        <div class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg border-l-4 transition-colors duration-75 group"
                          ...
                          <!-- The checkbox in its OWN label. p-1.5 -m-1.5 buys a
                               generous tap target without moving anything: the
                               padding grows the hit box and the negative margin
                               hands the space straight back to the layout. -->
                          <label class="flex items-center p-1.5 -m-1.5 cursor-pointer shrink-0">
                          ...
                          <button type="button" @click="startTodoEdit(it)" x-show="editId !== it.id"
                                  class="opacity-0 group-hover:opacity-100 [@media(hover:none)]:opacity-100 text-base-content/30 hover:text-primary transition-opacity duration-75 shrink-0"
                                  title="Edit"><i class="ph ph-pencil-simple"></i></button>
                          <button type="button" @click="deleteTodo(it)" x-show="editId !== it.id"
                                  class="opacity-0 group-hover:opacity-100 [@media(hover:none)]:opacity-100 text-base-content/30 hover:text-error transition-opacity duration-75 shrink-0"
                        ...
                        <template x-if="todoShowDone">
                          <div class="flex flex-col gap-1">
                            ...
                            <template x-for="it in todoDone" :key="it.id">
                              <div class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg border-l-4 border-transparent transition-colors duration-75 hover:bg-primary/10 group">
                                <label class="flex items-center p-1.5 -m-1.5 cursor-pointer shrink-0">
                                  <input type="checkbox" :checked="!!it.done" @change="toggleTodo(it)"
                                         class="checkbox checkbox-sm">
                                </label>
                  ...
                  <div x-show="!jotLoading" class="flex-1 min-h-0 overflow-y-auto -mx-1 px-1">
                    <div class="flex flex-col gap-1">
                      ...
                      <template x-for="it in jotPile" :key="it.id">
                        <div class="flex items-start gap-2.5 px-2 py-1.5 rounded-lg transition-colors duration-75 hover:bg-primary/10 group">
