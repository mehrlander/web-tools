...
${opts.nested ? '' : `
    ...
    return {
            ...
            <div x-show="authed && sessionLens === 'table'" class="flex flex-col min-w-0"
              ...
              <div class="flex items-center gap-2 mb-2 flex-wrap">
                ...
                <label class="input input-sm flex items-center gap-1.5 grow min-w-32 max-w-64">
                  ...
                  <input type="search" x-model="tableQuery" @input.debounce.200ms="applyTableQuery()"
                         placeholder="Filter any column" class="grow text-base" aria-label="Filter rows">
                </label>
              ...
              <!-- NO HEIGHT CLASS. This carried h-[max(26rem,calc(100dvh-18rem))]
                   and that arbitrary value generated no rule at all, so the host
                   took its content height (1863px inside a 743px scroller) and
                   the PAGE scrolled while the table did not. Tabulator is given
                   a measured pixel height instead: see tableHeight, which reads
                   the scroller's own bottom rather than assuming how much chrome
                   sits above. -->
              <div x-ref="tableHost" class="min-w-0"></div>
            </div>
      ...
      // DURATION, ON A LOG SCALE, and the distribution is the reason. `mins` is
      // the span from the opening ask to the last activity, and across the
      // store it runs 4 minutes to 35,826: the median is 527 and the 99th
      // percentile 5,570, so a linear bar puts half the rows under 1.5% of the
      // width and spends the rest on a single session someone left open for
      // 25 days. Log against the WHOLE set rather than the filtered view, so a
      // bar means the same thing before and after a filter; scaling to what is
      // on screen would redraw every row on every keystroke and compare rows
      // only to their neighbours.
      //
      // It measures elapsed time and not work: a session open across two nights
      // outranks an hour of hard editing. That is why Calls sits beside it
      // rather than under it, and why neither is called "effort".
      barCell(value, max, label, tone){
        const v = Math.max(0, Number(value) || 0);
        const pct = max > 0 ? Math.round(100 * Math.log1p(v) / Math.log1p(max)) : 0;
        const fill = tone || 'bg-primary';
        return '<div class="flex items-center gap-2">'
             + '<div class="h-1.5 w-12 shrink-0 rounded-full bg-base-content/10 overflow-hidden">'
             + '<div class="h-full rounded-full ' + fill + '" style="width:' + pct + '%"></div></div>'
             + '<span class="tabular-nums text-base-content/70">' + window.esc(label) + '</span></div>';
      },
      ...
      chipCell(text, cls){
        ...
        return '<span class="flex items-center gap-1 overflow-hidden">' + parts.map(p =>
          '<span class="badge badge-sm badge-ghost font-normal shrink-0 ' + (cls || '') + '">'
      ...
      NUM_FILTER: { headerFilter: 'number', headerFilterPlaceholder: '≥',
                    headerFilterFunc: '>=', hozAlign: 'right', width: 92 },
      get tableColumns(){
        const G = this.sessionGrain;
        const rows = this.grainRows;
        const maxOf = (k) => rows.reduce((m, r) => Math.max(m, Number(r[k]) || 0), 0);
        ...
        const text = (title, field, minWidth) => ({
          title, field, minWidth, headerFilter: 'input',
          formatter: (c) => '<span class="block truncate">' + window.esc(c.getValue() || '') + '</span>',
        });
        const dateCol = { title: 'Last', field: 'at', width: 116, headerFilter: 'input',
          headerFilterPlaceholder: 'date',
          formatter: (c) => '<span class="tabular-nums text-base-content/60">'
                          + window.esc((c.getValue() || '').slice(0, 10)) + '</span>' };

        if (G === 'branch') {
          const maxDays = maxOf('days');
          return [
            { title: 'Repo', field: 'repo', width: 150, headerFilter: 'list',
              headerFilterParams: { valuesLookup: true, clearable: true },
              formatter: (c) => this.chipCell(c.getValue()), dflt: true },
            { ...text('Branch', 'name', 240),
              formatter: (c) => '<span class="block truncate font-mono">' + window.esc(c.getValue() || '') + '</span>',
              dflt: true },
            { title: 'State', field: 'group', width: 120, headerFilter: 'list',
              headerFilterParams: { valuesLookup: true, clearable: true },
              formatter: (c) => this.chipCell(c.getValue()), dflt: true },
            { ...dateCol, dflt: true },
            { ...this.NUM_FILTER, title: 'Open', field: 'days', sorter: 'number',
              width: 156, hozAlign: 'left',
              formatter: (c) => this.barCell(c.getValue(), maxDays,
                                             c.getValue() ? c.getValue() + 'd' : 'same day'), dflt: true },
            { title: 'PR', field: 'pr', width: 100, sorter: 'number', headerFilter: 'input',
              formatter: (c) => {
            ...
            { title: 'PR state', field: 'prState', width: 110, headerFilter: 'list',
              headerFilterParams: { valuesLookup: true, clearable: true },
        ...
        if (G === 'edge') {
          return [
            this.stateColumn(),
            { title: 'Session', field: 'id', width: 118, headerFilter: 'input',
              formatter: (c) => '<span class="font-mono">' + window.esc(c.getValue() || '') + '</span>', dflt: true },
            { ...text('Name', 'title', 220), dflt: true },
            { title: 'Repo', field: 'repo', width: 150, headerFilter: 'list',
              headerFilterParams: { valuesLookup: true, clearable: true },
              formatter: (c) => this.chipCell(c.getValue()), dflt: true },
            { ...text('Branch', 'branch', 240),
              formatter: (c) => '<span class="block truncate font-mono">' + window.esc(c.getValue() || '') + '</span>',
              dflt: true },
            { title: 'Branch state', field: 'group', width: 130, headerFilter: 'list',
              headerFilterParams: { valuesLookup: true, clearable: true },
              formatter: (c) => this.chipCell(c.getValue()), dflt: false },
            num('Lines', 'lines', { dflt: true }),
            { ...dateCol, dflt: true },
            { ...text('Ask', 'ask', 280), dflt: false },
          ];
        }

        const maxMins = maxOf('mins'), maxCalls = maxOf('calls');
        return [
          this.stateColumn(),
          { title: 'Session', field: 'id', width: 118, headerFilter: 'input',
            formatter: (c) => '<span class="font-mono">' + window.esc(c.getValue() || '') + '</span>', dflt: true },
          { ...text('Name', 'title', 220), dflt: true },
          { ...dateCol, dflt: true },
          { ...this.NUM_FILTER, title: 'Ran', field: 'mins', sorter: 'number',
            width: 168, hozAlign: 'left',
            formatter: (c) => this.barCell(c.getValue(), maxMins, this.durLabel(c.getValue() || 0)),
            dflt: true },
          // A WIDTH, not a minWidth, and the difference is the whole layout.
          // fitDataFill pours the container's slack into whichever column can
          // grow, so the one flexible column here took 490px and pushed Failed
          // off the right edge on a 1500px screen. Fixed at three chips wide,
          // the default set totals 1258 and fits with room over.
          { title: 'Repos', field: 'repos', width: 300, headerFilter: 'input',
            formatter: (c) => this.chipCell(c.getValue()), dflt: true },
          ...
          { ...this.NUM_FILTER, title: 'Calls', field: 'calls', sorter: 'number',
            width: 152, hozAlign: 'left',
            formatter: (c) => this.barCell(c.getValue(), maxCalls,
                                           this.shortNum(c.getValue()), 'bg-base-content/40'),
            dflt: true },
          num('Failed', 'failures', { dflt: true, width: 92,
          ...
          { title: 'Tools', field: 'tools', minWidth: 240, headerFilter: 'input',
            formatter: (c) => this.chipCell(c.getValue()), dflt: false },
          { title: 'Skills', field: 'skills', minWidth: 180, headerFilter: 'input',
            formatter: (c) => this.chipCell(c.getValue()), dflt: false },
          { ...text('Ask', 'ask', 280), dflt: false },
        ];
      },
      ...
      stateColumn(){
        return { title: 'State', field: 'state', width: 92, headerFilter: 'list',
          headerFilterParams: { valuesLookup: true, clearable: true },
          hozAlign: 'center',
          formatter: (c) => {
      ...
      // never even generated a rule, so the table grew to its content and the
      // page scrolled instead of the table.
      //
      ...
      // One listener for the component's life, not one per mount: mountTable
      // runs on every grain change and destroys the table each time, so
      // attaching here would stack a listener per switch.
      watchTableHeight(){
        if (this._tableResize) return;
        this._tableResize = () => {
          if (!this.tabulator) return;
          try { this.tabulator.setHeight(this.tableHeight()); } catch {}
        };
        window.addEventListener('resize', this._tableResize);
      },
      _tableResize: null,
      ...
      tableShown: 0,
      // The global box, kept beside the per-column filters rather than replaced
      // by them: the column filters are how you ask a precise question, and this
      // is how you find a word when you do not yet know which column holds it.
      applyTableQuery(){
        const t = this.tabulator;
        if (!t) return;
        const q = this.tableQuery.trim().toLowerCase();
        const fields = this.tableColumns.filter(c => this.tableCols.includes(c.field)).map(c => c.field);
        if (!q) { t.clearFilter(true); return; }
        const terms = q.split(/\s+/);
        t.setFilter((data) => {
          const hay = fields.map(f => String(data[f] ?? '')).join(' ').toLowerCase();
