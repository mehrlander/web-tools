                ...
                <div class="rounded-lg border-l-4 pl-3 pr-3 py-2 transition-colors hover:brightness-[1.02]"
                  ...
                  <div class="flex items-center gap-2 min-w-0">
                    ...
                    <a x-show="row.pr" :href="row.pr ? prUrl(row.repo, row.pr.number) : '#'" target="_blank"
                       ...
                       class="font-mono text-base font-medium shrink-0 hover:underline"
                       :class="row.pr?.draft ? 'text-warning' : 'text-success'"
                  ...
                  <div class="flex items-center flex-wrap gap-x-4 gap-y-1.5 mt-2 text-base">
                    <button @click="stageBranchDiff(row.repo, row.name, row.def)" :disabled="isStaging(row.repo, row.name)"
                            class="flex items-center gap-1.5 text-base-content/70 hover:text-primary transition-colors disabled:opacity-60"
                            title="Stage this branch's changed files">
                      <i class="ph text-lg" :class="isStaging(row.repo, row.name) ? 'ph-circle-notch animate-spin' : 'ph-stack'"></i>Stage</button>
                    ...
                    <span x-show="row.ahead !== null || row.behind !== null"
                          class="ml-auto flex items-center gap-2.5 font-mono tabular-nums"
