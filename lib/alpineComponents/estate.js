document.addEventListener('alpine:init', function() {
  ...
  Alpine.data('estate', function() {
    ...
    const ACTIVITY_TABS = ['activity', 'sessions', 'writes', 'chats', 'state'];
...
${opts.nested ? '' : `
                    ...
                    <a x-show="row.session" :href="row.session" target="_blank""
                      ...
                      <span class="flex shrink-0"
                            x-html="window.claudeMark.svg({ cls: 'w-4 h-4 shrink-0' })"></span><span
                    ...
                    <!-- ── THE PAGES, AS SOMETHING TO OPEN ────────────────
                         Ahead of the routes and behind everything fixed, which
                         is the same width rule the routes carry: both are a
                         chip per changed thing, so both belong at the end where
                         a wrap displaces nothing. Pages lead the pair because
                         they are the branch's own files, where a route chip is
                         the deployed app running the branch's library.

                         The chip is branch-brief's chip exactly, down to the
                         disc and the mono label, because it is the same act one
                         level up: the branch detail has offered this row since
                         2026-09-07 and its own note says the link "was still
                         asked for in chat every time". A tile under a session
                         card is where that asking actually happens.

                         THE REF IS THE BRANCH, not row.sha, and it differs from
                         the route chip beside it on purpose. A route pins with
                         ?use=, which interpolates the ref into a
                         raw.githubusercontent path where a slashed branch name
                         breaks; a toss address takes the ref whole (git forbids
                         ':' in a ref, which is what makes the grammar
                         unambiguous), so it can name the branch. What a reader
                         taps this for is where the work GOT to, which is the
                         tip and not the commit the last crawl happened to see.
                         The one case it loses is a branch deleted since that
                         crawl, where the row itself is describing something
                         that is gone. -->
                    <!-- THREE, THEN A NOTE. Measured inside a session card's
                         tile at 390px, where this box is 223px wide and a mono
                         chip runs 100 to 170px: three chips is two lines and the
                         six a branch may carry would be four, in a tile whose
                         whole height is six. The overflow takes the slot and the
                         wording rule the route chips' "+N shared" already uses,
                         and points at the one surface that draws the whole set.
                         A scroller was the other option and is branch-brief's,
                         where the row is the full width of the page; at 223px it
                         would hide most of the strip behind a swipe. -->
                    <template x-if="branchPageStrip(row).chips.length">
                      <span class="flex items-center gap-1 flex-wrap min-w-0">
                        <template x-for="t in branchPageStrip(row).chips" :key="t.addr">
                          <a :href="t.url" target="_blank" rel="noopener" @click.stop
                             :title="t.title"
                             class="shrink-0 rounded-full px-2 py-0.5 text-sm font-mono
                                    bg-base-200/70 hover:bg-base-200 transition-colors
                                    flex items-center gap-1">
                            <span class="text-[13px] leading-none" x-text="t.mark"></span><span
                              x-text="t.short || t.label"></span></a>
                        </template>
                        <span x-show="branchPageStrip(row).more"
                              :data-note="branchPageStrip(row).more + ' more page(s) this branch changed. The branch itself draws the whole set: open it from its name.'"
                              class="shrink-0 rounded-full px-2 py-0.5 text-sm
                                     bg-base-200/70 text-base-content/40"
                              x-text="'+' + branchPageStrip(row).more"></span>
                      </span>
                    </template>
    ...
    return {
        ...
        <div :class="tab==='lists' && 'flex-1 min-h-0 flex flex-col'">
          ...
          <div class="flex-1 min-w-0" :class="tab==='sessions' ? '' : 'hidden'"
            ...
            <div x-show="authed && sessionLens === 'list' && sessionNodes.length && sessionRailSpan.ms"
              ...
              <div class="flex justify-between mt-0.5 font-mono text-[9px] leading-none text-base-content/30 tabular-nums">
            ...
            <div x-show="authed && sessionLens === 'list' && sessionNodes.length" class="flex flex-col gap-2">
              <template x-for="n in sessionNodes" :key="n.key">
                ...
                <div x-data="{ get row(){ return n.row } }">
                  ...
                  <div x-show="sessionRailSpan.ms && row.started" class="mt-2 -mb-0.5">
                    <div class="relative h-3" :data-note="sessionRailNote(row)">
                      ...
                      <template x-for="(p, i) in sessionRailTicks(row)" :key="i">
                        <div class="absolute top-1/2 w-px h-2 -translate-y-1/2 bg-base-content/40"
                             :style="'left:' + p + '%'"></div>
                      </template>
                    </div>
                  </div>
              ...
              <div x-show="sessionOrphans.length || sessionOrphansOutOfWindow"
                ...
                <div x-show="orphansOpen && sessionOrphans.length" x-cloak class="flex flex-col gap-1 mt-2">
                  ...
                  <button type="button" @click="goSub('activity')"
              ...
              <div class="flex-1 min-h-0 flex flex-col">
                      ...
                      <div class="mb-1.5">
                        ...
                        <div class="grid gap-x-4 gap-y-0.5" style="grid-template-columns:repeat(auto-fill,minmax(20rem,1fr))">
                          <template x-for="it in g.items" :key="it.id">
                            <div class="flex items-start gap-2 px-2 py-1 rounded-lg transition-colors hover:bg-base-300/60 group min-w-0">
                              <i class="ph ph-push-pin text-base-content/30 group-hover:text-base-content/60 transition-colors mt-0.5 shrink-0"></i>
                              <div class="flex-1 min-w-0">
                                ...
                                <template x-if="editId === it.id">
                                  <div class="flex flex-col gap-1 py-0.5">
                                    <input x-model="editDraft" x-init="$el.focus()" placeholder="title"
                                           @keydown.enter.prevent="commitPinEdit(it)"
                                           @keydown.escape.prevent.stop="cancelEdit()"
                                           class="input input-bordered input-sm w-full">
                                    <input x-model="editNote" placeholder="note"
                                           @keydown.enter.prevent="commitPinEdit(it)"
                                           @keydown.escape.prevent.stop="cancelEdit()"
                                           class="input input-bordered input-sm w-full">
                              ...
                              <button type="button" @click="startPinEdit(it)" x-show="editId !== it.id"
                                      class="opacity-0 group-hover:opacity-100 [@media(hover:none)]:opacity-100 text-base-content/30 hover:text-primary transition-opacity shrink-0 mt-0.5"
                ...
                <section class="flex-1 min-h-0 flex flex-col">
                  ...
                  <div x-show="!todoLoading" class="flex-1 min-h-0 overflow-y-auto -mx-1 px-1">
                    <div class="flex flex-col gap-1">
                      ...
                      <!-- THE ROW TINT WAS SPECIFIED TOO QUIETLY TO SEE. Every
                           row in this view carried hover:bg-base-200/60 until
                           2026-09-15, and the reader reported that only the pin
                           appeared to respond. It was not a missing rule: in
                           winter, base-200 is 97.466% lightness against a
                           base-100 of pure white, so at 60% the hover moved the
                           row by 1.5 points and landed under the threshold
                           where an eye picks it up. The pin read because its
                           TITLE carries a second hover, hover:text-primary,
                           which is a chroma jump of 0.2 rather than a lightness
                           nudge. base-300/60 is a 4.0-point drop, still a wash
                           rather than a block of colour.

                           Three signals, all in channels the row already owns:
                           the tint, the left rail (border-l-4 is reserved on
                           every row for the urgent state, so colouring it on
                           hover costs no layout), and the leading icon
                           brightening. transition-colors is what makes the set
                           read as one gesture instead of three snaps.

                           Tint and rail ride the BINDING, not the static list,
                           because a hover:bg-* in the static class would win
                           over the urgent row's bg-error/10 and turn a flagged
                           row grey under the pointer. The hot branch keeps its
                           own colour and deepens it instead. Note also that
                           these are hover:, not group-hover:, since the group
                           element cannot match its own group-hover variant. -->
                      <template x-for="it in todoOpen" :key="it.id">
                        <label class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg border-l-4 transition-colors group"
                               :class="isHot(it) ? 'border-error bg-error/10 hover:bg-error/20'
                                                : 'border-transparent hover:border-base-300 hover:bg-base-300/60'">
                          <input type="checkbox" :checked="!!it.done" @change="toggleTodo(it)"
                                 class="checkbox checkbox-sm">
                          <span x-show="editId !== it.id" class="text-base flex-1 min-w-0"
                          ...
                          <!-- The edit field, mounted by x-if rather than
                               ...
                               opened gets a new element and a new focus.
                               @click.stop keeps a tap in the field from
                               reaching the wrapping label, which would toggle
                               done under the reader mid-typo. -->
                          <template x-if="editId === it.id">
                            <input x-model="editDraft" x-init="$el.focus()" @click.stop
                          ...
                          <template x-if="editId === it.id">
                            <button type="button" @click="cancelEdit()" title="Discard"
                                    class="text-base-content/40 hover:text-error shrink-0">
                              <i class="ph ph-x"></i></button>
                          </template>
                          ...
                          <span x-show="editId !== it.id" class="relative shrink-0 flex items-center">
                          ...
                          <button type="button" @click="startTodoEdit(it)" x-show="editId !== it.id"
                                  class="opacity-0 group-hover:opacity-100 [@media(hover:none)]:opacity-100 text-base-content/30 hover:text-primary transition-opacity shrink-0"
                        ...
                        <template x-if="todoShowDone">
                          <div class="flex flex-col gap-1">
                            ...
                            <template x-for="it in todoDone" :key="it.id">
                              <label class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg border-l-4 border-transparent transition-colors hover:border-base-300 hover:bg-base-300/60 group">
                                <input type="checkbox" :checked="!!it.done" @change="toggleTodo(it)"
                                       class="checkbox checkbox-sm">
                ...
                <section class="flex-1 min-h-0 flex flex-col border-t border-base-300/60 mt-3 pt-3">
                  ...
                  <div x-show="!jotLoading" class="flex-1 min-h-0 overflow-y-auto -mx-1 px-1">
                    <div class="flex flex-col gap-1">
                      <!-- border-l-4 is new here rather than reserved: a jot has
                           no urgent state, so this row never needed the channel.
                           It takes it now for the hover rail, and the 4px it
                           costs is what finally lines a jot's icon up with a
                           to-do's checkbox directly above it. The done-to-do
                           rows already carry a transparent border for exactly
                           this reason. -->
                      <template x-for="it in jotPile" :key="it.id">
                        <div class="flex items-start gap-2.5 px-2 py-1.5 rounded-lg border-l-4 border-transparent transition-colors hover:border-base-300 hover:bg-base-300/60 group">
                          <i class="ph ph-lightbulb text-base-content/30 group-hover:text-base-content/60 transition-colors mt-1 shrink-0"></i>
                          ...
                          <span x-show="editId !== it.id" class="text-base flex-1" x-text="it.text"></span>
                          <template x-if="editId === it.id">
                            <input x-model="editDraft" x-init="$el.focus()"
                                   @keydown.enter.prevent="commitJotEdit(it)"
                                   @keydown.escape.prevent.stop="cancelEdit()"
                                   class="input input-bordered input-sm flex-1 min-w-0">
                          </template>
                          ...
                          <template x-if="editId === it.id">
                            <button type="button" @click="commitJotEdit(it)" title="Save"
                                    class="text-base-content/40 hover:text-primary shrink-0 mt-0.5">
                              <i class="ph ph-check"></i></button>
                          </template>
                          <template x-if="editId === it.id">
                            <button type="button" @click="cancelEdit()" title="Discard"
                                    class="text-base-content/40 hover:text-error shrink-0 mt-0.5">
                              <i class="ph ph-x"></i></button>
                          </template>
      ...
      paneNeeds(tab){
        return {
          activity: tab === 'repos' || tab === 'activity' || tab === 'sessions' || tab === 'writes',
      ...
      sessionRailNote(row){
        if (!Array.isArray(row?.beats))
          return 'This row was summarised before turn times were carried (row version 22), '
               + 'so its rail shows when the session ran and not when it spoke. '
               + 'Refresh this pane to fill it in.';
        const sp = this.sessionRailSpan;
        const t0 = Date.parse(row.started || '');
        const before = row.beats.filter(m => t0 + m * 60000 < sp.a).length;
        const n = row.beats.length;
        return n + (n === 1 ? ' user turn' : ' user turns')
             + ', placed across ' + this.sessionRailWord
             + (before ? '. ' + before + ' of them fell before this window and are not drawn; '
                       + 'the session started earlier than the span shows.'
                       : '.');
      },
      ...
      startTodoEdit(it){ this.editId = it.id; this.editDraft = it.text || ''; },
      startJotEdit(it){
        this.editId = it.id;
