document.addEventListener('alpine:init', function() {
  Alpine.data('proposals', function() {
    ...
    return {
              ...
              <div class="card bg-base-100 border border-base-300 shadow-sm">
                <div class="p-3 sm:p-4 flex flex-col gap-2">
                  ...
                  <div class="flex items-start gap-2 flex-wrap">
                    <i class="ph text-lg mt-0.5" :class="p.kind === 'delete-issue' ? 'ph-trash' : p.kind === 'put-file' ? 'ph-file-plus' : p.kind === 'unset-json-field' ? 'ph-eraser' : 'ph-brackets-curly'"></i>
                    <div class="min-w-0">
                      <div class="font-mono text-sm break-all">
                        <span x-text="p.repo" class="opacity-60"></span><span class="opacity-40" x-text="p.isMutation ? '#' : ':'"></span><span x-text="p.isMutation ? p.issue : p.path"></span>
                      </div>
                  ...
                  <div x-show="p.stale || p.gone"
                    ...
                    <div class="font-medium" x-text="p.isMutation ? 'The issue changed after this was proposed.' : (p.gone ? 'The target file no longer exists.' : 'The target changed after this was proposed.')"></div>
                    <div class="text-xs opacity-70" x-text="staleDetail(p)"></div>
                  </div>

                  <template x-if="p.resolved && p.isMutation">
                    <div class="text-sm rounded-lg border border-error/30 bg-error/10 p-2 flex flex-col gap-1">
                      <div class="text-xs uppercase tracking-wide opacity-50">Will be deleted, permanently</div>
                      <div class="font-medium break-words" x-text="p.issueTitle || ('issue #' + p.issue)"></div>
                      <div class="text-xs opacity-70">
                        <span x-text="(p.issueState || '').toLowerCase()"></span><span x-show="p.issueState"> · </span>
                        <span x-text="p.issueComments"></span> comment<span x-show="p.issueComments !== 1">s</span>
                      </div>
                      <div class="text-xs opacity-60">GitHub keeps no tombstone: the number is not reused and the thread is not recoverable.</div>
                    </div>
                  </template>

                  <template x-if="p.resolved && !p.isMutation && p.kind !== 'put-file'">
                      ...
                      <div class="text-xs font-mono bg-base-200/50 rounded-lg p-2 max-h-64 overflow-auto">
                        <template x-for="(r, i) in p.diffRows" :key="i">
                          <div class="whitespace-pre px-1"
                               :class="r.t === 'add' ? 'bg-success/10 text-success' : r.t === 'del' ? 'bg-error/10 text-error' : 'text-base-content/50'"
                  ...
                  <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <template x-if="!p.done && p.isMutation && confirming !== p.id && applying !== p.id">
                      <button class="btn btn-sm gap-1.5 btn-outline btn-error"
                              :disabled="!!(!p.resolved || p.blocked)"
                              @click="arm(p, 'commit')">
                        <i class="ph" :class="p.stale ? 'ph-warning-octagon' : 'ph-trash'"></i>
                        <span x-text="p.stale ? 'Delete anyway' : 'Delete issue'"></span>
                      </button>
                    </template>
                    <template x-if="!p.done && !p.isMutation && confirming !== p.id && applying !== p.id">
      ...
      staleDetail(p) {
        // An issue's staleness is counted in comments, not in shas, so the
        // detail line has to speak that currency or it prints an empty prefix.
        if (p.isMutation) {
          return 'Written when it had ' + (p.expectComments ?? '?') + ' comment(s); it now has ' +
            (p.issueComments ?? '?') + '. Deleting is not reversible, so it is worth a look first.';
        }
      ...
      kindHelp(kind) {
        return kind === 'delete-issue' ? 'deletes the issue, permanently'
          : kind === 'put-file' ? 'replaces the whole file'
      ...
      async load() {
        ...
        try {
          ...
          for (const name of names) {
            ...
            row.isMutation = (P.MUTATION_KINDS || []).includes(rec.kind);
            row.targetGh = row.isMutation
              ? 'https://github.com/' + rec.repo + '/issues/' + rec.issue
              : 'https://github.com/' + rec.repo + '/blob/' + (rec.ref || 'HEAD') + '/' + rec.path;
            ...
            if (!r.ok) { row.resolveErr = r.error; row.checks = P.checks(rec, r).list; row.blocked = true; }
              ...
              // A mutation resolves to an object, so the card reads the issue's
              // identity off the resolution rather than off the record: the
              // point of resolving is that the reviewer sees it as it stands.
              if (row.isMutation && r.issue) {
                Object.assign(row, { issueTitle: r.issue.title, issueState: r.issue.state,
                  issueComments: r.issue.comments, issueUrl: r.issue.url });
              }
      ...
      async retire(p) {
        ...
        try {
          ...
          }, `Retire proposal ${p.id} (already applied) via Web Tools`);
        ...
        try {
          ...
          if (result.ok) {
            await this.reg().save(P.APPLIED_DIR + '/' + p.name, result,
              `Apply proposal ${p.id} (${p.repo}:${p.path}) via Web Tools`);
            ...
            try {
              await this.reg().save(P.ATTEMPT_DIR + '/' + p.id + '-' + stamp + '.json', result,
                `Record a failed apply of proposal ${p.id} via Web Tools`);
            } catch {}
          }
