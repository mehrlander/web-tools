...
`;
    ...
    return {
            ...
            <div class="min-w-0 flex flex-col">
              <h3 class="text-base font-mono uppercase tracking-widest text-base-content/40 mb-2 flex items-center gap-2">
                ...
                <button @click="fetchActivityGraphQL(graphQLRepo, true)" :disabled="graphQLLoading"
                  ...
                  <span class="hidden sm:inline" x-text="graphQLLoading ? 'Querying…' : 'Fetch live'"></span>
                </button>
              </h3>
              ...
              <div class="rounded-box border border-base-300 bg-base-100 p-3 shadow-sm flex flex-col gap-3">
                ...
                <div class="text-xs text-base-content/70 bg-base-200/50 rounded-lg p-2.5 flex items-start gap-2 leading-relaxed">
                  ...
                  <span>
                    This GraphQL query powers the branch and session discovery across the estate. In one round trip, it retrieves branches ordered chronologically by commit date (<code>TAG_COMMIT_DATE</code>) and inspects recent commit history (<code>depth: 8</code>) to extract <code>Claude-Session: &lt;url&gt;</code> git trailers, eliminating N+1 API calls and ensuring newly active branches are discovered immediately.
                  </span>
                </div>
                      ...
                      <template x-for="b in (currentGraphQL?.branches || [])" :key="b.name">
                        <tr class="border-b border-base-200/40 hover:bg-base-200/30 font-mono text-xs">
                          ...
                          <td class="whitespace-nowrap">
                            <template x-if="currentGraphQL?.sessions?.[b.name] || b.sessionUrl">
                              <a :href="currentGraphQL?.sessions?.[b.name] || b.sessionUrl" target="_blank" rel="noopener"
                                 class="badge badge-xs badge-primary badge-outline gap-1 font-mono hover:badge-primary"
                                 :title="currentGraphQL?.sessions?.[b.name] || b.sessionUrl">
                                <i class="ph ph-arrow-square-out text-[10px]"></i>
                                <span x-text="(currentGraphQL?.sessions?.[b.name] || b.sessionUrl).split('/').pop().slice(0, 15) + '…'"></span>
                              </a>
                            </template>
                            <template x-if="!currentGraphQL?.sessions?.[b.name] && !b.sessionUrl">
      ...
      init() {
        ...
        if (!this.graphQLRepo) {
          this.graphQLRepo = (typeof window !== 'undefined' && window.__lastActivityGraphQL?.repo) || REGISTRY();
        }
      ...
      get currentGraphQL() {
        const repo = this.graphQLRepo || REGISTRY();
        if (typeof window !== 'undefined') {
          return window.__graphQLByRepo?.[repo] ||
            (window.__lastActivityGraphQL?.repo === repo ? window.__lastActivityGraphQL : null) ||
      ...
      async fetchActivityGraphQL(repo, force = false) {
        const r = repo || this.graphQLRepo || REGISTRY();
        this.graphQLRepo = r;
        this.graphQLErr = '';
        if (!force && window.__graphQLByRepo?.[r]) return;
