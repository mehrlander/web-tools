document.addEventListener('alpine:init', function () {
  Alpine.data('stateView', function () {
      ...
      <div class="-my-0.5 flex flex-col gap-1">
        ...
        <div class="flex items-center gap-2">
          ...
          <span class="shrink-0 w-6 text-right text-[10px] leading-none text-base-content/40 tabular-nums"
                :data-note="pulseTitle(${r}, s)" data-note-bare
                x-text="spanLabel(s)"></span>
          <div class="relative h-3 flex-1 cursor-pointer"
            ...
            <!-- THIS RAIL'S CURSOR: zero-width span parked at the hovered
                 mark, opening the standard Note tooltip centered above it. -->
            <span data-rail-cursor data-note-bare
                  class="absolute top-0 bottom-0 w-0 pointer-events-none"></span>
          </div>
        </div>
    ...
    // THE CARD for the tick under the pointer was retired: hovering a tick
    // now opens a standard Note via data-rail-cursor at the hovered mark rather
    // than an inline note card under the rail.
    const PEEK = () => '';
    ...
    // anything. docs/views/state.md names them as deliberately not chips.
    ...
    const VIEWS = {
      ...
      branches: { label: 'Branches', icon: 'ph-git-branch',       go: 'goBranches' },
      activity: { label: 'Branches', icon: 'ph-git-branch',       go: 'goBranches' },
    ...
    const CARD = `
                <div :id="'state-' + r.key"
                  ...
                  <!-- WHO USES IT, as chips that go there. The prose this
                       ...
                       the view names is in docs/views/state.md. The two file
    ...
    return {
      ...
      template: `
        <div class="w-full">
          ...
          <!-- Responsive 2-column layout on desktop: derived caches on the left, activity discovery on the right -->
          ...
          <div class="flex flex-col gap-2">
            ...
            <!-- ── Activity Discovery (GraphQL Inspector) ────────────────── -->
            <div class="min-w-0 flex flex-col">
              <h3 class="text-base font-mono uppercase tracking-widest text-base-content/40 mb-2 flex items-center gap-2">
                <span>Discovery</span>
                <span class="badge badge-sm badge-ghost font-mono text-xs normal-case">GraphQL</span>
                <div class="grow"></div>
                <button @click="fetchActivityGraphQL(graphQLRepo, true)" :disabled="graphQLLoading"
                        class="btn btn-ghost btn-sm gap-1.5 text-primary" title="Send live GraphQL query to GitHub">
                  <i class="ph" :class="graphQLLoading ? 'ph-circle-notch animate-spin' : 'ph-arrows-clockwise'"></i>
                  <span class="hidden sm:inline" x-text="graphQLLoading ? 'Querying…' : (graphQLRepo === 'all' ? 'Fetch all live' : 'Fetch live')"></span>
                </button>
              </h3>

              <div class="rounded-box border border-base-300 bg-base-100 p-3 shadow-sm flex flex-col gap-3">
                <!-- Top bar: Operation name & Repo selector -->
                <div class="flex items-center justify-between flex-wrap gap-2 pb-2 border-b border-base-200">
                  <div class="flex items-center gap-2">
                    <i class="ph ph-brackets-curly text-primary text-lg"></i>
                    <span class="font-semibold text-base">BranchesDatedSessions</span>
                  </div>
                  <div class="flex items-center gap-2">
                    <label class="text-xs text-base-content/60">Repo:</label>
                    <select x-model="graphQLRepo" @change="fetchActivityGraphQL(graphQLRepo, false)"
                            class="select select-bordered select-xs font-mono">
                      <template x-for="r in graphQLReposList" :key="r">
                        <option :value="r" x-text="r === 'all' ? 'All Repositories (' + (graphQLReposList.length - 1) + ')' : (r.split('/')[1] || r)"></option>
                      </template>
                    </select>
                  </div>
                </div>
                ...
                <div class="text-xs text-base-content/70 bg-base-200/50 rounded-lg p-2.5 flex items-start gap-2 leading-relaxed">
                  <i class="ph ph-lightbulb text-warning text-base shrink-0 mt-0.5"></i>
                  <span>
                    GitHub's GraphQL schema scopes queries to a single repository (<code>repository(owner, name)</code>). To discover activity across the entire estate without N+1 REST calls, the crawler executes <code>BranchesDatedSessions</code> for each active repo in parallel. Each query inspects commit dates (<code>TAG_COMMIT_DATE</code>) and recent history (<code>depth: 8</code>) for <code>Claude-Session: &lt;url&gt;</code> git trailers.
                  </span>
                </div>

                <!-- Error message if any -->
                <div x-show="graphQLErr" class="rounded-lg bg-error/10 border border-error/30 text-error p-2.5 text-xs font-mono"
                     x-text="graphQLErr"></div>

                <!-- Metrics bar -->
                <div x-show="currentGraphQL" class="flex items-center flex-wrap gap-x-4 gap-y-1 text-xs text-base-content/60 font-mono py-1 border-b border-base-200">
                  <span>Queried: <span class="text-base-content font-medium" x-text="currentGraphQL?.at ? (new Date(currentGraphQL.at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit', second:'2-digit'})) : 'live'"></span></span>
                  <span x-show="currentGraphQL?.duration != null">Duration: <span class="text-base-content font-medium" x-text="(currentGraphQL?.duration ?? 0) + 'ms' + (currentGraphQL?.isAll ? ' (' + (currentGraphQL?.repoCount ?? 0) + ' repos)' : '')"></span></span>
                  <span>Branches: <span class="text-base-content font-semibold" x-text="currentGraphQL?.branches?.length ?? currentGraphQL?.branchesCount ?? 0"></span></span>
                  <span>Sessions linked: <span class="text-primary font-semibold" x-text="currentGraphQL?.sessions ? Object.keys(currentGraphQL.sessions).length : (currentGraphQL?.sessionsCount ?? 0)"></span></span>
                  <span x-show="currentGraphQL?.isAll">Scope: <span class="text-primary font-semibold" x-text="(currentGraphQL?.repoCount ?? 0) + ' repos'"></span></span>
                  <span>Order: <span :class="currentGraphQL?.ordered ? 'text-success' : 'text-warning'" x-text="currentGraphQL?.ordered ? 'TAG_COMMIT_DATE' : 'fallback'"></span></span>
                </div>

                <!-- Sub-tabs header & Copy button -->
                <div class="flex items-center justify-between border-b border-base-200 pb-1">
                  <div class="flex items-center gap-1">
                    <button @click="graphQLTab = 'response'"
                            class="px-2.5 py-1 text-xs font-medium rounded-md transition-colors"
                            :class="graphQLTab === 'response' ? 'bg-primary/10 text-primary font-semibold' : 'text-base-content/60 hover:text-base-content'">
                      Response JSON
                    </button>
                    <button @click="graphQLTab = 'extracted'"
                            class="px-2.5 py-1 text-xs font-medium rounded-md transition-colors"
                            :class="graphQLTab === 'extracted' ? 'bg-primary/10 text-primary font-semibold' : 'text-base-content/60 hover:text-base-content'">
                      Extracted Data (<span x-text="currentGraphQL?.branches?.length ?? currentGraphQL?.branchesCount ?? 0"></span>)
                    </button>
                    <button @click="graphQLTab = 'query'"
                            class="px-2.5 py-1 text-xs font-medium rounded-md transition-colors"
                            :class="graphQLTab === 'query' ? 'bg-primary/10 text-primary font-semibold' : 'text-base-content/60 hover:text-base-content'">
                      Query & Variables
                    </button>
                  </div>
                  <button @click="copyGraphQLText(graphQLTab === 'query' ? currentGraphQL?.query : (graphQLTab === 'extracted' ? currentGraphQL?.branches : currentGraphQL?.raw))"
                          class="btn btn-ghost btn-xs gap-1 text-xs text-base-content/60 hover:text-base-content"
                          title="Copy current tab contents">
                    <i class="ph" :class="graphQLCopied ? 'ph-check text-success' : 'ph-copy'"></i>
                    <span x-text="graphQLCopied ? 'Copied!' : 'Copy'"></span>
                  </button>
                </div>

                <!-- Sub-tab 1: Raw Response JSON -->
                <div x-show="graphQLTab === 'response'">
                  <div x-show="!currentGraphQL && !graphQLLoading" class="text-xs text-base-content/50 py-6 text-center">
                    <span x-text="graphQLRepo === 'all' ? 'No GraphQL response recorded yet. Click &quot;Fetch all live&quot; above to run the query across the estate.' : 'No GraphQL response recorded yet for this repo. Click &quot;Fetch live&quot; above to query GitHub.'"></span>
                  </div>
                  <div x-show="graphQLLoading" class="flex justify-center py-8">
                    <span class="loading loading-dots loading-sm opacity-40"></span>
                  </div>
                  <pre x-show="currentGraphQL && !graphQLLoading"
                       class="bg-base-200/80 rounded-lg p-3 text-xs font-mono max-h-[30rem] overflow-y-auto overflow-x-auto text-base-content/80 select-all leading-relaxed"
                       x-text="currentGraphQL?.raw ? JSON.stringify(currentGraphQL.raw, null, 2) : 'No raw response available'"></pre>
                </div>

                <!-- Sub-tab 2: Extracted Data Table -->
                <div x-show="graphQLTab === 'extracted'" class="overflow-x-auto max-h-[30rem] overflow-y-auto border border-base-200 rounded-lg">
                  <table class="table table-xs w-full">
                    <thead>
                      <tr class="text-base-content/60 bg-base-200/60 border-b border-base-200">
                        <th x-show="currentGraphQL?.isAll">Repo</th>
                        <th>Branch</th>
                        <th>Committed</th>
                        <th>Headline</th>
                        <th>Session Trailer</th>
                      </tr>
                    </thead>
                    <tbody>
                      <template x-for="b in (currentGraphQL?.branches || [])" :key="(b.repo || '') + '@' + b.name">
                        <tr class="border-b border-base-200/40 hover:bg-base-200/30 font-mono text-xs">
                          <td x-show="currentGraphQL?.isAll" class="text-base-content/60 font-sans font-medium" x-text="b.repo ? b.repo.split('/').pop() : ''"></td>
                          <td class="font-semibold text-primary" x-text="b.name"></td>
                          <td class="whitespace-nowrap text-base-content/60" x-text="b.ago || (b.committedDate ? b.committedDate.slice(0, 16).replace('T', ' ') : b.date)"></td>
                          <td class="truncate max-w-xs text-base-content/80 font-sans" :title="b.subject || b.headline" x-text="b.subject || b.headline"></td>
                          <td class="whitespace-nowrap">
                            <template x-if="b.sessionUrl">
                              <a :href="b.sessionUrl" target="_blank" rel="noopener"
                                 class="badge badge-xs badge-primary badge-outline gap-1 font-mono hover:badge-primary"
                                 :title="b.sessionUrl">
                                <i class="ph ph-arrow-square-out text-[10px]"></i>
                                <span x-text="b.sessionUrl.split('/').pop().slice(0, 15) + '…'"></span>
                              </a>
                            </template>
                            <template x-if="!b.sessionUrl">
                              <span class="text-base-content/30 italic font-sans text-xs">none</span>
                            </template>
                          </td>
                        </tr>
                      </template>
                ...
                <!-- Sub-tab 3: GraphQL Query & Variables -->
                <div x-show="graphQLTab === 'query'" class="space-y-2">
                  <div x-show="currentGraphQL?.isAll" class="text-xs text-base-content/60 italic">
                    GitHub's GraphQL schema requires querying a single repository at a time. The query below is dispatched per repository:
                  </div>
                  <div>
                    <div class="text-xs font-semibold text-base-content/70 mb-1">Query Document</div>
                    <pre class="bg-base-200/80 rounded-lg p-3 text-xs font-mono max-h-56 overflow-y-auto overflow-x-auto text-base-content/80 select-all leading-relaxed"
                         x-text="currentGraphQL?.query || (window.GH?.queries?.branchesDatedSessions || 'query BranchesDatedSessions...')"></pre>
                  </div>
                  <div>
                    <div class="text-xs font-semibold text-base-content/70 mb-1">Variables</div>
                    <pre class="bg-base-200/80 rounded-lg p-3 text-xs font-mono max-h-32 overflow-y-auto overflow-x-auto text-base-content/80 select-all"
                         x-text="currentGraphQL?.isAll ? JSON.stringify((currentGraphQL.repos || []).map(r => ({ owner: r.split('/')[0], name: r.split('/')[1] })), null, 2) : (currentGraphQL?.variables ? JSON.stringify(currentGraphQL.variables, null, 2) : '{}')"></pre>
                  </div>
                </div>
      ...
      init() {
        ...
        // right one exists.
        this._done = (e) => {
          // Activity and Sessions hand the document they just built along with
          // the event. Keep that exact copy for the source strip: an immediate
          // GET of the moving `main` URL can still return the browser's cached
          // pre-write response. Events without a document keep the ordinary
          // committed-file fallback below.
          const key = e?.type === 'web-tools:activity-refreshed' ? 'activity'
                    : e?.type === 'web-tools:sessions-refreshed' ? 'sessions' : '';
          const seed = key && e?.detail?.cache ? { [key]: e.detail.cache } : {};
          this.load(seed);
      ...
      // ── GraphQL Activity Discovery Inspector ───────────────────────────
      get allGraphQL() {
        const byRepo = (typeof window !== 'undefined' && window.__graphQLByRepo) || {};
        ...
        for (const r of repos) {
          const snap = byRepo[r];
          if (!snap) continue;
          if (snap.duration) totalDuration += snap.duration;
          if (snap.at && (!newestAt || snap.at > newestAt)) newestAt = snap.at;
          if (Array.isArray(snap.branches)) {
            for (const b of snap.branches) {
              totalBranches.push({
                ...b,
                repo: r,
                sessionUrl: snap.sessions?.[b.name] || b.sessionUrl,
              });
            }
          }
          if (snap.sessions) {
            for (const [k, v] of Object.entries(snap.sessions)) {
              totalSessions[`${r}@${k}`] = v;
            }
          }
          rawAll[r] = snap.raw || snap;
        }

        totalBranches.sort((a, b) => {
          const ta = Date.parse(a.committedDate || a.date || 0) || 0;
          const tb = Date.parse(b.committedDate || b.date || 0) || 0;
          return tb - ta;
        });
        ...
        return {
          ...
          branchesCount: totalBranches.length,
          sessions: totalSessions,
          sessionsCount: Object.keys(totalSessions).length,
          ordered: true,
          raw: rawAll,
          query: (typeof window !== 'undefined' && window.GH?.queries?.branchesDatedSessions) || 'query BranchesDatedSessions...',
        };
      },
      get currentGraphQL() {
        if (this.graphQLRepo === 'all' || !this.graphQLRepo) {
          const all = this.allGraphQL;
          return (all && all.repoCount > 0) ? all : null;
        }
        const r = this.graphQLRepo;
        if (typeof window !== 'undefined') {
          return window.__graphQLByRepo?.[r] ||
            (window.__lastActivityGraphQL?.repo === r ? window.__lastActivityGraphQL : null) ||
            null;
        }
        return null;
      },
      get graphQLReposList() {
        const set = new Set();
        if (window.__shell?.estateRepos) {
          for (const r of window.__shell.estateRepos) {
            const name = r?.repo || (typeof r === 'string' ? r : '');
            if (name && name.includes('/')) set.add(name);
          }
        }
        if (window.__shell?.estateConfigs) {
          for (const k of Object.keys(window.__shell.estateConfigs)) {
            if (k && k.includes('/')) set.add(k);
          }
        }
        const reg = REGISTRY();
        if (reg && reg.includes('/')) set.add(reg);
        set.add('mehrlander/web-tools');
        if (typeof window !== 'undefined' && window.__graphQLByRepo) {
          for (const r of Object.keys(window.__graphQLByRepo)) {
            if (r && r.includes('/')) set.add(r);
          }
        }
        return ['all', ...set];
      },
      async fetchActivityGraphQL(repo, force = false) {
        const target = repo || this.graphQLRepo || 'all';
        this.graphQLRepo = target;
        this.graphQLErr = '';
        this.graphQLLoading = true;
        try {
          const GH = window.GH || Object;
          const token = (typeof window !== 'undefined' && (
            (window.TOKEN && !window.TOKEN.includes('🎟')) ? window.TOKEN :
            (localStorage.getItem('ghToken') || window.__shell?.gh?.token || '')
          )) || '';

          if (!token) {
            throw new Error('Signed out: Please set a GitHub token in the account control to query GraphQL.');
          }

          if (target === 'all') {
            const repos = this.graphQLReposList.filter(r => r !== 'all' && typeof r === 'string' && r.includes('/'));
            if (!repos.length) throw new Error('No estate repositories found to query');
            const results = await Promise.allSettled(repos.map(async (r) => {
              const gh = new GH({ token, repo: r });
              if (typeof gh.branchesDatedSessions === 'function') {
                return await gh.branchesDatedSessions(25, 25, 8);
              }
              throw new Error(`GraphQL branchesDatedSessions not available on GH client for ${r}`);
            }));

            const failures = results.filter(res => res.status === 'rejected');
            if (failures.length === repos.length) {
              const firstErr = failures[0].reason?.message || String(failures[0].reason);
              throw new Error(`All ${repos.length} queries failed: ${firstErr}`);
            } else if (failures.length > 0) {
              const firstErr = failures[0].reason?.message || String(failures[0].reason);
              this.graphQLErr = `Warning: ${failures.length}/${repos.length} repos failed: ${firstErr}`;
            }
          } else {
            if (!force && window.__graphQLByRepo?.[target]) return;
            const gh = new GH({ token, repo: target });
            if (typeof gh.branchesDatedSessions === 'function') {
              await gh.branchesDatedSessions(25, 25, 8);
            } else {
              throw new Error('GraphQL branchesDatedSessions not available on GH client');
            }
          }
        } catch (e) {
          this.graphQLErr = e?.message || String(e);
        } finally {
          this.graphQLLoading = false;
        }
      },
      copyGraphQLText(text) {
        if (!text) return;
        const str = typeof text === 'string' ? text : JSON.stringify(text, null, 2);
        try {
          navigator.clipboard?.writeText(str).then(() => {
            this.graphQLCopied = true;
            setTimeout(() => { this.graphQLCopied = false; }, 2000);
          });
        } catch {}
      },
      ...
      //
      // A quiet arrival still reads the committed file: a gated pass may skip
      // before it has a document, and silence must not make the strip vanish.
      // A completed manual crawl is different. Its refresh event carries the
      // exact document it just wrote, so use that copy rather than immediately
      // asking GitHub for the same moving URL, which can return the browser's
      // cached pre-write response. `seed` is per stream; every missing stream
      // retains the committed-file fallback.
      // ── The spans ────────────────────────────────────────────────────────
      ...
      noteSoon(el) {
        clearTimeout(this._noteT);
        window.Note?.close?.();
        if (el) this._noteT = setTimeout(() => window.Note?.open?.(el), 140);
      },
      tickNote(r, m) {
        const when = this.stamp(new Date(m.t).toISOString());
        const ago = this.ago(new Date(m.t).toISOString());
        const timeLine = when ? (ago ? when + ' (' + ago + ')' : when) : ago;
        const name = m.name || '';
        const detail = m.detail || '';
        const title = name ? (detail ? name + ' \u00b7 ' + detail : name) : detail;
        const extra = m.branch || m.msg || m.title || '';
      ...
      peekAt(r, span, ev) {
        ...
        this.peek = {
          key: r.key, span, left: best.left, name: best.name, detail: best.detail,
          branch: best.branch, title: best.title, msg: best.msg,
        ...
        if (cur) {
          const { title, note } = this.tickNote(r, best);
          const changed = cur.getAttribute('data-note') !== note || cur.style.left !== best.left + '%';
          if (changed) {
            cur.style.left = best.left + '%';
            if (title) cur.setAttribute('data-note-title', title);
            else cur.removeAttribute('data-note-title');
            cur.setAttribute('data-note', note);
          }
          if (ev.type === 'click') {
            clearTimeout(this._noteT);
            window.Note?.open?.(cur);
          } else if (changed) {
            this.noteSoon(cur);
          }
        }
      ...
      async loadPulse(seed = {}) {
        ...
        const jobs = CACHES.filter(c => c.stream).map(async (c) => {
          try {
            const doc = seed[c.key] || await read('state/' + c.file);
      ...
      eventsOf(c, doc) {
        ...
        const events = rows.map(x => {
          const repo = (x.repos && x.repos[0]?.name) || x.repo || '';
          const branch = (x.branches && x.branches[0]) || (x.repos && x.repos[0]?.branch) || '';
          return {
            t: Date.parse(x.ended || x.started),
            name: this.shortKey(repo) || 'session',
            branch: branch || '',
            title: x.title || '',
