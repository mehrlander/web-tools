            ...
            <div class="min-w-0 flex flex-col">
              ...
              <div class="rounded-box border border-base-300 bg-base-100 p-3 shadow-sm flex flex-col gap-3">
                ...
                <div x-show="currentGraphQL" class="flex items-center flex-wrap gap-x-4 gap-y-1 text-xs text-base-content/60 font-mono py-1 border-b border-base-200">
                  ...
                  <span x-show="currentGraphQL?.duration != null">Duration: <span class="text-base-content font-medium" x-text="currentGraphQL.duration + 'ms' + (currentGraphQL.isAll ? ' (' + currentGraphQL.repoCount + ' repos)' : '')"></span></span>
                  ...
                  <span x-show="currentGraphQL?.isAll">Scope: <span class="text-primary font-semibold" x-text="currentGraphQL.repoCount + ' repos'"></span></span>
      ...
      get currentGraphQL() {
        ...
        if (typeof window !== 'undefined') {
          return window.__graphQLByRepo?.[r] ||
            ...
            window.__lastActivityGraphQL || null;
        }
        return null;
      },
      get graphQLReposList() {
        ...
        if (window.__shell?.estateRepos) {
          for (const r of window.__shell.estateRepos) {
            ...
            else if (typeof r === 'string') set.add(r);
          }
        }
        if (window.__shell?.estateConfigs) {
          for (const k of Object.keys(window.__shell.estateConfigs)) set.add(k);
        }
        const reg = REGISTRY();
        if (reg) set.add(reg);
      ...
      async fetchActivityGraphQL(repo, force = false) {
        ...
        try {
          ...
          if (target === 'all') {
            ...
            const gh = new GH({ token: window.TOKEN, repo: target });
            if (typeof gh.branchesDatedSessions === 'function') {
              await gh.branchesDatedSessions(25, 25, 8);
            } else {
