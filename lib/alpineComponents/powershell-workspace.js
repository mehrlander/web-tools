...
document.addEventListener('alpine:init', () => {
  Alpine.data('powershellWorkspace', function (project) {
    ...
    return {
      ...
      menu: '', checkedAt: '',
      template: `
        <section class="@container flex flex-col min-w-0 min-h-0 bg-base-200 text-base-content font-sans" :class="focusMode ? 'fixed inset-0 z-40' : 'h-full'" data-powershell-workspace>
          <header class="flex items-center gap-1 px-2 py-2 shrink-0" data-workspace-toolbar>
            <button @click="toggleExplorer()" class="inline-flex items-center justify-center size-12 shrink-0 rounded-full text-base-content/80 hover:bg-base-content/10 active:bg-primary/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--color-primary)] disabled:opacity-40 disabled:cursor-default transition-colors" :aria-expanded="explorer" aria-label="Files" data-note-bare data-note="Files"><i aria-hidden="true" class="ph ph-folders text-xl"></i></button>
            <button @click="openMenu('source', $event)" aria-label="GitHub source" aria-haspopup="dialog" class="flex flex-col justify-center min-w-0 grow min-h-12 px-2 rounded-xl text-left hover:bg-base-content/10 focus-visible:outline-2 focus-visible:outline-[var(--color-primary)]"><span class="text-lg font-semibold tracking-tight truncate w-full" x-text="project.name || basename(project.path)"></span><span class="text-xs flex items-center gap-1.5 text-base-content/70 min-w-0"><i aria-hidden="true" class="ph ph-git-branch text-xl"></i><span class="truncate" x-text="refreshing ? 'Checking…' : ref"></span><i aria-hidden="true" class="ph ph-caret-down text-xs"></i></span></button>
          ...
          <div x-show="!loading" class="flex min-w-0 min-h-0 grow overflow-hidden">
            <aside x-show="explorer" class="w-full @3xl:w-72 @3xl:shrink-0 min-h-0 overflow-y-auto flex flex-col px-2 pb-2" :class="active && 'hidden @3xl:flex'">
            ...
            <main class="flex flex-col min-w-0 min-h-0 grow bg-base-100 rounded-t-3xl overflow-hidden @3xl:mr-3" :class="!active && explorer && 'hidden @3xl:flex'">
              <div class="flex min-w-0 shrink-0 px-2 pt-1" data-file-toolbar>
                ...
                <button x-show="!!doc" @click="openMenu('file', $event)" aria-label="File actions" aria-haspopup="dialog" data-note-bare data-note="File actions" class="inline-flex items-center justify-center size-12 shrink-0 rounded-full text-base-content/80 hover:bg-base-content/10 active:bg-primary/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--color-primary)] disabled:opacity-40 disabled:cursor-default transition-colors"><i aria-hidden="true" class="ph ph-file-text text-xl"></i></button>
              </div>
              ...
              <div x-show="!!doc" id="wps-document" role="tabpanel" :aria-label="doc ? basename(doc.path) : 'Document'" class="flex flex-col min-h-0 grow">
                <div class="hidden @3xl:block px-5 pb-1 shrink-0 font-mono text-xs truncate text-base-content/70" x-text="doc?.path"></div>
                <div class="flex items-center px-3 border-b border-base-200 shrink-0" role="tablist" aria-label="Code views" @keydown="navigateTabs($event)">
                  <template x-for="tab in ['code', 'diff', 'history']" :key="tab"><button role="tab" :id="'wps-tab-' + tab" :aria-controls="'wps-pane-' + tab" @click="setPane(tab)" :aria-selected="pane === tab" :tabindex="pane === tab ? 0 : -1" class="relative flex min-h-12 min-w-12 items-center justify-center gap-2 px-4 text-sm font-medium rounded-t-xl hover:bg-base-200 focus-visible:outline-2 focus-visible:outline-[var(--color-primary)]" :class="pane === tab ? 'text-base-content after:absolute after:bottom-0 after:h-0.5 after:bg-primary after:inset-x-4 after:rounded-full' : 'text-base-content/70'"><i aria-hidden="true" class="ph text-lg" :class="tab === 'diff' ? 'ph-git-diff' : tab === 'code' ? 'ph-code' : 'ph-clock-counter-clockwise'"></i><span x-text="tab === 'diff' ? 'Compare' : tab === 'code' ? 'Code' : 'History'"></span></button></template>
                  <span class="grow"></span>
                  <button x-show="!!companion" @click="open(companion)" class="btn btn-ghost rounded-full min-h-12 hidden @3xl:inline-flex">Companion</button>
                </div>
                ...
                <div x-show="pane === 'code'" id="wps-pane-code" role="tabpanel" aria-labelledby="wps-tab-code" class="flex min-h-0 grow flex-col">
                  <div class="flex items-center gap-1 px-3 shrink-0" data-editor-toolbar>
                    ...
                    <button @click="editorCommand('find')" aria-label="Find" data-note-bare data-note="Find" class="inline-flex items-center justify-center size-12 shrink-0 rounded-full text-base-content/80 hover:bg-base-content/10 active:bg-primary/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--color-primary)] disabled:opacity-40 disabled:cursor-default transition-colors"><i aria-hidden="true" class="ph ph-magnifying-glass text-xl"></i></button>
                    <button @click="panel = panel ? '' : 'outline'; refreshEditor()" aria-label="Code insights" :aria-expanded="!!panel" data-note-bare data-note="Code insights" class="inline-flex items-center justify-center size-12 shrink-0 rounded-full text-base-content/80 hover:bg-base-content/10 active:bg-primary/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--color-primary)] disabled:opacity-40 disabled:cursor-default transition-colors" :class="!!panel && '!text-primary !bg-primary/10'"><i aria-hidden="true" class="ph ph-list-magnifying-glass text-xl"></i></button>
                    <span class="grow"></span><span class="text-xs text-base-content/70 truncate hidden @xl:inline" x-text="saveState"></span>
                  </div>
                  ...
                  <div class="grid grow min-h-48 min-w-0 overflow-hidden" :class="companionOpen ? 'grid-cols-2' : 'grid-cols-1'">
                    <div class="min-w-0 min-h-0 flex flex-col">
                      <div x-show="editorReady" class="grow min-h-0 overflow-hidden [&_.CodeMirror-gutters]:!bg-base-100 [&_.CodeMirror-gutters]:!border-base-300 [&_.CodeMirror-cursor]:!border-base-content [&_.CodeMirror-selected]:!bg-primary/20 [&_.CodeMirror-linenumber]:!text-base-content/60"><div x-ignore data-editor-host class="h-full"></div></div>
                    ...
                    <section x-show="companionOpen" class="min-w-0 min-h-0 flex flex-col border-l border-base-300" aria-label="Companion source">
                      ...
                      <div x-ignore data-companion-host class="grow min-h-0 overflow-hidden [&_.CodeMirror-gutters]:!bg-base-100 [&_.CodeMirror-gutters]:!border-base-300"></div>
                    </section>
                  </div>
