...
document.addEventListener('alpine:init', () => {
  Alpine.data('powershellWorkspace', function (project) {
    ...
    let companionText = '', menuTrigger = null;
    return {
      ...
      template: `
        <section class="@container flex flex-col min-w-0 min-h-0 bg-base-100" :class="focusMode ? 'fixed inset-0 z-40' : 'h-full'" data-powershell-workspace>
          <header class="flex items-center gap-1 border-b border-base-300 px-2 py-1 shrink-0" data-workspace-toolbar>
            <button @click="toggleExplorer()" class="btn btn-ghost btn-sm" :aria-expanded="explorer">Files</button>
            <button @click="openMenu('source', $event)" aria-label="GitHub source" aria-haspopup="dialog" :title="repo + '@' + ref + ' · ' + short(revision)" class="btn btn-ghost btn-sm font-mono min-w-0 grow justify-start"><span class="truncate" x-text="refreshing ? 'Checking…' : ref"></span><span class="text-base-content/60 hidden @xl:inline" x-text="short(revision)"></span><span aria-hidden="true" class="text-base-content/60">⌄</span></button>
            <button @click="reviewPublish()" :disabled="!!(!dirtyDocs.length || publishing || loading)" class="btn btn-primary btn-sm shrink-0">Review changes <span class="badge badge-sm" x-text="dirtyDocs.length"></span></button>
            <button @click="openMenu('workspace', $event)" aria-label="Workspace actions" aria-haspopup="dialog" class="btn btn-ghost btn-sm btn-square text-xl shrink-0">⋯</button>
          </header>
          ...
          <div x-show="!loading" class="flex min-w-0 min-h-0 grow overflow-hidden">
            ...
            <main class="flex flex-col min-w-0 min-h-0 grow" :class="!active && explorer && 'hidden @3xl:flex'">
              <div class="flex min-w-0 shrink-0 border-b border-base-300">
              <div role="tablist" aria-label="Open files" class="flex overflow-x-auto min-w-0 grow">
              ...
              <button x-show="!!doc" @click="openMenu('file', $event)" aria-label="File actions" aria-haspopup="dialog" class="btn btn-ghost btn-sm btn-square shrink-0 self-center text-xl">⋯</button>
              </div>
              ...
              <div x-show="!!doc" class="flex flex-col min-h-0 grow">
                <div class="hidden @3xl:block px-3 py-1 border-b border-base-200 shrink-0 font-mono text-xs truncate text-base-content/70" :title="doc?.path" x-text="doc?.path"></div>
                <div class="flex flex-wrap gap-1 items-center px-2 py-1 border-b border-base-300 shrink-0" role="tablist" aria-label="Code views">
                  <template x-for="tab in ['code', 'diff', 'history']" :key="tab"><button role="tab" @click="setPane(tab)" :aria-selected="pane === tab" class="btn btn-ghost btn-sm" :class="pane === tab && 'text-primary bg-primary/10'" x-text="tab === 'diff' ? 'Compare' : tab === 'code' ? 'Code' : 'History'"></button></template>
                  <span class="grow"></span>
                  <button x-show="!!companion" @click="open(companion)" class="btn btn-ghost btn-sm hidden @3xl:inline-flex">Companion</button>
                </div>
                ...
                <div x-show="pane === 'code'" class="flex min-h-0 grow flex-col">
                  <div class="flex flex-wrap gap-1 items-center px-2 py-1 shrink-0">
                    <button @click="editorCommand('find')" class="btn btn-ghost btn-sm">Find</button>
                    <button @click="editorCommand('undo')" aria-label="Undo" title="Undo" class="btn btn-ghost btn-sm btn-square text-lg">↶</button><button @click="editorCommand('redo')" aria-label="Redo" title="Redo" class="btn btn-ghost btn-sm btn-square text-lg">↷</button>
                ...
                <div x-show="pane === 'diff'" class="flex flex-col min-h-0 grow">
                  <div class="flex flex-wrap gap-2 items-center px-3 py-2 shrink-0"><select x-model="diffAgainst" @change="buildDiff()" aria-label="Compare draft against" class="select select-bordered select-sm"><option value="base">Original GitHub version</option><option value="current">Last checked GitHub</option><option value="local">Received copy</option></select><label class="text-sm flex gap-2 items-center"><input type="checkbox" x-model="onlyChanges" class="checkbox checkbox-xs">Changes only</label><button @click="openMenu('receive', $event)" aria-haspopup="dialog" class="btn btn-ghost btn-sm ml-auto">Receive copy</button></div>
                  <div x-show="diffAgainst === 'local'" class="px-3 pb-2 flex flex-wrap items-center gap-2 text-sm">
                    <span class="grow" x-text="comparing ? 'Comparing received copy…' : localCheck ? 'Received ' + localCheck.incomingName + ' · ' + (localCheck.exact ? 'Matches original GitHub version' : localCheck.lineEndingsOnly ? 'Line endings differ from original' : 'Differs from original GitHub version') : 'Paste through the app or drop a file to compare it here.'"></span>
                    <button x-show="!!localCheck" @click="confirmation = 'local'" :disabled="!!(publishing || changingDraft || comparing)" class="btn btn-outline btn-sm">Use as draft…</button>
                  </div>
          ...
          <div x-show="!!menu" x-cloak class="fixed inset-0 z-50 bg-black/30 flex items-end justify-center p-2 @xl:items-start @xl:justify-end @xl:p-4" @click.self="closeMenu()" @keydown.escape.stop.prevent="closeMenu()" @keydown.tab="trapMenu($event)">
            <section x-ref="actions" role="dialog" aria-modal="true" :aria-label="menuTitle" tabindex="-1" class="bg-base-100 border border-base-300 rounded-box shadow-xl w-full max-w-sm max-h-[85dvh] overflow-y-auto p-2">
              <div class="flex items-center gap-2 px-2 pb-2 border-b border-base-200"><strong class="grow text-sm" x-text="menuTitle"></strong><button @click="closeMenu()" class="btn btn-ghost btn-sm btn-square" aria-label="Close actions">×</button></div>
              <div x-show="menu === 'workspace'" class="flex flex-col gap-1 pt-1">
                <button @click="exportDrafts(); closeMenu()" :disabled="!dirtyDocs.length" class="btn btn-ghost justify-start">Back up drafts<span class="badge badge-sm ml-auto" x-text="dirtyDocs.length"></span></button>
                <button @click="$refs.bundlePicker.click()" :disabled="!!(publishing || changingDraft)" class="btn btn-ghost justify-start">Restore drafts…</button>
                <button @click="toggleFocus(); closeMenu()" class="btn btn-ghost justify-start" x-text="focusMode ? 'Exit focus' : 'Focus editor'"></button>
              </div>
              <div x-show="menu === 'source'" class="flex flex-col gap-2 p-2 text-sm">
                ...
                <button @click="closeMenu(); refreshUpstream()" :disabled="!!(loading || refreshing || publishing)" class="btn btn-outline btn-sm" x-text="refreshing ? 'Checking…' : 'Check GitHub'"></button>
              </div>
              <div x-show="menu === 'file'" class="flex flex-col gap-1 pt-1">
                ...
                <button @click="copyDraft(); closeMenu()" :disabled="publishing" class="btn btn-ghost justify-start">Copy</button>
                <button @click="downloadDraft(); closeMenu()" class="btn btn-ghost justify-start">Download</button>
                <button @click="openMenu('receive', $event)" class="btn btn-ghost justify-start">Receive copy</button>
                <button @click="findReplace()" class="btn btn-ghost justify-start">Find and replace</button>
                ...
                <button x-show="!!companion" @click="closeMenu(); open(companion)" class="btn btn-ghost justify-start">Companion</button>
                <button x-show="!!companion" @click="closeMenu(); showCompanion()" class="btn btn-ghost justify-start hidden @3xl:inline-flex">Split companion</button>
                <a :href="fileUrl" target="_blank" rel="noopener" @click="closeMenu()" class="btn btn-ghost justify-start">Open on GitHub ↗</a>
                <button @click="closeMenu(); installation()" class="btn btn-ghost justify-start">Installation</button>
                <div class="border-t border-base-200 my-1"></div>
                <button @click="closeMenu(); confirmation = 'discard'" :disabled="!!(!changed(doc) || publishing)" class="btn btn-ghost text-error justify-start">Discard edits…</button>
              </div>
              <div x-show="menu === 'receive'" class="flex flex-col gap-3 p-2">
                ...
                <button @click="closeMenu(); pasteCopy()" :disabled="!!(comparing || publishing || changingDraft)" class="btn btn-primary">Paste</button>
                <button @click="$refs.copyPicker.click()" class="btn btn-outline">Choose file…</button>
                <label class="text-sm flex flex-col gap-1">File encoding<select x-model="localEncoding" class="select select-bordered select-sm" aria-label="Local file encoding"><option value="auto">Auto encoding</option><option value="utf-16le">UTF-16 LE</option><option value="utf-16be">UTF-16 BE</option><option value="windows-1252">Windows-1252</option></select></label>
      ...
      async openMenu(name, event) {
        ...
        this.menu = name;
        await this.$nextTick();
        if (this.menu === name) this.$refs.actions?.focus();
      },
      closeMenu() { this.menu = ''; if (menuTrigger?.isConnected) menuTrigger.focus(); menuTrigger = null; },
      trapMenu(event) {
        if (!this.menu) return;
      ...
      buildDiff() {
        ...
        if (this.diffAgainst === 'current') { before = d.currentText; if (before === undefined) this.diffWarning = 'Open GitHub source above and choose Check GitHub.'; if (before === null) this.diffWarning = 'This file was removed from GitHub.'; }
        if (this.diffAgainst === 'local') before = this.localCheck?.content;
