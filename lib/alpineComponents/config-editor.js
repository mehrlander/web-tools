document.addEventListener('alpine:init', function () {
  ...
  Alpine.data('configEditor', function () {
    ...
    // Seeded into the editor for a repo with no config yet, so the shape is
    // visible from the start. All-empty, so saving it as-is is harmless: every
    // consumer treats these falsy values as unset.
    const TEMPLATE = { icon: '', landing: '', pins: [], stage: { files: [], targets: [] } };
    const dialogHtml = `
      <dialog id="configModal" class="modal" onclick="if(event.target===this)this.close()">
        <div class="modal-box shadow-none border border-base-300 bg-base-100 p-4 max-w-xl">
          ...
          <div class="mt-3 rounded-lg bg-base-200/50 border border-base-200 p-2.5">
    ...
    return {
      ...
      open() {
        ...
        // A configured repo shows its own manifest; an unconfigured one is
        // seeded with the empty template so the shape is there to fill in.
        this.draft = JSON.stringify(s.config || TEMPLATE, null, 2);
        this.saving = false;
