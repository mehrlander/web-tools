...
//   - bare:      the host names the file itself, so drop the collapsed row AND
//                the github menu. The menu is about WHICH COPY of this file you
//                are reading, which is the same question the name answers, so a
//                host that took the name took the links with it. Sitting in the
//                card's row it also sat nowhere near the name it belongs to,
//                which is the convention this estate keeps: a filename pointing
//                at GitHub carries that dropdown beside it. pages/approve.html
//                is the only consumer of this flag (checked 2026-09-18) and it
//                draws the menu against the deck header's title.
//   - hosted:    the host draws this card's CHROME as well as its name, so the
//                card draws neither row and is content alone. It implies bare,
//                and it owes the host a way to keep up: `onChrome` fires
//                whenever what the host should be drawing has changed. The
//                host reads `viewModes`/`pickView` and `ghLinks` off the
//                component, so the controls exist in one place and are placed
//                in another rather than being built twice.
//   - fill:      the host bounds and scrolls this card, so the panes must not
//                bound or scroll themselves (see paneClass), the control row
//                pins to the host's scroller, and copy stands down: a card that
//                fills a panel is a place to read, not a clipboard source.
//
...
document.addEventListener('alpine:init', function () {
  Alpine.data('fileReview', function (opts) {
    ...
    return {
        ...
        <div class="bg-base-100 border-b border-base-200 last:border-b-0">
          ...
          <div x-show="open" x-collapse>
            ...
            <div class="flex flex-col gap-2"
                 :class="bare ? 'pt-1' : (fill ? 'px-3 py-2 bg-base-100' : 'border-t border-base-200 px-3 py-2 bg-base-200/20')">
              ...
              <!-- STICKY WHERE THE HOST SCROLLS THE CARD. fill means the
                   panel around this card owns the height and the overflow, so
                   the row scrolls out of sight with the prose and the reader
                   loses the file's name and its layout icons a screen into a
                   long document. Pinned to the scroller's top it stays the
                   card's header, which is what it became when the collapsed
                   row stood down.
                   Opaque, and full-bleed against the wrapper's px-3, because
                   the pane under it is -mx-3: a transparent band, or one three
                   units narrow, shows the prose sliding through it.
                   Not on an unfilled reading card: there the PANE is the
                   scroller and the row sits outside it, so sticky would pin
                   the row to the page rather than to the text it names.
                   Outside the lock the panel keeps overflow-hidden and only
                   loses its max-height, and hidden is still a scroll container,
                   so the row stays confined to the panel and scrolls off the
                   screen with it rather than pinning to the viewport under the
                   page's own sticky row. Measured 2026-09-07 at 844x390 with
                   the clip expanded: the row held at 9px inside the panel
                   across a 600px page scroll. The guarantee is that clip's
                   overflow-hidden, which branch-brief-groups holds.
                   (No backticks in this markup: it is a JS template literal.) -->
              <div x-show="!hosted" class="flex items-center gap-1"
                   :class="fill && 'sticky top-0 z-10 -mx-3 -mt-2 px-3 py-1.5 bg-base-200 border-b border-base-300'">
                ...
                <!-- One class attribute, not two. It carried order-4 and the
                     tab classes as separate attributes, and the HTML parser
                     keeps the first duplicate and discards the rest, so the
                     strip has been shipping with none of its tabs styling:
                     the labels run together as ReadDiffPatchNewBase. Found
                     2026-09-16 by pages/approve.html mounting this card and
                     shooting it. No backticks in this comment: the template
                     it sits in is a JS template literal, and one ends it. -->
                <div role="tablist" x-show="!read && panes.length > 1"
                     class="order-4 tabs tabs-box tabs-xs bg-base-200 p-0.5 flex-nowrap min-w-0
                ...
                <div class="order-6 join shrink-0" x-show="!read && tab==='diff' && diffable">
                  <button class="btn btn-sm join-item" :class="view==='split' ? 'btn-active' : 'btn-ghost'"
                          @click="setView('split')"><i class="ph ph-columns text-base"></i></button>
                  <button class="btn btn-sm join-item" :class="view==='unified' ? 'btn-active' : 'btn-ghost'"
                          @click="setView('unified')"><i class="ph ph-rows text-base"></i></button>
                </div>
                <!-- What the comparison is against, and how to change it. The
                     ...
                     (No backticks in this markup: it is a JS template literal.) -->
                <!-- IT SHRINKS BEFORE ANYTHING ELSE DOES, and hardest. The ref
                     here is whatever the sidebar last picked, so it is an
                     arbitrary branch name rather than a short base: with
                     shrink-0 and no truncation, claude/bookmarklets-shortcuts-
                     iphone-safari-rjakex took 362px of a 398px row and pushed
                     the layouts 88px past its right edge and copy 120px, off
                     the screen entirely. shrink-[9999] is the same weight
                     dirPart carries two blocks up and for the same reason: the
                     row has two shrinkable things, and between the file's own
                     path and the ref it is being compared against, the path is
                     the one a reader needs whole.
                     (No backticks in this markup: it is a JS template literal.) -->
                <details class="dropdown min-w-0 shrink-[9999]" x-ref="cmpMenu" x-show="comparePicker"
                         :class="read ? 'order-3 dropdown-start' : 'order-7 dropdown-end'">
                  <summary class="btn btn-ghost btn-sm gap-1 px-1.5 min-w-0 max-w-full flex-nowrap cursor-pointer font-normal"
                           title="What this file is compared against">
                    <i class="ph ph-git-diff text-base shrink-0"
                       :class="compareOff ? 'opacity-40' : ''"></i>
                    <span class="font-mono text-xs min-w-0 truncate"
                          x-show="tab==='diff'" x-text="baseName"></span>
                  </summary>
                ...
                <!-- The reading surface's whole strip: the file, then the
                     ...
                     TWO SPACERS OR ONE, and the copy button is what decides.
                     Where copy holds the end, a second spacer centres this
                     between the identity cluster and it, rather than letting it
                     butt against whichever end happens to be shorter. On a
                     filled card copy is gone, so the second spacer goes with it
                     and this takes the end itself. Both collapse to nothing in
                     a list, where read is false and neither shows. -->
                <div class="order-4 grow" x-show="read"></div>
                <!-- THE RENDERED COMPARISON'S OWN CONTROLS, in this row rather
                     than in a strip of their own. md-diff draws a readout and a
                     swipe/side toggle, and left to itself it puts them in a
                     sticky strip INSIDE the document, which on a reading card
                     meant two rows of chrome one above the other saying
                     different halves of the same thing: this row said which
                     comparison, the strip under it said how many changes and
                     how to lay them out. The kit already hands its controls to
                     a host that asks (its controls option), which is the same
                     arrangement this card makes with viewModes, so the card
                     asks. Empty and therefore invisible on every other pane.
                     (No backticks in this markup: it is a JS template literal.) -->
                <div x-ref="mdControls" class="order-5 flex items-center gap-2 shrink-0 empty:hidden"></div>
                <div class="order-6 join shrink-0" x-show="read && viewModes.length > 0">
                  <template x-for="m in viewModes" :key="m.key">
                    <button class="btn btn-sm btn-square join-item"
                      ...
                      <i class="ph text-base max-sm:text-lg" :class="m.icon"></i></button>
                  </template>
                </div>
                <!-- Position in multi-file review strip (e.g. 1/3, 2/3) -->
                <span x-show="!!pager"
                      class="badge badge-sm font-mono text-xs opacity-70 shrink-0 select-none"
                      :class="read ? 'order-7' : 'order-8'"
                      x-text="pager"></span>
                <div class="order-7 grow" x-show="read && !fill"></div>
                <!-- ONE copy button, and it copies WHAT IS SHOWING.
                     ...
                     AND NOT AT ALL ON A FILLED CARD. A panel in the branch
                     page's strip is a place to READ the file; taking its text
                     is a different errand, and the row it was costing is the
                     one the file's own name is truncated on at 390px. The menu
                     beside the name still has Raw, so the text is one tap
                     further rather than gone. It stays on a list row and on the
                     file deck's slides, which are full-screen readers with no
                     other route to the clipboard. -->
                <button x-show="copyable !== null && !fill" @click="$clip(copyable)" :title="copyTitle"
                        class="btn btn-sm btn-square btn-ghost shrink-0"
                        :class="read ? 'order-8' : 'order-9'">
                  <i class="ph ph-copy text-base"></i></button>
                <button x-show="!!action" @click="action && action.onClick(path)"
                        :title="action?.label"
                        class="btn btn-sm btn-square btn-ghost shrink-0"
                        :class="read ? 'order-9' : 'order-10'">
                  <i class="ph text-base" :class="action?.icon || 'ph-cards-three'"></i></button>
                <!-- BESIDE THE NAME on a reading card, at the end of the row in
                     ...
                     (No backticks in this markup: it is a JS template literal.) -->
                <details class="dropdown shrink-0" x-ref="ghMenu" x-show="!bare"
                         :class="read ? 'order-2 dropdown-start' : 'order-11 dropdown-end'">
                  <summary class="btn btn-ghost btn-sm gap-0.5 px-1.5 cursor-pointer" title="This file on GitHub">
              ...
              <div x-show="tab==='read'" :class="paneClass">
                <div x-ref="readBox" :class="proseCls" x-html="readHtml"></div>
              </div>

              <!-- The same pair, rendered and marked, through kits/md-diff.js.
                   The host is plain: the kit gives each block its own guide
                   body, since an unchanged block and a changed one inside a
                   container are two different boxes. -->
              <div x-show="tab==='mddiff'" :class="paneClass">
                <div x-ref="mdDiffHost"></div>
              </div>
              ...
              <!-- ── The page, running ─────────────────────────────────────
                   ...
                   own empty state inside the frame rather than the page. A
                   footer link to the same render in its own tab rode under the
                   frame for that case until 2026-09-24. It was removed as
                   confusing: the branch masthead already links the page's
                   render, and a list row's menu carries a Render row. -->
              <template x-if="tab==='render'">
                <div class="overflow-hidden bg-base-100"
                  ...
                  <iframe :src="tossUrl" :title="'Render of ' + path" loading="lazy"
                          ...
                          class="w-full border-0 bg-base-100"
                          :class="fill ? 'h-full min-h-[20rem]' : 'h-[60vh]'"></iframe>
                </div>
              </template>
      ...
      read: !!opts.read,
      // The pane to land on, where the host knows better than the card does.
      // A file deck exists to read a file, so it opens on the document; an
      // approval surface exists to judge a change, so opening on the document
      // leaves the reviewer hunting for what moved. Only honoured when the
      // named pane can actually show something, so a host may ask for the
      // rendered comparison and get the document on a file that has none.
      openOn: opts.openOn || '',
      // hosted implies bare: naming the file is part of the chrome, so a host
      // that draws the controls is drawing the name too (the file deck's
      // header already does, on every swipe).
      bare: !!opts.bare || !!opts.hosted,
      hosted: !!opts.hosted,
      onChrome: typeof opts.onChrome === 'function' ? opts.onChrome : null,
      fill: !!opts.fill,
      _pager: opts.pager ?? '',
      get pager() {
        if (typeof this._pager === 'function') return this._pager();
        return this._pager || '';
      },
      loaded: false,
      loading: false,
      ...
      _homeBase: opts.base || '',
      // Named comparisons a host can offer beyond the base it mounted with,
      // [{ base, label }], listed first in the compare menu. The branch view
      // passes the merge base and the base branch's tip.
      baseChoices: Array.isArray(opts.baseChoices) ? opts.baseChoices : [],
      // The branch's own commits, [sha], which is what a version of this file
      // is chosen from: the commits on the branch that touched it. The branch
      // view passes them off its compare; without them there are no versions.
      versionScope: Array.isArray(opts.versionScope) ? opts.versionScope : [],
      versions: null,
      versionsLoading: false,
      // THE SCOPE OF THE CHANGE, for a host that shows it: { lines, added,
      // removed } for the two texts on screen, whatever the comparison. Counted
      // with jsdiff, the engine behind the rendered comparison, so the figures
      // follow a pick in the compare menu; against the merge base they match
      // GitHub's own +/- (measured 2026-09-23 on four files of #754). Only a
      // host that asks pays for the count: `stats: true`.
      stats: !!opts.stats,
      changeStats: null,
      ...
      // WHAT A HOST WOULD BE DRAWING, as one string to watch. A hosted card's
      // controls live on this component and are placed by someone else, so the
      // placer has to be told when they change: which layouts exist and which
      // is lit, and whether the github menu grew its commit row. Watching a
      // signature rather than five fields keeps the callback to one, and keeps
      // the list of what matters beside the getters it is made of.
      get chromeKey() {
        return [this.tab, this.view, this.loaded, this.compareOff,
                this.viewModes.map(m => m.key + (m.on ? '!' : '')).join(','),
                this.ghLinks.length].join('|');
      },

      init() {
        this.$el.innerHTML = this.template;
        // A hosted card owes its host a nudge on every change to the controls
        // the host is drawing for it. KEYED ON `hosted`, not on the callback
        // being present: nothing else needs the watcher, and chromeKey reads
        // viewModes and ghLinks, so arming it on thirty list rows would re-run
        // both getters on every reactive change for an answer nobody reads.
        // The callback is read at fire time so a host can attach one late.
        if (this.hosted) this.$watch('chromeKey', () => this.onChrome && this.onChrome(this));
        if (this.stats) this.$watch(() => [this.loaded, this.loading, this.base, this.compareOff].join('|'),
                                    () => this._countChange());
      ...
      async load() {
        ...
        try {
          ...
          // THE PANE A CARD OPENS ON HAS TO BE DRAWN BY THE LOAD, and this line
          // was missing, so the one pane a host can ask to land on was the one
          // that never arrived. `_reloadBase` had it; `load` did not. Every
          // other route reaches the renderer through `setTab`, which is why the
          // app was fine and only `openOn` was broken: pages/approve.html asks
          // to open on the rendered comparison, landed on an empty box, and the
          // card read as though the view had been dropped. Measured 2026-09-18
          // in the tossed page: tab mddiff, diffable true, both sides fetched,
          // host with no children.
          if (this.tab === 'mddiff') this._renderMdDiff();
            this._render();
          this._fetchLastCommit();
        } catch (e) {
          this.error = 'Load failed: ' + (e.message || e);
      ...
      async _reloadBase() {
        ...
        // The rendered comparison is the one pane whose content changes when
        // only the BASE moves: the file pane is the new side and does not.
        if (this.tab === 'mddiff') this._renderMdDiff();
      },
      ...
      _tabUsable(t) {
        return t === 'patch' ? !!this.patch
             ...
             : t === 'mddiff' ? this.diffable && this.kind === 'markdown'
      ...
      get panes() {
        ...
        if (this.read) {
          p.push(shown
            ...
            : { id: 'new', label: 'File' });
          if (this.comparable) {
            p.push({ id: 'diff', label: 'Compare' });
            // And, for a document, the comparison as the DOCUMENT. Markdown
            // alone, because a rendered difference needs something rendered to
            // lay it over; every other kind here is source, and source is what
            // the CM6 panes already do well.
            if (this.kind === 'markdown') p.push({ id: 'mddiff', label: 'Compare, rendered' });
          }
          return p;
        }
      ...
      _defaultTab() {
        const shown = this.shownPane;
        if (this.openOn && this.panes.some(p => p.id === this.openOn)
            && this._tabUsable(this.openOn)) return this.openOn;
        if (this.read) {
      ...
      get viewModes() {
        ...
        const out = [
          { key: 'file', id: first.id, view: '', icon: 'ph-square',
            label: first.label, on: this.tab !== 'diff' && this.tab !== 'mddiff' },
        ];
        // Rendered first among the comparisons, for a document: the reviewer's
        // question about prose is what it now SAYS, and the two source panes
        // answer a question about the markup. It is a view here rather than a
        // tab because it is the same pair drawn a third way, which is exactly
        // what this control is.
        if (this.kind === 'markdown') {
          out.push({ key: 'rendered', id: 'mddiff', view: '', icon: 'ph-article',
                     label: 'Compare, rendered', on: this.tab === 'mddiff' });
        }
        out.push(
      ...
      get compareChoices() {
        ...
        for (const c of this.baseChoices) add(c.base, c.label);
        add(this._homeBase, this._homeBaseName);
        // A version picked from versionChoices is listed there, not twice.
        if (!(this.versions || []).some(v => v.sha === this.base)) add(this.base, this.baseName);
      ...
      // ONLY WHERE A COMPARISON IS ON SCREEN. It rode every reading card, which
      // put a git-diff glyph on a page a reader is looking at rather than
      // diffing; the ref beside it was already conditional on the diff tab for
      // the same reason, so the icon was half-hidden already. What it picks has
      // no meaning until there is a comparison to point at, and the three
      // layout icons are how a reader gets to one.
      //
      // The compareOff arm is the way BACK and cannot go with it: with the pair
      // turned off, `comparable` is false, so viewModes offers no diff icon, so
      // nothing else on the card can turn it on again.
      get comparePicker() {
        return this.read && ((this.comparable && this.tab === 'diff') || this.compareOff);
      },
      async _countChange() {
        if (!this.loaded || this.loading || this.newText === null) { this.changeStats = null; return; }
        const neu = this.newText, old = this.baseText, key = this.base + '|' + this.compareOff;
        const lines = (t) => (t === '' ? 0 : t.replace(/\n$/, '').split('\n').length);
        const out = { lines: lines(neu), added: null, removed: null };
        if (!this.compareOff) {
          if (old === null) { out.added = out.lines; out.removed = 0; }
          else if (old === neu) { out.added = 0; out.removed = 0; }
          else {
            try {
              if (!window.mdDiff && window.gh?.load) await window.gh.load('kits/md-diff.js');
              const D = window.mdDiff ? await window.mdDiff.needDiff() : window.Diff;
              // Capped, so a vast generated file costs a quarter second at most
              // and then says only its length rather than a wrong count.
              const parts = D && D.diffLines(old, neu, { timeout: 250 });
              if (parts) {
                out.added = 0; out.removed = 0;
                for (const p of parts) {
                  if (p.added) out.added += p.count;
                  else if (p.removed) out.removed += p.count;
                }
              }
            } catch { /* the length alone, then */ }
          }
        }
        if (key !== this.base + '|' + this.compareOff || neu !== this.newText) return;  // moved meanwhile
        this.changeStats = out;
      },

      // ── The file's versions on this branch ────────────────────────────────
      //
      // Each commit on the branch that touched this file is a version of it,
      // newest first and numbered from the oldest, so v1 is the file's first
      // state on the branch. Read once, when the reader opens the compare menu:
      // one call, the same commits query _fetchLastCommit makes with a longer
      // page, kept to the branch's own commits so main's history stays out.
      async loadVersions() {
        if (this.versions || this.versionsLoading || !this.versionScope.length) return;
        this.versionsLoading = true;
        try {
          const list = await this._gh(this.ref).req(
            'commits?path=' + encodeURIComponent(this.path)
            + '&sha=' + encodeURIComponent(this.ref) + '&per_page=50');
          const own = new Set(this.versionScope);
          const rows = (Array.isArray(list) ? list : []).filter(c => own.has(c.sha));
          this.versions = rows.map((c, i) => ({
            sha: c.sha, n: rows.length - i, current: i === 0,
            // A merge of the base into the branch can touch the file without
            // anyone editing it here; it is a version, but said as a merge.
      ...
      get versionChoices() {
        ...
        return (this.versions || []).map(v => ({
          key: 'v:' + v.sha, base: v.sha, current: v.current, merge: v.merge, subject: v.subject,
          label: 'v' + v.n + (v.merge ? ' merge' : ''),
          title: v.sha.slice(0, 7) + ' \u00b7 ' + v.subject,
          when: when(v.date),
          on: !this.compareOff && this.base === v.sha,
        }));
      },
      // THIS CARD ONLY. A version belongs to one file, so the pick is adopted
      // here rather than published on the compare channel, which every card on
      // the page hears. The file compared against its own earlier state.
      pickVersion(c) {
        if (c.current) return;
        this.adoptCompare({ repo: this.repo, ref: this.ref, base: c.base, baseName: c.label });
        if (this.tab !== 'diff' && this.tab !== 'mddiff') this.setTab('diff');
      },
      pickCompare(c) {
      ...
      get paneClass() {
        ...
        // `fill`: the HOST is the scroller, so this must not be a second one.
        // A card in the branch page's document strip sits in a panel with a
        // definite height and its own overflow, and max-h-[70vh] there is 591px
        // of pane inside a 352px panel: the pane's scrollbar lands below the
        // panel's cut, where nothing can reach it. That is what the more/less
        // expander over it was standing in for.
        if (this.read && this.fill) return '-mx-3 px-2 py-1 bg-base-100';
      ...
      // REACTIVE STATE, not a getter over a global, and the difference is the
      // whole rendering. `window.GuideRender` is a plain global: Alpine cannot
      // track it, so a getter reading it is evaluated once, at first paint,
      // when the kit has not registered yet. The binding then cached the
      // fallback forever, and since bodyClass() is also what INJECTS the guide
      // stylesheet, the sheet never landed either. Every markdown Read pane in
      // this estate was rendering guide-flavoured markup under bare `prose`
      // with no guide CSS at all: headings flat, tables unruled, the wide-table
      // scroller inert. Measured 2026-09-16 by probing the mounted card, which
      // reported GuideRender present, the box classed `prose !max-w-none`, and
      // no stylesheet in the document.
      //
      // _renderRead already awaits the kit, so it is the one place that knows
      // the answer is available; it sets this, and the pane restyles.
      proseCls: 'prose !max-w-none',
      ...
      // The bytes, through the client at the pane's ref. A hand-rolled
      // fallback for a client without bytes() lived here while gh-api.js and
      // this file were separate jsDelivr cache entries that could disagree for
      // hours after a merge. Since 2026-09-26 the client comes from Pages (a
      // ten-minute cache) or raw, and bytes() long predates either.
      _bytes(ref) { return this._gh(ref).bytes(this.path); },
      ...
      async _drawPdf() {
        ...
        try {
          ...
          if (!window.pdf) await gh.load('kits/pdf.js');
      ...
      // The frontmatter header: a label column and a value column, at the
      // chrome tier so it reads as metadata and the document keeps the page's
      // one content size. A long value (a skill's description is the case) is
      // clamped rather than truncated, since the whole of it is in the source
      // tabs and a header is a summary.
      _metaHtml(fields) {
        const esc = window.esc;   // lib/vanilla-bundle.js, the one helper
        const rows = fields.map((f) =>
          '<dt class="font-mono text-xs opacity-60 pt-1">' + esc(f.key) + '</dt>' +
          '<dd class="min-w-0 m-0 text-pretty' +
          (f.value.length > 220 ? ' line-clamp-4' : '') + '">' + esc(f.value) + '</dd>'
        ).join('');
        return '<dl class="grid grid-cols-[auto_minmax(0,1fr)] gap-x-3 gap-y-1 mb-4 ' +
               'border-l-2 border-base-300 pl-3">' + rows + '</dl>';
      },
      ...
      async _renderRead() {
        ...
        try {
          ...
          // Now that the kit is here, take its body class. This also injects the
          // guide stylesheet, which is why it cannot wait for a getter.
          this.proseCls = window.GuideRender.bodyClass('page');
        } catch { return; }
        ...
        // Frontmatter as METADATA, not as a wall of fenced YAML. Fencing was the
        // whole treatment, so a skill file opened on twenty lines of grey
        // monospace and the document began below the fold. splitFrontmatter
        // hands back the fields, which render as a short labelled header, and
        // fencing stays as the fallback for frontmatter too odd to read
        // shallowly. The raw block is one tab away either way.
        let src = this.newText;
        let metaHtml = '';
        try {
          if (!window.SourcePeek) await window.gh?.load('kits/source-peek.js');
          const sp = window.SourcePeek;
          const fm = sp?.splitFrontmatter ? sp.splitFrontmatter(src) : null;
          if (fm && fm.fields.length) { metaHtml = this._metaHtml(fm.fields); src = fm.body; }
          else if (sp?.fenceFrontmatter) src = sp.fenceFrontmatter(src);
        ...
        this.readHtml = metaHtml + out.html;
      ...
      setTab(t) {
        ...
        // The rendered comparison's controls belong to the rendered
        // comparison, so leaving it takes them with it. Without this the
        // readout said "10 changes" over a pane with no changes in it.
        if (t !== 'mddiff' && this.$refs.mdControls) this.$refs.mdControls.replaceChildren();
        if (t === 'diff') this._render();
        if (t === 'read') this._renderRead();
        if (t === 'mddiff') this._renderMdDiff();
      },
      setView(v) {
        this.view = v;
      ...
      // The rendered comparison. Both sides lose their frontmatter first, and
      // for the same reason the read pane strips it: a `---` block renders as a
      // run of prose, so leaving it in makes a document whose date changed open
      // on a change to its first paragraph.
      async _renderMdDiff() {
        const host = this.$refs.mdDiffHost;
        if (!host || this.newText == null || this.baseText == null) return;
        ...
        const body = (t) => {
          try {
            const fm = window.SourcePeek?.splitFrontmatter?.(t || '');
            return fm && fm.fields.length ? fm.body : (t || '');
          } catch { return t || ''; }
        };
        try {
          if (!window.mdDiff) throw new Error('kit unavailable');
          // `controls` moves the kit's readout and layout toggle into this
          // card's own row; without it the kit draws them in a sticky strip of
          // its own inside the document, which is a second row of chrome.
          // Cleared first, since render() APPENDS and a re-render would
          // otherwise leave two readouts side by side.
          // NOT WHEN THE HOST DRAWS THE CHROME. Under `hosted` this card's own
          // control row is display:none, so handing the kit a slot inside it
          // would hide the readout and the layout toggle outright. With no
          // slot the kit floats its own strip over the document, which is what
          // a hosted surface wants: kits/file-deck.js and pages/approve.html
          // both put the file's controls in the deck header and have nowhere
          // in the slide to put a control about the COMPARISON.
          const slot = this.hosted ? null : this.$refs.mdControls;
          if (slot) slot.replaceChildren();
          await window.mdDiff.render(host, body(this.baseText), body(this.newText),
                                     slot ? { controls: slot } : {});
        } catch (e) {
          if (mine !== this._mdSeq) return;
          host.innerHTML = '<div class="p-3 text-sm opacity-60 italic">'
            + 'Rendered comparison unavailable (' + ((e && e.message) || e) + '). '
            + 'The source comparison is one tap away.</div>';
        }
      },
      ...
      _teardown() {
        ...
        // The rendered comparison holds a swipe track per change and a
        // ResizeObserver per track, so leaving it mounted across a base change
        // keeps both alive over content that has gone.
        this._mdSeq = (this._mdSeq || 0) + 1;
        if (this.$refs.mdDiffHost) this.$refs.mdDiffHost.innerHTML = '';
        if (this.$refs.mdControls) this.$refs.mdControls.replaceChildren();
      },
