    ...
    return {
      ...
      async loadDrafts() {
        ...
        try {
          ...
          this.draftList = rows;
        } catch { this.drafts = {}; this.draftList = []; }
      },
      reviewPublish() {
        ...
        const ctx = {
          ...
          read: item => {
            if (!reads.has(item.path)) {
              ...
              // Windows-1252, which is what Windows PowerShell 5.1 assumes for
              // a file with no BOM. The slide keeps such a file read-only.
              const args = { gh, path: item.path, revision, blobSha: item.blobSha };
              reads.set(item.path, W.readFile(args).then(f => ({ ...f, encoding: 'utf-8' }), async e => {
          ...
          publish: () => { handle?.close(); this.loadDrafts().then(() => { if (this.draftList.length && !this.publishOpen) this.reviewPublish(); }); },
