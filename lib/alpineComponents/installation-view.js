// The Installation tab of the project view: one workspace's PowerShell
// material, each file with where it is meant to go under the work
    ...
    return {
      ...
      template: `
        <div class="@container flex flex-col gap-4" data-installation>
          <div class="flex flex-wrap items-center gap-x-3 gap-y-1">
            ...
            <span class="font-mono text-xs text-base-content/50" x-text="root"></span>
            <div class="grow"></div>
            ...
            <div class="flex items-center gap-1">
              ...
              <button @click="reload()" class="btn btn-ghost btn-sm btn-square" title="Reload the tree, the ledger and this browser's checks">
          ...
          <div x-show="!loading" class="grid gap-6 @3xl:grid-cols-[minmax(0,26rem)_minmax(0,1fr)]">
            ...
            <div class="flex flex-col gap-5 min-w-0" :class="selected ? 'hidden @3xl:flex' : 'flex'">
              ...
              <section x-show="!filter && localAreas.length">
                ...
                <div class="flex flex-col gap-2">
                  <template x-for="a in localAreas" :key="a.name">
                    <div class="rounded-lg border border-dashed border-base-300 px-3 py-2 flex flex-col gap-0.5">
                      <div class="flex items-center gap-2">
                        <i class="ph ph-folder-dashed text-base-content/50"></i>
                        <span class="font-mono text-base" x-text="a.name"></span>
                        <span class="grow"></span>
                        <span class="badge badge-sm badge-ghost" x-text="a.status === 'local-only' ? 'local only' : 'unresolved'"></span>
                      </div>
                      <div class="text-base text-base-content/70" x-text="a.policy"></div>
                      <div x-show="a.shape" class="text-xs text-base-content/50" x-text="a.shape"></div>
                    </div>
                  </template>
                </div>
              </section>
                ...
                <div class="flex flex-col gap-4">
                  <div class="flex items-start gap-2">
                    ...
                    <div class="min-w-0 grow">
                      ...
                      <div class="font-mono text-xs text-base-content/50 break-all" x-text="item.path + ' · ' + short(item.blobSha)"></div>
                    </div>
                  </div>
                  ...
                  <div class="flex flex-wrap gap-1.5">
                    ...
                    <button @click="goStage()" class="btn btn-ghost btn-sm gap-1" title="The Stage holds the last comparison's pair and the transfer toward GitHub">
                  ...
                  <div x-show="pending" x-cloak class="border border-primary/40 rounded-lg p-3 flex flex-col gap-2 bg-primary/10">
                    ...
                    <pre class="font-mono text-xs whitespace-pre-wrap break-all bg-base-100 rounded p-2" x-text="pending?.line"></pre>
                    <div class="font-mono text-xs text-base-content/60" x-text="'appends to ' + repo + '@' + ref + ':' + ledgerPath"></div>
                    <div class="flex flex-wrap gap-1.5">
                    ...
                    <div class="flex flex-col">
                      <template x-for="(r, i) in historyOf(item)" :key="r.date + r.local_sha256">
                        <div class="flex flex-wrap items-baseline gap-x-2 gap-y-0.5 py-1 text-base" :class="i && 'border-t border-base-200'">
                          ...
                          <span class="font-mono text-xs text-base-content/50" x-text="'at ' + short(r.revision) + (r.blob_sha === item.blobSha ? ' (current)' : ' (superseded)')"></span>
                          <span class="text-xs text-base-content/50" x-text="r.method"></span>
                    ...
                    <div class="flex flex-col">
                      <template x-for="(c, i) in checksOf(item)" :key="c.id">
                        <div class="flex flex-wrap items-center gap-x-2 gap-y-0.5 py-1 text-base" :class="i && 'border-t border-base-200'">
                          ...
                          <span class="font-mono text-xs text-base-content/50" x-text="short(c.revision) + ' · ' + c.source"></span>
                          <div class="grow"></div>
                          <button x-show="!isRecorded(c)" @click="askCheck(c)" class="btn btn-outline btn-xs">Record</button>
                          <span x-show="isRecorded(c)" class="text-xs text-base-content/50">recorded</span>
      ...
      observedLine(it) {
        ...
        return when(s.latest.date) + ' · ' + this.rowLabel(s.latest) + ' at ' + short(s.latest.revision)
          + (s.latest.blob_sha === it.blobSha ? ', which is what GitHub holds now' : ', and GitHub has changed since');
      },
