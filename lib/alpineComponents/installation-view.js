...
document.addEventListener('alpine:init', function () {
  Alpine.data('installationView', function (project) {
    ...
    let sourceGeneration = 0, inline = null, draftsLoad = Promise.resolve();
    ...
    return {
      ...
      fileCtx() {
        const view = this;
        return inline = this.fileContext({
          get repo() { return view.repo; }, get ref() { return view.ref || 'main'; }, get revision() { return view.revision; },
          drafts: draftRecords,
          ready: () => draftsLoad,
          selected: () => this.item,
          active: () => 'inline',
          has: p => this.items.some(it => it.path === p && it.comparable),
          go: p => this.select(p),
          publish: () => this.openPublish(),
          onDraft: () => { this.loadDrafts(); },
          view: () => this.sourceView,
          onPane: p => { this.sourceView = p === 'work' ? 'changes' : 'code'; },
          shown: state => { this.sourceState = state; },
          workCopy: () => this.hasChanges ? { label: this.baseline.label, rows: this.diffRows } : null,
        });
      },
