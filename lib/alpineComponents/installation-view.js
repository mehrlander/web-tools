  ...
  Alpine.data('installationView', function (project) {
    ...
    return {
              ...
              <template x-if="item">
                <div class="flex flex-col gap-4">
                  ...
                  <!-- The source at the captured revision, read-only: the Code
                       workspace is where it is edited. Below @3xl the detail
                       column is its own screen, so the code stays collapsed and
                       Record installed stays above the fold. A <pre> stands in
                       when the editor kit cannot load. -->
                  <div class="flex flex-col gap-1.5" data-source>
                    <div class="flex items-center gap-2 text-sm font-medium text-base-content/60">
                      <span>Source</span>
                      <span x-show="sourceState === 'loading'" class="loading loading-dots loading-xs opacity-50"></span>
                      <div class="grow"></div>
                    ...
                    <div :class="sourceOpen ? 'block' : 'hidden @3xl:block'" data-source-body>
                      ...
                      <div :class="sourceState === 'loading' || sourceState === 'editor' ? '' : 'hidden'" data-source-editor
                           class="h-96 @3xl:h-[32rem] rounded-lg border border-base-300 overflow-hidden text-sm"></div>
                      <pre :class="sourceState === 'plain' ? '' : 'hidden'" data-source-plain
                           class="max-h-96 @3xl:max-h-[32rem] overflow-auto rounded-lg border border-base-300 p-2 font-mono text-sm" x-text="source"></pre>
                    </div>
                  </div>
                </div>
              </template>
      ...
      select(path) {
        ...
        this.source = ''; this.sourceOpen = false;
