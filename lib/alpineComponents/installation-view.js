...
document.addEventListener('alpine:init', function () {
  Alpine.data('installationView', function (project) {
    ...
    const STATUS = {
      ...
      changed: { short: 'Update pending', label: 'Update pending: GitHub changed since it was installed', cls: 'ph-fill ph-arrow-circle-up text-warning' },
      ...
      'local-differs': { short: 'Differs', label: 'Differs: a copy compared in this browser', cls: 'ph-fill ph-warning-circle text-error' },
    ...
    const statusMenu = (path, side) => `
      <div class="relative shrink-0" @click.outside="menuFor === ${path} && (menuFor = '')" @keydown.escape.window="menuFor = ''">
        <button @click.stop="toggleMenu(${path})" class="btn btn-ghost btn-sm btn-square" data-status
    ...
    return {
                  ...
                  <div class="flex items-start gap-2">
                    ...
                    <div class="min-w-0 grow">
                      <div class="flex flex-wrap items-center gap-x-2">
                        ...
                        <span class="text-base text-base-content/70" x-text="statusOf(item.path).label"></span>
                      </div>
                  ...
                  <dl class="grid grid-cols-[auto_minmax(0,1fr)] gap-x-4 gap-y-1 text-base">
                    ...
                    <template x-if="item.companion">
                      <dd><button @click="select(item.companion)" class="link link-hover font-mono" x-text="item.companion.split('/').pop()"></button>
                        <i class="ml-1" :class="statusOf(item.companion).cls" :title="statusOf(item.companion).short"></i></dd></template>
                    <dt class="text-base-content/50">GitHub now</dt>
                    <dd class="font-mono" x-text="short(revision) + ' on ' + ref"></dd>
                    <template x-if="stateOf(item.path).latest">
                      <dt class="text-base-content/50">last observed</dt></template>
                    <template x-if="stateOf(item.path).latest">
                      <dd x-text="observedLine(item)"></dd></template>
                  </dl>
      ...
      get headline() {
        const counts = K().summary(Object.values(this.derived));
        // Counted by the list's own words, so two states sharing an icon
        // (a pending update, GitHub changed since) read as one count.
        const byLabel = {};
        for (const s of K().STATES) if (counts[s]) { const w = (STATUS[s] || STATUS.unknown).short.toLowerCase(); byLabel[w] = (byLabel[w] || 0) + counts[s]; }
        const parts = [this.items.length + ' files', ...Object.entries(byLabel).map(([w, n]) => n + ' ' + w)];
      ...
      // A copy compared in this browser, newer than the ledger's last word and
      // not yet recorded, is shown: a paste that differed should not leave
      // the file reading as in sync.
      statusOf(path) {
        const s = this.stateOf(path), check = this.checks.find(c => c.path === path);
        if (check && !check.exact && !check.lineEndingsOnly && !this.isRecorded(check)
            && (!s.latest || String(check.checkedAt) >= String(s.latest.date))) return STATUS['local-differs'];
        return STATUS[s.state] || STATUS.unknown;
      },
      ...
      actionsFor(path) {
        ...
        if (it.comparable && (['changed', 'pending-changed', 'differs', 'differs-changed'].includes(st) || this.statusOf(path) === STATUS['local-differs']))
