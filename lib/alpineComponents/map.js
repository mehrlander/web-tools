document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    // docs/CONSTELLATION.md), in twelve top-level tabs. Docs and Harness each
    // carry a local second level for two readings of one subject.
    ...
    // is held to. Not a registry: the suite holds a registry to being a CSV, and
    ...
    return {
      ...
      template: `
        <div class="w-full">
          ...
          <!-- Growth is a reading of Docs, and Tests is a kind of harness.
          ...
          <section x-show="mapTab==='data'" class="min-w-0">
            <div class="flex flex-wrap items-center gap-2 mb-4">
              ...
              <select class="select select-bordered select-sm w-full shrink-0 sm:w-auto sm:shrink max-w-full" aria-label="Repository scope"
                      x-model="dataScope">
                <option value="" x-text="hasToken() ? 'All repositories' : 'Public repositories'"></option>
                ...
                <div class="mb-5 min-w-0">
                  ...
                  <template x-for="f in g.files" :key="f.repo + ':' + f.path">
                    <button type="button" @click="openDataFile(f)"
                            class="w-full min-w-0 grid grid-cols-3 sm:grid-cols-[minmax(0,1fr)_5rem_5rem_6rem] gap-x-3 gap-y-1 px-2 py-2 text-left rounded hover:bg-base-200 focus-visible:outline focus-visible:outline-primary border-b border-base-200">
...
<!-- Say which axis this is. The badge on every row below is a KIND,
      ...
      themesErr: '',
      lens: 'clusters',
      // The twelve top-level tabs as data, which is what makes the lede below possible.
      ...
      TABS: [
        ...
        { k: 'kits', n: 'Kits', i: 'ph-toolbox',
          g: 'Every logic module on the kit shelf: what its own header says it is, what loads it, and whether a demo shows it running.' },
      ...
      SUBVIEW_PARENT: { growth: 'docs', tests: 'harness' },
      ...
      // ── Data, federated ───────────────────────────────────────────────────
      // The census is authored by each repository's generator and declared in
      // its own manifest. The config cache supplies breadth; one CSV fetch per
      // declaration supplies rows. The source files are never read here.
      dataCensus: null,
      dataLoading: false,
      dataErr: '',
      dataScope: '',
      dataQ: '',
      dataLimit: 200,
      async loadDataCensus(){
        if (this.dataCensus || this.dataLoading) return;
        this.dataLoading = true;
        this.dataErr = '';
        const configs = new Map();
        try {
          const hub = new window.GH({ token: window.TOKEN, repo: this.hub(), ref: useRef() });
          configs.set(this.hub(), { config: JSON.parse((await hub.get('.web-tools.json')).text), ref: useRef() });
        } catch (e) { this.dataErr = 'Hub declaration unavailable: ' + (e?.message || e); }
        if (this.hasToken()) {
          try {
            ...
            for (const [repo, entry] of Object.entries(cache?.repos || {})) {
              if (repo !== this.hub() && entry?.config?.estate === true)
                configs.set(repo, { config: entry.config, ref: 'main' });
            }
          } catch (e) { this.dataErr = 'Estate declarations unavailable: ' + (e?.message || e); }
        }
        const sources = [...configs].map(([repo, entry]) => {
          const path = entry.config?.data?.census;
          return { repo, ref: entry.ref, path: typeof path === 'string' ? path : '',
                   state: typeof path === 'string' && path ? 'loading' : 'undeclared', files: [] };
        }).sort((a, b) => (a.repo === this.hub() ? -1 : b.repo === this.hub() ? 1 : 0)
                           || a.repo.localeCompare(b.repo));
        await Promise.all(sources.filter(s => s.state === 'loading').map(async s => {
          try {
            if (!this.validDataPath(s.path)) throw new Error('invalid declaration path');
            const gh = new window.GH({ token: window.TOKEN, repo: s.repo, ref: s.ref });
            const raw = (await gh.get(s.path)).text;
            const heading = window.Csv.parseLine(raw.split(/\r?\n/, 1)[0]);
            if (heading.join(',') !== 'path,rows,columns,bytes,headers') throw new Error('unexpected census columns');
            const rows = window.Csv.rows(raw);
            const seen = new Set();
            s.files = rows.map(r => {
              if (!this.validDataPath(r.path) || seen.has(r.path)) throw new Error('invalid or duplicate CSV path');
              seen.add(r.path);
              const headers = JSON.parse(r.headers);
              const numbers = ['rows', 'columns', 'bytes'].map(k => Number(r[k]));
              if (!Array.isArray(headers) || !headers.every(h => typeof h === 'string') ||
                  numbers.some(n => !Number.isSafeInteger(n) || n < 0) || headers.length !== numbers[1])
                throw new Error('invalid CSV measurements');
              return { repo: s.repo, ref: s.ref, path: r.path, rows: numbers[0],
      ...
      openDataFile(f){
        const folder = f.path.includes('/') ? f.path.slice(0, f.path.lastIndexOf('/')) : '';
        window.__shell?.goSearch?.({ mode: 'names', repo: f.repo,
          ref: f.ref === 'main' ? '' : f.ref, path: folder,
          file: f.repo + (f.ref === 'main' ? '' : '@' + f.ref) + ':' + f.path });
      },
