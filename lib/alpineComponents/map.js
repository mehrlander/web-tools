  ...
  Alpine.data('map', function() {
    ...
    const SURF_SIBLINGS = [
      ...
      { path: 'docs/surfacing-extended.md', heading: 'Surfacing, extended',
        gloss: 'fetched when an uncommon carrier applies' },
    ];
    ...
    // which; this carrier can, one row per comparison, authored from the code:
                        ...
                        <div class="border border-base-300 rounded-lg p-3 bg-base-100">
                          <div class="flex items-baseline gap-2 flex-wrap">
                            ...
                            <span x-show="r.span === 'estate'" class="badge badge-sm badge-accent"
                                  data-note="The population extends past this repository, so this carrier is an aggregate and a complete enumeration needs the other repos read.">estate</span>
                            <span class="grow"></span>
...
<!-- Say which axis this is. The badge on every row below is a KIND,
                ...
                <!-- ── In the estate ──────────────────────────────────────
                     Other repos' checks, each read from the carrier that repo
          ...
          <!-- ── Views: the app's own destinations, dated ────────────────
               ...
               route with no narrow carrier is a route with no code of its own,
      ...
      loadTab(tab){
        ...
        // Views dates one repo's own carriers, so it reads the hub's history
      ...
      async loadAppViews(force){
        ...
        try {
          ...
          // One last-commit read per declared carrier. The shell's own file
      ...
      async loadTestsReg(){
        ...
        try {
          ...
          // rather than in the carrier, so the declaration and the rows say
