document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    // docs/CONSTELLATION.md), in ten top-level tabs. Docs carries Inventory,
...
<!-- Say which axis this is. The badge on every row below is a KIND,
      ...
      themesErr: '',
      lens: 'clusters',
      // The ten top-level tabs as data, which is what makes the lede below possible.
      ...
      aimsMd(){
        const d = this.aims;
        if (!d) return '';
        return ['# Purpose', '', '## Mission', '', d.mission, '', '## Goals', '']
      ...
      async loadAims(){
        ...
        try {
          const gh = new window.GH({ token: window.TOKEN, repo: this.hub(), ref: useRef() });
          const [raw, goals, reading] = await Promise.all(
            [AIMS_MANIFEST, AIMS_GOALS, AIMS_READING].map(p => gh.get(p).then(r => r.text)));
          const parsed = JSON.parse(raw);
      ...
      deckKnownPaths(path){
        const out = new Set([
          ...
          ROUTES_KINDS, ROUTES_PASTE, SHOWING_MECHANISMS, DOCS_MANIFEST,
