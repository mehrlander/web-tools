document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    // docs/CONSTELLATION.md), in eleven top-level tabs. Docs carries Inventory,
    // Purpose, and Growth; Harness carries Automation and Tests.
    ...
    // 2026-09-10 for that reason and surfacing-extended.md left on 2026-09-07.
    //
    ...
    // with the shape, so tools/test/map-view.test.mjs now checks every declared
    // path exists rather than checking every gloss names a heading. It also
    // catches what the old derivation never could: surfacing-extended.md had
    // been a sibling for three days and no door reached it.
    const SURF_SIBLINGS = [
      { path: 'docs/surfacing-course.md', heading: 'The surfacing course',
        gloss: 'arrives when a pull request is created' },
      { path: 'docs/surfacing-extended.md', heading: 'Surfacing, extended',
        gloss: 'fetched when an uncommon way of handing something over applies' },
    ];
    ...
    // curated gallery of utility PAGES, show-repo's Tools view and
    ...
    // are disjoint. .claude/skills/ holds the skills that travel in the
    // portable plugin and show on Distribution; skills/ holds a separate library
    ...
    const KITS_BUILDER = 'tools/build/kits-index.mjs';
    ...
    const INVOKE_TONE = {
      ...
      argv: 'badge-primary', git: 'badge-accent', session: 'badge-accent',
    ...
    const PREFIXED_INVOKE = ['npm', 'git', 'session', 'hook', 'ci'];
    ...
    return {
      description: 'Map view: the coordination layer made inspectable, in eleven top-level tabs, with Inventory, Purpose, and Growth under Docs and Automation, Tests, and Context under Harness. Docs/Purpose (docs/aims.json with docs/aims-goals.csv and docs/aims-reading.csv): the estate\'s mission, five goals, and reading paths beginning with the repo README, agent contract, and docs index. Distribution (docs/portable.csv, the delivery crosswalk; rows are grouped by how an artifact travels, name its kind, and link to the Skills, Docs, or Automation inventory that owns it; a row\'s title opens the file in the same swipe deck the Docs tab uses, while a `dir` row opens its folder and every row\'s GitHub icon carries the source peek); Surfacing (docs/surfacing.csv, the gated index of SURFACING.md\'s primitives, the prose staying authoritative, with a door under the header to each of the doc\'s other regions and when it reaches a session; the header\'s Read opens the doc in the same swipe deck the Docs tab uses, so docked the prose reads beside the cards rather than replacing them); Showing (docs/routes.json): which link reaches which kind of change and what each one misses, then the address grammar, toss-render\'s delivery modes and their trust postures, and the toss routes; and Docs/Inventory (docs/docs.csv): the documentation registry, every doc\'s subject, status (living, record, or measured), reach (injected, in context, by a skill, by the app, or orphan; derived from the repo and filterable from the strip at the top), size in words with its share of the folder, readership (distinct sessions that opened the file, from the private registry\'s sessions cache; token-gated, and an injected doc says so rather than reading the zero no file tool can avoid giving it), and maintenance behind one details toggle for the whole registry, all navigated from a folder rail whose rows roll up counts and words and carry their own GitHub links; a row\'s title opens the document in a fullscreen swipe deck paging the folder\'s files, while its GitHub icon, inline with the badges, carries the source peek. Docs/Growth frames the repository\'s doc-growth payload as a corpus-wide chart. Themes (docs/themes.csv and docs/owners.csv, ?tab=claims): what the estate says twice, from both ends. Four derived lenses over the duplication graph (clusters, an arc diagram, the repeated passages themselves, and a matrix ordered so a cluster is a block on the diagonal) share one weight threshold, because a cluster is what the dial makes: one blob at 3, isolated pairs at 30, and around 12 a set of clusters each of which is a theme a person would name. Beside them the Owners lens carries the curated registry, for a statement the coordination layer repeats: its one authoritative file, typed repetitions, and per-repetition checks, with an absent check in the warning tone rather than omitted. The accent means one thing throughout, that the registry names neither end of a pair, so the gap between what is measured and what is accounted for is the tab\'s default reading rather than a claim somebody has to make. Named Claims until 2026-08-09 and Owners until 2026-08-31; ?tab=claims still resolves here. Reach counts files and words weighs them, and the strip shows both because on this folder they disagree: the orphans are the larger count and the smaller mass; readership is the third of that set, since reach says who can get to a doc and readership says who did. Harness has three subviews. Context (pages/session-context.html): six current entry routes, each route\'s evidence and limits, an optional session-record inspector, and the explicit gap between context and durable output. Tests (docs/tests.csv): the same registry pointed at the suite, every check\'s kind (gate or behavior) and what breaks if it is deleted, with assertions, method, runner and boot-smoke count derived from the files and gated against the registry. The strip cuts the total by kind, since a pass count cannot tell a boot check from an adversarial gate; a browser check reports no assertion count rather than zero, because test() is not its unit. Automation (docs/harness.csv): the harness registry, one row per executable the repository runs on itself: code under tools/ and scripts/ (tools/test/ excluded; the Tests registry owns it), the git hooks, the session and plugin hooks, and the CI workflows, each row an authored role plus derived layer, lines, invocation route (pulled: npm script, scenario driver, imported helper, argv; event-fired: git:<name>, session:SessionStart, hook:<event>, ci:<events>; or none found, the warning state), whether it writes files, whether prose names it and a test exercises it; navigated from the same folder rail as Docs (the tree as it exists on disk, counts and blank-role figures rolled up to ancestors), with the invocation pills as a separate filter layer that re-weights the rail. And Kits (docs/kits.csv): the browser JavaScript shelf under lib/kits/*.js, one row per module, with nothing authored: the namespace and the gloss are read off the file\'s own header, then whether every chain-boot page pays for it, whether a demo page exists (opened in-app as an app view, so the FAB\'s layer strip and takes aim at it), how many other files load it, and whether a test names it. The strip filters on those, and the three warning states (a blank gloss, a blank namespace, a kit nothing loads) render in the warning tone rather than being hidden, since the shelf held 68 kits before the app showed one of them. Data (data/csv-census.csv): each repository\'s generated inventory of tracked CSV paths, headers, and physical measurements, opened through the shared Files viewer. And Registries (docs/registries.csv): the declaration table the other views hang off, one card per registry (title, gloss, kind, path, gate, target grain, scope, per-declaration chips), grouped by area (files or names), with derived totals rather than prose-carried counts, and per row the derived renders_in list: the app files that name the registry in code, each a peekable link, with a warning badge and a strip figure for registries no app surface reads, since an unread claim is where the registry audits keep finding rot. Per-repo scope and adoption live on the Repos cards. The operational face of the constellation doctrine.',
      ...
      template: `
        <div class="w-full">
          ...
          <!-- Purpose and Growth read Docs; Tests and Context belong with Harness.
               Keep their established address keys while presenting each as a
               smaller choice inside the subject that owns it. -->
          <template x-if="subviews.length">
            <div class="flex items-center gap-0.5 rounded-lg bg-base-200/40 p-0.5 mb-4 w-fit"
                 role="tablist" :aria-label="displayTab + ' views'">
              <template x-for="s in subviews" :key="s.k">
                <button role="tab" @click="setTab(s.k)" :aria-selected="mapTab === s.k"
                        class="flex items-center gap-1.5 px-2.5 py-1 rounded-md text-sm font-medium transition-colors"
              ...
              <template x-for="sec in setSections" :key="sec.key">
                <div>
                  <div class="flex items-baseline gap-2 mb-2">
                    ...
                    <span class="text-sm tabular-nums text-base-content/30" x-text="sec.items.length"></span>
                  </div>
                  <p class="text-sm text-base-content/50 mb-2" x-text="sec.gloss"></p>
                      ...
                      <div class="flex items-start gap-2.5 px-2 py-1.5 rounded-lg hover:bg-base-200/60 group">
                        ...
                        <div class="min-w-0 flex-1">
                          <div class="flex items-center gap-2 flex-wrap">
                            ...
                            <span class="badge badge-outline badge-sm" x-text="kindLabel(it)"></span>
                            ...
                            <button type="button" @click="openSetOwner(it)"
                          ...
                          <code class="block text-xs text-base-content/40 truncate" x-text="it.path"></code>
...
${chip('rendererPath', 'renders')}${regChip('SHOWING_MECHANISMS')}${regChip('ROUTES_MODES')}${regChip('ROUTES_ROUTES')}${regChip('ROUTES_PASTE')}${chip('ROUTES_MANIFEST', 'measured')}
                      ...
                      <div class="border border-base-300 rounded-lg p-3 bg-base-100"
                        ...
                        <div class="flex flex-wrap gap-1.5 mt-2">
                          <template x-for="t in (m.subject || '').split(';').filter(Boolean)" :key="t">
                            <span class="badge badge-ghost badge-sm" :data-note="subjectGloss(t)" x-text="'subject: ' + t"></span>
                          </template>
                    ...
                    <template x-for="k in routes.kinds" :key="k.kind">
                      <div class="border border-base-300 rounded-lg p-2.5 bg-base-100 flex flex-col gap-1.5">
                        <div class="flex items-baseline gap-2 flex-wrap">
                          ...
                          <div class="grow"></div>
                          <span class="badge badge-ghost badge-sm" :data-note="subjectGloss(k.subject)" x-text="k.subject"></span>
                        </div>
                        <div class="text-base text-base-content/60">
                          <span class="text-base-content/40">recognized by</span>
                          <span x-text="k.detect"></span>
                          <span x-show="k.exclusive" class="text-base-content/40" data-note="An exclusive module: it outranks any default mode a host sets."> (exclusive)</span>
                        </div>
                        <div class="flex flex-wrap items-center gap-1">
                          <span x-show="k.view" class="badge badge-sm badge-outline" :data-note="'Opens in the ' + k.view + ' viewer mode'" x-text="'view: ' + k.view"></span>
                          <span x-show="k.route" class="badge badge-sm badge-outline" :data-note="'Addressed by the ' + k.route + ' toss route'" x-text="'#' + k.route + '='"></span>
                          <template x-for="m in (k.shown_by || '').split(';').filter(Boolean)" :key="m">
                            <span class="badge badge-sm badge-ghost" data-note="A showing mechanism from the table above" x-text="m"></span>
                          </template>
                        </div>
                  ...
                  <div class="flex flex-col gap-1">
                    <template x-for="m in routes.modes" :key="m.form">
                      <div class="flex items-start gap-2.5 px-2 py-1.5 rounded-lg hover:bg-base-200/60">
                        <i class="ph mt-1 text-base-content/40 shrink-0" :class="modeIcon(m)" data-note-bare :data-note="m.trust"></i>
                        <div class="min-w-0 flex-1">
                          <div class="flex items-center gap-2 flex-wrap">
                ...
                <div>
                  <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <span class="text-sm text-base-content/40 tabular-nums"
                          :data-note="'Every file under docs/, counted as whitespace-delimited tokens'"
                    ...
                    <template x-if="docGrowthTotal">
                      <span class="text-sm font-medium tabular-nums text-base-content/70"
                            :data-note="docGrowthTotal.n + ' of these files are in the growth payload, which starts ' + docGrowthTotal.from"
                      ...
                      <template x-for="d in docDirFiles" :key="d.path">
                        <div class="flex items-start gap-2.5 px-2 py-1.5 rounded-lg hover:bg-base-200/60">
                          <i class="ph mt-1 text-base-content/40 shrink-0"
                             :class="d.status === 'record' ? 'ph-archive' : 'ph-file-text'" data-note-bare :data-note="d.status"></i>
                          <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-2 flex-wrap">
                              <button type="button" class="text-base font-medium hover:text-primary text-left"
                              ...
                              <span class="badge badge-sm badge-outline" :class="reachMeta(d.reach).tone"
                                    :data-note="reachMeta(d.reach).hint" x-text="reachMeta(d.reach).label"></span>
                              ...
                              <span class="text-sm tabular-nums"
                                    ...
                                    :data-note="d.words.toLocaleString() + ' words, ' + docShare(d) + '% of docs/'"
                              ...
                              <template x-if="growthOf(d.path)">
                                <span class="inline-flex items-center gap-1" :data-note="growthHint(d.path)">
                            ...
                            <p class="text-base text-base-content/60">
                              <span x-text="d.subject"></span><em x-show="docReads && docReadLabel(d)"
                                 ...
                                 :data-note="docReadHint(d)" x-text="docReadLabel(d)"></em>
                            </p>
                  ...
                  <template x-for="c in (ownersReg.owners || [])" :key="c.subject">
                    <div class="border border-base-300 rounded-lg p-3 bg-base-100">
                      <div class="flex items-baseline gap-2 flex-wrap">
                        ...
                        <span x-show="c.kind === 'family'" class="badge badge-ghost badge-sm" :data-note="c.applies_to">family rule</span>
                      </div>
                    ...
                    <div class="border border-base-300 rounded-lg p-3 bg-base-100">
                      <div class="flex items-baseline gap-2 flex-wrap">
                        ...
                        <em x-show="skillUseLabel(s.command)" class="text-sm not-italic text-base-content/40"
                            ...
                            :data-note="skillUseHint(s.command)" x-text="skillUseLabel(s.command)"></em>
                          ...
                          <div class="border border-base-300 rounded-lg p-3 bg-base-100">
                            <div class="flex items-baseline gap-2 flex-wrap">
                              ...
                              <em x-show="skillUseLabel(s.name)" class="text-sm not-italic text-base-content/40"
                                  ...
                                  :data-note="skillUseHint(s.name)" x-text="skillUseLabel(s.name)"></em>
                              <span class="grow"></span>
                  ...
                  <div class="mt-8">
                    <div class="flex items-center gap-3 flex-wrap mb-3">
                      ...
                      <span x-show="estateSkillTotals.forked" class="badge badge-warning badge-sm"
                            data-note="A copy of a hub skill. The plugin ships the current version to that repo already, so the committed copy is what fires and what ages.">
          ...
          <section x-show="mapTab==='data'" class="min-w-0">
            <div class="flex flex-wrap items-center gap-2 mb-4">
              ...
              <label class="input input-bordered input-sm flex items-center gap-2 min-w-0 w-full shrink-0 sm:w-auto sm:shrink sm:flex-1 sm:max-w-sm">
            ...
            <div x-show="dataCensus" class="min-w-0">
              <div class="flex flex-wrap gap-x-5 gap-y-1 border-y border-base-300 py-2 mb-4 text-sm">
              ...
              <template x-for="g in dataFolders" :key="g.repo + ':' + g.folder">
                <div class="mb-5 min-w-0">
                  <div class="flex items-center gap-2 text-sm font-medium border-b border-base-300 pb-1 mb-1 min-w-0">
                    <i class="ph ph-folder text-base-content/50 shrink-0"></i>
                    <span class="truncate" x-text="(dataScope ? '' : g.repo + ' / ') + g.folder"></span>
                    <span class="ml-auto tabular-nums text-base-content/40" x-text="g.files.length"></span>
                  </div>
                  <template x-for="f in g.files" :key="f.repo + ':' + f.path">
                    <button type="button" @click="openDataFile(f)"
                      ...
                      <span class="min-w-0 col-span-3 sm:col-span-1">
                        <span class="flex items-start gap-1.5 text-base font-medium min-w-0">
                          <i class="ph ph-file-csv text-lg text-base-content/50 shrink-0"></i>
                          <span class="break-all" x-text="f.path.slice(f.path.lastIndexOf('/') + 1)"></span>
                        </span>
                        <span class="block text-sm text-base-content/50 break-all pl-6" x-text="f.headers.join(' · ')"></span>
                      </span>
                      <span class="text-sm tabular-nums text-base-content/60 sm:text-right"><span x-text="f.rows.toLocaleString()"></span> rows</span>
                      <span class="text-sm tabular-nums text-base-content/60 sm:text-right"><span x-text="f.columns.toLocaleString()"></span> cols</span>
                      <span class="text-sm tabular-nums text-base-content/60 sm:text-right" x-text="dataSize(f.bytes)"></span>
                    </button>
                  </template>
                </div>
              </template>
              ...
              <button type="button" x-show="dataMatches.length > dataLimit" @click="dataLimit += 200"
                      class="btn btn-ghost btn-sm mb-6" x-text="'Show 200 more of ' + dataMatches.length"></button>
            </div>
          </section>
            ...
            <template x-if="propsReg">
              <div class="">
                <div class="flex items-center gap-4 flex-wrap mb-5 text-sm">
                  ...
                  <span class="text-base-content/50"
                        data-note="Registries whose population extends past this repository, so this checkout is an aggregate rather than the whole set. The rest are bounded by this repo and say nothing about the estate.">
                  ...
                  <span x-show="registryTotals.unrendered" class="text-warning"
                        data-note="Registry files no code under lib/, pages/ or app/ names: committed and gated, read by nobody. The number this tab exists to make impossible to ignore.">
                        ...
                        <div class="border border-base-300 rounded-lg p-3 bg-base-100">
                          <div class="flex items-baseline gap-2 flex-wrap">
                            ...
                            <span x-show="r.inherits" class="badge badge-sm badge-ghost"
                                  data-note-bare :data-note="'descriptions inherited from ' + r.inherits"
                            ...
                            <span x-show="r.span === 'estate'" class="badge badge-sm badge-accent"
                                  data-note="The population extends past this repository, so this registry is an aggregate and a complete enumeration needs the other repos read.">estate</span>
                            <span class="grow"></span>
                            <span x-show="!(r.renders_in || []).length" class="badge badge-warning badge-sm"
                                  data-note="No file under lib/, pages/ or app/ names this registry's file in code, so nothing in the app reads or shows it. A GitHub-rendered projection or a runtime-configured read does not count, and may be the honest answer; the badge asks the question rather than settling it.">no app surface</span>
                            ...
                            <span x-show="!hasGate(r)" class="badge badge-warning badge-sm"
                                  data-note="Nothing fails when this registry and the repo disagree">no gate</span>
                          </div>
                          ...
                          <div class="flex items-baseline gap-2 flex-wrap mt-1 text-sm">
                            ...
                            <span x-show="r.fields !== 'governed'" class="badge badge-warning badge-sm"
                                  :data-note="'fields: ' + r.fields" x-text="r.fields"></span>
                          </div>
                            ...
                            <template x-for="d in r.decls" :key="d.property">
                              <div class="text-sm leading-snug">
                                ...
                                <span x-show="d.required === 'none'"
                                      ...
                                      data-note="optional, or filled by practice with no gate behind it">optional</span>
                                ...
                                <span x-show="d.textKind" class="badge badge-outline badge-xs align-middle ml-1"
                                      data-note-bare :data-note="'a ' + d.textKind + '-kind prose field (docs/text-fields.csv)'"
            ...
            <template x-if="testsReg">
              <div class="flex flex-col gap-8">
                ...
                <div class="flex flex-col gap-2">
                  ...
                  <div class="flex items-center gap-2 flex-wrap">
                    <span class="text-sm text-base-content/40 tabular-nums"
                          :data-note="'Top-level test() calls across the suite. Browser checks are excluded: they assert with their own harness, so test() is not their unit.'"
                  ...
                  <template x-for="d in testDimensions" :key="d.key">
                    <div class="flex items-baseline gap-2 flex-wrap">
                      <span class="text-sm uppercase tracking-wide text-base-content/30 w-16 shrink-0"
                            :data-note="d.question" x-text="d.label"></span>
...
<!-- Say which axis this is. The badge on every row below is a KIND,
                      ...
                      <div class="flex flex-col gap-1">
                        <template x-for="t in grp.tests" :key="t.path">
                          <div class="flex items-start gap-2.5 px-2 py-1.5 rounded-lg hover:bg-base-200/60 group">
                            <i class="ph mt-1 text-base-content/40 shrink-0"
                               :class="t.runner === 'suite' ? 'ph-flask' : 'ph-browser'" data-note-bare :data-note="t.runner"></i>
                            <div class="min-w-0 flex-1">
                              <div class="flex items-center gap-2 flex-wrap">
                                <button type="button" class="text-base font-medium hover:text-primary text-left"
                                ...
                                <!-- Guard on length, not on the array: boot_smoke
                                     ...
                                     row in the registry. -->
                                <span x-show="testExplained && changedSince(t.path)" class="text-sm text-warning/70"
                                      data-note="This file's contents changed after its comparisons were written; the rows describe an older version">changed since read</span>
                                <span x-show="t.boot_smoke?.length" class="text-sm text-warning/70 tabular-nums"
                                      :data-note="'assertions that only check the component mounts; turn on names to see which'"
                              ...
                              <!-- One block per comparison the file makes, read
                                   ...
                                   prove. -->
                              <div x-show="testExplained && explainOf(t).length"
                                ...
                                <template x-for="(n, i) in (t.assertion_names || [])" :key="i">
                                  <li class="flex gap-2 text-sm"
                                    ...
                                    <span x-show="smokeSet(t).has(i)"
                                          ...
                                          data-note="A boot check: it proves the component mounted and logged nothing, and nothing more.">smoke</span>
                                  </li>
                                </template>
                              </ol>
          ...
          <!-- ── Harness / Context ─────────────────────────────────────────
               The graph and record inspector share one page and one model.
               Map owns the destination; the page can still open full-size. -->
          <section x-show="mapTab==='context'">
            <div class="flex items-center gap-2 mb-4 flex-wrap text-sm">
              <button type="button" @click="setTab('docs')" class="text-primary hover:underline">Documents</button>
              <span class="text-base-content/30" aria-hidden="true">/</span>
              <button type="button" @click="setTab('skills')" class="text-primary hover:underline">Skills</button>
              <span class="text-base-content/30" aria-hidden="true">/</span>
              <button type="button" @click="setTab('harness')" class="text-primary hover:underline">Automation</button>
              <span class="grow"></span>
              <a :href="contextUrl" target="_blank" rel="noopener" class="text-base-content/50 hover:text-primary">Full view ↗</a>
              <a :href="deliveryUrl" target="_blank" rel="noopener" class="text-base-content/50 hover:text-primary">Delivery history ↗</a>
            </div>
            <template x-for="u in (contextSeen ? [contextEmbedUrl] : [])" :key="u">
              <iframe :src="u" loading="lazy" title="Session context routes and record inspector"
                      class="w-full rounded-xl border border-base-300 bg-base-100"
                      style="height:clamp(620px,82vh,1050px)"
                      sandbox="allow-scripts allow-same-origin allow-popups"></iframe>
            </template>
          </section>
                      ...
                      <template x-for="t in harnessDirFiles" :key="t.path">
                        <div class="flex items-start gap-2.5 px-2 py-1.5 rounded-lg hover:bg-base-200/60 group">
                          <i class="ph ph-terminal mt-1 text-base-content/40 shrink-0"
                             data-note-bare :data-note="t.emits ? 'emits: writes a file' : 'reads only'"></i>
                          <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-2 flex-wrap">
                              <button type="button" class="text-base font-medium hover:text-primary text-left"
                                      :title="'Read ' + t.path + ' here'"
                              ...
                              <span x-show="!t.named && t.invocation !== 'driver'"
                                    class="text-sm text-warning/70" data-note="no prose names this file">unnamed</span>
                              <span x-show="t.tested" class="text-sm text-base-content/40"
                                    data-note="a file under tools/test/ exercises it">tested</span>
                            </div>
          ...
          <section x-show="mapTab==='kits'">
            <div class="flex items-center gap-2 mb-1 flex-wrap">
              ...
              <a x-blob="peek(KITS_BUILDER)"
                 class="text-base-content/30 hover:text-primary"
                 title="tools/build/kits-index.mjs stamps every field from the tree; nothing here is authored">
                <i class="ph ph-function"></i></a>
              ...
              <div class="flex flex-col gap-5">
                ...
                <div class="grid grid-cols-1 xl:grid-cols-2 gap-x-8 gap-y-1">
                  <template x-for="k in kitRows" :key="k.path">
                    <div class="flex items-start gap-2.5 px-2 py-1.5 rounded-lg hover:bg-base-200/60 group">
                      <i class="ph mt-1 shrink-0"
                         :class="k.boot ? 'ph-lightning text-info' : 'ph-toolbox text-base-content/40'"
                         data-note-bare :data-note="k.boot ? 'loaded on every chain-boot page' : 'loaded by the pages that ask for it'"></i>
                      <div class="min-w-0 flex-1">
                        <div class="flex items-center gap-2 flex-wrap">
                          <button type="button" class="text-base font-medium hover:text-primary text-left"
                                  :title="'Read ' + k.path + ' here'"
                                  @click="openKitDeck(k)" x-text="k.name"></button>
                          <code class="font-mono text-sm" :class="k.namespace ? 'text-base-content/50' : 'text-warning/70'"
                                :data-note="k.namespace ? 'window.' + k.namespace : 'no window.<name> assignment found: nothing can reach it by name'"
                                x-text="k.namespace ? 'window.' + k.namespace : 'no namespace'"></code>
                          <a x-show="k.demo" :href="kitDemoHref(k)"
                          ...
                          <span class="text-sm tabular-nums" :class="k.users ? 'text-base-content/40' : 'text-warning/70'"
                                :data-note="k.users ? k.users + ' file(s) under lib/, pages/ or app/ load it' : 'nothing under lib/, pages/ or app/ loads it'"
                                x-text="k.users ? k.users + (k.users === 1 ? ' load' : ' loads') : 'nothing loads it'"></span>
                          <span x-show="!k.tested" class="text-sm text-warning/70" data-note="no file under tools/test/ names it">untested</span>
                          <span class="text-sm text-base-content/30 tabular-nums" x-text="k.lines + ' lines'"></span>
                        </div>
                        <p class="text-base" :class="k.gloss ? 'text-base-content/60' : 'text-warning/70'"
                           x-text="k.gloss || 'gloss unstated: the header comment does not open with a sentence'"></p>
                      </div>
                      <a x-blob="peek(k.path)" title="Open on GitHub"
                         class="opacity-0 group-hover:opacity-100 focus:opacity-100 text-base-content/30 hover:text-primary transition-opacity shrink-0 mt-1">
                        <i class="ph ph-github-logo"></i></a>
                    </div>
                  </template>
                  ...
                  <template x-for="r in s.rows" :key="r.key">
                    <div :class="s.solo ? '' : 'min-w-0'">
                      ...
                      <div class="flex items-baseline gap-2 flex-wrap">
                        ...
                        <span x-show="!viewIsOpenable(r)" class="text-base font-medium shrink-0"
                      ...
                      <div class="flex items-baseline gap-2 mt-0.5">
                        ...
                        <button @click="toggleViewRow(r.key)"
      ...
      contextSeen: false, // the Context frame loads only when its subview opens
      docsErr: '',
      ownersReg: null,
      ownersLoading: false,
      ...
      themesErr: '',
      lens: 'clusters',
      // The eleven top-level tabs as data, which is what makes the lede below possible.
      ...
      // control. Before 2026-08-31 all but one had nothing at all and the
      // remaining tab had three sentences, the last of which was mechanics.
      ...
      TABS: [
        { k: 'set', n: 'Distribution', i: 'ph-package',
          g: 'The artifacts selected to travel, grouped by delivery route and linked back to the inventory that owns each one.' },
        ...
        { k: 'docs', n: 'Docs', i: 'ph-books',
          g: 'The estate\'s purpose, the documents that describe it, and the changing shape of the documentation corpus.' },
        ...
        { k: 'harness', n: 'Harness', i: 'ph-wrench',
          g: 'Every executable the repository runs on itself, by the route execution arrives on: asked for, or fired by a git, session, hook, or CI event.' },
        { k: 'kits', n: 'Kits', i: 'ph-toolbox',
          g: 'Every browser JavaScript module under lib/kits/*.js: what its own header says it is, what loads it, and whether a demo shows it running.' },
        ...
        { k: 'data', n: 'Data', i: 'ph-table',
          g: 'The tracked CSV files each repository declares, with their paths, physical dimensions, and column headers.' },
        { k: 'views', n: 'Views', i: 'ph-signpost',
          g: 'Every address the app can be sent to, ranked by when the code behind it last moved, and what is open against it now.' },
      ...
      SUBVIEWS: {
        docs: [
          ...
          { k: 'aims', n: 'Purpose', i: 'ph-target',
            g: 'The mission this estate\'s material serves, the five goals it is held to, and the paths into its documentation.' },
        ...
        harness: [
          ...
          { k: 'context', n: 'Context', i: 'ph-git-branch',
            g: 'Six routes by which material enters a session, the evidence each leaves, and the missing causal join between context and durable work.' },
        ],
      },
      SUBVIEW_PARENT: { aims: 'docs', growth: 'docs', tests: 'harness', context: 'harness' },
      get displayTab(){ return this.SUBVIEW_PARENT[this.mapTab] || this.mapTab; },
      get subviews(){ return this.SUBVIEWS[this.displayTab] || []; },
      get tabGloss(){
        return this.subviews.find(s => s.k === this.mapTab)?.g
          || this.TABS.find(t => t.k === this.displayTab)?.g || '';
      },
      THEME_LENSES: [
      ...
      init(){
        ...
        // The note kit. Every legend in these eleven tabs and their subviews is a note now, and
      ...
      loadTab(tab){
        ...
        else if (tab === 'context') this.contextSeen = true;
      ...
      get setSections(){
        ...
        return defs
          ...
          .filter(s => s.total && (!this.setQ.trim() || s.items.length));
      },
      get setTally(){
      ...
      setOwner(it){
        ...
        const pluginSkill = String(it.path || '').match(/^\.claude\/skills\/([^/]+)(?:\/|$)/)?.[1] || '';
      ...
      // ── Views: the app's own destinations, dated ──────────────────────────
      // Every address show-repo can be sent to, ranked by when the code behind
            ...
            try {
              ...
              for (const [repo, entry] of Object.entries(cache?.repos || {})) {
                if (repo !== hubRepo && entry?.config?.estate === true)
                  configs.set(repo, { config: entry.config, ref: 'main' });
              }
      ...
      async openDataFile(f){
        ...
        if (browser.repo !== f.repo || browser.ref !== f.ref) return;
      ...
      // The current content routes and a session-record inspector. The page
      // links the earlier delivery snapshots; ?use= must reach the new HTML.
      get contextUrl(){
        const ref = useRef();
        return ref && ref !== 'main'
          ? '../pages/toss-render.html#gh=' + this.hub() + '@' + ref + ':pages/session-context.html?use=' + encodeURIComponent(ref)
          : '../pages/session-context.html';
      },
      get contextEmbedUrl(){
      ...
      get deliveryUrl(){
        const ref = useRef();
        return ref && ref !== 'main'
          ? '../pages/toss-render.html#gh=' + this.hub() + '@' + ref + ':pages/delivery.html?use=' + encodeURIComponent(ref)
          : '../pages/delivery.html';
      },
      ...
      get docQueryRows(){
        return (this.docsReg?.documents || []).filter(d =>
          this.docTextHit(d) && (!this.docReach || d.reach === this.docReach));
      },
      get docTally(){
        return { shown: this.docQueryRows.length, total: (this.docsReg?.documents || []).length };
      },
      selectDocFolder(dir){
        if (this.docQ.trim()) this.docSearchDir = dir;
        else this.docDir = dir;
      },
      docFolderSelected(dir){
        return this.docQ.trim() ? this.docSearchDir === dir : this.docDir === dir;
      },
      ...
      get docFolders(){
        const agg = new Map();
        for (const d of (this.docsReg?.documents || [])) {
          const hit = this.docTextHit(d) && (!this.docReach || d.reach === this.docReach);
      ...
      get docDirFiles(){
        ...
        if (this.docQ.trim()) {
          ...
          rows = rows.filter(d => d.path.slice(0, d.path.lastIndexOf('/')) === this.docDir);
        }
      ...
      get docDirGloss(){
        if (this.docQ.trim()) return '';
        const row = (this.docsReg?.documents || []).find(d => d.path === this.docDir + '/README.md');
        return row ? row.subject : '';
      },
      get docResultHeading(){
        if (!this.docQ.trim()) return this.docDir + '/';
        return this.docSearchDir ? this.docSearchDir + '/' : 'Search results';
      },
      docResultTitle(d){ return this.docQ.trim() ? d.path : this.docTitle(d); },
      ...
      deckKnownPaths(path){
        const out = new Set([
          ...
          TESTS_MANIFEST, TOOLS_MANIFEST, KITS_MANIFEST, SKILLS_MANIFEST,
      ...
      async docDeckRead(host, path){
        ...
        const paths = reading === 'rendered' ? await this.deckRepoPaths(path) : new Set();
        ...
        if (actual === 'rendered') window.mdDoc?.linkRepoFiles?.(host, {
          path,
          paths,
      ...
      openDocDeck(d){
        ...
        return this.openFileDeck(files, files.findIndex(f => f.path === d.path), {
          ...
          key: searchKey
            ...
            : 'docs:' + this.docDir + ':' + this.docReach + ':' + this.docSort,
          ...
          label: (d2) => ({ title: searchKey ? d2.path : this.docTitle(d2), subtitle: d2.subject || '' }),
        });
      },
      ...
      get docReachCounts(){
        ...
        const rows = (this.docsReg?.documents || []).filter(d => this.docTextHit(d));
        for (const d of rows) {
      ...
      skillSetGloss(key){
        ...
        }[key] || 'Three sets, one search: the plugin installs in every session and fires unasked, so each skill costs context everywhere; the library waits for /load-skill and costs nothing until then; the estate rides only the repo that committed it.';
      },
      ...
      checkingSources(cache, query){
        ...
        for (const [repo, e] of Object.entries(cache?.repos || {})) {
          ...
          out.push({ repo, ref: 'main', files: c.files || '', comparisons: c.comparisons, asOf: c.as_of || '', from: 'crawl' });
        }
        for (const part of String(query || '').split(';').map(x => x.trim()).filter(Boolean)) {
          ...
          const src = { repo, ref: m[2] || 'main', files: m[3], comparisons: m[4], from: 'query' };
      ...
      get harnessInvokeCounts(){
        ...
        return ['npm', 'driver', 'imported', 'argv', 'git', 'session', 'hook', 'ci', 'none found'].map(key => ({
          key,
      ...
      get kitFilterCounts(){
        ...
        const defs = [
          ...
          { key: 'untested', label: 'untested', tone: 'badge-warning', gloss: 'no file under tools/test/ names it', hit: k => !k.tested },
        ];
