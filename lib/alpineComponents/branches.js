      ...
      template: `
        <div>
          <div class="flex items-center gap-2 mb-3">
            ...
            <span x-show="!loading" class="text-sm text-base-content/40 font-mono" x-text="list.length + ' total'"></span>
            <span x-show="surveying" class="flex items-center gap-1.5 text-sm text-base-content/40 font-mono">
              <span class="loading loading-spinner loading-sm"></span>
              <span x-text="'surveying ' + surveyedCount + '/' + surveyTarget"></span>
            </span>
            <span x-show="fromCache && !surveying" class="text-sm text-base-content/40 flex items-center gap-1"
            ...
            <div class="grow"></div>
            <button @click="reload()" :disabled="surveying || loading" class="btn btn-ghost btn-sm gap-1"
          ...
          <div x-show="!loading && !error && list.length">
                    ...
                    <tr class="hover:bg-base-200/40 align-top">
                      <td>
                        <button @click="openBranch(r)" :title="'Browse ' + r.name + ' here'"
                                class="font-mono text-sm font-medium hover:text-primary transition-colors cursor-pointer
                        ...
                        <span class="text-sm text-base-content/50 truncate block" :title="r.subject" x-text="r.subject"></span>
                      </td>
                      <td x-show="activeTab!=='active'" class="text-right whitespace-nowrap font-mono text-sm align-middle">
                        <span x-show="r.state==='pending'" class="loading loading-dots loading-sm opacity-30"></span>
                      ...
                      <td class="text-right whitespace-nowrap text-sm text-base-content/40 align-middle" x-text="r.ago"></td>
            ...
            <div x-show="activeTab!=='active' && unsurveyed > 0" class="flex items-center gap-2 mt-3 px-1 text-sm text-base-content/50">
              <span x-text="'Surveyed ' + surveyedCount + ' of ' + older.length + ' older branches.'"></span>
              <button @click="surveyAll()" :disabled="surveying" class="btn btn-ghost btn-sm gap-1 text-primary">
                <i class="ph ph-magnifying-glass"></i>Survey all
              </button>
            </div>

            <p x-show="treeTruncated" class="text-sm text-warning flex items-center gap-1.5 mt-3 px-1">
