      ...
      template: `
        <div>
          ...
          <p class="text-sm text-base-content/60 mb-1">
            Advisory, read-only. Per branch: which uniquely-touched paths hold bytes the default branch
            holds right now (same path, or moved anywhere in the tree). <span class="font-medium">Landed</span> is that count;
            <span class="font-medium">missing</span> is paths absent in both path and bytes, the strong stranded evidence.
            The delete / reconcile call stays yours, per branch, on GitHub.
          </p>
          <p class="text-xs text-base-content/40 mb-4">
            Ahead-counts marked * span the branch's whole line (no shared ancestor with today's default branch), so read
            the content columns, not the counts. Surveys up to 500 branches (100 without a token); per-branch diffs read up to 300 files.
          </p>
