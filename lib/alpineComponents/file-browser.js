...
const registerFileBrowser = function() {
  Alpine.data('fileBrowser', function(opts) {
    ...
    // tree column, which does not contain the reader, so a tapped file loaded
    // and was never shown. Opening from the address ran from init and passed.
    let root = null;
    return {
      ...
      async open(path) {
        ...
        const picker = root?.querySelector('[x-data^="pathPicker"]')?.__pathPicker;
        ...
        try {
          ...
          this.busy = false;
          await this.$nextTick();
          await root?.querySelector('[data-file-viewer]')?.__viewer
