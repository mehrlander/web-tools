  ...
  Alpine.data('fileBrowser', function(opts) {
    ...
    return {
      ...
      init() {
        ...
        // Keyed on the folder as well as its files: a folder with none never
        // changes the list, and still needs the picker's cursor cleared.
        this.$watch(() => (this.picker?.scope || []).map(n => n.name).join('/') + '\n' + this.folderFiles.join('\n'),
          () => this.buildStrip());
      ...
      buildStrip() {
        ...
        this.place(true);
        // A folder with no files has no selection, yet the picker starts its
        // cursor on the top row, lit like one. Clear it; a hover still sets it.
        if (!stripFiles.length && this.picker) this.picker.active = -1;
      },
