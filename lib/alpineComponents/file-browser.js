    ...
    return {
      ...
      template: `
        <div class="h-full min-h-0 flex flex-col gap-2" data-file-browser>
          ...
          <div class="min-h-0 basis-2/5 shrink-0" @path-pick="pick($event.detail.path)">
            <div class="h-full" x-data="pathPicker({ inline: true, trigger: false, base: { repo: fbRepo, ref: fbRef, dir: fbDir }, start: fbStart })"></div>
          </div>

          <!-- The swiper: one header row, then the strip. -->
          <div class="relative flex-1 min-h-0 flex flex-col rounded-box border border-base-300 bg-base-100"
               @pointerenter="hovered = true" @pointerleave="hovered = false" @pointerdown="hovered = true">
            <div class="flex items-center gap-1 shrink-0 pl-2 pr-1 py-1 bg-base-200 border-b border-base-300 rounded-t-box min-h-10">
              ...
              <span x-show="at < 0" class="text-sm text-base-content/50">Pick a file to read it.</span>
              <div class="grow"></div>
              ...
              <!-- The deck door over the same files, full screen. Classes and
                   wording are swipeDeck.entry()'s, held by
      ...
      init() {
        ...
        this.$watch('at', () => {
          ...
          this.mountNear();
          if (this.picker) this.picker.picked = this.at >= 0 ? this.sel : '';
          this.revealInTree();
        });
        keyHandler = (e) => {
      ...
      // The slide nearest the strip's left edge; the gap between slides makes
      // scrollLeft over width drift, the branch detail's reading.
      stripScroll() {
        const s = this.strip();
      ...
      mountSlide(path, slide, pad = 'p-3') {
        if (!path || !slide) return;
        const el = document.createElement('div');
        el.className = 'flex flex-col h-full w-full min-w-0 overflow-hidden ' + pad;
