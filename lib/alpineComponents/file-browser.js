      ...
      async openDeck() {
        ...
        try {
          ...
          const opts = {
            ...
            onSlide: (i) => {
              const h = this._deck, c = this._deckChrome(i);
              if (h) { h.setTitle(c.title); h.setSubtitle(c.subtitle); h.setIcon(c.icon); h.setLink(c.link); }
              if (this._deckFiles?.[i]) this.open(this._deckFiles[i]);
            },
