const ViewRegistry = {
    ...
    },
    {
      ...
      after: (f, ctx) => {
        requestAnimationFrame(async () => {
          ...
          try {
            const delimited = ['csv', 'tsv'].includes(f.ext) ? ViewRegistry.tableDelimited(f) : null;
            const rows = delimited ? delimited.rows : ViewRegistry.tableRows(f);
            const filter = ctx.opts?.filter;
            const filterOpts = filter && delimited ? {
              ...ctx.opts,
              filter: { ...filter, col: delimited.columns.find(c => c.title === filter.col)?.field ?? filter.col },
            } : ctx.opts;
            ...
            if (window.TabularExplorer) {
              ...
              ViewRegistry.showHeaderFilter(exp.table, filterOpts);
              ...
              const table = new Tabulator(target, {
                data: rows,
                ...(delimited ? {
                  columns: delimited.columns.map(c => ({ ...c, title: ViewRegistry.esc(c.title), headerFilter: 'input' })),
                ...
                layout: "fitData",
                height: "100%",
                ...ViewRegistry.headerFilter(filterOpts),
              });
              ViewRegistry.showHeaderFilter(table, filterOpts);
    ...
    },
    {
      ...
      after: async (f, ctx) => {
        ...
        try {
          ...
          if (!window.pdf) await ViewRegistry.loadLib('kits/pdf.js');
          if (stale()) return;
    ...
    },
    {
      ...
      after: async (f, host) => {
        ...
        try {
          ...
          if (!window.XlsxExtract) await ViewRegistry.loadLib('kits/xlsx-extract.js').catch(() => {});
          if (stale()) return;
          ...
          const wireExtract = () => {
            ...
            box.querySelector('[data-xs="ex-save"]').onclick = async () => {
              ...
              if (!window.io) await ViewRegistry.loadLib('kits/io.js').catch(() => {});
  ...
  parseDelimited(text, ch) {
    ...
    const rows = [];
    let row = [], field = '', quoted = false, record = false;
    for (let i = 0; i < s.length; i++) {
      ...
      if (quoted) {
        ...
        row.push(field); rows.push(row); row = []; field = ''; record = false;
        if (c === '\r' && s[i + 1] === '\n') i++;
      }
      else field += c;
    }
    if (record || row.length) { row.push(field); rows.push(row); }
    return rows;
  },
  ...
  // Source titles stay separate from unique record keys, including for a file
  // with only a header. A fallback key must not overwrite an earlier column.
  tableDelimited(f) {
    ...
    const columns = rows[0].map((h, i) => {
      ...
      let key = base && !seen.has(base) ? base : 'col' + (i + 1);
      while (seen.has(key)) key += '_';
      seen.add(key);
      return { field: key, title: String(h) };
    });
    return {
      rows: rows.slice(1).map(r => Object.fromEntries(columns.map((c, i) => [c.field, r[i] ?? '']))),
      columns,
    };
  },
  tableRows(f) {
    if (f.ext === 'csv' || f.ext === 'tsv') return this.tableDelimited(f).rows;
    const parsed = JSON.parse(f.content);
    if (!Array.isArray(parsed)) throw new Error('expected a JSON array of records');
    return parsed;
  },
  // A lib file, through the page's loader. gh.load honours ?use=, so a branch
  // preview reads the branch's copy of the kit rather than main's. Every page
  // that embeds the viewer boots gh (lib/entry.js), so there is no second route.
  // Async so a missing loader rejects, and a caller's .catch still holds.
  async loadLib(path) { return window.gh.load(path); },
  ...
  async openWorkbook(f, host) {
    ...
    if (!window.xlsxKit) await this.loadLib('kits/xlsx.js');
    ...
    if (!window.Note) await this.loadLib('kits/note.js').catch(() => {});
    let result;
    try {
  ...
  mountSheetTabs(tabs, sheets, show, onThemeChange) {
    ...
    if (onThemeChange) {
      ...
      themeBox.className = 'ml-auto flex items-center gap-1.5 text-xs text-base-content/70';
      themeBox.innerHTML = `
        <span class="font-medium opacity-60">Theme</span>
        <select data-xl-theme class="select select-xs select-bordered py-0 h-6 min-h-0 text-xs">
