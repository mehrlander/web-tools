        ...
        try {
          ...
          const cellLabel = (kind, n) => {
            ...
            return `<span class="truncate leading-tight">${esc(kind.label)}</span>
                    <span class="font-mono text-xs leading-tight ${n ? 'opacity-70' : 'opacity-30'}">${kept}</span>`;
          };
          ...
          const cellBtn = (sheetName, kind) => {
            ...
            return `<button data-ex-cell="${esc(kind.id)}" data-ex-sheet="${sheetName == null ? '' : esc(sheetName)}"
                      class="btn btn-xs h-auto min-h-0 flex-col items-stretch gap-0 px-2 py-1 text-left border ${on ? ON : OFF}"
                      ${n ? '' : 'disabled'}>${cellLabel(kind, n)}</button>`;
          };

          const sheetBlock = (sh) => `<div class="rounded border border-base-300 overflow-hidden">
            <button data-ex-sheetrow="${esc(sh.name)}"
                    class="w-full flex items-center gap-2 px-2 py-1 text-left text-sm bg-base-100 ${
                      picked.sheets.has(sh.name) ? 'text-primary' : ''}">
            ...
            <div class="grid gap-1 p-1" style="grid-template-columns:repeat(auto-fit,minmax(7.5rem,1fr))">
          ...
          const extractSurface = () => {
            ...
            return `<div data-xs="ex" class="flex flex-col gap-3 p-3">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span data-xs="ex-fig" class="font-mono text-xs opacity-70 basis-full sm:basis-auto sm:grow">${figures()}</span>
              ...
              <div class="flex flex-wrap items-stretch gap-1">
                ${survey.kinds.map((k, i) => {
                  const any = k.scope === 'workbook'
                    ? survey.workbook[k.id]
                    : survey.sheets.some(sh => sh.counts[k.id]);
                  const on = picked.kinds.has(k.id);
                  // The rule between the two scopes, rather than a heading
                  // over each: the matrix below already separates them into
                  // sheet blocks and a workbook block, so this only has to
                  // say that the strip has two halves.
                  const rule = i && survey.kinds[i - 1].scope !== k.scope
                    ? '<span class="w-px bg-base-300 mx-1"></span>' : '';
                  return `${rule}<button data-ex-kind="${esc(k.id)}" ${any ? '' : 'disabled'}
                            class="btn btn-xs border ${on ? ON : OFF}"
                            >${esc(k.label)}</button>`;
                }).join('')}
              </div>
              <div class="flex flex-col gap-2">
                ${survey.sheets.map(sheetBlock).join('')}
                <div class="rounded border border-dashed border-base-300 overflow-hidden">
                  <div class="px-2 py-1 bg-base-100 flex items-center gap-2 text-sm">
