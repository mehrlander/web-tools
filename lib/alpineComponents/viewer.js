const ViewRegistry = {
  ...
  modules: [
    ...
    {
      // THE DOCUMENT WITH ITS HISTORY. The collection's revisions say one passage
      // became another in a commit. kits/md-history.js finds the blocks some
      // revision led into, composes the file with each block's predecessor in
      // place, and hands both to md-diff, so a paragraph is a swipeable
      // container between what it said and what it says, with the chain of
      // commits and the proposals made along it underneath.
      id: 'history', label: 'History', icon: 'ph-clock-counter-clockwise',
      test: (f) => f.ext === 'md',
      render: () => `<div class="overflow-auto h-full w-full bg-base-100">
          <div class="px-6 py-4" data-md-history>
            <div class="text-sm opacity-60 italic">Reading the revisions…</div>
          </div>
        </div>`,
      after: async (f, ctx) => {
        ...
        try {
          ...
          if (!handle.count) {
            const none = document.createElement('div');
            none.className = 'mb-3 text-sm opacity-60 italic';
            none.textContent = 'No revision in the collection leads to a paragraph of this file as it stands.';
            box.prepend(none);
          }
        } catch (error) {
          if (!alive()) return;
