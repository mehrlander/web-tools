      ...
      after: async (f, host) => {
        ...
        try {
          ...
          const cellLabel = (kind, n) => {
            ...
            return `<span class="truncate">${esc(kind.label)}</span>
                    <span class="font-mono text-xs ${n ? 'opacity-70' : 'opacity-30'}">${kept}</span>`;
          };
          ...
          const cellBtn = (sheetName, kind) => {
            ...
            return `<button data-ex-cell="${esc(kind.id)}" data-ex-sheet="${sheetName == null ? '' : esc(sheetName)}"
                      class="btn btn-xs justify-between gap-2 ${on ? 'btn-primary' : 'btn-ghost border border-base-300'}"
          ...
          const sheetBlock = (sh) => `<div class="rounded border border-base-300 overflow-hidden">
            <button data-ex-sheetrow="${esc(sh.name)}"
                    class="w-full flex items-center gap-2 px-2 py-1 text-left ${
                      picked.sheets.has(sh.name) ? 'bg-primary/10 text-primary' : 'bg-base-100'}">
              ...
              <div class="flex flex-wrap items-stretch gap-1">
                ${survey.kinds.map((k, i) => {
                  ...
                  return `${rule}<button data-ex-kind="${esc(k.id)}" ${any ? '' : 'disabled'}
                            class="btn btn-xs ${on ? 'btn-primary' : 'btn-ghost border border-base-300'}"
                            >${esc(k.label)}</button>`;
                }).join('')}
              </div>
          ...
          const repaintExtract = () => {
            ...
            if (!box) return;
            for (const b of box.querySelectorAll('[data-ex-kind]')) {
              b.classList.toggle('btn-primary', picked.kinds.has(b.dataset.exKind));
              b.classList.toggle('btn-ghost', !picked.kinds.has(b.dataset.exKind));
            }
            for (const b of box.querySelectorAll('[data-ex-sheetrow]')) {
              ...
              b.classList.toggle('bg-base-100', !on);
