    ...
    },
    {
      ...
      after: async (f, host) => {
        ...
        try {
          ...
          // Which sheet the Columns view profiles. Held out here because the
          // picker rebuilds the table without re-reading the workbook.
          let pick = 0;
          const profileRows = () => {
            const s = sheets[pick]?.s;
            if (!s) return [];
            return K.profileColumns(s, xl).map(c => ({
              Column: c.letter,
              Header: c.header,
              Role: c.role,
              Kind: c.kind,
          ...
          const VIEWS = [
            { id: 'sheets', label: 'Sheets', rows: () => K.views.connections(result) },
          ...
          const paint = () => {
            stage.innerHTML = current.m ? mSource() : `${
              current.picker && sheets.length > 1 ? `<div class="px-3 py-2 border-b border-base-300 bg-base-100">
                <select data-xs="pick" class="select select-sm select-bordered">${
                  sheets.map(({ key, s }, i) =>
