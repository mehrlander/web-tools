      ...
      after: async (f, host) => {
        ...
        try {
          ...
          const paintFigures = (err) => {
            ...
            for (const [k, on] of [['ex-save', any], ['ex-copy', linkable], ['ex-open', linkable], ['ex-stage', any]]) {
              const b = stage.querySelector(`[data-xs="${k}"]`);
              if (b) b.disabled = !on;
            }
          };
          ...
          const sheetState = (sh) => {
            const available = SHEET_KINDS.filter(k => sh.counts[k.id]).length;
            const selected = picked.sheets.get(sh.name)?.size || 0;
            return selected ? (selected === available ? 'all' : 'some') : 'none';
          };

          const sheetBand = (sh) => `<button type="button" data-ex-sheetrow="${esc(sh.name)}"
                    class="flex w-full items-center gap-2 rounded bg-base-200/70 px-2 py-1.5 text-left text-sm ${sheetState(sh) !== 'none' ? 'text-primary' : ''}"
                    aria-pressed="${sheetState(sh) === 'all'}" ${SHEET_KINDS.some(k => sh.counts[k.id]) ? '' : 'disabled'}>
              <i class="ph ${sheetState(sh) === 'all' ? 'ph-check-square' : sheetState(sh) === 'some' ? 'ph-minus-square' : 'ph-square'}"></i>
          ...
          const receptionBand = () => {
            ...
            const left = reception.omitted.length
              ? `Left behind by declaration: ${esc(reception.omitted.join(', '))}. ` : '';
            return `<div data-xs="ex-reception" class="rounded border border-primary/40 bg-primary/10 p-2 text-sm">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <button data-xs="ex-stage" class="btn btn-xs btn-primary ml-auto" disabled>
                  <i class="ph ph-stack"></i> Stage for ${esc(r.repo.split('/').pop())}</button>
              </div>
              <div class="pt-1 font-mono text-xs opacity-70">→ ${esc(dest)}/${esc(t.file)}</div>
              <div class="pt-1 text-xs opacity-70">${left}The workbook comes off the stage as the extract goes on, so a Send carries only the extract.</div>
              ${r.note ? `<div class="pt-1 text-xs opacity-70">${esc(r.note)}</div>` : ''}
            </div>`;
          };
          ...
          const extractSurface = () => {
            ...
            return `<div data-xs="ex" class="flex min-w-0 flex-col gap-3 p-3">
              <p class="text-sm text-base-content/70">Choose sheets, then untick any readings to leave out. Related objects are separate choices.</p>
              <p class="text-xs text-base-content/60">Included items may still mention an omitted sheet by name. Charts and VBA are not extracted yet.</p>
          ...
          const repaintExtract = () => {
            ...
            for (const b of box.querySelectorAll('[data-ex-sheetrow]')) {
              ...
              const state = sheetState(sh);
              b.classList.toggle('text-primary', state !== 'none');
              b.setAttribute('aria-pressed', String(state === 'all'));
              const i = b.querySelector('i');
              if (i) i.className = 'ph ' + (state === 'all' ? 'ph-check-square' : state === 'some' ? 'ph-minus-square' : 'ph-square');
            }
          ...
          const wireExtract = () => {
            ...
            box.addEventListener('click', (e) => {
              ...
              if (cell) {
                ...
                if (sheetState(sh) === 'all') picked.sheets.delete(name);
