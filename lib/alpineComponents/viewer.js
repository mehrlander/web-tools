      ...
      after: async (f, host) => {
        ...
        try {
          ...
          const VIEWS = [
            ...
            { id: 'columns', label: 'Columns', rows: profileRows, picker: true },
          ...
          const paint = () => {
            stage.innerHTML = current.m ? mSource() : `${
              current.picker ? `<div class="px-3 py-2 border-b border-base-300 bg-base-100 flex flex-wrap items-center gap-2">
                ${sheets.length > 1 ? `<select data-xs="pick" class="select select-sm select-bordered">${
                  sheets.map(({ key, s }, i) =>
                    `<option value="${i}"${i === pick ? ' selected' : ''}>${esc(s.name || key)}</option>`).join('')
                }</select>` : ''}
                <label class="flex items-center gap-1.5 text-xs">Header row
            ...
            if (picker) picker.onchange = (e) => { pick = Number(e.target.value); paint(); };
