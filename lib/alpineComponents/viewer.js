  ...
  modules: [
    {
      ...
      test: () => true,
      render: (f) => `<pre class="m-0 p-4 h-full overflow-auto text-sm leading-5 font-mono whitespace-pre-wrap text-base-content">${ViewRegistry.esc(f.content)}</pre>`
    },
    {
    ...
    },
    {
      ...
      render: () => `<div class="flex flex-col h-full w-full">
        <div class="flex items-center gap-4 px-3 py-1.5 border-b border-base-300 bg-base-200/50 text-sm shrink-0">
      ...
      after: (f) => {
        requestAnimationFrame(() => {
          ...
          try {
            ...
            target.innerHTML = `<div class="p-4 text-error font-mono text-sm">Invalid JSON Array for Table View</div>`;
          }
        });
      }
    },
      ...
      after: (f) => {
        requestAnimationFrame(async () => {
          ...
          try {
            ...
            target.innerHTML = `<pre class="p-4 text-error font-mono text-sm">Failed to load JSON editor: ${ViewRegistry.esc(e?.message || e)}</pre>`;
            return;
          }
          let parsed;
          try { parsed = JSON.parse(f.content); }
          catch (e) {
            target.innerHTML = `<pre class="p-4 text-error font-mono text-sm">Invalid JSON: ${ViewRegistry.esc(e.message)}</pre>`;
            return;
          }
...
document.addEventListener('alpine:init', function() {
  Alpine.data('viewer', function(opts) {
    ...
    return {
      ...
      template: `
        <div class="flex items-center justify-between mb-2 gap-2" x-show="file">
          <div class="flex items-baseline gap-2 min-w-0">
            <span class="text-sm font-mono truncate" x-text="file" :title="file"></span>
            <span class="text-sm text-base-content/50 font-mono whitespace-nowrap" x-text="stats"></span>
          </div>
      ...
      get ref() {
        if (this.origin) return this.origin.ref || 'main';
        return Alpine.store('browser')?.ref || 'main';
      },
