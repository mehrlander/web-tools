const ViewRegistry = {
  ...
  modules: [
    ...
    },
    {
      ...
      test: (f) => ['js','ts','py','sh','ps1','psm1','psd1','ps1xml','xaml','html','md','json','yml','css','rb','rs','go','java','cpp','c','sql','xml'].includes(f.ext),
    ...
    {
      id: 'ast', label: 'Outline', icon: 'ph-tree-structure',
      test: (f) => ['ps1', 'psm1', 'psd1', 'xaml'].includes(f.ext),
      render: (f) => {
        const esc = ViewRegistry.esc;
        if (!window.PowerShellIde) {
          return '<div class="p-6 font-mono text-sm text-base-content/50">PowerShell IDE intelligence loading...</div>';
        }
        if (f.ext === 'xaml') {
          const info = window.PowerShellIde.parseXaml(f.content);
          return `
            <div class="h-full overflow-auto p-4 flex flex-col gap-4 text-xs font-mono">
              <div class="flex items-center justify-between border-b border-base-200 pb-2">
                <span class="font-semibold text-primary">${esc(info.rootTag)} Root Element</span>
                <span class="badge badge-sm badge-ghost">${info.lineCount} lines</span>
              </div>
              <div class="flex flex-col gap-2">
                <span class="font-semibold uppercase tracking-wider text-base-content/50 text-2xs">Named Controls (${info.controls.length})</span>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  ${info.controls.map(c => `
                    <div class="rounded border border-base-300 bg-base-100 p-2 flex items-center justify-between">
                      <span class="font-bold text-primary">${esc(c.name)}</span>
                      <span class="badge badge-xs badge-neutral">&lt;${esc(c.tag)}&gt;</span>
                    </div>`).join('') || '<span class="text-base-content/40 italic">No named controls</span>'}
                </div>
              </div>
              ${info.dynamicResources.length ? `
                <div class="flex flex-col gap-1">
                  <span class="font-semibold uppercase tracking-wider text-base-content/50 text-2xs">Dynamic Resources (${info.dynamicResources.length})</span>
                  <div class="flex flex-wrap gap-1">
                    ${info.dynamicResources.map(r => `<span class="badge badge-xs badge-info font-mono">${esc(r)}</span>`).join('')}
                  </div>
                </div>` : ''}
            </div>`;
        }

        const info = window.PowerShellIde.parseScript(f.content);
        return `
          <div class="h-full overflow-auto p-4 flex flex-col gap-4 text-xs">
            <div class="flex flex-wrap items-center justify-between border-b border-base-200 pb-2 gap-2 font-mono">
              <div class="flex items-center gap-2">
                <span class="font-semibold text-primary">${esc(f.name)}</span>
                ${info.compatibility?.length ? `
                  <span class="badge badge-sm badge-warning">${info.compatibility.length} PS 5.1 issue(s)</span>
                ` : '<span class="badge badge-sm badge-success">PS 5.1 OK</span>'}
              </div>
              <span class="badge badge-sm badge-ghost">${info.lineCount} lines</span>
            </div>
            ...
            ${info.compatibility?.length ? `
              <div class="flex flex-col gap-1.5 border border-warning/40 bg-warning/10 rounded-lg p-3">
                <span class="font-mono font-semibold text-warning text-2xs uppercase tracking-wider">PS 5.1 Compatibility Warnings</span>
                <div class="flex flex-col gap-1">
                  ${info.compatibility.map(c => `
                    <div class="rounded bg-base-100 p-2 text-2xs flex flex-col gap-0.5 border border-warning/30">
                      <div class="flex items-center justify-between font-mono font-bold text-warning">
                        <span>${esc(c.rule)}</span>
                        <span>L${c.line}</span>
                      </div>
                      <p class="text-base-content/80">${esc(c.message)}</p>
                      <pre class="bg-base-200/50 p-1 rounded font-mono truncate text-base-content/70">${esc(c.snippet)}</pre>
                    </div>`).join('')}
                </div>
              </div>` : ''}

            <div class="flex flex-col gap-2">
              <span class="font-mono font-semibold uppercase tracking-wider text-base-content/50 text-2xs">Functions & Cmdlets (${info.functions.length})</span>
              <div class="flex flex-col gap-1.5">
                ${info.functions.map(fn => `
                  <div class="rounded border border-base-300 bg-base-100 p-2.5 flex flex-col gap-1">
                    <div class="flex items-center justify-between font-mono">
                      <span class="font-semibold text-primary">${esc(fn.name)}</span>
                      <span class="text-2xs text-base-content/40">Line ${fn.line}</span>
                    </div>
                    ${fn.synopsis ? `<p class="text-2xs italic text-base-content/70">${esc(fn.synopsis)}</p>` : ''}
                    ${fn.params?.length ? `
                      <div class="flex flex-wrap gap-1 mt-0.5">
                        ${fn.params.map(p => `<span class="badge badge-xs badge-ghost font-mono">${esc(p.type ? '[' + p.type + '] ' : '')}$${esc(p.name)}</span>`).join('')}
                      </div>` : ''}
                  </div>`).join('') || '<span class="font-mono text-base-content/40 italic">No functions declared</span>'}
              </div>
            </div>

            ${info.dotSources.length ? `
              <div class="flex flex-col gap-1">
                <span class="font-mono font-semibold uppercase tracking-wider text-base-content/50 text-2xs">Dot-Sourced Companions (${info.dotSources.length})</span>
                <div class="flex flex-col gap-1 font-mono text-2xs">
                  ${info.dotSources.map(d => `<div class="rounded border border-base-200 bg-base-100 px-2 py-1 text-base-content/80 truncate">. ${esc(d.raw)}</div>`).join('')}
                </div>
              </div>` : ''}

            ${info.imports.length ? `
              <div class="flex flex-col gap-1">
                <span class="font-mono font-semibold uppercase tracking-wider text-base-content/50 text-2xs">Imported Modules (${info.imports.length})</span>
                <div class="flex flex-wrap gap-1 font-mono text-2xs">
                  ${info.imports.map(m => `<span class="badge badge-xs badge-secondary">${esc(m.module)}</span>`).join('')}
                </div>
              </div>` : ''}
          </div>`;
      }
    ...
    },
    {
      ...
      // proposed, with who proposed it underneath. Read-only, like the rest of
      // that lane: nothing here applies a proposal.
      //
      // The projection is read from mehrlander/home, which is private, so a
    ...
    {
      // HISTORY IS NOT ANOTHER PROPOSAL MODE. Proposals join the exact current
      // block to a suggested replacement. This reading starts at that same raw
      // Markdown block, but follows a curated, occurrence-scoped predecessor
      // connection through an observed document revision and an explicit
      // continuation into the pinned current file blob. Both inferences and
      // the revision evidence remain visible; md-diff is only the display once
      // those records have named both sides.
      id: 'history', label: 'History', icon: 'ph-clock-counter-clockwise',
      test: (f) => f.ext === 'md',
      render: () => `<div class="overflow-auto h-full w-full bg-base-100">
          <div class="px-6 py-4" data-md-history>
            <div class="text-sm opacity-60 italic">Reading passage history…</div>
          </div>
        </div>`,
      after: async (f, ctx) => {
        ...
        try {
          ...
          if (!window.mdHistory) throw new Error('the passage history kit is unavailable');
          const index = await window.mdHistory.index();
          if (!alive()) return;
          const text = ViewRegistry.stripFrontmatter(f.content);
          box.textContent = '';
          const handle = await window.mdHistory.render(box, text, index, {
            repo: f.repo,
            path: f.name,
            ref: f.ref,
            source: f.content,
          ...
          if (!alive()) return;
          if (!handle.count && !handle.ambiguous) {
            const none = document.createElement('div');
            none.className = 'mb-3 text-sm opacity-60 italic';
            none.textContent = 'No retained predecessor connection matches a passage of this file as it stands.';
            box.prepend(none);
          }
        } catch (error) {
          if (!alive()) return;
          ...
          message.textContent = `Passage history unavailable (${error?.message || error}). The file itself is on Preview.`;
          box.append(message);
        }
      }
    },
