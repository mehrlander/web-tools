const ViewRegistry = {
      ...
      after: (f, ctx) => {
        requestAnimationFrame(() => {
          ...
          try {
            ...
            const filterColumn = delimited?.columns.find(c => c.title === ctx.opts?.filter?.col);
            const filterOpts = filterColumn
              ? { ...ctx.opts, filter: { ...ctx.opts.filter, col: filterColumn.field } }
              : ctx.opts;
            const table = new Tabulator(target, {
              data: delimited ? delimited.rows : ViewRegistry.tableRows(f),
              ...(delimited
  ...
  // is the visible header. Blank and duplicate names get positional INTERNAL
  // fields, but the grid title remains the exact string in the source file.
  tableDelimited(f) {
    ...
    return { rows: rows.slice(1).map(r => Object.fromEntries(columns.map((c, i) => [c.field, r[i] ?? '']))), columns };
  },
  tableRows(f) {
