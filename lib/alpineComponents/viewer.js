    ...
    },
    {
      ...
      after: async (f, host) => {
        ...
        try {
          ...
          // ---- EXTRACT: the two axes, as one surface ------------------
          //
          // The picker IS the preview. The kit takes a cross product (which
          // sheets, which kinds), so the controls are two independent lists
          // rather than a tree, and the matrix below them is not a third
          // control: it is the result, drawn. Ticking a kind lights a column
          // of cells with their row counts, which is what a pick produces, so
          // nothing on the surface has to explain what a pick does.
          //
          // EVERY CELL CARRIES ITS OWN KIND LABEL rather than relying on its
          // column position. That is what lets the matrix reflow to two
          // columns on a phone with no second markup and no breakpoint:
          // position stops carrying meaning, so nothing is lost when it moves.
          // (A breakpoint would have been the wrong tool anyway. This draws
          // inside a pane, and a viewport query measures the window.)
          const survey = X ? X.survey(result) : null;
          const SHEET_KINDS = survey ? survey.kinds.filter(k => k.scope === 'sheet') : [];
          const BOOK_KINDS = survey ? survey.kinds.filter(k => k.scope === 'workbook') : [];
          const cellCount = (sheetName, kindId) => sheetName == null
            ? survey.workbook[kindId]
          ...
          const offered = survey
            ? survey.sheets.reduce((n, sh) => n + SHEET_KINDS.filter(k => sh.counts[k.id]).length, 0)
              + BOOK_KINDS.filter(k => survey.workbook[k.id]).length
            : 0;
          ...
          const VIEWS = [
            ...
            // Last in the strip, because it is the one tab that reads ACROSS
            // the eight before it rather than showing one of them. Its count
            // is what there is to pick: cells of the matrix the workbook has
            // anything in.
          ...
          // The selection, which is exactly the kit's `pick`: two flat lists.
          const picked = { sheets: new Set(), kinds: new Set() };
          ...
          const envelope = () => X.extract(result, {
            sheets: [...picked.sheets], kinds: [...picked.kinds], headerRow,
          }, { source: source(), maxRows: cap });
          ...
          // serialising every value of a large sheet does not. The matrix
          ...
          const rebuild = () => {
            ...
            built = null;
            paintFigures();
            if (!picked.kinds.size) return;
            buildTimer = setTimeout(async () => {
              let env, json;
              try { env = envelope(); json = JSON.stringify(env); }
          ...
          const paintFigures = (err) => {
            ...
            // A pick can be non-empty and still produce nothing: a kind with
            // every sheet unticked is a real state for the matrix to be in.
          ...
          const lit = (sheetName, kindId) =>
            picked.kinds.has(kindId) && cellCount(sheetName, kindId) > 0 &&
            (sheetName == null || picked.sheets.has(sheetName));

          // A cell says what it holds and, when the cap bites, what it will
          // hand over against that. pivotRecords' habit: report the cut rather
          // than apply it silently.
          // TWO LINES, ALWAYS, so a cut figure cannot squeeze the label off the
          // cell. On the toy fixture every count was one digit and one line
          // looked right; on a real workbook "2000 of 2409" clipped `Styles`
          // to `Sty…` at pane width and not at phone width, which is the worst
          // shape a layout defect can take. Both lines are fixed height, so
          // the matrix still reads as a grid rather than a ragged stack.
          // WRAPPING IS THE WHOLE TRICK. Label and count are two flex items in
          // a wrapping box: side by side when the column is wide enough, which
          // is most of a desktop, and stacked when it is not, which is a phone
          // and any cut figure long enough to crowd the label. Fixing either
          // arrangement was wrong in the other place: one line clipped
          // "Styles" to "Sty…" at pane width, and two lines left a desktop
          // cell mostly empty.
          const cellLabel = (kind, n) => {
            const kept = cap != null && n > cap ? `${cap} of ${n}` : String(n);
            return `<span class="truncate leading-tight">${esc(kind.label)}</span>
                    <span class="font-mono text-xs leading-tight ml-auto ${n ? 'opacity-70' : 'opacity-30'}">${kept}</span>`;
          };

          // THE WEIGHT OF THE ACCENT, not its hue. Selection is the one thing
          // colour carries here, and the first build spent a solid `btn-primary`
          // on every lit cell, every ticked chip and every picked sheet row:
          // seven filled blocks, at which point the accent reads as the
          // surface's texture rather than as a mark on it. The repo's own
          // answer is quieter and is what the estate view's scope toggles use
          // (`bg-primary/10 text-primary`): across lib/ and pages/, `text-primary`
          // and `bg-primary/10` outnumber `btn-primary` four to one.
          const ON = 'bg-primary/10 text-primary border-primary';
          const OFF = 'btn-ghost border border-base-300';

          // ONE TEMPLATE FOR EVERY BLOCK, which is what makes this read as a
          // matrix rather than as four unrelated rows of buttons. The sheet
          // blocks carry eight cells and the workbook block four; with each
          // block sizing its own columns, the four stretched across the whole
          // width and lined up with nothing. Declaring the column count once
          // puts the workbook's four in the first four columns of the same
          // grid.
          //
          // `minmax(7.5rem, 1fr)` rather than auto-fit: at 2000px auto-fit gave
          // eight columns of 230px each holding a two-word label and a number,
          // which is the "awkward space" this surface was carrying. The cell
          // content wraps instead (see cellLabel), so a wide column puts the
          // label and its count on one line and a narrow one stacks them.
          ...
          const cellBtn = (sheetName, kind) => {
            ...
            return `<button data-ex-cell="${esc(kind.id)}" data-ex-sheet="${sheetName == null ? '' : esc(sheetName)}"
          ...
          // A ROW HEADER, not a box around a block. Each sheet was its own
          // bordered card with its own grid inside it, which is why the
          // columns did not line up: auto-fit sizes tracks per grid, so the
          // workbook's four cells stretched to the full width while a sheet's
          // eight sat at a third of it, and four blocks of buttons read as
          // four unrelated rows rather than as a matrix. Everything is one
          // grid now and a band spans it, so the workbook's four cells land in
          // the same first four columns the sheets use.
          const BAND = 'col-span-full flex items-center gap-2 px-2 py-1 mt-1 first:mt-0'
            + ' text-left text-sm rounded bg-base-200/70';

          const sheetBand = (sh) => `<button data-ex-sheetrow="${esc(sh.name)}"
                    class="${BAND} ${picked.sheets.has(sh.name) ? 'text-primary' : ''}">
              <i class="ph ${picked.sheets.has(sh.name) ? 'ph-check-square' : 'ph-square'}"></i>
              <span class="truncate">${esc(sh.name)}</span>
          ...
          const extractSurface = () => {
            ...
            const caps = [[500, '500 rows'], [2000, '2,000 rows'], [20000, '20,000 rows'], [null, 'every row']];
            return `<div data-xs="ex" class="flex flex-col gap-3 p-3">
              <div class="flex flex-wrap items-center gap-2">
                <!-- The figure takes the line it needs (about 40 characters of
                     mono, wider than a phone), and the four controls share the
                     next one. Letting the select grow put it alone on a line
                     and pushed the buttons to a third, which cost 40px of a
                     844px screen to say nothing. -->
              ...
              <!-- THE SHEET KINDS ONLY, which is what makes this strip mean one
                   thing: toggle this column across every sheet that has it. It
                   carried the four workbook-scoped kinds too, behind a rule,
                   and each of those toggles exactly ONE cell, the one sitting
                   in the Whole workbook block below. Two controls for one cell
                   is what made the strip wrap to a second row with a gap in it
                   and read as a list of twelve unrelated things. -->
              <div class="flex flex-wrap gap-1">
                ${SHEET_KINDS.map(k => {
                  const any = survey.sheets.some(sh => sh.counts[k.id]);
                  const on = picked.kinds.has(k.id);
                  return `<button data-ex-kind="${esc(k.id)}" ${any ? '' : 'disabled'}
                            class="btn btn-xs border ${on ? ON : OFF}"
                            >${esc(k.label)}</button>`;
                }).join('')}
              </div>
              <div style="${GRID}">
                ${survey.sheets.map(sh =>
                  sheetBand(sh) + SHEET_KINDS.map(k => cellBtn(sh.name, k)).join('')).join('')}
                <div class="${BAND}">
                  <i class="ph ph-books opacity-50"></i><span>Whole workbook</span>
                </div>
                ${BOOK_KINDS.map(k => cellBtn(null, k)).join('')}
              </div>
            </div>`;
          };

          // Repaint the matrix in place rather than through paint(), which
          // rewrites the whole stage and scrolls it to the top. A tick must
          // not move the thing that was ticked.
          const repaintExtract = () => {
            ...
            for (const b of box.querySelectorAll('[data-ex-kind]')) mark(b, picked.kinds.has(b.dataset.exKind));
            for (const b of box.querySelectorAll('[data-ex-sheetrow]')) {
              const on = picked.sheets.has(b.dataset.exSheetrow);
              b.classList.toggle('text-primary', on);
              const i = b.querySelector('i');
              if (i) i.className = 'ph ' + (on ? 'ph-check-square' : 'ph-square');
            }
            for (const b of box.querySelectorAll('[data-ex-cell]')) {
              const sheetName = b.dataset.exSheet || null;
          ...
          const wireExtract = () => {
            ...
            box.addEventListener('click', (e) => {
              ...
              const kindBtn = e.target.closest('[data-ex-kind]');
              const sheetBtn = e.target.closest('[data-ex-sheetrow]');
              // A CELL IS A SHORTHAND FOR ITS TWO AXES, not a third kind of
              // state. Tapping one turns on the kind and the sheet it sits at,
              // or turns off whichever of them is on: the selection stays the
              // cross product the kit takes, so the matrix can never show a
              // pick the envelope cannot express.
              if (cell) {
                const kindId = cell.dataset.exCell;
                const sheetName = cell.dataset.exSheet || null;
                if (lit(sheetName, kindId)) picked.kinds.delete(kindId);
                else {
                  picked.kinds.add(kindId);
                  if (sheetName) picked.sheets.add(sheetName);
                }
              } else if (kindBtn) {
                const id = kindBtn.dataset.exKind;
                if (picked.kinds.has(id)) picked.kinds.delete(id);
                else {
                  picked.kinds.add(id);
                  // A sheet kind with no sheet picked produces nothing, which
                  // reads as a broken tick rather than as an empty selection.
                  if (!picked.sheets.size && survey.kinds.find(k => k.id === id)?.scope === 'sheet')
                    for (const sh of survey.sheets) if (sh.counts[id]) picked.sheets.add(sh.name);
                }
              } else if (sheetBtn) {
                const name = sheetBtn.dataset.exSheetrow;
                if (picked.sheets.has(name)) picked.sheets.delete(name);
                else picked.sheets.add(name);
              } else return;
              repaintExtract();
            });
