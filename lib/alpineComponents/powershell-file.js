...
//
// Edits are browser drafts in the same store the Code tab uses
// (PowerShellWorkspace.saveDraft), so a draft begun here is there too and
// publication still goes through the Code tab's reviewed flow. Nothing here
...
document.addEventListener('alpine:init', () => {
  Alpine.data('powershellFile', function () {
    ...
    return {
      ...
      text: '', base: null, draft: false, saveState: '', discardAsk: false,
      ...
      async init() {
        ...
        try {
          ...
          if (!alive) return;
          const d = ctx.drafts.get(this.item.path);
          ...
          if (d && this.item.blobSha && d.baseBlob !== this.item.blobSha) this.notice = 'GitHub changed this file after the draft began. Review it in the Code tab before publishing.';
          this.saveState = this.draft ? 'Browser draft' : 'GitHub ' + short(source.revision);
          this.analyze();
          this.loading = false;
          await this.$nextTick();
          try {
      ...
      // ── Editing ──────────────────────────────────────────────────────────
      setEditing(on) {
        if (on && !editor) { this.error = 'The editor did not load, so this file can only be read here.'; return; }
        this.editing = !!on;
      ...
      header() {
        ...
        h.setActions(this.editing
          ...
          : [...(editor ? [act('ph-pencil-simple', 'Edit', () => this.setEditing(true))] : []),
      ...
      menuRows() {
        ...
        if (this.draft) {
          ...
          rows.push({ label: 'Review and publish in Code', icon: 'ph-git-pull-request', onClick: () => ctx.publish(it.path) });
        }
      ...
      async copyScript() {
        try {
          const script = window.Installation.transferScript({ path: this.item.path, name: this.item.name, installs: this.item.installs,
            root: ctx.root, revision: source.revision, blobSha: source.sha, bytes: new TextEncoder().encode(source.text) });
