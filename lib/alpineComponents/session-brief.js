    ...
    return {
      ...
      template: `
        ...
        <div class="w-full flex flex-col gap-2.5 px-4 pt-3"
          ...
          <!-- ── WHAT TO DO ABOUT IT ──────────────────────────────────────
               The one act this page offers, and it sits under the facts because
               it operates on one of them: the repos fact says where the session ran,
               and this starts new work in the same place.

               A ROW WITH WORDS, NOT AN ICON, and that is the whole lesson of
               the version before it. Two marks went onto every row of the
               Sessions list on 2026-09-06, a second Claude logomark and a
               microphone, and they came off the same day: a control whose
               entire value is in specifying what will happen cannot live where
               there is no room to say it. Here there is room, so it says it.

               IT CARRIES THE SCOPE AND NOTHING ELSE. Which target, which of the
               repositories, and whether to send at all are choices the far end
               already owns (pages/dictate.html's Send sheet), and rebuilding
               any of them here would be a second copy that drifts. So this
               chooses nothing: it opens the page that does, with the
               repositories filled in and the words still to be spoken.

               The to=send parameter paints that destination as the lead rather than acting
               on it, which is that page's own rule about an address arriving
               with intent. -->
          <div x-show="scopeUrl" class="shrink-0">
            <a :href="scopeUrl" target="_blank" rel="noopener"
               class="flex items-center gap-2 rounded-lg bg-base-200/50 px-3 py-2 text-sm
                      hover:bg-base-200 transition-colors"
               :data-note="'Opens the dictation page with ' + scopeNames
                           + ' already chosen. Speak the prompt there, then Send opens a new '
                           + 'Claude Code session on them. Nothing starts until you send it.'">
              <i class="ph ph-microphone text-lg opacity-60 shrink-0"></i>
              <span class="font-medium">Work on this scope</span>
              <!-- The repositories are NOT repeated here. They are the strip's
                   repos fact one line up, and printing them twice put the
                   same list on screen at two widths, the second one truncated
                   to nothing useful. The note carries them in full for a reader
                   who wants to check before tapping. -->
              <i class="ph ph-arrow-up-right text-sm opacity-40 ml-auto shrink-0"></i>
            </a>
          </div>
