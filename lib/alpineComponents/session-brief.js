...
//   pages     render chips for what this session's branches changed, lent by a
//             host that can join a record to the crawl (see the Look row)
...
(function () {
  ...
  Alpine.data('sessionBrief', function (opts) {
    ...
    return {
      ...
      loading: false,
      err: '',
      pane: ['raw', 'files'].includes(o.pane) ? o.pane : (o.file ? 'files' : 'outline'),
      rawBuilt: false,
      outlineBuilt: false,
      deckOpening: false,

      // Swiper & card state for the Files tab:
      at: 0,
      listOpen: false,
      swipeHovered: false,
      mounted: {},
        ...
        <div class="w-full flex flex-col gap-2.5 px-4 pt-3"
          ...
          <div class="flex flex-wrap items-center gap-x-2.5 gap-y-0.5 font-mono text-[13px] shrink-0">
            <template x-for="(f, i) in strip" :key="f.k">
              <!-- Nowrap holds a value and its unit word together, which is
                   what every fact here is except one. Repos carries a LIST, and
                   from record schema 8 it is the container's whole scope rather
                   than the working directories alone: two names became four,
                   and four names held nowrap overrun the measure instead of
                   wrapping. It is the only prose-shaped value on the strip, so
                   it is the only one allowed to break, and it is last, so it
                   carries no trailing dot to strand. -->
              <span :class="f.k === 'repos' ? 'min-w-0 break-words' : 'whitespace-nowrap'">
                <span :data-note="f.t"
                ...
                <!-- ── WHAT TO DO ABOUT IT ──────────────────────────────
                     The one act this page offers, and it rides the repos fact
                     because that is the fact it operates on: repos says where
                     the session ran, and this starts new work in the same
                     place. A row of words carried it until 2026-09-08, which
                     was more surface than a control worth one tap deserves.

                     INSIDE THE FACT, not beside the strip. As a flex item of
                     its own it wrapped to a fourth line alone at 390px, where
                     the repo names already take two: an orphan mark under a
                     list, further from the fact it acts on than the row it
                     replaced. Inline after the names it wraps WITH them, and
                     on one line it sits where it reads, at the end of the
                     scope. Keyed on the fact rather than placed by position,
                     so a strip that grows a fact after repos does not carry
                     this into the middle of it.

                     IT CARRIES THE SCOPE AND NOTHING ELSE. Which target, which
                     of the repositories, and whether to send at all are choices
                     the far end already owns (pages/dictate.html's Send sheet),
                     and rebuilding any of them here would be a second copy that
                     drifts. So this chooses nothing: it opens the page that
                     does, with the repositories filled in and the words still
                     to be spoken. The to=send parameter paints that destination
                     as the lead rather than acting on it, which is that page's
                     own rule about an address arriving with intent.

                     THE ICON IS LEGIBLE HERE AND WAS NOT ON THE SESSIONS LIST,
                     which is the distinction the worded row was drawn to
                     respect and over-served. A note reaches a touch screen and
                     a screenshot (kits/note.js), so the words are one tap away
                     rather than absent; on a list row, where the same mark
                     repeated forty times, nothing said which session it would
                     act on. Here there is exactly one, at the end of the scope
                     it carries. -->
                <!-- x-if, not x-show: inside an x-for, x-show renders the
                     anchor once per FACT and hides all but one, which put five
                     display:none copies of this control in the DOM. Harmless
                     on screen and not harmless to anything reading the markup:
                     a hover driver picks the first match and finds it
                     invisible, which is how the surplus was noticed. -->
                <template x-if="f.k === 'repos' && scopeUrl">
                  <a :href="scopeUrl" target="_blank" rel="noopener"
                     class="ml-1 -my-1 inline-flex items-center rounded p-1 align-middle
                            text-base-content/40 hover:text-base-content transition-colors"
                     aria-label="Prepare a follow-up session"
                     data-note-bare
                     :data-note="'Prepare a follow-up session on ' + scopeNames
                                 + '. Opens the dictation page with them already chosen; speak the '
                                 + 'prompt there and Send starts it. Nothing runs until you send it.'">
          ...
          <!-- ── Look: what this session was building ─────────────────────
               The pages this session's branches changed, each opening at the
               BRANCH so it shows where the work got to rather than the commit
               some crawl happened to see. branch-brief carries the same row for
               one branch, in the same place and with the same chip; this is its
               fold across the branches one session left behind, which is the
               join the detail could not make for itself.

               LENT, NEVER COMPUTED, on the same contract the facts strip
               uses one level up: a
               record names its checkouts and their branches and stops there,
               and what each branch CHANGED is the estate's crawl to know. A
               cold pages/session.html is lent nothing and draws no row, which is
               the honest answer rather than a blank one, since the alternative
               is this page fetching a compare per branch on open.

               ONE LINE THAT SCROLLS, for the reason branch-brief's own Look row
               gives: a wrap spends a whole band on the chip that did not fit,
               and the partial chip at the edge is the affordance. -->
          <!-- shrink-0, and it is not decoration: the head is a column flex at
               h-full, so a child that does not claim its height is squeezed by
               whatever grows below it. Measured at 3px tall before this, with
               every chip inside it correct and invisible. -->
          <div x-show="pageChips.length"
               class="shrink-0 flex items-center gap-1.5 flex-nowrap min-w-0 mt-2
                      overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <span class="text-sm shrink-0 select-none leading-none opacity-80"
                  data-note="Pages this session changed, rendered via toss"
                  data-note-bare>🥏</span>
          ...
          <!-- ── Active branch row: checkouts and branches touched by this session ── -->
          <div x-show="branchChips.length"
               class="shrink-0 flex items-center gap-1.5 flex-nowrap min-w-0 mt-1
                      overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <i class="ph ph-git-branch text-sm text-base-content/40 shrink-0"
               data-note="Branches this session worked on" data-note-bare aria-hidden="true"></i>
            <template x-for="b in branchChips" :key="b.key">
              <a :href="b.url" target="_blank" rel="noopener"
                 :title="'Open branch ' + b.branch + ' on ' + b.repo + ' in Branch view'"
                 class="shrink-0 inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-mono
                        bg-base-200/70 text-base-content/80 hover:bg-base-200 hover:text-base-content transition-colors">
                <span class="font-semibold" x-text="b.repo"></span>
                <span class="opacity-70" x-text="'@' + b.branch"></span>
              </a>
            </template>
          </div>
          ...
          <div x-ref="chrome" class="flex items-center gap-2 shrink-0"
            ...
            <div role="tablist" class="tabs tabs-box tabs-sm flex-nowrap bg-base-200 p-0.5 min-w-0
              ...
              <a role="tab" class="tab gap-1 flex-nowrap whitespace-nowrap"
                 ...
                 @click="pane = 'files'; $nextTick(() => { markNear(); settlePane(); })">
          ...
          <!-- FILES: what the session opened, which the strip counts and had no
               ...
               ways. -->
          <!-- FILES: what the session opened, which the strip counts.
               Horizontal scroll-snap swiper with lazy card mounting,
               branch-grouped dropdown jump list, and slide chrome. -->
          <div x-show="pane === 'files'" x-cloak class="pb-3 flex flex-col"
               :class="framed && 'min-h-0 grow overflow-y-auto'">
            <div x-show="!fileRows.length" class="text-sm opacity-60 py-3" x-text="filesNote"></div>

            <div x-show="fileRows.length > 0" data-swipe-section
                 @pointerenter="swipeHovered = true" @pointerleave="swipeHovered = false"
                 @pointerdown="swipeHovered = true"
                 class="relative rounded-lg border border-base-300 bg-base-100 flex flex-col min-h-0"
                 :class="framed ? 'flex-1 min-h-0'
                                : 'h-[80vh] roomy:h-auto roomy:flex-1 roomy:min-h-0'">

              <!-- ONE HEADER ROW: list mark, file name, tool activity annotations, github menu, view modes, pager -->
              <div data-swipe-header class="relative flex items-center gap-1 max-sm:gap-0.5 shrink-0 pl-1 pr-1 py-1 bg-base-200 border-b border-base-300 rounded-t-lg">
                <button type="button" data-file-list-btn class="btn btn-sm btn-ghost btn-square shrink-0"
                        @click="listOpen = !listOpen" :disabled="!fileRows.length"
                ...
                <span x-show="!fileRows[at]" class="text-xs uppercase tracking-wide opacity-60">Files</span>

                <!-- Slide name over its activity counts -->
                <div x-show="!!fileRows[at]" data-slide-name class="min-w-0 shrink flex flex-col leading-tight"
                     :title="fileRows[at]?.path">
                  <span class="font-mono text-sm min-w-0 flex items-baseline">
                    <span x-show="(fileRows[at]?.path || '').includes('/')" class="opacity-40 truncate shrink-[9999]"
                          x-text="(fileRows[at]?.path || '').split('/').slice(0, -1).join('/') + '/'"></span>
                    <span class="min-w-0 truncate shrink" x-text="(fileRows[at]?.path || '').split('/').pop()"></span>
                  </span>
                  <!-- File activity annotations: session tool counts (r3, e1, etc.) -->
                  <span data-change-stats class="font-mono text-[11px] tabular-nums whitespace-nowrap truncate flex items-center gap-1.5">
                    <template x-for="k in (fileRows[at]?.kinds || [])" :key="k.label">
                      <span class="shrink-0 font-mono text-[11px] tabular-nums"
                            :class="k.label === 'edit' || k.label === 'write' ? 'text-warning font-semibold' : 'opacity-60'"
                            :title="k.n + ' ' + k.label + (k.n === 1 ? '' : 's')">
                        <span x-text="k.label.slice(0, 1)"></span><span x-text="k.n"></span>
                      </span>
                    </template>
                    <span x-show="fileRepoBranch(fileRows[at])" class="opacity-40 text-[10px]"
                          x-text="'· ' + fileRepoBranch(fileRows[at])"></span>
                  </span>
                </div>
                ...
                <!-- Pager -->
                <div x-show="fileRows.length > 1" data-pager class="flex items-center shrink-0">
                  ...
                  <span data-pager-label class="font-mono text-xs opacity-60 tabular-nums px-1" x-text="(at + 1) + '/' + fileRows.length"></span>
                  <button type="button" class="btn btn-xs btn-ghost btn-square max-sm:hidden"
                          @click="go(at + 1)" :disabled="at >= fileRows.length - 1"
                          title="Next file"><i class="ph ph-caret-right"></i></button>
                </div>
              </div>

              <!-- Dropdown jump list grouped by branch -->
              <div data-file-list x-show="listOpen"
                ...
                <template x-for="g in branchGroups" :key="g.repo + '@' + g.branch">
                  <div class="border-b border-base-200/80 last:border-b-0">
                    <!-- Branch header cap -->
                    <div class="flex items-center gap-2 px-3 py-1.5 bg-base-200/60 border-b border-base-300 flex-wrap">
                      <span class="font-semibold text-xs text-base-content/80 font-mono" x-text="g.repo"></span>
                      <template x-if="g.branch">
                        <a x-show="g.branchUrl" :href="g.branchUrl" target="_blank" rel="noopener"
                           class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-mono bg-base-300/60 hover:bg-primary hover:text-primary-content transition-colors"
                           :title="'Open branch ' + g.branch">
                          <i class="ph ph-git-branch text-[11px]"></i>
                          <span x-text="g.branch"></span>
                        </a>
                      </template>
                      <template x-if="g.branch && !g.branchUrl">
                        <span class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-mono bg-base-300/40">
                          <i class="ph ph-git-branch text-[11px]"></i>
                          <span x-text="g.branch"></span>
                        </span>
                      </template>
                      <div class="grow"></div>
                      <span class="font-mono text-xs opacity-50" x-text="plural(g.files.length, 'file')"></span>
                    </div>
                    <!-- Files list for this branch -->
                    <div class="p-1 space-y-0.5">
                      <template x-for="f in g.files" :key="f.path">
                        <div class="flex items-baseline gap-2 py-1 px-2 border-b border-base-200/60 last:border-0 hover:bg-base-200/30 transition-colors"
                             :class="fileIndexOf(f.path) === at ? 'bg-primary/10' : ''">
                          <button type="button" @click="go(fileIndexOf(f.path), true); listOpen = false"
                                  class="shrink-0 w-5 text-right font-mono text-xs tabular-nums"
                                  :class="fileIndexOf(f.path) === at ? 'text-primary font-bold' : 'opacity-40'"
                                  :title="'Jump to slide ' + (fileIndexOf(f.path) + 1)"
                                  x-text="fileIndexOf(f.path) + 1"></button>
                          <a :href="f.href || null"
                             ...
                             :title="f.path"
                             @click="if (!f.href) { go(fileIndexOf(f.path), true); listOpen = false; }">
                            <span class="opacity-45" x-text="f.dir"></span><span x-text="f.name"></span>
                          </a>
                          <div class="grow cursor-pointer" @click="go(fileIndexOf(f.path), true); listOpen = false"></div>
                          <template x-for="k in f.kinds" :key="k.label">
                            <span class="shrink-0 font-mono text-xs tabular-nums cursor-pointer"
                                  @click="go(fileIndexOf(f.path), true); listOpen = false"
                                  :class="k.label === 'edit' || k.label === 'write' ? 'text-warning font-semibold' : 'opacity-45'"
                                  :title="k.n + ' ' + k.label + (k.n === 1 ? '' : 's')">
              ...
              <!-- Horizontal scroll-snap swiper strip with lazy card mounting -->
              <div class="flex-1 min-h-0 flex flex-col">
                <div x-ref="fileStrip" @scroll.passive="stripScroll()"
                     class="flex-1 min-h-0 items-stretch flex gap-3 snap-x snap-mandatory
                            overflow-x-auto overflow-y-hidden overscroll-x-contain
                            [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                  <template x-for="(f, i) in fileRows" :key="f.path">
                    <div data-slide class="relative w-full shrink-0 snap-center flex flex-col min-h-0">
                      <div class="overflow-hidden overflow-y-auto flex-1 min-h-0 px-3 pb-3 pt-1">
      ...
      init() {
        ...
        // Swiper: mount slide in view and neighbours when files pane is active
        this.$watch('at', () => this.markNear());
        this.$watch(() => this.fileRows.map(f => f.path).join('\n'), () => {
          if (this.file && this.fileRows.length) {
            const idx = this.fileIndexOf(this.file);
            if (idx >= 0) this.at = idx;
          }
          if (this.at >= this.fileRows.length) this.at = Math.max(0, this.fileRows.length - 1);
          this.markNear();
        });
        this.$watch('pane', (p) => {
          if (p === 'files') {
            this.$nextTick(() => {
              this.ensureFileReview();
              this.markNear();
              this.go(this.at, true);
              this.settlePane();
            });
          }
        });
        ...
        this._onKey = (e) => {
          ...
          // The swiper takes arrows while pointer or focus is in it
          const sec = this.$refs.fileStrip?.closest('[data-swipe-section]');
          const active = this.swipeHovered || (sec && sec.contains(document.activeElement));
          if (!active || this.fileRows.length < 2) return;
          e.preventDefault();
      ...
      destroy() {
        this.chromeObs?.disconnect(); this.chromeObs = null;
        // The outline's rail listens for scroll on the window, because that is
        // the only listener that catches both this component's layouts; it is
        // the one thing index() builds that does not die with the tree, so it
        // is released here. On the estate's swiper this runs per slide left.
        this.outlineOff?.(); this.outlineOff = null;
        this._rawExplorer?.destroy?.(); this._rawExplorer = null;
      ...
      get strip() {
        ...
        return [
          ...
          // THE UNION, not `repos` alone. Record schema 8 added `attached`, the
          // container's own list, and `repos` is where the shell actually stood: a
          // checkout worked entirely through absolute paths appears in one and not
          // the other. Showing the narrower field here while the row below acts on
          // the wider one put two different answers to one question a line apart,
          // which a shot caught before this comment existed. An older record
          // carries no `attached` and reads as it always did.
          ['repos', this.scopeNames, '', "The checkouts this session had. From schema 8 that is the container's own list; before it, only the ones that were the working directory, which misses a repo worked entirely through absolute paths"],
        ].filter(([, v]) => v || v === 0).map(([k, v, unit, t]) => ({ k, v, unit, t }));
      },

      // ── The scope, as an address ─────────────────────────────────────────
      // The checkouts this session stood in, turned into the link above.
      //
      // THE OWNER COMES FROM THE STORE, not from a guess and not from a list
      // this component does not have. A record names checkouts ("web-tools"),
      // not repositories, and resolving one to the other needs an owner; the
      // store is already mounted with one (the `repo` option, defaulting to STORE) and
      // every repository in this estate sits under it. A host that mounts a
      // store elsewhere gets that owner, which is the right answer for the same
      // reason it is the right store.
      //
      // The honest limit, and it is the recorder's rather than this file's:
      // `repos` is where the shell stood, so a checkout worked entirely through
      // absolute paths never appears. Record schema 8 added `attached` for
      // exactly that, and it is preferred here where a record carries it.
      get scopeRepos() {
        const r = this.rec || {};
        const worked = (r.repos || []).map((x) => x && x.name).filter(Boolean);
        const attached = Array.isArray(r.attached) ? r.attached : [];
        return [...new Set([...attached, ...worked])].sort();
      },
      get scopeNames() { return this.scopeRepos.join(', '); },

      // What the Look row draws. A plain read of the lent list, so a host that
      // knows nothing draws nothing and this page never fetches on its behalf.
      get pageChips() { return Array.isArray(o.pages) ? o.pages : []; },
      get scopeUrl() {
        const names = this.scopeRepos;
        if (!names.length) return '';
        const owner = String(this.repo || STORE).split('/')[0];
        if (!owner) return '';
        const slugs = names.map((n) => owner + '/' + n).join(',');
        return 'https://mehrlander.github.io/web-tools/pages/dictate.html'
             + '?to=send&target=code&repo=' + encodeURIComponent(slugs);
      },
      ...
      get fileRows() {
        ...
        const repos = this.record?.repos || [];
        return Object.entries(files)
          .map(([path, kinds]) => {
            const i = path.lastIndexOf('/');
            const slash = path.indexOf('/');
            const repoName = slash >= 0 ? path.slice(0, slash) : '';
            const relPath = slash >= 0 ? path.slice(slash + 1) : path;
      ...
      // Files grouped by repository checkout and active branch:
      get branchGroups() {
        ...
        for (const r of repos) {
          if (r?.name) repoMap.set(r.name, r);
        }
        ...
        for (const f of rows) {
          ...
          const repoName = slash >= 0 ? f.path.slice(0, slash) : '';
          const relPath = slash >= 0 ? f.path.slice(slash + 1) : f.path;
          const relDirIdx = relPath.lastIndexOf('/');
          const relDir = relDirIdx >= 0 ? relPath.slice(0, relDirIdx + 1) : '';
          const relName = relDirIdx >= 0 ? relPath.slice(relDirIdx + 1) : relPath;
          ...
          if (repoName && repoMap.has(repoName)) {
            ...
            byRepo.get(repoName).push(fileItem);
          } else {
            unassigned.push(fileItem);
          }
        }

        const owner = String(this.repo || STORE).split('/')[0] || 'mehrlander';

        for (const r of repos) {
          const files = byRepo.get(r.name) || [];
          if (files.length > 0) {
            const fullRepo = r.name.includes('/') ? r.name : `${owner}/${r.name}`;
            groups.push({
              repo: r.name,
              branch: r.branch || '',
              files,
              branchUrl: r.branch ? `https://mehrlander.github.io/web-tools/pages/branch.html#gh=${encodeURIComponent(fullRepo)}@${encodeURIComponent(r.branch)}` : '',
            });
          }
        }

        for (const [repoName, files] of byRepo.entries()) {
      ...
      // Branches touched by this session, as active branch chips
      get branchChips() {
        const repos = this.record?.repos || this.factsRecord?.repos || [];
        const owner = String(this.repo || STORE).split('/')[0] || 'mehrlander';
        const out = [];
        for (const r of repos) {
          if (r?.name && r?.branch) {
            ...
            out.push({
              ...
              repo: r.name,
              branch: r.branch,
              url: `https://mehrlander.github.io/web-tools/pages/branch.html#gh=${encodeURIComponent(fullRepo)}@${encodeURIComponent(r.branch)}`,
            });
          }
        }
        return out;
      ...
      // ── Swiper & Card Management ──────────────────────────────────────────
      get card() {
        this._chromeTick; this.at;
        const p = this.panels()[this.at];
        const el = p?.querySelector('[x-data^="fileReview"]');
      ...
      markNear() {
        ...
        for (let i = Math.max(0, this.at - 1); i <= Math.min(files.length - 1, this.at + 1); i++) {
          if (files[i] && !this.mounted[files[i].path]) {
            this.mounted = { ...this.mounted, [files[i].path]: true };
          }
        }
        this.$nextTick(() => { this._chromeTick++; });
      },
      panels() {
        const el = this.$refs.fileStrip;
      ...
      go(i, jump = false) {
        const n = this.fileRows.length;
        if (!n) return;
        i = Math.max(0, Math.min(n - 1, i));
        this.at = i;
        const el = this.$refs.fileStrip, k = this.panels()[i];
        if (!el || !k) return;
      ...
      fileIndexOf(p) {
        if (!p) return -1;
        const idx = this.fileRows.findIndex(f => f.path === p);
        if (idx >= 0) return idx;
        return this.fileRows.findIndex(f => f.path.endsWith('/' + p) || f.path === p);
      },
      fileRepoBranch(f) {
        if (!f) return '';
        const slash = f.path.indexOf('/');
        ...
        const repos = this.record?.repos || [];
        const r = repos.find(x => x?.name === repoName);
        return repoName ? (repoName + (r?.branch ? '@' + r.branch : '')) : '';
      },
      slideCardOpts(f) {
        if (!f) return {};
        const slash = f.path.indexOf('/');
        ...
        const repos = this.record?.repos || [];
        const r = repos.find(x => x?.name === repoName);
        const branch = r?.branch || this.branch || 'main';
        ...
        return {
          ...
          fill: true,
          stats: true,
          openOn: md ? 'mddiff' : 'diff',
          onChrome: () => { this._chromeTick++; },
        };
      },
      async ensureFileReview() {
        if (typeof window !== 'undefined' && !window.fileReview && window.gh?.load) {
          try { await window.gh.load('alpineComponents/file-review.js'); } catch {}
        }
      },
      ...
      mountOutline() {
        ...
        try {
          // Released before the new one registers, not after: replaceChildren
          // drops the old tree either way, and its rail would otherwise keep
          // reading a detached list off every scroll for the life of the page.
          this.outlineOff?.();
          const built = window.sessionExport.index(this.record,
            { onOpen: (i) => this.openDeck(i), flow: !this.framed });
          this.outlineOff = built.destroy || null;
          box.replaceChildren(built.el);
      ...
      // runs into the hundreds of kilobytes and an interactive tree over it is
      ...
      async mountRaw() {
        ...
        if (!box) return;
        try {
          if (!window.JsonExplorer && window.gh?.load) {
            await window.gh.load('kits/json-explorer.js');
          }
          if (window.JsonExplorer) {
            this._rawExplorer = window.JsonExplorer.mount(box, {
              data: this.record,
              name: (this.shortId || 'session') + '.json',
              fill: this.framed,
            });
          } else if (window.chatRender) {
