  ...
  Alpine.data('fab', function() {
    return {
      ...
      _takeActionLabel(a) {
        ...
        if (a.key === 'render') return 'Copy HTML';
        if (a.key === 'brief') return 'Copy LLM brief';
        if (a.key === 'capture') return 'Copy capture';
        if (a.key === 'stage') return 'Open Stage';
        if (a.key === 'region') return 'Pick region';
        if (a.key === 'shot-view') return 'Preview visible view';
        if (a.key === 'shot-page') return 'Preview full page';
        if (a.key === 'shot-region') return 'Pick region';
        if (a.key === 'export') return 'Save Zip';
        if (a.key === 'offline') return 'Save Zip + code';
        return a.label;
      },

      _takeEffect(a) {
        if (a.kind === 'page') return 'Runs on the page';
        return ({ Copy: 'Copies to the clipboard', Open: 'Opens another surface',
                  Image: 'Builds a DOM-rendered PNG', Save: 'Downloads a file' })[a.group]
          || 'Runs this output';
      },

      // Cards show the output itself when the page already holds enough data
      // to describe it faithfully. Everything else says what work remains,
      // rather than presenting every action as the same empty placeholder.
      _takeDeferredReason(a) {
        if (a.kind === 'page') return 'The page supplies this action, so it runs when chosen.';
        if (a.key === 'render') return 'Repository source and modules are inlined when you copy it.';
        if (a.key === 'brief') return 'Repository source is fetched and assembled when you copy it.';
        if (a.key === 'stage') return 'The page file manifest is still being identified.';
        if (a.key === 'region' || a.key === 'shot-region') return 'An element has to be selected on the page first.';
        if (a.key === 'shot-view' || a.key === 'shot-page') return 'A fresh DOM render is produced when you preview it.';
        if (a.key === 'export' || a.key === 'offline') return 'Repository files are fetched and the archive is built when you save it.';
        return 'This output is produced when you choose its action.';
      },
      ...
      async openTakeDeck(start = 0) {
        ...
        const syncHeader = i => {
          ...
          handle.setSubtitle(a.group + ' \u00b7 ' + this.takeSubject);
          handle.setIcon(a.icon || 'ph-lightning');
        };
        const render = (i, slide) => {
          const a = items[i];
          const title = h('h2', { class: 'sr-only' }, a.label);
          const desc = h('p', { class: 'text-base leading-relaxed text-base-content/75' }, a.desc || a.label);
          const effect = h('div', { class: 'inline-flex w-fit items-center gap-1.5 rounded-full border border-base-300 bg-base-200 px-2.5 py-1 text-xs text-base-content/60' },
            h('i', { class: 'ph ph-arrow-right' }), this._takeEffect(a));
          let available = null;
          if (a.key === 'capture') {
            const json = JSON.stringify(this.captureData(), null, 2);
            available = h('div', { class: 'overflow-hidden rounded-xl border border-base-300 bg-base-200/60' },
              h('div', { class: 'flex items-center gap-1.5 border-b border-base-300 px-3 py-2 text-xs font-semibold text-success' },
                h('i', { class: 'ph ph-check-circle' }), 'Available now'),
              h('pre', { class: 'max-h-64 overflow-auto whitespace-pre-wrap break-words p-3 text-[11px] leading-relaxed text-base-content/70' }, json));
            ...
            available = h('div', { class: 'overflow-hidden rounded-xl border border-base-300 bg-base-200/60' },
              h('div', { class: 'flex items-center gap-1.5 border-b border-base-300 px-3 py-2 text-xs font-semibold text-success' },
                h('i', { class: 'ph ph-check-circle' }), 'Available now'),
              h('div', { class: 'max-h-64 space-y-3 overflow-auto p-3 text-xs' },
                h('div', { class: 'font-mono text-base-content/60' }, plan.repo + ' @ ' + plan.ref),
                h('ul', { class: 'space-y-1 font-mono text-base-content/75' },
                  ...files.map(path => h('li', { class: 'break-all' }, path))),
                ...(plan.reads.length ? [h('div', { class: 'space-y-1 border-t border-base-300 pt-3' },
                  h('div', { class: 'font-semibold text-base-content/55' }, 'Runtime data'),
                  h('ul', { class: 'space-y-1 font-mono text-base-content/75' },
                    ...plan.reads.map(path => h('li', { class: 'break-all' }, path))))] : [])));
          } else if (a.key === 'render' && this.takeTarget.payload) {
            const source = String(this.takeTarget.payload);
            const preview = source.length > 6000 ? source.slice(0, 6000) + '\n\u2026' : source;
            available = h('div', { class: 'overflow-hidden rounded-xl border border-base-300 bg-base-200/60' },
              h('div', { class: 'flex items-center gap-1.5 border-b border-base-300 px-3 py-2 text-xs font-semibold text-success' },
                h('i', { class: 'ph ph-check-circle' }), 'Available now'),
              h('pre', { class: 'max-h-64 overflow-auto whitespace-pre-wrap break-words p-3 text-[11px] leading-relaxed text-base-content/70' }, preview));
          }
          const readiness = available || h('div', { class: 'flex items-start gap-2 rounded-xl border border-dashed border-base-300 bg-base-200/35 p-3 text-sm text-base-content/60' },
            h('i', { class: 'ph ph-clock mt-0.5 shrink-0' }),
            h('div', {}, h('div', { class: 'font-semibold text-base-content/70' }, 'Made when chosen'),
              h('div', { class: 'mt-0.5' }, this._takeDeferredReason(a))));
          const status = h('div', { class: 'min-h-5 break-all font-mono text-xs text-base-content/55', 'aria-live': 'polite' });
          const action = h('button', { class: 'btn btn-primary w-full sm:w-auto gap-2' },
            h('i', { class: 'ph ' + (a.icon || 'ph-lightning') }), this._takeActionLabel(a));
          action.addEventListener('click', async () => {
            ...
            try {
              // An image preview is another reading level, so from this
              // catalog it drills down and returns here with a Back. The same
              // actions used from Quick actions still open at the top level.
              if (a.key === 'shot-view') await this.previewDomShot('viewport', null, handle);
              else if (a.key === 'shot-page') await this.previewDomShot('page', null, handle);
              else await this.runGridItem(a);
              status.textContent = this.outError || this.outMsg || 'Done';
            } finally { action.disabled = false; }
          });
          slide.append(
            h('article', { class: 'mx-auto flex min-h-full max-w-2xl flex-col justify-center gap-6 py-4' },
              title, desc, effect, readiness,
              h('div', { class: 'space-y-2 pt-2' }, action, status))
          );
        };
