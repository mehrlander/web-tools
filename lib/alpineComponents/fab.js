    ...
    return {
      ...
      async openTakeDeck(start = 0) {
        ...
        const render = (i, slide) => {
          const a = items[i];
          const mark = h('div', { class: 'flex size-14 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-primary' },
            h('i', { class: 'ph ' + (a.icon || 'ph-lightning') + ' text-3xl' }));
          const title = h('h2', { class: 'text-2xl font-bold leading-tight' }, a.label);
          const group = h('div', { class: 'text-[11px] font-semibold uppercase tracking-widest text-base-content/45' }, a.group);
          ...
          slide.append(
            h('article', { class: 'mx-auto flex min-h-full max-w-2xl flex-col justify-center gap-6 py-4' },
              h('div', { class: 'flex items-center gap-4' }, mark, h('div', { class: 'min-w-0 space-y-1' }, group, title)),
              desc, effect,
        ...
        handle = window.swipeDeck.open({
          count: items.length,
          start: Math.max(0, Math.min(items.length - 1, start)),
          title: 'Take outputs',
          subtitle: this.takeSubject,
          icon: 'ph-cards-three',
