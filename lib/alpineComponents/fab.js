...
const registerFab = function() {
  Alpine.data('fab', function() {
    return {
            ...
            <div class="flex-1 min-h-0 flex flex-col overflow-hidden">
              ...
              <div x-show="activeTab === 'text'" class="min-h-0 flex-1 overflow-y-auto overscroll-contain">
                ...
                <template x-if="textStats">
                  <div>
                    <!-- A bounded projection over retained proposals, not the
                         full text collection. It leads because this is the
                         contextual workflow: start with the text in front of
                         the reader, then inspect prior work on the same string. -->
                    <div class="border-b border-base-300/60">
                      ...
                      <template x-if="textPriorState === 'done' && textPrior">
                        <div>
                          ...
                          <!-- Exact is edge-trimmed equality. Contained is the
                               same literal string inside a larger selection or
                               page, not a semantic-neighbour claim. -->
                              ...
                              <template x-for="p in band.items" :key="p.id">
                                <article class="px-2.5 py-2 border-t border-base-300/30">
                                  ...
                                  <div class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px]">
                                    <span class="uppercase tracking-wide text-primary" x-text="p.action"></span>
                                    <span class="text-base-content/40" x-text="p.purpose"></span>
                                    <span x-show="band.key === 'contained' && p.spans && p.spans.length"
                                          class="text-base-content/30 tabular-nums"
                                          x-text="(p.spans || []).length + ((p.spans || []).length === 1 ? ' occurrence' : ' occurrences')"></span>
                                  </div>

                                  <!-- Applying a proposal is deliberately not
                                       an action here. The expandable record is
                                       for inspecting why and where it existed. -->
                                  <details class="mt-1.5 text-[12px] text-base-content/60">
                                    <summary class="cursor-pointer select-none text-base-content/45">provenance</summary>
                                    <div class="mt-1.5 pl-3 border-l border-base-300 space-y-2">
                                      <div x-show="p.author || p.purpose">
                                        <div x-show="p.author" x-text="p.author"></div>
                                        <div x-show="p.purpose" class="text-base-content/45" x-text="p.purpose"></div>
                                      </div>
                                      <template x-if="p.analysis">
                                        <div class="text-base-content/50">
                                          <span x-show="p.analysis.ordinary_reading"
                                                x-text="'ordinary reading: ' + p.analysis.ordinary_reading"></span>
                                          <span x-show="p.analysis.closed_in_situ"
                                                x-text="' · closed in place: ' + p.analysis.closed_in_situ"></span>
                                          <span x-show="p.analysis.fix_words !== null"
                                                x-text="' · added words: ' + p.analysis.fix_words"></span>
                                        </div>
                                      </template>
                                      <template x-for="o in p.origins" :key="o.source.source_id + ':' + o.record">
                                        <div class="space-y-1">
                                          <div class="flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5">
                                            <span class="text-base-content/35">catalog source</span>
                                            <a :href="o.source.url" target="_blank" rel="noopener"
                                               class="font-mono link link-hover break-all"
                                               x-text="o.source.path + (o.record !== null ? ' · record ' + o.record : '')"></a>
                                            <span class="font-mono text-base-content/30"
                                                  x-text="o.source.ref + (o.source.pinned ? '' : ' (not pinned)')"></span>
                                          </div>
                                          <template x-if="o.context">
                                            <div>
                                              <div class="flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5">
                                                <span class="text-base-content/35">context</span>
                                                <a :href="o.context.source.url" target="_blank" rel="noopener"
                                                   class="font-mono link link-hover break-all"
                                                   x-text="o.context.source.path + (o.context.section ? ' · ' + o.context.section : '')"></a>
                                                <span class="text-base-content/30"
                                                      x-text="[o.context.date, o.context.session_id_prefix].filter(Boolean).join(' · ')"></span>
                                              </div>
                                              <div x-show="o.context.markdown" class="mt-1 whitespace-pre-wrap text-base-content/55"
                                                   x-text="o.context.markdown"></div>
                                            </div>
                                          </template>
                                          <template x-if="o.patient">
                                            <div class="flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5">
                                              <span class="text-base-content/35">historical patient</span>
                                              <a :href="o.patient.url" target="_blank" rel="noopener"
                                                 class="font-mono link link-hover break-all"
                                                 x-text="o.patient.repo + '@' + o.patient.commit.slice(0, 8) + ':' + o.patient.path"></a>
                                            </div>
                                          </template>
                                          <template x-if="o.document && o.document.path">
                                            <div class="space-y-0.5">
                                              <div class="flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5">
                                                <span class="text-base-content/35">scanned document</span>
                                                <a :href="o.document.url" target="_blank" rel="noopener"
                                                   class="font-mono link link-hover break-all"
                                                   x-text="o.document.label || (o.document.repo + '/' + o.document.path)"></a>
                                              </div>
                                              <div x-show="o.agent || o.targetRatio !== null || (o.document && o.document.import_id)"
                                                   class="text-base-content/45">
                                                <span x-show="o.agent" x-text="o.agent"></span>
                                                <span x-show="o.signedAt"
                                                      x-text="(o.agent ? ' · ' : '') + 'signed ' + o.signedAt"></span>
                                                <span x-show="o.targetRatio !== null && o.targetRatio !== undefined"
                                                      x-text="' · targetRatio ' + o.targetRatio"></span>
                                                <span x-show="o.document && o.document.import_id"
                                                      x-text="' · import ' + o.document.import_id + ' (provenance only)'"></span>
                                              </div>
                                            </div>
                                          </template>
                                          <div x-show="o.original_status || o.original_note" class="text-base-content/50">
                                            <span x-show="o.original_status" x-text="o.original_status"></span>
                                            <span x-show="o.original_note"
                                                  x-text="(o.original_status ? ' · ' : '') + o.original_note"></span>
                                          </div>
                                          <div x-show="o.rationale" x-text="o.rationale"></div>
                                          <template x-if="o.relocation && o.relocation.destination">
                                            <div class="text-base-content/50">
                                              <span x-text="'relocation: ' + o.relocation.destination"></span>
                                              <span x-show="o.relocation?.note" class="text-base-content/40"
                                                    x-text="' · ' + o.relocation?.note"></span>
                                            </div>
                                          </template>
                                          <template x-for="(ev, ei) in o.evidence" :key="ei">
                                            <div class="text-base-content/45">
                                              <span x-show="ev.type" class="uppercase tracking-wide text-base-content/30" x-text="ev.type + ' '"></span>
                                              <span x-show="ev.source" class="font-mono" x-text="ev.source + ': '"></span>
                                              <span x-text="ev.quote || ''"></span>
                                            </div>
                                          </template>
                                        </div>
                                      </template>
      ...
      TEXT_PROPOSALS_SPEC: 'projects/text/current-sources.json',
      ...
      get textPriorCount() {
        ...
        return (this.textPrior.exact || []).length + (this.textPrior.contained || []).length;
      },
      ...
      get textPriorBands() {
        ...
        return [
          ...
          {
            key: 'contained',
            label: this.textScope === 'selection'
              ? 'Same string found in this selection'
              : 'Same string found on this page',
            items: this.textPrior.contained || [],
          },
      ...
      async _textPriorLoad() {
        if (!window.Csv) {
          const ok = await this._selfLoad('kits/csv.js', () => !!window.Csv);
          if (!ok) throw new Error('the CSV reader needed by the proposal index is unavailable');
        }
        if (!window.TextProposals) {
          const ok = await this._selfLoad('kits/text-proposals.js', () => !!window.TextProposals);
        ...
        return window.TextProposals.load(home, {
          specPath: this.TEXT_PROPOSALS_SPEC,
          quiet: true,
        });
      },
      ...
      async textPriorRun() {
        ...
        try {
          ...
          const result = await Promise.resolve(window.TextProposals.lookup(index, body, { contained: true }));
