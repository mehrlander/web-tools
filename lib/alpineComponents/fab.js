document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
                    ...
                    <div class="border-t border-base-300/60 pt-2">
                      ...
                      <button @click="downloadBundle()" :disabled="bundleBusy" class="btn btn-sm gap-1 w-full">
                        <span x-show="!bundleBusy" class="flex items-center gap-1">
                          <i class="ph ph-file-archive"></i>Download page + data
                      ...
                      <div class="text-[10px] text-base-content/40 leading-snug mt-1">Page source + the data it <code>read()</code>s, zipped so unzip-and-open resolves local. Code still loads from the CDN — local data, not offline.</div>
                    </div>
      ...
      reads: [],
      bundleBusy: false, bundleMsg: '', bundleError: '',
      ...
      // (self-loaded on first use, like the console panel). Local-DATA only:
      // the saved page still boots its code from the CDN.
      async downloadBundle() {
        ...
        try {
          ...
          const r = await window.bundle.page({ path: this.path });
