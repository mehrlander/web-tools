...
const registerFab = function() {
  Alpine.data('fab', function() {
    return {
          ...
          <div x-show="fabMenu" x-cloak x-transition.opacity.duration.120ms
            ...
            <template x-if="!hasToken">
              <div class="border-b-4 border-error/20">
                ...
                <button @click="fabMenu = false; pasteToken()"
                        class="w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-base-200 border-t border-base-200"
                        title="Read the clipboard, save what is on it as this browser's token, and reload">
      ...
      // NOTHING IS AWAITED BEFORE THE CLIPBOARD READ, the rule pasteToStage
      // above carries for the same reason: iOS spends the tap's user
      // activation on the first await, and a clipboard read that arrives after
      // one is refused. The kit was warmed on the press that opened the menu.
      pasteToken() {
        ...
        return window.io.pasteItems().then((flavors) => {
          ...
          if (!t) throw new Error(this._noTokenOnClipboard(flavors));
      ...
      // the first flavor that IS a token rather than the first flavor.
      //
      // The shapes are GitHub's own prefixes plus the 40-hex classic token.
      // Refusing what does not match is the point: a wrong string saved here
      // fails later as a 401 on some unrelated read, which is exactly the
      // silent failure this whole section exists to end.
      _tokenFromFlavors(flavors) {
        for (const f of (flavors || [])) {
          if (!f || f.kind !== 'text') continue;
          const t = String(f.text || '').trim();
          if (/^(gh[pousr]_|github_pat_)[A-Za-z0-9_]{10,}$/.test(t)) return t;
