...
// A small rendering copy is cheaper to reveal than to explain. This is an
// estimate over the live page, its own modules, and known read() payloads; a
// large or unknown closure keeps the explicit Load control.
const TAKE_HTML_AUTO_BYTES = 1024 * 1024;

// ── The typed subject ────────────────────────────────────────────────────
//
// Everything this drawer describes is an OBJECT: a thing with a type,
// properties and verbs. The announcement record (repo, ref, path, route,
// label, type) is its properties; the page's `actions`, `toggles` and
// `menu` contracts are its verbs; the tabs are lenses on it; and the drawer
// is the object's inspector. docs/subjects.csv is the list of object types,
// and a type's page (a renderer for a file, a form for a branch or a
// session, the app for the estate) is where its verbs are declared.
//
// The layer strip's ROLE says where a row sits in the frame stack (app,
// renderer, page). Its TYPE says what the row IS, in the terms of
// docs/subjects.csv: the estate, a repo, a project, a branch, a session, a
// page, a file, data, a fileset. The two are different readings of one walk,
// and captioning a lone unframed layer "page" was the stack word standing in
// for the type word: on the deployed Web Tools app it read as one more file
// being rendered. The registry owns the detect rules and these glyphs;
// routes-manifest.test.mjs holds this literal to its icon column, the way
// ViewRegistry.KIND_VIEW is held to routes-kinds.
const SUBJECT_ICON = {
  estate: 'ph-toolbox', repo: 'ph-git-fork', project: 'ph-folder-notch', branch: 'ph-git-branch',
  session: 'ph-chats', page: 'ph-file-html', file: 'ph-file', data: 'ph-table', fileset: 'ph-files',
};
// The ?view= keys whose subject is one repo rather than the estate: the rows
// docs/app-routes.csv files under group=repo, held to that column by the same
// test. Read only where the shell itself is not on the window to ask.
const REPO_VIEWS = ['landing', 'pages', 'atlas', 'config', 'files', 'project'];

// ── The Shortcuts bridge ─────────────────────────────────────────────────
//
// A home-screen web app on iOS gets its OWN storage partition: the token saved
// in Safari is not the token the installed app reads, and the app's own copy
// went missing overnight at least once (reported 2026-09-19). Why it went is
// not settled, and these rows do not depend on settling it. They make
// re-supplying the credential cost a tap and a paste instead of a trip to
// GitHub's token page on a phone, which is worth doing whatever the cause
// turns out to be.
//
// AN ANCHOR, NOT location.href. Handing a custom scheme to the system is a
// navigation a standalone web app does not come back from if iOS declines it,
// and an <a href> tapped by a finger is the form pages/shortcuts.html has used
// since before this file existed. Whether that form reaches Shortcuts FROM A
// HOME-SCREEN APP is the open question these rows were added to answer.
//
// SO THE FIRST ROW FILES ITS OWN ANSWER rather than asking for one. Log-Repo
// puts its input on the clipboard and commits it to shortcuts/log/ in
// web-tools-private, so a tap that reaches Shortcuts is legible from a
// checkout (shortcut-tools: python3 tools/log.py) and a tap that does not is
// legible as silence. The payload carries the browsing context because the log
// cannot learn that any other way, and the standalone/Safari split is the whole
// subject here.
const runShortcut = (name, text) =>
  'shortcuts://run-shortcut?name=' + encodeURIComponent(name)
  + (text == null ? '' : '&input=text&text=' + encodeURIComponent(text));

// The gesture router already on the device: one label in, that label's
// shortcut run on the clipboard, falling back to the back-tap dispatcher when
// the label is unknown (shortcut-tools workflows/route-gesture.json). Adding a
// route is one line in its table, so the web side can name its label now and
// the device side can learn it later. Until it does, this label is unknown and
// the tap raises the back-tap menu, which is a visible answer to the same
// question the bridge row answers silently.
const GESTURE_ROUTER = 'Route-Gesture';
const TOKEN_LABEL    = 'web-token';

// Every element the launcher's own subtree activates by a tap. holdTouch
// cancels touchstart across that subtree to hold a drag inside a sheet-
// presented in-app browser, and a cancelled touchstart takes the compatibility
// click AND an anchor's navigation with it, so anything left out of this
// selector is a control that does nothing on a phone and says nothing about
// why. Named once here because two places have to agree about it: the guard,
// and the test that reads the template back.
    ...
    return {
      ...
      template: `
        <div :style="'transform:translate(' + x + 'px,' + y + 'px)'"
             ...
             class="fixed bottom-6 right-6 group touch-none z-[2147482900]">
          <!-- A CONTROL THAT OWNS A SURFACE MUST SIT ABOVE IT, which is why
               this is a raw number rather than the z-[80] it was until
               2026-09-06. kits/annotate.js mounts its card at 2147482600, near
               the top of the range, because it has to cover whatever page it is
               annotating; bottom-anchored and 351px wide, it also covers this
               launcher on a phone. That was a nuisance while the card carried
               its own collapse toggle and became a dead end the moment the
               launcher menu became the only way to put the card down: raise the
               card, and the control that lowers it is behind it. Measured at
               390x640, the launcher disc entirely inside the card's box.

               The residue, named rather than papered over: the launcher now
               floats over the card's bottom-right corner, where Clear sits.
               Both surfaces drag, so either can be moved off the other, and a
               reachable dismiss is worth more than a clear corner.
          ...
          <div tabindex="0" role="button" aria-label="Web-tools panel"
               class="relative size-14 rounded-2xl border flex items-center justify-center cursor-grab active:cursor-grabbing outline-none transition-all duration-300"
               :class="offRef
            ...
            <!-- NO TOKEN, SAID ON THE ONE CONTROL THAT IS ALWAYS ON SCREEN.
                 Until now a token-less browser looked exactly like a
                 token-bearing one until a read failed, which on a home-screen
                 app is most of a session: the reads that need a token are the
                 private ones, and a page can paint a long way before it makes
                 one. Reported 2026-09-19, from an installed app that had lost
                 its token overnight.

                 A BADGE RATHER THAN A TINT, because the launcher's colour is
                 already spoken for: warning means the view is rendered off the
                 default branch, and it is the only thing saying so. A second
                 meaning on that channel would make each of them unreadable, so
                 the credential gets its own corner and its own colour. Same
                 reasoning as the drawer layer strip, which keeps selection off
                 the opacity channel because sealed already owns it.

                 It rides inside the launcher disc (position:relative above)
                 rather than in the drag wrapper, so it travels with the button
                 and never needs its own transform. -->
            <div x-show="!hasToken" x-cloak
                 class="absolute -top-1 -right-1 size-5 rounded-full bg-error text-error-content flex items-center justify-center shadow ring-2 ring-[var(--color-base-100)]"
                 aria-label="No GitHub token in this browser"
                 title="No GitHub token in this browser. Hold this button to fetch one.">
              <i class="ph ph-key text-[11px]"></i>
            </div>
          </div>
          ...
          <!-- The launcher's second gesture: a long press (or a right-click)
               ...
               get something in. What it costs to be everywhere is a place to
               hold the paste (kits/stage-handoff.js): off the app it is parked in
               ...
               every view. show-repo fills it with the header switch, the
               Render tab's control reached without knowing which tab holds
               it, which is what an app view needs since it opens with none on
               screen. This file claimed that row before it existed, and a
               reader who knew the control was there still could not find it
               (2026-09-08); the claim is now true. Rows are read at open time (readPageMenu), not
               ...
               drawer under it. -->
          <!-- THE SELECTION OFFER, teleported to the body and placed on the
               passage. Select text anywhere on the page and it appears beside
               it: one tap loads the annotator, aims it at the document the
               passage is in, and opens the composer on that passage. Until
               2026-09-08 there was no way to it at all from here, since the
               press that opens the launcher menu is what collapses the
               selection, and the menu behind it offers every aim but the text.

               IT SAT ABOVE THE LAUNCHER FIRST, and that was wrong for the
               reason a reader gives immediately: the offer is about the words,
               so it belongs at the words. The bottom-right corner is where the
               fab lives, not where you are reading, and an offer that far from
               its subject reads as a notification rather than as a control on
               the passage.

               TELEPORTED, because the launcher's own root carries a
               transform:translate for the drag, and a transform is a
               containing block for fixed descendants: a position:fixed child
               inside it is positioned against that box rather than the
               viewport, and moves whenever the launcher is dragged. Rendering
               into the body is what lets viewport coordinates mean what they
               say. (No backticks in here: this template IS a template
               literal.)

               NO QUOTE ON IT. The launcher-anchored version carried the
               passage's first words because it was nowhere near them; on the
               text that is the text saying itself twice, and the width it cost
               is width over the thing being read.

               BELOW THE SELECTION BY DEFAULT, which is the half of this that
               is a guess worth stating: iOS puts its own callout above a
               selection when there is room, so the underside is the freer one
               more often. It flips above where there is no room below. -->
          <template x-teleport="body">
            <div x-show="selOffer &amp;&amp; !fabMenu &amp;&amp; !open" x-cloak
                 @pointerdown.prevent.stop @contextmenu.stop
                 data-fab-sel-offer
                 class="fixed z-[2147482800] flex items-center gap-1 rounded-box border border-warning/50 bg-base-100 py-1 pl-2 pr-1 shadow-xl"
                 :style="selOffer ? ('left:' + selOffer.left + 'px;top:' + selOffer.top + 'px') : 'left:-9999px;top:-9999px'">
              <i class="ph ph-highlighter text-[15px] text-warning shrink-0"></i>
              <button @click="noteSelection()" class="btn btn-warning btn-xs shrink-0 whitespace-nowrap"
                      :title="selOffer ? ('Note: ' + selOffer.quote) : 'Note the text you selected'">+ note</button>
              <button @click="dropSelOffer()" class="btn btn-ghost btn-xs btn-square shrink-0"
                      aria-label="Forget this selection" title="Forget this selection">
                <i class="ph ph-x text-[14px]"></i>
              </button>
            </div>
          </template>
          <!-- THE PASTE CARD, which is where the token actually arrives.
               A FIELD RATHER THAN A CLIPBOARD READ, and the reason is the
               platform. navigator.clipboard.read() on iOS raises a system
               Paste confirmation that a page cannot style, cannot explain and
               does not always get, so a row that reads the clipboard silently
               is a row that sometimes does nothing for reasons nobody can see.
               A field takes the platform's own long-press Paste, which every
               reader already knows, and the clipboard button beside it is the
               shortcut for when the read IS permitted. Two routes, one field,
               and the field shows what landed either way.

               IT OPENS ITSELF ON THE WAY BACK. The trip is: tap Get token,
               Shortcuts runs, swipe back. Asking for a second long press at
               that point is asking the reader to remember what the first one
               was for, which is the cost "a probe carries its own instructions"
               exists to remove. So a Get token tap arms _awaitingToken, and the
               next time this document becomes visible with the token still
               missing, the card is already up.

               TELEPORTED TO THE BODY, for the reason the selection offer is:
               the launcher's root carries a transform for the drag, and a
               transform is a containing block for fixed descendants, so a
               fixed child inside it is positioned against that box and travels
               with the launcher.

               AT THE TOP, not above the launcher. The keyboard rises from the
               bottom and this card exists to be typed into.

               ABOVE THE LAUNCHER in the stack, since it is raised BY the
               launcher and covering it with the thing that opened it is the
               dead end the annotate card was corrected for. -->
          <template x-teleport="body">
            <div x-show="tokenCard" x-cloak x-transition.opacity.duration.150ms
                 @pointerdown.stop @pointerup.stop @pointermove.stop @contextmenu.stop
                 data-fab-token-card
                 class="fixed inset-x-4 top-6 z-[2147483000] mx-auto max-w-sm rounded-box border border-base-300 bg-base-100 p-3 shadow-2xl">
              <div class="flex items-center gap-2 mb-2">
                <i class="ph ph-key text-[17px] text-primary"></i>
                <span class="text-sm font-semibold">Paste your token</span>
                <button @click="tokenCard = false" aria-label="Close"
                        class="btn btn-ghost btn-xs btn-square ml-auto">
                  <i class="ph ph-x text-[14px]"></i>
                </button>
              </div>
              <div class="flex gap-2">
                <input x-model="tokenDraft" type="password" placeholder="ghp_..."
                       autocomplete="off" autocapitalize="off" autocorrect="off" spellcheck="false"
                       @keydown.enter="saveTokenDraft()"
                       class="input input-bordered input-sm flex-1 min-w-0 font-mono">
                <button @click="pasteIntoToken()" aria-label="Read the clipboard"
                        class="btn btn-sm btn-square" title="Read the clipboard into the field">
                  <i class="ph ph-clipboard-text text-[16px]"></i>
                </button>
                <button @click="saveTokenDraft()" class="btn btn-sm btn-primary">Save</button>
              </div>
              <p x-show="tokenMsg" x-cloak class="mt-2 text-xs text-error" x-text="tokenMsg"></p>
            </div>
          </template>
          <div x-show="fabMenu" x-cloak x-transition.opacity.duration.120ms
            ...
            <!-- THE CREDENTIAL SECTION, and only where the credential is
                 missing. The menu's standing rule is that a row which cannot
                 act does not appear (the section aim, "Hide notes"), and with a
                 token saved there is nothing here to fetch and nothing to
                 paste. It sits FIRST because a token-less browser cannot do the
                 thing most of the rows below it are for.

                 TWO ROWS, in the order they are used. "Get token" leaves for
                 Shortcuts; "Paste token" reads what came back and saves it.
                 The pair is the whole loop, and neither half asks for anything
                 to be typed on a phone.

                 THE BRIDGE TEST IS NOT HERE. It was, for one round, and that
                 was wrong for a reason the owner gave immediately: gating the
                 only row that runs a shortcut on a missing token means a
                 browser that HAS one cannot find out whether the mechanism
                 works at all. It is an instrument, so it lives with the
                 instruments below, in every state. -->
            <template x-if="!hasToken">
              <div class="border-b-4 border-error/20">
                <div class="flex items-center gap-2.5 px-3 py-2 bg-error/10">
                  <i class="ph ph-key text-[17px] text-error"></i>
                  <span class="text-sm font-semibold text-error">No token</span>
                </div>
                <a :href="tokenUrl" @click="runShortcutRow($event, tokenUrl)"
                   class="w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-base-200 border-t border-base-200"
                   title="Hand this browsing context off to Shortcuts, which puts a token where Paste token can read it">
                  <i class="ph ph-device-mobile text-[17px] text-primary"></i>
                  <span class="text-sm font-semibold">Get token</span>
                </a>
                <button @click="fabMenu = false; openTokenCard()"
                        class="w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-base-200 border-t border-base-200"
                        title="Open a field to paste the token into, with a button that reads the clipboard for you">
                  <i class="ph ph-clipboard-text text-[17px] text-primary"></i>
                  <span class="text-sm font-semibold">Paste token</span>
                </button>
              </div>
            </template>
            <!-- ONE ROW WHERE THERE WERE FOUR. The card's aim menu still has
                 ...
                 ITS GLYPH COMES FROM THE SAME PLACE, since 2026-09-06. It was
                 ph-text-align-left written out here, which is a paragraph
                 alignment control standing in for a markdown document: wrong
                 about markdown, and wrong in a way that would have travelled to
                 whatever declared next, since only the label was ever going to
                 change. The three built-in aims keep their glyphs inline
                 because they are this file's own; the declared one is the
                 kind's. -->
            <div class="w-full flex items-center gap-2.5 px-3 py-2 hover:bg-base-200">
              ...
              <div class="ml-auto flex gap-1">
                ...
                <button x-show="annKind" @click="fabMenu = false; annAim('section')"
                  ...
                  <i class="ph text-[19px] text-primary"
                     :class="(annKind &amp;&amp; annKind.aimIcon) || 'ph-file-md'"></i>
                </button>
            ...
            <!-- PUTTING THE CARD AWAY, which nothing anywhere could do until
                 2026-09-06. The card had a collapsed state that read as putting
                 it away and was not: it stayed, smaller, still over the page.
                 The card has one state now, so raising it and putting it down
                 are the two halves of this menu's job. The set survives:
                 disable() unmounts the card and keeps the notes, so the next
                 aim opens onto them again.

                 ITS OWN ROW, not a fifth glyph on the aims above, and both
                 reasons matter. A fifth key overflowed the w-56 menu and was
                 clipped away entirely (measured: the aims row wants 176px in
                 140px of room). And an X at the end of four aims reads as a
                 fifth aim, which is the peer problem the annotate card's own
                 readings strip was just corrected for. Every other verb here is
                 a labelled row; this is a verb.

                 HIDE, not close: the word says what happens to the card and not
                 to the notes, which stay. Only where there IS a card, which is
                 what annOn reads at open time; offered always it would be the
                 dead control the section aim above already refuses to be. -->
            <button x-show="annOn" @click="fabMenu = false; annOff()"
                    aria-label="Hide the note card"
                    class="w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-base-200 border-t border-base-200"
                    title="Put the note card away. The notes stay: the next aim opens onto them.">
              <i class="ph ph-x text-[17px] text-primary"></i>
              <span class="text-sm font-semibold">Hide notes</span>
            </button>
            ...
            <!-- The probe, beside Peek because they answer the two halves of
                 the same question. Peek says what a thing IS; the probe says
                 what the page is DOING, and neither is reachable from inside a
                 drawer at the moment it is wanted. Its label carries its state
                 rather than a second row, since this is one control with two
                 meanings and a menu read in the half-second before a finger
                 lifts has no room for a pair. -->
            <!-- THE ROW GREW VERBS, and the Note row above is why it has the
                 shape it has: a label that acts, and the things you do with it
                 as icon buttons beside it. Every one of the probe's own verbs
                 was a keyboard chord, which is no verb at all on the device
                 this instrument is aimed at. A reader who armed a probe on a
                 phone, watched the fault happen and wanted to send it had
                 nothing to press. Reported 2026-09-10, from a phone.

                 THREE, and each is here because a phone cannot reach it
                 otherwise. Send is the one that matters (alt+shift+P). Hold
                 freezes the trace so it can be read and screenshotted, which
                 is what the reader was doing by hand when the first capture
                 landed. Move hops the overlay off whatever it is covering,
                 which on a small screen is most things. Clear and the window
                 stay keyboard-only: one is rarely wanted and the other is a
                 popup, which a phone does not do usefully.

                 SHOWN ONLY WHILE A PROBE IS ON, the way "Hide notes" is shown
                 only where there is a card. Off, there is nothing to send. -->
            <div class="w-full flex items-center gap-1 pr-2 hover:bg-base-200 border-t border-base-200">
              <!-- The nowrap is not a nicety: with three icon buttons beside
                   it the label is the only item that can give, so "Probe off"
                   broke across two lines at 390px and took the row's height
                   with it. Measured, not guessed. (No backticks in here: this
                   whole template is a JS template literal, so one ends the
                   string and the component stops parsing.) -->
              <button @click="fabMenu = false; openProbe()"
                      class="flex flex-1 min-w-0 items-center gap-2.5 px-3 py-2.5 text-left whitespace-nowrap"
                      :title="probeArmed
                        ? 'Take the probe off this page: the overlay, its window and its listeners all go'
                        : 'Draw the live conditions of this page in the corner, with a trace of every popup guard decision. alt+shift+W opens it in a window, alt+shift+P files the capture.'">
                <i class="ph text-[17px] text-primary" :class="probeArmed ? 'ph-eye-slash' : 'ph-pulse'"></i>
                <span class="text-sm font-semibold" x-text="probeArmed ? 'Probe off' : 'Probe'"></span>
              </button>
              <template x-if="probeArmed">
                <div class="flex gap-0.5">
                  <button @click="fabMenu = false; probeVerb('send')" aria-label="Send the capture"
                          class="btn btn-ghost btn-sm btn-square"
                          :title="probeCanFile
                            ? 'File the capture to the log repo, the same as alt+shift+P'
                            : 'No token in this browser, so this hands the capture to the share sheet instead'">
                    <i class="ph text-[19px] text-primary"
                       :class="probeCanFile ? 'ph-cloud-arrow-up' : 'ph-share-network'"></i>
                  </button>
                  <button @click="probeVerb('hold')" :aria-label="probeHeld ? 'Resume the trace' : 'Hold the trace'"
                          class="btn btn-ghost btn-sm btn-square"
                          :title="probeHeld ? 'let the trace run again' : 'freeze the trace so it can be read'">
                    <i class="ph text-[19px] text-primary" :class="probeHeld ? 'ph-play' : 'ph-pause'"></i>
                  </button>
                  <button @click="probeVerb('move')" aria-label="Move the overlay to the next corner"
                          class="btn btn-ghost btn-sm btn-square" title="hop it to the next corner">
                    <i class="ph ph-arrows-out-cardinal text-[19px] text-primary"></i>
                  </button>
                </div>
              </template>
            </div>
            <!-- THE BRIDGE, beside Peek and the probe because it is the same
                 kind of thing: an instrument that answers one question about
                 the conditions this view is running under. Peek says what a
                 thing is, the probe says what the page is doing, and this says
                 whether this browsing context can hand a custom scheme to the
                 system at all.

                 IN EVERY STATE, not only where the token is missing. Gating it
                 on the credential was this file's first arrangement and it made
                 the one testable row unreachable from any browser that already
                 had a token, which is most of them and every desktop one.

                 IT FILES ITS OWN ANSWER. Log-Repo needs nothing installed and
                 commits whatever it is handed, so a tap that reaches Shortcuts
                 is legible from a shortcut-tools checkout (python3 tools/log.py)
                 and nobody has to report what appeared on a screen. What that
                 cannot say is the negative, since a link the system declines
                 looks exactly like one that was never tapped, which is what
                 runShortcutRow watches for from this side. -->
            <a :href="bridgeUrl" @click="runShortcutRow($event, bridgeUrl)"
               class="w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-base-200 border-t border-base-200"
               title="Hand a shortcuts:// link to the system and file one line in the device log saying it arrived. Nothing is installed. If nothing takes the link, this says so rather than going quiet.">
              <i class="ph ph-broadcast text-[17px] text-primary"></i>
              <span class="text-sm font-semibold">Test the bridge</span>
            </a>
            <button @click="fabMenu = false; pasteToStage()"
                    class="w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-base-200 border-t border-base-200"
                    title="Paste from the clipboard into the app">
              <i class="ph ph-clipboard-text text-[17px] text-primary"></i>
        ...
        <div class="absolute inset-y-0 right-0 transition-transform duration-300 ease-out pointer-events-none"
          ...
          <!-- CRUMBS ALONG THE WALK. The drawer's construction is where an
               iPhone loses the web process (measured 2026-09-08: every stage of
               toggle() finished inside ten milliseconds, then the tab died
               before $nextTick could fire, which puts the death inside this
               x-if's build). Alpine walks the tree in document order and runs
               x-init as it reaches each node, so these turn the walk itself
               into a trail: the last one written names the section the build
               did not get past. They cost one function call each. -->
          <div x-init="_crumbD('d:body')"
               class="h-full bg-base-100 border-l border-base-300 shadow-2xl flex flex-col pointer-events-auto overscroll-contain">
                ...
                <div class="flex items-stretch gap-0.5">
                  <template x-for="(L, i) in layers" :key="i">
                    <button @click="selectLayer(i)" :disabled="!!L.sealed" :title="layerTitle(L)"
                            ...
                            :class="[i === layerIndex ? 'bg-base-100 shadow-sm ring-1 ring-[var(--color-primary)]/40'
            ...
            <div class="flex-1 min-h-0 flex flex-col overflow-hidden">
              ...
              <div x-init="_crumbD('d:render-head')" x-show="activeTab === 'render'" class="shrink-0">
                <div class="relative p-2" @click.outside="ghMenu = false">
                  ...
                  <!-- THE TYPED SUBJECT LEADS. The repo and path under it are
                       the file that draws the view, which on the app is not
                       what the reader chose: they chose Activity, or a repo's
                       Atlas, and the strip above says so in one word. This line
                       says it in the identity block's own terms, so the picker
                       under it reads as "render a file instead" rather than as
                       the name of the thing on screen. -->
                  <div x-show="subjectHead" class="px-1 pb-1 flex items-center gap-1.5 text-[11px] text-base-content/60">
                    <i class="ph shrink-0" :class="subjectIcon"></i>
                    <span class="min-w-0 truncate" x-text="subjectHead"></span>
                  </div>
                  <div class="px-1 flex items-start gap-1.5">
                    ...
                    <button @click.stop="subjectLocal || togglePicker()"
                            :title="subjectLocal ? 'This came off the clipboard, so there is no repo to browse'
                                    : subjectIsApp ? 'Render a file instead, from any repo; this leaves the app'
                    ...
                    <!-- EDIT, a verb on the file itself, so it sits with the
                         file's identity rather than in the GitHub menu, whose
                         rows are all somewhere else. Markdown only: it opens
                         pages/dictate.html in file mode at the ref on display,
                         where the document is edited rendered and saved back. -->
                    <button x-show="editUrl" x-cloak @click="_go(editUrl)"
                            :title="'Edit ' + (path || '').split('/').pop() + ' at ' + viewingRef + ': rendered, by tap, voice and typing, saved back to GitHub'"
                            aria-label="Edit this file"
                            class="mt-0.5 shrink-0 flex items-center opacity-50 hover:opacity-100 hover:text-primary transition-opacity">
                  ...
                  <template x-for="b in visibleBranches" :key="b.name">
                    ...
                    <div class="flex flex-col gap-0.5 px-1.5 py-1 rounded transition-colors"
                      ...
                      <div class="flex items-center gap-1.5 text-[12px] pl-[18px] w-full opacity-70">
                        ...
                        <template x-if="branchAssistantMark(b)">
                          <a :href="branchSessionUrl(b)" :target="branchSessionUrl(b) ? '_blank' : null"
                             :title="branchAssistantTitle(b)"
                             class="shrink-0 flex items-center hover:opacity-75 transition-opacity"
                             :class="branchSessionUrl(b) ? 'cursor-pointer' : 'cursor-default pointer-events-none'"
                             x-html="branchAssistantMark(b)"></a>
                        </template>
              ...
              <div x-init="_crumbD('d:inspect')" x-show="activeTab === 'inspect'" class="min-h-0 flex-1 flex flex-col">
              ...
              <div x-init="_crumbD('d:traffic')" x-show="activeTab === 'traffic'" class="min-h-0 flex-1 overflow-y-auto overscroll-contain">
              ...
              <!-- TEXT. What the page SAYS, as against the four tabs about how
                   it was delivered. Local figures read the rendered document
                   without a model or network. Retained proposals and named files
                   make bounded reads of committed catalogs, while generation,
                   semantic matching, and source edits remain absent. Rows, not
                   stat cards, per the house style, and every figure that needs
                   a denominator carries one. -->
              <div x-show="activeTab === 'text'" class="min-h-0 flex-1 overflow-y-auto overscroll-contain">
                ...
                <template x-if="textStats">
                  <div>
                    <!-- The retained proposals whose text is on this page. It
                         leads because this is the contextual workflow: start
                         with the text in front of the reader, then inspect
                         prior work on the same string. Inspect only: nothing
                         here applies a proposal. -->
                    <div class="border-b border-base-300/60">
                      <div class="flex items-center gap-2 px-2.5 pt-2 pb-0.5">
                        <div class="text-[12px] uppercase tracking-wider text-base-content/50 font-semibold">Retained proposals</div>
                        <span x-show="textPriorState === 'loading'" class="loading loading-dots loading-xs text-primary"></span>
                        <template x-if="textPriorState === 'done' && textPrior">
                          <span class="text-[12px] text-base-content/40 tabular-nums" x-text="textPriorCount"></span>
                        </template>
                        <span class="grow"></span>
                        <a :href="TEXT_PROPOSALS_PAGE" target="_blank" rel="noopener"
                           class="text-[12px] link link-hover text-primary">browse all</a>
                      </div>

                      <div x-show="textPriorState === 'error'"
                           class="px-2.5 pb-2 text-[13px] text-error break-words" x-text="textPriorError"></div>

                      <template x-if="textPriorState === 'done' && textPrior">
                        <div>
                          <div x-show="!textPriorCount"
                               class="px-2.5 pb-2 text-[13px] text-base-content/50 italic"
                               x-text="textScope === 'selection'
                                 ? 'No retained proposal source text matches this selection.'
                                 : 'No retained proposal source text appears on this page.'"></div>

                          <!-- Exact is edge-trimmed equality with a proposal's
                               text, and that is the only match made. -->
                          <template x-for="band in textPriorBands" :key="band.key">
                            <section class="border-t border-base-300/40">
                              <div class="px-2.5 pt-2 pb-0.5 text-[11px] uppercase tracking-wider text-base-content/40 font-semibold"
                                   x-text="band.label"></div>
                              <template x-for="p in band.items" :key="p.id">
                                <article class="px-2.5 py-2 border-t border-base-300/30">
                                  <div class="flex items-start gap-1.5 min-w-0 text-[13px] leading-snug">
                                    <span class="min-w-0 break-words text-base-content/70" x-text="p.from.text"></span>
                                    <i class="ph ph-arrow-right shrink-0 mt-0.5 text-base-content/30"></i>
                                    <span class="min-w-0 break-words font-semibold" x-text="p.to.text"></span>
                                  </div>
                                  <div class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px]">
                                    <span class="uppercase tracking-wide text-primary" x-text="p.purpose"></span>
                                    <span class="text-base-content/40" x-text="p.author"></span>
                                  </div>
                          ...
                          <template x-for="(warning, wi) in textPrior.warnings" :key="wi">
                            <div class="px-2.5 py-1.5 border-t border-warning/20 bg-warning/5 text-[12px] text-base-content/55 leading-snug"
                                 x-text="warning"></div>
                          </template>
                        </div>
                      </template>
                    ...
                    <div class="px-2.5 py-2 border-t border-base-300/60 text-[12px] text-base-content/50 leading-snug">
                      Literal retained proposals and registered file names only, and no model. The estate
                      keeps no committed vocabulary keyed by the words prose actually uses, so terms of art
                      and semantic neighbours are not looked up here.
                    </div>
                  </div>
                </template>
              </div>

              <div x-init="_crumbD('d:render-body')" x-show="activeTab === 'render'" class="min-h-0 flex-1 flex flex-col">
                ...
                <template x-if="path">
                  <div class="p-2 flex flex-col gap-2 min-h-0 flex-1">
                    ...
                    <div x-init="_crumbD('d:rb-guide')" class="min-h-0 flex-1 flex flex-col">
                    ...
                    <div x-init="_crumbD('d:rb-take')" class="contents"></div>
                    <!-- The take browser. A phone cannot expose the descriptions
                         that used to live only in title attributes, so a row of
                         terse buttons asked the reader to act before they knew
                         what each output contained. One house-deck door now
                         turns those outputs into full cards: swipe to compare,
                         then use the one explicit verb on the card. The compact
                         grid survives under Quick actions for repeat work where
                    ...
                    <div class="border-t border-base-300/60 pt-2 pb-10 flex flex-col gap-1.5 shrink-0">
                      ...
                      <button @click="openTakeDeck()" data-fab-take-browser
                              :disabled="outBusy || !takeDeckItems.length"
                              class="btn btn-sm w-full justify-start gap-2 border-base-300 bg-base-100 shadow-none">
                        <i class="ph ph-cards-three text-lg text-primary"></i>
                        <span class="font-semibold">Browse outputs</span>
                        <span class="ml-auto font-mono text-[11px] opacity-50"
                              x-text="takeDeckItems.length"></span>
                        <i class="ph ph-caret-right text-xs opacity-40"></i>
                      </button>
                      <details class="group/take">
                        <summary class="cursor-pointer select-none px-1 text-[11px] opacity-45 hover:opacity-80">
                          Quick actions
                        </summary>
                        <div class="mt-1.5 flex flex-col gap-1.5" data-fab-take-quick>
                          <!-- A row per destination: the gutter says what happens
                               to you, the buttons say what you get. Kept as the
                               fast path, not the first explanation. -->
            ...
            <div class="shrink-0 border-t border-base-300 flex flex-col">
              ...
              <div x-init="_crumbD('d:rb-console')" x-show="consoleOpen" class="border-t border-base-300/60 flex flex-col" style="max-height: 40vh;">
                ...
                <!-- x-if, NOT x-show, and this is the line that was killing the
                     tab. x-show hides a subtree that has already been built, so
                     this list rendered one node per captured console line, with
                     six bindings each, every time the drawer opened, whether or
                     not the console was expanded and whether or not the rich
                     panel had replaced it. With consoleLogs uncapped (see the
                     cap on the push side), a page that logged freely handed
                     Alpine an unbounded walk inside a single reactive flush.
                     Measured on an iPhone 2026-09-08: the build reached this
                     node at 72ms, having taken 12ms to get through the other
                     three tab panes, and the web process died there. -->
                <template x-if="!consolePanelReady && consoleOpen">
                <div id="__fab-console-panel" class="overflow-y-auto overscroll-contain p-1 flex flex-col gap-0.5" style="max-height: 40vh;">
            ...
            <!-- THE LAST NODE OF THE WALK, and the crumb that separates two
                 answers the trail could not tell apart. Every mark above is a
                 node in the tree, so a trail ending at one of them says the
                 walk stopped there OR that the walk finished and something
                 after it died: the dom: mark is written from a $nextTick, which is not
                 synchronous, so the gap between the last mark and it contains
                 the browser's own layout and paint. d:end fires while the walk
                 is still running, so a trail that reaches it puts the death
                 past the DOM build entirely. -->
            <div x-init="_crumbD('d:end')" class="hidden"></div>
            </div>
          </template>
      ...
      fabMenu: false,
      pageMenu: [],
      // The selection the page has right now, offered ON it (see the
      // template): { left, top, quote }, in viewport coordinates. The Range
      // and its document ride on the component off the reactive path
      // (_selRange, _selDoc), since a Range is spent by handing it to the
      // annotator, not read by a template.
      selOffer: null,
      ...
      annKind: null,
      // Whether a card is up right now, read at menu-open time for the same
      // reason annKind is: it changes under one page load, and the row that
      // offers to put the card away must not appear when there is none.
      annOn: false,
      // Whether a probe is on the watched view, and a MIRROR rather than the
      // live read, for the reason annOn is one: `probeOn` reaches across into
      // another realm's `window.Probe`, which Alpine cannot track, so a label
      // bound to it renders once and then lies. Measured 2026-09-10 in a
      // browser: the row still worked and still read "Probe", which is the
      // worse half, since a reader with no keyboard has only this row to turn
      // the thing off and no reason to believe it will. Refreshed at menu-open
      // time and at the moment it changes.
      probeArmed: false,
      // Two more mirrors of the same cross-realm state, and refreshed in the
      // same breath. `probeHeld` puts the pause key's state on its own glyph;
      // `probeCanFile` decides which verb the send button IS, which has to be
      // known BEFORE the tap rather than after a failed write: iOS spends the
      // gesture on the first await, so a share offered as a fallback to filing
      // arrives with no transient activation left and is refused.
      probeHeld: false,
      probeCanFile: true,
      // Whether this browser holds a GitHub token. TRUE until the first read,
      // so a badge never flashes over a launcher on a page that has one; init
      // settles it before paint and the read is cheap enough to repeat at
      // menu-open time, which is what catches a token saved on another surface
      // of the same page.
      hasToken: true,
      // The paste card: whether it is up, what is in its field, and what it
      // has to say about the last attempt. Held here rather than in the card
      // because the card is teleported and its lifetime is the component's.
      tokenCard: false,
      tokenDraft: '',
      tokenMsg: '',
      consoleOpen: false,
      ...
      // Text tab. Local figures are decidable from the rendered document with
      // no network or model. Two bounded catalog reads add registered paths and
      // retained literal proposals with provenance. Semantic matching,
      // vocabulary inference, generation, and source edits are deliberately
      // absent rather than stubbed, so this pane never shows a number it cannot
      // stand behind. The design and the measured limits are in
      // docs/text-tools.md.
      //
      ...
      textPrior: null, textPriorState: 'idle', textPriorError: '',
      _textPriorEpoch: 0,
      TEXT_PROPOSALS_REPO: 'mehrlander/home',
      TEXT_PROPOSALS_REF: 'main',
      TEXT_PROPOSALS_PAGE: 'https://mehrlander.github.io/web-tools/pages/text-lab.html?pane=proposals',
      ...
      get textPriorCount() {
        if (!this.textPrior) return 0;
        return (this.textPrior.exact || []).length;
      },

      get textPriorBands() {
        if (!this.textPrior) return [];
        return [
          { key: 'exact', label: 'Exact source text', items: this.textPrior.exact || [] },
        ].filter(b => b.items.length);
      },

      async _textPriorLoad() {
        if (!window.TextCollection) {
          const ok = await this._selfLoad('kits/text-collection.js', () => !!window.TextCollection);
          if (!ok) throw new Error('the proposal index kit is unavailable');
        }
        // Resolve the saved credential at the moment of the read. window.TOKEN
        // is kept for shortcut-injected pages, but an app account switch must
        // not leave this private Home read on the credential from page boot.
        const saved = window.ghAuth?.resolve?.() || '';
        const embedded = window.TOKEN && !String(window.TOKEN).includes('🎟')
          ? window.TOKEN : '';
        const home = new window.GH({ token: saved || embedded,
          repo: this.TEXT_PROPOSALS_REPO, ref: this.TEXT_PROPOSALS_REF });
        return window.TextCollection.load(home, { quiet: true });
      },

      async textPriorRun() {
        const body = (this.textStats && this.textStats.visible) || '';
        const epoch = ++this._textPriorEpoch;
        this.textPrior = null;
        this.textPriorError = '';
        if (!body) { this.textPriorState = 'idle'; return; }
        this.textPriorState = 'loading';
        try {
          const index = await this._textPriorLoad();
          const result = await Promise.resolve(window.TextCollection.lookup(index, body));
          if (epoch !== this._textPriorEpoch) return;
          this.textPrior = result;
          this.textPriorState = 'done';
        } catch (e) {
          if (epoch !== this._textPriorEpoch) return;
          this.textPriorError = e.message || String(e);
          this.textPriorState = 'error';
        }
      },
      ...
      _textRead(input) {
        ...
        const hidden = (el) => {
          ...
          let answer = el.hidden || el.hasAttribute('x-cloak')
            || String(el.getAttribute('aria-hidden') || '').toLowerCase() === 'true';
          if (!answer) {
            const inline = el.style || {};
            answer = inline.display === 'none' || inline.visibility === 'hidden'
              || inline.visibility === 'collapse' || inline.opacity === '0';
          }
          if (!answer) {
            try {
              const css = doc.defaultView?.getComputedStyle?.(el);
              answer = css?.display === 'none' || css?.visibility === 'hidden'
                || css?.visibility === 'collapse' || css?.opacity === '0'
                || css?.contentVisibility === 'hidden';
            } catch {}
          }
          visibility.set(el, !!answer);
          return !!answer;
        };
          ...
          acceptNode: (n) => {
            for (let el = n.parentElement; el; el = el.parentElement) {
              if (SKIP.has(el.tagName) || el.hasAttribute('data-annotate-ui')
                  || el.closest?.('[x-data^="fab"]') === el || hidden(el)) return 2 /* REJECT */;
              if (el === root) break;
            }
            return 1 /* ACCEPT */;
          },
        });
        // A block that preserves whitespace keeps its runs verbatim. Memoised
        // like hidden(), since a computed-style read per ancestor per text node
        // is the one part of this walk that is not cheap.
        const PRE_TAGS = new Set(['PRE', 'TEXTAREA']);
        const preserving = new WeakMap();
        const preserves = (el) => {
          if (preserving.has(el)) return preserving.get(el);
          let answer = PRE_TAGS.has(el.tagName);
          if (!answer) {
            const inline = String(el.style?.whiteSpace || '');
            answer = inline.startsWith('pre') || inline === 'break-spaces';
          }
          if (!answer) {
            try {
              const css = String(doc.defaultView?.getComputedStyle?.(el)?.whiteSpace || '');
              answer = css.startsWith('pre') || css === 'break-spaces';
            } catch {}
          }
          preserving.set(el, !!answer);
          return !!answer;
        };
        const kept = (node) => {
          for (let el = node.parentElement; el; el = el.parentElement) {
            if (preserves(el)) return true;
            if (el === root) break;
          }
          return false;
        };

        // TWO DERIVATIONS, because the two consumers want opposite things from
        // the same walk, and one read serving both is what made this wrong.
        //
        // BODY is the prose figures. Text runs join with a SPACE inside one
        // block and a NEWLINE between blocks, and the difference decides
        // whether "longest sentence" means anything. Joining every run with a
        // newline splits a sentence that merely contains an <a> or a <code>,
        // which is most of this estate's prose; joining every run with a space
        // runs a table's cells together into one paragraph-long
        // pseudo-sentence, because a cell carries no terminal punctuation for
        // the splitter to find.
        //
        ...
        //
        // VISIBLE is proposal matching, and it must not invent a separator.
        // The retained store keeps internal whitespace significant as a matter
        // of identity (home projects/text/DESIGN.md), so a fabricated space is
        // not cosmetic: it decides whether a string matches. Trimming each run
        // and joining with a space got both directions wrong. It reported
        // "Web Tools" on a page rendering Web<em>Tools</em>, a positive claim
        // about a string the reader cannot find, and it flattened a <pre>
        // newline so that two of the seventy-one retained source texts could
        // never match anything at all. So: concatenate the runs inside a block
        // as the reader sees them, collapse whitespace the way CSS does, and
        // leave a whitespace-preserving block alone.
        let body = '', chrome = 0, runs = 0, n;
        let prevBlock = null;
        const seen = [];
        let buffer = '', bufferPre = false, bufferBlock = null;
        const closeBlock = () => {
          const text = (bufferPre ? buffer : buffer.replace(/\s+/g, ' ')).trim();
        ...
        while ((n = walker.nextNode())) {
          ...
          if (block !== bufferBlock) { closeBlock(); bufferBlock = block; }
          // A whitespace-only run is dropped from the prose figures below and
          // kept here: between two inline elements it IS the separator, and
          // dropping it is what glued "Web" to "Tools".
          buffer += n.data;
          if (kept(n)) bufferPre = true;
        ...
        return {
          ...
          body,
          // Proposal matching is about the text the reader can see, including
          // links and controls, and about seeing it with the spacing it
          // actually has. Body excludes chrome and rejoins runs for the
          // sentence splitter, so it cannot serve both jobs.
          visible,
      ...
      async _registered(repo, ref) {
        ...
        await Promise.all(this.MATCH_REGISTRIES.map(async (r) => {
          try {
            // Every registry here is a CSV, so one reader covers all five and a
            // new one is a row rather than a shape. It was JSON.parse against
            // five nested shapes until 2026-08-18, and each registry that became
      ...
      consoleLogs: [],
      // How many the drawer keeps, and the ceiling the take already assumed.
      CONSOLE_CAP: 300,
      consoleDropped: 0,
      loadedScripts: [],
      highlighted: null,
      ...
      briefReady: false, briefLoading: false, _briefWait: null,
      ...
      init() {
        ...
        // Saving or clearing a token is an account boundary. Drop results read
        // from the previous private account at once and invalidate any in-flight
        // paint by epoch. The account control reloads the page immediately, so
        // do not start another private read in the transient pre-reload document.
        this.hasToken = this._readToken();
        // Coming back from Shortcuts. Nothing else here cares about the page
        // becoming visible, so this listener exists for the token trip alone
        // and disarms itself after one return.
        this._visListener = () => {
          if (document.visibilityState !== 'visible') return;
          if (!this._awaitingToken) return;
          this._awaitingToken = false;
          this.hasToken = this._readToken();
          if (!this.hasToken) this.openTokenCard();
        };
        document.addEventListener('visibilitychange', this._visListener);
        this._authListener = () => {
          this.hasToken = this._readToken();
          this._textPriorEpoch++;
          this.textPrior = null;
          this.textPriorError = '';
          this.textPriorState = 'idle';
        };
        window.addEventListener('web-tools:token-changed', this._authListener);
        ...
        // The one listener this component arms on the host document, and it
        // is armed on purpose: see _watchSelection.
        this._watchSelection(document);
        this.infer();
        ...
        // After adoptSubject below has run at least once, so the report can say
        // what this fab was pointed at. Deliberately not awaited: a report is
        // never allowed to hold up the launcher.
        setTimeout(() => { this._reportUnfinished(); }, 1200);
        ...
        // CAPPED, because nothing else was. The array grew for the life of the
        // page, and the take already slices it to 300, so anything past that
        // was being kept only to be walked by the fallback list above and then
        // thrown away. The oldest go first: a console read after a failure is
        // read from the bottom.
        const capLogs = () => {
          const over = this.consoleLogs.length - this.CONSOLE_CAP;
          if (over > 0) { this.consoleLogs.splice(0, over); this.consoleDropped = (this.consoleDropped || 0) + over; }
        };
        if (window.consoleKit) {
          this._offConsole = console.subscribe(e => {
            if (e.clear) { this.consoleLogs = []; this.consoleDropped = 0; return; }
        ...
        this.$watch('open', v => { if (v) this.opened = true; });
        if (this.open) this.opened = true;
        // THE RICH PANEL WAITS FOR THE CONSOLE TO BE OPENED, not for the drawer.
        // It loads two more modules and initTrees a component that renders every
        // retained line: measured here, 7,021 elements for 300 lines, roughly
        // three times the whole rest of the drawer. Doing that on the first
        // drawer open charged every reader of the Render tab for a panel that
        // was collapsed and that most of them never expand. It is also the
        // heaviest thing running in the window where an iPhone loses the web
        // process, immediately after the last node of the walk.
        this.$watch('consoleOpen', v => { if (v) this.$nextTick(() => this._mountConsolePanel()); });
        if (this.consoleOpen && this.open) this.$nextTick(() => this._mountConsolePanel());
      ...
      destroy() {
        ...
        if (this._authListener) window.removeEventListener('web-tools:token-changed', this._authListener);
      ...
      // fails if they drift.
      // `u` is url's SHORT ALIAS. It was the one member of this list the
      // registry did not describe, pinned in routes-manifest.test.mjs as a
      // known gap, and it gained a row on 2026-09-17 when `carries` became the
      // boundary kits/visit-log.js filters a stored visit on: a mode the table
      // omits is invisible to anything reading the table to decide what may be
      // stored, which is the one gap an allowlist cannot fail safe through.
      ...
      adoptSubject() {
        // In a readable toss the page a reader selects text on is the subject
        // frame's, so its document is watched as well. Each render is a new
        // frame document; the WeakSet in _watchSelection makes this idempotent.
        try {
          const d = window.__tossFrame && window.__tossFrame.contentDocument;
          if (d && d.body) this._watchSelection(d);
      ...
      _afterIdentityChange(was) {
        ...
        // Text is a lens on the selected layer too. Repointing the layer while
        // this tab is open must replace its page/selection read; otherwise the
        // identity strip changes while the figures and retained proposals remain
        // attached to the layer just left.
        if (this.open && this.activeTab === 'text' && (refChanged || pathChanged)) {
          this.textScan();
        }
      },
      ...
      //
      // AN ANCHOR IS A ROW TOO, and this read 'button' alone until 2026-09-20,
      // which is how the credential rows shipped dead on every touch device.
      // They are anchors because that is the only form that hands a custom
      // scheme to the system reliably, and cancelling touchstart takes an
      // anchor's activation with the compatibility click, so the tap did
      // nothing at all: no navigation, no click handler, and therefore not even
      // the report the row carries for a hand-off nothing accepts. Measured
      // from the device against a control: the identical shortcuts:// link
      // tapped in a chat client filed its row, and tapped here filed nothing.
      //
      // The general rule this missed, stated so the next row does not repeat
      // it: a guard that cancels a gesture on a whole subtree has to name every
      // element the subtree activates, and the menu's vocabulary is button and
      // a. tools/test/fab-menu.test.mjs holds the two together by reading the
      // template rather than by trusting this list.
      holdTouch(e) {
        if (e.target && e.target.closest && e.target.closest(TAPPABLE)) return;
        e.preventDefault();
      },
      ...
      onDown(e) {
        ...
        //
        // This is one line's worth of listener on the fab's OWN element. The
        // host document IS watched now (selectionchange, _watchSelection, since
        // 2026-09-08), for the selection offer above the launcher; this
        // snapshot stays because it is taken at the tap, which is the last
        // moment the selection exists, and the offer's own reading is
        // debounced and may have been dropped by the reader already.
        this._grabSelection();
        // A SECONDARY BUTTON IS NOT A TAP AND NOT A DRAG. It raises the menu
        // and nothing else, which the button number settles here rather than
        // leaving to onContextMenu's flag: see the note on _secondary.
        if (this._secondary(e)) return;
        this.down = true;
        this.dragged = false;
        ...
        //
        // 450ms WAS TOO SHORT FOR A TAP, which is not a taste call: a reader
        // reported the launcher dead, and a probe logging pointer timings on
        // their own phone came back 59, 563, 595 and 732ms for four ordinary
        // taps. Three of the four were long presses by that threshold, so the
        // menu opened, the fired long press ate the pointerup, and the drawer
        // never came. From the outside that is a button that does nothing.
        //
        // LONG_PRESS clears their slowest tap. It is still a threshold, so a
        // slower press than any measured will read as a hold again; what has
        // changed is that an ordinary tap on a phone no longer does.
        this._lpFired = false;
      ...
      // WHICH BUTTON, WHICH IS THE ONLY ORDER-FREE READING OF A RIGHT-CLICK.
      //
      // The first fix for "right-click also opens the drawer" (2026-09-08) was
      // onContextMenu's `_lpFired = true` alone, on the stated premise that a
      // right-click "delivers pointerdown and pointerup AROUND the contextmenu
      // event". That premise is platform-specific and it is where the bug
      // survived: macOS and Linux raise the context menu on the PRESS, so the
      // flag is set before pointerup reads it, while Windows raises it on the
      // RELEASE, so pointerup ran first with the flag still false, toggled the
      // drawer, and the menu then opened over an open drawer. Reproduced in
      // jsdom by dispatching the two orders against this component
      // (tools/test/fab-menu.test.mjs, "right-click raises the menu without
      // toggling the drawer, in either platform's event order").
      //
      // PointerEvent.button carries the same fact on every event of the
      // gesture, before any of them, so reading it decides without knowing
      // which platform is underneath. Primary press and every touch or pen
      // contact report 0; a right-click reports 2 and a middle-click 1, and
      // pointercancel reports -1, which is why the test is `> 0` rather than
      // a truthiness check that would also swallow the cancel path.
      //
      // onContextMenu's flag STAYS, and not as belt and braces: Safari on
      // macOS reports button 0 for a ctrl+click and raises contextmenu from
      // it, so that gesture arrives here as an ordinary press and the flag is
      // the only thing that spends it.
      _secondary(e) { return !!e && typeof e.button === 'number' && e.button > 0; },

      // Right-click raises the SAME menu the long press does, and has to spend
      // the gesture the same way: the flag onUp reads is what stops the
      // release behind the menu from toggling the drawer as well.
      onContextMenu() {
        this._clearLongPress();
        this._lpFired = true;
        this.openFabMenu();
      ...
      // ── The credential ───────────────────────────────────────────────────
      //
      // Read rather than remembered. Two same-origin surfaces can save a token
      // between two paints of this component (the app's own account control is
      // one), and gh-auth announces every such change, so the cost of asking
      // again is one localStorage hit and the cost of not asking is a badge
      // that lies.
      _readToken() {
        try {
          if (window.ghAuth) return !!window.ghAuth.resolve();
          return !!localStorage.getItem('ghToken');
        } catch (e) { return false; }
      },

      // Where "Get token" goes: the gesture router, carrying a label rather
      // than a shortcut name. Naming the router is what makes the device side
      // a one-line change instead of a second install, and an unknown label
      // there falls back to the back-tap menu rather than to silence.
      get tokenUrl() { return runShortcut(GESTURE_ROUTER, TOKEN_LABEL); },

      // Where "Test the bridge" goes. Everything the sender can decide, the
      // sender decides, so the payload carries the one fact the log has no
      // other way to learn: which browsing context tapped it. navigator
      // .standalone is the iOS answer and the display-mode query is everyone
      // else's; either one is enough, so both are asked and the first that
      // says standalone wins.
      get bridgeUrl() {
        let mode = 'browser';
        try {
          if (window.navigator.standalone) mode = 'standalone';
          else if (window.matchMedia('(display-mode: standalone)').matches) mode = 'standalone';
        } catch (e) {}
        // standalone/browser does not separate a phone from a laptop, and the
        // two fail for different reasons: a desktop browser may have no handler
        // at all. maxTouchPoints is the coarse answer that iPadOS does not lie
        // about, unlike the user-agent string.
        let plat = 'desktop';
        try { if ((navigator.maxTouchPoints || 0) > 1) plat = 'touch'; } catch (e) {}
        return runShortcut('Log-Repo', JSON.stringify({
          op: 'web-fab', name: 'bridge', mode, plat,
          page: location.pathname.split('/').pop() || 'index.html'
        }));
      },

      // ── Handing a custom scheme to the system ───────────────────────────
      //
      // The anchor's own href does the work; this only closes the menu and
      // then WATCHES, because a scheme nothing is registered for fails in
      // total silence. There is no error event, no rejected promise and no
      // change to the page: a desktop browser with no Shortcuts app and a tap
      // that was never made are indistinguishable from script. That silence is
      // what sent the first report back as "it did not work on desktop" with
      // nothing to say whether the link was wrong or the platform was.
      //
      // WHAT IS OBSERVABLE IS LEAVING. A handoff the system takes backgrounds
      // this document, which fires visibilitychange or pagehide first. So arm
      // both, wait, and if the document is still here and still visible, say
      // so and show the exact URL that was tried. A reader who can see the
      // string can test it somewhere that does have a handler.
      //
      // IT REPORTS A CONDITION, NOT A VERDICT, and the difference is not
      // pedantry. A modal "Open in Shortcuts?" prompt leaves the page visible
      // while it is up, so a wait that expires under one would call a working
      // link broken. The elapsed check covers the other direction: a timer
      // that comes back far later than it was set for was throttled, which
      // only happens to a document that went away, so that reads as leaving.
      runShortcutRow(ev, url) {
        this.fabMenu = false;
        if (!url) return;
        // A Get token tap is the one whose answer comes back on the clipboard,
        // so the return trip is part of the row rather than a second gesture
        // the reader has to remember to make.
        if (url === this.tokenUrl) this._awaitingToken = true;
        let left = false;
        ...
        setTimeout(() => {
          ...
          this._reportInDrawer(
            'This page never went away, so nothing here took the link. That is '
            + 'what a browser with no Shortcuts app does, and also what a blocked '
            + 'hand-off looks like; only a device with the app tells them apart. '
            + 'Tried: ' + url);
        }, 2000);
      },

      // Raising the card, from the menu row or from the way back. _warmPaste
      // loads the clipboard kit now so the button inside has something to call
      // without awaiting on the tap that presses it, which is the rule
      // pasteToStage carries: iOS spends a tap's user activation on the first
      // await, and a clipboard read that arrives after one is refused.
      openTokenCard() {
        this.tokenMsg = '';
        this.tokenDraft = '';
        this._warmPaste();
      ...
      // The clipboard button. It FILLS the field rather than saving, so what
      // arrived is visible before it is committed: a read that returns the
      // wrong thing is the case worth seeing, and saving it silently would
      // spend the trip and fail later as a 401 on some unrelated request.
      pasteIntoToken() {
        this.tokenMsg = '';
        if (!window.io || !window.io.pasteItems) {
          this.tokenMsg = 'the clipboard kit is still loading; paste into the field instead';
          return Promise.resolve();
        }
        return window.io.pasteItems().then((flavors) => {
          const t = this._tokenFromFlavors(flavors);
          if (t) { this.tokenDraft = t; return; }
          this.tokenMsg = this._noTokenOnClipboard(flavors);
        }).catch((e) => {
          // A refused read is the ordinary case on iOS, not a fault: the field
          // is still there and the platform's own Paste still fills it.
          this.tokenMsg = 'Could not read the clipboard ('
            + ((e && e.message) || String(e)) + '). Paste into the field instead.';
        });
      },

      // Saving is where the shape is checked, whichever route filled the field.
      // gh-auth owns the write and the announcement; the reload is what
      // repo.js's own account control does, and for the same reason: a token
      // is an account boundary and half a page painted without one is not
      // worth reconciling.
      saveTokenDraft() {
        const t = String(this.tokenDraft || '').trim();
        if (!t) { this.tokenMsg = 'Nothing in the field yet'; return; }
        if (!this._looksLikeToken(t)) {
          this.tokenMsg = 'That is ' + t.length + ' characters and does not look like a '
            + 'GitHub token (ghp_, github_pat_, or 40 hex characters)';
          return;
        }
        window.ghAuth.save(t);
        this.tokenDraft = '';
        this.tokenCard = false;
        location.reload();
      },

      // GitHub's own prefixes, plus the 40-hex classic token. Refusing what
      // does not match is the point: a wrong string saved here fails later as
      // a 401 on some unrelated read, which is the silent failure this whole
      // section exists to end.
      _looksLikeToken(t) {
        return /^(gh[pousr]_|github_pat_)[A-Za-z0-9_]{10,}$/.test(t) || /^[a-f0-9]{40}$/.test(t);
      },

      // A GitHub token is one word, and the clipboard usually offers the same
      // string more than once: a copy from a web page arrives as text/plain and
      // text/html, and the second is the token wrapped in tags. So this takes
      // the first flavor that IS a token rather than the first flavor, asking
      // the same question saveTokenDraft asks, from the one place that answers
      // it. Two copies of that test would be two answers to give a reader.
      _tokenFromFlavors(flavors) {
        for (const f of (flavors || [])) {
          if (!f || f.kind !== 'text') continue;
          const t = String(f.text || '').trim();
          if (this._looksLikeToken(t)) return t;
        }
        return '';
      },

      // Say what WAS there, since "nothing looks like a token" on its own
      // cannot tell an empty clipboard from a token with a newline on it.
      _noTokenOnClipboard(flavors) {
        const texts = (flavors || []).filter(f => f && f.kind === 'text');
        if (!texts.length) return 'Nothing on the clipboard to read';
        const longest = texts.reduce((n, f) => Math.max(n, String(f.text || '').trim().length), 0);
        return 'Nothing on the clipboard looks like a GitHub token ('
          + texts.length + ' text flavor' + (texts.length === 1 ? '' : 's')
      ...
      openFabMenu() {
        ...
        this.annOn = !!(A && A.enabled);
        this.probeArmed = this.probeOn;
        this.probeHeld = this._probeRead((P) => !!P.held, false);
        this.probeCanFile = this._probeCanFile();
        this._warmPaste();
        this.fabMenu = true;
      ...
      onUp(e) {
        // The release of a secondary press, which onDown never armed. It is
        // dropped here rather than allowed to fall through, because on a
        // platform that raises the context menu on the RELEASE this event
        // arrives before onContextMenu has set the flag below.
        if (this._secondary(e)) { this.down = false; this.dragged = false; return; }
      ...
      // CRASH BREADCRUMB. A web-process crash is not a JS exception: no handler
      // runs, nothing on the page can report it, and the tab is replaced by
      // Safari's "A problem repeatedly occurred". The only channel that outlives
      // the process is storage, so each stage of a tap writes its name before it
      // runs and the whole trail is dropped when the tap completes. A key left
      // behind names the stage that did not return, and the page reads it on the
      // NEXT load (pages/audit-render.html, paintFaults). Every call is wrapped:
      // an instrument that can throw is a second defect (snag
      // diagnostic-took-the-page-down), and a browser refusing storage (private
      // mode, quota) must cost nothing.
      //
      // A TRAIL, NOT A MARK. One overwritten key names the last stage entered
      // and nothing else, and the reported failure is a freeze of a couple of
      // seconds before the process dies: what that shape needs is where the
      // time went and where the memory went, which only a sequence shows. Each
      // stage records its own elapsed reading and the JS heap where the browser
      // offers one (Safari does not, so the field is often null and the timings
      // carry the diagnosis alone).
      // THE TRAIL FILES ITSELF, FROM ANY PAGE. The crash report lived in
      // pages/audit-render.html, which meant every test had to be run on that
      // one page: the moment a control needed a different page or a different
      // delivery, the answer came back as a sentence in chat instead of a file.
      // The trail belongs to the fab, so the reporting does too. A tap that
      // never finished is found on the next load here, filed with whatever the
      // address says the case is, and cleared.
      //
      // It files ONLY when there is an unfinished tap to report, which is rare
      // and always interesting, so an ordinary reader of an ordinary page
      // writes nothing. kits/page-report carries the rest of the gates.
      async _reportUnfinished() {
        let raw = null;
        try { raw = localStorage.getItem('fab:step'); } catch { }
        if (!raw) return;
        try { localStorage.removeItem('fab:step'); } catch { }
        let trail = null;
        try { trail = JSON.parse(raw); } catch { }
        window.__fabCrumb = trail || raw;
        try {
          if (!window.PageReport) await this._selfLoad('kits/page-report.js', () => !!window.PageReport);
          if (!window.PageReport) return;
          window.PageReport.watch();
          const q = new URLSearchParams(location.search);
          await window.PageReport.send({
            // The case tag is the whole point of the matrix: a link carries
            // ?case=<name> and the filed report says which cell it came from,
            // so several tests can be run in any order without anyone having
            // to remember or relay which was which.
            // From the trail, not from this page: see the stamp in _crumb.
            case: trail?.where?.case || null,
            where: trail?.where || null,
            filedOn: q.get('case') || null,
            reason: 'fab tap never finished',
            crumb: trail ? { stage: trail.steps?.at(-1)?.stage, steps: trail.steps, at: trail.at } : { raw },
            // The two delivery variables that decide what the fab is even
            // running: which lib ref, and whether the page came through a toss.
            use: trail?.where?.use || null,
            viaToss: !!window.__tossSubject || !!window.__tossFrame || !!window.__fabHosted,
            hosted: !!window.__fabHosted,
            subject: (() => { try { return window.__tossSubject || null; } catch { return null; } })(),
            fabRef: this.ref || null,
            offRef: !!this.offRef,
            width: (() => { try { return { w: innerWidth, h: innerHeight, dpr: devicePixelRatio }; } catch { return null; } })(),
          });
        } catch (e) { }
      },

      // `soft` marks a write that must never START a trail, only join one that
      // is open. The async completions are soft: they land whenever the network
      // does, and a late one arriving after the trail was cleared used to open a
      // fresh trail holding one entry, which then reported itself on the next
      // load as a tap that never finished. The trail belongs to the key, so
      // anything that can arrive late has to say it is a passenger.
      // DETAIL IS OPT-IN. The coarse trail (a handful of marks) stays on for
      // everyone, because a crash in the wild is exactly what nobody is set up
      // to catch. The per-node marks along the drawer's build are a dozen more
      // synchronous localStorage writes per open, which is not a price to charge
      // a reader who is not debugging anything. `?crumb=1` in the address, or
      // the `fab:crumb` key, turns them on.
      get _crumbDetail() {
        try {
          if (/[?&]crumb=1\b/.test(location.search)) return true;
          return localStorage.getItem('fab:crumb') === '1';
        } catch { return false; }
      },
      _crumbD(stage, soft) { if (this._crumbDetail) this._crumb(stage, soft); },

      _crumb(stage, soft) {
        try {
          const t = Date.now();
          let trail = JSON.parse(localStorage.getItem('fab:step') || 'null');
          // A STALE TRAIL IS NOT THIS TAP'S. Appending to whatever was in
          // storage merged a crashed session's trail with a fresh tap: one
          // filed report carried two taps and a stage from a build that had
          // already been withdrawn, spanning 42 seconds. Ten seconds is well
          // past any tap and well short of a reader coming back to the page.
          if (trail && !(t - (trail.at || trail.start || 0) < 10000)) trail = null;
          if (!trail && soft) return;
          // WHERE THE TAP HAPPENED, STAMPED AT TAP TIME. The report used to read
          // the case tag and the address off the page that FILED it, which is
          // the next page opened, not the one that crashed. Running a matrix in
          // order therefore shifted every result by one cell: tap A, crash,
          // open B, and B's fab files A's trail wearing B's tag. Six cells came
          // back looking identical because they were, in the sense that each
          // was the previous cell's crash. Found 2026-09-08 by a cell the
          // reader had confirmed WORKS filing a trail that stopped dead.
          trail = trail || { start: t, steps: [], where: (() => {
            try {
              const q = new URLSearchParams(location.search);
              return { case: q.get('case') || null, use: q.get('use') || null,
                       lp: q.get('lp') || null, url: location.href.slice(0, 300),
                       frame: !!document.getElementById('frame'),
                       subject: !!window.__tossSubject, hosted: !!window.__fabHosted };
            } catch { return null; }
          })() };
          const heap = (() => { try { return performance.memory
            ? Math.round(performance.memory.usedJSHeapSize / 1048576) : null; } catch { return null; } })();
          trail.steps.push({ stage, ms: t - trail.start, heap });
          trail.at = t;
          // Capped, because a stage reached in a loop must not grow the key
          // without bound on a device that is already short of memory.
          if (trail.steps.length > 24) trail.steps.splice(0, trail.steps.length - 24);
          localStorage.setItem('fab:step', JSON.stringify(trail));
        } catch {}
      },
      _crumbDone() {
        try { localStorage.removeItem('fab:step'); } catch {}
      ...
      toggle() {
        this._crumb('toggle:enter');
        if (this.open) { this.close(); this._crumbDone(); return; }
        this._crumb('detect');
        this.detect();
        this._crumb('refreshLayers');
        this.refreshLayers();
        this._crumb('open');
        this.open = true;
        ...
        // THE ASYNC THREE, AND WHEN THEY SETTLE. Their start crumbs said only
        // that they were kicked off, all three inside six milliseconds, which
        // is why they read as innocent. What they actually do happens later,
        // in the same one-to-two-second window where the device freezes: a
        // module load, two API reads, a markdown render. So each one now marks
        // its own completion, and a trail that carries one of those marks
        // before it stops puts the death after that work rather than in the
        // paint.
        const settles = (name, p) => { try { Promise.resolve(p)
          .then(() => this._crumbD(name + ':ok', true), () => this._crumbD(name + ':err', true)); } catch { } };
        this._crumb('ensureBrief');
        settles('ensureBrief', this.ensureBrief());
        this._crumb('loadVersion');
        settles('loadVersion', this.loadVersion());
        ...
        this._crumb('loadGuide');
        if (this.activeTab === 'render') settles('loadGuide', this.loadGuide());

        // PAST HERE IS NOT THIS COMPONENT'S WORK, WHICH IS THE POINT. The first
        // trail off a device (2026-09-08) put every stage above inside ten
        // milliseconds, then died: the freeze a reader sees for a couple of
        // seconds is not in any of them, so the interesting question is what
        // the browser does with the drawer after the state flips. Splitting the
        // old single `paint` crumb into three separates the answers that were
        // sharing one name.
        //
        //   dom:<n>   Alpine has finished building the drawer, and n is how
        //             many elements the document holds now. Missing means the
        //             build itself did not survive.
        //   frame1    the browser laid out and painted it once.
        //   (cleared) it painted twice, so the drawer is up.
        this._crumb('paint');
        // TIMERS, NOT $nextTick AND NOT requestAnimationFrame. Both were used
        // here and NEITHER EVER FIRED ON THE DEVICE: a filed trail from a tap
        // that opened the drawer perfectly well stops at d:end exactly like one
        // from a tap that killed the tab. So the absence of the frame marks was
        // never evidence of anything, and two rounds of "the browser died
        // drawing" rested on it. Measured 2026-09-08 by running a control that
        // was known to work and getting the same trail back.
        //
        // A setTimeout fires on a timer the page cannot starve by not painting,
        // so these three say something the earlier pair could not: t0 means the
        // task after the open ran at all, t300 means the tab was alive 300ms
        // later, and a cleared key means it survived a second and a half.
        setTimeout(() => this._crumb('t0:' + (() => { try { return document.querySelectorAll('*').length; } catch { return '?'; } })()), 0);
        setTimeout(() => this._crumb('t300'), 300);
        setTimeout(() => this._crumbDone(), 1500);
      },
      ...
      captureData() {
        ...
        // The retained set IS the take's set now: the drawer caps at
        // CONSOLE_CAP, so this slice can no longer be the thing that drops
        // anything, and the count of what went comes off the cap instead.
        const logs = this.consoleLogs.slice(-this.CONSOLE_CAP);
        ...
        return {
          ...
          ...(this.consoleDropped ? { consoleDropped: this.consoleDropped } : {}),
        };
      },
      ...
      // The dictation page in file mode, on the file this drawer describes, at
      // the ref on display. Beside showRepoBase rather than hardcoded, so a page
      // that points the drawer at another deployment points this there too.
      get editUrl() {
        if (!this.repo || !this.path || this.subjectLocal || !/\.(md|markdown)$/i.test(this.path)) return '';
        let page;
        try { page = new URL('../pages/dictate.html', this.showRepoBase).href; } catch { return ''; }
        return page + '?file=' + encodeURIComponent(this.repo + '@' + this.viewingRef + ':' + this.path);
      },
      ...
      readLayers(root) {
        ...
        for (let depth = 0; depth < 6; depth++) {
          ...
          if (named) {
            ...
            let hash = '';
            try { hash = String(loc.hash || ''); } catch (e) {}
            out.push({ repo: named.repo, path: named.path, ref: use || 'main', hash });
          } else {
        ...
        this._typeLayers(out, root || window);
        return out;
      },

      // THE TYPE IS THE SECOND READING OF THE SAME WALK. A carrier (the
      // renderer, or the app framing a promoted page) has no type of its own,
      // which is what leaves the innermost row as the one the reader chose.
      // Applies docs/subjects.csv's detect column to each row's address;
      // nothing here is remembered from an announcement.
      _typeLayers(out, top) {
        let search = '', topHash = '';
        try { search = String(top.location.search || ''); topHash = String(top.location.hash || ''); } catch (e) {}
        let q; try { q = new URLSearchParams(search); } catch (e) { q = new URLSearchParams(''); }
        let shell = null; try { shell = top.__shell || null; } catch (e) {}
        out.forEach((L, i) => {
          L.type = ''; L.typeLabel = '';
          if (L.sealed || L.role === 'renderer') return;
          const path = L.path || '';
          const hash = i === 0 ? topHash : (L.hash || '');
          if (/(^|\/)branch\.html$/.test(path)) {
            L.type = 'branch'; L.typeLabel = this._briefLabel(hash, 'branch'); return;
          }
          if (/(^|\/)session\.html$/.test(path)) {
            L.type = 'session'; L.typeLabel = this._briefLabel(hash, 'session'); return;
          }
          const isShell = i === 0 && (!!shell || /(^|\/)app\/index\.html$/.test(path));
          if (isShell) {
            // Framing a promoted page, the app is the carrier and the page
            // below it is the subject; the row keeps its role and no type.
            if (q.has('app') || q.get('view') === 'app') return;
            const view = q.get('view') || (q.has('repo') ? 'landing' : '');
            const stop = this._viewStop(shell, view);
            if (view === 'project' || q.has('project')) {
              const proj = (q.get('project') || '').split('/').filter(Boolean).pop() || '';
              L.type = 'project';
              L.typeLabel = [q.get('repo') || L.repo, proj].filter(Boolean).join(' / ') + (stop ? ' › ' + stop : '');
              return;
            }
            const repoScoped = view === 'public'
              || (q.has('repo') && (shell ? !shell.estateCtx : REPO_VIEWS.includes(view)));
            if (repoScoped) {
              L.type = 'repo';
              L.typeLabel = (q.get('repo') || L.repo || '') + (stop && stop !== 'Overview' ? ' › ' + stop : '');
              return;
            }
            L.type = 'estate'; L.typeLabel = stop; return;
          }
          // Content: the coarse split by extension. routes-kinds refines it one
          // level down, in the viewer that opens the file.
          const ext = (path.split('.').pop() || '').toLowerCase();
          if (L.route === 'chat-results') L.type = 'fileset';
          else if (ext === 'html') L.type = 'page';
          else if (ext === 'json' || ext === 'csv' || ext === 'tsv') L.type = 'data';
          else L.type = 'file';
        });
      },

      // What the brief is about, read off its own address: the branch a
      // branch.html carries (#gh=owner/repo@branch, or &pr=N), the id a
      // session.html carries (#id=, or the store path under #gh=).
      _briefLabel(hash, kind) {
        let h; try { h = new URLSearchParams(String(hash || '').replace(/^#/, '')); } catch (e) { return ''; }
        if (kind === 'branch') {
          if (h.get('pr')) return 'PR #' + h.get('pr');
          const gh = h.get('gh') || '';
          return gh.includes('@') ? gh.slice(gh.indexOf('@') + 1) : '';
        }
        if (h.get('id')) return h.get('id');
        const gh = h.get('gh') || '';
        return gh.split(':').pop().split('/').pop() || '';
      },

      // The view's reader-facing name. The shell's own nav knows it (Activity
      // for sessions, Files for search); a page with no shell on the window
      // falls back to the key, which is what a test sees.
      _viewStop(shell, view) {
        if (!view) return '';
        const pretty = k => k === 'landing' ? 'Overview' : k.charAt(0).toUpperCase() + k.slice(1);
        let nav = null;
        try { nav = (shell?.estateNav || []).find(n => n.view === view || (n.views || []).includes(view)) || null; } catch (e) {}
        if (!nav) return pretty(view);
        return nav.view === view ? nav.label : nav.label + ' › ' + pretty(view);
      },

      // The caption a row wears: its type where it has one, else its place in
      // the stack. One rule for the strip's rows and its one-line label, so a
      // reader meets the same word in every context.
      layerCaption(L) {
        if (!L) return '';
        return L.type || L.role || '';
      },
      // The name slot: what the subject is called, else the file. A typed
      // shell row says which view it is on; a brief says which branch.
      layerLabel(L) {
        if (!L) return '';
        if (L.sealed) return 'sealed';
        if (L.typeLabel && (L.type === 'estate' || L.type === 'repo' || L.type === 'project')) return L.typeLabel;
        const name = this.layerName(L);
        return L.typeLabel ? name + ' · ' + L.typeLabel : name;
      },
      // The selected layer, typed, for the Render tab's identity block.
      get subjectHead() {
        const L = this.layers[this.layerIndex];
        if (!L || L.sealed || !L.type) return '';
        const t = L.type.charAt(0).toUpperCase() + L.type.slice(1);
        return L.typeLabel ? t + ' · ' + L.typeLabel : t;
      },
      get subjectIcon() {
        const L = this.layers[this.layerIndex];
        return (L && SUBJECT_ICON[L.type]) || '';
      },
      // Whether the selected layer is the app itself, where the file picker
      // is a way OUT rather than a way to another view.
      get subjectIsApp() {
        const L = this.layers[this.layerIndex];
        return !!L && (L.type === 'estate' || L.type === 'repo' || L.type === 'project');
      },
      ...
      layerIcon(L) {
        ...
        if (L.type && SUBJECT_ICON[L.type]) return SUBJECT_ICON[L.type] + ' opacity-50';
      ...
      branchAssistant(b){
        return window.assistantMark?.classify?.({ ...b, repo: b?.repo || this.repo }) || 'unknown';
      },
      branchAssistantMark(b){
        const a = this.branchAssistant(b);
        return window.assistantMark?.svg?.(a, { cls: 'w-3.5 h-3.5' }) || '';
      },
      branchAssistantTitle(b){
        const a = this.branchAssistant(b);
        const label = window.assistantMark?.label?.(a) || a;
        const url = this.branchSessionUrl(b);
        if (url) return 'Open the ' + label + ' session that authored this branch';
        return label + ' branch';
      },
      branchSessionUrl(b){
        const a = this.branchAssistant(b);
        return a === 'claude' ? (b?.session || '') : '';
      },
      ...
      async ensureBrief() {
        if (this.briefReady) return;
        if (this._briefWait) return this._briefWait;
        this.briefLoading = true;
        this._briefWait = (async () => {
          try {
          ...
          finally { this.briefLoading = false; this._briefWait = null; }
        })();
        return this._briefWait;
      },
      ...
      get takeGroups() {
        ...
        return [
          ...
          { kind: 'Image', items: [
            { key: 'shot-view', icon: 'ph-crop', label: 'View',
              desc: 'Preview the visible part of this page as a PNG reconstructed from the DOM. '
                  + 'It may differ where CSS or cross-origin media cannot be reproduced; download it from the preview.' },
            { key: 'shot-page', icon: 'ph-file-image', label: 'Page',
              desc: 'Preview the full scrolling page as a PNG reconstructed from the DOM. '
                  + 'Very long pages may be downscaled to fit the browser canvas; download it from the preview.' },
            { key: 'shot-region', icon: 'ph-selection-background', label: 'Region',
              desc: 'Pick an element, step to the ancestor you mean, then preview the renderer’s PNG of it.'
                  + (this.annBlind ? ' Subject frame unreadable: the pick lands on this shell.' : '') },
          ] },
          { kind: 'Save', items: [
      ...
      get takeGrid() {
        const order = ['Copy', 'Open', 'Image', 'Save', 'Page'];
      ...
      get takeDeckItems() {
        // A deck is a reader for concrete outputs. Region pickers and page
        // actions start another interaction on the page; representing either
        // as an empty slide with a button confuses navigation with showing.
        return this.takeGrid
          .flatMap(g => g.items.map(a => ({ ...a, group: g.kind })))
          .filter(a => a.kind !== 'page' && a.key !== 'region' && a.key !== 'shot-region');
      },

      _takeHtmlEstimate() {
        const t = this.takeTarget;
        if (t.payload) return String(t.payload).length;
        const p = this.takePlan;
        if (!p) return Infinity;
        let bytes = Number(p.bytes) || 0;
        try { bytes += this._annDoc().documentElement?.outerHTML?.length || 0; } catch (e) {}
        let files = t.files;
        try { files ||= this._annDoc().defaultView?.__ghFiles; } catch (e) {}
        files ||= window.__ghFiles || {};
        for (const path of p.reads || []) bytes += Number(files[path]?.bytes) || 0;
        return bytes;
      },

      _takeActionLabel(a) {
        if (a.kind === 'page') return a.label;
        if (a.key === 'region') return 'Pick region';
        if (a.key === 'shot-region') return 'Pick region';
        return a.label;
      },

      _takeLoadLabel(a) {
        if (a.key === 'render') return 'Load HTML';
        if (a.key === 'brief') return 'Load LLM brief';
        if (a.key === 'stage') return 'Load manifest';
        if (a.key === 'shot-view') return 'Render visible view';
        if (a.key === 'shot-page') return 'Render full page';
        if (a.key === 'export') return 'Load Zip contents';
        if (a.key === 'offline') return 'Load Zip + code contents';
        return this._takeActionLabel(a);
      },

      async openTakeDeck(start = 0) {
        // The drawer starts this read in the background, but a fast tap can
        // reach the deck first (and callers may open it directly). The plan is
        // what makes the small-HTML decision bounded; if it cannot load,
        // _takeHtmlEstimate() returns Infinity and preserves the Load button.
        if (!this.briefReady) await this.ensureBrief();
        await this._ensureShotDeck();
        const items = this.takeDeckItems;
        if (!items.length) return null;
        const startAt = Math.max(0, Math.min(items.length - 1, start));
        const states = items.map(() => ({ status: 'idle', value: null, error: '' }));
        const urls = new Set();
        let handle = null;
        let afterClose = null;
        ...
        const stageText = plan => {
          const files = [plan.path, ...plan.own.map(m => 'lib/' + m.path)];
          const lines = [plan.repo + ' @ ' + plan.ref, '', 'Files', ...files.map(path => '  ' + path)];
          if (plan.reads.length) lines.push('', 'Runtime data', ...plan.reads.map(path => '  ' + path));
          return lines.join('\n');
        };
        ...
        const archiveText = value => [value.filename, '', 'Files',
          ...value.files.map(file => '  ' + file.path + '  ' + byteSize(file.data).toLocaleString() + ' bytes')].join('\n');
        const asText = (text, meta, type = 'text') => ({ type, text: String(text), meta });
        const seed = (i, value) => { states[i] = { status: 'loaded', value, error: '' }; };

        items.forEach((a, i) => {
          if (a.key === 'capture') {
            const text = JSON.stringify(this.captureData(), null, 2);
            seed(i, asText(text, Math.max(1, Math.round(text.length / 1024)) + 'K JSON'));
          } else if (a.key === 'stage' && this.takePlan) {
            const plan = this.takePlan;
            seed(i, { type: 'stage', text: stageText(plan), plan,
                      meta: (1 + plan.own.length) + ' files' });
          } else if (a.key === 'render' && this.takeTarget.payload) {
            const text = String(this.takeTarget.payload);
            seed(i, asText(text, Math.max(1, Math.round(text.length / 1024)) + 'K HTML', 'html'));
          }
        });

        const ensureIo = async method => {
          if (!window.io?.[method]) await this._selfLoad('kits/io.js', () => !!window.io?.[method]);
          if (!window.io?.[method]) throw new Error('io.' + method + ' unavailable (kits/io.js failed to load)');
        };
        const ensureBriefKit = async () => {
          if (!window.brief) await this._selfLoad('kits/brief.js', () => !!window.brief);
          this.briefReady = !!window.brief;
          if (!window.brief) throw new Error('LLM brief unavailable (kits/brief.js failed to load)');
        };
        const markAction = async (button, icon, work) => {
          if (button) button.disabled = true;
          const mark = button?.querySelector('i');
          const before = mark?.className;
          if (mark) mark.className = 'ph ph-spinner-gap animate-spin';
          try {
            await work();
            if (mark) mark.className = 'ph ph-check';
            setTimeout(() => { if (mark?.isConnected) mark.className = before || ('ph ' + icon); }, 1200);
          } catch (e) {
            if (mark) mark.className = 'ph ph-warning';
            if (button) button.title = (e && e.message) || String(e);
          } finally { if (button) button.disabled = false; }
        };
        const actionsFor = (a, state) => {
          if (state.status !== 'loaded' || !state.value) return [];
          const value = state.value;
          if (value.type === 'text' || value.type === 'html') return [{
            icon: 'ph-copy', title: a.key === 'capture' ? 'Copy capture' : a.key === 'brief' ? 'Copy LLM brief' : 'Copy HTML',
            onClick: (_deck, button) => markAction(button, 'ph-copy', async () => {
              await ensureIo('copy');
              await window.io.copy(value.text);
            }),
          }];
          if (value.type === 'stage') return [{
            icon: 'ph-arrow-square-out', title: 'Open Stage',
            onClick: (_deck, button) => markAction(button, 'ph-arrow-square-out', () => this.openStage()),
          }];
          if (value.type === 'image') return [{
            icon: 'ph-download-simple', title: 'Download PNG',
            onClick: (_deck, button) => markAction(button, 'ph-download-simple', async () => {
              window.DomShot.download(value.result.blob, value.result.filename, value.doc);
            }),
          }];
          if (value.type === 'archive') return [{
            icon: 'ph-download-simple', title: 'Download ' + value.build.filename,
            onClick: (_deck, button) => markAction(button, 'ph-download-simple', async () => {
              await ensureIo('saveZip');
              await window.io.saveZip(value.build.files, value.build.filename);
            }),
          }];
          return [];
        };
        const syncHeader = i => {
          ...
          handle.setTitle(a.label);
          handle.setSubtitle((state.value?.meta || a.group) + ' \u00b7 ' + this.takeSubject);
          handle.setIcon(a.icon || 'ph-lightning');
          handle.setActions(actionsFor(a, state));
        };

        const obtain = async a => {
          const t = this.takeTarget;
          if (a.key === 'render') {
            await this._ensureExporter();
            const result = t.payload
              ? { html: t.payload, bytes: t.payload.length }
              : await window.exporter.renderCopy({ path: t.path, gh: t.gh, scripts: t.scripts, reads: t.reads });
            return asText(result.html, Math.max(1, Math.round(result.bytes / 1024)) + 'K HTML', 'html');
          }
          if (a.key === 'brief') {
            await ensureBriefKit();
            const result = await window.brief.assemble({ path: t.path, gh: t.gh, scripts: t.scripts,
                                                         reads: t.reads, files: t.files });
            return asText(result.text, '~' + result.tokens.toLocaleString() + ' tokens');
          }
          if (a.key === 'stage') {
            await ensureBriefKit();
            const plan = this.takePlan;
            if (!plan) throw new Error('Stage manifest unavailable');
            return { type: 'stage', text: stageText(plan), plan,
                     meta: (1 + plan.own.length) + ' files' };
          }
          if (a.key === 'shot-view' || a.key === 'shot-page') {
            await this._ensureDomShot();
            const doc = this._annDoc();
            const target = doc.documentElement;
            const result = await window.DomShot.capture(target, { mode: a.key === 'shot-page' ? 'page' : 'viewport' });
            result.filename = window.DomShot.filename(target, result.mode);
            const objectUrl = URL.createObjectURL(result.blob);
            urls.add(objectUrl);
            return { type: 'image', result, objectUrl, doc,
                     meta: result.width.toLocaleString() + '\u00d7' + result.height.toLocaleString() + ' PNG' };
          }
          if (a.key === 'export' || a.key === 'offline') {
            await this._ensureExporter();
            const build = await window.exporter.build({ path: t.path, gh: t.gh, scripts: t.scripts,
                                                        reads: t.reads, offline: a.key === 'offline' });
            return { type: 'archive', build, text: archiveText(build),
                     meta: build.files.length + ' files' };
          }
          return null;
        };

        const redraw = (i, slide) => {
          // A far slide can be pruned while its load is in flight. Its empty
          // node is rebuilt from cached state when the reader comes back.
          if (slide.isConnected && slide.childNodes.length) draw(i, slide);
        };
        const load = async (i, slide) => {
          if (states[i].status === 'loading') return;
          states[i] = { status: 'loading', value: null, error: '' };
          draw(i, slide);
          try {
            const value = await obtain(items[i]);
            states[i] = { status: 'loaded', value, error: '' };
          } catch (e) {
            states[i] = { status: 'error', value: null, error: (e && e.message) || String(e) };
          }
          redraw(i, slide);
          if (handle?.deck.active() === i) syncHeader(i);
        };
        ...
        const draw = (i, slide) => {
          ...
          if (state.status === 'loaded') {
            const value = state.value;
            if (value.type === 'image') {
              slide.append(h('div', { class: 'flex h-full w-full items-start justify-center overflow-auto bg-base-200' },
                h('img', { class: 'block h-auto max-w-none bg-white', src: value.objectUrl,
                           alt: a.label + ' DOM render' })));
            } else {
              slide.append(h('pre', { class: 'h-full w-full overflow-auto whitespace-pre-wrap break-words bg-base-100 p-4 font-mono text-xs leading-relaxed text-base-content sm:p-6' }, value.text));
            }
            return;
          }
          if (state.status === 'error') {
            const retry = h('button', { class: 'btn btn-primary gap-2' }, h('i', { class: 'ph ph-arrow-clockwise' }), 'Try again');
            retry.addEventListener('click', () => load(i, slide));
            slide.append(h('div', { class: 'flex h-full w-full flex-col items-center justify-center gap-4 p-6 text-center' },
              h('p', { class: 'max-w-lg text-sm text-error', role: 'alert' }, state.error), retry));
            return;
          }
          const leavesDeck = a.kind === 'page' || a.key === 'region' || a.key === 'shot-region';
          const button = h('button', { class: 'btn btn-primary gap-2' },
            h('i', { class: 'ph ' + (state.status === 'loading' ? 'ph-spinner-gap animate-spin' : (a.icon || 'ph-lightning')) }),
            state.status === 'loading' ? 'Loading' : leavesDeck ? this._takeActionLabel(a) : this._takeLoadLabel(a));
          button.disabled = state.status === 'loading';
          button.addEventListener('click', () => {
            if (leavesDeck) {
              afterClose = () => this.runGridItem(a);
              handle.close();
            } else load(i, slide);
          });
          slide.append(h('div', { class: 'flex h-full w-full items-center justify-center p-6' }, button));
        };
        const render = (i, slide) => {
          draw(i, slide);
          // HTML is the first slide and, for ordinary pages, already small
          // enough to be the answer. Only ask for another tap when the known
          // closure is large; other costly outputs remain explicitly loaded.
          if (items[i].key === 'render' && states[i].status === 'idle'
              && this._takeHtmlEstimate() <= TAKE_HTML_AUTO_BYTES) void load(i, slide);
        };
        handle = window.swipeDeck.open({
          count: items.length,
          start: startAt,
          title: items[startAt].label,
          subtitle: items[startAt].group + ' \u00b7 ' + this.takeSubject,
          icon: items[startAt].icon || 'ph-lightning',
          index: i => ({ title: items[i].label, subtitle: items[i].desc,
                         icon: items[i].icon, group: items[i].group }),
          render,
          onSlide: syncHeader,
          slideScroll: false,
        ...
        // open() reports slide zero while constructing its handle. Apply the
        // requested start once the handle exists; later swipes use onSlide.
        syncHeader(startAt);
        // The catalog is tool chrome. A screenshot generated from one of its
        // slides must render the page beneath it, not the catalog itself.
        handle.el?.setAttribute('data-dom-shot-ignore', '');
        this.open = false;
        return handle;
      },
      ...
      async runTake(key) {
        ...
        try {
          ...
          else if (key === 'shot-view') await this.previewDomShot('viewport');
          else if (key === 'shot-page') await this.previewDomShot('page');
          else if (key === 'shot-region') await this.openShotPicker();
      ...
      async openPeek(o = {}) {
        ...
        window.Peek.enable({ doc, view: o.view || 'facts', onTake: o.onTake, takeLabel: o.takeLabel });
        this.open = false;
      ...
      async _ensureDomShot() {
        if (!window.DomShot) await this._selfLoad('kits/dom-shot.js', () => !!window.DomShot);
        if (!window.DomShot) throw new Error('DOM screenshot kit unavailable (kits/dom-shot.js failed to load)');
      },

      async _ensureShotDeck() {
        if (!window.swipeDeck) await this._selfLoad('kits/swipe-deck.js', () => !!window.swipeDeck);
        if (!window.swipeDeck) throw new Error('Image preview unavailable (kits/swipe-deck.js failed to load)');
      },

      _shotResult(r) {
        const size = r.width.toLocaleString() + '\u00d7' + r.height.toLocaleString();
        return 'Previewing ' + r.filename + ' (' + size + ' DOM render'
          + (r.warnings.length ? '; ' + r.warnings.join('; ') : '') + ')';
      },

      _openDomShotPreview(r, parent = null) {
        const labels = { viewport: 'Visible view', page: 'Full page', element: 'Selected region' };
        const objectUrl = window.URL.createObjectURL(r.blob);
        let released = false;
        const release = () => {
          if (released) return;
          released = true;
        ...
        const options = {
          count: 1,
          title: labels[r.mode] || 'DOM render',
          subtitle: r.width.toLocaleString() + '\u00d7' + r.height.toLocaleString() + ' PNG',
          icon: 'ph-image',
          innerClass: 'mx-auto max-w-5xl space-y-4',
          actions: [{
            icon: 'ph-download-simple',
            title: 'Download PNG',
            onClick: () => {
              window.DomShot.download(r.blob, r.filename, document);
              this.outMsg = 'Saved ' + r.filename;
            },
          }],
          render: (_i, slide) => {
            const note = document.createElement('div');
            note.className = 'rounded-box border border-base-300 bg-base-200/60 px-3 py-2 text-xs text-base-content/70';
            note.textContent = 'Reconstructed from the DOM by ' + r.renderer + '.'
              + (r.warnings.length ? ' ' + r.warnings.join('; ') + '.' : '');
            const image = document.createElement('img');
            image.src = objectUrl;
            image.alt = (labels[r.mode] || 'Page') + ' DOM render';
            image.className = 'block h-auto max-w-full rounded-box border border-base-300 bg-white shadow-sm';
            const frame = document.createElement('figure');
            frame.className = 'flex w-full justify-center';
            frame.append(image);
            slide.append(note, frame);
          },
          release,
          onClose: release,
        };
        const deck = parent
          ? window.swipeDeck.drill(parent, options)
          : window.swipeDeck.open(options);
        // The FAB remains reachable over an open deck. If it starts another
        // shot, the preview is tooling around the page, not part of the page.
        deck.el?.setAttribute('data-dom-shot-ignore', '');
        this.open = false;
        return deck;
      },

      async previewDomShot(mode, node, parent = null) {
        await Promise.all([this._ensureDomShot(), this._ensureShotDeck()]);
        const doc = node?.ownerDocument || this._annDoc();
        const target = node || doc.documentElement;
        const r = await window.DomShot.capture(target, { mode });
        r.filename = window.DomShot.filename(target, r.mode);
        this._openDomShotPreview(r, parent);
        this.outMsg = this._shotResult(r);
        return r;
      },

      async openShotPicker() {
        await this._ensureDomShot();
        await this.openPeek({
          view: 'facts',
          takeLabel: 'Preview PNG',
          onTake: async el => {
            const r = await this.previewDomShot('element', el);
            // Peek's fixed panel sits above the house takeover. Once it has
            // supplied the element, put the picker away so the image is the
            // surface the reader sees.
            if (window.Peek?.enabled) window.Peek.disable();
            return 'Previewing ' + r.width.toLocaleString() + '\u00d7' + r.height.toLocaleString();
          },
        });
        this.outMsg = 'Region PNG: tap an element, step to the ancestor you mean, then Preview PNG';
      },

      // ── The probe, standing equipment rather than an address ──────────────
      //
      // kits/probe.js draws a page's live conditions and a rolling trace of
      // every popup guard decision, and until now `?probe=` on the address was
      // the only way in: a reader already looking at the fault had to retype
      // the URL to instrument it, on the phone where that class of fault
      // lives. The precedent is this file's own crash trail, which left
      // pages/audit-render.html for exactly this reason (see
      // _reportUnfinished): a diagnostic that lives on one page can only be
      // run on one page.
      //
      // IT IS ARMED IN THE WATCHED REALM, not in this one, and that is the one
      // way it differs from openPeek() below. Peek can run here and paint into
      // the subject frame's document because it only ever touches the DOM; the
      // probe listens for window events and reads `window.__cardProbe`, and
      // both of those belong to the frame's own realm. So the load goes
      // through the subject's own gh chain, which makes this the same code
      // path the address takes rather than a second one to keep in step.
      //
      // The one view it cannot reach is a framed page with no loader of its
      // own, and it says so rather than appearing to have worked: there is
      // nothing to hand the file to, and the address still is.
      // The window `_annDoc()` reads the document of, and READ THE SAME WAY:
      // by asking the frame, not by reading `subjectReached`. That flag is set
      // by the drawer's own scan, so a path that never opens the drawer, which
      // is every path through the launcher menu, would find it false and arm
      // the probe on the shell while the reader watched the page. Measured
      // 2026-09-10 in a real browser, which is the only place the two differ.
      _probeWin() {
        if (this.viaToss && window.__tossFrame) {
          try {
            const w = window.__tossFrame.contentWindow;
      ...
      get probeOn() {
        try { const w = this._probeWin(); return !!(w && w.Probe && w.Probe.on); }
        catch (e) { return false; }
      },
      // One guarded read of the probe in the watched realm, since every caller
      // here wants the same try/catch around a cross-origin frame.
      _probeRead(fn, dflt) {
        try {
          const w = this._probeWin();
          const P = w && w.Probe;
          return (P && P.on) ? fn(P) : dflt;
        } catch (e) { return dflt; }
      },

      // Whether a filed capture can actually be written, asked of the store
      // rather than of gh: kits/page-report.js carries the note that the token
      // is not on the gh instance after construction, because gh-auth.js patches
      // the `headers` getter and only pulls it in when a request is about to go
      // out. localStorage is what that getter reads, and the shell and a blob
      // frame share this origin, so either realm answers the same.
      _probeCanFile() {
        try { return !!localStorage.getItem('ghToken'); } catch (e) { return false; }
      },

      // The verbs on the probe row. Each one exists because a phone cannot
      // press the chord that used to be the only way to it.
      async probeVerb(verb) {
        const w = this._probeWin();
        let P = null;
        try { P = w && w.Probe; } catch (e) {}
        if (!P || !P.on) { this.probeArmed = false; this.outMsg = 'No probe on this view'; return; }

        if (verb === 'hold') {
          let held = false;
          try { held = !!P.hold(); } catch (e) {}
          this.probeHeld = held;
          this.outMsg = held ? 'Trace held: it will not move while you read it' : 'Trace running again';
          return;
        }
        if (verb === 'move') {
          try { this.outMsg = 'Overlay moved to the ' + P.move(); } catch (e) {}
          return;
        }
        if (verb !== 'send') return;

        // FILE, OR SHARE, DECIDED BEFORE THE AWAIT. With a token the capture
        // goes where a session can read it without the reader relaying
        // anything, which is the whole point of filing. Without one there is
        // nothing to write with, and the honest answer on a phone is the share
        // sheet: the same JSON, handed to whatever the reader wants to put it
        // in. The clipboard is the last resort, for a browser with no share.
        if (this.probeCanFile) {
          this.outMsg = 'Filing the capture…';
          let res = null;
          try { res = await P.file('probe:fab row'); } catch (e) { res = { ok: false, why: String(e && e.message || e) }; }
          this.outMsg = (res && res.ok)
            ? 'Filed ' + String(res.path || '').split('/').pop()
            : 'Not filed: ' + ((res && res.why) || 'unknown');
          return;
        }
        let cap = null;
        try { cap = P.state(); } catch (e) {}
        if (!cap) { this.outMsg = 'Nothing to send'; return; }
        const text = JSON.stringify(cap, null, 2);
        const name = 'probe-' + (cap.probeId || 'capture') + '.json';
        // SAID IN THE TRACE, not only in the drawer. Filing reports itself on
        // the overlay through the kit's own note; this path builds the JSON
        // here, so nothing would appear where the reader is looking. A trace
        // line is the surface they already have open.
        const said = (line) => { try { P.log('probe:send', line); } catch (e) {} };
        try {
          const f = new File([text], name, { type: 'application/json' });
          if (navigator.canShare && navigator.canShare({ files: [f] })) {
            await navigator.share({ files: [f], title: name });
            this.outMsg = 'Capture shared';
            said('handed to the share sheet');
            return;
          }
        } catch (e) {
          // A reader who backs out of the share sheet has not failed at anything.
          if (e && e.name === 'AbortError') { this.outMsg = 'Share cancelled'; said('share cancelled'); return; }
        }
        try {
          await navigator.clipboard.writeText(text);
          const kb = Math.round(text.length / 1024);
          this.outMsg = 'Capture copied to the clipboard (' + kb + 'KB)';
          said('copied to the clipboard, ' + kb + 'KB');
        } catch (e) {
          this.outMsg = 'Could not send the capture: no token, no share sheet, no clipboard';
          said('nowhere to send it: no token, no share sheet, no clipboard');
        }
      },

      async openProbe() {
        const w = this._probeWin();
        if (!w) { this.outMsg = 'The probe cannot reach this view'; return; }
        // Already on: the same row takes it off, whole. stop() is the kit's,
        // so the listeners, the overlay, the stylesheet, the window and the
        // card hook all go back where they were.
        let P = null;
        try { P = w.Probe; } catch (e) {}
        if (P && P.on) {
          try { P.stop(); } catch (e) {}
          this.probeArmed = false;
          this.probeHeld = false;
          this.open = false;
          this.outMsg = 'Probe off';
          return;
        }
        // Loaded already but idle (a page that pulled the kit with no ?probe=):
        // arm the object, since gh.load caches by path and a second load would
        // return the first one's promise without re-executing the file.
        if (P && typeof P.arm === 'function') {
          try { P.arm(); } catch (e) {}
        } else {
          try { w.__probeArm = true; } catch (e) {}
          if (w === window) {
            await this._selfLoad('kits/probe.js', () => !!(window.Probe && window.Probe.on));
          } else if (w.gh && typeof w.gh.load === 'function') {
            try { await w.gh.load('kits/probe.js', { by: 'alpineComponents/fab.js' }); } catch (e) {}
          }
        }
        let ok = false;
        try { ok = !!(w.Probe && w.Probe.on); } catch (e) {}
        this.probeArmed = ok;
        this.probeHeld = false;
        this.probeCanFile = this._probeCanFile();
        // WHERE THE ANSWER IS READ. `outMsg` is a line inside the drawer's take
        // area, so a row tapped from the launcher menu writes its result into a
        // panel that is shut. On success that costs nothing, since the overlay
        // appearing is the answer and is unmistakable. On failure it costs
        // everything: the reader taps Probe, nothing happens, and the sentence
        // saying why is behind a drawer they have no reason to open. So a
        // failure opens it.
        this.open = !ok;
        this.outMsg = ok
          ? 'Probe on: the corner reads this page\u2019s conditions. alt+shift+W opens the window, alt+shift+P files the capture.'
          : 'The probe cannot load into this view. Add ?probe= to its own address instead.';
      },

      // `docOverride` is for the selection offer, which knows which document
      // the passage is in (the shell's or the subject frame's) because that
      // is the document whose selectionchange it heard; `_annDoc()` below is
      // the answer for a caller with no better one. (It read "the probe below"
      // until 2026-09-10, when a literal Probe arrived two methods up.)
      async openAnnotate(docOverride) {
        if (!window.Annotate) await this._loadAnnotate();
        const doc = docOverride || this._annDoc();
        let subject = null;
        if (doc !== document) {
      ...
      // ── The selection offer ───────────────────────────────────────────────
      // WHY A LISTENER ON THE HOST DOCUMENT, when this component has declined
      // one for years: the Text tab snapshots the selection at the launcher
      // tap rather than watching it, and the annotator's own chip lives in the
      // kit, so that a page with a fab on it paid nothing for a feature it
      // might never use. What that bought was a fab with no way from a
      // selection to a note (the launcher's press collapses the selection,
      // and the menu behind it offers every aim but the text). The listener is
      // one handler, debounced, that reads the selection and sets a string;
      // the kit loads only when the offer is taken. Asked for on 2026-09-08.
      //
      // WHAT IT DECLINES TO OFFER: a collapsed selection (a caret), one inside
      // this component or inside the annotator's own UI, one in a field the
      // reader is typing in, and any selection while the annotator is already
      // on, since the card and its chip are offering the same thing better.
      _watchSelection(doc) {
        if (!doc || typeof doc.addEventListener !== 'function') return;
        if (!this._selWatched) this._selWatched = new WeakSet();
        if (this._selWatched.has(doc)) return;
        this._selWatched.add(doc);
        // THE OFFER FOLLOWS THE PASSAGE. Scrolling moves the words and fires no
        // selectionchange, so without this the offer stays where the passage
        // used to be, pointing at whatever scrolled into its place. Capture,
        // since the scroller is often an element rather than the document;
        // passive, since this only reads. Both cost nothing while no offer is
        // up, which is nearly always.
        const follow = () => { if (this.selOffer) this._placeSelOffer(); };
        doc.addEventListener('scroll', follow, { passive: true, capture: true });
        (doc.defaultView || window).addEventListener('resize', follow, { passive: true });
        doc.addEventListener('selectionchange', () => {
          // selectionchange fires on every step of a drag; the offer is about
          // where it settles, so a short trailing debounce.
          if (this._selTimer) clearTimeout(this._selTimer);
          this._selTimer = setTimeout(() => { this._selTimer = null; this._readSelection(doc); }, this.SEL_SETTLE);
        });
      },
      SEL_SETTLE: 180,

      // WHERE THE OFFER GOES, read from the range rather than from the pointer:
      // a selection made by dragging, by double-tapping a word, or from the
      // keyboard all end somewhere different from where they are, and the
      // passage is the subject either way.
      //
      // OFF-SCREEN IS GONE, not clamped. A rect of zero size means the range
      // was detached by a re-render, and a rect scrolled clear of the viewport
      // means the reader has moved on; pinning the offer to an edge in either
      // case leaves a control pointing at nothing.
      SEL_GAP: 8,
      SEL_H: 34,
      _placeSelOffer() {
        const r = this._selRange, doc = this._selDoc;
        if (!r || !doc) return this.dropSelOffer();
        let box;
        try { box = r.getBoundingClientRect(); } catch { return this.dropSelOffer(); }
        if (!box || (!box.width && !box.height)) return this.dropSelOffer();
        const w = doc.defaultView || window;
        const vw = w.innerWidth || 800, vh = w.innerHeight || 600;
        if (box.bottom < 0 || box.top > vh) return this.dropSelOffer();
        const below = box.bottom + this.SEL_GAP;
        const top = (below + this.SEL_H <= vh) ? below
          : Math.max(this.SEL_GAP, box.top - this.SEL_GAP - this.SEL_H);
        // Aligned to the passage's left edge and kept inside the viewport; 176
        // is the offer's own width at its widest (mark, button, dismiss).
        const left = Math.max(this.SEL_GAP, Math.min(box.left, vw - 176));
        if (this.selOffer) this.selOffer = { ...this.selOffer, left: Math.round(left), top: Math.round(top) };
        return this.selOffer;
      },

      _readSelection(doc) {
        const clear = () => {
          if (this._selDoc === doc || !this.selOffer) { this.selOffer = null; this._selRange = null; this._selDoc = null; }
        };
        try {
          const A = this._annKit();
          if (A && A.enabled) return clear();
          ...
          if (!el || !doc.body || !doc.body.contains(el)) return clear();
          if (this.$el.contains(el) || el.closest('[data-annotate-ui]')) return clear();
          const ae = doc.activeElement;
          if (ae && (/^(INPUT|TEXTAREA)$/.test(ae.tagName) || ae.isContentEditable)) return clear();
          this._selRange = range.cloneRange();
          this._selDoc = doc;
          this.selOffer = { quote: text.length > 60 ? text.slice(0, 59).trimEnd() + '\u2026' : text,
                            left: -9999, top: -9999 };
          this._placeSelOffer();
        } catch (e) { clear(); }
      },
      ...
      // Take the offer: load the trio if it is not here, turn the annotator on
      // aimed at the document the passage is in, and open the composer on the
      // passage. The Range carried from the read is handed over, so the note
      // is about what was offered even where the tap, or the platform, has
      // collapsed the live selection by now; with the selection still live the
      // kit's fallback would read the same passage.
      async noteSelection() {
        const range = this._selRange, doc = this._selDoc;
        this.dropSelOffer();
        if (!range || !doc) return;
        try {
          if (!window.Annotate) await this._loadAnnotate();
        } catch (e) { this.outMsg = 'Annotator unavailable (kits/annotate.js failed to load)'; return; }
        let K = this._annKit();
        if (!(K && K.enabled)) { await this.openAnnotate(doc); K = this._annKit(); }
        if (!K || !K.noteSelection) return;
        if (!K.noteSelection({ range })) this.outMsg = 'Could not anchor that selection';
      },

      // The other half of the same row, and the reason it exists: the card has
      // one state now, so the only way to stop looking at it is to put it away.
      // disable() unmounts the card and KEEPS the set, which is what makes this
      // safe to tap: the notes are the kit's, not the card's, and the next aim
      // opens onto them again.
      annOff() {
        const A = this._annKit();
        if (A && A.enabled) A.disable();
        this.annOn = false;
        this.open = false;
      },
      ...
      // page loads its own kits and this component is the shell's. It asks for
      // kits/src-doc.js, not for any one kind's kit: src-doc answers for every
      // kind that declares, so this menu learns a new one without learning its
      // name.
      _srcDoc() {
      ...
      // annotator answers for itself; otherwise src-doc is asked directly,
      ...
      // No src-doc anywhere means nothing has declared, the same answer.
      //
      // src-doc's own test is the one that matters here: a kind offering an
