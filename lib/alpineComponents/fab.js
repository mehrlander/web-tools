  ...
  Alpine.data('fab', function() {
    return {
      ...
      _crumb(stage) {
        try {
          const t = Date.now();
          const trail = JSON.parse(localStorage.getItem('fab:step') || 'null') || { start: t, steps: [] };
      ...
      toggle() {
        ...
        const settles = (name, p) => { try { Promise.resolve(p)
          .then(() => this._crumb(name + ':ok'), () => this._crumb(name + ':err')); } catch { } };
        ...
        this._crumb('paint');
        this.$nextTick(() => {
          this._crumb('dom:' + (() => { try { return document.querySelectorAll('*').length; } catch { return '?'; } })());
          // Frames are counted rather than cleared at the second, because the
          // question changed: the build now finishes (d:end) and the tab dies
          // before frame1, so how many frames the browser managed is the
          // measurement, and the trail is dropped a full second later, once
          // the async three have had time to land in it.
          let f = 0;
          const tick = () => { this._crumb('frame' + (++f)); if (f < 3) requestAnimationFrame(tick); };
          requestAnimationFrame(tick);
          setTimeout(() => this._crumbDone(), 2500);
        });
      },
