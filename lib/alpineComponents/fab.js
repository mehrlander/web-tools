  ...
  Alpine.data('fab', function() {
    return {
        ...
        <div class="absolute inset-y-0 right-0 transition-transform duration-300 ease-out pointer-events-none"
          ...
          <div x-init="_crumb('d:body')"
            ...
            <div class="flex-1 min-h-0 flex flex-col overflow-hidden">
              ...
              <div x-init="_crumb('d:render-head')" x-show="activeTab === 'render'" class="shrink-0">
              ...
              <div x-init="_crumb('d:inspect')" x-show="activeTab === 'inspect'" class="min-h-0 flex-1 flex flex-col">
              ...
              <div x-init="_crumb('d:traffic')" x-show="activeTab === 'traffic'" class="min-h-0 flex-1 overflow-y-auto overscroll-contain">
              ...
              <div x-init="_crumb('d:render-body')" x-show="activeTab === 'render'" class="min-h-0 flex-1 flex flex-col">
                ...
                <template x-if="path">
                  <div class="p-2 flex flex-col gap-2 min-h-0 flex-1">
                    ...
                    <div x-init="_crumb('d:rb-guide')" class="min-h-0 flex-1 flex flex-col">
                    ...
                    <div x-init="_crumb('d:rb-take')" class="contents"></div>
            ...
            <div class="shrink-0 border-t border-base-300 flex flex-col">
              ...
              <div x-init="_crumb('d:rb-console')" x-show="consoleOpen" class="border-t border-base-300/60 flex flex-col" style="max-height: 40vh;">
            ...
            <div x-init="_crumb('d:end')" class="hidden"></div>
            </div>
          </template>
      ...
      init() {
        ...
        // After the report, so a probe that kills the page cannot stop the
        // PREVIOUS run's trail from being filed.
        setTimeout(() => { this._probe(); }, 1600);
      ...
      // 800ms by default (see the threshold note on the timer below), and
      // ADDRESSABLE, because it is the one line by which this branch differs
      // from main in the code that runs on a bare page, and the device says
      // main is fine where the branch is not. A matrix cannot test that with
      // two different libs, since only one of them can file a report; ?lp=450
      // puts main's threshold inside the branch's lib so the variable moves
      // on its own.
      get LONG_PRESS() {
        try {
          const v = parseInt(new URLSearchParams(location.search).get('lp'), 10);
      ...
      // A PROBE THAT BUILDS THE CELL, so the renderer stops being a variable.
      // Every test so far has been a toss, which means toss-render.html was in
      // every cell and could not be ruled in or out. ?probe= puts the pieces on
      // an ORDINARY deployed page instead, one at a time:
      //
      //   frame        add a full-viewport iframe behind the page (about:blank)
      //   frame:lib    the same iframe, loading a page that boots the library
      //   nodes:<n>    append n plain divs, no fab involved
      //
      // Off unless the address asks, so nothing changes for any reader. The
      // point is that pages/peek.html?use=<ref> is a CONFIRMED SURVIVOR: adding
      // exactly one piece to it and watching what dies is the difference
      // between a correlation and a cause.
      async _probe() {
        let spec = '';
        try { spec = new URLSearchParams(location.search).get('probe') || ''; } catch { }
        if (!spec) return;
        for (const part of spec.split(',')) {
          const [what, arg] = part.split(':');
          try {
            if (what === 'frame') {
              const f = document.createElement('iframe');
              f.style.cssText = 'position:fixed;inset:0;width:100%;height:100%;border:0;z-index:-1';
              f.src = arg === 'lib'
                ? 'https://mehrlander.github.io/web-tools/pages/peek.html' + location.search.replace(/[?&]probe=[^&]*/, '')
                : 'about:blank';
              document.body.appendChild(f);
              this._crumb('probe:frame' + (arg ? ':' + arg : ''));
            } else if (what === 'nodes') {
              const n = Math.min(20000, parseInt(arg, 10) || 0);
              const frag = document.createDocumentFragment();
              for (let i = 0; i < n; i++) {
                const d = document.createElement('div');
                d.textContent = 'probe node ' + i;
                frag.appendChild(d);
              }
              ...
              this._crumb('probe:nodes:' + n);
            }
          } catch (e) { this._crumb('probe:err'); }
        }
      },
      ...
      toggle() {
        ...
        const settles = (name, p) => { try { Promise.resolve(p)
          .then(() => this._crumb(name + ':ok', true), () => this._crumb(name + ':err', true)); } catch { } };
