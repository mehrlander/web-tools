document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
                ...
                <template x-if="path">
                  <div class="p-2 flex flex-col gap-2">
                    ...
                    <div class="border-t border-base-300/60 pt-2">
                      <div class="text-[10px] uppercase tracking-wider opacity-50 font-semibold mb-1">Bundle</div>
                      <label class="flex items-center gap-1.5 text-[11px] mb-1 cursor-pointer">
                        <input type="checkbox" x-model="bundleOffline" class="checkbox checkbox-xs" />
                        <span>Fully offline (inline code)</span>
                      </label>
                      <button @click="downloadBundle()" :disabled="bundleBusy" class="btn btn-sm gap-1 w-full">
                        <span x-show="!bundleBusy" class="flex items-center gap-1">
                          <i class="ph ph-file-archive"></i><span x-text="bundleOffline ? 'Download offline page' : 'Download page + data'"></span>
                          <span x-show="reads.length" class="opacity-60" x-text="'(' + reads.length + ')'"></span>
                        </span>
                        <span x-show="bundleBusy" class="loading loading-spinner loading-xs"></span>
                      </button>
                      <div x-show="bundleMsg" class="text-[10px] text-success font-mono break-all mt-1" x-text="bundleMsg"></div>
                      <div x-show="bundleError" class="text-[10px] text-error font-mono break-all mt-1" x-text="bundleError"></div>
                      <div x-show="!bundleOffline" class="text-[10px] text-base-content/40 leading-snug mt-1">Page source + the data it <code>read()</code>s, zipped so unzip-and-open resolves local. Code still loads from the CDN — local data, not offline.</div>
                      <div x-show="bundleOffline" class="text-[10px] text-base-content/40 leading-snug mt-1">Page + data + its whole <code>gh.load</code> chain inlined, so unzip-and-open works with no network. Third-party libs (Tailwind/Alpine) still come from the CDN.</div>
                    </div>
      ...
      reads: [],
      bundleBusy: false, bundleMsg: '', bundleError: '', bundleOffline: false,
      ...
      // Download this page + the data it read()s as one zip, via the bundle kit
      ...
      async downloadBundle() {
        this.bundleError = ''; this.bundleMsg = ''; this.bundleBusy = true;
        try {
          if (!window.bundle) await this._selfLoad('kits/bundle.js', () => !!window.bundle);
          if (!window.bundle) throw new Error('bundle kit unavailable (kits/bundle.js failed to load)');
          const r = await window.bundle.page({ path: this.path, offline: this.bundleOffline });
          this.bundleMsg = 'Saved ' + r.filename +
            (r.offline ? ' (+' + r.codeFiles + ' code' + (r.reads.length ? ', +' + r.reads.length + ' data)' : ')')
                       : (r.reads.length ? ' (+' + r.reads.length + ' data)' : ' (no data read yet)'));
        } catch (e) {
          this.bundleError = (e && e.message) || String(e);
        } finally {
