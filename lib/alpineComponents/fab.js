...
const registerFab = function() {
  Alpine.data('fab', function() {
    return {
      ...
      template: `
        <div :style="'transform:translate(' + x + 'px,' + y + 'px)'"
          ...
          <!-- The launcher's second gesture: a long press (or a right-click)
               ...
               drawer under it. -->
          <!-- THE SELECTION OFFER. Select text anywhere on the page and this
               appears above the launcher: the passage's first words, "+ note",
               and a way to forget it. One tap turns the annotator on aimed at
               that passage and opens the composer on it, so noting a selection
               no longer costs turning the annotator on first and selecting
               again. Until 2026-09-08 there was no way to it at all from here:
               the launcher menu offers the page, an element, a section and a
               region, and the one aim a reader makes by hand, the text, was
               reachable only once the card was already up, because the press
               that opens the menu is what collapses the selection.

               ABOVE THE LAUNCHER, NOT BESIDE THE TEXT. The annotator's own
               chip floats by the passage, which on a phone is exactly where
               the platform puts its own callout, so it was routinely under it
               or gone with it (the card grew a selection bar for that reason).
               The launcher is a fixed place clear of the callout, and this
               offer belongs to the launcher: it is the fab's, made before the
               kit is loaded.

               pointerdown is REFUSED, the same way the annotator's offers
               refuse it: the tap's default is what collapses the selection,
               and a control that clears the passage it is offering to note
               reads as having missed. The range is also carried across the
               load on its own, so a selection the platform drops anyway still
               arrives. -->
          <div x-show="selOffer && !fabMenu" x-cloak x-transition.opacity.duration.120ms
               @pointerdown.prevent.stop @pointerup.stop @pointermove.stop
               @contextmenu.stop
               data-fab-sel-offer
               class="absolute bottom-full right-0 mb-2 flex items-center gap-1.5 pl-3 pr-1 py-1 rounded-box border border-warning/40 bg-base-100 shadow-2xl"
               style="max-width: min(20rem, calc(100vw - 3rem));">
            <i class="ph ph-highlighter text-[17px] text-warning shrink-0"></i>
            <span class="text-xs italic text-base-content/70 truncate min-w-0" x-text="selOffer &amp;&amp; selOffer.quote"></span>
            <button @click="noteSelection()" class="btn btn-warning btn-xs shrink-0 whitespace-nowrap"
                    title="Note the text you selected">+ note</button>
      ...
      fabMenu: false,
      pageMenu: [],
      // The selection the page has right now, offered above the launcher as
      // "+ note" (see the template). Reactive copy of the words only; the
      // Range and its document ride on the component off the reactive path
