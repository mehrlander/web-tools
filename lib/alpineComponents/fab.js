document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
      description: 'Draggable floating button: opens a right-side drawer with tabs for Alpine components on the page (tap to outline in place), scripts pulled in via gh.load() (with per-entry status), and a Render tab that surveys which branches carry a different version of this page (blob-compare against the default branch) and re-renders it at any ref, plus a collapsible console (rich debugConsole panel with expandable JSON-tree / table views, falling back to a plain log list). The header carries a version readout: the booted ref, tip sha, and the latest PR merge included, plus any commits sitting on top of it (direct pushes or an unmerged branch). Singleton per viewport: a hosting shell (toss-render, or the fab’s own overlay) stamps __fabHosted and a fab booting under it declines to mount; toss-render hands the rendered subject down via __tossSubject and the shell fab adopts it',
