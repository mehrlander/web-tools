    ...
    return {
      ...
      template: `
        <div :style="'transform:translate(' + x + 'px,' + y + 'px)'"
             ...
             @contextmenu.prevent="openFabMenu()"
          ...
          <!-- The launcher's second gesture: a long press (or a right-click)
               ...
               THE SECOND BUILT-IN ROW IS THE WAY OUT: "Web Tools home", which
               leaves for the deployed app at the default branch. The launcher
               is the one control floating over every view, including a toss of
               a branch page and a ?use= pin at some other ref, and from inside
               one of those there is otherwise no way back to the app itself
               that does not go through the address bar. It is a fixed address
               rather than a ref switch, which is what separates it from the
               Render tab's return-to-live button: that one re-addresses THIS
               view at the default branch, and this one leaves the view behind.
               ...
               UNDER IT, WHAT THE PAGE CONTRIBUTES: menu, the third opt-in
               ...
               every view. show-repo contributes the app-wide paste there,
               which on a phone is the only intake gesture the platform gives
               at all. Rows are read at open time (readPageMenu), not at scan
               time, because the drawer's own scan is detect() and a menu that
               had to wait for it would open empty.
          ...
          <div x-show="fabMenu" x-cloak x-transition.opacity.duration.120ms
            ...
            <button @click="fabMenu = false; annNotePage()"
                    class="w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-base-200">
              <i class="ph ph-note-pencil text-[15px] text-primary"></i>
              <span class="text-sm font-semibold">Take a note</span>
            </button>
            <button @click="fabMenu = false; goHome()"
              ...
              <i class="ph ph-house text-[15px] text-primary"></i>
              <span class="text-sm font-semibold">Web Tools home</span>
            </button>
            <template x-for="(m, i) in pageMenu" :key="m.label + ':' + i">
              <button @click="fabMenu = false; runMenuRow(m)"
                      class="w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-base-200 border-t border-base-200"
                      :title="m.title || ''">
                <i class="ph text-[15px] text-primary" :class="m.icon || 'ph-dot-outline'"></i>
        ...
        <div class="absolute inset-y-0 right-0 transition-transform duration-300 ease-out pointer-events-none"
          ...
          <div class="h-full bg-base-100 border-l border-base-300 shadow-2xl flex flex-col pointer-events-auto overscroll-contain">
            ...
            <!-- Text and Notes are the two tabs the readout sits out, and it is
                 the same reason twice: the first three tabs describe the page
                 LOAD, so the strip is context for all of them, while these two
                 are about the document's own content, where a byte count is
                 noise. Both also need the height for their own head. -->
            <button x-show="trafficLine && activeTab !== 'notes' && activeTab !== 'text'" @click="activeTab = 'traffic'; refreshTraffic()"
            ...
            <div class="flex-1 min-h-0 flex flex-col overflow-hidden">
              ...
              <!-- TEXT. What the page SAYS, as against the four tabs about how
                   ...
                   Rows, not stat cards, per docs/HTML-STYLE.md, and every
                ...
                <template x-if="textStats">
                  <div>
                    ...
                    <div class="px-2.5 pb-2 space-y-1">
                      ...
                      <div class="flex items-baseline justify-between gap-2 text-sm">
                        <span class="text-base-content/70" title="Path-shaped tokens outside any link">Bare paths</span>
                        <span class="font-mono tabular-nums"
              ...
              <!-- NOTES. The annotator's review surface, in the drawer rather
                   than in a modal of its own. The kit (kits/annotate.js) owns
                   the data, the anchoring and the on-page card; this owns the
                   reading and editing of the set, which is the part that wanted
                   room. Reads through window.Annotate and re-reads on the kit's
                   'annotate:change' event, so nothing here polls. -->
              <div x-show="activeTab === 'notes'" class="min-h-0 flex-1 flex flex-col">
                <!-- Its own head: what is being annotated, and the on/off state
                     of the annotator itself. -->
                <div class="shrink-0 flex items-center gap-2 px-2.5 py-1.5 border-b border-base-300 bg-base-200/40">
                  <i class="ph ph-note-pencil text-[13px] shrink-0 text-base-content/40"></i>
                  <span class="text-[12px] truncate min-w-0 flex-1"
                        :class="annOn ? 'text-base-content/70' : 'text-base-content/40'"
                        x-text="annSubject"></span>
                  <!-- The count lived on the tab label until it cost the tab
                       strip its last 20px of width. Here it has room, and it
                       sits beside the thing it counts. -->
                  <span x-show="annItems.length" class="text-[12px] text-base-content/50 shrink-0"
                        x-text="annItems.length + (annItems.length === 1 ? ' note' : ' notes')"></span>
                </div>

                <!-- ONE SWITCH, in one place. Start sat in the body and Off in
                     the head, so the two halves of a single state lived at
                     opposite ends of the pane and the reader had to find the
                     other one to undo the first. Same button, same spot, colour
                     and label carrying which way it is going.
                     The paragraph that explained what an annotation is went with
                     the split (2026-08-12): four lines of instruction above a
                     button, read once and scrolled past forever after, on a
                     surface whose own chips already name the three targets. -->
                <div class="shrink-0 p-3 flex flex-col gap-2 items-start">
                  <div x-show="annBlind && !annOn" class="text-[12px] text-warning flex items-start gap-1.5">
                    <i class="ph ph-warning-octagon shrink-0 mt-0.5"></i>
                    <span>Subject frame unreadable: notes would attach to this renderer shell, not the page inside it.</span>
                  </div>
                  <button @click="annOn ? annStop() : annStart()"
                          class="btn btn-sm" :class="annOn ? 'btn-error' : 'btn-primary'"
                          :title="annOn ? 'Turn the annotator off and drop the on-page card (the notes keep)'
                                        : 'Turn the annotator on and put the card on the page'">
                    <i class="ph" :class="annOn ? 'ph-stop-circle' : 'ph-note-pencil'"></i>
                    <span x-text="annOn ? 'Stop annotating' : 'Start annotating'"></span>
                  </button>
                </div>

                <template x-if="annOn">
                  <div class="min-h-0 flex-1 flex flex-col">
                    <!-- Three readings of one set: the notes as a working list,
                         and the two serializations exactly as they will land on
                         the clipboard. -->
                    <div class="shrink-0 flex gap-0 px-2.5 pt-2">
                      <!-- Positional CSS cannot do this segmenting: x-for
                           inserts its clones AFTER the <template> element, so
                           the template is the flex container's first child and
                           NO button ever matches :first-child. The left border
                           was stripped from every segment including the first,
                           which is why the active tab looked open on its left
                           edge. Keying off the index says what is meant. -->
                      <template x-for="(t, i) in annTabs" :key="t.key">
                        <button @click="annSetTab(t.key)"
                                class="flex-1 px-2 py-1 text-[12px] font-semibold border border-base-300 transition-colors"
                                :class="[annTab === t.key ? 'bg-primary/10 text-primary border-primary/40' : 'text-base-content/60 hover:bg-base-200',
                                         i === 0 ? 'rounded-l' : 'border-l-0',
                                         i === annTabs.length - 1 ? 'rounded-r' : '']"
                                x-text="t.label"></button>
                      </template>
                    </div>

                    <div x-show="!annItems.length" class="text-sm text-base-content/50 italic px-3 py-6 text-center">
                      No notes yet. Select text on the page, or use Page / Element / Region on the card.
                    </div>

                    <!-- Notes: the working list. Each card carries the quote it
                         is pinned to, the DOM address under it, and an editor
                         with room to write in. -->
                    <div x-show="annTab === 'notes' && annItems.length"
                         class="min-h-0 flex-1 overflow-y-auto overscroll-contain p-2 space-y-1.5">
                      <template x-for="(it, i) in annItems" :key="it.id">
                        <div class="rounded border overflow-hidden transition-colors"
                             :class="it.id === annSel ? 'border-warning bg-warning/10' : 'border-base-300/60 bg-base-200/40'">
                          <div class="flex items-start gap-1.5 px-2 pt-1.5">
                            <button @click="annSelect(it.id)"
                                    class="flex-1 min-w-0 text-left text-[12px] italic text-base-content/70 hover:text-base-content"
                                    :title="'Select this note and scroll to it'"
                                    x-text="(i + 1) + '. ' + annHead(it)"></button>
                            <!-- The note's WORDS alone, which is the errand the
                                 two serialization buttons below cannot do: they
                                 carry the whole set with each note's quote and
                                 address, and this row is one note being taken
                                 somewhere else. The card's rows carry the same
                                 verb; the kit owns it, so both read alike. -->
                            <button @click="annCopyOne(it.id)" class="btn btn-ghost btn-xs btn-square shrink-0"
                                    title="Copy this note's text" aria-label="Copy">
                              <i class="ph ph-copy text-base-content/40"></i>
                            </button>
                            <button @click="annRemove(it.id)" class="btn btn-ghost btn-xs btn-square shrink-0"
                                    title="Remove this note" aria-label="Remove">
                              <i class="ph ph-x text-base-content/40"></i>
                            </button>
                          </div>
                          <div x-show="annAddr(it)" class="px-2 font-mono text-[11px] text-base-content/40 break-all"
                               x-text="annAddr(it)"></div>
                          <!-- text-[16px]: anything smaller zooms iOS Safari on
                               focus, and this field exists to be typed into. -->
                          <textarea x-init="$el.value = it.note || ''"
                                    @input="annEdit(it.id, $el.value)"
                                    @focus="annSelect(it.id, false)"
                                    rows="2" placeholder="Note…"
                                    class="textarea textarea-bordered w-full text-[16px] leading-snug min-h-16 my-1.5 mx-0 rounded-none border-x-0 border-b-0 bg-base-100/60 focus:outline-none"></textarea>
                        </div>
                      </template>
                    </div>

                    <!-- Markdown / JSON: what Copy puts on the clipboard, shown
                         rather than described. -->
                    <div x-show="annTab !== 'notes' && annItems.length"
                         class="min-h-0 flex-1 overflow-auto overscroll-contain p-2">
                      <pre class="font-mono text-[12px] leading-relaxed whitespace-pre-wrap break-words select-text text-base-content/80" x-text="annOut"></pre>
                    </div>

                    <!-- Every action names its verb. A button labelled by its
                         format that copies without saying so is the thing this
                         tab replaced. -->
                    <!-- pr-14 reserves the bottom-right corner: the launcher
                         disc floats there by default and would sit half over
                         whatever button landed last, which on this row is the
                         destructive one. -->
                    <div x-show="annItems.length" class="shrink-0 border-t border-base-300 p-2 pr-14 flex flex-wrap items-center gap-1.5">
                      <!-- Clear sits with the others rather than pushed right:
                           the launcher disc floats over the drawer's
                           bottom-right corner, so anything at that edge is
                           half under it, and the destructive one is the worst
                           candidate for a tap you cannot quite see. -->
                      <!-- The other reading of the set, and the only action
                           here that operates on the PAGE: every note drawn
                           where it is pinned, with its words beside it, which
                           is the picture a screenshot wants and the one this
                           list cannot be. The card carries the same toggle. -->
                      <button @click="annPlace()" class="btn btn-xs"
                              :class="annInPlace ? 'btn-warning' : ''"
                              :title="annInPlace ? 'Take the notes back off the page'
                                                 : 'Draw every note on the page, beside what it is pinned to'">
                        <i class="ph" :class="annInPlace ? 'ph-eye-slash' : 'ph-eye'"></i>
                        <span x-text="annInPlace ? 'Off page' : 'On page'"></span>
                      </button>
                      <button @click="annCopy('md')" class="btn btn-xs btn-primary">Copy markdown</button>
                      <button @click="annCopy('json')" class="btn btn-xs">Copy JSON</button>
                      <button @click="annJot()" class="btn btn-xs">Save jot</button>
                      <button @click="annClear()" class="btn btn-xs btn-ghost text-error ml-1">Clear</button>
                    </div>
                    <div x-show="annMsg" class="shrink-0 px-2 pb-1.5 text-[12px] font-mono break-all"
                         :class="annErr ? 'text-error' : 'text-success'" x-text="annMsg"></div>
                  </div>
                </template>
              </div>
                ...
                <template x-if="path">
                  <div class="p-2 flex flex-col gap-2 min-h-0 flex-1">
                    ...
                    <div class="min-h-0 flex-1 flex flex-col">
                      ...
                      <div x-show="!pageBranchesLoading" class="min-h-0 flex-1 overflow-y-auto overscroll-contain">
      ...
      // loaded, what that cost), and the last two operate on what it SAYS.
      // Text and Notes are the read and write halves of the same subject, so
      // they sit adjacent and share the document text index.
      TABS: [
        { key: 'render',  label: 'Render',  icon: 'ph-monitor-play',     on: 'loadPageBranches' },
        ...
        { key: 'notes',   label: 'Notes',   icon: 'ph-note-pencil',      on: 'annOpen' },
      ],
      ...
      // Notes tab. A VIEW of kits/annotate.js, never a second store: annItems
      // holds the kit's own item objects (same references, so an x-for keyed by
      // id patches rather than rebuilds, which is what keeps a cursor in an
      // open textarea). annOut is the serialization for whichever format tab is
      // showing, recomputed on change rather than bound, since building both on
      // every keystroke is work nobody is looking at.
      annTab: 'notes',
      annTabs: [{ key: 'notes', label: 'Notes' }, { key: 'md', label: 'Markdown' },
                { key: 'json', label: 'JSON' }],
      annItems: [], annSel: null, annOn: false, annOut: '', annInPlace: false,
      annMsg: '', annErr: false,
      groups: [],
      ...
      init() {
        ...
        // The annotator announces changes on the window rather than exposing a
        // subscribe hook, so this view stays current without polling and the
        // kit stays ignorant of the drawer. 'annotate:review' is the kit's
        // Review button asking for this tab; preventDefault is how the drawer
        // answers "taken", which lets the kit say so plainly when no drawer is
        // listening instead of appearing to do nothing.
        this._annListener = () => this.annSync();
        window.addEventListener('annotate:change', this._annListener);
        this._annReviewListener = (e) => {
          if (e.preventDefault) e.preventDefault();
          // A TOGGLE, not a one-way open. Asking for the Notes tab while the
          // Notes tab is the thing already in front of you can only mean "put
          // it away", and the old one-way reading is why the button appeared
          // to do nothing on exactly the taps a reader made deliberately.
          if (this.open && this.activeTab === 'notes') { this.close(); return; }
          this.open = true;
          this.activeTab = 'notes';
          this.annOpen();
        };
        window.addEventListener('annotate:review', this._annReviewListener);
        // The other direction: the kit's review button says what a tap will do
        // only if it knows whether the drawer is open and on which tab. Watched
        // rather than announced from each mutation site, so the broadcast
        // cannot drift from the state it reports.
        this._annDrawerQuery = () => this._annBroadcastDrawer();
        window.addEventListener('annotate:drawer-query', this._annDrawerQuery);
        this.$watch('open', () => this._annBroadcastDrawer());
        this.$watch('activeTab', () => this._annBroadcastDrawer());
        this._annBroadcastDrawer();
        this.annSync();
      ...
      destroy() {
        ...
        if (this._annListener) window.removeEventListener('annotate:change', this._annListener);
        if (this._annReviewListener) window.removeEventListener('annotate:review', this._annReviewListener);
        if (this._annDrawerQuery) window.removeEventListener('annotate:drawer-query', this._annDrawerQuery);
      },
      ...
      _afterIdentityChange(was) {
        ...
        if (this.open && !this.subjectLocal) {
          ...
          if (this.activeTab === 'render' && (refChanged || pathChanged)) this.loadPageBranches();
        }
      },
      ...
      // The menu's second built-in row. One fixed address, the deployed app at
      ...
      toggle() {
        ...
        this.ensureBrief();
        this.loadVersion();
        // Render is the default tab, so populate its branch scan on open the
        ...
        return [
          ...
          { kind: 'Open', items: [
            ...
            { key: 'annotate', icon: 'ph-note-pencil', label: 'Annotate',
              desc: 'Pin notes to this page: select text, pick elements, or note the page as a whole, then copy the set out or save it as a jot.'
      ...
      // ── Notes tab ─────────────────────────────────────────────────────────
      // The take grid's Annotate entry closes the drawer and hands you the page
      // to select on; this tab is the other half, where the set is read and
      // edited. Both drive the same kit.

      // WHICH kit, though: in a toss the annotated page runs in the subject
      // frame and loads its own copy, while this drawer is the shell's. So the
      // notes being taken can live in a window this component does not own, and
      // reading window.Annotate alone finds nothing (or, worse, a second empty
      ...
      // throw on access, which is the blind case annBlind already declares.
      // The annotator's two entry points both go through here, because the kit
      // arrives as a PAIR: kits/annotate.js owns the notes, kits/dictate.js is
      // the voice buffer its composer reads at the point of use. Loading only
      // the first leaves a composer with no microphone, silently, since the
      // annotator treats a missing Dictate as a browser without speech.
      // Dictate first, so the annotator never mounts its UI in the window
      // between the two loads and decides there is nothing to dictate with.
      async _loadAnnotate() {
        if (!window.Dictate) {
      ...
      // claim that they are.
      get annSubject() {
        if (!this.annOn) return 'Not annotating';
        const k = this._annKit();
        const s = k && k.subject;
        if (s && s.title) return s.title;
        return this.path || document.title || 'this page';
      },
      // A #gz= sandbox is opaque, so notes taken here would pin to the renderer
      // shell rather than the page inside it. Worth saying before the fact.
      //
      ...
      // subject arrived inside a frame.
      get annBlind() { return !!(this.subjectFramed && !this.subjectReached); },

      annHead(it) {
        const t = it.target || {};
        if (t.type === 'text') return '“' + this._annClip(t.quote && t.quote.exact) + '”';
        if (t.type === 'element') return '⌖ ' + this._annClip(t.excerpt || t.selector);
        if (t.type === 'page') return '▤ this page';
        return '▭ ' + this._annClip(t.excerpt);
      },
      annAddr(it) {
        const A = this._annKit();
        return (A && A._addressText) ? A._addressText(it.target || {}) : '';
      },
      _annClip(s) {
      ...
      // Opening the tab loads the kit if it is not here yet, so the tab is
      // never a dead end. It does NOT enable the annotator: turning it on puts
      // a card on the page and arms a selection listener, which is a decision
      // rather than a side effect of looking.
      async annOpen() {
        if (!this._annKit()) {
          try { await this._loadAnnotate(); } catch (e) {}
        }
        this.annSync();
      },
      // A page that turned the annotator on itself (pages/annotate.html does,
      // on load) already has one, in the frame. Enabling a second from the
      // shell would give the same document two sets of highlights and two
      // cards, so adopt rather than start.
      async annStart() {
        const A = this._annKit();
      ...
      // The launcher menu's one row. Turns the annotator on if it is off and
      // stages a page draft: composer open, microphone OFF. That last part is
      // the whole shape of it. What arrives is an offer, not a recorder that
      // started itself, and it costs nothing to ignore: select a passage or
      // tap a chip and the page draft is replaced by the one you aimed at.
      // The drawer stays out of the way (openAnnotate closes it) because the
      // card is where a note is written; the drawer is where the set is read,
      // one tap away on the card's own sidebar button.
      async annNotePage() {
        const A = this._annKit();
        if (!(A && A.enabled)) await this.openAnnotate();
        const K = this._annKit();
        if (K && K.notePage) K.notePage({ listen: false });
        this.open = false;
        this.annSync();
      },

      // Say where the drawer stands, on this window and into the toss frame:
      // under a toss the kit runs in the subject frame, and its review button
      // is the thing asking. Detail only, so a listener in either realm reads
      // it the same way.
      _annBroadcastDrawer() {
        const detail = { open: !!this.open, tab: this.activeTab };
        const fire = (w) => {
          try { w.dispatchEvent(new CustomEvent('annotate:drawer', { detail })); } catch (e) {}
        };
        fire(window);
        try {
          const f = window.__tossFrame && window.__tossFrame.contentWindow;
          if (f && f !== window) fire(f);
        } catch (e) {}
      },

      // Re-read the kit. Called on the kit's change event, so a note added by
      // selecting text on the page appears here without the tab being reopened.
      annSync() {
        const A = this._annKit();
        this.annOn = !!(A && A.enabled);
        this.annItems = A ? A.items : [];
        this.annSel = (A && A.selected) ? A.selected.id : null;
        this.annInPlace = !!(A && A.inPlace);
        if (this.annTab !== 'notes') this.annRender();
      },

      // Hand the set to the page and get out of its way: the drawer covers the
      // half of the screen the notes were just drawn on, so showing them and
      // staying open would hide what was asked for. Closing is the whole
      // gesture; the card's own toggle brings the list back.
      annPlace() {
        const A = this._annKit();
        if (!A || !A.showInPlace) return;
        const on = A.showInPlace(!A.inPlace);
        this.annInPlace = on;
        if (on) this.open = false;
      },
      annRender() {
        const A = this._annKit();
        if (!A) { this.annOut = ''; return; }
        this.annOut = this.annTab === 'json' ? JSON.stringify(A.toJSON(), null, 2) : A.toMarkdown();
      },
      annSetTab(key) { this.annTab = key; if (key !== 'notes') this.annRender(); },

      annEdit(id, value) { const A = this._annKit(); if (A) A.update(id, value); },
      annRemove(id) { const A = this._annKit(); if (A) A.remove(id); },
      // scroll defaults on for a tap and off for a focus: bringing the page to
      // the passage is the point of tapping a row, and a jolt while the
      // keyboard is opening is not.
      annSelect(id, scroll = true) {
        const A = this._annKit();
        if (A) A.select(id === this.annSel && scroll ? null : id, { scroll });
      },
      annClear() { const A = this._annKit(); if (A) A.clear(); this._annSay('Cleared'); },

      async annCopy(kind) {
        try {
          const t = await this._annKit().copy(kind);
          this._annSay(kind === 'json' ? 'Copied JSON'
            : 'Copied ' + Math.max(1, Math.round(t.length / 1024)) + 'K markdown');
        } catch (e) { this._annSay(e.message || String(e), true); }
      },
      // One note's words. The kit reports through its own status line, which
      // lives on the card and may not be on screen when the drawer is the
      // surface in use, so the drawer says it again where it was asked.
      async annCopyOne(id) {
        const A = this._annKit();
        if (!A) return;
        const ok = await A.copyNote(id);
        this._annSay(ok ? 'Note copied' : 'Nothing to copy', !ok);
      },
      async annJot() {
        try { this._annSay('Saving jot…'); await this._annKit().saveJot(); this._annSay('Jot saved'); }
        catch (e) { this._annSay('Jot: ' + (e.message || e), true); }
      },
      _annSay(msg, isErr) { this.annMsg = msg || ''; this.annErr = !!isErr; },
