    ...
    return {
        ...
        return [
          ...
          { kind: 'Image', items: [
            { key: 'shot-view', icon: 'ph-crop', label: 'View',
              desc: 'The visible part of this page as a PNG reconstructed from the DOM. '
                  + 'It may differ where CSS or cross-origin media cannot be reproduced.' },
            { key: 'shot-page', icon: 'ph-file-image', label: 'Page',
              desc: 'The full scrolling page as a PNG reconstructed from the DOM. '
                  + 'Very long pages may be downscaled to fit the browser canvas.' },
            { key: 'shot-region', icon: 'ph-selection-background', label: 'Region',
              desc: 'Pick an element, step to the ancestor you mean, then save the renderer’s PNG of it.'
                  + (this.annBlind ? ' Subject frame unreadable: the pick lands on this shell.' : '') },
          ] },
          { kind: 'Save', items: [
      ...
      async runTake(key) {
        ...
        try {
          ...
          else if (key === 'shot-view') await this.saveDomShot('viewport');
          else if (key === 'shot-page') await this.saveDomShot('page');
      ...
      _shotResult(r) {
        ...
        return 'Saved ' + r.filename + ' (' + size + ' DOM render'
          + (r.warnings.length ? '; ' + r.warnings.join('; ') : '') + ')';
      },

      async saveDomShot(mode, node) {
        await this._ensureDomShot();
        const doc = node?.ownerDocument || this._annDoc();
        const target = node || doc.documentElement;
        const r = await window.DomShot.save(target, { mode, downloadDocument: document });
      ...
      async openShotPicker() {
        ...
        await this.openPeek({
          ...
          takeLabel: 'Save PNG',
          onTake: async el => {
            const r = await this.saveDomShot('element', el);
            return 'Saved ' + r.width.toLocaleString() + '\u00d7' + r.height.toLocaleString();
          },
        });
        this.outMsg = 'Region PNG: tap an element, step to the ancestor you mean, then Save PNG';
      },
