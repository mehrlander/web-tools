  ...
  Alpine.data('fab', function() {
    return {
                                      ...
                                      <template x-for="o in p.origins" :key="o.source.source_id + ':' + o.record">
                                        <div class="space-y-1">
                                          <div class="flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5">
                                            ...
                                            <span class="font-mono text-base-content/30" x-text="o.source.ref"></span>
                                          </div>
                                          ...
                                          <template x-for="(ev, ei) in o.evidence" :key="ei">
                                            <div class="text-base-content/45">
                                              ...
                                              <span x-text="ev.quote || ev.note || ''"></span>
                                            </div>
                                          </template>
                                        </div>
                                      </template>
      ...
      _textRead(input) {
        ...
        let body = '', visible = '', chrome = 0, runs = 0, n;
        let prevBlock = null, prevVisibleBlock = null;
        while ((n = walker.nextNode())) {
          if (!n.data.trim()) continue;
          const text = n.data.replace(/\s+/g, ' ').trim();
          const block = this._block(n, root);
          if (visible) visible += (block === prevVisibleBlock ? ' ' : '\n');
          visible += text;
          prevVisibleBlock = block;
          let isChrome = false;
        ...
        return {
          ...
          body,
          // Proposal matching is about the literal text the reader can see,
          // including links and controls. Body deliberately excludes that
          // chrome for prose figures, so it cannot safely serve both jobs.
          visible,
