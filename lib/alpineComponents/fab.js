        ...
        <div :style="'transform:translate(' + x + 'px,' + y + 'px)'"
          ...
          <div x-show="fabMenu" x-cloak x-transition.opacity.duration.120ms
            ...
            <!-- THE CREDENTIAL SECTION, and only where the credential is
                 ...
                 TWO ROWS AND A TEST, in the order they are used. "Get token"
                 leaves for Shortcuts; "Paste token" reads what came back and
                 saves it. The pair is the whole loop, and neither half asks for
                 anything to be typed on a phone.

                 THE THIRD ROW IS THIS ROUND'S INSTRUMENT. It fires the same
                 kind of link at Log-Repo, which needs nothing installed and
                 commits what it is handed, so whether a home-screen web app can
                 reach Shortcuts at all is answered in the repo rather than by
                 asking what appeared on a screen. It comes out when that is
                 settled, unless it has earned its place by then. -->
            <template x-if="!hasToken">
              <div class="border-b-4 border-error/20">
                ...
                <a :href="tokenUrl" @click="fabMenu = false"
                ...
                <a :href="bridgeUrl" @click="fabMenu = false"
                   class="w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-base-200 border-t border-base-200"
                   title="File one line in the device log saying this browsing context reached Shortcuts. Nothing is installed and nothing appears on screen.">
