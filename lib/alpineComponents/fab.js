document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
      description: 'Draggable floating button: opens a right-side drawer with two tabs. Render (the default) surveys which branches carry a different version of this page (blob-compare against the default branch) and re-renders it at any ref, and carries the repo/version/links context block. Inspect merges the page scripts (loaded via gh.load(), with per-entry status) and Alpine components (tap to outline in place) into one scroll, skipping that context block. A header hard-refresh button reloads bypassing the browser cache (clears the Cache Storage API and re-fetches through a one-shot cache-bust token), for Safari on iOS where no keyboard gesture does. Plus a collapsible console (rich debugConsole panel with expandable JSON-tree / table views, falling back to a plain log list). The header carries a version readout: the booted ref, tip sha, and the latest PR merge included, plus any commits sitting on top of it (direct pushes or an unmerged branch). Singleton per viewport: a hosting shell (toss-render, or the fab’s own overlay) stamps __fabHosted and a fab booting under it declines to mount; toss-render hands the rendered subject down via __tossSubject and the shell fab adopts it',
      ...
      template: `
        ...
        <div class="absolute inset-y-0 right-0 transition-transform duration-300 ease-out pointer-events-none"
          ...
          <div class="h-full bg-base-100 border-l border-base-300 shadow-2xl flex flex-col pointer-events-auto">
            ...
            <div class="flex-1 overflow-y-auto">
              <div x-show="activeTab === 'render'" class="p-2 border-b border-base-300/60">
                <template x-if="repo">
                ...
                <div x-show="viaToss" class="px-1 mt-0.5 flex items-center gap-1 text-[10px] text-base-content/50">
                  <i class="ph ph-disc shrink-0"></i>
                  <span>tossed page @ <span class="font-mono text-base-content/70" x-text="ref"></span> — Components &amp; Scripts describe the toss-render shell</span>
                </div>
              ...
              <div x-show="activeTab === 'inspect'">
                <div x-show="viaToss" class="px-2.5 pt-2 flex items-center gap-1 text-[10px] text-base-content/50">
                  <i class="ph ph-disc shrink-0"></i>
                  <span>These describe the toss-render shell, not the tossed page.</span>
                </div>

                <div class="px-2.5 pt-2 pb-0.5 text-[10px] uppercase tracking-wider text-base-content/50 font-semibold">
                  Scripts <span x-show="loadedScripts.length" class="font-mono normal-case opacity-60" x-text="'(' + loadedScripts.length + ')'"></span>
                </div>
                <div x-show="loadedScripts.length === 0" class="text-sm text-base-content/50 italic px-3 py-3 text-center">
                  No scripts tracked. gh-boot.js installs the registry; older cached gh-api.js won't populate it.
                </div>
                <div x-show="loadedScripts.length > 0" class="p-2 pt-1 space-y-1">
                ...
                <div class="flex items-center justify-between px-2.5 pt-2 pb-0.5 border-t border-base-300/60">
                  <div class="text-[10px] uppercase tracking-wider text-base-content/50 font-semibold">
                    Components <span x-show="totalInstances" class="font-mono normal-case opacity-60" x-text="'(' + totalInstances + ')'"></span>
                  </div>
                  <button @click="clearHighlight()" x-show="highlighted" class="text-[10px] font-normal link link-hover">clear</button>
                </div>
                <div x-show="groups.length > 0" class="p-2 pt-1 space-y-2">
                ...
                <div x-show="groups.length === 0" class="text-sm text-base-content/50 italic px-3 py-3 text-center">
                ...
                <template x-if="path">
                  <div class="p-2 flex flex-col gap-2">
                    <div>
                      ...
                      <div x-show="viaToss" class="font-mono text-[10px] text-base-content/50 truncate" x-text="'@ ' + repo"></div>
                    </div>

                    <div>
                      <div class="flex items-center justify-between mb-1">
                        <div class="text-[10px] uppercase tracking-wider opacity-50 font-semibold">This page across branches</div>
                      ...
                      <div x-show="branchNote" class="text-[10px] text-base-content/50 leading-snug mb-1" x-text="branchNote"></div>
                      <div x-show="pageBranchesLoading" class="flex justify-center py-3">
                        <span class="loading loading-dots loading-md opacity-50"></span>
                      </div>
                      <div x-show="!pageBranchesLoading" class="flex flex-col gap-0.5 overflow-y-auto" style="max-height: 45vh;">
                    ...
                    <div>
                      <div class="text-[10px] uppercase tracking-wider opacity-50 font-semibold mb-0.5">Any ref</div>
                    ...
                    <button @click="renderAtRef()" class="btn btn-primary gap-1 w-full">
                      <i class="ph ph-play"></i><span x-text="viaToss ? 'Re-toss at ref' : 'Render'"></span>
                    </button>
                    <template x-if="tossUrl">
                      <a :href="tossUrl" target="_blank" class="btn btn-ghost gap-1 w-full">
                        <i class="ph ph-disc"></i>Open at ref in toss-render
                      </a>
                    </template>
                    <div x-show="frameError" class="text-[10px] text-error font-mono break-all" x-text="frameError"></div>

                    <div x-show="!viaToss" class="border-t border-base-300/60 pt-2">
                      ...
                      <button @click="exportPage()" :disabled="exportBusy" class="btn gap-1 w-full">
                    ...
                    <div class="text-[10px] text-base-content/40 leading-snug"
                         x-text="viaToss ? 'Re-renders the tossed page at the selected ref, in place.'
                                         : 'Re-renders this page at the selected ref in an overlay box.'"></div>
                  </div>
                </template>
              </div>
            </div>
        ...
        <div x-show="frameOpen" style="display:none" class="fixed inset-0 z-[60] flex flex-col bg-base-300/40 backdrop-blur-sm">
          <div class="shrink-0 bg-base-100 border-b border-base-300 px-3 py-1.5 flex items-center gap-2">
            ...
            <span class="font-mono text-sm font-semibold truncate" x-text="frameLabel"></span>
      ...
      // Same render, as a shareable tab instead of an overlay — toss-render's
      // address mode, which only accepts allowlisted owners (so no link for
      // other repos). Inside a toss the fab IS that tab, so no link there.
      get tossUrl() {
        if (this.viaToss) return '';
      ...
      // The Render button. In a toss, re-rendering means re-addressing the
      // shell at the picked ref (toss-render re-fetches, re-stamps the
      // subject, and this fab re-adopts); outside one, the overlay iframe.
      renderAtRef() {
        if (this.viaToss && this.repo && this.path && typeof window.__tossNavigate === 'function') {
          window.__tossNavigate(this.repo + '@' + (this.frameRef || 'main') + ':' + this.path);
          return;
        }
        this.openFrame();
      },
      ...
      frameHtml(text) {
        ...
        const prelude =
          '<base href="' + base + '">' +
          // __fabHosted: the overlaid page must not mount a second fab (the
          // hosting page's fab stays the one in charge of the viewport).
          '<script>window.__fabHosted=true;window.__ref=' + r + ';(function(){var R=' + r + ',g=URLSearchParams.prototype.get;' +
