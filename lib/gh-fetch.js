(() => {
  ...
  proto.graphql = async function(query, variables = {}, opts = {}) {
    try {
      ...
      for (let attempt = 1; ; attempt++) {
        try {
          const fn = typeof window !== 'undefined' && window.fetch ? window.fetch : fetch;
          res = await fn('https://api.github.com/graphql', {
            method: 'POST',
      ...
      if (json.errors && json.data != null) {
        ...
        try {
          if (typeof window !== 'undefined') {
            window.__lastGraphQLCall = {
              query,
              variables,
              operation: opOf(query),
  ...
  proto.branchesDatedSessions = async function(per = 100, max = 500, depth = 8) {
    ...
    const out = [];
    const sessions = {};
    let cursor = null, ordered = false, lastData = null;
    while (out.length < max) {
    ...
    const res = { branches, sessions, ordered, capped: out.length >= max };
    try {
      if (typeof window !== 'undefined') {
        const duration = Math.round(((typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now()) - t0);
        window.__graphQLByRepo = window.__graphQLByRepo || {};
        const lastErrors = (window.__lastGraphQLCall?.operation === 'BranchesDatedSessions') ? window.__lastGraphQLCall.errors : null;
        const rawPayload = lastErrors && lastData ? { ...lastData, _errors: lastErrors } : lastData;
        const snapshot = {
          repo: this.repo || (owner + '/' + name),
          at: new Date().toISOString(),
          duration,
          query: QUERIES.branchesDatedSessions,
          variables: { owner, name, per: Math.min(per, max), depth, order: ordered ? REF_ORDER : null },
          raw: rawPayload,
          errors: lastErrors || null,
          branches,
          sessions,
          ordered,
          capped: res.capped,
        };
        window.__graphQLByRepo[snapshot.repo] = snapshot;
        window.__lastActivityGraphQL = snapshot;
      }
    } catch {}
    return res;
  };
