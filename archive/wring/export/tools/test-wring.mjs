 ...
 * modules (snapshot under archive/wring/, originals in mehrlander/wring).
 *
 * Usage: node export/tools/test-wring.mjs        (from the wring repo)
 *        node tools/test-wring.mjs               (suggested home in web-tools)
 */

import { readFileSync } from 'node:fs';

const KIT_PATHS = ['../lib/kits/wring.js', '../../export/lib/kits/wring.js'];
let body = null;
for (const rel of KIT_PATHS) {
  try { body = readFileSync(new URL(rel, import.meta.url), 'utf8'); break; } catch { /* try next */ }
}
if (!body) { console.error('kit not found; run build-kit.mjs first'); process.exit(1); }
