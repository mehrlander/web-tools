...
Prose in a `.js` or `.html` file has no data file behind it and no registry row, so nothing
...
script counts it, in three classes that want three different answers:
    ...
    text-table   an object literal whose values are prose, keyed by something:
                 ...
                 This is CONTENT, not code. It wants a CSV or JSON file with
...
document, which content has no data file behind it, and how much text ships to the browser
on every load.
...
MIN_TABLE_ROWS = 3
TAG = re.compile(r"<[^>]+>")
# A generated payload's text DOES have a data file behind it: whatever its builder
# read. Counting it as content with none inverts the finding, so it is detected and
...
def main(argv):
    ...
    for rel in sorted(files):
        ...
        # Either banner says the same thing: the text here has a data file
    ...
    if "--tables" in opts:
        ...
        for rel, gen, ln, nrows, nwords, keys in sorted(all_tables, key=lambda r: -r[4]):
            print("%6dw %3d rows  %s:%d%s\n         keys: %s" % (
                nwords, nrows, rel, ln, "   [generated: the builder's input holds the text]" if gen else "", keys))
        return 0
    if "--dated" in opts:
        import datetime
    ...
    print("  text-table  %s words in %s tables, no data file behind them" % (
    ...
    print("  inline      %s words in %s runs, no data file behind them" % (
    ...
    if tables:
        print("\n  files holding text tables (content with no data file)")
    ...
    print("content whose only home is a source line.")
