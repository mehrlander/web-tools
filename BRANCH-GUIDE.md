...
- Backlog fully built, including veins (vein-to-skin) and watch. Remaining future candidates, unscheduled: recipe journal (ops log → replayable one-liner), `glom.semantics()` (JSON-LD/microdata grab), core.js kernel mod + `glom:set` event bus (dedupes upath/clean/SCOPE copies; deck stops monkey-patching), the playground page (real-layout verification + demo), `tap.replay`/`tap.walk` (mutate a captured request's params; paginate an API without scrolling), style-fingerprint grow, `glom.join` (relational joins over named sets).
