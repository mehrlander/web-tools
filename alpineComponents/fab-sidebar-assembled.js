document.addEventListener('alpine:init', function() {
  Alpine.data('fabSidebar', function() {
    return {
      ...
      detect() {
        ...
        document.querySelectorAll('[x-data]').forEach(el => {
          ...
          const id = '__fabsb_' + (this._instanceCounter++);
      ...
      highlight(id) {
        ...
        if (!el) return;
        el.classList.add('__fabsb-highlight');
        this.highlighted = id;
      ...
      clearHighlight() {
        ...
        if (el) el.classList.remove('__fabsb-highlight');
        this.highlighted = null;
      },

      _ensureHighlightStyle() {
        if (document.getElementById('__fabsb-highlight-style')) return;
        const style = document.createElement('style');
        style.id = '__fabsb-highlight-style';
        style.textContent =
          '.__fabsb-highlight {' +
          '  outline: 3px dashed var(--color-primary, #f59e0b) !important;' +
          '  outline-offset: 4px !important;' +
          '  animation: __fabsb-pulse 1.4s ease-in-out infinite;' +
          '  scroll-margin: 6rem;' +
          '}' +
          '@keyframes __fabsb-pulse {' +
